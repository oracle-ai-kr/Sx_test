...
async function _diagSystem(){
  ...
  // [S214] KRX는 2400+ 종목 일괄 조회라 일반 단일 API보다 느림 — 임계값 별도 적용
  // [S215] 임계값 현실화 — 캐시 미스 시 KRX 원본 호출은 5~8s가 정상 범위
  //   기존 3s/5s는 캐시 히트 시 기준이라 첫 호출(콜드캐시)에서 항상 빨강 처리됨
  //   조정: 4s 양호(캐시 히트) / 8s 보통(캐시 미스 정상) / 8s+ 느림(KRX 부하)
  const _krxColor = (ms) => ms < 4000 ? 'var(--buy)' : ms < 8000 ? 'var(--accent)' : 'var(--sell)';
  try{
    const t0 = performance.now();
    const res = await fetch(base + '/krx/market-cap?market=STK', {signal: AbortSignal.timeout(12000)});
    const ms = Math.round(performance.now() - t0);
    const json = await res.json().catch(()=>null);
    const cnt = (json && json.items||[]).length;
    const fb = json && json.fallback ? ' (폴백)' : '';
    if (cnt > 0) {
      // [S215] 5~8초 응답이면 "캐시 미스" 안내 (정상 범위지만 사용자 안내)
      const _hint = (ms >= 5000 && ms < 8000) ? ' (캐시 미스)' : '';
  ...
  try{
    ...
    const ms = Math.round(performance.now() - t0);
    const json = await res.json();
    // [S214] 응답 구조 안전 파싱 — 업비트는 보통 배열 그대로,
    //   워커가 {data:[...]}로 감싸거나 {error:'...'}로 응답하는 경우도 방어
    //   기존 (json.data||json||[]).length 는 json이 빈 객체 {}일 때 undefined 반환되는 버그 있었음
    let cnt = 0;
...
async function _diagData(){
  ...
  try{
    ...
    const json = await res.json();
    const candles = json.candles || json.data || [];
    // [S214] 누락 체크 — 네이버 워커 응답은 한글 필드(종가)를 사용
    //   기존 코드는 c.close만 봐서 정상 데이터도 모두 누락으로 잡혔음
