// ════════════════════════════════════════════════════════════
//  SIGNAL X — Diagnostic Module v1.4 <!-- S107 Phase3-B-9a 데이터 확장 진단 신규 (_diagDataExtension): 모바일 환경 console.log 접근 불가 문제 해결. [S107-9a]/[fetchExt] 태그 로그를 전역 버퍼(window._s107Logs 최대 100개)에 수집하고 진단 탭 6번 카테고리에 표시. 내용: ①현재 분석 종목 상태(확장 플래그, BT 거래 수, 시장 지원), ②localStorage 캐시 현황(sx_ext_* 키 개수/용량), ③fetchCandlesExtended 함수 로드 확인, ④최근 로그 20개(시간 역순, level별 색상). "로그 지우기" 버튼으로 수동 초기화. console.log/warn/error 후킹은 sx_diag.js 로드 시 자동 활성화 (원본 console 유지). --> v1.3 <!-- S103-fix7 Phase3-B-7.1 _diagProjectC 로직을 SXC.checkConsistency 위임으로 전환 (단일 소스 원칙). 내부 헬퍼 _isConsistentWithC/_diagCStatus/_checkLayerSeparation 삭제 가능하나 하위호환 위해 유지. --> v1.2b <!-- S103-fix7 Phase3-B-0b _buildVerdict 정합 체크 개선(Phase3-A-3 구조상 자동정합 인식 — _threeStageReport 저장 안 되어도 verdictAction 직접 수신 구조라 정합 보장) --> v1.2a <!-- S103-fix7 Phase3-B-0a 종목 탐색 fallback --> v1.2 <!-- S103-fix7 Phase3-B-0 --> v1.1 <!-- S78 -->
//  S103-fix7 Phase3-B-7.1: _diagProjectC 본체를 SXC.checkConsistency + SXC.renderConsistencyHTML로 이전하고 여기서는 SXC 위임 wrapper로 축소. 이전 9개 체크 로직은 전부 sx_project_c.js로 이동 — "C의 단일성" 원칙 강화 + 분석탭/BT탭 도움말 모달에서 동일 로직 재사용. 하위 호환: _isConsistentWithC/_diagCStatus/_checkLayerSeparation 내부 헬퍼는 그대로 유지(외부 호출 추적 안 돼서 보수적으로 남김, 사용 안 됨).
