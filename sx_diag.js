...
async function _diagSystem(){
  ...
  //   _msColor(500/1500ms)는 단건 호출용 → KRX엔 부적합
  //   KRX 임계값: 3s 양호 / 5s 보통 / 5s+ 느림
  const _krxColor = (ms) => ms < 3000 ? 'var(--buy)' : ms < 5000 ? 'var(--accent)' : 'var(--sell)';
  try{
    const t0 = performance.now();
    ...
    if (cnt > 0) {
      rows.push({label:'KRX 시세', value:`${cnt}종목 ${ms}ms${fb}`, color: _krxColor(ms)});
    } else {
