// ════════════════════════════════════════════════════════════
//  SIGNAL X — Diagnostic Module v1.2a <!-- S103-fix7 Phase3-B-0a 종목 탐색 fallback: closeAnalysis가 currentAnalStock=null 초기화해도 searchResults에서 최근 분석 종목 자동 탐색(실사용 UX 반영) --> v1.2 <!-- S103-fix7 Phase3-B-0 프로젝트 C 정합 진단 카테고리 추가(관찰만, UI 영향 없음) --> v1.1 <!-- S78 -->
//  S103-fix7 Phase3-B-0a: _diagProjectC 종목 탐색 fallback — currentAnalStock이 null(=분석 오버레이 닫힘)인 경우 searchResults 배열에서 _svVerdict를 가진 가장 최근 분석된 종목을 자동 탐색. "진단 대상" 행으로 어느 종목 기준인지 명시. 종목 미분석 시에도 searchResults 상태(X/Y개) 표시.
...
function _diagProjectC(){
  ...
  //    stock._threeStageReport.verdict.action 또는 stock._threeStageReport.verdict.label로 접근
  const _report = _stock._threeStageReport || null;
  if(_report && _report.verdict){
    // Phase 3-A-3 이후는 verdict.action = verdictAction (C와 동일)
    // 그 이전 레거시는 verdict.label = '보유 확인','관망' 등 5종
    ...
    rows.push({label: '_buildVerdict 정합', value: 'report 미생성', color: 'var(--text3)'});
  }
