/* ════════════════════════════════════════════════════════════════════
 * sx_recipe_signal.js — [S795] 레시피 신호감지 카드 (분석탭)
...
(function(){
  ...
  // [S797] 탭(풀)별 통계 바 — 관찰중·확정임박·적중평균(이 종목, 풀 내 진짜+가짜 풀링)
  function _statHtml(ps){
    ...
    if(ps.total>0){
      ...
      var rr=(ps.totalR>0)?(Math.round(100*ps.hitsR/ps.totalR)+'%'):'—', rcol=(ps.totalR>0 && ps.hitsR/ps.totalR>=0.5)?GR:RD;
      var ff=(ps.totalF>0)?(Math.round(100*ps.hitsF/ps.totalF)+'%'):'—', fcol=(ps.totalF>0 && ps.hitsF/ps.totalF>=0.5)?GR:RD;
      rate = '<span style="color:var(--text3)"> · 적중률 </span><b style="color:'+(ov>=50?GR:RD)+'">'+ov+'%</b>'
           + '<span style="color:var(--text3)"> (진짜 </span><b style="color:'+rcol+'">'+rr+'</b>'
           + '<span style="color:var(--text3)"> · 가짜 </span><b style="color:'+fcol+'">'+ff+'</b><span style="color:var(--text3)">)</span>';
