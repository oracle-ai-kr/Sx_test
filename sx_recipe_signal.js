...
(function(){
  ...
  function _renderBody(fNow, ltNow, maBullNow){
    ...
    return { inner:tab+sections+note, fireCount:(fireReal+fireFake), fireReal:fireReal, fireFake:fireFake, ltLabel:ltLabel };
  }
  ...
  async function _populate(sym){
    ...
    // 카테고리별 관찰중 요약 (확정임박 N봉 · 관찰중 M개) — 배지 인벤토리와 동일 소스(_scanStock 캐시 공유)
    try {
      var pend = await _pendingByCat(sym, rows);
      if(pend && window._sxRecipeActiveSym===sym){
        CATS.forEach(function(cat){
          var pel=document.getElementById('rcpcat_'+cat.pool+'_'+cat.kind); if(!pel) return;
          var p=pend[cat.pool+'|'+cat.kind];
