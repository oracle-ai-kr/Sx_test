/* ════════════════════════════════════════════════════════════════════
 * sx_recipe_signal.js — [S809] 레시피 신호감지 카드 (분석탭)
...
(function(){
   ...
   */
  async function _realFireBars(sym, rows){
   ...
   *   _fires가 정렬+단기약세 매칭하니 역배봉=deadcat-real만·정배봉=pullback-real만 자동 집계(풀 구분 공짜).
   *   반환 [{bar, lt('bear'/'bull'), k(겹침수), complete(N10확정), hit(real=후반평균 [+6..+10] 상승), ret}].
   *   교차검증 '레시피 겹침' 도구가 종목 전수로 모아 겹침수×풀별 N10 진짜비율+표본 집계. N10=실전 판정기준(S806).
   */
  async function _overlapScan(sym, rows){
    ...
    for(var i=0;i<scan.length;i++){
      var s=scan[i], k=0;
      for(var j=0;j<reals.length;j++){ if(_fires(reals[j], s.f, s.lt, s.maBull)) k++; }
      if(k===0) continue;                                            // 발동 봉만(겹침 측정 대상)
      var comp=_lateComplete(rows, s.bar), ret=comp?_lateRet(rows, s.bar):null;
      out.push({ bar:s.bar, lt:s.lt, k:k, complete:comp, hit:(comp&&ret!=null)?(ret>0):null, ret:ret });
    }
    return out;
  }
  ...
  window.SXRecipeSignal = { buildCard:buildCard, toggle:toggle, tab:_tab, _populate:_populate, _pendingByCat:_pendingByCat, realFireBars:_realFireBars, overlapScan:_overlapScan, RECIPES:RECIPES };
})();
