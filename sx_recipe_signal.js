/* ════════════════════════════════════════════════════════════════════
 * sx_recipe_signal.js — [S781] 레시피 신호감지 카드 (분석탭)
...
(function(){
  ...
  // 비동기: 과거 스캔 후 각 레시피 적중률 채움 (종목 가드)
  async function _scanFill(){
    var ctx = window._sxRecipeCtx; if(!ctx) return;
    var sym=ctx.sym, rows=ctx.rows;
    var body=document.getElementById('sxRecipeBody'); if(body) body.setAttribute('data-scanned','1');
    var scan; try { scan = await _scanStock(sym, rows); } catch(e){ return; }
    if(window._sxRecipeActiveSym!==sym) return;            // 다른 종목으로 이동 → 폐기
  ...
  function _wrap(inner, summaryHtml){
    ...
    return '<div class="anal-card" style="background:var(--surface);border:1px solid var(--border);border-radius:12px;margin:0 0 8px;overflow:hidden">'
      + '<div onclick="window.SXRecipeSignal&&SXRecipeSignal.toggle()" style="display:flex;align-items:center;gap:7px;padding:12px 14px;cursor:pointer">'
        ...
        + '<span style="margin-left:auto;font-size:10px;font-weight:700">'+summaryHtml+'</span>'
      + '</div>'
      + '<div id="sxRecipeBody" data-scanned="0" style="display:'+(open?'block':'none')+';padding:0 4px 8px">'+inner+'</div>'
    + '</div>';
  }

  function buildCard(stock, indicators){
    var rows = (indicators && indicators._advanced && indicators._advanced.rows) ? indicators._advanced.rows : null;
    var sym = (stock && (stock.code||stock.name)) || '?';
    window._sxRecipeActiveSym = sym;

    if(_curTf()!=='day'){
      return _wrap('<div style="padding:12px 12px;font-size:10px;color:var(--text3);line-height:1.5">레시피는 <b>일봉 기준</b>이라 현재 시간프레임에선 평가하지 않아요. 일봉으로 전환 시 표시됩니다.</div>', '<span style="color:var(--text3)">일봉 전용</span>');
    }
    if(!Array.isArray(rows) || rows.length<60){
      return _wrap('<div style="padding:12px 12px;font-size:10px;color:var(--text3)">데이터 부족 — 분석 완료 후 표시됩니다.</div>', '<span style="color:var(--text3)">–</span>');
    }
    window._sxRecipeCtx = { sym:sym, rows:rows };

    // 현재봉 발동 (동기 · calcAllScreener 1회)
    ...
    inner += '<div style="font-size:8px;color:var(--text3);padding:4px 10px 2px;line-height:1.5">발동 = 현재봉이 (그 풀 정렬+단기약세 + 재료조건) 충족. 적중 = 과거 발동 후 <b>후반평균([+6..+10]) 방향</b>이 신호와 일치(진짜=상승/가짜=하락). 적중률·최근결과는 <b>이 종목</b> 과거 기준 · 표본/후반은 풀 전체. 일봉 기준.</div>';

    var summary = fireCount>0
      ? ('<span style="color:'+GR+'">● 신호 '+fireCount+'개 발동</span> <span style="color:var(--text3);font-weight:500">· '+ltLabel+'</span>')
      : ('<span style="color:var(--text3)">관망 · '+ltLabel+'</span>');

    // 펼쳐진 상태면 렌더 직후 과거스캔 자동 실행
    if(window._sxRecipeOpen){ setTimeout(function(){ try{ _scanFill(); }catch(e){} }, 60); }

    return _wrap(inner, summary);
  }
  ...
  function toggle(){
    try {
      ...
      if(!open && body.getAttribute('data-scanned')!=='1'){ _scanFill(); }
    } catch(e){}
  }

  window.SXRecipeSignal = { buildCard:buildCard, toggle:toggle, _scanFill:_scanFill, RECIPES:RECIPES };
})();
