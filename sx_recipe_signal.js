...
var RECIPES_BY_MKT = {
    ...
    POOLS.forEach(function(p){
      ...
      CATS.filter(function(c){ return c.pool===p.key; }).forEach(function(cat){
        ...
        catBlocks += '<div style="border-top:1px solid var(--border)">'
          ...
          + '</div>'
          + '<div id="'+catId+'" style="display:none"><div data-rcpfireempty="'+catFire+'" style="display:'+((FIRE_ONLY&&catFire===0)?'block':'none')+';font-size:8px;color:var(--text3);padding:7px 10px">현재봉 발동 없음 — [발동만] 해제 시 전체 '+recs.length+'개</div>'+rowsHtml+'</div>'
        + '</div>';
      });
      var fireChk='<div style="display:flex;justify-content:flex-end;align-items:center;padding:4px 10px 0"><label style="display:inline-flex;align-items:center;gap:4px;font-size:9px;font-weight:700;color:var(--text3);cursor:pointer"><input type="checkbox" data-rcpfireonly="1" '+(FIRE_ONLY?'checked':'')+' onchange="window.SXRecipeSignal&&SXRecipeSignal.fireOnly(this.checked)" style="width:12px;height:12px;accent-color:var(--accent)">발동만 보기</label></div>';   // [S853]
  ...
  // [S853] 발동만 보기 — 상단 활성 섹션 제거 대체. 기본 ON(발동 레시피만 노출), 해제 시 전체(3시장 공통). 재스캔 없음(display 토글)이라 즉시 반영.
  var FIRE_ONLY=true;
  function _fireOnlySet(v){
    ...
    try{
      ...
      for(var j=0;j<em.length;j++){ em[j].style.display=(FIRE_ONLY && em[j].getAttribute('data-rcpfireempty')==='0')?'block':'none'; }
  ...
  window.SXRecipeSignal = { buildCard:buildCard, toggle:toggle, tab:_tab, catToggle:_catToggle, _populate:_populate, _pendingByCat:_pendingByCat, realFireBars:_realFireBars, pullbackSignalBars:_pullbackSignalBars, overlapScan:_overlapScan, baseRateScan:_baseRateScan, deadcatTrajScan:_deadcatTrajScan, deadcatConfirmScan:_deadcatConfirmScan, deadcatOverlapHzScan:_deadcatOverlapHzScan, deadcatHzBtScan:_deadcatHzBtScan, fireOnly:_fireOnlySet, recipesFor:_recipesFor };
