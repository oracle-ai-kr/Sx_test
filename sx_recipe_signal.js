/* ════════════════════════════════════════════════════════════════════
 * sx_recipe_signal.js — [S816] 레시피 신호감지 카드 (분석탭)
...
(function(){
  ...
  async function _overlapScan(sym, rows){
    ...
    for(var i=0;i<scan.length;i++){
      ...
      out.push({ bar:s.bar, lt:s.lt, realK:rk, fakeK:fk, k:rk, complete:comp, ret:ret, hit:(comp&&ret!=null)?(ret>0):null });
    }
    return out;
  }
  ...
  window.SXRecipeSignal = { buildCard:buildCard, toggle:toggle, tab:_tab, _populate:_populate, _pendingByCat:_pendingByCat, realFireBars:_realFireBars, pullbackSignalBars:_pullbackSignalBars, overlapScan:_overlapScan, RECIPES:RECIPES };
})();
