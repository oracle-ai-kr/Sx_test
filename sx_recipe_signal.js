...
(function(){
  ...
  function _statHtml(ps){
    if(!ps) ps={pend:0,oldest:-1,hits:0,total:0};
    var pend = ps.pend>0
    ...
    var rate = ps.total>0
      ? ('<span style="color:var(--text3)"> · 적중평균 </span><b style="color:'+(ps.hits/ps.total>=0.5?GR:RD)+'">'+Math.round(100*ps.hits/ps.total)+'%</b><span style="color:var(--text3)"> ('+ps.hits+'/'+ps.total+')</span>')
      : '<span style="color:var(--text3)"> · 적중 표본없음</span>';
    return pend+rate;
  }
  ...
  async function _populate(sym){
    ...
    var catPend={}, poolStat={pullback:{pend:0,oldest:-1,hits:0,total:0}, deadcat:{pend:0,oldest:-1,hits:0,total:0}};
    RECIPES.forEach(function(rec){
      ...
      var ps=poolStat[rec.pool]; if(!ps) ps=poolStat[rec.pool]={pend:0,oldest:-1,hits:0,total:0};
      ...
      if(h.total>0){ ps.hits+=h.hits; ps.total+=h.total; }
    });
    if(window._sxRecipeActiveSym!==sym) return;
