...
(function(){
        ...
        if(_CELLON && typeof _sxCellSignalCore==='function'){
          try{ var _c0=_sxCellSignalCore(_CELLMK, ind, rows, bi);
            if(_c0){ var _r7=0,_d7=0,_f7=0, _sgA=_c0.sig||[];
              for(var _q7=0;_q7<_sgA.length;_q7++){ var _sg=_sgA[_q7]; if(_sg&&_sg.hit){ if(_sg.kind==='real')_r7++; else if(_sg.kind==='down')_d7=1; else if(_sg.kind==='fake')_f7=1; } }   // [S1117] real=히트 개수
  ...
  /* ───────── [S1119] 커스텀 재료 조건 봉맵 — 크로스 상세 '🧪 재료 조건'용 ─────────
   *   conds 스키마 = 자유 조합 빌더 동일: {key,type:'num',dir:'lt'|'gt',th} / {key,type:'bool'} — feats733(s.f) 어휘.
   *   buy=모두 충족(AND) · sell=하나라도(OR). 스캔은 _cs 키로 전략 카드와 캐시 공유(첫 회만 무겁고 임계 수정=재매핑만).
   *   반환 {buy:{barIdx:true}|null, sell:{barIdx:true}|null} — 웜업(스캔 시작 전) 봉은 맵에 없음(소비측이 차단 처리).
   */
  async function _customCondBars(sym, rows, mk, buy, sell){
    var EMPTY={buy:null, sell:null};
    if(!Array.isArray(rows) || rows.length<260) return EMPTY;
    var hasB=Array.isArray(buy)&&buy.length>0, hasS=Array.isArray(sell)&&sell.length>0;
    if(!hasB && !hasS) return EMPTY;
    var _pv=_CELLON, _pm=_CELLMK; _CELLON=true; _CELLMK=(mk==='us'||mk==='coin')?mk:'kr';
    var scan=null;
    try{ scan=await _scanStock(sym, rows); }catch(e){ scan=null; }
    finally{ _CELLON=_pv; _CELLMK=_pm; }
    if(!Array.isArray(scan)) return EMPTY;
    var _ck=function(c,f){ var v=f?f[c.key]:null;
      if(c.type==='num'){ if(v==null || !isFinite(v)) return false; return c.dir==='lt' ? v<c.th : v>c.th; }
      return v===1; };
    var mB=hasB?{}:null, mS=hasS?{}:null;
    for(var i=0;i<scan.length;i++){ var s=scan[i]; if(!s||!s.f) continue;
      if(hasB){ var ok=true; for(var a=0;a<buy.length;a++){ if(!_ck(buy[a], s.f)){ ok=false; break; } } if(ok) mB[s.bar]=true; }
      if(hasS){ for(var b=0;b<sell.length;b++){ if(_ck(sell[b], s.f)){ mS[s.bar]=true; break; } } }
    }
    return {buy:mB, sell:mS};
  }
  ...
  async function _profileScan(sym, rows){
    ...
    for(var i=0;i<scan.length;i++){
      ...
      var rk=0, fk=0, _firedR=[];
      for(var jr=0;jr<reals.length;jr++){ if(_fires(reals[jr], s.f, s.lt, s.maBull)){ rk++; _firedR.push(reals[jr]); } }
      for(var jf=0;jf<fakes.length;jf++){ if(_fires(fakes[jf], s.f, s.lt, s.maBull)) fk++; }
      // [S881] 임계 접근방향 — 발동 real 레시피들의 수치 조건별 전봉 대비 Δf 부호(+1 상승/−1 하강, 0=보합 제외). key_dir 단위 dedup.
      //   lt+상승=회복형(밑에서 올라옴)·lt+하강=낙하형 / gt+상승=점화형·gt+하강=소진형. 전봉 비연속(bar 갭)이면 null.
      var _ap=null;
      if(rk>0 && i>0 && scan[i-1] && scan[i-1].bar===b-1 && scan[i-1].f){
        _ap={}; var _fp=scan[i-1].f;
        for(var _fr=0;_fr<_firedR.length;_fr++){ var _cs=_firedR[_fr].conds||[];
          for(var _ci=0;_ci<_cs.length;_ci++){ var _c=_cs[_ci]; if(_c.type!=='num') continue;
            var _kk=_c.key+'_'+_c.dir; if(_ap[_kk]!==undefined) continue;
            var _v=s.f[_c.key], _pv=_fp[_c.key];
            if(typeof _v!=='number'||typeof _pv!=='number') continue;
            _ap[_kk]=(_v>_pv)?1:((_v<_pv)?-1:0);
          } }
      }
      if(rk>0){ if(side && b-lastReal>5) events.push({ bar:b, lt:side, kind:'real', k:rk, xk:fk, tp:s.tp, md:_mdAt(scan,i), ap:_ap, cum:cum }); lastReal=b; }   // [S857] xk · [S861] tp/md · [S881] ap
