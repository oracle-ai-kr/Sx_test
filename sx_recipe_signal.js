/* ════════════════════════════════════════════════════════════════════
 * sx_recipe_signal.js — [S829] 레시피 신호감지 카드 (분석탭)
...
(function(){
   ...
   */
  async function _deadcatTrajScan(sym, rows){
    if(!Array.isArray(rows) || rows.length<260) return [];
    var scan; try { scan = await _scanStock(sym, rows); } catch(e){ return []; }
    var reals = RECIPES.filter(function(r){ return r.kind==='real' && r.pool==='deadcat'; });
    var out=[];
    for(var i=0;i<scan.length;i++){
      var s=scan[i];
      if(s.lt!=='bear') continue;
      var fired=false;
   ...
   */
  async function _deadcatConfirmScan(sym, rows){
    if(!Array.isArray(rows) || rows.length<260) return [];
    var scan; try { scan = await _scanStock(sym, rows); } catch(e){ return []; }
    var reals = RECIPES.filter(function(r){ return r.kind==='real' && r.pool==='deadcat'; });
    var out=[];
    for(var i=0;i<scan.length;i++){
      ...
      var rec={imm:null,c1:null,c2:null};
      if(_lateComplete(rows,bar)){ var r0=_lateRet(rows,bar); if(r0!=null) rec.imm={ret:r0,real:r0>0}; }   // 즉시(진입가=신호봉 종가)
      var c1=(rows[bar+1]&&typeof rows[bar+1].close==='number')?rows[bar+1].close:null;
      if(c1!=null && c1>ep && _lateComplete(rows,bar+1)){ var r1=_lateRet(rows,bar+1); if(r1!=null) rec.c1={ret:r1,real:r1>0}; }   // t+1 확인(살아남음→t+1 종가 진입)
      var c2=(rows[bar+2]&&typeof rows[bar+2].close==='number')?rows[bar+2].close:null;
      if(c2!=null && c2>ep && _lateComplete(rows,bar+2)){ var r2=_lateRet(rows,bar+2); if(r2!=null) rec.c2={ret:r2,real:r2>0}; }   // t+2 확인
      if(rec.imm||rec.c1||rec.c2) out.push(rec);
    }
    return out;
  }
