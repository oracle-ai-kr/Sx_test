...
var RECIPES_BY_MKT = {
   ...
   *   반환 { events:[{bar,lt,kind,k(겹침수),cum:[20]}], base:{bear:{n,hit[20](상승수),sum[20]},bull:{...}} } | null. _scanStock 캐시 공유 · 측정전용.
   */
  async function _profileScan(sym, rows){
    ...
    for(var i=0;i<scan.length;i++){
      ...
      if(rk>0){ if(side && b-lastReal>5) events.push({ bar:b, lt:side, kind:'real', k:rk, cum:cum }); lastReal=b; }
      if(fk>0){ if(side && b-lastFake>5) events.push({ bar:b, lt:side, kind:'fake', k:fk, cum:cum }); lastFake=b; }
    }
  ...
  window.SXRecipeSignal = { buildCard:buildCard, toggle:toggle, tab:_tab, catToggle:_catToggle, _populate:_populate, _pendingByCat:_pendingByCat, realFireBars:_realFireBars, pullbackSignalBars:_pullbackSignalBars, overlapScan:_overlapScan, profileScan:_profileScan, baseRateScan:_baseRateScan, deadcatTrajScan:_deadcatTrajScan, deadcatConfirmScan:_deadcatConfirmScan, deadcatOverlapHzScan:_deadcatOverlapHzScan, deadcatHzBtScan:_deadcatHzBtScan, fireOnly:_fireOnlySet, recipesFor:_recipesFor };
