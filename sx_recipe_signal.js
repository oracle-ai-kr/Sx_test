/* ════════════════════════════════════════════════════════════════════
 * sx_recipe_signal.js — [S800] 레시피 신호감지 카드 (분석탭)
 ...
 *   [S795] _pendingByCat(sym,rows) 노출 — 카테고리별 '관찰중'(최근발동 0-5봉) 집계 · 배지 인벤토리가 당겨씀(동일 _scanStock 캐시).
...
(function(){
  ...
  /* ───────── 카테고리별 '관찰중' 집계 (최근발동 0-5봉=결과 미확정) · _scanStock 캐시 공유 · 배지 인벤토리가 당겨씀 ───────── */
  ...
  // [S797] 탭(풀)별 통계 바 — 관찰중·확정임박·적중률(이 종목, 풀 내 진짜+가짜 풀링) · [S800] 적중률 옆 표본수(N%/n) + 진짜/가짜 분해(X%/x · Y%/y)
  function _statHtml(ps){
    if(!ps) ps={pend:0,oldest:-1,hits:0,total:0,hitsR:0,totalR:0,hitsF:0,totalF:0};
    var pend = ps.pend>0
    ...
    var rate;
    if(ps.total>0){
      var ov=Math.round(100*ps.hits/ps.total);
      var rr=(ps.totalR>0)?('<b style="color:'+(ps.hitsR/ps.totalR>=0.5?GR:RD)+'">'+Math.round(100*ps.hitsR/ps.totalR)+'%</b><span style="color:var(--text3)">/'+ps.totalR+'</span>'):'<b style="color:var(--text3)">—</b>';
      var ff=(ps.totalF>0)?('<b style="color:'+(ps.hitsF/ps.totalF>=0.5?GR:RD)+'">'+Math.round(100*ps.hitsF/ps.totalF)+'%</b><span style="color:var(--text3)">/'+ps.totalF+'</span>'):'<b style="color:var(--text3)">—</b>';
      rate = '<span style="color:var(--text3)"> · 적중률 </span><b style="color:'+(ov>=50?GR:RD)+'">'+ov+'%</b><span style="color:var(--text3)">/'+ps.total+'</span>'
           + '<span style="color:var(--text3)"> (진짜 </span>'+rr
           + '<span style="color:var(--text3)"> · 가짜 </span>'+ff
           + '<span style="color:var(--text3)">)</span>';
    } else { rate = '<span style="color:var(--text3)"> · 적중 표본없음</span>'; }
    return pend+rate;
  }
  ...
  async function _populate(sym){
    ...
    var catPend={}, poolStat={pullback:{pend:0,oldest:-1,hits:0,total:0,hitsR:0,totalR:0,hitsF:0,totalF:0}, deadcat:{pend:0,oldest:-1,hits:0,total:0,hitsR:0,totalR:0,hitsF:0,totalF:0}};
    RECIPES.forEach(function(rec){
      ...
      var ps=poolStat[rec.pool]; if(!ps) ps=poolStat[rec.pool]={pend:0,oldest:-1,hits:0,total:0,hitsR:0,totalR:0,hitsF:0,totalF:0};
      ...
      if(h.total>0){ ps.hits+=h.hits; ps.total+=h.total; if(rec.kind==='real'){ ps.hitsR+=h.hits; ps.totalR+=h.total; } else { ps.hitsF+=h.hits; ps.totalF+=h.total; } }
    });
    if(window._sxRecipeActiveSym!==sym) return;
  ...
  window.SXRecipeSignal = { buildCard:buildCard, toggle:toggle, tab:_tab, _populate:_populate, _pendingByCat:_pendingByCat, RECIPES:RECIPES };
})();
