/* ════════════════════════════════════════════════════════════════════
 * sx_recipe_signal.js — [S789] 레시피 신호감지 카드 (분석탭)
...
(function(){
  ...
  function _renderBody(fNow, ltNow, maBullNow){
    ...
    var POOLS=[{key:'deadcat',label:'역배열'},{key:'pullback',label:'정배열'}];
    var fireCount=0;
    // 탭 바 (현재 레짐 탭에 ● 표시)
    ...
    POOLS.forEach(function(p){
      ...
      CATS.filter(function(c){ return c.pool===p.key; }).forEach(function(cat){
        ...
        sec += '<div style="font-size:10px;font-weight:800;color:'+cat.tone+';padding:8px 9px 2px">'+cat.label+' <span style="color:var(--text3);font-weight:600">('+recs.length+')</span></div>';
        recs.forEach(function(r){ var fr=_fires(r, fNow, ltNow, maBullNow); if(fr) fireCount++; sec += _recipeRow(r, fr); });
      });
      sec+='</div>';
      sections+=sec;
    });
    ...
    return { inner:tab+sections+note, fireCount:fireCount, ltLabel:ltLabel };
  }
  ...
  window.SXRecipeSignal = { buildCard:buildCard, toggle:toggle, tab:_tab, _populate:_populate, RECIPES:RECIPES };
})();
