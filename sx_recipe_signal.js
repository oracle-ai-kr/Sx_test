...
var RECIPES_BY_MKT = {
  ...
  async function _profileScan(sym, rows){
    ...
    for(var i=0;i<scan.length;i++){
      ...
      if(rk>0){ if(side && b-lastReal>5) events.push({ bar:b, lt:side, kind:'real', k:rk, xk:fk, cum:cum }); lastReal=b; }   // [S857] xk=같은 봉 상대 kind(fake) 동시발동 수 — 오염 측정용
      if(fk>0){ if(side && b-lastFake>5) events.push({ bar:b, lt:side, kind:'fake', k:fk, xk:rk, cum:cum }); lastFake=b; }
    }
