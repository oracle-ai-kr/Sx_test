...
(function(){
  ...
  function predict(rows, opts){
    ...
    // 유사도% 표시 스케일 = 전체 후보거리의 90퍼센타일(='꽤 다름' 기준)
    var ds = cand.map(function(c){ return Math.sqrt(c.d); }).sort(function(x, y){ return x - y; });
    var ref = ds[Math.min(ds.length - 1, Math.floor(ds.length * 0.9))] || (ds[ds.length - 1] || 1e-9);

    // 아날로그별 forward 경로 + 최종수익 + 구간내 최대낙폭
    ...
    for(var p = 0; p < picked.length; p++){
      ...
      var sim = Math.max(0, Math.min(100, Math.round((1 - Math.sqrt(picked[p].d) / ref) * 100)));
      analogs.push({
        e:pe, date:(rows[pe] && rows[pe].date) || null,
        sim:sim, ret:Math.round(fin * 1000) / 10, maxDD:Math.round(dd * 1000) / 10, path:path
      });
    }
