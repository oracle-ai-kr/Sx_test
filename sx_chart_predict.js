...
(function(){
  ...
  function predict(rows, opts){
    ...
    // [S670] 변동폭(분산) — 방향(상승비율)보다 신뢰할 수 있는 정보. 80% 구간(p10~p90).
    var _pc = function(q){ if(!rets.length) return 0; var ix=(rets.length-1)*q, lo=Math.floor(ix), hi=Math.ceil(ix); return lo===hi?rets[lo]:rets[lo]+(rets[hi]-rets[lo])*(ix-lo); };
    var _p10=_pc(0.10), _p90=_pc(0.90);

    return {
      active:true, n:nA, win:win, h:h,
      upProb: Math.round(up / nA * 100),                 // 상승비율(%) — 유사패턴 중 상승 카운트(방향예측 아님)
      avg:    Math.round((sum / nA) * 1000) / 10,        // 평균수익(%)
      median: Math.round(_median(rets) * 1000) / 10,     // 중앙수익(%)
      max:    Math.round(rets[rets.length - 1] * 1000) / 10, // 최대수익(%)=상방
      min:    Math.round(rets[0] * 1000) / 10,           // 최소수익(%)=하방
      p10:    Math.round(_p10 * 1000) / 10,              // [S670] 하위10% 분위
      p90:    Math.round(_p90 * 1000) / 10,              // [S670] 상위10% 분위
      band:   Math.round(((_p90 - _p10) / 2) * 1000) / 10, // [S670] ±변동폭(80% 구간 반폭)
      bins:   bins,
