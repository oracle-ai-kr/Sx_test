...
//    + [S606] 기본값 16×10 (풀검증 최적). 창 길이(win=8/12/16)·K(이웃수) 파라미터화
//    + [S608] 이웃 품질 게이팅(opts.gateRatio) — 닮은꼴이 충분히 밀집할 때만 신호. tightRatio=k이웃평균거리/중앙거리. — 백테스트 모달에서 토글로 A/B 비교. 가중치는 win에 맞춰 동적 생성.
//  로드 순서: sx_render.js 다음, sx_candle_bt.js 이전 (BT가 SXKNN을 호출).
// ════════════════════════════════════════════════════════════════
(function(){
  'use strict';

  var WIN = 16;       // [S606] 기본 창 16봉 (풀 검증: 16×10 @≥30 최적·robust)
  var CTX = 20;       // 지지/저항 위치 산출 룩백
  var MIN_BANK = 40;  // 최소 후보 창 수 (미만이면 inactive — 자기유사도 표본 부족)
  var K = 10;         // [S606] 기본 이웃 10
  ...
  // [S612] 날짜→비교가능 정수 (풀링 뱅크 교차종목 룩어헤드 가드용). 일봉=YYYYMMDD, 분봉=YYYYMMDDhhmm. 같은 TF 내 일관.
  function _dnum(d){
    if(d==null) return 0;
    var s=String(d), m=s.match(/(\d{4})-?(\d{2})-?(\d{2})/), base = m ? ((+m[1])*10000+(+m[2])*100+(+m[3])) : 0;
    var tm=s.match(/(\d{2}):(\d{2})/); if(tm) base = base*10000 + (+tm[1])*100 + (+tm[2]);
    return base;
  }
  ...
  // [S612] 후보배열({d,up})에서 거리가중 가우시안 투표 + tightness 게이팅. scoreAt(자기)·scorePooled(풀링) 공유 — 드리프트 방지.
  function _vote(cand, opts){
    opts = opts || {};
    ...
    var nearSum=0; for(var t=0;t<k;t++) nearSum+=Math.sqrt(cand[t].d);
    var nearMean=nearSum/k;
    var medAll=Math.sqrt(cand[Math.floor(cand.length/2)].d) || 1e-9;
    var tightRatio=Math.round((nearMean/medAll)*1000)/1000;
    if(opts.gateRatio!=null && tightRatio > opts.gateRatio){
      return { active:false, gated:true, tightRatio:tightRatio, reason:'analog 희박(tight '+tightRatio+'>'+opts.gateRatio+')' };
    }
    return {
      active:true, score:sc, upFrac:Math.round(upFrac*1000)/10,
      k:k, nUp:nUp, nDn:k-nUp, bank:cand.length, tightRatio:tightRatio,
  ...
  function scoreAt(rows, sigE, pre, opts){
    ...
    if(cand.length < MIN_BANK) return { active:false, reason:'후보 부족('+cand.length+')' };
    var res = _vote(cand, opts);
    if(res.active) res.win = win;
    return res;
  }

  // [S612] 다종목 풀링 뱅크 — 여러 종목의 과거 창벡터를 하나의 후보풀로. 각 항목에 결과봉 날짜(oDate)·결과(up) 태그.
  //   stocksRows: [{name, rows}]. 반환: { bank:[{vec,oDate,up}], win, W }. (창끝 e의 결과=rows[e+1] 필요 → e ≤ last-1)
  function buildPoolBank(stocksRows, win){
    win = win || WIN;
    var bank=[], W=_W(win);
    for(var si=0; si<stocksRows.length; si++){
      var rows = stocksRows[si] && stocksRows[si].rows;
      if(!Array.isArray(rows) || rows.length < win+1) continue;
      var _src = (stocksRows[si] && (stocksRows[si].code || stocksRows[si].name)) || '';   // [S644] 출처종목 — 풀에서 자기 제외용
      var last = rows.length-1;
      for(var e=win-1; e<=last-1; e++){
        var v=_vec(rows,e,win); if(!v) continue;
        bank.push({ vec:v, oDate:_dnum(rows[e+1].date), up:_isUp(rows[e+1]), src:_src });
      }
    }
    return { bank:bank, win:win, W:W };
  }

  // [S612] 풀링 뱅크 kNN — 쿼리벡터 qVec(현재종목 bar i) + 쿼리날짜 qDate(=현재종목 bar i 날짜).
  //   룩어헤드 가드: 후보의 결과봉 날짜 oDate ≤ qDate 만 사용(자기·타종목 모두 인과적). 투표는 _vote 공유.
  function scorePooled(qVec, qDate, pool, opts){
    opts = opts || {};
    if(!qVec || !pool || !pool.bank) return { active:false, reason:'풀 없음' };
    var W=pool.W, bank=pool.bank, cand=[];
    for(var i=0;i<bank.length;i++){
      var b=bank[i];
      if(b.oDate > qDate) continue;   // 결과가 쿼리시점 이후 → 제외(룩어헤드 차단)
      cand.push({ d:_dist2(qVec,b.vec,W), up:b.up });
    }
    var res = _vote(cand, opts);
    if(res.active) res.win = pool.win;
    return res;
  }
  ...
  // [S611] 라이브 카드용 빠른 kNN 백테스트 — 모달 runBacktest의 kNN 경로 미러(win16/k10/게이팅off/≥30, 평가 마지막 150봉).
  //   룩어헤드: scoreAt 인덱스필터로 차단. 룰은 평가범위(슬라이스 450봉+)서 항상 active라 모달 표본과 동일 → 메인 카드 적중률 = [검증] 모달과 일치.
  function backtestHit(rows, opts){
    opts = opts || {};
    if(!Array.isArray(rows)) return null;
    var win = opts.win || WIN, k = opts.k || K, thr = opts.thr || 30;
    var maxTests = opts.maxTests || 150, MINB = 60, n = rows.length;
    if(n < MINB + 5) return null;
    var pre; try { pre = buildVecs(rows, win); } catch(e){ return null; }
    if(!pre) return null;
    var startI = Math.max(MINB, n - 1 - maxTests);
    var pred=0, hit=0, bullP=0, bullH=0, bearP=0, bearH=0;
    for(var i=startI; i<=n-2; i++){
      var r; try { r = scoreAt(rows, i, pre, { k:k }); } catch(e2){ continue; }
      if(!r || !r.active || Math.abs(r.score) < thr) continue;
      var up = (+rows[i+1].close) >= (+rows[i+1].open);
      var pu = r.score > 0, ok = (pu === up);
      pred++; if(ok) hit++;
      if(pu){ bullP++; if(ok) bullH++; } else { bearP++; if(ok) bearH++; }
    }
    return {
      hitRate: pred ? Math.round(hit/pred*1000)/10 : null, pred: pred, evalN: (n-1)-startI,
      bullHit: bullP?Math.round(bullH/bullP*1000)/10:null, bullPred:bullP,
      bearHit: bearP?Math.round(bearH/bearP*1000)/10:null, bearPred:bearP
    };
  }

  // [S633] 크로스 임박 D-day — MA수렴(해석적) + 골든/데드 라벨 kNN. 조건검색·라이브 카드 공용(미러 드리프트 방지).
  //   반환: {active, type:'gc'|'dc', dday, prob(%)} | {active:false}. 룩어헤드: 후보 e≤li-maxLead, 라벨 e+maxLead≤li.
  function _smaArr(arr, p){ var out=new Array(arr.length).fill(null), s=0; for(var i=0;i<arr.length;i++){ s+=arr[i]; if(i>=p) s-=arr[i-p]; if(i>=p-1) out[i]=s/p; } return out; }
  function crossDday(rows, opts){
    ...
    var n=rows.length;
    if(n < win + MIN_BANK) return { active:false, reason:'데이터 부족' };
    var close=[]; for(var i=0;i<n;i++){ var c=rows[i]; close.push(+(c.close!=null?c.close:c.c)); }
    ...
    var pre=buildVecs(rows,win), W=pre.W, q=pre.vecs[li-pre.from];
    if(!q) return { active:false };
    var _lab=function(e){ for(var j=e+1;j<=e+maxLead;j++){ if(j<1||j>=n) break; if(maS[j]==null||maL[j]==null||maS[j-1]==null||maL[j-1]==null) continue; if(type==='gc'){ if(maS[j]>maL[j]&&maS[j-1]<=maL[j-1]) return true; } else { if(maS[j]<maL[j]&&maS[j-1]>=maL[j-1]) return true; } } return false; };
    var cand=[];
    for(var e=win-1;e<=li-maxLead;e++){ var v=pre.vecs[e-pre.from]; if(!v) continue; cand.push({ d:_dist2(q,v,W), up:_lab(e) }); }
    var res=_vote(cand,{k:opts.k||K});
    if(!res.active) return { active:false, reason:res.reason };
    var prob=res.upFrac/100, dday=Math.max(1,Math.round(btc));
    if(prob < thr) return { active:false, type:type, dday:dday, prob:res.upFrac };
    return { active:true, type:type, dday:dday, prob:res.upFrac, bank:res.bank };
  }

  // [S621] window→self: 워커(importScripts)에서도 동작. 메인스레드선 self===window라 window.SXKNN 그대로 유효.
  (typeof self !== 'undefined' ? self : window).SXKNN = { score:score, scoreAt:scoreAt, backtestHit:backtestHit, buildVecs:buildVecs, buildPoolBank:buildPoolBank, scorePooled:scorePooled, crossDday:crossDday, _dnum:_dnum, _vec:_vec, WIN:WIN, K:K, MIN_BANK:MIN_BANK, WINS:[8,12,16], KS:[10,25,50] };
})();
