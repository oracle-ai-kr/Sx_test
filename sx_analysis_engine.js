...
function scrQuickScore(rows, tf) {
  ...
  const _sf = SXE._safetyFlags || {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,bbUpper:true,resistNear:true,debtRatio:true,foreignSell:true,highBeta:true};
  const _wasBuySafety = (action === 'BUY'); // [S361] 안전필터 진입 시점 BUY 여부 (위반 전수 수집 게이트)
  ...
  // [S361] 신규 안전필터 — BB 상단 이탈(과열) / 저항(피벗 R1) 근접
  ...
  // [S361] C 캡 전용 위반 전수 수집 — reasons/action 로직과 독립 (조건검색 _safety_clean 카운트 불변).
  //   scrQuickScore의 reasons는 action===BUY 가드로 첫 HOLD 후 일부 항목이 누락되므로,
  //   C(unifiedVerdictV2)의 항목별 캡(관망/관심) 판정을 위해 진입 후보(_wasBuySafety)의 모든 위반을 별도 수집.
  const _safetyViol = [];
  if (_wasBuySafety) {
  ...
  return {
    ...
    _safetyViol, // [S361] C 캡 전용 안전필터 위반 전수 목록 (🔒...)
    // S80: 3단 점수
...
//        [S361] bbUpper(BB상단이탈)·resistNear(저항근접) 추가 → 12개 전수 ON.
//        주의: 스크리너 _loadSafetyFilterUI가 첫 로드 시 localStorage 값으로 덮어씀 — 이 초기값은
//        스크리너 로드 전 BT/실시간 신호가 안전필터 호출하는 레이스 윈도우에서만 사용됨.
SXE._safetyFlags = {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,bbUpper:true,resistNear:true,debtRatio:true,foreignSell:true,highBeta:true};
