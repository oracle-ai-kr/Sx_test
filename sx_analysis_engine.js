...
const _MODE_MA_PAIR = { profit:[5,20], balanced:[20,60], safe:[60,120] };
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    if (!pos && !pendingSignal) {
      const _scorePass = rawScore >= buyTh && _scrMomOscPass(mom, osc, tf);
      // [S563] 정배열 게이트 — 모드 MA쌍이 정배열일 때만 점수 진입 허용 (진입 전용). 게이트 OFF(_gatePair=null)면 기존 동작.
      if (_scorePass && _gatePair && !_btModeAligned(rows, i, _gatePair)) { _modeGateSkips++; }
      else if (_scorePass) {
