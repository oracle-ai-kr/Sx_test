...
//   기존 모드 체계(profit/balanced/safe)에서 시장 체계(kr/us/crypto)로 전환.
//   사용자 성향 축 → 시장 종류 축으로 의미 변경.
//
//   초기 하드코딩 값은 기존 모드 매핑을 출발점으로 설정 (C안 — 깨끗이 시작):
//     · kr     ← 기존 balanced (안정/중간 임계) — 한국 시장 일반 변동성에 적합
//     · us     ← 기존 safe     (보수/높은 임계) — 미국 대형주 추세 추종
//     · crypto ← 기존 profit   (공격/낮은 임계) — 코인 큰 변동성·강한 추세
//   사용자가 시장별로 옵티마이저 재실행 권장.
//
// 공통: MA 5/20/60, RSI 14, ATR 14 (step=0 고정 파라미터)
// [v3.17 SELL-TH-FIX] sellTh 값 정상화 (8 → 38, 6 → 35)
//   기존 버그: sellTh:8/6은 점수 0~10 영역으로 의미 없음. 분석엔진 line 2967의
//             clamp(sellTh, 20, 50)에 의해 항상 20으로 강제 변환됨 → 옵티마이저로
//             sellTh를 어떻게 설정하든 효과 없음 (clamp가 일률적으로 20으로 올림).
//   원인: 과거 sellTh가 "신호 카운트(MACD 음수 봉 수 등)" 의미로 쓰이다 점수 기반
//        (rawScore <= sellTh로 SELL 판정)으로 통합되었으나 시장별 기본값 미업데이트.
//   수정: clamp(20, 50) 안의 의미 있는 값으로 변경
//     - ON  슬롯 sellTh: 38 (TF별 day 기본값과 일치, 옵티마이저 fallback 38과 동일)
//     - OFF 슬롯 sellTh: 35 (보수형은 약세 더 빨리 잡고 빠지기 — buyTh ON<OFF 패턴 대칭)
//   영향: 분석/BT의 SELL 판정 정상화. 기존 사용자가 _saveAnalParams로 저장한 값은
//        그대로 유지(우선순위: SCR_ANAL_PARAMS > CUSTOM > 하드코딩). 신규 설치/리셋 시에만 적용.
const SCR_ANAL_MARKET_DEFAULTS = {
  // [S212] 단기·스윙 + 밸런스형 추천값 (도움말 사진 기준)
  // 레짐 무관 fallback — 각 시장 score 높은 쪽
  kr:     { rsiLen:14, bbLen:11, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:42, sellTh:38, tpMult:2.5, slMult:1.5 },
  us:     { rsiLen:14, bbLen:23, bbMult:1.8, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:48, sellTh:38, tpMult:2.5, slMult:1.6 },
  crypto: { rsiLen:14, bbLen:10, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:32, sellTh:38, tpMult:3.0, slMult:1.8 }
};
// S211: 시장×레짐 6개 조합별 기본값
// [S212] 단기·스윙 + 밸런스형 추천값 적용
//   ON  슬롯: 추천값 (추세장 진입)
//   OFF 슬롯: 보수적 변형 (횡보·약세장에서는 buyTh 살짝 ↑, slMult 살짝 ↑, sellTh 살짝 ↓)
// [v3.17 SELL-TH-FIX] sellTh ON 38 / OFF 35로 통일 (이전 8/6은 clamp(20,50)에 의해 무력화되던 값)
const SCR_ANAL_MARKET_REGIME_DEFAULTS = {
  kr: {
    on:  { rsiLen:14, bbLen:11, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:42, sellTh:38, tpMult:2.5, slMult:1.5 },
    off: { rsiLen:14, bbLen:14, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:46, sellTh:35, tpMult:2.25, slMult:1.6 }
  },
  us: {
    on:  { rsiLen:14, bbLen:23, bbMult:1.8, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:48, sellTh:38, tpMult:2.5, slMult:1.6 },
    off: { rsiLen:14, bbLen:26, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:52, sellTh:35, tpMult:2.25, slMult:1.7 }
  },
  crypto: {
    on:  { rsiLen:14, bbLen:10, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:32, sellTh:38, tpMult:3.0, slMult:1.8 },
    off: { rsiLen:14, bbLen:12, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:36, sellTh:35, tpMult:2.75, slMult:1.9 }
  }
};
...
function _getPresetSourceLabel() {
  ...
  // 옵티마이저 대표 프리셋 확인 (저장 스키마는 시장 키로 통일)
  // [S176-fix2] V4 키로 수정 — V3는 _runS211Migration에서 삭제되는 레거시
  try {
    ...
    if (bucket && bucket.representId && bucket.ranks) {
      ...
      if (rep) {
        ...
        // [S177] regime 비교 버그 수정
        //   기존: `rep.regime ? '⚡ON' : '⚡OFF'` — JS truthy 평가는
        //         문자열 'OFF'를 truthy로 간주하므로 항상 ⚡ON 반환되는 버그
        //   원인: 옵티마이저는 entry.regime을 문자열('ON'/'OFF')로 저장
        //         (sx_optimizer.js 3432줄, 3624줄 등 일관)
        //   해결: 옵티마이저와 동일한 방식으로 명시적 비교
        const regOn = (rep.regime === 'ON' || rep.regime === true);
...
//
//   [폴백 체인]
//     ① 매트릭스 슬롯 (사용자/옵티마이저 저장값)
//     ② SCR_ANAL_MARKET_REGIME_DEFAULTS 하드코딩 (시장×레짐 실측값)
//     ③ SCR_ANAL_DEFAULTS (최종 폴백)
//
//   [C안 마이그레이션] — 레거시 키 1회 자동 삭제
//     · SX_SCR_ANAL_PARAMS_MATRIX (v1, 모드 기반) 삭제
//     · SX_SCR_ANAL_PARAMS (단일 키) 삭제
//     · SX_SCR_ANAL_PARAMS_KR/US/COIN (시장별 멀티슬롯) 삭제
//     · SX_OPT_BEST3 (옵티마이저 대표 프리셋, 모드 키) 삭제
//   사용자에게는 "시장별 옵티마이저 재실행 권장" 안내.
// ════════════════════════════════════════════════════════════
const SCR_ANAL_PARAMS_MATRIX_KEY = 'SX_SCR_ANAL_PARAMS_MATRIX_V2'; // S211: v2 (시장 기준)
const SCR_ANAL_PARAMS_LEGACY_MATRIX_KEY = 'SX_SCR_ANAL_PARAMS_MATRIX'; // 레거시 (모드 기준)
const S211_MIGRATION_DONE_KEY = 'SX_S211_MIGRATED'; // 1회성 마이그레이션 플래그
...
// S211: C안 1회성 레거시 정리 — 깨끗이 시작
//   호출 시점: _loadAnalParams() 첫 호출 시 자동 트리거.
//   사용자가 신규 환경에 진입하면 자동으로 정리되고 안내.
let _s211MigrationChecked = false;
function _runS211Migration(){
  if(_s211MigrationChecked) return;
  _s211MigrationChecked = true;
  try {
    if(localStorage.getItem(S211_MIGRATION_DONE_KEY) === '1') return; // 이미 처리됨

    const removedKeys = [];
    const keysToRemove = [
      SCR_ANAL_PARAMS_LEGACY_MATRIX_KEY,        // 모드 기반 매트릭스
      SCR_ANAL_PARAMS_KEY,                      // 단일 키
      'SX_SCR_ANAL_PARAMS_KR',                  // 시장별 멀티슬롯
      'SX_SCR_ANAL_PARAMS_US',
      'SX_SCR_ANAL_PARAMS_COIN',
      'SX_OPT_BEST3'                            // 모드 키 기반 대표 프리셋
    ];
    ...
    localStorage.setItem(S211_MIGRATION_DONE_KEY, '1');

    if(removedKeys.length > 0){
      console.log('[S211] 시장별 개편 마이그레이션 완료 — 레거시 키 삭제:', removedKeys);
      // SXE 객체에 플래그 노출 → UI에서 토스트 안내 가능
      if(typeof SXE !== 'undefined'){
        SXE._s211MigrationOccurred = true;
      }
    }
  } catch(_){}
}
...
// [save-safe v1] 매트릭스 저장 — 안전 저장 + round-trip 검증
//   기존 silent catch 문제: localStorage 용량 부족/프라이빗 모드/3rd-party 차단 등으로 실패해도
//                          호출자가 모르고, 사용자에겐 "✅ 저장됨" 토스트만 떠서 미스매치 발생.
//   해결: ① _sxSafeSetItem (메인 스레드 전역 정의) 우선 사용 — QuotaExceeded 시 자동 청소+재시도
//        ② 직후 round-trip 검증 (실제로 다시 읽어서 buyTh 일치 확인)
//        ③ 실패 시 콘솔 + 반환값으로 호출자에게 보고 → 사용자 토스트 가능
//   반환: true (성공) | false (실패)
function _saveParamsMatrix(matrix){
  const payload = JSON.stringify(matrix || {});
  // 메인 스레드: 전역 _sxSafeSetItem이 있으면 그걸로 안전 저장 (자동 청소)
  let ok = false;
  try {
    if(typeof _sxSafeSetItem === 'function'){
      ok = _sxSafeSetItem(SCR_ANAL_PARAMS_MATRIX_KEY, payload);
    } else {
      // 워커 환경 또는 헬퍼 미로드: raw setItem (try-catch만)
      try { localStorage.setItem(SCR_ANAL_PARAMS_MATRIX_KEY, payload); ok = true; }
      catch(e){ console.warn('[save-safe] _saveParamsMatrix raw err', e); ok = false; }
    }
  } catch(e){
    console.warn('[save-safe] _saveParamsMatrix outer err', e);
    ok = false;
  }
  return ok;
}
...
//   [save-safe v1] 반환: true (성공·검증완료) | false (실패)
//     ① 매트릭스 갱신 → 저장
//     ② round-trip 검증: localStorage에서 다시 읽어 buyTh 일치 확인
//     ③ 실패 시 호출자에게 false 반환 → 사용자 알림 가능
function _saveSlotParams(marketOrMode, regimeOn, params){
  ...
  const saved = _saveParamsMatrix(matrix);
  if(!saved){
    console.warn(`[save-safe] _saveSlotParams 저장 실패: ${market}.${regKey}`);
    return false;
  }
  // round-trip 검증: 실제로 저장됐는지 다시 읽어서 확인
  //   목적: 메모리 객체 변경은 OK였지만 localStorage 쓰기가 silent fail 한 경우 캐치
  //         _sxSafeSetItem 자체의 검증이 정확하면 이 단계는 보조 안전망
  try {
    const verify = _loadParamsMatrix();
    const vSlot = verify[market] && verify[market][regKey];
    if(!vSlot || vSlot.buyTh !== params.buyTh){
      console.warn(`[save-safe] _saveSlotParams round-trip 검증 실패: ${market}.${regKey} (저장 후 재조회 시 불일치)`);
      return false;
    }
  } catch(e){
    console.warn('[save-safe] _saveSlotParams 검증 중 예외', e);
    // 검증 자체가 예외면 보수적으로 false (사용자에게 재시도 권장)
    return false;
  }
  return true;
}
...
// S211: 레거시 마이그레이션 → 자동 정리(C안)로 대체
//   기존 _migrateLegacyAnalParams() 함수는 _runS211Migration()으로 통합.
//   _loadAnalParams 첫 호출 시 자동 트리거.

function _loadAnalParams() {
  // S211: 레거시 정리 (C안 — 깨끗이 시작)
  _runS211Migration();
...
function _resetAllAnalParams() {
  try {
    ...
    // 레거시 키도 같이 정리 (혹시 남아있을 수 있음)
    localStorage.removeItem(SCR_ANAL_PARAMS_KEY);
    localStorage.removeItem(SCR_ANAL_PARAMS_LEGACY_MATRIX_KEY);
...
const MACD = {
  calc(closes, fast = 12, slow = 26, sig = 9) {
    ...
    //   기존: 초기 구간을 0으로 채워서 emaArray(line, sig)의 SMA 초기값이 0에 의해 왜곡됨
    //   수정: line의 valid 부분만 잘라 emaArray에 넣고, 결과를 원래 인덱스에 매핑
    let firstValid = -1;
...
SXE.sanitizeRows = function(rows){
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (c > h || c < l || o > h || o < l) { _needSanitize = true; break; }
  }
  ...
  return rows.map(r => {
    ...
    if (h >= maxV && l <= minV) return r; // 정합성 OK
    return Object.assign({}, r, { high: maxV, low: minV });
  });
};
...
SXE._btEarlyExit = (function(){
  ...
  return {
    ...
    time:      false,  // maxBars 보유 + 수익률 < timeMinPnl%
    // 파라미터
    trailMult: 1.0,    // 트레일링 거리 (×ATR)
    trailMode: 'profit', // 'immediate' = 진입 즉시 트레일링 / 'profit' = 수익권 진입 후 작동
    trailTrigger: 1.0, // trailMode='profit'일 때 발동 기준 (×ATR) — 피크가 entry+N×ATR 도달 후 트레일링 시작
    beTrigger: 1.0,    // 본전 보호 발동 기준 (×ATR)
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  // S126: 레짐 보정 적용 플래그 (옵티마이저 전용 경로)
  //   · 실시간 분석/BT(분석탭 엔진판단 검증, 단일검증 백테스트)는 false 기본 → 슬롯값 그대로 사용 (이중 보정 방지)
  //   · 옵티마이저는 "슬롯에 저장될 최적 기저값(base)"을 탐색하는 목적이므로 true 전달
  //     → BT 내부가 레짐 감지 시 기저값에 보정을 더해 시뮬레이션
  //     → 옵티마이저가 저장하는 건 "보정 전 기저값" (슬롯에 저장 후 실시간은 그 값을 그대로 씀)
  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    // S125/S126: 봉별 임계값 초기화 (기저값) + 옵티마이저 경로일 때만 레짐 보정 적용
    //   · 실시간 분석/BT: _doRegimeAdj=false → 슬롯 저장값(base) 그대로 사용
    //   · 옵티마이저: _doRegimeAdj=true → 봉별 ind.regime 감지해 buyTh/sellTh/tpMult/slMult 보정
    //     이렇게 해야 ON 라운드와 OFF 라운드가 실제로 다른 시뮬레이션 결과를 내서 2라운드 탐색이 의미를 가짐.
    //   · clamp 범위: S125 이전 40~85 → S126 20~85로 완화. 옵티마이저가 20대 buyTh를 찾으므로 하한 40은 너무 높음.
  ...
  //   기존: avgWin / avgLoss → 거래 횟수가 다르면 의미 왜곡 (예: 승10회 평균2%, 패5회 평균3%일 때
  //         표준 PF=20/15=1.33인데 기존 방식은 0.67로 정반대 신호)
  //   수정: totalWin / totalLoss
