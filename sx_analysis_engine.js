...
function scrQuickScore(rows, tf) {
  ...
  // S71: 레짐 적응형 임계값
  // S125 변경: buyTh/sellTh에 ra.buyThAdj/sellThAdj 가감 보정 제거.
  //   [이유] 이제 _loadAnalParams()가 현재 모드×레짐 좌표의 "이미 레짐 반영된 슬롯값"을 반환하므로
  //          여기에 또 보정을 더하면 이중 보정이 됨. 레짐은 "감지"와 "슬롯 선택"까지만 담당.
  //   [유지] _regimeAdapt는 UI 표시/해석용으로 계산만 수행 (sx_render.js의 레짐 상세 카드가 사용).
  const th = _getEffectiveTh(tf);
  let buyTh = th.buyTh, sellTh = th.sellTh;
  let _regimeAdapt = null;
  if(SXE.regimeAdaptEnabled() && ind.regime){
    _regimeAdapt = SXE.regimeAdapt(ind.regime); // UI 표시용만, buyTh/sellTh에는 미적용
  }
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  // S71/S125/S126: 레짐 보정은 applyRegimeAdjust 플래그가 true이고 레짐 적응이 ON일 때만 적용
  const _raEnabled = SXE.regimeAdaptEnabled();
  const _doRegimeAdj = applyRegimeAdjust && _raEnabled;
