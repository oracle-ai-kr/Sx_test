...
function scrQuickScore(rows, tf, market) {
  ...
  // [S361→S362] C 캡 전용 위반 전수 수집 — reasons/action 로직과 독립 (조건검색 _safety_clean 카운트 불변).
...
SXE._checkSafetyFilters = function(ind, rawScore, buyTh, volSoft, tf){
  ...
  // [S454] 되돌림주의 — ind.candles로 dumpWarn 계산(changeRate 유도·market 미상=비코인 기본, graceful)
  if(_sf.dumpWarn && typeof SXE.calcDumpWarn === 'function' && ind.candles){
    try { const _c=ind.candles, _l=_c[_c.length-1], _p=_c[_c.length-2]; const _cr=(_p&&_p.close)?((_l.close/_p.close)-1)*100:0;
