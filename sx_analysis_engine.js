...
SXE._checkSafetyFilters = function(ind, rawScore, buyTh, volSoft, tf, candles, opts){
  ...
  const _sf = SXE._safetyFlags || {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,macdNeg:true,bbUpper:true,resistNear:true,fakeBreakout:true,volResist:true,chaseGuard:true,dumpWarn:true,supportBreak:true,debtRatio:true,foreignSell:true,highBeta:true,riskSignature:false};
  // [S1016] A그룹(stochRsi/ma60resist/deadCrossGuard) 안전필터 철거 — S1013 최종심.
  ...
  // [S662] kNN 음봉전이 BT 게이트 — S658(분석탭+스캐너)에서 실시간으로 추가한 것을 BT 진입에도 반영.
  //   설정탭 "kNN 음봉전이 경고 [실험]" 체크박스(SXE._safetyFlags.knnBearish)가 ON일 때만 작동.
  //   BT slice(진입 시점까지 자른 캔들)가 candles 인자로 전달되므로 룩어헤드 없음.
  //   SXKNN 미로드·candles 부족(56봉 미만)이면 자동 통과 — BT 호환성 우선.
  if(_sf.knnBearish && Array.isArray(candles) && candles.length >= 56){
    try {
      const _kn = (typeof SXKNN !== 'undefined' && SXKNN.score) ? SXKNN.score(candles, { win: 16, k: 10 }) : null;
      if(_kn && _kn.active && typeof _kn.score === 'number' && _kn.score <= -30) return { pass: false, reason: '🔒kNN음봉전이' };
    } catch(_eKnn){}
  }
...
// [S682] 순수 MA크로스 모드 (분석 BT) — ON 시 sxRunBtEngine이 5×9 골든크로스(점수≥buyTh)로 진입 → 데드크로스 OR ATR×2(SL) 청산.
//   TP·sellTh점수청산·재진입 OFF, 시장별 고정 임계(SCR_ANAL_MARKET_DEFAULTS). 옵티마이저/슬롯 모드와 격리(_maCrossPure 플래그). 기본 OFF.
SXE._btMaCross = (function(){
  try {
    const v = localStorage.getItem('SX_BT_MACROSS');
    if(v === 'on')  return true;
    if(v === 'off') return false;
  } catch(_) {}
  return false;  // 기본 OFF (기존 TP/SL 유지)
})();

SXE._btMaCrossSave = function(){
  try { localStorage.setItem('SX_BT_MACROSS', SXE._btMaCross ? 'on' : 'off'); } catch(_) {}
};

SXE.setBtMaCross = function(on){
  SXE._btMaCross = !!on;
  SXE._btMaCrossSave();
  console.log(`[setBtMaCross] 순수 MA크로스 모드: ${SXE._btMaCross ? 'ON (5×9 골든+점수≥buyTh 진입 / 데드크로스 OR ATR×2 청산)' : 'OFF (기존 TP/SL)'}`);
};

// [S684] MA크로스 모드 선택 옵션 — 기본 둘 다 OFF(= 순수 골든→데드크로스, 검증 단기추세와 동일).
//   _btMaCrossScoreGate: ON 시 골든진입에 점수≥buyTh 게이트 추가 (너무 낮은 점수 거름).
//   _btMaCrossSL:        ON 시 ATR×2 재앙손절 활성 (OFF면 데드크로스 청산만, 손절 없음).
SXE._btMaCrossScoreGate = (function(){
  try { const v = localStorage.getItem('SX_BT_MACROSS_GATE'); if(v==='on') return true; if(v==='off') return false; } catch(_){}
  return false;
})();
SXE._btMaCrossSL = (function(){
  try { const v = localStorage.getItem('SX_BT_MACROSS_SL'); if(v==='on') return true; if(v==='off') return false; } catch(_){}
  return false;
})();
SXE.setBtMaCrossScoreGate = function(on){
  SXE._btMaCrossScoreGate = !!on;
  try { localStorage.setItem('SX_BT_MACROSS_GATE', SXE._btMaCrossScoreGate ? 'on' : 'off'); } catch(_){}
  console.log(`[setBtMaCrossScoreGate] MA크로스 점수 게이트: ${SXE._btMaCrossScoreGate ? 'ON (점수<buyTh 골든 거름)' : 'OFF (순수 골든)'}`);
};
SXE.setBtMaCrossSL = function(on){
  SXE._btMaCrossSL = !!on;
  try { localStorage.setItem('SX_BT_MACROSS_SL', SXE._btMaCrossSL ? 'on' : 'off'); } catch(_){}
  console.log(`[setBtMaCrossSL] MA크로스 ATR×2 재앙손절: ${SXE._btMaCrossSL ? 'ON' : 'OFF (데드크로스 청산만)'}`);
};
...
const _MACROSS_PAIR = [5, 9];  // [S682] 순수 MA크로스 모드 검증쌍 (단기추세 5×9 골든/데드크로스)
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  // [S682] 순수 MA크로스 모드 — SXE._btMaCross ON 시: MA쌍 5×9 강제 + 점수게이트 골든진입 + TP제거/SL=ATR×2 + sellTh청산 제거 + 시장별 고정 임계.
  //   기존 _gatePair 모드(옵티마이저 _btModeOverride / 슬롯 _mode)와 완전 격리 — _maCrossPure 플래그로만 동작 변경, OFF면 영향 0.
  const _maCrossPure = !!(typeof SXE !== 'undefined' && SXE._btMaCross);
  const _maCrossScoreGate = _maCrossPure && !!(typeof SXE !== 'undefined' && SXE._btMaCrossScoreGate);  // [S684] 점수 게이트 토글 (OFF=순수 골든)
  const _maCrossSL = _maCrossPure && !!(typeof SXE !== 'undefined' && SXE._btMaCrossSL);                 // [S684] ATR×2 재앙손절 토글 (OFF=손절 없음)
  let _gatePair = _gateMode ? _MODE_MA_PAIR[_gateMode] : null;
  let _macrossBuyTh = baseBuyTh, _macrossSellTh = baseSellTh;
  if(_maCrossPure){
    _gatePair = _MACROSS_PAIR;  // 5×9 검증쌍 강제 (드롭다운/옵티마이저 무관)
    const _mcMkt = (opts && opts.market) || (typeof currentMarket !== 'undefined' ? currentMarket : null) || 'kr';
    const _mcDef = SCR_ANAL_MARKET_DEFAULTS[_mcMkt] || SCR_ANAL_MARKET_DEFAULTS.kr;
    _macrossBuyTh = _mcDef.buyTh; _macrossSellTh = _mcDef.sellTh;  // 시장별 고정 (저장 프리셋 무시)
  }
  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    // [S682] 순수 MA크로스: 시장별 고정 임계 + ATR×2 disaster SL (레짐보정/TP 무관 — 진입 pos.sl에 slMult=2 반영됨)
    if(_maCrossPure){ buyTh = _macrossBuyTh; sellTh = _macrossSellTh; if(_maCrossSL) slMult = 2; }  // [S684] SL 토글 ON일 때만 ATR×2 (OFF면 SL 비활성 → 아래 _slHit 차단)
    ...
    if (!pos && !pendingSignal) {
      ...
      if (_gatePair) {
        ...
        if (_goldenCross && (!_maCrossScoreGate || rawScore >= buyTh)) {
          _entryAllowed = true; _entryType = 'golden';            // [S684] 점수게이트 토글 ON이면 점수≥buyTh 필요 / OFF면 순수 골든(점수 무관)
        } else if (!_maCrossPure && _alignedNow && rawScore >= buyTh && _scrMomOscPass(mom, osc, tf) && (i - _lastExitIdx > 1)) {
      ...
      // [S728] BT 약세 데드캣 게이트 (flag default OFF) — 역배열 ltAlign + 단기약세(MA5>20>60 미충족) 진입 차단. C S726의 BT 전파.
      //   기본 OFF=바이트동일. _gateBlocks/_gateReasons 재사용(기존 차단 UI에 '🐻약세데드캣'으로 노출). 단봉 ind 기반.
      let _bearBlock728 = false;
      if(SXE._btBearGate && _entryAllowed){
        // [S731] ★버그수정: maAlignLT(MA60>120>200)는 200봉+ 필요한데 BT 슬라이스(150봉)론 항상 gateOn:false='off'
        //   → 게이트가 'bear'를 절대 못 봐 영구 무력화(OFF=ON). 장기배열만 250봉 슬라이스로 별도 계산.
        //   단기배열(ind.maAlign.bullish=MA5>20>60)은 60봉이면 충분 → 150봉 ind 그대로 사용.
        let _lt728 = 'off';
        try {
          const _clLong728 = rows.slice(Math.max(0, i - 249), i + 1).map(function(r){ return r.close; });
          _lt728 = (typeof _ltAlignStr === 'function') ? _ltAlignStr(_maAlignLT(_clLong728, tf)) : 'off';
        } catch(_e728){}
        if(_lt728 === 'bear' && !(ind.maAlign && ind.maAlign.bullish)){
          _bearBlock728 = true; _gateBlocks++; _gateReasons['🐻약세데드캣'] = (_gateReasons['🐻약세데드캣'] || 0) + 1;
        }
      }
      if (_entryAllowed && !_bearBlock728) {
      ...
      if (_gatePair) {
        ...
        } else if (!_maCrossPure) {  // [S682] 순수모드: sellTh-점수 청산 제거 → 데드크로스 OR ATR×2(SL)만
      ...
      const _slHit = (_exitClose ? (c <= pos.sl) : (l <= pos.sl)) && !(_maCrossPure && !_maCrossSL);  // [S684] 순수모드+손절OFF면 SL 비활성 (데드크로스 청산만)
      const _tpHit = (_exitClose ? (c >= pos.tp) : (h >= pos.tp)) && !_maCrossPure;  // [S682] 순수모드: TP 제거 (데드크로스 OR ATR×2만)
      if (_slHit) {
        ...
        if(!_earlyExit && (!_gatePair || _maCrossPure) && ee.score && rawScore <= sellTh){  // [S683] 순수 MA모드에서도 점수청산 토글 허용 (안전망 — 즉시청산, 진동카운터 미적용)
          _earlyExit = 'SCORE';
        }
