...
// ══════════════════════════════════════════════════════════════
//  ⚠★[S1374] **모듈 스코프로 이동.** 원래 `calcAllScreener` 안에 있었는데
//    `SXE.calcAxisScore`(파일 아래)가 `_axisQ`를 못 봐서 **rs 병합이 조용히 건너뛰어졌다**
//    (`typeof _axisQ === 'function'` 이 false). 배터리 C군이 잡았다 — S1265 계열(오프라인 통과 ≠ 배선 확인).
//    ⇒ 밖으로 빼면 ①`calcAxisScore`가 쓸 수 있고 ②**매 호출마다 객체가 재생성되던 낭비**도 없어진다.
//  [S1373] SPEC_S1371 ②단계 — **5분위 경계 각인 + 분위 매핑.**
//   ★재현 게이트 통과(4/4 전건 정확 일치): 같은 파이프로 S1257 `conv`·S1259 `disp20`의 kr·us 경계를
//     경계값·표본수까지 재현했다(kr 62,980 / us 32,467). ⇒ **이 파이프가 그 측정과 같다**는 증명이며,
//     그래야 신규 15종 경계를 믿을 수 있다.
//   프레임: `START=250 · OUT=15` · **전수(stride 1)** · 본 스냅 · **빈티지 2026-07-01 (3시장 정렬)**
//     kr `snap_kr.json` 188종 62,980 / us `snap_us.json` 97종 32,467 / coin `snap_coin_union_vin_20260701.json` 109종 36,469 · err 0
//   ⚠★**coin 스냅 교체** — 기존 `snap_coin.json`은 **450봉·baseDate 08-06**이라 kr·us(600봉·07-01)와 어긋나 있었다.
//     표본 수 산술로 증명됐다(111종×185=20,535 = S1257 기록과 정확히 일치). 교체 후 36,469(1.78배)·기준일 정렬.
//     그 결과 coin 경계가 이동했다: `conv` [1.76,2.94,4.17,5.63]→[1.3,2.31,3.64,5.55] · `disp20` [-8.9,-4.7,-1.6,2.1]→[-9.3,-4.8,-1.4,2.9].
//     ⇒ **기존 coin 경계 폐기**(지우지 않고 여기 남긴다·S1283). kr·us 경계는 재현됐으므로 그대로다.
//
//  ★★**연속 12종은 5분위, 이산 6종은 직접 매핑**으로 나눈다.
//   이유(실측): `maAlign`·`swing`·`obv`·`ad`는 값이 **−1/0/1 셋뿐**이라 5분위 경계가 중복되고
//   (`flow.obv` kr = `[-1,1,1,1]`) 구역이 **2~3칸 빈다**(kr obv는 Q1·Q2만 쓰고 Q3~Q5가 0건).
//   `ichimoku`도 조건 5개가 함께 켜지고 꺼져 **±9에 몰린다**(kr +9 32.3% · coin −9 47.7%) — kr Q5가 637건(1%), coin Q2가 0건.
//   ⇒ 이산에 분위를 씌우면 **구역이 붕괴한다.** 직접 매핑이 옳다.
//   ⚠이산 6종은 **시장별 경계가 없다.** SPEC §6의 '시장별 분위'와 어긋나 보이나 어긋나지 않는다 —
//     이산값은 이미 절대적 의미(정배열/역배열·구름 위/아래)를 갖고 분포로 정할 것이 없다.
//     단 그 결과 **축 평균이 시장별로 치우칠 수 있다**(coin은 `maAlign` 중위가 −1). 해석 시 유의.
const _AXIS_EDGES = {
  //  연속 12종 — 시장별 5분위 경계(20/40/60/80 백분위 · 선형보간 · `dec`자리 반올림 · 우폐구간).
  //  구역 균형 실측: 최대 구역비 20.1~20.9%(거의 균등). `rel.psycho`만 34.0% — 12봉 이산이라 값이 13종뿐이다.
  'pos.disp20': { dec:1, kr:[-4.6, -0.7, 3.2, 9.2], us:[-3.6, -0.7, 1.7, 5], coin:[-9.3, -4.8, -1.4, 2.9] },
  'pos.disp60': { dec:1, kr:[-4.8, 1.9, 9.1, 21.5], us:[-6.3, -1.1, 3.8, 10.2], coin:[-20.3, -11.1, -4.2, 3] },
  'pos.struct20': { dec:1, kr:[20.2, 41, 62.5, 81.6], us:[21.3, 43.9, 67.3, 86.5], coin:[12.1, 22.3, 35.2, 55.6] },
  'pos.struct60': { dec:1, kr:[27.2, 48.2, 65.8, 82.3], us:[20.7, 43.3, 68.9, 87.4], coin:[8.9, 16.5, 27.3, 47.1] },
  'pos.vwapGap': { dec:2, kr:[-5.73, -1.55, 2.15, 7.35], us:[-3.42, -0.56, 1.84, 5.15], coin:[-11.02, -6.49, -2.85, 1.2] },
  'pos.pivotPos': { dec:1, kr:[31.2, 44.4, 54.9, 70.7], us:[31.1, 45.7, 56.8, 71.2], coin:[31.7, 43.1, 52.2, 64] },
  'vol.atrPct': { dec:2, kr:[3.37, 4.34, 5.35, 6.83], us:[2.1, 2.61, 3.24, 4.34], coin:[4.78, 5.86, 6.96, 8.45] },
  'vol.bbWidth': { dec:2, kr:[9.32, 13.54, 18.84, 27.84], us:[7.69, 10.92, 15.36, 23.28], coin:[8.56, 12.04, 16.22, 23.3] },
  'trend.pct20': { dec:1, kr:[-7.1, -0.3, 6.8, 18.9], us:[-6.3, -1.1, 3.2, 9.6], coin:[-16.4, -8.5, -2.3, 5.3] },
  'trend.adx': { dec:1, kr:[16.9, 21.8, 27.9, 37.2], us:[16.2, 20.4, 25.3, 32.1], coin:[17.5, 22.7, 28.6, 36.3] },
  'trend.conv': { dec:2, kr:[1.44, 2.24, 3.09, 4.17], us:[1.54, 2.39, 3.37, 4.76], coin:[1.3, 2.31, 3.64, 5.55] },
  'flow.volRatio': { dec:2, kr:[0.53, 0.72, 0.94, 1.31], us:[0.71, 0.85, 0.99, 1.24], coin:[0.35, 0.57, 0.82, 1.25] },
  //  [S1374] 상대강도 — 지수 스냅(`snap_index_vin_20260701` · *'RS SSOT · _fetchIndexCloses 심볼과 동일'*) 기준.
  //    지수 매칭은 실코드와 동일: us `^GSPC` · coin `KRW-BTC` · kr `^KS11`(코스닥이면 `^KQ11`).
  //    ⚠**날짜로 맞췄다** — 종목 봉 인덱스로 지수를 찾으면 휴장일 차이로 어긋난다.
  //    표본이 다른 재료와 정확히 일치(kr 62,980 / us 32,467 / coin 36,469) · 구역 20.0±0.4%로 균등.
  //    ⚠**kr 코스닥 판별이 스냅에서 불가능했다** — `snap_kr.json` 종목 필드가 `{name, rows}`뿐이라
  //      `market`이 없어 **188종 전부 `^KS11`로 산출**됐다. 실기기는 `OracleData`로 코스닥을 가르므로
  //      코스닥 종목의 `rs`는 이 경계와 기준 지수가 다르다. ⇒ 백로그(스냅에 `market` 필드 추가 후 재산출).
  'rel.rs20': { dec:1, kr:[-13.2, -5.8, 0.6, 11.3], us:[-7.4, -2.4, 2, 8.2], coin:[-11.6, -6.1, -1.3, 4.8] },
  'rel.rs60': { dec:1, kr:[-23.7, -10.2, 2.9, 26.1], us:[-13.8, -4.7, 3.9, 15.6], coin:[-21.8, -13.3, -6.4, 3.1] },
  //  ⚠[S1373] `rel.psycho` 는 여기서 **뺐다** — 12봉 이산이라 균등 5분할이 불가능했다(아래 `_AXIS_DISCRETE`).
  //    폐기된 경계(기록): kr `[33.3,41.7,50,58.3]` · us `[41.7,50,58.3,66.7]` · coin `[33.3,41.7,50,58.3]`
  //    그 경계의 실측 구역: us `34.5/23.2/21.0/12.7/8.6` — 최빈값 `50`이 경계에도 들어가 더 나빠졌다.
};
//  이산 6종 — 직접 매핑(시장 공통). 값이 곧 방향이라 분포로 정할 것이 없다.
//  ⚠`ichimoku`의 `±4` 경계는 임의값이 아니다 — `score`에서 **'가격 대 구름' 항이 ±4**이므로
//    `≤−4 / −3~3 / ≥4` 가 곧 **구름 아래 / 안 / 위**다. 화면 `고급 분석`의 `구름 아래(음운)` 표기와 같은 분류다.
const _AXIS_DISCRETE = {
  'trend.maAlign': v => v > 0 ? 90 : (v < 0 ? 10 : 50),   // 정배열 / 혼조 / 역배열
  'trend.swing':   v => v > 0 ? 90 : (v < 0 ? 10 : 50),   // HH / 혼조 / LL
  'trend.ichimoku':v => v >= 4 ? 90 : (v <= -4 ? 10 : 50),// 구름 위 / 안 / 아래
  'flow.obv':      v => v > 0 ? 90 : (v < 0 ? 10 : 50),
  'flow.ad':       v => v > 0 ? 90 : (v < 0 ? 10 : 50),
  //  [S1373] 심리도 — `Psycho.zone`의 교과서 문턱을 5단계로. 12봉 중 상승일 비율(값 간격 8.3).
  'rel.psycho':    v => v >= 75 ? 90 : (v >= 60 ? 70 : (v <= 25 ? 10 : (v <= 40 ? 30 : 50))),
};
//  원값 → 5단계 분위값(10/30/50/70/90). 경계 없거나 값 없으면 `null`(축 평균에서 빠진다).
//  ⚠**계단이지 연속 백분위가 아니다**(SPEC §5 정정) — 기존 `disp`/`slope`/`conv` 관용구와 같은 형태이며,
//    경계값만 각인하면 되어 산출물이 작고, 과도한 정밀도 주장을 피한다.
function _axisQ(key, raw, mkt){
  if(raw == null || !isFinite(raw)) return null;
  const D = _AXIS_DISCRETE[key];
  if(D) return D(raw);
  const E = _AXIS_EDGES[key]; if(!E) return null;
  const ed = E[mkt] || E.kr; if(!ed) return null;
  const m = Math.pow(10, E.dec), v = Math.round(raw * m) / m;
  return 10 + ((v > ed[0] ? 1 : 0) + (v > ed[1] ? 1 : 0) + (v > ed[2] ? 1 : 0) + (v > ed[3] ? 1 : 0)) * 20;
}
...
function calcAllScreener(rows, tf) {
  ...
  const _axisRaw = (function(){
    ...
    try{
      ...
      //  ★[S1373] 심리도 — **이산 취급**(`_AXIS_DISCRETE`)이고 창은 **12봉 그대로**다.
      //    ⚠20봉 판본을 만들어 봤으나 **소용이 없었다**(실측): 20봉이어도 값 간격이 5라
      //      경계 `[45,50,55,60]`가 값 위에 정확히 놓이고, 분포가 중앙에 몰려 있어
      //      최대 구역이 us 33.8% · coin 34.6%로 **12봉(us 34.5%)과 거의 같았다.**
      //      ⇒ **이산값에 5분위를 씌우는 한 창을 늘려도 균등 분할은 불가능하다.**
      //    ⇒ `Psycho.zone`의 **교과서 문턱**(`≥75 과매수 / ≥60 강세 / 40~60 중립 / ≤40 약세 / ≤25 과매도`)을
      //      5단계로 그대로 쓴다. 임의값이 아니라 심리선의 표준 문턱이다.
      //    ⇒ 창을 12봉으로 되돌리니 **기존 도넛 `심리도`와 값이 같아져** '창 다른 두 심리도 공존' 문제도 사라졌다.
      //      SPEC §12 열린항목 1(창 불일치)은 **분위를 안 쓰게 되면서 무의미해졌다** — 분류는 창과 무관하게 같은 문턱을 쓴다.
    ...
    //  [S1373] 분위값 부착 — `R.q[축][재료] = 10/30/50/70/90`. 원값(`R[축][재료]`)은 그대로 둔다.
    //    ⚠**원값 병기 필수**(SPEC §6) — 분위만 두면 하락장에서 '상위 10%'가 '괜찮다'로 오독된다.
    const _mk1373 = (typeof currentMarket !== 'undefined' && currentMarket) ? currentMarket : 'kr';
    R.q = { pos:{}, vol:{}, trend:{}, flow:{}, rel:{} };
    for(const _a of ['pos','vol','trend','flow','rel']){
      for(const _f in R[_a]){
        const _v = _axisQ(_a + '.' + _f, R[_a][_f], _mk1373);
        if(_v != null) R.q[_a][_f] = _v;
      }
    }
    R._mkt = _mk1373;
    R._vintage = '2026-07-01';   // [S1373] 경계 빈티지 — SPEC §6 명시 의무
    ...
    R._nq = { pos:Object.keys(R.q.pos).length, vol:Object.keys(R.q.vol).length, trend:Object.keys(R.q.trend).length,
              flow:Object.keys(R.q.flow).length, rel:Object.keys(R.q.rel).length };
    return R;
  })();
...
// ════════════════════════════════════════════════════════════
//  ══════ [S1368] rawScore 점수 체계 **감사** 각인 — 코드 동작 변경 0 ══════
//   프레임: OOS 스냅 3종(kr181·us196·coin77) · START250 · **stride 12** · 봉 5,427/5,878/2,303 · err 0
//   재구현 0 — `scrQuickScore`를 태워 `_breakdown` 수확 · `s` 6항 분해는 잔차 ±18 검산으로 정합 증명.
//   산출물 `S1368_rawScore_audit.md` · `S1368_raw_{kr,us,coin}.json`
//
//  ⚠★**`volSoft`는 volume이 아니라 volatility다.** `volSoft = ATR.soften(ind.atr.pct, null, null)`이고
//    `cache`가 null이라 `soften`은 **그냥 통과**한다(S254가 종목 간 누수 차단으로 캐시 제거 → 무동작 통과기).
//    즉 `volSoft = ATR%` 그대로다. 이름이 `vol`이라 거래량으로 읽힌다 — S1300·S1322·S1327·S1365 **이름 충돌 계열**이고
//    이 감사 작성자도 처음에 거래량으로 잘못 읽었다.
//
//  ★★실측 — 6항이 선언대로 작동하지 않는다 (|중위| = 실제 기여 · kr/us/coin):
//    ①`추세%`   |중위| **28 / 19.96 / 28** · 포화 **60.4 / 35.8 / 61.9%** ⇒ |중위|가 상한 그 자체다.
//       `trend.pct×4.0`이라 ±7%만 넘으면 붙고 이 풀은 대부분 넘는다 ⇒ **크기가 아니라 `+28/중간/−28` 3값에 가깝다.**
//    ②`추세기울기` |중위| **0.13 / 0.08 / 0.15** — 선언 상한 ±18의 **0.4~0.8%**만 쓴다. 포화 0%.
//       ⇒ **점수를 움직이지 않는다.** 계수 `18.0`이 이 지표 단위와 안 맞는다(62/38·±38과 같은 스케일 불일치 계열).
//    ③`변동성`  |중위| **17.78 / 12.91 / 17.92** · 전부 **하한** 포화(33.6/5.1/42.6%) ⇒ 변별이 아니라 **상수 감점**.
//       문턱 `ATR%=1.6`인데 풀 중위는 us≈2.9 / kr≈4.6 / coin≈6.9다. 곡선: 1.6→0 · 2.9→−14.8 · 4.6→−17.8 · 6.9+→−18.0(포화).
//       ⇒ 기준점을 50에서 **32 근처로 내리는 오프셋**이고, ★**`buyTh 30`이 '낮아 보이는' 이유가 이것이다**(실측 rawScore 중위 kr37/us40/coin25).
//       ⚠원장에서 `변동성`은 **kr `pass`(강)**(S1349 [A]군 1번·cGap +29.5) — **가장 강하게 측정된 재료가 여기서는 포화돼 변별을 못 한다.**
//         S1337이 `bb.width` 문턱 5곳에서 찾은 것과 **같은 구조**다.
//    ④실측 기여 순위 = 추세%(28) > 변동성(17.8) > 구조위치(6.4~7.3) > **RSI(5.1~6.1)** > MACD(4.5~4.9) > 추세기울기(0.1)
//       ⚠선언 상한만 보면 RSI(±32)가 최대다 — **선언과 실측이 어긋난다. 상한으로 중요도를 읽지 말 것.**
//
//  ★보조 13종 — 발동률이 높으면 변별이 아니다(us): A/D **85.3%**(중위 +4) · OBV 74.1%(+5) · 캔들 71.3%(−5) ·
//    일목 **66.3%**(+3) · MA정배열 62.1%(+5) ⇒ 상수 가점에 가깝다. 반대로 거래량패턴 7.4% · OBV다이버전스 13.7%는 드물다.
//
//  ⚠**같은 재료 이중 계상 4건**: `RSI 다이버전스`가 **같은 조건문으로 두 번**(`s` 본문 ±8 + 보조 `rsiDiv` ±6×0.5 ⇒ 실효 ±11) ·
//    `A/D`(보조 ±4 + aux ±4) · `일목`(보조 ±3 + aux ±6) · `MACD`(s ±18 + aux 3봉 감쇠).
//    ⚠뒤 3건은 `signal`(방향)과 `score`(강도)로 **다른 양일 수 있어** 이중 계상이라 단정하지 않는다 — 확인은 별건.
//
//  ⚠**도달 불가 clamp 2건**: `aux` 최종 `clamp(±38)` 실측 범위 −21~+17 ⇒ **3시장 발동 0%** ·
//    그 앞 `clamp(aux,−16,16)`은 그 시점 aux가 **감점만 하는 구간**이라 **상한 16이 구조적으로 도달 불가**(실질 `max(aux,−16)`).
//  ⚠**하방 비대칭**: `combined` 하한 0 발동이 coin **10.86%**/kr 7.13%인데 상한 100은 0~0.02% ⇒ 그 봉들이 전부 `rawScore=9`로 뭉친다.
//    **나쁜 쪽의 구별이 사라진다.**
//  · tanh(1.15)는 거의 선형이다 — 실효는 양끝 당김뿐(combined 0→raw 9 · 100→91). **rawScore는 9~91이고 0·100에 도달하지 않는다.**
//
//  ★★원장 대조(이름 대응 · **같은 양 검증 아님**): 19항 중 원장 이름이 대응하는 9종 = `pass` 4 / `desc` 14 / `rev` 3.
//    나머지 **10종은 원장 미등재**(RSI·MACD·ADX·CCI·거래량OSC·일목·캔들·거래량패턴·TRIX·priceAction) — 즉 절반 이상이 **측정 이력 없음**.
//    ⚠**'rawScore가 무의미하다'로 읽지 말 것**: ㉠원장 판정은 그 프레임(FWD10·crash−10%·5분위)의 판정이고 rawScore는 진입 타이밍이라 **같은 질문이 아니다**
//      ㉡`SPEC_S1286` 조항1의 **같은 양 검증이 되어 있지 않다**(S1298이 이 계열 오매핑 4건을 뗀 전례 — 원장 `거래량`은 MFI+VR+OBV 합성이라 다른 양).
//
//  ⚠한계: stride 12 부표본 · OOS 스냅(본 스냅과 다른 표본이라 S1329·S1330 수치와 직접 비교 금지) ·
//    `변동성` 항은 잔차로 구했다(다른 항 오차가 흡수될 수 있다) · **성과를 재지 않았다** —
//    *'rawScore가 수익을 예측하는가'* 는 이 감사의 질문이 아니고 물으려면 **별도 사전등록**이 필요하다.
//  ⚠S1329 기록(`396종 중위 19·p75 34·p90 52`)과 내 실측(중위 kr37/us40/coin25)이 **크게 다르다**.
//    원인 후보 ㉠S1329는 스캔 시점(종목당 최신 1봉)/나는 전 구간 ㉡스냅 상이 ㉢시장 혼합. **원인 미규명으로 병기한다**(S1335).
...
// ══════════════════════════════════════════════════════════════
//  [S1374] SPEC_S1371 ③단계 — **축 집계.** `_axisRaw` → 5축 점수 → `rawScore`(상태 점수).
//   집계식(SPEC §5): `재료 → 시장별 5단계 분위 → 축 내부 단순평균 → 축 5개 단순평균`
//   ⚠**가중치 없음.** 가중을 두면 그 순간 임의값이 생기고, S1369가 `tpMult/slMult`를 철거한 논리
//     (*'임계값이 임의였고 종목마다 달랐다 — 측정 의미가 없다'*)에 그대로 걸린다.
//   ⚠**축 내부 평균 → 축끼리 평균**(전체 재료 평균이 아니다). 축마다 재료 수가 달라도
//     (자리 6 · 흔들림 2) **재료 수가 가중치가 되지 않는다.** 5축을 나눈 이유가 그것이다.
//   ⚠축의 재료가 하나도 산출되지 않으면 그 축은 **평균에서 제외**하고 `null`로 돌려준다(0으로 세지 않는다).
//
//  ★★**`excl` 필터를 쓰지 않는다** (SPEC §9-2 — 이 함수의 존재 이유):
//   기존 도넛 `_sxbAvg`는 측정 미달(`desc`)·역방향(`rev`) 재료를 평균에서 뺀다. 그 결과가
//   **삼성전자 `추세·구조 0`** 이다 — 도넛 셋(20/100/49) 중 `추세강도`는 kr `추세신호` desc라 `excl` 제외,
//   `상대강도`는 S1268 중립 전환으로 제외, 남은 `변동성`이 `inverse`라 `100−100=0`.
//   **화면은 세 숫자를 보여주는데 그룹은 하나만, 그것도 뒤집어서 셌다.**
//   ⇒ 여기서는 **빼지 않는다.** 원장의 기각은 *'예측력이 있나'* 를 물었고 이 점수는 *'지금 어디에 있나'* 를 묻는다 —
//     **서로 다른 질문이므로 기각이 재료를 무효화하지 않는다.** 측정 상태는 **각주로 병기**한다(④단계 패널).
//   ⚠이 문서를 *'측정으로 걷어낸 것을 되돌리는 근거'* 로 쓰지 말 것. 도넛의 **역할 정의**가 바뀌는 것이지
//     원장 판정이 뒤집히는 것이 아니다. 원장은 그대로다.
//
//  인자: `R` = `_axisRaw` · `rs` = `{rs20, rs60}`(엔진 밖 주입 — 지수 시세가 필요하다. 없으면 생략)
SXE._axisQ = _axisQ;              // [S1374] 분위 매핑 노출 — 화면(④단계)이 재료별 분위를 다시 계산할 때 쓴다
SXE._AXIS_EDGES = _AXIS_EDGES;    // [S1374] 경계표 노출 — 패널이 '이 시장 상위 N%' 를 적을 때 참조
SXE.calcAxisScore = function(R, rs){
  if(!R || !R.q) return null;
  const mkt = R._mkt || 'kr';
  //  [S1374] 상대강도 병합 — `indicators.rs`는 `sx_render.js`가 지수 fetch 후 주입하므로 엔진 안에서는 못 본다.
  const raw = { pos:{...R.pos}, vol:{...R.vol}, trend:{...R.trend}, flow:{...R.flow}, rel:{...R.rel} };
  const q   = { pos:{...R.q.pos}, vol:{...R.q.vol}, trend:{...R.q.trend}, flow:{...R.q.flow}, rel:{...R.q.rel} };
  if(rs && typeof _axisQ === 'function'){
    for(const k of ['rs20','rs60']){
      const v = rs[k];
      if(v != null && isFinite(v)){
        raw.rel[k] = v;
        const _qq = _axisQ('rel.' + k, v, mkt);
        if(_qq != null) q.rel[k] = _qq;
      }
    }
  }
  const AX = [['pos','자리'],['vol','흔들림'],['trend','추세'],['flow','수급'],['rel','상대·심리']];
  const axes = {}; const vals = [];
  for(const [a, label] of AX){
    const keys = Object.keys(q[a]);
    const items = keys.map(k => ({ k, raw: raw[a][k], q: q[a][k] })).sort((x, y) => x.k.localeCompare(y.k));
    const v = keys.length ? Math.round(keys.reduce((t, k) => t + q[a][k], 0) / keys.length) : null;
    axes[a] = { label, v, n: keys.length, items };
    if(v != null) vals.push(v);
  }
  return {
    score: vals.length ? Math.round(vals.reduce((t, x) => t + x, 0) / vals.length) : null,
    axes, nAxis: vals.length, mkt, vintage: R._vintage || null,
    method: '재료→시장별 5단계 분위→축 내부 단순평균→축 5개 단순평균 · 가중치 없음 · excl 미적용(SPEC §9-2)',
  };
};
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    if (!ind) continue;
    try { const _es = EC.entrySignalAt(mk, ind, rows, i, { srcOn: _srcOn }); _btSig[i] = _es; _btVotes[i] = _es.votes || 0; } catch (_) {}   // [S1397] mGate(레짐5) 철거 — 크로스 라우팅은 코어 내장(장기 정배·S1396)
  }

  // 라이프사이클(코어) — 진입 3원(recipe>bullVol>v2 상호배타·S1201)/청산(이중ATR+MA5×20 데드·유예10)
  const _cfgX = Object.assign({}, EC.CFG, (opts && opts.exitCfg) || {});   // [S1397] 청산 4종+게이트 칩(sx_bt UI) 오버라이드 — S1215/16 레짐 옵션 철거(시즌2 부재·3×3판 게이트가 대체)
