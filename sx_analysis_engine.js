  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    if (!pos && !pendingSignal) {
      ...
      if (_slHit) {
        ...
        //   안전필터 supportBreak(진입 보호 S469)의 청산판. 당봉 이탈=매봉검사(데드크로스 청산과 동일 lookback 개념). _regimeAllowProtective 게이트 공유.
        //   모드별 켜짐: 단타=20·60·120 / 스윙=60·120 / 중기=120 (매트릭스 셀). 종가 고정.
        if(!_earlyExit && ee.supportBreakExit && _regimeAllowProtective && i >= 20){
          const _sbHit = (len) => {
