...
function scrQuickScore(rows, tf) {
  ...
  const _sf = SXE._safetyFlags || {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,bbUpper:true,resistNear:true,fakeBreakout:true,volResist:true,debtRatio:true,foreignSell:true,highBeta:true};
...
SXE.calcRS = function(stockCloses, indexCloses){
  ...
  const _ret = (arr, n) => {
    ...
    const cur = arr[arr.length - 1], past = arr[arr.length - 1 - n];
    return (past > 0 && cur > 0) ? (cur / past - 1) * 100 : null;
  };
  const _diff = (n) => {
    const s = _ret(stockCloses, n), i = _ret(indexCloses, n);
...
SXE._safetyFlags = {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,bbUpper:true,resistNear:true,fakeBreakout:true,volResist:true,debtRatio:true,foreignSell:true,highBeta:true};
