...
function scrQuickScore(rows, tf, market) {
  ...
  {
    const _BEAR_CONT = ['데드크로스','지지선이탈','RSI다이버전스','MACD음전','MA60저항','매물대저항','되돌림주의','이브닝','슈팅','하락장악'];
    const _bearN = _safetyViol.filter(v => _BEAR_CONT.some(k => v.indexOf(k) >= 0)).length;
    const _bearPenalty = Math.min(_bearN * 5, 20);
    if (_bearPenalty > 0) rawScore = Math.max(0, rawScore - _bearPenalty);
  }
  return {
