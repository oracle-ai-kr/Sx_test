...
const SCR_ANAL_MARKET_DEFAULTS = {
  ...
  kr:     { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:53, sellTh:30, tpMult:4.6, slMult:2.0 },
  us:     { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:52, sellTh:25, tpMult:2.8, slMult:1.4 },
  crypto: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:63, sellTh:35, tpMult:6.0, slMult:3.0 }
};
...
const SCR_ANAL_MARKET_REGIME_DEFAULTS = {
  kr: {
    on:  { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:53, sellTh:30, tpMult:4.6, slMult:2.0 },
    off: { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:52, sellTh:30, tpMult:5.6, slMult:2.0 }
  },
  us: {
    on:  { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:52, sellTh:25, tpMult:2.8, slMult:1.4 },
    off: { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:25, tpMult:3.1, slMult:1.4 }
  },
  crypto: {
    on:  { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:63, sellTh:35, tpMult:6.0, slMult:3.0 },
    off: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:60, sellTh:35, tpMult:6.0, slMult:3.0 }
  }
};
...
SXE._btEarlyExit = (function(){
  ...
  //   변경: score true→false, trailMode immediate→profit,
  //         maBreak false→true, maBreakLen 20→120, nHigh false→true, nHighBars 20→60
  //   사유: 사용자 요청 — 스윙 매매 스타일이 기본 동선이 되도록 단일 소스(_optSyncEarlyExitToMode의 balanced 분기)에서 복사
  //   주의: sx_screener.html의 _eeEnsureDefaults + resetEarlyExit과 반드시 같이 수정.
  return {
    enabled:   true,   // 마스터 스위치
    score:     false,  // [S306] true→false (스윙: rawScore<=sellTh로 청산하지 않음)
    ...
    trailMode: 'profit', // [S306] 'immediate'→'profit' (스윙: 수익권 진입 후 작동) — 'immediate' = 진입 즉시
    ...
    // [S300] 추가 청산 룰 (실험) — [S306] 스윙 기본값으로 ON 전환
    //   bbUpper:    종가 ≥ ind.bb.upper 시 익절 (스윙 텍스트 룰 "BB 상단 터치") — 스윙 기본 OFF
    //   maBreak:    종가 < MA(maBreakLen) 시 손절 (스윙=MA120 권장) — 스윙 기본 ON
    //   nHigh:      진입 후 직전 nHighBars봉 최고가 도달 시 익절 (스윙 "직전 고점 저항선") — 스윙 기본 ON
    //   주의: 워커(sx_scan_worker.js)는 startScan config로 _btEarlyExit를 받지 않으므로 현재 이 기본값 그대로 사용
    //        메인 스크리너의 _eeEnsureDefaults와 권위 동기화 필수
    bbUpper:    false,
    maBreak:    true,  // [S306] false→true
    maBreakLen: 120,   // [S306] 20→120
    nHigh:      true,  // [S306] false→true
    nHighBars:  60     // [S306] 20→60
  };
})();
