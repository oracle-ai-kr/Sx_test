...
// [S401] 방향 중심 종합 추세 점수(trendPure) — 표시·verdict용 trendScore (RSI/변동성/다이버전스 제외).
//   배경: 기존 trendScore=rawScore(scrComputeScore)에는 (50−RSI) 역추세 항이 섞여 있어
//          강세 추세주(고RSI·정배열·신고가)가 추세방향 점수에서 깎이고 verdict 추세경로도 못 잡혔음.
//   설계: 방향이 주축(20일방향·기울기·SAR) + 견고함 보조(MA배열·ADX×방향) + 구조위치.
//          ※ ADX는 '순수 세기'가 아니라 +DI/−DI와 결합한 '방향 견고함'으로 참조 —
//            전광판 '추세강도(ADX)' 항목(세기 단독)과는 용도가 구분됨(같은 지표, 다른 관점).
...
function scrTrendPure(ind){
  ...
  const score = Math.round(clamp(50 + 50 * Math.tanh(((t - 50) / 50) * (SCR_SCORING?.tanh ?? 1.0)), 0, 100));
  return { score, parts };
}
// ════════════════════════════════════════════════════════════
...
function scrQuickScore(rows, tf) {
  ...
  return {
    ...
    trendScore: _tpRes.score, trendBreakdown: _tpRes.parts, // [S401/402] 방향 중심 종합 추세 + 분해 (진입/BT는 score=rawScore 그대로)
...
function scoreMomentum(rows, tf, lookback) {
  ...
  return {
    ...
    _normV: 6, // [S357~S360] 점수 계산 버전 (정규화·점수로직 변경 시 +1 → 구버전 캐시 자동 재계산). v5=[S360] 크로스 전환조건 / v6=[S401] trendScore=방향 중심 종합 추세(trendPure)로 변경
  };
}
