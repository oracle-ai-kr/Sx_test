...
function calcAllScreener(rows, tf) {
  ...
  // [PATCH-13] OHLC 정합성 보정 — 외부 API 오염 데이터(close > high 등) 방어
  //   강한 보정 없이 안전 범위로 clamp: high=max(o,h,l,c), low=min(o,h,l,c)
  //   오염이 드물고 대부분 정상이므로 inplace 수정 대신 새 배열 생성 (원본 보존)
  //   동작 영향 최소화: 정상 데이터는 그대로 유지, 오염 데이터만 정제
  if (rows && rows.length) {
    const _sample = rows[rows.length - 1];
    const _needSanitize = _sample && (
      (_sample.close != null && _sample.high != null && _sample.close > _sample.high) ||
      (_sample.close != null && _sample.low != null && _sample.close < _sample.low) ||
      (_sample.open != null && _sample.high != null && _sample.open > _sample.high) ||
      (_sample.open != null && _sample.low != null && _sample.open < _sample.low)
    );
    if (_needSanitize) {
      rows = rows.map(r => {
