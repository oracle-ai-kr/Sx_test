...
const SCR_ANAL_MARKET_DEFAULTS = {
  ...
  kr:     { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:30, sellTh:20, tpMult:10.6, slMult:0.8 },
  us:     { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:31, sellTh:22, tpMult:2.1, slMult:1.5 },
  crypto: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:30, sellTh:46, tpMult:5.8, slMult:0.5 }
};
...
const SCR_ANAL_MARKET_REGIME_DEFAULTS = {
  kr: {
    on:  { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:30, sellTh:20, tpMult:10.6, slMult:0.8 },
    off: { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:30, sellTh:20, tpMult:10.6, slMult:0.8 }
  },
  us: {
    on:  { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:31, sellTh:22, tpMult:2.1, slMult:1.5 },
    off: { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:30, sellTh:23, tpMult:2.1, slMult:1.0 }
  },
  crypto: {
    on:  { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:30, sellTh:46, tpMult:5.8, slMult:0.5 },
    off: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:34, sellTh:31, tpMult:3.7, slMult:0.7 }
  }
};
...
SXE._btEarlyExit = (function(){
  ...
  return {
    ...
    trailMode: 'immediate', // [S298] 기본값 변경: 'profit'(수익권 진입 후) → 'immediate'(진입 즉시) / 'profit' = 수익권 진입 후 작동
    ...
    // [S300] 추가 청산 룰 (실험) — 모두 OFF 시작, 사용자가 모달에서 직접 켜서 영향 측정
    //   bbUpper:    종가 ≥ ind.bb.upper 시 익절 (스윙 텍스트 룰 "BB 상단 터치")
    //   maBreak:    종가 < MA(maBreakLen) 시 손절 (스윙=MA20 / 중기=MA60 권장)
    //   nHigh:      진입 후 직전 nHighBars봉 최고가 도달 시 익절 (스윙 "직전 고점 저항선")
    //   주의: 워커(sx_scan_worker.js)는 startScan config로 _btEarlyExit를 받지 않으므로 현재 이 기본값 그대로 사용 (모두 OFF)
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  const _volTargetMeta = _vtActive ? {
    active: true,
    market: _vtMarket,
    targetPct: (function(){
      const c = SXE._volTargetCache;
