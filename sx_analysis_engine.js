...
const SCR_ANAL_MARKET_DEFAULTS = {
  ...
  kr:     { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:45, sellTh:30, tpMult:4.8, slMult:2.1 },
  us:     { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:55, sellTh:26, tpMult:3.6, slMult:1.5 },
  crypto: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:36, tpMult:8.0, slMult:3.0 }
};
...
const SCR_ANAL_MARKET_REGIME_DEFAULTS = {
  kr: {
    on:  { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:45, sellTh:30, tpMult:4.8, slMult:2.1 },
    off: { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:46, sellTh:30, tpMult:5.1, slMult:2.1 }
  },
  us: {
    on:  { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:55, sellTh:26, tpMult:3.6, slMult:1.5 },
    off: { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:54, sellTh:25, tpMult:2.8, slMult:1.4 }
  },
  crypto: {
    on:  { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:36, tpMult:8.0, slMult:3.0 },
    off: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:40, tpMult:8.8, slMult:3.0 }
  }
};
...
SXE._btEarlyExit = (function(){
  ...
  // [S311] 스윙 기본값 사진 동기화 — score:true, maBreakLen:60, nHigh:false
  //   변경: score false→true, maBreakLen 120→60, nHigh true→false
  //   사유: 사용자 사진 기준 — _optSyncEarlyExitToMode의 balanced 분기와 단일 소스 동기화
  //   주의: sx_screener.html의 _eeEnsureDefaults + resetEarlyExit과 반드시 같이 수정.
  return {
    enabled:   true,   // 마스터 스위치
    score:     true,   // [S311] false→true (스윙도 rawScore<=sellTh 청산 활용)
    ...
    // [S300] 추가 청산 룰 — [S306] 스윙 전환 — [S308] BB_UPPER/NHIGH 장중 high+트리거 가격 청산 — [S311] 사진 동기화
    //   bbUpper:    장중 high ≥ ind.bb.upper 시 BB.upper에서 익절 — 스윙 기본 OFF
    //   maBreak:    종가 < MA(maBreakLen) 시 c에서 손절 — 스윙 기본 ON (MA60) [S311] MA120→MA60
    //   nHigh:      장중 high ≥ 직전 nHighBars봉 max(high) 도달 시 _maxH에서 익절 — 스윙 기본 OFF [S311] true→false
    ...
    bbUpper:    false,
    maBreak:    true,    // 스윙 기본 ON
    maBreakLen: 60,      // [S311] 120→60
    nHigh:      false,   // [S311] true→false
    nHighBars:  60
  };
})();
