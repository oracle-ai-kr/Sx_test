...
function scrTrendPure(ind){
  let t = 50;
  t += clamp((ind.trend?.pct ?? 0) * 4.0, -28, 28);          // 20일 추세 방향
  t += clamp((ind.trend?.slope ?? 0) * 18.0, -18, 18);       // 기울기
  // MA 배열 (정배열↑ / 역배열↓ / 단기>중기 약가점)
  if (ind.maAlign?.bullish) t += 16;
  else if (ind.maAlign?.bearish) t -= 16;
  else if (ind.maAlign?.short != null && ind.maAlign?.mid != null && ind.maAlign.short > ind.maAlign.mid) t += 6;
  // ADX × 방향 (추세 강할수록 ±, +DI>-DI면 가점)
  const adxV = ind.adx?.adx ?? 0, pdi = ind.adx?.pdi, mdi = ind.adx?.mdi;
  const adxStr = clamp((adxV - 20) * 0.7, 0, 20);
  if (pdi != null && mdi != null) {
    if (pdi > mdi) t += adxStr;
    else if (mdi > pdi) t -= adxStr;
  }
  // 구조위치 — 추세추종 방향 (고점/신고가 = 강세 지속 = 가점, scrComputeScore와 반대 부호)
  const sp = ind.trend?.struct?.pos;
  if (sp != null) t += clamp((sp - 0.5) * 20, -10, 10);
  // SAR 추세
  if (ind.psar?.trend === 'up') t += 4; else if (ind.psar?.trend === 'down') t -= 4;
  return Math.round(clamp(50 + 50 * Math.tanh(((t - 50) / 50) * (SCR_SCORING?.tanh ?? 1.0)), 0, 100));
}
// ════════════════════════════════════════════════════════════
...
function scrUpsideScore(ind) {
  ...
  const score = Math.round(clamp(50 + 50 * Math.tanh((s / 45) * 0.95), 0, 100));
...
function scrQuickScore(rows, tf) {
  ...
  return {
    ...
    trendScore: scrTrendPure(ind), // [S401] 표시·verdict용 순수 추세강도 (진입/BT는 score=rawScore 그대로)
...
function scoreMomentum(rows, tf, lookback) {
  ...
  return {
    ...
    _normV: 5, // [S357~S360] 점수 계산 버전 (정규화·점수로직 변경 시 +1 → 구버전 캐시 자동 재계산). v5=[S360] 크로스 전환조건(정·역배열 전에만)
  };
}
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  return { winRate, profitFactor: pf, totalPnl, mdd: +maxDD.toFixed(2), totalTrades: closed, avgWin, avgLoss, maxConsecLoss, scores: _btScores.map(s => s.t), scores3: _btScores, trades, rowsLength: rows.length, gateBlocks: _gateBlocks, gateReasons: _gateReasons, volTarget: _volTargetMeta };
}
