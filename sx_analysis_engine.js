  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    if (!pos && !pendingSignal) {
      ...
      if (_slHit) {
        ...
        if(!_earlyExit && ee.deadCross && i >= 20){
        ...
        if(!_earlyExit && ee.dumpExit && typeof SXE.calcDumpWarn === 'function'){
          try {
        ...
        // [S300] BB 상단 익절 — [S308] 종가 c → 장중 high h 로 변경
        //   배경: 텍스트 룰 "BB 상단 터치 익절"의 "터치"는 장중 high 도달을 자연스럽게 함의.
        //         종가 봉 마감 전 BB 상단 터치 후 후퇴하는 케이스가 빈번 — 종가 고정이면 그 봉 놓침.
        //   청산가: ind.bb.upper (limit 매도 가정, TP/SL과 동일 패턴) — 슬리피지는 청산 시 일괄 적용
        //   _btExitMode 무관: 보조 룰은 룰 정의 자체로 종가/장중 고정 ('터치' = 장중 high)
        if(!_earlyExit && ee.bbUpper && ind.bb && ind.bb.upper > 0 && h >= ind.bb.upper){
          _earlyExit = 'BB_UPPER';
          _earlyExitPrice = ind.bb.upper; // [S308] 트리거 가격에서 청산
        }
        ...
        if(_earlyExit){
          // [S308] 청산가 분기: BB_UPPER/NHIGH는 장중 트리거 가격, 나머지(MA_BREAK/SCORE/TIME)는 종가
