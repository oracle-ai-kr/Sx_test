...
function scoreMomentum(rows, tf, lookback) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    try {
      ...
      history.push({ bar: -i, score: qs.readyScore ?? qs.score, trendScore: qs.score, entryScore: qs.entryScore ?? 0, readyScore: qs.readyScore ?? 0, upsideScore: qs.upsideScore ?? 0 });
    } catch (e) {
      break;
    }
  }
