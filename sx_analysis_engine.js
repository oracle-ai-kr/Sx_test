...
function scrQuickScore(rows, tf) {
  ...
  if (_sf.threshold && action === 'BUY' && rawScore < th.buyTh + 2) { reasons.push('🔒임계값'); action = 'HOLD'; }
  { const vf = _scrVolFilter(volSoft, tf);
    if (_sf.volExtreme && volSoft >= vf.hard) { reasons.push('🔒변동성극단'); action = 'HOLD'; }
    else if (_sf.volHigh && volSoft >= vf.softTh && action === 'BUY' && rawScore < th.buyTh + vf.bonus) { reasons.push('🔒변동성과다'); action = 'HOLD'; }
  }
