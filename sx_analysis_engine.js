...
// ════════════════════════════════════════════════════════════
//  S125: 모드×레짐 6칸 매트릭스 저장소 (3모드 × 2레짐 = 6개 독립 슬롯)
//
//                  [수익형]        [안정형]        [보수형]
//   레짐 ON   ┃  profit.on    balanced.on     safe.on
//   레짐 OFF  ┃  profit.off   balanced.off    safe.off
//
//   [설계 원칙]
//     · 각 슬롯은 완전 독립 — 모드·레짐 전환 시 상호 덮어쓰기 없음
//     · 저장값 자체가 "해당 레짐 조건으로 BT 최적화된 값" → 이중 보정 금지
//     · 레짐의 역할은 "감지"와 "어느 행을 선택할지"까지. 임계값 가감 보정은 폐기(S125)
//
//   [저장 스키마]
//     localStorage.SX_SCR_ANAL_PARAMS_MATRIX = {
//       profit:   { on: {buyTh, sellTh, tpMult, slMult, rsiLen, ...}, off: {...} },
//       balanced: { on: {...}, off: {...} },
//       safe:     { on: {...}, off: {...} }
//     }
//
...
//     ② SCR_ANAL_MODE_REGIME_DEFAULTS 하드코딩 (모드×레짐 실측값)
//     ③ SCR_ANAL_DEFAULTS (최종 폴백)
// ════════════════════════════════════════════════════════════
const SCR_ANAL_PARAMS_MATRIX_KEY = 'SX_SCR_ANAL_PARAMS_MATRIX';

// 현재 모드 조회 (_analMode는 sx_screener.html 전역 — undefined일 때 balanced 기본)
function _getCurrentMode(){
  // S125: 워커 환경에서는 SXE._workerCurrentMode 우선(메인이 scanPayload에 주입)
  if(typeof SXE !== 'undefined' && SXE._workerCurrentMode) return SXE._workerCurrentMode;
  return (typeof _analMode !== 'undefined' && _analMode) ? _analMode : 'balanced';
}
// 현재 레짐 ON/OFF 조회 (순환참조 방지: SXE.regimeAdaptEnabled 정의 이전에도 동작해야 함)
function _getCurrentRegimeOn(){
  // S125: 워커 환경에서는 SXE._workerRegimeOn 우선
  ...
  try {
    ...
    if(v === '1' || v === '0') return v === '1'; // S124 레거시 호환
    return v !== 'off';
  } catch(_){ return true; }
}
...
// 특정 (mode, regimeOn) 좌표의 슬롯 파라미터 조회 — 폴백 체인 ①②③ 순차 시도
function _getSlotParams(mode, regimeOn){
  const regKey = regimeOn ? 'on' : 'off';
  // ① 매트릭스 저장값 (buyTh>0 이어야 유효로 간주 — 0은 '미저장' 의미와 충돌하지 않음)
  const matrix = _loadParamsMatrix();
  const slot = matrix[mode] && matrix[mode][regKey];
  if(slot && typeof slot === 'object' && slot.buyTh > 0){
    return { ...SCR_ANAL_DEFAULTS, ...slot };
  }
  // ② 모드×레짐 하드코딩 기본값
  const def = SCR_ANAL_MODE_REGIME_DEFAULTS[mode] && SCR_ANAL_MODE_REGIME_DEFAULTS[mode][regKey];
  if(def){ return { ...SCR_ANAL_DEFAULTS, ...def }; }
  // ③ 최종 폴백
  return { ...SCR_ANAL_DEFAULTS };
}

// 특정 (mode, regimeOn) 좌표에 파라미터 저장
function _saveSlotParams(mode, regimeOn, params){
  if(!params || typeof params !== 'object') return;
  const regKey = regimeOn ? 'on' : 'off';
  const matrix = _loadParamsMatrix();
  if(!matrix[mode]) matrix[mode] = {};
  matrix[mode][regKey] = { ...params };
  _saveParamsMatrix(matrix);
}

// S125 마이그레이션: 구 단일 키(SX_SCR_ANAL_PARAMS)의 값을 현재 좌표 슬롯으로 이동
//   [1회성] 매트릭스 해당 좌표가 비어 있을 때만 이동 (기존 옵티마이저 저장값 보존)
//   [안전] 단일 키는 마이그레이션 후에도 유지 (롤백 가능 + _loadAnalParams 하위 호환)
let _matrixMigrated = false;
function _migrateLegacyAnalParams(){
  if(_matrixMigrated) return;
  _matrixMigrated = true;
  try {
    const legacy = JSON.parse(localStorage.getItem(SCR_ANAL_PARAMS_KEY) || 'null');
    if(!legacy || typeof legacy !== 'object' || !(legacy.buyTh > 0)) return;
    const mode = _getCurrentMode();
    const regOn = _getCurrentRegimeOn();
    const matrix = _loadParamsMatrix();
    const regKey = regOn ? 'on' : 'off';
    // 해당 좌표에 이미 값 있으면 마이그레이션 스킵(옵티마이저 저장 우선)
    if(matrix[mode] && matrix[mode][regKey] && matrix[mode][regKey].buyTh > 0) return;
    if(!matrix[mode]) matrix[mode] = {};
    matrix[mode][regKey] = { ...legacy };
    _saveParamsMatrix(matrix);
    console.log(`[S125] 레거시 파라미터 → 매트릭스[${mode}.${regKey}] 마이그레이션 완료`);
  } catch(_){}
}

function _loadAnalParams() {
  // S125: 매트릭스 우선 — 현재 모드/레짐 좌표의 슬롯 반환
  _migrateLegacyAnalParams(); // 1회성 마이그레이션
  const mode = _getCurrentMode();
  const regOn = _getCurrentRegimeOn();
  return _getSlotParams(mode, regOn);
}
function _saveAnalParams(p) {
  // S125: 매트릭스 현재 좌표에 저장 + 레거시 단일 키에도 미러링(하위 호환)
  const mode = _getCurrentMode();
  const regOn = _getCurrentRegimeOn();
  _saveSlotParams(mode, regOn, p);
  try { localStorage.setItem(SCR_ANAL_PARAMS_KEY, JSON.stringify(p)); } catch(_){}
}
...
// S125: 모드×레짐 매트릭스 API export (sx_optimizer.js / sx_screener.html에서 사용)
...
SXE._getCurrentMode = _getCurrentMode;
  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    if (!pos && !pendingSignal) {
      ...
      if (c >= pos.tp) {
        ...
        const pnl = ((exitP - pos.entry) / pos.entry) * 100;
        equity *= (1 + pnl / 100);
        // S210: 트레일링/본전보호로 끌어올려진 SL이면 결과에 따라 WIN/LOSS 분류
        //   기존: SL 도달 = 무조건 LOSS
        //   신규: pnl 부호로 분류 (트레일링 익절도 가능해짐)
        const _exitType = pnl >= 0 ? 'WIN' : 'LOSS';
        const _exitReason = (SXE._btEarlyExit && SXE._btEarlyExit.enabled && pos.sl > pos.entry)
          ? 'TRAIL' : (pos.sl === pos.entry ? 'BE' : 'SL');
