...
const SCR_ANAL_MARKET_DEFAULTS = {
  ...
  kr:     { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:20, tpMult:9.5, slMult:4.0 },
  us:     { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:20, tpMult:5.5, slMult:8.5 },
  crypto: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:35, sellTh:20, tpMult:5.0, slMult:2.5 }
};
...
const SCR_ANAL_MARKET_REGIME_DEFAULTS = {
  kr: {
    on:  { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:20, tpMult:9.5, slMult:4.0 },
    off: { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:40, sellTh:20, tpMult:9.0, slMult:4.0 }
  },
  us: {
    on:  { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:50, sellTh:20, tpMult:5.5, slMult:8.5 },
    off: { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:65, sellTh:20, tpMult:7.0, slMult:6.5 }
  },
  crypto: {
    on:  { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:35, sellTh:20, tpMult:5.0, slMult:2.5 },
    off: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:35, sellTh:20, tpMult:5.0, slMult:3.0 }
  }
};
...
function calcAllScreener(rows, tf) {
  ...
  // [S436] 매물대 저항 — 최근 120봉 가격대별 거래량 분포에서 현재가 위쪽 +1~8% 구간에 거래량이 집중(전체 20%↑)되면
  //   머리 위 매물(과거 그 가격에 물린 물량의 본전 매도)이 상승 저항으로 작용 = 진입 부담. 안전필터 volResist 단일소스.
  //   Volume Profile(POC·매물대) 개념을 안전필터용으로 단순화 — 봉 종가 기준 거래량 누적(고저 균등배분 대비 경량).
  base._volResist = null;
  try {
    const _vrN = Math.min(120, closes.length);
    if (_vrN >= 30) {
      const _vrCloses = closes.slice(-_vrN);
      const _vrVols = rows.slice(-_vrN).map(r => r.volume || 0);
      const _vrCur = closes[closes.length - 1];
      const _vrTotal = _vrVols.reduce((a, b) => a + b, 0);
      if (_vrTotal > 0 && _vrCur > 0) {
        const _vrLo = _vrCur * 1.01, _vrHi = _vrCur * 1.08; // 현재가 위쪽 +1% ~ +8%
        let _vrZone = 0;
        for (let i = 0; i < _vrN; i++) { if (_vrCloses[i] >= _vrLo && _vrCloses[i] <= _vrHi) _vrZone += _vrVols[i]; }
...
function scrUpsideScore(ind) {
  ...
  // ── ① 추세 정렬 (max ~28) ──
  if (ma.bullish) { s += 18; notes.push('이평선 정배열(5>20>60)'); }
  ...
  // 골든크로스 (최근 발생일수록 강력) — [S360] 정배열 전(전환 시점)에만 가점, 정배열이면 +18로 이미 반영
  ...
  if (adxV >= 30 && pdi != null && mdi != null && pdi > mdi) { s += 14; notes.push('강한 상승추세(ADX ' + Math.round(adxV) + ')'); }
  ...
  // ── ④ 수급 (max ~18) ──
  if (ind.obv?.trend === 'up') { s += 8; notes.push('OBV 상승(수급 유입)'); }
...
function scrQuickScore(rows, tf, market) {
  ...
  const _sf = SXE._safetyFlags || {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,bbUpper:true,resistNear:true,fakeBreakout:true,volResist:true,chaseGuard:true,dumpWarn:true,debtRatio:true,foreignSell:true,highBeta:true};
...
SXE._checkSafetyFilters = function(ind, rawScore, buyTh, volSoft, tf, candles){
...
SXE._btEarlyExit = (function(){
  ...
  //   사유: 사용자 사진 기준 9칸 매트릭스 — 스윙 3개 시장 모두 score=F, maBreak=F
  //   주의: sx_screener.html의 _eeEnsureDefaults + resetEarlyExit,
  //        sx_optimizer.js의 _EARLY_EXIT_MATRIX.kr.balanced 모두 동일하게 동기화됨.
  //        변경 시 3곳을 같이 수정. 페이지 진입 후 [S307]/[S333] 시작 동기화가
  //        실제 사용자 시장×모드 셀로 즉시 덮어씀.
  return {
    enabled:   true,   // 마스터 스위치
    score:     false,  // [S334] true→false (9칸 매트릭스 동기화 — 스윙 score=F)
    ...
    // 추가 청산 룰 — 매트릭스 kr.balanced 셀: bbUpper=F, maBreak=F, nHigh=F
    ...
    bbUpper:    false,
    maBreak:    false,   // [S334] true→false (9칸 매트릭스 동기화 — 스윙 maBreak=F)
    maBreakLen: 60,
    nHigh:      false,
    nHighBars:  60,
    deadCross:  false,   // [S460] 데드크로스 청산 — 종가 MA5×MA20 하향 교차 시. 매트릭스 미포함(localStorage 유지)
    dumpExit:   false,   // [S460] 되돌림주의 청산 — calcDumpWarn(투매·RSI/OBV 다이버전스·과열) on 시
    regimeAdaptive: false, // [S462] 추세 적응 — ON이면 데드크로스·되돌림을 하락추세(종가<MA60)에서만 적용. 매트릭스 미포함(localStorage 유지)
    beEscape:   false,   // [S463] 본전 탈출 — 진입 후 손실 무장 후 본전 근처 회복 시 청산 (트레일링 거울상)
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
    ...
    if (!pos && !pendingSignal) {
      if (rawScore >= buyTh && _scrMomOscPass(mom, osc, tf)) {
        ...
        if(SXE._applySafetyToBt){
          const _safRes = SXE._checkSafetyFilters(ind, rawScore, buyTh, volSoft, tf, slice); // [S457] slice 전달 → 되돌림주의 BT 적용
      ...
      if (_slHit) {
        ...
        // [S300] MA 이탈 손절 — 종가 < MA(maBreakLen)
        //   순서: SL/TP 다음, 점수/시간 청산 앞 (손절 성격 우선)
        //   계산: rows[i-len+1..i].close 평균 (warmup BT_WARMUP=150 ≥ maxLen 120 보장)
        //   청산가: 종가 c (분봉 데이터 없으므로 일봉 종가 매매 가정)
        //   [S308] 종가 고정 유지 — 텍스트 룰 "MA 이탈"은 봉 마감 기준 표준, 장중 휩소 방지
        if(ee.maBreak){
          const _maLen = ee.maBreakLen || 20;
          if(i >= _maLen - 1){
            let _sum = 0;
            for(let j = i - _maLen + 1; j <= i; j++) _sum += rows[j].close;
            const _ma = _sum / _maLen;
            if(c < _ma) _earlyExit = 'MA_BREAK';
          }
        }
        // [S462] 추세 적응 게이트 — regimeAdaptive ON이면 데드크로스·되돌림 청산을 하락추세(종가<MA60)에서만 적용.
        //   상승추세(종가>=MA60)면 켜져 있어도 보류 → 추세 눌림목 승자절단 방지. MA60은 spec 고정(60봉 SMA, params 무관).
        ...
        //   [S462] _regimeAllowProtective: 추세 적응 게이트(상승추세면 보류)
        if(!_earlyExit && ee.deadCross && _regimeAllowProtective && i >= 20){
        ...
        if(!_earlyExit && ee.score && rawScore <= sellTh){
          _earlyExit = 'SCORE';
        }
        ...
        if(_earlyExit){
          // [S308→S461] 청산가 분기: NHIGH만 장중 트리거 가격(_earlyExitPrice), 나머지(MA_BREAK/SCORE/TIME/DEAD_CROSS/DUMP_WARN/BB_UPPER)는 종가 c
  ...
  return { winRate, profitFactor: pf, totalPnl, mdd: +maxDD.toFixed(2), totalTrades: closed, avgWin, avgLoss, maxConsecLoss, scores: _btScores.map(s => s.t), scores3: _btScores, trades, rowsLength: rows.length, gateBlocks: _gateBlocks, gateReasons: _gateReasons, gapSkips: _gapSkips, entryMode: (nextBar ? 'nextOpen' : 'close'), gapGuard: !!(nextBar && SXE._btGapGuard), volTarget: _volTargetMeta };
}
...
SXE._safetyFlags = {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,bbUpper:true,resistNear:true,fakeBreakout:true,volResist:true,chaseGuard:true,dumpWarn:true,debtRatio:true,foreignSell:true,highBeta:true};
