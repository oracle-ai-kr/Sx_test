...
const SCR_ANAL_MARKET_DEFAULTS = {
  ...
  kr:     { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:40, sellTh:30, tpMult:6.0, slMult:2.0 },  // [S683] 사용자 재설정 — buyTh 안전바닥(중립50 아래)/R:R 3:1
  us:     { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:35, sellTh:25, tpMult:9.0, slMult:3.0 },  // [S683]
  crypto: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:45, sellTh:35, tpMult:3.0, slMult:1.0 }   // [S683]
};
...
const SCR_ANAL_MARKET_REGIME_DEFAULTS = {
  // [S683] 사용자 재설정 — on/off 동일값(단일 시장 기본값). 레짐 동적보정은 regimeAdapt가 봉별로 별도 가산.
  kr: {
    on:  { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:40, sellTh:30, tpMult:6.0, slMult:2.0 },
    off: { rsiLen:14, bbLen:14, bbMult:1.9, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:40, sellTh:30, tpMult:6.0, slMult:2.0 }
  },
  us: {
    on:  { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:35, sellTh:25, tpMult:9.0, slMult:3.0 },
    off: { rsiLen:14, bbLen:20, bbMult:2.0, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:35, sellTh:25, tpMult:9.0, slMult:3.0 }
  },
  crypto: {
    on:  { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:45, sellTh:35, tpMult:3.0, slMult:1.0 },
    off: { rsiLen:14, bbLen:9,  bbMult:2.1, maShort:5, maMid:20, maLong:60, atrLen:14, buyTh:45, sellTh:35, tpMult:3.0, slMult:1.0 }
  }
};
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  for (let i = BT_WARMUP; i < rows.length; i++) {
    ...
    if(_maCrossPure){ buyTh = _macrossBuyTh; sellTh = _macrossSellTh; slMult = 2; }
    ...
    if (!pos && !pendingSignal) {
      ...
      if (_gatePair) {
        ...
        if (_goldenCross && (!_maCrossPure || rawScore >= buyTh)) {
          _entryAllowed = true; _entryType = 'golden';            // [S682] 순수모드: 점수≥buyTh 게이트 / 일반모드: 점수 무관(추세시작)
