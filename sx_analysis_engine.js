...
function scrUpsideScore(ind) {
  ...
  // ── [S711] 정배열 & ADX 동반 부스터 ──
  //   근거: SXUV 신호감사(S711) — AND(정배열&ADX)가 약세풀 lift 1.30(n=787)·강세풀에서도 견고. 개별 가점
  //   (정배열12·ADX11)은 풀마다 흔들리나 '공존'은 양 레짐 keeper(강세=돌파와 함께, 약세=희소성으로 캐리). 방향성
  //   검증서 상승만 예측(1.26)·하락 미예측(0.89)=동어반복 아님. 둘 다 점화한 고확신 추세 확증에만 소폭 가점.
  //   보수적 +5(개별 합 23 위 ~22%). 닫힌루프(C판정검증 추가상승 IC)로 효과 확인 후 조정 — 효과없으면 0으로 되돌림.
  if (ma.bullish && adxV >= 25 && pdi != null && mdi != null && pdi > mdi) {
    s += 5; notes.push('정배열+ADX 동반(추세 확증)');
  }
