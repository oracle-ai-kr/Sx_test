...
function scrQuickScore(rows, tf, market) {
  ...
  let { rawScore, mom, osc, _breakdown } = scrComputeScore(ind, volSoft, ctx.bonus); // [S527] rawScore let — 추가하락 차감 위해
  ...
  // [S527] 하락장악(bearish engulfing) — 약세 지속 패턴(candle.patterns). _sf.bearEngulf 미정의 시 기본 ON(!==false).
  if (_sf.bearEngulf !== false && action === 'BUY' && Array.isArray(ind.candle.patterns) && ind.candle.patterns.some(p => p && p.name === '하락장악')) { reasons.push('🔒하락장악'); action = 'HOLD'; }
  ...
  const _safetyViol = [];
  {
    ...
    if (Array.isArray(ind.candle.patterns) && ind.candle.patterns.some(p => p && p.name === '하락장악')) _safetyViol.push('🔒하락장악'); // [S527]
  }
  ...
  // [S527] 추가하락(약세 지속) 차감 — 감지된 약세-지속 패턴 수 × 5점, 상한 −20. (가) rawScore brake → 진입타이밍↓ · aTimingOn(S514 게이트)↓ → 진입대기·C관심.
  //   (나)는 위 _safetyViol 배지 그대로(중복 OK = 사용처 다름: rawScore=타이밍 / _safetyViol=위험배지). BT는 scrComputeScore 직접호출이라 불간섭(S514 원칙 유지).
  //   _BEAR_CONT 패턴은 모두 reasons에서도 BUY→HOLD라 action=BUY와 공존 안 함 → 보라▲ 불일치 없음. 차감은 표시 점수·게이트에 graded로 반영. 패턴당/상한은 차후 보정.
  let _aBearN = 0, _aBearPenalty = 0; // [S528] 외부 노출용 (render [추가하락] 배지)
  {
    const _BEAR_CONT = ['데드크로스','지지선이탈','RSI다이버전스','MACD음전','MA60저항','매물대저항','되돌림주의','이브닝','슈팅','하락장악','가짜돌파']; // [S529] 가짜돌파 추가 = #3 거래량없는반등(막 돌파했으나 OBV↓/거래량 미증)
  ...
  return {
    ...
    bearCont: { n: _aBearN, penalty: _aBearPenalty }, // [S528] 추가하락 표시용 — 발동 시 render [추가하락] 배지
