// ════════════════════════════════════════════════════════════
//  SIGNAL X — Analysis Engine v4.0 <!-- S100 -->
...
// S100: 3모드 × 2레짐 = 6개 기본 하드코딩 (사용자 검증 임계값)
const SCR_ANAL_MODE_DEFAULTS = {
  // 기존 호환 (레짐 무관 fallback — balanced OFF 기준)
  profit:   { rsiLen:14, bbLen:11, bbMult:2.1, maShort:2, maMid:15, maLong:20,  atrLen:14, buyTh:25, sellTh:5, tpMult:3.1, slMult:1.7 },
  balanced: { rsiLen:14, bbLen:10, bbMult:1.8, maShort:2, maMid:15, maLong:20,  atrLen:14, buyTh:25, sellTh:5, tpMult:2.5, slMult:1.5 },
  safe:     { rsiLen:14, bbLen:10, bbMult:2.1, maShort:4, maMid:5,  maLong:21,  atrLen:14, buyTh:55, sellTh:5, tpMult:2.5, slMult:1.7 }
};
// S100: 모드×레짐 6개 조합별 기본값
const SCR_ANAL_MODE_REGIME_DEFAULTS = {
  profit: {
    on:  { rsiLen:14, bbLen:13, bbMult:2.0, maShort:4, maMid:5,  maLong:102, atrLen:14, buyTh:46, sellTh:5, tpMult:2.5, slMult:1.6 },
    off: { rsiLen:14, bbLen:11, bbMult:2.1, maShort:2, maMid:15, maLong:20,  atrLen:14, buyTh:25, sellTh:5, tpMult:3.1, slMult:1.7 }
  },
  balanced: {
    on:  { rsiLen:14, bbLen:20, bbMult:1.9, maShort:3, maMid:5,  maLong:105, atrLen:14, buyTh:46, sellTh:5, tpMult:2.5, slMult:1.4 },
    off: { rsiLen:14, bbLen:10, bbMult:1.8, maShort:2, maMid:15, maLong:20,  atrLen:14, buyTh:25, sellTh:5, tpMult:2.5, slMult:1.5 }
  },
  safe: {
    on:  { rsiLen:14, bbLen:24, bbMult:1.8, maShort:2, maMid:5,  maLong:115, atrLen:14, buyTh:49, sellTh:5, tpMult:2.5, slMult:1.5 },
    off: { rsiLen:14, bbLen:10, bbMult:2.1, maShort:4, maMid:5,  maLong:21,  atrLen:14, buyTh:55, sellTh:5, tpMult:2.5, slMult:1.7 }
  }
};
...
function sxRunBtEngine(rawRows, tf, params, opts = {}) {
  ...
  return { winRate, profitFactor: pf, totalPnl, mdd: +maxDD.toFixed(2), totalTrades: closed, avgWin, avgLoss, maxConsecLoss, scores: _btScores.map(s => s.t), scores3: _btScores, trades };
}
