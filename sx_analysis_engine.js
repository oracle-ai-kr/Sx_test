...
const SCR_SCORING = { tanh: 1.15, ctx: 0.75, trendTanh: 0.80 }; // [S407] trendTanh: trendPure 전용 압축계수. tanh(1.15)는 rawScore/BT 공유라 불변, 추세방향만 0.80으로 완화해 강세 90+ 몰림을 펼침(변별력)
...
function scoreMomentum(rows, tf, lookback) {
  ...
  return {
    ...
    _normV: 7, // [S357~S360] 점수 계산 버전 (정규화·점수로직 변경 시 +1 → 구버전 캐시 자동 재계산). v5=[S360] 크로스 전환조건 / v6=[S401] trendScore=trendPure / v7=[S407] trendTanh 0.80 (강세 변별)
  };
}
