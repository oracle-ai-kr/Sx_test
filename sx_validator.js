...
(function () {
  ...
  function buildContext(rows, opt) {
    ...
    return { rows: rows, n: rows.length, s: s, l: l, close: close, vol: vol, high: high, low: low, maS: maS, maL: maL, obv: obv, obvSig: obvSig, macdHist: macdHist, rsi: rsi, rsiSig: rsiSig, ltAlign: ltAlign, ltTrip: ltTrip };
  }
  ...
  var PREDICTORS = {
    ...
    // ── [S708] 조합 테스트 — C upside 11조건 중 측정가능 8개를 C 실제 maxW로 가중합 → 임계 점화. ──
    //  "조합이 최고 개별재료보다 lift 높은가?"(C 레시피가 일하는가)를 직접 시험. 미포함: 정배열12·ADX11·밴드워킹6(=29점, buildContext 확장 필요).
    //  측정가능 가중합 = 85점(골든상태12+가격>MA10+HH10+저항14+OBV12+거래량13+MACD8+RSI6). 기본 임계 45 ≈ C게이트(60/114)를 85로 환산.
    upsideMix: {
      id: 'upsideMix', label: 'C상승 조합(8조건)',
      fire: function (c, t, p) {
        var thr = (p && p.mixThr != null) ? p.mixThr : 45;
        var P = PREDICTORS, s = 0;
        // 골든크로스 상태(12): 단기MA>장기MA (dc=반전)
        if (_g(c.maS[t]) && _g(c.maL[t]) && ((p.dir === 'dc') ? (c.maS[t] < c.maL[t]) : (c.maS[t] > c.maL[t]))) s += 12;
        if (P.closeVsMa.fire(c, t, p)) s += 10;   // 가격>이평선
        if (P.hhBreak.fire(c, t, p)) s += 10;     // 고점 갱신
        if (P.resBreak.fire(c, t, p)) s += 14;    // 저항/신고가 돌파
        if (P.obv.fire(c, t, p)) s += 12;         // OBV 수급
        if (P.volSurge.fire(c, t, p)) s += 13;    // 거래량 동반(방향무관)
        if (P.macdCross.fire(c, t, p)) s += 8;    // MACD 모멘텀(>0)
        if (P.rsi50.fire(c, t, p)) s += 6;        // RSI 강세
        return s >= thr;
      }
    }
  };
