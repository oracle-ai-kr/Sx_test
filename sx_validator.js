...
//  [S710] 정배열/ADX/밴드워킹 + AND조합 + upsideMix(11조건 114점)  ·  [S714] 게이트 ON/OFF 배리어효용(precOff/gateGap/gateLift) — 신호를 게이트로 본 ON↔OFF 직접비교
//  [S715] 엔진결합 어댑터(aGate/aBuy/momUp/aGateMom) — 봉별 scrQuickScore/scoreMomentum 메모이즈(룩어헤드0). #5 rawScore게이트·#6 모멘텀 측정. 캡+minWarm 권장.
//  [S717] 반등 2축(aReady=①반등준비·aEntry=②반등전환·aRebound=C 반등경로·aGateRebound=H1 반등매수) — readyScore/entryScore qs서 무료. 베이스 6축 감사 완성.
//  [S718] 재료 감사(ablation): readyScore 조건별 분해 미러(엔진 의존0·자가검증) + rdyAb0~15(재료 빼면 축≥60?). 재료 ON/OFF로 레시피 평가.
//  ────────────────────────────────────────────────────────────────────────
...
(function () {
  ...
  var PREDICTORS = {
    ...
    // ── [S715] 엔진결합 어댑터 신호 (봉별 scrQuickScore/scoreMomentum · _engAt 메모이즈 · 기본 OFF · 캡+warmup 권장) ──
    //   순수수학 신호와 달리 A엔진을 직접 호출 → C가 실제 쓰는 게이트와 100% 일치(평탄화 함정 회피). SXE 미로드면 비발화.
    aGate: {        // #5 — C가 쓰는 A 타이밍 게이트(rawScore≥buyTh+15, S514 · sx_project_c:353)
      id: 'aGate', label: '⚙A게이트(진입발화)', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.aTimingOn); }
    },
    aBuy: {         // 안전필터까지 통과한 풀 A매수(action==='BUY') — aGate 대비용(동일 qs서 공짜)
      id: 'aBuy', label: '⚙A매수(안전통과)', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.action === 'BUY'); }
    },
    momUp: {        // #6 — 모멘텀 방향 up(scoreMomentum.direction)
      id: 'momUp', label: '⚙모멘텀 up', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.momUp); }
    },
    aGateMom: {     // A게이트 AND 모멘텀up — 4축 자리 근사
      id: 'aGateMom', label: '⚙A게이트&모멘텀', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.aTimingOn && e.momUp); }
    },
    // ── [S717] 반등 2축 (C 듀얼경로의 반등 경로 · S357). 임계 readyScore≥60·entryScore≥60·trendScore≥50 = C(_count4axisPass) 정확복제. ──
    aReady: {       // ①반등준비 — scrReadyScore≥60 (지지선 근접 + 반전캔들 등)
      id: 'aReady', label: '⚙반등준비(ready≥60)', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.readyScore >= 60); }
    },
    aEntry: {       // ②반등전환 — scrEntryScore≥60
      id: 'aEntry', label: '⚙반등전환(entry≥60)', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.entryScore >= 60); }
    },
    aRebound: {     // C의 반등 경로 전체(역추세 바닥잡기): ready≥60 & entry≥60 & trend≥50 & mom↑
      id: 'aRebound', label: '⚙반등경로(C 4조건)', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.readyScore >= 60 && e.entryScore >= 60 && e.trendScore >= 50 && e.momUp); }
    },
    aGateRebound: { // H1 — 반등 경로 AND A게이트(=C 실제 반등 매수: passCount≥4 AND aTimingOn). 약세서 rawScore 단독 대신 이거면 허용?
      id: 'aGateRebound', label: '⚙반등매수(경로&게이트)', eng: true,
      fire: function (c, t, p) { var e = _engAt(c, t, p); return !!(e && e.aTimingOn && e.readyScore >= 60 && e.entryScore >= 60 && e.trendScore >= 50 && e.momUp); }
    }
  };

  // ── [S715] 엔진결합 어댑터 — 봉 t의 A엔진(scrQuickScore)·모멘텀(scoreMomentum)을 1회 계산·ctx 캐시. ──
  //   룩어헤드 0: rows.slice(0,t+1)만 사용(sx_verdict_val.js와 동일 검증된 워크). 모든 엔진 신호가 메모를 공유 → 봉당 1회만.
  //   반환 {aTimingOn, action, momUp}. SXE 미로드(노드 테스트 등) 또는 데이터부족(_noData)이면 null → 엔진 신호 비발화.
  function _engGet() { return (typeof SXE !== 'undefined' && SXE && SXE.scrQuickScore && SXE.scoreMomentum) ? SXE : null; }
  function _engAt(ctx, t, p) {
    var E = _engGet(); if (!E) return null;
    if (!ctx._engCache) ctx._engCache = {};
    if (ctx._engCache[t] !== undefined) return ctx._engCache[t];   // 메모이즈(aGate·aBuy·momUp·aGateMom 공유)
    var out = null;
    try {
      var slice = ctx.rows.slice(0, t + 1);
      var qs = E.scrQuickScore(slice, p.tf, p.market);
      if (qs && !qs._noData) {
        var mom = E.scoreMomentum(slice, p.tf, 5);
        out = { aTimingOn: qs.aTimingOn === true, action: qs.action, momUp: !!(mom && mom.direction === 'up'),
                readyScore: +qs.readyScore || 0, entryScore: +qs.entryScore || 0, trendScore: +qs.trendScore || 0,
                _ind: qs.ind };   // [S717] 반등 점수 · [S718] _ind=재료 분해 미러용(qs.ind 참조·비용0)
      }
    } catch (_e) { out = null; }
    ctx._engCache[t] = out;
    return out;
  ...
  // ── [S718] 재료 감사 — readyScore(scrReadyScore eng:2658) 조건별 분해 미러. 라이브 엔진은 안 건드림(전부 validator). ──
  //   각 재료의 배점·충족·그 재료 빼면 점수(scoreWithout)를 봉별 산출 → 재료 ON/OFF로 레시피(축) 평가(ablation).
  //   ★자가검증: 미러 score가 실제 readyScore와 다르면(드리프트) 어댑터 비활성 → 잘못된 감사 데이터 안 냄.
  //   ⚠ scrReadyScore 수정 시 이 미러도 동기화. clamp/tanh/_maGoldenCross 전부 여기 복제(엔진 의존 0).
  function _clmp(v, lo, hi) { return Math.max(lo, Math.min(hi, v)); }
  function _smaLast(arr, p) { if (!arr || arr.length < p) return null; var s = 0; for (var i = arr.length - p; i < arr.length; i++) s += arr[i]; return s / p; }
  function _goldenX(closes, sP, lP, lb) {   // _maGoldenCross(eng:2856) 복제
    if (!closes || closes.length < lP + lb + 1) return { crossed: false, barsAgo: null };
    for (var k = 0; k <= lb; k++) {
      var end = closes.length - k;
      var sNow = _smaLast(closes.slice(0, end), sP), lNow = _smaLast(closes.slice(0, end), lP);
      var sPrev = _smaLast(closes.slice(0, end - 1), sP), lPrev = _smaLast(closes.slice(0, end - 1), lP);
  ...
  var READY_FEATS = ['RSI과매도', 'RSI과매수(감점)', 'BB하단', 'BB상단(감점)', 'BB스퀴즈', 'Stoch과매도', 'CCI과매도', '거래량극감', 'RSI다이버전스', 'OBV다이버전스', '골든크로스', '지지선근접', '심리가격대', '추세과열(감점)', '하락충분도', '구름하단'];
  function _readyBreakdown(ind) {   // scrReadyScore 미러 → {score, bd:[{key,pts,hit,scoreWithout}]}
    if (!ind) return null;
    var rsi = (ind.rsi && ind.rsi.val != null) ? ind.rsi.val : 50;
    var pctB = (ind.bb && ind.bb.pctB != null) ? ind.bb.pctB : 0.5;
    var stK = (ind.stoch && ind.stoch.k != null) ? ind.stoch.k : 50;
    var cci = (ind.cci != null) ? ind.cci : 0;
    var volOsc = (ind.volOsc != null) ? ind.volOsc : 0;
    var tPct = (ind.trend && ind.trend.pct != null) ? ind.trend.pct : 0;
    var gc = _goldenX(ind.closes, 5, 20, 5);
    var gcPts = (gc.crossed && !(ind.maAlign && ind.maAlign.bullish)) ? (gc.barsAgo <= 1 ? 10 : gc.barsAgo <= 3 ? 7 : 4) : 0;
    var pts = {
      'RSI과매도': rsi <= 20 ? 22 : rsi <= 25 ? 18 : rsi <= 30 ? 14 : rsi <= 35 ? 8 : rsi <= 40 ? 3 : 0,
      'RSI과매수(감점)': rsi >= 70 ? -15 : rsi >= 65 ? -8 : 0,
      'BB하단': pctB <= 0 ? 18 : pctB <= 0.1 ? 14 : pctB <= 0.2 ? 8 : 0,
      'BB상단(감점)': pctB >= 0.9 ? -12 : 0,
      'BB스퀴즈': (ind.squeeze && ind.squeeze.squeeze) ? 10 : 0,
      'Stoch과매도': stK <= 15 ? 10 : stK <= 20 ? 7 : stK <= 30 ? 3 : 0,
      'CCI과매도': cci <= -200 ? 8 : cci <= -100 ? 5 : 0,
      '거래량극감': volOsc < -40 ? 8 : volOsc < -20 ? 4 : 0,
      'RSI다이버전스': (ind.rsi && ind.rsi.div === 'bullish') ? 12 : 0,
      'OBV다이버전스': (ind.obv && ind.obv.div === 'bullish') ? 10 : 0,
      '골든크로스': gcPts,
      '지지선근접': (ind.trend && ind.trend.struct && ind.trend.struct.nearSupport) ? 8 : 0,
      '심리가격대': (ind.psychLevel && ind.psychLevel.near) ? 4 : 0,
      '추세과열(감점)': tPct >= 15 ? -12 : tPct >= 10 ? -6 : 0,
      '하락충분도': tPct <= -15 ? 10 : tPct <= -10 ? 6 : 0,
      '구름하단': (ind.ichimoku && ind.ichimoku.priceVsCloud === 'below') ? 5 : 0
    };
    var s = 0, i; for (i = 0; i < READY_FEATS.length; i++) s += pts[READY_FEATS[i]];
    var map = function (sv) { var r = _clmp(sv + 50, 0, 100); return Math.round(_clmp(50 + 50 * Math.tanh(((r - 50) / 50) * 1.2), 0, 100)); };
    var score = map(s), bd = [];
    for (i = 0; i < READY_FEATS.length; i++) { var k = READY_FEATS[i], pp = pts[k]; bd.push({ key: k, pts: pp, hit: pp !== 0, scoreWithout: map(s - pp) }); }
    return { score: score, bd: bd };
  }

  // [S722] _maDeadCross(eng:2872) 복제 — upside 데드크로스 게이트용
  function _deadX(closes, sP, lP, lb) {
    if (!closes || closes.length < lP + lb + 1) return { crossed: false, barsAgo: null };
    for (var k = 0; k <= lb; k++) {
      var end = closes.length - k;
      var sNow = _smaLast(closes.slice(0, end), sP), lNow = _smaLast(closes.slice(0, end), lP);
      var sPrev = _smaLast(closes.slice(0, end - 1), sP), lPrev = _smaLast(closes.slice(0, end - 1), lP);
      if (sNow != null && lNow != null && sPrev != null && lPrev != null) { if (sPrev >= lPrev && sNow < lNow) return { crossed: true, barsAgo: k }; }
    }
    return { crossed: false, barsAgo: null };
  }

  // [S722] scrTrendPure(eng:2617) 미러 → {score, bd}. 6요소 합 t(=eng t-50), score=map(t). 기여 부호有(빼면 ↑/↓ 둘 다 가능).
  var TREND_FEATS = ['20일방향', '기울기', 'MA배열', 'ADX', '구조위치', 'SAR'];
  function _trendBreakdown(ind) {
    if (!ind) return null;
    var tr = ind.trend || {}, ma = ind.maAlign || {}, adx = ind.adx || {};
    var _dir = _clmp((tr.pct != null ? tr.pct : 0) * 4.0, -28, 28);
    var _slp = _clmp((tr.slope != null ? tr.slope : 0) * 18.0, -18, 18);
    var _ma = 0;
    if (ma.bullish) _ma = 16; else if (ma.bearish) _ma = -16; else if (ma.short != null && ma.mid != null && ma.short > ma.mid) _ma = 6;
    var adxV = (adx.adx != null ? adx.adx : 0), pdi = adx.pdi, mdi = adx.mdi;
    var adxStr = _clmp((adxV - 20) * 0.7, 0, 20), _adx = 0;
    if (pdi != null && mdi != null) { if (pdi > mdi) _adx = adxStr; else if (mdi > pdi) _adx = -adxStr; }
    var sp = (tr.struct && tr.struct.pos != null) ? tr.struct.pos : null, _st = 0;
    if (sp != null) _st = _clmp((sp - 0.5) * 20, -10, 10);
    var _sar = 0;
    if (ind.psar && ind.psar.trend === 'up') _sar = 4; else if (ind.psar && ind.psar.trend === 'down') _sar = -4;
    var pts = { '20일방향': _dir, '기울기': _slp, 'MA배열': _ma, 'ADX': _adx, '구조위치': _st, 'SAR': _sar };
    var sumC = _dir + _slp + _ma + _adx + _st + _sar;   // = eng t - 50
    var map = function (t) { return Math.round(_clmp(50 + 50 * Math.tanh((t / 50) * 0.80), 0, 100)); };
    var score = map(sumC), bd = [], i;
    for (i = 0; i < TREND_FEATS.length; i++) { var k = TREND_FEATS[i], pp = pts[k]; bd.push({ key: k, pts: Math.round(pp), hit: pp !== 0, scoreWithout: map(sumC - pp) }); }
    return { score: score, bd: bd };
  }

  // [S722] scrUpsideScore(eng:2898) 미러 → {score, bd}. s=Σ기여(부호有), score=round(clamp(50+50*tanh((s/45)*0.80),0,100)).
  var UPSIDE_FEATS = ['역배열게이트', '저점하락LL', '데드크로스', '정배열', '가격>이평선', '골든크로스', 'ADX방향', '기울기', '20일추세', '고점갱신HH', '저항돌파', '신고가돌파', '밴드워킹', 'OBV수급', '거래량정배열', '돌파거래량', 'AD축적', 'MACD모멘텀', 'RSI구간', 'TRIX'];
  function _upsideBreakdown(ind) {
    if (!ind) return null;
    var price = ind.price, ma = ind.maAlign || {}, trend = ind.trend || {}, adx = ind.adx || {}, closes = ind.closes, pts = {};
    pts['역배열게이트'] = ma.bearish ? -30 : 0;
    pts['저점하락LL'] = (ind.swingStruct && ind.swingStruct.lowerLows) ? -14 : 0;
    var dc = _deadX(closes, 5, 20, 5);
    pts['데드크로스'] = (dc.crossed && !ma.bearish) ? -(dc.barsAgo <= 1 ? 12 : dc.barsAgo <= 3 ? 8 : 5) : 0;
    pts['정배열'] = ma.bullish ? 12 : ((ma.short != null && ma.mid != null && ma.short > ma.mid) ? 6 : 0);
    var _pma = 0; if (ma.mid != null && price > ma.mid) _pma += 6; if (ma.long != null && price > ma.long) _pma += 4;
    pts['가격>이평선'] = _pma;
    var gc = _goldenX(closes, 5, 20, 5);
    pts['골든크로스'] = (gc.crossed && !ma.bullish) ? (gc.barsAgo <= 1 ? 12 : gc.barsAgo <= 3 ? 8 : 5) : 0;
    var adxV = (adx.adx != null ? adx.adx : 0), pdi = adx.pdi, mdi = adx.mdi, _ax = 0;
    if (adxV >= 30 && pdi != null && mdi != null && pdi > mdi) _ax = 11;
    else if (adxV >= 25 && pdi != null && mdi != null && pdi > mdi) _ax = 10;
    else if (adxV >= 20 && pdi != null && mdi != null && pdi > mdi) _ax = 5;
    if (adxV >= 25 && pdi != null && mdi != null && mdi > pdi) _ax -= 8;
    pts['ADX방향'] = _ax;
    pts['기울기'] = ((trend.slope != null ? trend.slope : 0) > 0) ? 4 : 0;
    var tpct = (trend.pct != null ? trend.pct : 0), _t20 = 0;
    if (tpct > 3 && tpct < 25) _t20 = 4; else if (tpct >= 35) _t20 = -8;
    pts['20일추세'] = _t20;
    pts['고점갱신HH'] = (ind.swingStruct && ind.swingStruct.higherHighs) ? 10 : 0;
    var stt = trend.struct || {}, _res = 0;
    if (stt.nearResistance && (trend.slope != null ? trend.slope : 0) > 0) _res = 8;
    else if (stt.pos != null && stt.pos > 0.7) _res = 4;
    pts['저항돌파'] = _res;
    pts['신고가돌파'] = (stt.hi != null && price >= stt.hi) ? 6 : 0;
    var pctB = (ind.bb && ind.bb.pctB != null) ? ind.bb.pctB : 0.5, _bw = 0;
    if (closes && closes.length >= 2) { var rising = closes[closes.length - 1] > closes[closes.length - 2]; if (pctB >= 0.8 && pctB <= 1.05 && rising) _bw = 6; else if (pctB > 1.05) _bw = 2; }
    pts['밴드워킹'] = _bw;
    pts['OBV수급'] = (ind.obv && ind.obv.trend === 'up') ? 12 : ((ind.obv && ind.obv.trend === 'down') ? -6 : 0);
    var vMA = ind.volumeMA || {};
    pts['거래량정배열'] = (vMA.arrangement === 'bullish') ? 5 : 0;
    pts['돌파거래량'] = (vMA.breakout && closes && closes.length >= 2 && closes[closes.length - 1] > closes[closes.length - 2]) ? 8 : 0;
    pts['AD축적'] = (ind.ad != null && ind.chaikinOsc && ind.chaikinOsc.value > 0) ? 3 : 0;
    var _mac = 0;
    if (ind.macd && ind.macd.hist > 0 && ind.macd.line > ind.macd.sig) _mac = 8;
    else if (ind.macd && ind.macd.hist > 0) _mac = 4;
    else if (ind.macd && ind.macd.hist < 0) _mac = -6;
    pts['MACD모멘텀'] = _mac;
    var rsi = (ind.rsi && ind.rsi.val != null) ? ind.rsi.val : 50, _rsi = 0;
    if (rsi >= 50 && rsi < 70) _rsi = 6; else if (rsi >= 70 && rsi < 80) _rsi = 3; else if (rsi >= 80) _rsi = -8; else if (rsi < 40) _rsi = -6;
    pts['RSI구간'] = _rsi;
    pts['TRIX'] = (ind.trix && ind.trix.signal != null && ind.trix.value > ind.trix.signal) ? 2 : 0;
    var s = 0, i; for (i = 0; i < UPSIDE_FEATS.length; i++) s += pts[UPSIDE_FEATS[i]];
    var map = function (sv) { return Math.round(_clmp(50 + 50 * Math.tanh((sv / 45) * 0.80), 0, 100)); };
    var score = map(s), bd = [];
    for (i = 0; i < UPSIDE_FEATS.length; i++) { var k = UPSIDE_FEATS[i], pp = pts[k]; bd.push({ key: k, pts: pp, hit: pp !== 0, scoreWithout: map(s - pp) }); }
    return { score: score, bd: bd };
  }

  // [S723] scrEntryScore(eng:2739) 미러 → {score, bd}. 13재료(시계열: rsiArr/hist 배열·Stoch prevK/prevD·캔들패턴·S524컨플루언스). map=ready와 동일(s+50→tanh1.2).
  var ENTRY_FEATS = ['RSI반등', 'MACD전환', 'BB반등', 'Stoch교차', '캔들반전', '컨플루언스', 'OBV수급', '거래량패턴', 'RSI다이버전스', 'OBV다이버전스', 'Chaikin양전환', 'TRIX상향', '추세과열(감점)'];
  function _entryBreakdown(ind) {
    if (!ind) return null;
    var closes = ind.closes, pts = {};
    var _rsiC = 0, rsiArr = ind.rsi && ind.rsi.arr;
    if (rsiArr && rsiArr.length >= 3) {
      var cur = rsiArr[rsiArr.length - 1]; if (cur == null) cur = 50;
      var prev = rsiArr[rsiArr.length - 2]; if (prev == null) prev = 50;
      var prev2 = rsiArr[rsiArr.length - 3]; if (prev2 == null) prev2 = 50;
      if (prev <= 30 && cur > prev) _rsiC += 18;
      else if (prev <= 35 && cur > prev && prev2 >= prev) _rsiC += 12;
      else if (cur > prev && cur <= 45 && cur >= 30) _rsiC += 6;
      if (prev >= 70 && cur < prev) _rsiC -= 12;
    }
    pts['RSI반등'] = _rsiC;
    var _macC = 0, hist = ind.macd && ind.macd.arr && ind.macd.arr.hist;
    if (hist && hist.length >= 3) {
      var h1 = hist[hist.length - 1] || 0, h2 = hist[hist.length - 2] || 0, h3 = hist[hist.length - 3] || 0;
      if (h1 > 0 && h2 <= 0) _macC += 20;
      else if (h1 > h2 && h2 > h3 && h1 < 0) _macC += 12;
      else if (h1 > h2 && h1 > 0) _macC += 6;
      if (h1 < 0 && h2 >= 0) _macC -= 15;
    }
    pts['MACD전환'] = _macC;
    var pctB = (ind.bb && ind.bb.pctB != null) ? ind.bb.pctB : 0.5, _bbC = 0;
    if (closes && closes.length >= 3) {
      var n = closes.length;
      if (pctB <= 0.15 && pctB >= 0 && closes[n - 1] > closes[n - 2]) _bbC += 14;
      if (pctB >= 0.9 && closes[n - 1] < closes[n - 2]) _bbC -= 10;
    }
    pts['BB반등'] = _bbC;
    var stK = (ind.stoch && ind.stoch.k != null) ? ind.stoch.k : 50, stD = (ind.stoch && ind.stoch.d != null) ? ind.stoch.d : 50;
    var stPK = (ind.stoch && ind.stoch.prevK != null) ? ind.stoch.prevK : 50, stPD = (ind.stoch && ind.stoch.prevD != null) ? ind.stoch.prevD : 50, _stC = 0;
    if (stK < 30 && stK > stD && stPK <= stPD) _stC += 14;
    else if (stK < 30 && stK > stD) _stC += 8;
    if (stK > 70 && stK < stD && stPK >= stPD) _stC -= 12;
    pts['Stoch교차'] = _stC;
    var _cdC = 0;
    if (ind.candle && ind.candle.bullish && ind.candle.patterns) {
      var revN = ['망치형', '역망치형', '장악형', '관통형', '샛별형', '도지', '잠자리도지', '비석도지'];
      var hasRev = ind.candle.patterns.some(function (p) { return revN.some(function (nm) { return p.name && p.name.indexOf(nm) >= 0; }); });
      _cdC += hasRev ? 10 : 5;
    }
    if (ind.candle && ind.candle.bearish) _cdC -= 6;
    pts['캔들반전'] = _cdC;
    var _cfC = 0, _stStruct = ind.trend && ind.trend.struct;
    if (_stStruct) {
      var _pats = (ind.candle && ind.candle.patterns) || [];
      var _hbr = _pats.some(function (p) { return p.dir > 0 && p.score >= 7; });
      var _hbj = _pats.some(function (p) { return p.dir < 0 && p.score >= 7; });
      if (_stStruct.nearSupport && _hbr) _cfC += 8;
      if (_stStruct.nearResistance && _hbj) _cfC -= 8;
    }
    pts['컨플루언스'] = _cfC;
    var _obC = 0;
    if (ind.obv && ind.obv.trend === 'up') _obC += 6;
    if (ind.obv && ind.obv.trend === 'down') _obC -= 4;
    pts['OBV수급'] = _obC;
    var _vpC = 0;
    if (ind.volPattern && ind.volPattern.bullish) _vpC += 8;
    if (ind.volPattern && ind.volPattern.bearish) _vpC -= 6;
    pts['거래량패턴'] = _vpC;
    pts['RSI다이버전스'] = (ind.rsi && ind.rsi.div === 'bullish') ? 8 : 0;
    pts['OBV다이버전스'] = (ind.obv && ind.obv.div === 'bullish') ? 6 : 0;
    pts['Chaikin양전환'] = (ind.chaikinOsc && ind.chaikinOsc.value > 0 && ind.chaikinOsc.prev <= 0) ? 6 : 0;
    pts['TRIX상향'] = (ind.trix && ind.trix.signal && ind.trix.value > ind.trix.signal) ? 4 : 0;
    var trendPct = (ind.trend && ind.trend.pct != null) ? ind.trend.pct : 0, _htC = 0;
    if (trendPct >= 20) _htC -= 15; else if (trendPct >= 10) _htC -= 6;
    pts['추세과열(감점)'] = _htC;
    var s = 0, i; for (i = 0; i < ENTRY_FEATS.length; i++) s += pts[ENTRY_FEATS[i]];
    var map = function (sv) { var r = _clmp(sv + 50, 0, 100); return Math.round(_clmp(50 + 50 * Math.tanh(((r - 50) / 50) * 1.2), 0, 100)); };
    var score = map(s), bd = [];
    for (i = 0; i < ENTRY_FEATS.length; i++) { var k = ENTRY_FEATS[i], pp = pts[k]; bd.push({ key: k, pts: pp, hit: pp !== 0, scoreWithout: map(s - pp) }); }
    return { score: score, bd: bd };
  }
  function _rdyBdAt(ctx, t, p) {   // 봉별 메모이즈 + 드리프트 가드(미러 score ≠ 실제 readyScore면 null=비활성)
    var e = _engAt(ctx, t, p); if (!e || !e._ind) return null;
    if (!ctx._rdyBdCache) ctx._rdyBdCache = {};
    if (ctx._rdyBdCache[t] !== undefined) return ctx._rdyBdCache[t];
    var out = null;
    try { var r = _readyBreakdown(e._ind); if (r && r.score === e.readyScore) out = r.bd; } catch (_e) { out = null; }
    ctx._rdyBdCache[t] = out;
    return out;
  }
  function _rdyFeat(bd, key) { if (!bd) return null; for (var i = 0; i < bd.length; i++) if (bd[i].key === key) return bd[i]; return null; }
  // ── [S718] ablation 신호 등록 — 'ready−<재료>' = 그 재료 빼면 readyScore 여전히 ≥60인가. ──
  //   aReady(전체 ready≥60) lift와 비교 → ablation lift ≥ baseline = 그 재료는 죽은무게/해로움. 미러 드리프트면 비발화.
  READY_FEATS.forEach(function (k, idx) {
    var id = 'rdyAb' + idx;
    PREDICTORS[id] = {
      id: id, label: k, eng: true, ablate: true, axis: 'ready', _featKey: k,   // [S719] 순수 재료명 · [S720] axis=소속 축(cascade)
      fire: function (c, t, p) { var f = _rdyFeat(_rdyBdAt(c, t, p), k); return !!(f && f.scoreWithout >= 60); }
    };
  });
  ...
  function run(rows, rulerId, predIds, opt) {
    ...
    var p = { dir: dir, N: opt.N || (ruler.type === 'event' ? 3 : 5), thr: opt.thr, tp: opt.tp, sl: opt.sl, regime: opt.regime, brkN: opt.brkN, mixThr: opt.mixThr, tf: opt.tf, market: opt.market, minWarm: opt.minWarm };
  ...
  function _accumulate(ctx, ruler, preds, p, cap, acc) {
    ...
    var elig = [];
    for (var i = Math.max(warm, p.minWarm || 0); i <= n - 1 - p.N; i++) { if (ruler.eligible(ctx, i, p) && _regimeOk(ctx, i, p)) elig.push(i); }   // [S715] minWarm: 엔진 신호(scrQuickScore 이력) 보장 · 기본 0=기존동일
  ...
  function _finalize(acc, preds, ruler, p, pooled) {
    ...
    var rows = preds.map(function (pr) {
      ...
      // ── [S714] 게이트 ON/OFF 배리어효용 ──
      //   신호를 "게이트"로 봤을 때: 켤 때(ON=발화 봉) vs 끌 때(OFF=여집합 봉)의 사건율(승률) 비교.
      //   precOff/nOff/gateGap/gateLift는 전부 기존 누산기(eligN·eligHit, s.n·s.hit)에서 유도 → _accumulate 무변경 = 리그레션 안전.
      //   왜 lift로 부족: lift=prec/baseRate인데 baseRate는 ON봉을 포함한 가중평균. 게이트가 거의 항상 발화하면 baseRate≈prec→lift≈1이라
      //   OFF가 나빠도 안 보임. 게이트 효용은 gateLift(=ON÷OFF 직접비교)로 봐야 정확. gateGap(=ON−OFF, %p)은 precOff=0일 때도 안전(=prec).
      var nOff = acc.eligN - s.n;                                       // 게이트 OFF(미발화) 적격 봉수
      var hitOff = acc.eligHit - s.hit;                                 // 그중 사건 발생
      var precOff = nOff > 0 ? hitOff / nOff : null;                    // 끌 때 사건율(승률)
      var gateGap = (prec != null && precOff != null) ? (prec - precOff) : null;          // ON−OFF (사건율 차, ×100=%p)
      var gateLift = (prec != null && precOff != null && precOff > 0) ? (prec / precOff) : null;  // ON÷OFF 배수(precOff=0이면 null→gateGap로 판정)
      return {
        ...
        precOff: precOff, nOff: nOff, gateGap: gateGap, gateLift: gateLift   // [S714] 게이트 ON/OFF 효용
      };
    });
  ...
  var SXUV = {
    ...
    readyBreakdown: _readyBreakdown,   // [S721] C-IC 연동: 봉별 ready 재료 분해(scoreWithout) 외부 노출
    READY_FEATS: READY_FEATS,
    upsideBreakdown: _upsideBreakdown, // [S722] 추가상승 재료 분해
    UPSIDE_FEATS: UPSIDE_FEATS,
    trendBreakdown: _trendBreakdown,   // [S722] 추세방향 재료 분해
    TREND_FEATS: TREND_FEATS,
    entryBreakdown: _entryBreakdown,   // [S723] 반등전환 재료 분해
