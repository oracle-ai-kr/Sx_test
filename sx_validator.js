//  sx_validator.js — [S704] 범용 검증기 (SXVAL)
//  ────────────────────────────────────────────────────────────────────────
...
(function () {
  ...
  function buildContext(rows, opt) {
    ...
    return { rows: rows, n: rows.length, s: s, l: l, close: close, vol: vol, maS: maS, maL: maL, obv: obv, obvSig: obvSig, macdHist: macdHist, rsi: rsi, rsiSig: rsiSig };
  }
  ...
  var RULERS = {
    ...
    drawdown: {
      id: 'drawdown', label: '미래 하락(MDD)', type: 'return', polarity: 'bear',
      warm: function (ctx) { return ctx.l + 1; },
      eligible: function (ctx, t) { return ctx.close[t] > 0; },
      occurred: function (ctx, t, p) {
  ...
  function run(rows, rulerId, predIds, opt) {
    ...
    var p = { dir: dir, N: opt.N || (ruler.type === 'event' ? 3 : 5), thr: opt.thr };
  ...
  function runPool(rowsList, rulerId, predIds, opt) {
    ...
    var out = _finalize(acc, preds, ruler, { dir: dir, N: opt.N || (ruler.type === 'event' ? 3 : 5), thr: opt.thr }, true);
    out.stocks = used;
  ...
  function _finalize(acc, preds, ruler, p, pooled) {
    ...
    return { ruler: ruler.id, rulerLabel: ruler.label, type: ruler.type, dir: p.dir, N: p.N, thr: p.thr, baseRate: baseRate, totalEvent: totalEvent, nTests: acc.eligN, eligTotal: acc.eligTotal, pooled: !!pooled, rows: rows };
  }
  ...
  var SXVAL = {
    ...
    // 확장: 자/신호 하나씩 추가
    registerRuler: function (def) { if (def && def.id) RULERS[def.id] = def; return SXVAL; },
    registerPredictor: function (def) { if (def && def.id) PREDICTORS[def.id] = def; return SXVAL; },
  ...
  if (typeof module !== 'undefined' && module.exports) module.exports = SXVAL;
  if (typeof window !== 'undefined') window.SXVAL = SXVAL;
  else if (typeof globalThis !== 'undefined') globalThis.SXVAL = SXVAL;
})();
