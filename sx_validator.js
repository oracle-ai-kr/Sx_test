...
(function () {
  ...
  function run(rows, rulerId, predIds, opt) {
    ...
    var p = { dir: dir, N: opt.N || (ruler.type === 'event' ? 3 : 5), thr: opt.thr, tp: opt.tp, sl: opt.sl, regime: opt.regime, brkN: opt.brkN };
  ...
  function runPool(rowsList, rulerId, predIds, opt) {
    ...
    var out = _finalize(acc, preds, ruler, { dir: dir, N: opt.N || (ruler.type === 'event' ? 3 : 5), thr: opt.thr, tp: opt.tp, sl: opt.sl, regime: opt.regime, brkN: opt.brkN }, true);
    out.stocks = used;
  ...
  function _finalize(acc, preds, ruler, p, pooled) {
    ...
    return { ruler: ruler.id, rulerLabel: ruler.label, type: ruler.type, dir: p.dir, N: p.N, thr: p.thr, tp: p.tp, sl: p.sl, regime: p.regime, brkN: p.brkN, baseRate: baseRate, totalEvent: totalEvent, nTests: acc.eligN, eligTotal: acc.eligTotal, pooled: !!pooled, rows: rows };
  }
