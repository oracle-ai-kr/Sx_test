...
(function(global){
  ...
  function _count4axisPass(scores, momDir){
    if(!scores) return 0;
    // 반등 경로 (역추세)
    var rebound = 0;
    ...
    if(scores.maAlignBull === true)     trendPath++;
    if(momDir === 'up')                 trendPath++;
    return Math.max(rebound, trendPath);
  }

  // [S357] 합격 경로 판별 — 화면 해석용 ('rebound'|'trend'|'none')
  function _passPathOf(scores, momDir){
  ...
  function _safetyCapIndex(safetyViol){
    ...
    var idx = 0;
    for(var i=0;i<safetyViol.length;i++){
      var capI = (_safetyCapLevel(safetyViol[i]) === 'watch') ? 2 : 1;
      if(capI > idx) idx = capI;
    }
    return idx;
  }
  ...
  function unifiedVerdictV2(btState, scores, momentum, btStateObj){
    ...
    // [S361] 안전필터 위반 목록 (scrQuickScore._safetyViol → _scores4.safetyViol로 주입). 진입 후보의 모든 🔒 사유.
    ...
    // ─── 2) 4축 1차 판정 (모멘텀 포함) ───
    var passCount = _count4axisPass(scores, momDir);
    ...
    // [S361] 보유중 안전필터 과열 → '부분 익절 고려' 통합.
    //   · TP/SL 힌트가 없을 때만 (TP/SL 우선). 'watch'(관망) 등급 위반만 트리거.
    //   · 저항근접·MA60저항('interest') 같은 돌파 여지 위반은 보유 유지 (익절 권고 안 함).
    //   · [S362] 진입 전용 항목(임계값·변동성과다 = rawScore 마진 부족)은 제외.
    //     위반 수집 게이트 제거([S362]) 후 저점수 보유 종목이 임계값을 수집하므로,
