// ════════════════════════════════════════════════════════════
//  SIGNAL X — Project C Module v1.5
//
...
//  └─ B (runBtEngine)         ─┘
//
//  [v1.5 변경] buy_signal × watch → '관망' 재분류
//    · "관심" 의미 단일화: A긍정×B대기 (선행 포착)만 해당
//    · LG화학 유형 (A경계×B매수) = '관망'으로 분리
// ════════════════════════════════════════════════════════════
(function(global){
  'use strict';
  ...
  //      · "관심"의 의미 단일화: 'A긍정 × B대기'(선행 포착)로만 사용
  //      · 'A경계 × B매수'(LG화학 유사 후행 경고)는 '관망'으로 이동 → 발굴 목적에서 분리
  //
  //   btState:     'buy_signal'|'holding'|'sell_signal'|'waiting'
  //   svJudge:     supervisorJudge 결과 6종
  //   btScore:     BT 전략점수 (empty에서 보조 게이트용, nullable)
  //   btStateObj:  stock._btState 원본 (매도 시 pnl/isWin 등 맥락, nullable)
  ...
  function unifiedVerdict(btState, svJudge, btScore, btStateObj) {
    ...
    if (btState === 'buy_signal') {
      if (_aPos) {
        ...
        var _buyLabel = (svJudge === 'buy_ready') ? 'BT 매수 + 상승 전이중'
                       ...
                       : 'BT 매수 + A 보유권';
        return {action:'매수', icon:'🟢', color:'#22c55e', chartMarker:'buy', label:_buyLabel};
      }
      if (_aNeu) {
        // [v1.5] 관망 — B는 매수 신호지만 A는 경계 (후행 경고, LG화학 유사 유형)
        //   이전: '관심'으로 분류했으나 "A긍정×B대기"의 선행 포착과 의미가 섞여 '관망'으로 분리
        return {action:'관망', icon:'🟡', color:'#f59e0b', chartMarker:null, label:'BT 매수 + A 경계 — 함정 가능성, 관망'};
      }
      // 회피 — B 매수 신호지만 A가 부정 (LG화학 유형)
      return {action:'회피', icon:'⚪', color:'#9e9e9e', chartMarker:null, label:'BT 매수 신호 — A 하락 경고로 진입 보류'};
    }
    ...
    if (btState === 'holding') {
      ...
      if (_aNeu)
        return {action:'청산 준비', icon:'🟡', color:'#f59e0b', chartMarker:null, label:'청산 준비 — A 하락 조짐'};
      if (svJudge === 'avoid')
        return {action:'청산 검토', icon:'🟠', color:'#ff8c00', chartMarker:null, label:'청산 검토 — A 하락 진행'};
    ...
    if (_aPos) {
      ...
      if (_btWeak)
        return {action:'관망', icon:'🟡', color:'#f59e0b', chartMarker:null, label:'A 관심 — BT 전략 약함, 함정 주의'};
      return {action:'관심', icon:'🔵', color:'#3b82f6', chartMarker:null, label:'A 긍정 — 타이밍 대기'};
    }
    if (_aNeu) {
      // A 경계 → 관망
      return {action:'관망', icon:'🟡', color:'#f59e0b', chartMarker:null, label:'A 경계 — 진입 보류'};
    }
