...
(function(global){
  ...
  function _count4axisPass(scores, momDir, entryGate){
    ...
    // [S726] 역배열 단기약세(MA5>20>60 미충족) rebound 차단 — 가드플래그 default-OFF · 비보유 진입만.
    ...
    //   ★엔진/BT 아닌 C 자문판정 변경 → 가드플래그로 C-IC 닫힌루프 A/B 후 커밋(S712). globalThis=worker 호환.
    if((typeof globalThis!=='undefined') && globalThis.SX_REBOUND_BEARGATE && entryGate !== false && scores.ltAlign === 'bear' && !scores.maAlignBull) rebound = 0;
    // 추세 경로 (순추세) [S357]
    var trendPath = 0;
  ...
  function _passPathOf(scores, momDir, entryGate){
    ...
    if((scores.trendScore || 0) >= 50) rebound++;
    if(momDir === 'up') rebound++;
    // [S726] 역배열 단기약세 rebound 차단 — passCount(_count4axisPass)와 표시경로 정합. 가드플래그 default-OFF.
    if((typeof globalThis!=='undefined') && globalThis.SX_REBOUND_BEARGATE && entryGate !== false && scores.ltAlign === 'bear' && !scores.maAlignBull) rebound = 0;
    var trendPath = 0;
    if((scores.upsideScore || 0) >= 60) trendPath++;
    if((scores.trendScore  || 0) >= 55) trendPath++;
