// ════════════════════════════════════════════════════════════
//  SIGNAL X — Project C Module v1.7
//
...
(function(global){
  ...
  function supervisorJudge(analScore, momentum, rr) {
    ...
    if (analScore >= 50 && analScore <= 70 && dir === 'flat') {
      // [BUGFIX] 기존: `rr >= 1.0 ? 'hold' : 'hold'` — 삼항 양쪽 값 동일, 분기 무의미
      //   레거시 v1.x 함수이며 v2.0(unifiedVerdictV2)이 메인이라 동작 영향 없음 → 정리만
      return 'hold';
    }
    // 고점 유지
   ...
   */
  function _count4axisPass(scores){
    if(!scores) return 0;
    var n = 0;
    if((scores.readyScore  || 0) >= 60) n++;
    if((scores.entryScore  || 0) >= 60) n++;
    if((scores.trendScore  || 0) >= 50) n++;
    if((scores.btScore     || 0) >= 60) n++;
    return n;
  }
  ...
  function momentumBadge(momentum){
    ...
    var deltas = {ready:rD, entry:eD, trend:tD};
    if(up >= 2 && up > down){
  ...
  function unifiedVerdictV2(btState, scores, momentum, btStateObj){
    ...
    if(btState === 'sell_signal'){
      ...
      return {
        ...
        chartMarker:'sell', label:sellLabel,
        partialHint:sellHint, momBadge:momBadge,
        passCount:_count4axisPass(scores),
        verdictBeforeShift:'매도 완료'
      };
    }

    // ─── 2) 4축 1차 판정 ───
    var passCount = _count4axisPass(scores);
    ...
    // ─── 3) 모멘텀 보정 (한 단계 승급/강등) ───
    var verdictFinal = _applyMomentumShift(verdictRaw, momDir, ladder);

    // ─── 4) 메타 조립 ───
    var m = _verdictMeta(verdictFinal);
    var labelExt = m.label;
    if(verdictFinal !== verdictRaw){
      // 보정 발생 시 라벨에 명시
      if(momDir === 'up')   labelExt = m.label + ' (모멘텀↑ 승급: ' + verdictRaw + '→' + verdictFinal + ')';
      if(momDir === 'down') labelExt = m.label + ' (모멘텀↓ 강등: ' + verdictRaw + '→' + verdictFinal + ')';
    }
    ...
    return {
      ...
      chartMarker:chartMarker, label:labelExt,
      partialHint:partialHint, momBadge:momBadge,
      passCount:passCount,
  ...
  function checkConsistency(stock, options){
    ...
    // [S215] 메시지 명확화
    //   기존 "구버전 엔진" → 사용자가 무엇을 해야 할지 모호
    //   _btResult가 있어도 rowsLength 미생성 케이스가 있어서 단정적 표현 부적합
    //   "_btResult 없음" → 진단대상은 잡혔는데 BT 결과만 빈 모순 상태 → 가이드 추가
