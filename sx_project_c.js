// ════════════════════════════════════════════════════════════
//  SIGNAL X — Project C Module v2.1  [S892] 반등축 레시피 접지(SX_CV2_AXES 토글)
//
...
(function(global){
  ...
  function _count4axisPass(scores, momDir, entryGate){
    ...
    var rebound = 0; var _cv2on=((typeof globalThis!=='undefined')&&globalThis.SX_CV2_AXES&&scores.cv2); var _rdS=_cv2on?(scores.cv2.ready||0):(scores.readyScore||0); var _enS=_cv2on?(scores.cv2.entry||0):(scores.entryScore||0);   // [S892] SX_CV2_AXES ON=레시피 접지 점수(cv2)로 반등 판정 — [S896] 기본 ON(render서 미정의 시 true·워커=미정의+cv2 미주입이라 자동 엔진 폴백). OFF(🧪 토글)=엔진 원점수 · [S903] 주석 정정
    if(_rs){
