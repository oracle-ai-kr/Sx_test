...
//
//  [v1.7 변경]
//    ③ buy_signal 강등 게이트 — "BT 매수 신호인데 BT 자체 성적이 나쁘면 관심으로 강등"
//       · 발동: btScore<40 OR (winRate<45% AND totalTrades≥5)
//       · 강등: 매수 → 관심 / chartMarker 제거 / label에 사유 명시
//       · 배경: 수젠텍 사례 — BT 승률 39%+매수 배너와 "BT 점수 낮음" 해설 모순 해결
//       · 대칭: waiting 분기의 btScore<40 강등(관심→관망)과 짝을 이룸
//    ④ btStateObj에 winRate/totalTrades 주입 (render/screener 호출부)
//       · 기존 tp/sl/entry/pnl/currentPrice에 BT 성적 2개 추가
//
//  [v1.6 변경]
  ...
  function unifiedVerdict(btState, svJudge, btScore, btStateObj) {
    ...
    if (btState === 'buy_signal') {
      if (_aPos) {
        ...
        if (_btWeakBuy) {
          // [v1.8] label 앞 '[강등]' 마커 — 렌더부가 _dirText("상승 전이중") 표시 여부 판단용
          //   같은 action='관심'이라도 (waiting+A긍정=선행 포착) vs (buy_signal+BT저신뢰=함정 경고)
          //   는 의미가 정반대 → label 마커로 강등 케이스만 식별
          return {action:'관심', icon:'🔵', color:'#3b82f6', chartMarker:null,
