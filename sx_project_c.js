...
(function(global){
  ...
  function _count4axisPass(scores, momDir, entryGate){
    ...
    var rebound = 0;
    if(_rs){
      // [S862] 표 재설계 — S861 측정: trend·mom 동반표 3시장 전부 무가치/역효과(bear 역효과 확정) + 동시율 12~26%가 S859 진입급 굶주림(54→1)의 원인 → 동반표 제거.
      //   votes(0~4)=호출측이 시장별 겹침 사다리로 계산(kr 순수 1/2/3/4+→1/2/3/4 · us 정배 순수3·겹침3+면 4/역배 상한2 · coin 1 · 혼재=0). C는 시장 무지 유지.
      //   votes 미전달(S859 구콜백 호환)=구 2표+trend+mom 폴백.
      if(typeof _rs.votes==='number'){ rebound = Math.max(0, Math.min(4, Math.round(_rs.votes))); }
      else {
        if(_rs.pure) rebound++;
        if(_rs.pure && _rs.strong) rebound++;
  ...
  function _passPathOf(scores, momDir, entryGate){
    ...
    // [S859] 레시피 반등표 A/B — _count4axisPass와 대칭(경로 라벨 정합). [S862] votes 방식 동기화.
    var _rs = ((typeof globalThis!=='undefined') && globalThis.SX_RECIPE_REBOUND && scores.recipeSig) ? scores.recipeSig : null;
    var rebound = 0;
    if(_rs){
      if(typeof _rs.votes==='number'){ rebound = Math.max(0, Math.min(4, Math.round(_rs.votes))); }
