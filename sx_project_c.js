...
(function(global){
  ...
  function _passPathOf(scores, momDir, entryGate){
    ...
    if(rebound > 0){
      // [S693] 중립(off) rebound 억제 — _count4axisPass(passCount=trendPath)와 동기화. 비보유 진입(entryGate)에만: trend 잔존 시 'trend', 없으면 'none'.
      if(entryGate !== false && scores.ltAlign !== 'bull' && scores.ltAlign !== 'bear') return trendPath > 0 ? 'trend' : 'none';
      return 'rebound';
    }
    return 'none';
  }
