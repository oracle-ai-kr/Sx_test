...
//    Phase 3-B-4f.4:  ③ 진입/매도 검토 C 전이 확률화 ← 현재
//                     (calcVerdictTransition + getTimingButtonLabel)
//
...
(function(global){
  ...
  // C 전이 확률 계산 — 현재 C 판정 + Layer 1 데이터 → 다음 상태 전이 확률(0~95)
  //   입력:
  //     currentVerdict : 9종 C 판정 문자열
  //     analScore      : 분석 점수 (0~100, readyScore 또는 score)
  //     mom            : scoreMomentum 객체 {delta, direction, history, ...} | null
  //     rr             : 손익비 (0~∞)
  //     btState        : stock._btState 객체 {pnl, isWin, state, ...} | null
  //   반환:
  //     {label, value, nextVerdict, polarity}
  //       label       : '청산 준비 전이' / '매수 전이' 등 (UI 표시용)
  //       value       : 0~95 (정수, 확률%)
  //       nextVerdict : 다음 C 상태 문자열
  //       polarity    : 'up' (매수/보유 방향) / 'down' (매도/회피 방향)
  //                     → UI 색상 규칙 결정용
  //                       up  : 낮음=회색, 중간=파랑, 높음=녹색 (기회)
  //                       down: 낮음=녹색(안전), 중간=노랑, 높음=빨강 (위험)
  function calcVerdictTransition(currentVerdict, analScore, mom, rr, btState){
    ...
    // ─── 보유자 경로 (매도 검토 버튼, polarity='down') ───
    //   높은 전이 확률 = 하락/청산 임박 = 위험 (빨강)

    if (currentVerdict === '보유 유지') {
      let prob = 0;
      if (dir === 'down') prob += 40;          // 하락 추세 40%
      if (score < 55) prob += 20;              // 점수 약세 +20%
      if (rrv < 1.0) prob += 15;               // RR 악화 +15%
      if (delta < -3) prob += 15;              // 모멘텀 급락 +15%
      if (pnl < 0) prob += 10;                 // 손실 중 +10%
      return {
        label: '청산 준비 전이',
        value: Math.min(prob, 95),
        nextVerdict: '청산 준비',
        polarity: 'down'
      };
    }

    if (currentVerdict === '청산 준비') {
      let prob = 20; // 이미 약화 신호 기본값
      if (score < 45) prob += 30;
      if (dir === 'down' && delta < -5) prob += 30;
      if (rrv < 0.5) prob += 20;
      if (pnl < -3) prob += 10;
      return {
        label: '청산 검토 전이',
    ...
    if (currentVerdict === '청산 검토') {
      let prob = 35; // 경고 단계
      if (score < 40) prob += 25;
      if (dir === 'down' && delta < -3) prob += 20;
      if (rrv < 0.5) prob += 15;
      if (pnl < -5) prob += 15;
      return {
        label: '즉시 청산 전이',
    ...
    if (currentVerdict === '즉시 청산') {
      // 즉시 청산 단계는 이미 임박 상태 → 매도 완료 전이 기본 매우 높음
      let prob = 60;
      if (dir === 'down') prob += 20;
      if (score < 35) prob += 15;
      return {
    ...
    if (currentVerdict === '매도 완료') {
      // 매도 완료 → 재진입 기회 대기 (관심 전이)
      let prob = 10;
      if (dir === 'up') prob += 25;
      if (score > 50) prob += 15;
      if (delta > 3) prob += 15;
    ...
    // ─── 비보유자 경로 (진입 검토 버튼, polarity='up') ───
    //   높은 전이 확률 = 매수/상승 임박 = 기회 (녹색)

    if (currentVerdict === '매수') {
      // 매수 판정 상태 → 보유 유지로 거의 확정 전이
      let prob = 75;
      if (dir === 'up') prob += 15;
      if (rrv >= 1.5) prob += 5;
      return {
    ...
    if (currentVerdict === '관심') {
      let prob = 30; // BT 검증됨 기본값
      if (dir === 'up' && delta > 5) prob += 35;
      if (score > 55) prob += 20;
      if (rrv > 1.5) prob += 15;
      return {
        label: '매수 전이',
    ...
    if (currentVerdict === '회피') {
      let prob = 5; // 회피 상태는 전이 저조가 기본
      if (dir === 'up' && delta > 5) prob += 25;
      if (score > 45) prob += 15;
      if (rrv > 1.0) prob += 10;
      return {
        label: '관망 전이',
  ...
  global.SXC = {
    ...
    // Layer 3.5: C 전이 확률 (Phase 3-B-4f.4 신규)
