...
function _buildAEntryLine(stock, svVerdict){
  ...
  const color  = svVerdict?.color  || 'var(--text3)';
  const BOX   = `margin-top:8px;padding:8px 10px;background:var(--surface2);border-radius:8px;border-left:3px solid ${color}`;
  const TITLE = 'font-size:11px;color:var(--text2);font-weight:700;margin-bottom:3px;letter-spacing:-.3px';
  const SUB   = 'font-size:10.5px;color:var(--text2);line-height:1.6;margin-top:2px';
  ...
  if(!isBuyGroup){
    ...
    return `<div style="${BOX}">
      <div style="${TITLE}">📍 A엔진 진입 분석</div>
      <div style="${SUB}">${_stateMsg}</div>
    </div>`;
  }
  ...
  const isKR = _mkt === 'KR' || _mkt === 'KOSPI' || _mkt === 'KOSDAQ' || (!_mkt && !(stock.code||'').endsWith('USDT'));
  ...
  return `<div style="${BOX}">
    <div style="${TITLE}">📍 A엔진 진입 분석 <span style="font-size:8px;color:var(--text3);font-weight:400">· ATR×TP${calc.tpMult}/SL${calc.slMult} · 슬리피지 0.1%</span></div>
    <div style="${SUB}">
      진입가 <b>${fmt(calc.ep)}</b>
      &nbsp;·&nbsp; 목표 <b style="color:#22c55e">${fmt(calc.tp)}</b> <span style="color:var(--text3)">(+${tpPct}%)</span>
      &nbsp;·&nbsp; 손절 <b style="color:#e8365a">${fmt(calc.sl)}</b> <span style="color:var(--text3)">(${slPct}%)</span>
    </div>
  </div>`;
}
