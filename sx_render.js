...
//   기존 displayed.map(...) 인라인 코드를 그대로 함수로 추출 (로직 변경 없음)
...
function saveSearchResults(){
  try{
    ...
    //   기존 누락 시: 브라우저 닫고 재진입하면 _svVerdict 사라져 배지 영역 통째로 숨김 + 종목 라인 아이콘이 숫자(_score)로 폴백되는 현상
  ...
  body.innerHTML = `
    ...
    ${(()=>{
      ...
      return `<div class="anal-section">
        ...
        ${(()=>{
          // [재무 종합평가 시각 카드] 다른 부문(거래량/추세신호/공시)과 동일한 형식
          //   상단 큰 글씨로 점수 + 등급 + 톤 색상 → 사용자가 한눈에 "재무가 좋은지/나쁜지" 파악
          //   기존 fi.label 토글은 그대로 유지 (상세 해석 보고 싶으면 펼침)
          if(fi.sectorScore == null) return '';  // 안전장치 — 옛 SXI.advFundamental 호환
          const toneColor = fi.tone === 'bullish' ? 'var(--buy,#26a269)'
                          : fi.tone === 'bearish' ? 'var(--sell,#e74c3c)'
                          : 'var(--text2)';
          const gradeBg = fi.tone === 'bullish' ? 'rgba(38,162,105,.12)'
                        : fi.tone === 'bearish' ? 'rgba(231,76,60,.12)'
                        : 'rgba(150,150,150,.10)';
          // 등급 한 글자 박스 (공시/거래량 카드와 동일한 형식)
          const gradeBox = `<div style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:6px;background:${gradeBg};color:${toneColor};font-size:14px;font-weight:800;margin-right:8px">${fi.sectorGrade}</div>`;
          return `<div style="margin-top:10px;padding:10px 12px;background:var(--surface2);border-radius:8px;border:1px solid var(--border)">
            <div style="display:flex;align-items:center">
              ${gradeBox}
              <div style="flex:1">
                <div style="font-size:11px;color:var(--text3);margin-bottom:1px">재무 종합평가</div>
                <div style="font-size:14px;font-weight:700;color:${toneColor}">${fi.sectorScore}점 · ${fi.label}</div>
              </div>
            </div>
            <div style="margin-top:6px;padding-top:6px;border-top:1px dashed var(--border);font-size:10px;color:var(--text2);line-height:1.5">${fi.summary}</div>
          </div>`;
        })()}
      ...
      .then(d=>{
        ...
        try {
          if(d && d.industry_code){
            ...
            //   기존: stock.industryCode 필드를 읽으려 하나 어디서도 채우지 않아 KSIC 분기가 죽은 코드였음
            //   수정: 여기서 한 번 박아두면 EV/EBITDA 금융업 비적용 판정의 KSIC 자동감지가 작동
