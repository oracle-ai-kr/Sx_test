    ...
    for(var i=0;i<use.length;i++){
      ...
      (function(rws){
        res.trades.forEach(function(t){
          ...
          if(!(ltAlign==='bear' && !maBull)) return;   // ★bear-deadcat 진입만(게이트가 막는 구간)
...
function _btDiscrimRender(rep, watch, mk){
  ...
  var out='<div style="border:1px solid var(--border);border-radius:10px;padding:10px;margin-top:8px">';
  out+='<div style="font-size:11px;font-weight:800;color:#0891b2;margin-bottom:4px">🧪 재료 변별력 · 분류기준 4종 · '+String(mk).toUpperCase()+'</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:5px">bear-deadcat 진입을 4가지로 가름: <b>손익</b>(거래 청산 손익)·<b>N3/N5/N10</b>(진입 후 그 봉수 뒤 종가>진입가). 각 재료가 어느 기준에서 진짜반등을 가르나. 데드캣은 짧게 꺾이니 <b>N별 차이 자체가 정보</b>. 대표+관심 합산.</div>';
  out+='<div style="font-size:8px;color:'+T3+';margin-bottom:6px">표본(진짜/데드): '+CRIT.map(function(c){ return c.label+' '+PC[c.key].win+'/'+PC[c.key].loss; }).join(' · ')+'</div>';
  ...
  out+='</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">읽기: <b style="color:'+GRN+'">강</b>=그 기준에서 진짜반등 잘 가름·<b style="color:'+AMB+'">중</b>=약하게·<b>약</b>=못 가름·<b>-</b>=표본부족. 한 재료가 특정 N에서만 강하면 그 시간축이 핵심. <b>손익 기준은 BT 청산 로직이 껴 노이즈</b>일 수 있어 N 기준이 더 순수. ※강만 신뢰. kNN=STEP 2.</div>';
  out+='</div>';
  return out;
}
...
function _btThreshRender(entries){
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:7px">강 수치재료의 <b style="color:'+GRN+'">진짜반등</b> vs <b style="color:'+RED+'">데드캣</b> 분포(Q1·중앙·Q3)와 최적 분리 임계값. "<b>재료 &lt;X → 진짜 Y%</b>"가 레시피 조건. N10(방향성)·N5(타이밍) 둘 다. 분리도(진짜비율 차)순.</div>';
  CR.forEach(function(C){
    ...
    out+='<div style="font-size:10px;font-weight:800;color:var(--text);margin:8px 0 4px">▸ '+C.label+' 기준 <span style="font-size:8px;color:'+T3+';font-weight:500">(진짜 '+win.length+'·데드 '+loss.length+')</span></div>';
    rows.forEach(function(r){
      ...
      if(r.bt){ var dirTxt=(r.bt.dir==='below'?'&lt; '+r.bt.X:'≥ '+r.bt.X); var goodR=(r.bt.dir==='below'?r.bt.rB:r.bt.rA); var badR=(r.bt.dir==='below'?r.bt.rA:r.bt.rB);
        out+='<span style="font-size:9px;font-weight:800;color:'+(strong?GRN:T3)+'">'+dirTxt+' → 진짜 '+goodR+'%<span style="color:'+T3+';font-weight:500"> (반대 '+badR+'%)</span></span>'; }
      out+='</div>';
      out+='<div style="display:grid;grid-template-columns:auto 1fr;gap:2px 8px;font-size:8.5px;color:'+T3+'">';
      out+='<span style="color:'+GRN+'">진짜반등</span><span style="color:var(--text2)">Q1 '+r.wQ1+' · 중앙 <b>'+r.wMed+'</b> · Q3 '+r.wQ3+'</span>';
      out+='<span style="color:'+RED+'">데드캣</span><span style="color:var(--text2)">Q1 '+r.lQ1+' · 중앙 <b>'+r.lMed+'</b> · Q3 '+r.lQ3+'</span>';
      out+='</div></div>';
    });
  });
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">읽기: <b style="color:'+GRN+'">초록 테두리</b>=분리도 큰 재료(진짜비율 차≥20%p). "재료 &lt;X → 진짜 Y%"는 그 조건으로 거른 진입의 진짜반등 비율. 중앙값이 진짜/데드 사이 크게 벌어진 재료가 좋은 조건. 여러 강 재료 AND로 묶으면 레시피.</div>';
  out+='</div>';
  return out;
}
...
function _btEnsembleRender(entries){
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:6px">서로 다른 유형의 진짜반등을 3경로로 포착 → OR 합집합. <b>AND와 달리 표본 유지하며 정밀도 오르나</b> 검증. 경로별 정밀도(진짜%)·커버리지(전체 중 비율), 합집합, 겹침.</div>';
  // 경로 정의
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">판단: 합집합(∪) <b>커버리지가 경로A보다 크게 늘면서 정밀도가 기저율보다 충분히 높으면</b>(<b style="color:'+GRN+'">+10%p↑</b>) → OR 앙상블 효과 있음(자유 빌더로 확장). 합집합 정밀도가 기저율과 비슷하면 → 약경로가 노이즈만 추가(거래량OSC 단독이 나음). 겹침에서 1경로 多=경로들이 다른 반등 포착(보완적).</div>';
  out+='</div>';
  return out;
}
...
function _btBuilderUI(){
  ...
  h+='<div style="font-size:8.5px;color:'+T3+';line-height:1.4;margin-bottom:6px">재료 켜고 임계값·방향 조정 → 조합의 진짜반등 비율·표본 즉시. AND(모두)/OR(하나라도). <b>표본 30 미만 경고</b>(과적합).</div>';
...
function _sxBuilderEval(){
  ...
  [{key:'n10',label:'N10'},{key:'n5',label:'N5'}].forEach(function(C){
    ...
    h+='<span style="font-size:10px">표본 <b style="color:'+(warn?RED:'var(--text)')+'">'+m.length+'</b><span style="color:'+T3+';font-size:8px">('+Math.round(100*cov)+'%)</span> · 진짜 <b style="color:'+pc+'">'+(prec!=null?Math.round(100*prec)+'%':'-')+'</b> <span style="font-size:8px;color:'+T3+'">기저'+Math.round(100*base)+'%'+(prec!=null?(' '+(prec-base>=0?'+':'')+Math.round(100*(prec-base))+'p'):'')+'</span></span>';
    h+='</div>';
