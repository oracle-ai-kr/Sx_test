        ...
        if(_btTradesNow < BT_MIN_TRADES && _curStage < 1 && _extSupported
          ...
          try{
            ...
            if(_extraCandles && _extraCandles.length > 0){
              ...
              //
              // 무한 루프 방지: stock._analCandlesExtendedStage = 1 이미 설정됨 (483줄)
              //                 → 재귀 진입 시 _curStage >= 1 조건으로 확장 재시도 안 함
              // ═══════════════════════════════════════════════════════
              stock._lastAnalCandles = _mergedRows.map(c => ({...c})); // 깊은 복사
              console.log(`[S112] ★ 확장 완료 — runAnalysis 재귀 호출로 분석+BT 재계산 (${_mergedRows.length}봉)`);
