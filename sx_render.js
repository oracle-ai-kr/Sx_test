// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v1.9 <!-- S79 -->
          ...
          <div class="sr-body">
            ...
            <div class="sr-row2">
              ...
              ${(()=>{
                ...
                const rc=r>=60?'var(--buy)':r>=40?'var(--accent)':'var(--text3)';
                const ec=e>=60?'var(--buy)':e>=40?'var(--accent)':'var(--text3)';
                const tc=t>=60?'var(--buy)':t>=40?'var(--accent)':'var(--text3)';
                return '<span class="sr-score-chip" style="font-size:8px"><span style="color:'+rc+'">준'+r+'</span>·<span style="color:'+ec+'">진'+e+'</span>·<span style="color:'+tc+'">추'+t+'</span></span>';
              })()}
              ${(()=>{
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _readyCls = _readySc>=60?'buy':_readySc>=40?'hold':'sell';
        const _entryCls = _entrySc>=60?'buy':_entrySc>=40?'hold':'sell';
        const _trendCls = _trendSc>=60?'buy':_trendSc>=40?'hold':'sell';
        ...
        if(_mom && _mom.history.length >= 2){
          ...
          const color = _mom.direction==='up' ? 'var(--buy,#27ae60)' : _mom.direction==='down' ? 'var(--sell,#e74c3c)' : 'var(--text3,#999)';
          const crossTag = _mom.cross==='golden' ? ' <span style="font-size:7px;background:var(--buy-bg);color:var(--buy);padding:1px 4px;border-radius:3px">50↑돌파</span>'
                         : _mom.cross==='dead' ? ' <span style="font-size:7px;background:var(--sell-bg);color:var(--sell);padding:1px 4px;border-radius:3px">50↓이탈</span>' : '';
          _momHTML = `<div style="font-size:10px;color:${color};font-weight:700;margin-top:2px">${arrow}${sign}${_mom.delta}${crossTag}</div>`;
        }

        // S79: B안 — 상세 토글 ID
        const _momDetailId = 'momDetail_' + Math.random().toString(36).slice(2,8);
        ...
        return `<div class="anal-score-wrap" style="flex-direction:column;gap:8px">
          <div style="display:flex;justify-content:center;gap:6px;width:100%">
            <div class="anal-score-box" style="flex:1">
              <div class="score-num" style="font-size:20px;color:var(--${_readyCls})">${_readySc}</div>
              <div class="score-label" style="font-size:9px">준비</div>
              <div style="font-size:7px;color:var(--text3);margin-top:1px">올라올까?</div>
            </div>
            <div class="anal-score-box" style="flex:1">
              <div class="score-num" style="font-size:20px;color:var(--${_entryCls})">${_entrySc}</div>
              <div class="score-label" style="font-size:9px">진입</div>
              <div style="font-size:7px;color:var(--text3);margin-top:1px">맞는거같아</div>
            </div>
            <div class="anal-score-box" style="flex:1">
              <div class="score-num" style="font-size:20px;color:var(--${_trendCls})">${_trendSc}</div>
              ${_momHTML}
              <div class="score-label" style="font-size:9px">추세</div>
              <div style="font-size:7px;color:var(--text3);margin-top:1px">맞았네</div>
            </div>
          </div>
          </div>
        </div>
        ${_mom && _mom.history.length >= 3 ? `
        <div style="text-align:center;margin-top:4px">
          <span style="font-size:9px;color:var(--accent);cursor:pointer" onclick="var el=document.getElementById('${_momDetailId}');el.style.display=el.style.display==='none'?'block':'none'">▶ 점수 추이 상세</span>
        </div>
        <div id="${_momDetailId}" style="display:none;margin-top:6px;padding:8px 12px;background:var(--surface2);border-radius:8px;border:1px solid var(--border)">
          <div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:6px">최근 ${_mom.lookback}봉 점수 추이</div>
          <div style="display:flex;align-items:flex-end;gap:2px;height:40px;margin-bottom:6px">
            ${(()=>{
              const rev = _mom.history.slice().reverse();
              return rev.map((h,i)=>{
                const pct = Math.max(4, h.score);
                const prev = i > 0 ? rev[i-1].score : h.score;
                const barColor = h.score > prev ? 'var(--buy,#e8365a)' : h.score < prev ? 'var(--sell,#3b82f6)' : 'var(--text3,#999)';
                return `<div style="flex:1;display:flex;flex-direction:column;align-items:center">
                  <div style="font-size:7px;color:var(--text3);margin-bottom:1px">${h.score}</div>
                  <div style="width:100%;max-width:24px;height:${pct*0.4}px;background:${barColor};border-radius:2px"></div>
                </div>`;
              }).join('');
            })()}
          </div>
          <div style="display:flex;justify-content:space-between;font-size:8px;color:var(--text3)">
            <span>${_mom.lookback-1}봉전</span>
            <span>현재</span>
          </div>
          <div style="margin-top:6px;font-size:9px;color:var(--text2);line-height:1.6">
            ${(()=>{
              const _buyTh = (typeof _scrTfTh === 'function') ? _scrTfTh(currentTF).buyTh : 62;
              const _guide = (typeof SXI!=='undefined' && SXI.scoreMomentumGuide) ? SXI.scoreMomentumGuide(_mom, _analSc, _btSc, _buyTh) : null;
              if(_guide) return _guide.text;
              const d = _mom.direction;
              const c = _mom.cross;
              let txt = `평균 ${_mom.avg}점 · 변화 ${_mom.delta>0?'+':''}${_mom.delta}점 · `;
              if(d==='up') txt += '점수 상승 추이 — 기술적 조건이 개선되고 있습니다.';
              else if(d==='down') txt += '점수 하락 추이 — 기술적 조건이 악화되고 있습니다.';
              else txt += '점수 횡보 — 뚜렷한 방향성이 없습니다.';
              if(c==='golden') txt += ' 50점 상향 돌파 구간으로, 진입 시점 접근 중입니다.';
              else if(c==='dead') txt += ' 50점 하향 이탈 구간으로, 관망이 권장됩니다.';
              return txt;
            })()}
          </div>
        </div>` : ''}`;
