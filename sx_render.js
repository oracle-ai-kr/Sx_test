...
function _cvPoolRender(rep, watch, mk){
  ...
  // [S691] 경로 게이트 실험 — bull-rebound 억제 타당성
  if((rep&&rep.ok&&rep.gate)||(watch&&watch.ok&&watch.gate)){
    ...
    out+='<div style="margin-top:11px;border-top:1px solid var(--border);padding-top:8px"><div style="font-size:10.5px;font-weight:800;color:#ef4444">🚪 경로 게이트 실험 — bull→rebound 억제 타당성</div>';
    out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin:2px 0 5px">rebound경로를 ltAlign로 분해. <b>bull-rebound 격차가 음(&lt;非bull)</b>이면 "강세 장기배열 종목은 rebound 금지" 정당. 추상IC(추가상승) +면 trend 재라우팅, −면 none(미진입).</div>';
    var gateRow=function(P,lbl){
      if(!P||!P.ok||!P.gate) return '';
      var b=P.gate.bull, nb=P.gate.nonbull;
      return '<div style="font-size:10px;margin-top:5px;line-height:1.4"><b style="color:'+PUR+'">'+lbl+'</b> · 🔵bull-reb n='+b.n+' 격차 <b style="color:'+gCol(b.gap)+'">'+gv(b.gap,'p')+'</b><span style="font-size:8px;color:'+T3+'">(진'+b.enN+'/회'+b.avN+')</span> 추상IC <span style="color:'+icCol(b.upIC)+'">'+(b.upIC==null?'—':b.upIC)+'</span><br><span style="color:'+T3+'">　　非bull-reb n='+nb.n+' 격차 <b style="color:'+gCol(nb.gap)+'">'+gv(nb.gap,'p')+'</b><span style="font-size:8px">(진'+nb.enN+'/회'+nb.avN+')</span></span></div>';
    };
    out+=gateRow(rep,'대표');
