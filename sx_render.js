...
async function _loadMoreCandles(stock){
  ...
  try{
    ...
    // runAnalysis 재호출 — UI 완전 재렌더
    //   runAnalysis는 stock._lastAnalCandles 있으면 그걸 사용하도록 로직 보강 필요
    //   여기서는 단순히 runAnalysis 재호출 → 재분석 중 자동 확장 조건(stage<1)은 만족 안 함
    //   → 기존 _existingCandles 그대로 쓰지만 우리가 이미 stock._btResult를 교체해뒀으므로
    //   → renderAnalysisResult가 최신 stock._btResult를 참조하여 600봉 기준 결과 표시
    if(typeof runAnalysis === 'function'){
      // stage 2 상태에서 runAnalysis 재실행 시 자동 확장은 skip (stage<1 조건 불충족)
      // 따라서 캔들은 다시 200봉으로 돌아가지만 stock._btResult는 600봉 결과 유지
      // UI 재렌더만 하면 됨 — 간단히 renderAnalysisResult 직접 호출
      runAnalysis(stock);
    }
  }catch(e){
