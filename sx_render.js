...
function _candleTransitionScore(rows, indicators){
  ...
  if(mode === 'count'){
    ...
    return { active:true, mode:'count', score, label, reasons, warn:null, redCnt, greenCnt, streak:0, red8:_red8, green8:_green8, counter8:_counter8 };
  }
  ...
  if(mode === 'flipUp' || mode === 'flipDown'){
    ...
    score = Math.max(-100, Math.min(100, Math.round(score)));
    const label = score >= 30 ? '양봉 전이 유망' : score <= -30 ? '음봉 전이 유망' : '중립';
    return { active:true, mode, score, label, reasons, warn, redCnt, greenCnt, streak:0, patName: pat.name, red8:_red8, green8:_green8, counter8:_counter8 };
  }
  ...
  score = Math.max(-100, Math.min(100, Math.round(score)));
  const label = score >= 30 ? '양봉 전이 유망' : score <= -30 ? '음봉 전이 유망' : '중립';
  return { active:true, mode, score, label, reasons, warn, redCnt, greenCnt, streak, red8:_red8, green8:_green8, counter8:_counter8 };
}
...
function _buildTransitionCard(stock, indicators){
  ...
  const top = `<div style="display:flex;align-items:baseline;gap:8px"><span style="font-size:26px;font-weight:800;color:${col}">${sign}${r.score}</span><span style="font-size:13px;font-weight:700;color:${col}">${r.label}</span><span style="font-size:10px;color:var(--text3);margin-left:auto">${r.mode==='rebound'?`음봉 ${r.streak}연속`:r.mode==='reversal'?`양봉 ${r.streak}연속`:r.mode==='flipUp'?`🔺 ${r.patName||'양전환'}`:r.mode==='flipDown'?`🔻 ${r.patName||'음전환'}`:`8봉 음${r.red8}·양${r.green8}`}</span></div>`;
