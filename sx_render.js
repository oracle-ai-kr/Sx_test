  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // S103-fix6c Phase2: 전이 확률 라벨 양방향 + 행동 확신 배지
        //   Layer 1(정적): 라벨 자체를 모드에 맞춰 "강세 전이" ↔ "약세 전이"로 치환
        //   Layer 2(동적): r2e(진입타이밍/매도타이밍 섹션)에만 배지 — 지금 행동하는 게 유력하다
        //     매수 모드 + r2e.prob >= 65 → [반등유력] (저점에서 올라갈 것)
        //     매도 모드 + r2e.prob >= 65 → [하락유력] (고점에서 떨어질 것)
        //   e2t(강세/약세 섹션)에는 배지 없음 — 판단 노이즈 방지
        ...
        const _txBadge = function(prob){
          ...
          // 모드 불문 prob >= 65일 때 배지 표시, 용어만 모드별 차이
          if(prob == null || prob < 65) return '';
          if(_isSellMode){
