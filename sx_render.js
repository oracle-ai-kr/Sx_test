...
//   - empty Stage: BT◯+분석◯는 경계(곧 buy_signal로 전이)=관심, BT◯+분석✗=관심(타이밍 대기), BT✗+분석◯=관망(함정 가능성), BT✗+분석✗=회피
//   - "대기" 값 완전 폐지 → "회피"로 통합 (비보유=진입안함의 의미로 동일)
//   - 매도완료: 배지 "하락유력" 부여 (차트 마커 ▼과 동기화 — badge 로직은 1276줄)
...
//   ※ Phase 3-B-7(sx_project_c.js 분리) 이후 SXC.mapVerdictToBtAction()으로 이동 예정
function _mapVerdictToBtAction(verdictAction){
...
function renderResults(){
  ...
  const _verdictMap = {'매수':'buy','관심':'interest','보유 유지':'hold','관망':'watch','청산 준비':'watch','회피':'avoid','청산 검토':'avoid','즉시 청산':'sell','매도 완료':'sell'};
  filtered.forEach(s => {
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    ...
    _svVerdict = unifiedVerdict(_btStateKey, svJudge, stock._btScore != null ? stock._btScore : null, _btSt);
    ...
    if(_svVerdict && _svVerdict.action){
      stock._btAction = _mapVerdictToBtAction(_svVerdict.action);
    } else {
    ...
    // 9) 통합 배너 렌더
    // S103-fix6c Phase2: 감독관 기반 행동 배지는 진입타이밍 버튼 위로 이동 (사용자 가시성 개선)
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // S103-fix6c Phase2: 전이 확률 라벨 양방향화 (Layer 1 정적 라벨)
        ...
        // S103-fix6c Phase2: 감독관 통합판정 기반 행동 배지 — 진입타이밍 버튼 위 왼쪽 정렬
        ...
        let _verdictBadgeTop = '';
        {
          const _svV = stock._svVerdict;
          if(_svV && _svV.action){
            const a = _svV.action;
            if(a === '매수')                                 _verdictBadgeTop = '<span class="tx-badge up">반등유력</span>';
            else if(a === '관심')                            _verdictBadgeTop = '<span class="tx-badge up-soft">반등조짐</span>';
            else if(a === '청산 준비' || a === '청산 검토') _verdictBadgeTop = '<span class="tx-badge down-soft">하락조짐</span>';
            else if(a === '즉시 청산' || a === '매도 완료') _verdictBadgeTop = '<span class="tx-badge down">하락유력</span>';
          }
        }
  ...
  const promises = otherTfs.map(async (t) => {
    try {
      ...
      if(typeof unifiedVerdict === 'function'){
        // S103-fix7 Phase3-A-3: btStateObj 4번째 인자 추가 (매도 완료 익절/손절 맥락용)
        svVerdict = unifiedVerdict(_btStateKey, svJudge, btScore != null ? btScore : null, btState);
      }
...
function _restoreFromTfCache(stock, cached){
  ...
  if(cached.svVerdict && cached.svVerdict.action){
    stock._btAction = _mapVerdictToBtAction(cached.svVerdict.action);
  } else {
...
async function _rerunBtForModeChange(stock){
  ...
  try {
    ...
    if(svVerdict && svVerdict.action){
      stock._btAction = _mapVerdictToBtAction(svVerdict.action);
    } else {
      stock._btAction = null;
    }
