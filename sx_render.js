...
function _bestThresh737(pts){   // pts: [{v, real}] → 최적 분리 임계값
  if(pts.length<20) return null;
  var vals=pts.map(function(p){return p.v;}).sort(function(a,b){return a-b;});
  var best=null;
  for(var q=0.15; q<=0.85; q+=0.05){
    var X=_quart737(vals, q);
    ...
    var rB=below.filter(function(p){return p.real;}).length/below.length;
    var rA=above.filter(function(p){return p.real;}).length/above.length;
    var sep=Math.abs(rB-rA);
    if(!best||sep>best.sep){ best={ X:+X.toFixed(1), rB:+(100*rB).toFixed(0), rA:+(100*rA).toFixed(0), nB:below.length, nA:above.length, sep:sep, dir:(rB>=rA?'below':'above') }; }
  }
  return best;
}
...
function _btThreshRender(entries){
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:7px">강 수치재료의 <b style="color:'+GRN+'">'+DP.good+'</b> vs <b style="color:'+RED+'">'+DP.bad+'</b> 분포(Q1·중앙·Q3)와 최적 분리 임계값. "<b>재료 &lt;X → '+DP.goodShort+' Y%</b>"가 레시피 조건. N10(방향성)·N5(타이밍) 둘 다. 분리도('+DP.goodShort+'비율 차)순.</div>';
  CR.forEach(function(C){
    ...
    var rows=numFeats.map(function(D){
      ...
      return {
        ...
        bt:bt, sep:bt?bt.sep:0
      };
    }).filter(Boolean).sort(function(a,b){return b.sep-a.sep;});
    ...
    rows.forEach(function(r){
      ...
      if(r.bt){ var dirTxt=(r.bt.dir==='below'?'&lt; '+r.bt.X:'≥ '+r.bt.X); var goodR=(r.bt.dir==='below'?r.bt.rB:r.bt.rA); var badR=(r.bt.dir==='below'?r.bt.rA:r.bt.rB);
        out+='<span style="font-size:9px;font-weight:800;color:'+(strong?GRN:T3)+'">'+dirTxt+' → '+DP.goodShort+' '+goodR+'%<span style="color:'+T3+';font-weight:500"> (반대 '+badR+'%)</span></span>'; }
      out+='</div>';
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">읽기: <b style="color:'+GRN+'">초록 테두리</b>=분리도 큰 재료('+DP.goodShort+'비율 차≥20%p). "재료 &lt;X → '+DP.goodShort+' Y%"는 그 조건으로 거른 진입의 '+DP.good+' 비율. 중앙값이 '+DP.goodShort+'/'+DP.badShort+' 사이 크게 벌어진 재료가 좋은 조건. 여러 강 재료 AND로 묶으면 레시피.</div>';
  out+='</div>';
  return out;
}
