...
// [S792] 최근 maxBack봉 스캔 — adv.rows에서 봉별 정렬/약세/피처. 배지 '조짐/유력' 결과대기 단계용(현재봉만이 아닌 최근 발동 추적).
function _sxBadgeScanRecent(adv, maxBack){
  var rows = adv && adv.rows;
...
// 배지 def의 최근 발동 {barsAgo, count} (없으면 null) — scan 오래된→최신이라 끝까지 갱신=가장 최근
function _sxBadgeLastFire(def, scan){
  ...
  var prim=fires[0];   // [S793] 결승선에 가장 가까운(barsAgo 최대=가장 오래·결과 임박) 1건 우선 — 끝나면 다음 신호로 자동 승계
  for(var j=1;j<fires.length;j++){ if(fires[j].barsAgo>prim.barsAgo) prim=fires[j]; }
  return { barsAgo:prim.barsAgo, count:prim.count, others:(fires.length-1) };
}
// 배지 1행 내부 HTML — lastFire 있으면 그 단계(발동0/조짐1-5/유력6-10), 없으면 현재봉(adv) 기준
function _sxBadgeRowInner(def, adv, lastFire){
  var GR='var(--text3)';
  var _e=function(t){ return String(t==null?'':t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); };
  var ev=_sxBadgeEval(def.id, adv);   // 현재봉
  var fire = (lastFire!=null) ? lastFire.barsAgo : (ev.fired ? 0 : null);
  var cnt  = (lastFire!=null) ? lastFire.count   : ev.matchCount;
  ...
  if(!ev.inPool){ statusTxt = ev.matchCount>0 ? ('재료충족 '+ev.matchCount+' · 풀 불일치') : '○ 풀 불일치'; statusCol=GR; }
  else if(stage==='now'){ statusTxt='✅ 발동 '+cnt+'/'+ev.total; statusCol=def.color; }
  else if(stage){ statusTxt='⏳ '+fire+'봉전 '+cnt+'개 발동 · 관찰중'; statusCol=def.color; }
  else { statusTxt='● 관망 0/'+ev.total; statusCol=GR; }
  var detail;
  if(stage==='now' && ev.matched && ev.matched.length){
    var top=ev.matched.slice(0,2).map(function(r){ return _e(r.label); }).join('<br>');
    detail='<div style="font-size:9px;color:var(--text2);line-height:1.5;margin-top:3px">'+top+(ev.matchCount>2?('<br><span style="color:'+GR+'">외 '+(ev.matchCount-2)+'개 충족</span>'):'')+'</div>';
  } else if(stage==='early' || stage==='late'){
    var othTxt=(lastFire&&lastFire.others>0)?(' · 외 '+lastFire.others+'건 관찰중'):'';
    detail='<div style="font-size:8.5px;color:'+def.color+';margin-top:3px">'+fire+'봉전 발동 → 결과 관찰중 ('+(stage==='early'?'~5봉':'~10봉')+' 윈도우)'+othTxt+'</div>';
  } else {
    detail='<div style="font-size:8.5px;color:'+GR+';margin-top:3px">충족 레시피 없음 ('+ev.total+'개 중 0)</div>';
  }
  return '<div style="display:flex;align-items:center;gap:7px;margin-bottom:4px">'+chip
    +    '<span style="margin-left:auto;font-size:9.5px;font-weight:700;color:'+statusCol+'">'+statusTxt+'</span></div>'
    +  '<div style="font-size:9px;color:'+GR+';line-height:1.5">풀: <b style="color:var(--text2)">'+def.poolName+'</b> '+(ev.inPool?('<span style="color:'+def.color+'">●일치</span>'):'<span>●불일치</span>')+' · 레시피 '+ev.total+'개</div>'
    +  detail
    +  '<div style="font-size:8px;color:'+GR+';margin-top:3px;font-style:italic">'+def.desc+'</div>';
}
function _buildBadgeInventoryCard(stock, indicators){
  var adv = (indicators && indicators._advanced) ? indicators._advanced : null;
  var token = (window._sxBadgeTok = (window._sxBadgeTok||0)+1);   // 네비게이션 가드(빠른 종목이동 시 옛 스캔 취소)
  var firedNow=0;
  var rowsHtml = SX_BADGE_ORDER.map(function(key){
    var def=SX_BADGE_DEFS[key];
    if(_sxBadgeEval(key, adv).fired) firedNow++;
    return '<div id="sxBadgeRow_'+key+'" style="padding:9px 10px;margin:6px 0;border-radius:9px;background:var(--surface2);border:1px solid var(--border)">'+_sxBadgeRowInner(def, adv, null)+'</div>';
  }).join('');
  if(adv && Array.isArray(adv.rows) && adv.rows.length>=260){   // 최근 10봉 스캔 → 조짐/유력 단계 비동기 갱신
    setTimeout(function(){ try{ _sxBadgePendingUpdate(token, adv); }catch(e){} }, 60);
  }
...
if(typeof window!=='undefined'){ window._buildBadgeInventoryCard = _buildBadgeInventoryCard; window._sxBadgePendingUpdate = _sxBadgePendingUpdate; window._sxBadgeScanRecent = _sxBadgeScanRecent; }
