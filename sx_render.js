...
function _trendBt(rows,cfg,bbP){
  ...
  const _MINB = (_KN&&_KN.MIN_BANK)||40, _KK=(_KN&&_KN.K)||10, _PTHR=0.5, _STRONG=0.65;
  ...
  const _predGc=(i)=>{
    ...
    if(_auto){
      if(prob>=_STRONG) return 2;                   // 강신호 → 2봉 선행
      if(btc<=1.5 && prob>=_PTHR) return 1;          // 모호 → 1봉 앞일 때만
      return 0;
    }
...
function _trendRenderInner(){
  ...
  const _predDesc=cfg.predLead==='auto'?'신호 강하면 2봉·모호하면 1봉 선행 진입':('kNN이 '+_predLeadLbl+' 내 골든크로스 예측 시 선행 진입');
  const predRow=`<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">`
    ...
    +(cfg.predict?`<span onclick="_sxVib(10);window._trendCycleLead&&_trendCycleLead()" title="1봉→2봉→자동 순환. 자동: 신호 강하면 2봉, 모호하면 1봉 선행." style="font-size:10px;font-weight:800;padding:5px 10px;border-radius:14px;border:1px solid #7c3aed66;background:#7c3aed18;color:#7c3aed;cursor:pointer">선행 ${_predLeadLbl}</span>`:'')
