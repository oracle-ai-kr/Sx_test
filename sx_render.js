...
function _sxvResultHtml(){
  ...
  var opt = { s:cfg.s, l:cfg.l, N:_sxvN, thr:_sxvThr, tp:_sxvTp, sl:_sxvSl };   // [S705-3] dir 제거 · [S706] 배리어 tp/sl
  if(_sxvSrc === 'cur'){
    var rows = (L.indicators && L.indicators._advanced && Array.isArray(L.indicators._advanced.rows)) ? L.indicators._advanced.rows
             : (L.stock && Array.isArray(L.stock._lastAnalCandles)) ? L.stock._lastAnalCandles : null;
...
function _sxvRenderResult(R, isPool){
  ...
  var head = '<div style="display:flex;align-items:baseline;justify-content:space-between;gap:8px;margin-bottom:3px"><span style="font-size:12px;font-weight:800;color:var(--text)">' + R.rulerLabel + ' <span style="color:' + dirCol + '">' + dirTxt + '</span></span><span style="font-size:9px;color:' + T3 + '">N=' + R.N + thrTxt + ' · ' + R.nTests + '시점' + (isPool ? (' · ' + R.stocks + '종목' + (R._capped ? '(상한20)' : '')) : '') + '</span></div>';
...
function _buildValidatorCard(stock, indicators){
  ...
  var ctrl2Row = '<div style="display:flex;align-items:center;gap:5px;margin-bottom:7px;flex-wrap:wrap">' + nBtn + thBtns + '</div>';
  // 신호 멀티칩
