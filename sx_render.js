...
function _sxbCircle(score, label, big, neutral, delta){
  ...
  return `<div class="sxb-circle${big?' sxb-big':''}">`
    ...
    + `</svg>`
    + (label?`<div class="sxb-circle-label">${label}${_sxbTri(delta, neutral)}</div>`:'')
    + `</div>`;
}
...
function _sxbHTML(){
  ...
  // [S357] 배너 판정 배지 (배너 흡수 포석) — sub 줄에 배치
  const V = B.verdict;
  let badge = '';
  if(V && V.action){
    const act = String(V.action).replace('청산','매도');
    badge = `<span class="sxb-badge" style="color:${V.color};background:${V.color}1A;border:1px solid ${V.color}">${V.icon||''} ${act}</span>`;
  }
  // 헤드 (항상 표시 · 클릭 시 펼침) — 톤 라벨 옆 종합 전이, sub 줄에 배너 배지
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    + _sxbCircle(total, '', true, false)
    + `<div class="sxb-head-info"><div class="sxb-head-tone" style="color:${tone.c}">${tone.t}${_sxbTri(headDelta,false)}</div>`
    + `<div class="sxb-head-sub"><span>종합 ${total} · ${_sxbGrade(total)}등급 · ${all.length}개 지표</span>${badge}</div></div>`
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict){
  D = D || {};
  const g1=[], g2=[], g3=[], g4=[];
  // ① 추세·구조
  ...
  window._sxBoard = { verdict: verdict||null, groups:[
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  try {
    ...
    const _h357 = _mom357 && _mom357.history;
    if(_h357 && _h357.length>=2){
      _boardDeltas.trendScore = _h357[0].trendScore - _h357[1].trendScore;
      _boardDeltas.readyScore = _h357[0].readyScore - _h357[1].readyScore;
      _boardDeltas.entryScore = _h357[0].entryScore - _h357[1].entryScore;
    }
  ...
  body.innerHTML = `
    ${chartHTML}
    ${btStateBannerHTML}
    ${_buildScoreBoard(scores, stock._svScores4, _boardStruct, _boardPb, _boardDeltas, stock._svVerdict)}
    ${tradeHistHTML}
