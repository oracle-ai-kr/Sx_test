// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S813 · build 20260630-0150 cache-bust]
// ════════════════════════════════════════════════════════════
...
function _trendWtKey(ctx){ return (ctx.name||'')+'|'+ctx.market+'_'+ctx.rows.length+'_'+(ctx.rows.length?ctx.rows[ctx.rows.length-1].close:0); }
...
function _trendWtEnsure(ctx, cb){
  ...
  setTimeout(async function(){
    let fire=null; try{ fire=await SXRecipeSignal.realFireBars(sym, ctx.rows); }catch(e){ fire={}; }
...
function _trendDetailInner(bt,cfg,ctx){
  ...
  const wtChip=`<div style="margin:2px 0 10px">`
    +`<div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:5px"><span onclick="_sxVib(10);window._trendCycleWtMode&&_trendCycleWtMode()" title="보유 중 진짜반등(정배 눌림목+역배 데드캣 real) 신호당 100만 추가입금. 탭으로 OFF→적금/DCA→물타기 동원 순환." style="font-size:10px;font-weight:800;padding:5px 11px;border-radius:14px;border:1px solid;cursor:pointer;${_trendWtMode!=='off'?'background:#0891b2;color:#fff;border-color:#0891b2':'background:var(--surface2);color:#0891b2;border-color:#0891b266'}">💧 ${_wtBtnLabel[_trendWtMode]}</span><span style="font-size:8.5px;color:var(--text3)">탭 전환 ↻</span></div>`
...
if(typeof window!=='undefined'){ window._trendOpenDetail=_trendOpenDetail; window._trendDetailClose=_trendDetailClose; window._trendCycleWtMode=_trendCycleWtMode; }
