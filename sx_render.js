...
function _cvPoolRender(rep, watch, mk){
  var GRN='#16a34a',RED='#e3493b',MUT='#64748b',T3='var(--text3)',PUR='#a855f7';
  ...
  function bracket(P, label){
    ...
    var h='<div style="margin-top:11px;font-size:11px;font-weight:800;color:'+PUR+'">'+label+' · '+P.stocks+'종목·'+P.n+'시점'+(P.capped?'(상한20)':'')+'</div>';
    h+='<table style="width:100%;border-collapse:collapse;margin-top:5px;font-size:10px"><tr style="color:'+T3+';font-weight:700"><td style="text-align:left;padding:2px 0">축</td><td style="text-align:right">IC종가</td><td style="text-align:right">IC익일</td><td style="text-align:right">고-저%</td></tr>';
    P.axes.forEach(function(a){
      var hl=a.hiLoO||a.hiLoC;
      h+='<tr><td style="text-align:left;padding:2px 0;font-weight:700">'+a.label+'</td><td style="text-align:right;color:'+icCol(a.icC)+'">'+(a.icC==null?'—':a.icC)+'</td><td style="text-align:right;font-weight:800;color:'+icCol(a.icO)+'">'+(a.icO==null?'—':a.icO)+'</td><td style="text-align:right;color:'+(hl&&hl.spread>0?GRN:hl&&hl.spread<0?RED:MUT)+'">'+(hl?sg(hl.spread):'—')+'</td></tr>';
    });
    h+='</table>';
    var eM=(P.entry&&P.entry.statO)||{}, aM=(P.avoid&&P.avoid.statO)||{};
    var sep=(eM.ret!=null&&aM.ret!=null)?Math.round((eM.ret-aM.ret)*100)/100:null;
    h+='<div style="margin-top:6px;font-size:10px;color:var(--text2)">등급 변별(익일): 진입급 <b style="color:'+(eM.ret>0?GRN:RED)+'">'+sg(eM.ret==null?null:eM.ret)+'%</b> vs 회피급 <b style="color:'+(aM.ret>0?GRN:RED)+'">'+sg(aM.ret==null?null:aM.ret)+'%</b> · 격차 <b style="color:'+(sep>0?GRN:sep<0?RED:MUT)+'">'+(sep==null?'—':sg(sep)+'p')+'</b></div>';
    return h;
  }
  ...
  out+='<div style="font-size:9px;color:'+T3+';line-height:1.5;margin-bottom:2px">다종목 합산으로 단일종목 IC 노이즈 상쇄 · 핵심은 <b>IC익일 부호 일관성</b> · |IC|<0.15는 정상(일봉 단기 효율적)</div>';
  out+=bracket(rep, '📊 대표목록 (강세 브래킷)');
  out+=bracket(watch, '👁️ 관심목록 (지연 브래킷)');
  if(rep&&rep.ok&&watch&&watch.ok){
    out+='<div style="margin-top:9px;font-size:9px;color:var(--text2);line-height:1.55;border-top:1px solid var(--border);padding-top:6px">두 브래킷에서 <b>같은 축이 같은 부호로 +IC</b>면 모집단 신호 — 단일종목 운에서 벗어난 진짜 변별. 부호가 갈리면 시장성격 의존(범용성 약). 등급 격차가 양 브래킷 다 +면 등급이 실제로 결과를 가름.</div>';
  }
  out+='</div>';
  return out;
}
