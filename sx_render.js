  ...
  body.innerHTML = `
    ...
    ${(()=>{
      ...
      return `<div class="anal-section">
        ...
        ${(()=>{
          // [v3.17 EPS-FIX] DART 데이터 안내문 — 사용자가 네이버 등 타 도구와 차이 의문 시 즉시 해소
          //   표시 조건: DART 출처 + 보고서 연도 정보 있을 때만
          if(fin._source !== 'dart' || !fin._reportYear) return '';
          ...
          // [v3.17 EPS-FIX] 클릭 시 설정 탭 → 드롭다운까지 자동 스크롤+강조
          //   _sxGotoFinReportSetting()은 sx_screener.html에 정의된 헬퍼
          ...
          const recommendBlock = (() => {
            ...
            // 현재 선택이 이미 최신이면 안내 생략 (잡음 방지)
            if(curType === latestType) return '';
            ...
            if(curType === 'annual'){
              ...
              // 분기/반기 선택 중 → 검증된 데이터는 연간 추천
              suggestion = `검증된 데이터는 <b>${stableYear}년 사업보고서</b> · 가장 최신은 <b>${latestLabel}</b> (${latestYear}년)`;
            }
          ...
          return `<div style="margin-top:8px;padding:6px 8px;background:var(--surface2);border-left:2px solid var(--accent);border-radius:4px;font-size:9px;color:var(--text3);line-height:1.5">
            ...
            <div style="margin-top:3px;color:var(--text2)">변경: <a onclick="_sxVib(8);_sxGotoFinReportSetting()" style="color:var(--accent);font-weight:600;cursor:pointer;text-decoration:underline">⚙ 설정 → 재무 보고서 종류</a></div>
            ${recommendBlock}
          </div>`;
        })()}
