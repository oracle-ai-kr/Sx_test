...
function _buildTransitionCard(stock, indicators){
  ...
  try {
    ...
    const _tgt = (typeof _btTargetBars==='function') ? _btTargetBars(_mktCT, _tfCT) : 600;
    if (rows.length >= Math.floor(_tgt * 0.95)) {
  ...
  // [S638] 게이트 작동 조건 = "충분 봉 확보됨"(_r600Done). BT 확장본 재사용 또는 폴백 fetch 해결 시 작동. 짧은 종목/실패도 최선 rows로 판정(영구 차단 방지).
  const _have600 = _r600Done;
  // [S525] 미니 백테스트 컨텍스트 저장 — 버튼 클릭 시 sx_candle_bt.js가 읽음. [S637] 600봉으로 통일(모달·게이트 동일 기준).
  try { window._sxCTBT = { rows: _knnRows, market: _mktCT, tf: _tfCT, name: (stock && (stock.name || stock.code)) || '' }; } catch(_eCtx){}
  ...
  if(_useKnn){
    const _hk = ((stock&&(stock.code||stock.name))||'') + '|' + _tfCT + '|' + (_lastDate||'') + '|' + _knnRows.length;
