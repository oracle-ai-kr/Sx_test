...
// 검증 실행 — 현재종목=즉시(SXUV.run) · 대표/관심=비동기 풀(fetchRows600 × CAP20 → SXUV.runPool)
async function _sxvRun(){
  ...
  if(typeof SXUV === 'undefined' || !SXUV.run){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">검증 엔진(SXUV) 미로드 — sx_validator.js를 같은 폴더에 올렸는지 확인.</div>'; return; }
  ...
  if(_sxvSrc === 'cur'){
    ...
    var R; try { R = SXUV.run(rows, _sxvRuler, preds, opt); } catch(e){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">계산 오류: ' + String((e && e.message) || e) + '</div>'; return; }
  ...
  var RP; try { RP = SXUV.runPool(rowsList, _sxvRuler, preds, opt); } catch(e){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">풀 계산 오류: ' + String((e && e.message) || e) + '</div>'; return; }
