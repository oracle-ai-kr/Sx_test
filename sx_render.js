...
// [S795] 배지 인벤토리 = 레시피카드 SSOT(_pendingByCat) 당겨씀 — '관찰중'(최근발동 0-5봉) 레시피 개수로 카운트. 레시피카드와 동일 _scanStock 캐시·동일 집계라 카운터·확정임박봉 완전 일치.
...
function _sxBadgeRowInner(def, st){
  ...
  var stage = (count<=0) ? null : (oldest<=0 ? 'now' : (oldest<=2 ? 'early' : 'late'));   // 0봉=발동 · 1-2봉=조짐 · 3-5봉=확정임박
  var chip;
  ...
  var detail;
  if(count>0){
    detail='<div style="font-size:8.5px;color:'+def.color+';margin-top:3px">'+((oldest<=0)?('방금 '+count+'개 발동 → 결과 관찰 시작(~5봉 윈도우)'):(oldest+'봉전 발동분 결과 임박 · 관찰중 '+count+'개 (최근발동 0-5봉)'))+'</div>';
  } else {
...
function _buildBadgeInventoryCard(stock, indicators){
  ...
  var desc = '<div style="font-size:8.5px;color:var(--text3);line-height:1.5;margin-bottom:4px">신호 배지 <b style="color:var(--text2)">시제품 보관소(SSOT)</b> — 다른 배지가 가져다 씁니다. <b>4종</b>: 역배열 바닥반등·데드캣 / 정배열 눌림목·되돌림. 카운트 = <b>레시피카드 \'관찰중\'(최근발동 0-5봉) 레시피 수</b>를 그대로 당겨옵니다(동일 소스라 카운터 일치). <b>확정임박 N봉</b> = 관찰중 중 가장 오래된 발동(곧 결과 확정). <b style="color:var(--text2)">회색=풀 불일치 또는 관찰중 0</b>.</div>';
