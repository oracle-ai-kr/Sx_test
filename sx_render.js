...
function _trendBt(rows,cfg,bbP){
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){
      ...
      if(dc || sellHit(i) || _predFail || _predExit || _earlyMa5 || _disSl){ const pnl=(close[i]/pos.entry-1)*100; trades.push({entry:pos.entry,exit:close[i],pnl:+pnl.toFixed(2),bars:i-pos.entryIdx,entryIdx:pos.entryIdx,exitIdx:i,re:!!pos.re,pred:!!pos.pred,predLead:pos.predLead||0,fail:!!_predFail,predExit:!!_predExit,early:!!_earlyMa5,disSl:!!_disSl,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||'',exitDate:(rows[i]&&(rows[i].date||rows[i].t))||''}); pos=null; _lastExitIdx=i; }
    }
  }
...
function _trendRenderInner(){
  ...
  const batchSection=`<div style="margin-top:10px;border-top:1px dashed var(--border);padding-top:9px">`
    +`<div style="display:flex;align-items:center;gap:6px;margin-bottom:7px"><span style="font-size:10px;font-weight:800;color:var(--text2)">🧪 배치 A/B</span>`+_bsChip(_bsRep,'대표풀','rep')+_bsChip(!_bsRep,'관심목록','watch')+`</div>`
    +`<button onclick="window._trendBatchUI&&_trendBatchUI()" style="width:100%;font-size:11px;font-weight:700;padding:8px 0;border-radius:8px;cursor:pointer;color:#0ea5e9;background:var(--surface);border:1px solid #0ea5e966">▶ ${_bsRep?'대표풀':'관심목록'} 전체 × 7설정 자동 BT (일봉)</button>`
    +`<div id="sxTrendBatchResult" style="margin-top:8px"></div></div>`;
  return head+maRow+chipRow+reChip+predRow+exitRow+_guardBadge+stateRow+grid+note+batchSection;
}
...
function _trendBatchConfigs(market){
  const d=_TREND_MKT_DEFAULT[market]||[5,9];
  const base=function(){ return { s:d[0], l:d[1], n:3, aux:{}, sell:{}, reentry:false, predict:false, predLead:1, nextOpen:false, earlyMa5:false, disSl:false, slAtr:2 }; };
  const C=[]; let c;
  c=base(); C.push({label:'순수 MA', cfg:c});
  c=base(); c.earlyMa5=true; C.push({label:'earlyMa5', cfg:c});
  c=base(); c.disSl=true; C.push({label:'disSl 2×', cfg:c});
  c=base(); c.reentry=true; C.push({label:'재진입', cfg:c});
  c=base(); c.earlyMa5=true; c.reentry=true; C.push({label:'early+재진입', cfg:c});
  c=base(); c.earlyMa5=true; c.reentry=true; c.sell={rsiDc:true,rsiHot:true}; C.push({label:'+RSI칩', cfg:c});
  c=base(); c.earlyMa5=true; c.reentry=true; c.sell={rsiDc:true,rsiHot:true}; c.disSl=true; C.push({label:'전체+disSl', cfg:c});
  return C;
}
...
async function _trendBatchRun(mk, tf, onProgress){
  ...
  return { ok:true, source:_trendBatchSource, market:mk, n:per.length, capped:capped, total:list.length, skipped:skipped, agg:agg };
}
function _trendBatchRender(res){
  ...
  return '<div style="padding:9px 10px;background:var(--surface);border-radius:9px;border:1px solid #0ea5e955">'
    +'<div style="font-size:11px;font-weight:800;color:#0ea5e9;margin-bottom:5px">🧪 배치 A/B · '+(res.source==='watch'?'관심목록':'대표풀')+' '+String(res.market).toUpperCase()+' · '+res.n+'종목'+(res.capped?' (상한20)':'')+'</div>'
    ...
    +rows
    +'<div style="font-size:9px;color:var(--text3);margin-top:6px;line-height:1.6">중앙수익=중앙값 수익률(괴물 종목에 안 휘둘림·주력) · 양수%=수익 낸 종목 비율 · MDD·손익비=평균/중앙값. 첫 줄(순수 MA)=기준선. 일봉 '+res.n+'종목 · 과적합 점검용.</div>'
    +'</div>';
}
...
function _svRenderSignalVal(r){
  ...
  function matrix(pick,colfn,fmt){
    var s='';
    r.signals.forEach(function(sig){
      s+='<div style="display:grid;grid-template-columns:58px repeat('+ncol+',1fr);gap:3px;align-items:center;padding:2.5px 2px">'
        ...
        +sig.byH.map(function(m){ var v=m.ok?pick(m):null; return '<span style="text-align:center;font-size:9.5px;font-weight:800;color:'+colfn(v)+'">'+(v!=null?fmt(v):'—')+'</span>'; }).join('')+'</div>';
    });
    return s;
  }
  ...
  h+='<div style="font-size:9.5px;font-weight:800;color:#334155;margin:2px 0 1px">균형정확도 — 방향 맞히기</div>'+hhead()+matrix(function(m){return m.balAcc;},bcol,function(v){return v+'';});
  h+='<div style="font-size:9.5px;font-weight:800;color:#334155;margin:9px 0 1px">방향 기대값 %/트레이드 — 크기까지(롱/숏)</div>'+hhead()+matrix(function(m){return m.dirExp;},ecol,function(v){return (v>0?'+':'')+v;});
  var improved=[], bestCell={v:-1,sig:'—',h:0}, posExp=[], bestLE={v:-1e9,sig:'—',h:0};
  r.signals.forEach(function(s){ var a=s.byH[0],z=s.byH[ncol-1];
    ...
    var pe=0; s.byH.forEach(function(m,i){ if(m.ok&&m.balAcc!=null&&m.balAcc>bestCell.v){bestCell={v:m.balAcc,sig:s.label,h:H[i]};}
      ...
      if(m.ok&&m.longEdge!=null&&m.longEdge>bestLE.v){bestLE={v:m.longEdge,sig:s.label,h:H[i]};} });
  ...
  h+='<div style="margin-top:9px;font-size:9px;color:'+T3+';line-height:1.6;border-top:1px solid #eef2f7;padding-top:7px">룩어헤드 0 · 신호 prob는 호라이즌 무관(1회 계산), 결과수익만 호라이즌별 · '+r.step+'봉 간격 표본 · kNN/LR은 다음봉 확률을 장기수익에 평가(h=1 native). ⚠️ 단일종목 '+r.nPts+'시점 — 노이즈 큼, 여러 종목 집계해야 신뢰.</div>';
  return h+close;
}
...
function _cvRender(R){
  ...
  h+='<div style="font-size:10px;color:'+MUT+';margin-bottom:8px">전체 드리프트 <b>'+sg(R.drift.ret)+'%</b>·승률 '+R.drift.winRate+'% ('+R.nPts+'시점)</div>';
  // verdict별
  h+='<div style="font-size:10.5px;font-weight:800;color:var(--text);margin:2px 0 3px">판정별 이후 '+R.h+'봉 수익</div>';
  h+='<div style="display:grid;grid-template-columns:54px 1fr 1fr 1fr;gap:4px;padding:0 2px 2px;font-size:8px;color:'+T3+';font-weight:700"><span>판정</span><span style="text-align:right">이후수익</span><span style="text-align:right">승률</span><span style="text-align:right">최악</span></div>';
  R.byVerdict.forEach(function(v){ var s=v.stat; if(!s.n)return;
    h+='<div style="display:grid;grid-template-columns:54px 1fr 1fr 1fr;gap:4px;align-items:center;padding:3px 2px;border-bottom:1px solid var(--border)">'
      ...
      +'<span style="text-align:right;font-size:10px;color:var(--text2)">'+s.winRate+'%</span>'
      +'<span style="text-align:right;font-size:10px;color:'+RED+'">'+s.worst+'%</span>'
      +'<span style="grid-column:1/-1;font-size:8px;color:'+T3+';text-align:right;margin-top:-2px">n='+s.n+'</span></div>'; });
  // passCount별 (간단)
  ...
  // 자동 해석
  var byV={}; R.byVerdict.forEach(function(v){ byV[v.action]=v.stat; });
  function wm(keys){ var s=0,nn=0; keys.forEach(function(a){ if(byV[a]&&byV[a].n){ s+=byV[a].ret*byV[a].n; nn+=byV[a].n; } }); return nn?{r:Math.round(s/nn*100)/100,n:nn}:null; }
  ...
  h+='<div style="margin-top:8px;font-size:8.5px;color:'+T3+';line-height:1.5;border-top:1px solid var(--border);padding-top:6px">진입판정(비보유) 기준 · 이후 '+R.h+'봉 종가수익 · '+R.step+'봉 간격 '+R.nPts+'시점'+(R.errN?'·'+R.errN+'오류':'')+'. ⚠️ 단일종목 노이즈 큼 — 여러 종목 집계해야 신뢰. C 진짜 가치는 방향보다 하방/리스크 관리.</div>';
  return h;
}
