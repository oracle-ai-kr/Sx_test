...
function _mtfTfPure(ind){
  if(!ind || !ind.rsi || !ind.adx || !ind.macd || !ind.obv || !ind.maAlign) return null;
  const rsi = (typeof ind.rsi === 'object' ? ind.rsi.val : ind.rsi) ?? 50;
  const adx = ind.adx.adx ?? 0, pdi = ind.adx.pdi ?? 0, mdi = ind.adx.mdi ?? 0;
  const maBull = !!ind.maAlign.bullish, maBear = !!ind.maAlign.bearish;
  const diUp = pdi > mdi;
  ...
  return { score: Math.max(0, Math.min(100, Math.round(s))), maBull, maBear, rsiOver: rsi > overTh };
}
function _computeMtfScore(){
  ...
  const htfOver = !!((_det.week && _det.week.rsiOver) || (_det.month && _det.month.rsiOver));
  const flags = {
...
function _sxbHTML(){
  ...
  let _mtfBadge = '';
  {
    ...
    if(_mtf){
      ...
      const _f = _mtf.flags || {};
      const _mk = (txt, col) => `<span class="sxb-badge" style="color:${col};background:${col}1A;border:1px solid ${col}">${txt}</span>`;
      if(_f.align)     _mtfBadge += _mk('🔥 3TF 정배열 정합', '#3b82f6');   // 정합 — 파랑
      if(_f.divergent) _mtfBadge += _mk('🔀 3TF 역배열 불일치', '#94a3b8'); // 불일치 — 회색
      if(_f.overheat)  _mtfBadge += _mk('⚠️ 상위TF 과열', '#0ea5e9');       // 과열 — 하늘(경고지만 정보 톤)
    }
  }
