...
function _sxbHTML(){
  ...
  // [S382] 종합점수 = 6요소 가중평균 (5카테고리 + 카운터배지). 시장별 비중, 합 1.0.
  //   · 5카테고리: _sxbAvg(inverse 역산 반영) / 배지: net(▲−▼)을 0~100 점수화
  //   · 코인은 모멘텀 시장이라 배지 비중↑·펀더 비중↓ / localStorage(_sxbGetWeights)로 오버라이드 가능
  const _SXB_W_DEFAULT = {
    kr:   { trend:0.22, mom:0.18, flow:0.22, fund:0.13, tech:0.13, badge:0.12 },
    us:   { trend:0.22, mom:0.18, flow:0.18, fund:0.18, tech:0.12, badge:0.12 },
    coin: { trend:0.25, mom:0.22, flow:0.15, fund:0.05, tech:0.18, badge:0.15 },
  };
  const _mkt = (typeof currentMarket !== 'undefined' && _SXB_W_DEFAULT[currentMarket]) ? currentMarket : 'kr';
  const _W = (typeof _sxbGetWeights === 'function' && _sxbGetWeights(_mkt)) || _SXB_W_DEFAULT[_mkt];
