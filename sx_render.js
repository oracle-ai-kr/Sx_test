...
function _candleTransitionScore(rows, indicators){
  ...
  const _cp = (indicators && indicators.candle && Array.isArray(indicators.candle.patterns)) ? indicators.candle
            : (indicators && indicators.patterns && Array.isArray(indicators.patterns.patterns)) ? indicators.patterns
            : (indicators && indicators.patternsLegacy && Array.isArray(indicators.patternsLegacy.patterns)) ? indicators.patternsLegacy
            : null;
  ...
  if(mode === 'flipUp' || mode === 'flipDown'){
    ...
    return { active:true, mode, score, label, reasons, warn, redCnt, greenCnt, streak:0, patName: pat.name };
  }
...
function _buildTransitionCard(stock, indicators){
  ...
  const top = `<div style="display:flex;align-items:baseline;gap:8px"><span style="font-size:26px;font-weight:800;color:${col}">${sign}${r.score}</span><span style="font-size:13px;font-weight:700;color:${col}">${r.label}</span><span style="font-size:10px;color:var(--text3);margin-left:auto">${r.mode==='rebound'?`음봉 ${r.streak}연속`:r.mode==='reversal'?`양봉 ${r.streak}연속`:r.mode==='flipUp'?`🔺 ${r.patName||'양전환'}`:`🔻 ${r.patName||'음전환'}`}</span></div>`;
