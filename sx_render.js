...
if(typeof window!=='undefined'){ window._dcContAgg=_dcContAgg; window._dcLatePill=_dcLatePill; }
...
function _btThreshRender(entries){
  ...
  var out='<div style="border:1px solid var(--border);border-radius:10px;padding:10px;margin-top:8px">';
  out+='<div style="font-size:11px;font-weight:800;color:#7c3aed;margin-bottom:4px">📏 강 재료 임계값 분석</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:7px">강 수치재료의 <b style="color:'+GRN+'">'+DP.good+'</b> vs <b style="color:'+RED+'">'+DP.bad+'</b> 분포(Q1·중앙·Q3)와 진짜율 최대 임계값. "<b>재료 &lt;X → '+DP.goodShort+' Y%</b>"가 레시피 조건. N10(방향성)·N5(타이밍) 둘 다. <b>'+DP.goodShort+'율 높은 순</b>(표본 충족분만).</div>';
  CR.forEach(function(C){
    ...
    if(C.key==='n10'){ window._sxThreshBest={}; rows.forEach(function(r){ if(r.bt && r.bt.X!=null) window._sxThreshBest[r.key]={dir:(r.bt.dir==='below'?'lt':'gt'), th:r.bt.X, lift:r.bt.lift}; }); }   // [S767] N10 기준 best 컷을 빌더 복사용으로 저장
...
function _sxCopyThreshToBuilder(){
  ...
  if(!B || !Object.keys(B).length){ try{alert('먼저 📏 임계값 도구를 실행해주세요 — N10 기준 best 컷을 가져옵니다.');}catch(_){} return; }
  var applied=0;
  _DISCRIM_FEATS.forEach(function(D){
...
function _sxBuilderAutoScan(scanMode){
  ...
  var results=cands.map(function(D){
    ...
    return { label:D.label, key:D.key, type:D.type, cond:c, n3:_rate(m,'n3'), n5:_rate(m,'n5'), n10:_rate(m,'n10'), hold:_rate(m,'hold'), late:_dcContAgg(m).late, n:m.length };
...
function _btBuilderUI(){
  ...
  h+='<button onclick="_sxCopyThreshToBuilder()" style="font-size:9px;padding:4px 8px;border-radius:6px;border:1px solid #7c3aed66;background:#7c3aed14;color:#7c3aed;font-weight:700">📏 임계값 복사</button>';
  h+='</div>';
...
function _sxBuilderEval(){
  ...
  h+='<div style="margin-top:5px;padding:6px 8px;border-radius:6px;background:#7c3aed10;border:1px solid #7c3aed33;font-size:9.5px;color:var(--text2)">📐 이 조합 '+_dcLatePill(_bM, _bBaseLate)+' <span style="font-size:8px;color:var(--text3)">(표본 '+_bM.length+')</span></div>';
  out.innerHTML=h;
}
