// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S833 · build 20260630-2350 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _pullbackBaseBracket(mk, onProgress){
  if(!(window.SXCandleBT&&SXCandleBT.fetchRows600)) return { ok:false, reason:'캔들 fetch 미연결' };
  ...
  var CAP=130, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
  var allN=0,allUp=0, bullN=0,bullUp=0;
  var ov={1:{t:0,h:0},2:{t:0,h:0},3:{t:0,h:0},4:{t:0,h:0}}, stocksUsed=0;   // 정배(bull) realK별 겹침 진짜비율
  for(var i=0;i<use.length;i++){
    var s=use[i];
    if(onProgress) onProgress(i+1, use.length, s.name);
    await _trendBatchSleep(0);
    ...
    if(br&&br.length){ for(var j=0;j<br.length;j++){ var b=br[j]; if(!b.complete||b.up==null)continue; allN++; if(b.up)allUp++; if(b.lt==='bull'){ bullN++; if(b.up)bullUp++; } } _u=true; }
    var os=null; try{ os=await SXRecipeSignal.overlapScan(s.code, rows); }catch(e3){}
    if(os&&os.length){ for(var k=0;k<os.length;k++){ var o=os[k]; if(o.lt!=='bull'||o.hit==null)continue; var rk=o.realK; if(rk<1)continue; var key=(rk>=4)?4:rk; ov[key].t++; if(o.hit)ov[key].h++; } }
    if(_u) stocksUsed++;
    await _trendBatchSleep(10);
  }
  if(allN<50) return { ok:false, reason:'표본 부족 (n='+allN+')' };
  return { ok:true, allN:allN, allUp:allUp, bullN:bullN, bullUp:bullUp, ov:ov, stocksUsed:stocksUsed };
}
async function _pullbackBaseRun(){
  var el=document.getElementById('pullbackBaseResult'); if(!el) return;
  var mk=(typeof currentMarket!=='undefined')?currentMarket:'kr';
  el.style.display='block';
  el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">🔵 정배 base rate 측정 중...</div>';
  var res;
  try{ res=await _pullbackBaseBracket(mk,function(i,t,n){ el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">🔵 스캔 '+i+'/'+t+' · '+(n||'')+'</div>'; }); }catch(e){ res={ok:false,reason:String(e&&e.message||e)}; }
...
function _pullbackBaseRender(res, mk){
  ...
  if(!res) return '<div style="font-size:9px;color:'+T3+'">—</div>';
  if(!res.ok) return '<div style="font-size:9.5px;color:'+AMB+';padding:6px">'+(res.reason||'측정 실패')+'</div>';
  var baseAll=res.allN?res.allUp/res.allN*100:null, baseBull=res.bullN?res.bullUp/res.bullN*100:null;
  ...
  var rows=baseRow('시장 전체 base', baseAll, res.allN, false)+baseRow('정배열 레짐 base', baseBull, res.bullN, true)
    +ovRow('겹침 1~2개', O12)+ovRow('겹침 3개', O3)+ovRow('겹침 4+개', O4);
  // 판정 — 4+ 겹침의 정배 레짐 대비 lift
  var v='';
  if(O4.rate!=null && baseBull!=null){
    var l4=O4.rate-baseBull, lAll=(baseAll!=null)?(O4.rate-baseAll):null;
    if(l4>=10) v='<span style="color:'+GRN+'">✅ <b>겹침 4+가 정배 레짐 base보다 +'+l4.toFixed(0)+'%p</b> — 겹침이 진짜 신호 힘(불장빨 너머). 하드코딩 안심'+(lAll!=null?' (시장 대비 +'+lAll.toFixed(0)+'%p)':'')+'</span>';
    else if(l4<=4) v='<span style="color:'+RED+'">⚠️ <b>겹침 4+가 정배 레짐 base보다 +'+l4.toFixed(0)+'%p뿐</b> — 82%는 대부분 정배+불장빨. 하드코딩 수치는 <b>약세장에서 무너질 위험</b> → 경고 표시 필요</span>';
    else v='<span style="color:'+AMB+'">겹침 4+의 정배 레짐 대비 lift +'+l4.toFixed(0)+'%p — 중간. 겹침이 약간 기여하나 상당부분 레짐/불장. 하드코딩 시 "불장 기준" 주석 권장</span>';
  }
  // 단조성 체크
  var mono='';
  if(O12.rate!=null&&O3.rate!=null&&O4.rate!=null){
    var up=(O3.rate>=O12.rate-1)&&(O4.rate>=O3.rate-1);
    mono='<div style="font-size:9px;margin-top:5px;color:'+(up?GRN:AMB)+'">'+(up?'✓ 겹침 늘수록 진짜비율 단조 상승 유지(정배의 핵심 패턴 재확인)':'⚠ 단조 상승 깨짐 — 겹침 수와 진짜비율 비례 약함')+'</div>';
  }
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px"><b>정배 겹침이 진짜 신호인가 불장빨인가</b>. <b>시장 base</b>=전체 봉 N10 상승% · <b style="color:var(--text2)">정배 레짐 base</b>=정배열(MA60>120>200) 봉만 — 발동 무관 기준선. <b>겹침</b>=pullback-real 동시발동 수별 진짜비율. <b style="color:'+BLU+'">lift</b>=겹침 − 정배 레짐 base(겹침 순수 기여). lift 크면 진짜 신호, 작으면 정배+불장빨. 대표+관심+top100·'+(res.stocksUsed||0)+'종목.</div>';
  return head
    +'<table style="width:100%;border-collapse:collapse;background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">'
...
if(typeof window!=='undefined'){ window._btDiscrimRun=_btDiscrimRun; window._btGateAuditRun=_btGateAuditRun; window._recipeOverlapRun=_recipeOverlapRun; window._volRegimeRun=_volRegimeRun; window._volPoolShow=_volPoolShow; window._deadcatShortRun=_deadcatShortRun; window._deadcatTrajRun=_deadcatTrajRun; window._deadcatConfirmRun=_deadcatConfirmRun; window._pullbackBaseRun=_pullbackBaseRun; }
// [S804] 배포 확인용 빌드 식별자(SSOT) — 배포마다 이 값을 갱신. 화면 배지에 찍혀 '실제 로드된 render.js 버전'을 노출(캐시 구버전이면 안 채워지거나 옛 값 → 즉시 진단).
if(typeof window!=='undefined'){
  window.SX_BUILD='S833';
