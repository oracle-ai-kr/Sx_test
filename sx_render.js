...
// [S357] BTC 도미넌스 — coingecko /global (market_cap_percentage.btc)
//   점수화: BTC.D 38~62% → 0~100 정규화. 방향 분기(byValue 통일, inverse 미사용):
//     · BTC 종목  : 도미넌스↑ = BTC 자금 쏠림 = 강세 → score=norm (정방향)
//     · 알트 종목 : 도미넌스↑ = 알트 약세       → score=100-norm (뒤집기)
async function _computeBtcDominanceScore(stock){
  try{
    ...
    const isBtc = (sym === 'BTC');
    const score = isBtc ? norm : (100 - norm);
    return { v: score, _dom: dom, _isBtc: isBtc };
  }catch(e){ return null; }
}
// [S357] 김치프리미엄 — 업비트 KRW-BTC 실거래가 vs coingecko bitcoin KRW환산가 (BTC 고정 시장지표)
//   김프 = (업비트가 / 글로벌KRW환산가 − 1)×100. 환율 별도 호출 불필요(coingecko가 KRW 환산 제공).
//   점수화: 김프 -3%~+5% → 0~100 후 뒤집기(score=100-norm). 과열(고김프)일수록 낮은 점수=빨강/▼.
async function _computeKimchiScore(){
  try{
    const base = (typeof WORKER_BASE!=='undefined') ? WORKER_BASE : 'https://stock-signal-proxy.cheaheechang.workers.dev';
    const upRes = await fetch(`${base}/upbit/ticker?markets=KRW-BTC`, {signal:AbortSignal.timeout(8000)});
    if(!upRes.ok) return null;
    const upJ = await upRes.json();
    const upbit = Array.isArray(upJ) && upJ[0] && upJ[0].trade_price;
    const gTarget = 'https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=krw';
    const gRes = await fetch(`${base}/proxy?url=${encodeURIComponent(gTarget)}`, {signal:AbortSignal.timeout(8000)});
    if(!gRes.ok) return null;
    const gJ = await gRes.json();
    const global = gJ && gJ.bitcoin && gJ.bitcoin.krw;
    if(!upbit || !global || !isFinite(upbit) || !isFinite(global)) return null;
    const km = (upbit / global - 1) * 100;
    const norm = Math.max(0, Math.min(100, Math.round((km + 3) / (5 + 3) * 100)));
    const score = 100 - norm; // 과열↑ → 점수↓ (byValue: ≤35→▼ 빨강)
    return { v: score, _km: km };
  }catch(e){ return null; }
}
