...
function _buildSimPositionLine(stock, btSt, svVerdict){
  ...
  if(btSt.state === 'holding' && btSt._isBuySignal){
    // [v2.0] C 판정이 '회피' 또는 '관망'이면 ▲ 매수 UI 가림
    //   (BT 내부 로직은 그대로 진행, chartMarker만 차단)
    //   v1.x 강등 분기('[강등]' 마커)는 v2.0에서 폐기 — 모멘텀 보정으로 대체
    const _vAct = svVerdict?.action || '';
    if(_vAct === '회피' || _vAct === '관망'){
      const _titleByAct = _vAct === '회피' ? '엔진시뮬 회피' : '엔진시뮬 관찰';
      const _subByAct = _vAct === '회피'
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    // 1) 4축 점수 수집
    const _scores4 = {
      readyScore: qs ? (qs.readyScore ?? 0) : 0,                  // 바닥신호
      entryScore: qs ? (qs.entryScore ?? 0) : 0,                  // 반등신호
      trendScore: qs ? (qs.trendScore ?? qs.score ?? 0) : 0,      // 추세강도
