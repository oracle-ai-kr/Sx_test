...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  ...
  if(extras.stoch && extras.stoch.v!=null) g5.push({k:'스토캐스틱', v:extras.stoch.v, byValue:true});  // [S363] %K — 값 정방향
  if(extras.bb && extras.bb.v!=null) g5.push({k:'볼린저%B', v:extras.bb.v, byValue:true});            // [S364] 밴드 내 위치
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  try {
    ...
    if(indicators){
      ...
      // [S362] 변동성 (ATR%) — 추세·구조. 높을수록 변동성 큼(inverse 색 → 심하면 빨강/양호 녹색)
      //   시장별 기준: 국내/미국 ATR% 1~5% / 코인 1~9% 를 0~100 매핑 (추정값, 실측 후 조정)
      if(indicators.atr && indicators.atr.ratio > 0){
        const _isCoin = (stock.market === 'COIN' || stock.market === 'coin');
        const _volaMax = _isCoin ? 9 : 5;
