...
function _sxbCircle(score, label, big, neutral, delta){
  ...
  const col = neutral?'var(--text3)':_sxbColor(sc);
  const fs = big?20:13;
  return `<div class="sxb-circle${big?' sxb-big':''}">`
    ...
    + `<text x="50%" y="50%" text-anchor="middle" dominant-baseline="central" font-size="${fs}" font-weight="800" fill="var(--text)">${Math.round(sc)}</text>`
    + `</svg>`
    + (label?`<div class="sxb-circle-label"${(!neutral&&typeof delta==='number'&&Math.abs(delta)>2)?` style="color:${delta>0?'var(--buy)':'var(--sell)'};font-weight:800"`:''}>${label}</div>`:'')
    + `</div>`;
}
...
function _sxbHTML(){
  ...
  groups.forEach(g=>g.items.forEach(it=>{
    ...
    if(!it.neutral&&typeof it.d==='number') allD.push(it.d);
  }));
  const total = all.length?Math.round(all.reduce((a,b)=>a+b,0)/all.length):0;
  const headDelta = allD.length?Math.round(allD.reduce((a,b)=>a+b,0)/allD.length):null;
  const tone = _sxbTone(total);
  // [S359] 전이 방향 배지 (momBadge 디자인 차용 + 전광판 종합 전이 headDelta 데이터)
  let transBadge = '';
  if(headDelta!=null){
    let bi,bl,bc;
    if(headDelta>2){ bi='✨'; bl='상승 흐름'; bc='var(--buy)'; }
    else if(headDelta<-2){ bi='⚠'; bl='하락 흐름'; bc='#f59e0b'; }
    else { bi='➡'; bl='보합'; bc='#94a3b8'; }
    transBadge = `<span class="sxb-badge" style="color:${bc};background:${bc}1A;border:1px solid ${bc}">${bi} ${bl}</span>`;
  }
  // [S359] 휩소주의 배지 — 방향 엇갈림(momBadge↔headDelta) AND 신뢰도 낮음(lowConf)
  //   단기 모멘텀은 한쪽인데 종합 전이는 따라오지 않고, 추세 약함/변동성 큼/횡보가 겹칠 때
  let whipBadge = '';
  const _momDir = B.verdict && B.verdict.momBadge && B.verdict.momBadge.direction;
  const _headDir = headDelta==null ? null : (headDelta>2?'up':headDelta<-2?'down':'flat');
  if(_momDir && (_momDir==='up'||_momDir==='down') && _headDir && _momDir!==_headDir && B.lowConf){
    const _P='#a855f7';
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    + _sxbCircle(total, '', true, false)
    + `<div class="sxb-head-info">`
    + `<div class="sxb-head-tone" style="color:${tone.c}">${tone.t}</div>`
    + ((transBadge||whipBadge)?`<div class="sxb-head-badges">${transBadge}${whipBadge}</div>`:'')
    + `<div class="sxb-head-sub">종합 ${total} · ${_sxbGrade(total)}등급 · ${all.length}개 지표</div>`
    + `</div>`
  ...
  groups.forEach(g=>{
    ...
    g.items.forEach(it=>{ det += _sxbCircle(it.v, it.k, false, !!it.neutral, it.d); });
    det += `</div></div>`;
  });
  det += `</div>`;
  return h + mini + det;
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf){
  D = D || {};
  const g1=[], g2=[], g3=[], g4=[];
  // ① 추세·구조
  ...
  if(structPos!=null) g1.push({k:'구조위치', v:structPos, neutral:true, d:D.struct});
  // ② 모멘텀·진입
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  body.innerHTML = `
    ${chartHTML}
    ${btStateBannerHTML}
    ${_buildScoreBoard(scores, stock._svScores4, _boardStruct, _boardPb, _boardDeltas, stock._svVerdict, _lowConf)}
    ${tradeHistHTML}
