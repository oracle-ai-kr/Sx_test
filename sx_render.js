...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  // [S859] 레시피 반등표 A/B — 토글 ON이면 봉별 발동 평가 콜백을 SXVVAL.run에 주입(→ scores.recipeSig → C 레시피표).
  //   qs.ind 재사용(calcAllScreener 재계산 없음). 강 기준=시장별(S856 t+k 근거): kr 겹침3+ / us 정배 발동 / coin 없음(real 약함).
  ...
  var _rcpCb=(_rcpOn && window.SXRecipeSignal && SXRecipeSignal.evalBar) ? function(qs, slice, e){
    ...
    if(!r) return null;
    var strong=(mk==='kr') ? (r.pure && r.realK>=3) : (mk==='us') ? (r.pure && r.lt==='bull') : false;
    return { pure:r.pure, mixed:r.mixed, strong:strong, realK:r.realK, fakeK:r.fakeK };
  } : null;
  for(var i=0;i<use.length;i++){
    var s=use[i];
    if(onProgress) onProgress(i+1, use.length, s.name);
