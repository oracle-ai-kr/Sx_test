...
async function runAnalysis(stock){
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  console.log('[fin-fix] runAnalysis 진입:', {code: stock.code, market: stock.market, currentMarket, hasFin: !!stock._financial, finSrc: stock._financial?._source, fnAvail: typeof fetchFinancialData === 'function'});
  if (currentMarket === 'kr' && stock.code && (!stock._financial || stock._financial._source === 'none')) {
    if (typeof fetchFinancialData === 'function') {
      try {
        ...
        console.log('[fin-fix] fetch 결과:', {code: stock.code, gotFin: !!fin, src: fin?._source, perVal: fin?.per, revVal: fin?.revenue, keys: fin ? Object.keys(fin).filter(k => fin[k] != null).length : 0});
        if (fin) stock._financial = fin;
      } catch (e) { console.warn('[fin-fix] fetch 에러:', e.message); }
    } else {
      console.warn('[fin-fix] fetchFinancialData 함수 없음');
    }
  } else {
    console.log('[fin-fix] 보강 스킵 (조건 미해당)');
  }
