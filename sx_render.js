...
function _sxbHTML(){
  ...
  groups.forEach(g=>g.items.forEach(it=>{
    // [S367] 종합점수 기여 — inverse(높을수록 나쁨: 구조위치/변동성/스토캐스틱/볼린저)는
    //   역산(100-v)해서 정방향으로 통일. 과열·과매수가 종합점수를 올바르게 끌어내림.
    if(typeof it.v==='number'){
      if(it.inverse) all.push(100 - it.v);
      else if(!it.neutral) all.push(it.v);
    }
