...
var _sxvPredsOn = { slope:1, slopeTurn:1, rsiGc:1, obv:1, macd:1, closeVsMa:1, rsi50:1, macdCross:1, slopeOrRsi:1, slopeAndRsi:1, resBreak:1, hhBreak:1, volSurge:1, align3:1, adx:1, bandWalk:1, andCore:1, andStrong:1, upsideMix:1, aGate:0, aBuy:0, momUp:0, aGateMom:0, aReady:0, aEntry:0, aRebound:0, aGateRebound:0 };   // [S709→S710] 신호 on/off · [S715] 엔진4종 · [S717] 반등4종(기본 OFF=무거움)
...
function _sxvTogglePred(id){ _sxvPredsOn[id] = _sxvPredsOn[id] ? 0 : 1; _sxvRerender(); }
...
function _buildValidatorCard(stock, indicators){
  ...
  var predChips = predList.map(function(id){ var on = !!_sxvPredsOn[id]; var ac = _sxvIsAbl(id) ? '#d97706' : _sxvIsEng(id) ? '#7c3aed' : '#0891b2'; var lab = (typeof SXUV !== 'undefined' && SXUV.PREDICTORS && SXUV.PREDICTORS[id]) ? SXUV.PREDICTORS[id].label : id; return '<span onclick="_sxVib(6);window._sxvTogglePred&&_sxvTogglePred(\'' + id + '\')" style="font-size:9px;font-weight:700;padding:3px 8px;border-radius:11px;cursor:pointer;border:1px solid ' + (on ? ac : 'var(--border)') + ';background:' + (on ? (ac + '1a') : 'var(--surface2)') + ';color:' + (on ? ac : 'var(--text3)') + (on ? '' : ';opacity:.55') + '">' + lab + '</span>'; }).join('');
  var predRow = '<div style="margin-bottom:7px"><div style="font-size:9px;color:var(--text3);margin-bottom:3px">신호 (탭하여 on/off · 점수에 들어가는 후보들)</div><div style="display:flex;gap:4px;flex-wrap:wrap">' + predChips + '</div></div>';
  // 출처
