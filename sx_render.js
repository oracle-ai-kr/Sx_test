...
// ════════ [S755] 📋 검증 배지 인벤토리 — 교차검증탭에서 검증 완료된 신호를 상수(SSOT)로 박아 분석탭에 표시. 회색=비활성 / 컬러=현재 종목이 풀(이 종목의 MA배열 상태)+재료 둘 다 충족 시 점등. cond는 {feat,dir,th} 기계가독 → 분석탭이 나중에 재료 매칭에 그대로 재사용. 표시 전용·엔진 무변경. ════════
var SX_VERIFIED_BADGES = [
  { id:'deadcat_genuine', icon:'🔴', name:'데드캣 진짜반등', color:'#dc2626',
    poolLt:'bear', poolName:'역배열+단기약세',
    cond:[{feat:'volOsc', dir:'lt', th:-39.6, label:'거래량OSC < −39.6'}],
    support:[],
    n10:78, n5:74, base:50, sample:'23', date:'2026-06-26', verdict:'adopt',
    note:'단독 사용. 투매·워시아웃에서 진짜 반등.' },
  { id:'pullback_healthy', icon:'🔵', name:'눌림목 건강', color:'#2563eb',
    poolLt:'bull', poolName:'정배열+단기조정',
    cond:[{feat:'bbPctB', dir:'gt', th:1.0, label:'BB%B > 1.0'}],
    support:[{feat:'dev20', dir:'gt', th:8, label:'MA20이격도 > 8'}],
    n10:74, n5:63, base:52, sample:'19~27', date:'2026-06-26', verdict:'adopt',
    note:'BB%B 단독 기본 · MA20이격도 보조. 표본30미만·재확인 필요.' }
];
if(typeof window!=='undefined'){ window.SX_VERIFIED_BADGES = SX_VERIFIED_BADGES; }
// 재료값 읽기(분석탭 adv = indicators._advanced). 켜짐 판정은 cond만(support는 표시용).
...
function _vbDetect(badge, adv){
  if(!adv) return { pool:false, cond:false, on:false };
  var ltAlign = (typeof _ltStr733==='function') ? _ltStr733(adv.maAlignLT) : 'off';
  var maBull = !!(adv.maAlign && adv.maAlign.bullish);
  var poolOk = (ltAlign===badge.poolLt && !maBull);
  var condOk = badge.cond.every(function(c){ return _vbCondMet(c, adv); });
  return { pool:poolOk, cond:condOk, on:(poolOk && condOk) };
}
function _buildBadgeInventoryCard(stock, indicators){
  var adv = (indicators && indicators._advanced) ? indicators._advanced : null;
  var GR='var(--text3)';
  var rowsHtml = SX_VERIFIED_BADGES.map(function(b){
    var d = _vbDetect(b, adv);
    var on = d.on, poolOnly = (d.pool && !d.cond);
    var chipStyle = on ? ('background:'+b.color+';color:#fff;border:1px solid '+b.color)
      : poolOnly ? ('background:'+b.color+'1A;color:'+b.color+';border:1px solid '+b.color+'66')
      : 'background:var(--surface2);color:var(--text3);border:1px solid var(--border)';
    var statusTxt = on ? '✅ 감지됨' : poolOnly ? '◐ 풀 일치 · 신호 대기' : '○ 비활성';
    var statusCol = (on||poolOnly) ? b.color : GR;
    var condTxt = b.cond.map(function(c){ var v=_vbFeatVal(c.feat, adv); var ok=_vbCondMet(c, adv); return '<span style="color:'+(ok?b.color:GR)+'">'+(ok?'✓ ':'')+c.label+(v!=null?' <span style="color:var(--text3)">('+v.toFixed(1)+')</span>':'')+'</span>'; }).join(' · ');
    var supTxt = b.support.length ? ' <span style="color:var(--text3)">+보조 '+b.support.map(function(c){ var ok=_vbCondMet(c, adv); return (ok?'✓':'')+c.label; }).join(', ')+'</span>' : '';
    var verdictBadge = b.verdict==='adopt' ? '<span style="color:#16a34a;font-weight:700">✅ 채택</span>' : b.verdict==='strengthen' ? '<span style="color:#d97706;font-weight:700">⚠️ 강화</span>' : '<span style="color:#dc2626;font-weight:700">❌ 제거</span>';
    return '<div style="padding:9px 10px;margin:6px 0;border-radius:9px;background:var(--surface2);border:1px solid '+(on?b.color+'66':'var(--border)')+'">'
      +   '<div style="display:flex;align-items:center;gap:7px;margin-bottom:5px">'
      +     '<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;'+chipStyle+'">'+b.icon+' '+b.name+'</span>'
      +     '<span style="margin-left:auto;font-size:9.5px;font-weight:700;color:'+statusCol+'">'+statusTxt+'</span>'
      +   '</div>'
      +   '<div style="font-size:9px;color:var(--text3);line-height:1.5">풀: <b style="color:var(--text2)">'+b.poolName+'</b> '+(d.pool?('<span style="color:'+b.color+'">●일치</span>'):'<span>●불일치</span>')+'</div>'
      +   '<div style="font-size:9.5px;line-height:1.6;margin-top:2px">신호: '+condTxt+supTxt+'</div>'
      +   '<div style="font-size:8.5px;color:var(--text3);margin-top:4px">N10 '+b.n10+'% · N5 '+b.n5+'% (기저 '+b.base+'%) · 표본 '+b.sample+' · '+b.date+' · '+verdictBadge+'</div>'
      +   (b.note?'<div style="font-size:8px;color:var(--text3);margin-top:2px;font-style:italic">'+b.note+'</div>':'')
      + '</div>';
  }).join('');
  var onCount = SX_VERIFIED_BADGES.filter(function(b){ return _vbDetect(b, adv).on; }).length;
  ...
  var rightHtml = '<span style="font-size:9.5px;font-weight:700;color:'+(onCount?'#16a34a':'var(--text3)')+'">'+(onCount? (onCount+' 감지'):(String(SX_VERIFIED_BADGES.length)+'종'))+'</span>';
  var desc = '<div style="font-size:8.5px;color:var(--text3);line-height:1.5;margin-bottom:4px">교차검증탭에서 검증 완료된 신호. <b style="color:var(--text2)">회색=비활성</b>, 현재 종목이 <b>풀(이 종목 MA배열 상태)+재료</b> 둘 다 충족하면 <b style="color:var(--text2)">컬러 점등</b>. 풀만 맞으면 ◐ 대기.</div>';
