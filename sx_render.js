...
function _sxbCircle(score, label, big, neutral, delta){
  ...
  const fs = big?20:13;
  // [S360] 점수 숫자에 전이 색 직접 적용 — 라벨보다 큰 글씨라 임팩트 강함. 라벨은 기본색 복귀
  const numColor = (!neutral && typeof delta==='number' && Math.abs(delta)>2)
    ? (delta>0?'var(--buy)':'var(--sell)') : 'var(--text)';
...
function _sxbHTML(){
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    + _sxbCircle(total, '', true, false, headDelta)
