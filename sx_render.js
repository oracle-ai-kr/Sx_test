...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  body.innerHTML = `
    ...
    ${(()=>{
      ...
      //   부문별 점수 펼쳤을 때 가장 먼저 보이도록 본문 최상단 배치
      ...
      const reconcileHTML = nar.reconcile
        ...
        : '';
      return `<div class="itp-card show" style="margin:0 0 10px;padding:10px 12px;border-left:3px solid ${toneColor};background:${toneBg};border-radius:8px">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
          <span style="font-size:10px;font-weight:700;color:${toneColor};letter-spacing:-.2px">📊 종합해석 가이드</span>
          <span style="font-size:9px;color:var(--text3)">· ${conflictLbl}</span>
        </div>
        <div style="font-size:13px;font-weight:700;color:var(--text);margin:2px 0 6px">${nar.headline}</div>
        ${reasonsHTML}
        <div style="font-size:11px;line-height:1.6;color:var(--text)">${nar.guide}</div>
        ${reconcileHTML}
        <div style="margin-top:8px;padding-top:6px;border-top:1px dashed var(--border);font-size:9px;color:var(--text3)">
