...
function _buildSimPositionLine(stock, btSt, svVerdict){
  ...
  if(btSt.state === 'holding' && btSt._isBuySignal){
    ...
    //   → 기존: "매수 신호 방금 발생 / 제안 진입가 / 목표 / 손절" 풀 안내
    //   → 수정: 행동 유도 문구 숨기고 "엇갈림" 명시 (외부 배너 '회피'와 일관)
    //   설계 의도(시뮬레이터 행적 표시)는 보존하되,
    //   사용자 혼란 방지를 위해 진입가/TP/SL 등 실행 유도 요소만 제거
    const _vAct = svVerdict?.action || '';
    if(_vAct === '회피'){
      return `<div style="${BOX_STYLE}">
        <div style="${TITLE_STYLE}">📊 엔진시뮬 신호포착 (참고)</div>
        <div style="${MAIN_STYLE}"><span style="color:var(--text2);font-weight:700">시뮬 매수 트리거 감지 · 현재 판정 회피</span></div>
        <div style="${SUB_STYLE}">BT 시뮬은 진입했으나 현시점 C 판정이 회피이므로 실행 보류 권장</div>
      </div>`;
    }
  ...
  if(btSt.state === 'holding'){
    ...
    // [FIX-2] 외부 C 판정이 회피면 타이틀에 (참고) 명시해 엇갈림 투명화
    const _vActHold = svVerdict?.action || '';
    const _titleSuffix = (_vActHold === '회피') ? ' (참고)' : '';
    const _conflictNote = (_vActHold === '회피')
      ? `<div style="${SUB_STYLE};color:var(--text3)">※ 현재 판정 회피 · 시뮬 포지션 유지 정보는 참고용</div>`
      : '';

    return `<div style="${BOX_STYLE}">
      <div style="${TITLE_STYLE}">📊 엔진시뮬 보유중${_titleSuffix}</div>
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        ${(()=>{
          ...
          html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:11px;font-weight:800;color:var(--text)">신호</span><span style="font-size:14px;font-weight:800;color:var(--'+rCls+')">'+_report.ready.score+_deltaTag(_readyDelta)+'</span><span style="font-size:9px;color:var(--text3)">'+_report.ready.level+' · '+_report.ready.progress+'</span></div>';
          ...
          if(_mom && _mom.history.length >= 3){
            ...
            html += '<div style="margin-bottom:10px">';
            html += '<div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:4px">신호 추이 (최근 '+_mom.lookback+'봉)</div>';
