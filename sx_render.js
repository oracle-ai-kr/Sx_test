    ...
    if(btCandles && btCandles.length >= 60 && typeof SXE!=='undefined' && SXE.runBtEngine){
      ...
      if(btR && !btR.error){
        ...
        if(_btTradesNow < BT_MIN_TRADES && !stock._analCandlesExtended && _extSupported
          ...
          console.log(`[runAnalysis] BT 거래 ${_btTradesNow}회 < ${BT_MIN_TRADES} → 자동 확장 시도`);
          ...
          try{
            ...
            if(_extraCandles && _extraCandles.length > 0){
              ...
              console.log(`[runAnalysis] 확장 성공: ${_extraCandles.length}봉 추가 → 총 ${_mergedRows.length}봉`);

              // BT 2차 실행 (확장 데이터로)
              const _btR2 = SXE.runBtEngine(_mergedRows, _analTF, _btParams);
              if(_btR2 && !_btR2.error){
                const _btTrades2 = _btR2.totalTrades ?? 0;
                console.log(`[runAnalysis] BT 재실행: 거래 ${_btTradesNow}회 → ${_btTrades2}회`);
                // 결과 교체
                _analBtResult = _btR2;
              ...
              console.log(`[runAnalysis] 확장 실패 (null 반환) — 기존 데이터 유지`);
            }
          }catch(extErr){
            console.warn('[runAnalysis] 자동 확장 에러:', extErr);
