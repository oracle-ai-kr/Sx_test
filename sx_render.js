...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  // [S372] indicators에 maAlign 누락 보강 — 모든 진입 경로 커버
    ...
    if(_candlesFix && _candlesFix.length >= 20){
      ...
      try {
        ...
        if(_fresh && _fresh.maAlign){
          ...
          _diagSnap.fixResult = `OK keys=${Object.keys(_fresh.maAlign).join(',')}`;
        } else {
          _diagSnap.fixResult = `FAIL (fresh=${!!_fresh}, fresh.maAlign=${!!(_fresh && _fresh.maAlign)})`;
        }
      } catch(e){ _diagSnap.fixResult = 'ERR ' + (e.message || 'unknown'); }
    } else {
      _diagSnap.fixResult = 'NO_CANDLES';
    }
