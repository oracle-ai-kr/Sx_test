// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S824 · build 20260630-1920 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _deadcatShortBracket(mk, onProgress){
  ...
  return { ok:true, both:both, mixed:mixed, stocksUsed:stocksUsed };
}
...
function _deadcatShortRender(res, mk){
  ...
  var grpDef=[ ['(하,하) 장기·단기 모두 역배', b, RED], ['(하,혼조) 장기 역배·단기 혼조', m, AMB] ];
  var rows='';
  for(var g=0;g<grpDef.length;g++){
    var nm=grpDef[g][0], st=grpDef[g][1], col=grpDef[g][2];
    var small=st.n<15, rcol=small?T3:(st.rate>=65?GRN:(st.rate>=50?AMB:RED));
    var avgStr=(st.avg!=null)?((st.avg>=0?'+':'')+st.avg.toFixed(1)+'%'):'—', avgCol=(st.avg==null)?T3:(st.avg>=0?GRN:RED);
    rows+='<tr>'
      +'<td style="padding:6px 8px;font-weight:800;color:'+col+';font-size:10.5px">'+nm+'</td>'
      +'<td style="padding:6px 8px;text-align:right;font-weight:800;color:'+rcol+';font-size:12px">'+st.rate.toFixed(0)+'%</td>'
  ...
  var diff=m.rate-b.rate;   // 혼조 - 둘다하 (양수=둘다하가 더 나쁨=차단가치)
  var verdict;
  if(b.n<15||m.n<15) verdict='<span style="color:'+AMB+'">표본 부족 — 한쪽 15봉 미만이라 판정 보류 (불장이라 (하,하) 자체가 드뭄일 수 있음)</span>';
  else if(diff>=15) verdict='<span style="color:'+GRN+'">✅ (하,하) 진짜비율이 (하,혼조)보다 <b>'+diff.toFixed(0)+'%p 낮음</b> — <b>(하,하) 진입 차단 가치 있음</b>. 단기까지 무너진 역배는 데드캣 비중↑'+((b.avg!=null&&b.avg<0)?' (평균수익도 음수)':'')+'</span>';
  else if(Math.abs(diff)<=3) verdict='<span style="color:'+AMB+'">차이 <b>±3%p 이내</b> — (하,하) 차단해도 진짜 반등까지 같이 버림. <b>S731 재판(차단 무의미)</b>. 역배 변별 불가 재확인</span>';
  else verdict='<span style="color:'+T2+'">약한 차이(Δ'+diff.toFixed(0)+'%p) — 차단 효과 제한적. ±3~15%p 회색지대</span>';
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px">역배열 <b>deadcat-real 발동봉</b>을 <b>단기 정렬</b>로 쪼갬: <b style="color:'+RED+'">(하,하)</b>=장기(60·120·200)+단기(5·20·60) 모두 역배 · <b style="color:'+AMB+'">(하,혼조)</b>=장기 역배·단기는 정배 아닌 혼조. 각 N10 진짜비율(상승)·평균수익. <b>(하,하)가 유의하게 나쁘면</b> 그 칸만 진입 차단할 가치. 대표+관심 전수·표본='+(res.both.length+res.mixed.length)+'봉·'+(res.stocksUsed||0)+'종목.</div>';
  return head
