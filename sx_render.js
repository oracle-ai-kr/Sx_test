...
function calcBtScore(btData, stock){
  ...
  let tradeCap;
  if(trades >= 20)      tradeCap = 100;
  else if(trades >= 10) tradeCap = 90;
  else if(trades >= 5)  tradeCap = 78;
  else                  tradeCap = 60;
...
// [S481] 캔들 전이 점수 카드 (분석탭 전광판 아래, 실험 영역)
// ===== [S549] 단기추세매매 (실험 카드) — MA 골든크로스 진입 → 데드크로스 청산. 보조지표 AND(최근 N봉). 독립 룰. =====
const _TREND_MKT_DEFAULT = { kr:[5,9], us:[5,14], coin:[2,5] };
const _TREND_AUX = [
...
function _trendCfg(market){
  ...
  let cfg={ s:d[0], l:d[1], n:3, aux:{} };
  try { const raw=localStorage.getItem('SX_TREND_'+market); if(raw){ const o=JSON.parse(raw); if(o){ cfg.s=+o.s||d[0]; cfg.l=+o.l||d[1]; cfg.n=+o.n||3; cfg.aux=(o.aux&&typeof o.aux==='object')?o.aux:{}; } } } catch(_){}
...
function _trendBt(rows,cfg){
  ...
  let rsi,rsiSig,mac,vMa,obv,obvSig,ma20;
  if(need.length){
    if(A.rsiGc){ rsi=_trRsi(close,14); rsiSig=_trEma(rsi,9); }
    if(A.macd0||A.macdGc){ mac=_trMacd(close); }
    if(A.vol){ vMa=_trSma(vol,5); }
    if(A.obv){ obv=_trObv(close,vol); obvSig=_trSma(obv,14); }
    if(A.ma20){ ma20=_trSma(close,20); }
  }
  // 각 선택 보조조건이 최근 N봉 안에 한 번이라도 충족되는지 (조건별 OR-in-window, 조건간 AND)
  const auxOkWindow=(i)=>{
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){ if(gc && auxOkWindow(i)) pos={entry:close[i],entryIdx:i}; }
    else if(dc){ const pnl=(close[i]/pos.entry-1)*100; trades.push({entry:pos.entry,exit:close[i],pnl:+pnl.toFixed(2),bars:i-pos.entryIdx,entryIdx:pos.entryIdx,exitIdx:i,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||'',exitDate:(rows[i]&&(rows[i].date||rows[i].t))||''}); pos=null; }
  }
  let open=null;
...
function _trendState(rows,cfg,bt){
  ...
  if(bt.open) return { label:`보유중 · 진입 ${bt.open.bars}봉차 · 평가 ${bt.open.pnl>=0?'+':''}${bt.open.pnl}%`, color: bt.open.pnl>=0?'#22c55e':'#e8365a' };
  // 마지막 크로스 판정
  ...
  if(lastGc>=0 && lastGc>lastDc){ const ago=(n-1)-lastGc; if(ago<=N) return { label:`🟢 진입 신호 (골든크로스 ${ago}봉 전)`, color:'#22c55e' }; return { label:`상승 추세 (골든크로스 ${ago}봉 전, 신호창 지남)`, color:'var(--text2)' }; }
  if(lastDc>=0){ const ago=(n-1)-lastDc; return { label:`🔴 청산/관망 (데드크로스 ${ago}봉 전)`, color:'#e8365a' }; }
...
function _trendRenderInner(){
  ...
  const bt=_trendBt(rows,cfg);
  const st=_trendState(rows,cfg,bt);
  ...
  const head=`<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:13px;font-weight:800;color:var(--text)">📈 단기추세매매</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span><span onclick="window._trendOpenDetail&&_trendOpenDetail()" title="거래내역·자산흐름 보기" style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;background:#7c3aed22;color:#7c3aed;border:1px solid #7c3aed66;cursor:pointer">검증</span><span style="font-size:10px;font-weight:700;color:${_dCol};margin-left:auto">${_isToday?'':'⚠️ '}${_dTxt} 종가기준</span></div>`;
  // MA 입력
  ...
  const maRow=`<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:8px;font-size:11px;color:var(--text2)">
    ...
    <span style="margin-left:8px">보조 최근</span>${_inp('sxTrendN',cfg.n)}<span>봉</span>
    <span onclick="window._trendReset&&_trendReset()" style="margin-left:auto;font-size:11px;font-weight:800;padding:5px 11px;border-radius:7px;background:var(--surface2);color:var(--text2);border:1px solid var(--border);cursor:pointer">리셋</span>
    <span onclick="window._trendApply&&_trendApply()" style="font-size:11px;font-weight:800;padding:5px 12px;border-radius:7px;background:#2563eb;color:#fff;cursor:pointer">적용</span>
  </div>`;
  // 보조지표 토글 칩
  const chips=_TREND_AUX.map(a=>{ const on=!!(cfg.aux&&cfg.aux[a.k]); const stl=on?'background:#2563eb;color:#fff;border-color:#2563eb':'background:var(--surface2);color:var(--text3);border-color:var(--border)'; return `<span onclick="window._trendToggleAux&&_trendToggleAux('${a.k}')" style="font-size:10px;font-weight:700;padding:4px 9px;border-radius:14px;border:1px solid;cursor:pointer;${stl}">${a.label}</span>`; }).join('');
  const chipRow=`<div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:9px">${chips}</div>`;
  // 상태
  ...
  if(bt && bt.totalTrades>0){
    ...
    grid=`<div style="display:flex;gap:5px;margin-bottom:5px">${_trendGridCell(bt.winRate+'%','승률',wc)}${_trendGridCell(bt.pf,'손익비',pfc)}${_trendGridCell((bt.totalPnl>=0?'+':'')+bt.totalPnl+'%','수익률',pc)}</div>
  ...
  const note=`<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · ${cfg.s}MA×${cfg.l}MA 골든크로스 진입 → 데드크로스 청산${Object.keys(cfg.aux||{}).filter(k=>cfg.aux[k]).length?` · 보조 ${Object.keys(cfg.aux).filter(k=>cfg.aux[k]).length}개 AND(최근${cfg.n}봉)`:' · MA 단독'}</div>`;
  return head+maRow+chipRow+stateRow+grid+note;
}
function _buildTrendCard(stock,indicators){
  ...
  // [S552] BT와 동일 봉수 정합 — KIS ON 700 / OFF 600 (주·월봉 400). _btTargetBars(sx_bt.js 전역)로 명시 슬라이스.
  try { if(typeof _btTargetBars==='function'){ const _tfTr=(typeof _analTF!=='undefined'&&_analTF)?_analTF:'day'; const _cap=_btTargetBars(market,_tfTr); if(_cap>0 && rows.length>_cap) rows=rows.slice(-_cap); } } catch(_){}
...
function _trendApply(){
  ...
  _trendSave(m,cfg); _trendRerender();
}
...
function _trendReset(){ const ctx=window._sxTrendCtx; if(!ctx) return; const m=ctx.market; const d=_TREND_MKT_DEFAULT[m]||[5,9]; _trendSave(m,{ s:d[0], l:d[1], n:3, aux:{} }); _trendRerender(); }
if(typeof window!=='undefined'){ window._trendApply=_trendApply; window._trendToggleAux=_trendToggleAux; window._trendReset=_trendReset; }
...
function _trendDetailInner(bt,cfg,ctx){
  ...
  const sub=`<div style="font-size:10px;color:var(--text3);margin-bottom:10px">${cfg.s}MA×${cfg.l}MA 골든→데드 · 보조: ${auxLbl}${auxOn.length?` (최근${cfg.n}봉)`:''}${nm}</div>`;
  ...
  return head+sub+flow+listHead+list;
}
function _trendOpenDetail(){
  ...
  const cfg=_trendCfg(ctx.market); const bt=_trendBt(ctx.rows,cfg);
  _trendCloseDetail();
  ...
  ov.innerHTML='<div style="width:100%;max-width:400px;max-height:86vh;overflow:auto;background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:16px 18px;box-shadow:0 12px 40px rgba(0,0,0,.3)">'+_trendDetailInner(bt,cfg,ctx)+'</div>';
...
function _buildTransitionCard(stock, indicators){
  ...
  // [S525] 미니 백테스트 컨텍스트 저장 — 버튼 클릭 시 sx_candle_bt.js가 읽음 (rows/market/tf)
  try { window._sxCTBT = { rows: rows, market: _mktCT, tf: _tfCT, name: (stock && (stock.name || stock.code)) || '' }; } catch(_eCtx){}
  ...
  const _dateLabel = _isToday ? `${_dateTxt} 종가기준 판단` : `⚠️ ${_dateTxt} 종가기준 판단`;   // [S505] 확정봉 종가 기준 명시
  const head = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:13px;font-weight:800;color:var(--text)">🧪 캔들 전이 점수</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span><span onclick="window.SXCandleBT&&SXCandleBT.open()" title="과거 데이터로 이 예측의 적중률 검증" style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;background:#7c3aed22;color:#7c3aed;border:1px solid #7c3aed66;cursor:pointer">검증</span><span style="font-size:10px;font-weight:700;color:${_dateCol};margin-left:auto">${_dateLabel}</span></div>`;
  const wrap = inner => `<div style="margin:8px 10px;padding:12px 14px;border-radius:12px;background:var(--surface);border:1px solid var(--border)">${head}${inner}</div>`;
  if(!r.active){
  ...
  const col = r.score>=30 ? '#e3493b' : r.score<=-30 ? '#2563eb' : '#f59e0b';   // [S489] 캔들 컨벤션 일치 — 양봉유망=빨강 / 음봉유망=파랑 (미니캔들 색과 동일)
  const sign = r.score > 0 ? '+' : '';
  const top = `<div style="display:flex;align-items:baseline;gap:8px"><span style="font-size:26px;font-weight:800;color:${col}">${sign}${r.score}</span><span style="font-size:13px;font-weight:700;color:${col}">${r.label}${r.nextBar?` <span style="font-size:11px;font-weight:600;color:var(--text3)">(${r.nextBar})</span>`:''}</span><span style="font-size:10px;color:var(--text3);margin-left:auto">${r.mode==='rebound'?`음봉 ${r.streak}연속`:r.mode==='reversal'?`양봉 ${r.streak}연속`:r.mode==='flipUp'?`🔺 ${r.patName||'양전환'}`:r.mode==='flipDown'?`🔻 ${r.patName||'음전환'}`:`8봉 음${r.red8}·양${r.green8}`}</span></div>`;
  // [S534] 신뢰도 배지 — 바스켓 실측 기반 (방향·점수구간·시장)
  const _conf = _ctConfidence(r.score, _mktCT, r.sbFired, r.regime);
  const confHtml = `<div style="margin-top:8px"><span style="font-size:11px;font-weight:800;color:${_conf.color};padding:3px 9px;border-radius:7px;background:${_conf.color}1a;border:1px solid ${_conf.color}55">신뢰도 ${_conf.label}</span><div style="font-size:10px;color:var(--text3);line-height:1.55;margin-top:5px">${_conf.why}</div></div>`;
  // [S482] 0중심 양방향 바 — 중앙(50%)이 0, 양수는 오른쪽(초록)/음수는 왼쪽(빨강)으로 채움
  const _mag = Math.min(50, Math.abs(r.score)/2);
  const _fill = r.score >= 0
  ...
  const warnHtml = r.warn ? `<div style="font-size:11px;color:#ef4444;margin:7px 0;padding:6px 9px;background:#ef444415;border-radius:7px;line-height:1.5">⚠️ ${r.warn}</div>` : '';
  const reasonsHtml = (r.reasons && r.reasons.length) ? `<div style="font-size:11px;color:var(--text2);line-height:1.75;margin-top:6px">${r.reasons.map(x=>`· ${x}`).join('<br>')}</div>` : '';
  const note = `<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · −100~+100 (양수=양봉 전이 유망, 음수=음봉 전이 유망)</div>`;
  ...
  { const _lb = _seq.length ? _seq[_seq.length-1] : null;
    ...
    if(_lb && (r.score>=30 || r.score<=-30) && _nmd(_lb.date)===r.nextBar){
      const _lbUp = (+_lb.close) >= (+_lb.open);
      const _predBull = r.score >= 30;
      const _hit = (_predBull && _lbUp) || (!_predBull && !_lbUp);
      _hitRow = `<div style="text-align:right;margin:3px 2px 0;font-size:9.5px;font-weight:800;color:${_hit?'#16a34a':'#f59e0b'}">현재봉 ${_nmd(_lb.date)} ${_lbUp?'양봉':'음봉'} · ${_hit?'예측 일치 ✓':'예측 불일치 ✗'}</div>`;
    } }
  return wrap(top + confHtml + bar + _seqRow + _hitRow + warnHtml + reasonsHtml + note);
}
...
//   방향: 강한상승+2 / 상승조짐+1 / 방향혼조0 / 하락조짐-1 / 강한하락-2
...
function _sxbHTML(){
  ...
  const _isWhip = _momDir && (_momDir==='up'||_momDir==='down') && _momDir!==_distBin && B.lowConf;
  if(_isWhip){
    const _P='#a855f7';
  ...
  // [S361] 헤드 큰 서클 점수 색 — 분포 방향 기준 (델타 인자에 분포 부호값 전달)
  const headColorDelta = diff>=2 ? 5 : (diff<=-2 ? -5 : 0);
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    ...
    + `<div class="sxb-head-badges">${transBadge}${whipBadge}${_dumpBadge}${_safetyBadge}${_mixBadge}${_mtfBadge}<span id="sxMacroSoonSlot">${_sxMacroBadgeHTML(_sxMacroEvents().find(e=>e.mode==='soon'))}</span><span id="sxMacroResultSlot">${_sxMacroBadgeHTML(_sxMacroEvents().find(e=>e.mode==='result'))}</span></div>`
...
function _classifyBoardDist(groups){
  ...
  if(diff >= 4){       distDir='strongUp';   dot='🟢'; label='강한상승'; color='var(--buy)'; }
  else if(diff >= 2){  distDir='up';         dot='🔵'; label='상승조짐'; color='var(--accent)'; }
  else if(diff >= -1){ distDir='flat';       dot='⚪'; label='방향혼조'; color='#94a3b8'; }
  else if(diff >= -3){ distDir='down';       dot='🟠'; label='하락조짐'; color='#f59e0b'; }
  else {               distDir='strongDown'; dot='🔴'; label='강한하락'; color='var(--sell)'; }
  return { greens, reds, diff, distDir, dot, label, color };
}
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
    ...
    <div class="anal-header-card">
      ...
      <div style="margin-top:10px;padding:10px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid ${
        ...
        ${(()=>{
          ...
          return `<div style="margin-bottom:8px;text-align:left"><span style="display:inline-flex;align-items:center;padding:2px 8px;background:var(--surface);border:1px solid ${_c};border-radius:10px;font-size:9.5px;font-weight:700;color:${_c};white-space:nowrap">${_t}</span></div>`;
        })()}
  ...
  // S31: 미니 캔들차트 그리기
  if(indicators?._advanced?.rows){
    (window._sxTrackedTimeout || setTimeout)(()=>_drawMiniCandleChart(indicators._advanced.rows, stock._btResult?.trades, stock._svChartMarker || stock._svVerdict), 50);
  }
  // S57: 재무 바차트 그리기
