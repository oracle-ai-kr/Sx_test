...
//   표시 전용: _buildBoardGroups엔 안 넣음(각 TF dist5 계산과 순환 회피) · 종합점수/분포 미반영
...
function _sxbHTML(){
  ...
  // [S388] MTF (멀티TF 추세 정합) — total/dist 계산 끝난 뒤 모멘텀 그룹에 표시 전용 추가
  //   byValue: 높을수록 상승추세(≥65 녹 / ≤35 빨) · _noCount: 종합/개수 미반영
  {
    const _mtf = _computeMtfScore();
    if(_mtf){
      const _momG = groups.find(g=>g.id==='mom');
      if(_momG && !_momG.items.some(it=>it.k==='MTF')) _momG.items.push({k:'MTF', v:_mtf.v, byValue:true, _noCount:true});
    }
  }
