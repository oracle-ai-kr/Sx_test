  ...
  if(!indicators){
    try{
      ...
      if(candles && candles.length >= 20){
        ...
        // ════════════════════════════════════════════════════════════
        // [S314] 종가매매 잠금 — KR + US 일봉 통합 (시장 무관 동작)
        // ════════════════════════════════════════════════════════════
        //   기존 [S312]는 KIS-PRICE-FIX 영역 안에 있어 KR 전용이었음.
        //   [S314] 미국 시장도 동일한 종가매매 일관성 보장을 위해 영역 밖으로 분리:
        //     - 시장별 마감 시각 판정 (KR=KST 15시, US=ET 15시, coin=미적용)
        //     - 시장별 today 날짜 판정 (KR=KST, US=ET — 서머타임 자동 처리)
        //     - 잠금 시 candles[-1] + stock.price 모두 동기화 (헤더-BT 일관성)
        //   localStorage 키 `SX_LOCKED_LASTBAR_v1`는 종목 코드로 구분되므로 시장 충돌 없음.
        if(_analTF === 'day' && candles.length > 0 && currentMarket !== 'coin'){
          try {
            // ── 헬퍼: 시장별 잠금 활성 시각 판정 ────────────────────
            //   KR: KST 15:00 ~ 다음날 09:00 (마감 30분 전 ~ 다음 거래일 개장 전)
            //   US: ET 15:00 ~ 다음날 09:00 (정규장 마감 1시간 전 ~ 다음 거래일 개장 전)
            //   coin: 항상 false (24시간 시장이라 종가 개념 없음)
            const _sxIsLockHour = (mkt) => {
              try {
                if(mkt === 'coin') return false;
                let _h;
                if(mkt === 'us'){
                  // Intl.DateTimeFormat이 서머타임(EDT/EST 전환) 자동 처리
                  const _fmt = new Intl.DateTimeFormat('en-US', {
                    timeZone: 'America/New_York', hour: 'numeric', hour12: false
                  });
                  _h = parseInt(_fmt.format(new Date()), 10);
                  if(isNaN(_h)) return false;
                } else {
                  // kr 등: KST 기준
            ...
            // ── 헬퍼: 시장별 today 날짜 (ISO YYYY-MM-DD) ──────────────
            //   거래일 기준 날짜 — KR=KST, US=ET (자정 넘어가도 같은 거래일로 묶임)
            const _sxMarketToday = (mkt) => {
              try {
                if(mkt === 'us'){
                  // en-CA 로케일이 'YYYY-MM-DD' ISO 형식으로 반환
            ...
            if(_sxIsLockHour(currentMarket)){
              const _todayMkt = _sxMarketToday(currentMarket);
              const _lockKey = 'SX_LOCKED_LASTBAR_v1';
              let _locks = {};
              try { _locks = JSON.parse(localStorage.getItem(_lockKey) || '{}'); } catch(_){}
              const _ex = _locks[stock.code];
              const _last = candles[candles.length - 1];

              if(_ex && _ex.date === _todayMkt && _last){
                // 잠긴 봉 복원 — close/open 그대로, high/low는 외부값과 안전 병합
                _last.close = _ex.close;
                _last.open  = _ex.open;
                _last.high  = Math.max(_ex.high || 0, _last.high || 0);
                _last.low   = (_last.low > 0 && _ex.low > 0)
                              ? Math.min(_ex.low, _last.low) : (_ex.low || _last.low);
                // [S314] stock.price도 동기화 — KR은 KIS-PRICE-FIX가 처리해주지만 US는 별도 처리 필요
                //   (헤더에 표시되는 가격과 BT 계산용 candles[-1].close가 동일해야 일관)
                stock.price = _ex.close;
                // [S313] 헤더 분기 판정용 메타
                stock._lockedClose = _ex.close;
                stock._lockedAt    = _ex.lockedAt;
                stock._lockedDate  = _ex.date;
                console.log(`[S314] 종가매매 잠금 복원 (${currentMarket}): ${stock.code} close=${_ex.close.toLocaleString()} (잠긴 시각: ${new Date(_ex.lockedAt).toLocaleTimeString('ko-KR')})`);
              } else if(_last && _last.close > 0){
                // 새로 잠금
                const _newLockedAt = Date.now();
                _locks[stock.code] = {
                  date: _todayMkt,
                  close: _last.close,
                  high:  _last.high,
                  low:   _last.low,
                  open:  _last.open,
                  lockedAt: _newLockedAt,
                  mkt: currentMarket  // 디버깅용
                };
                // 다른 날짜 캐시 자동 정리
                Object.keys(_locks).forEach(k => {
                  if(_locks[k] && _locks[k].date !== _todayMkt) delete _locks[k];
                });
                try { localStorage.setItem(_lockKey, JSON.stringify(_locks)); } catch(_){}
                // [S314] stock.price도 동기화
                stock.price = _last.close;
                stock._lockedClose = _last.close;
                stock._lockedAt    = _newLockedAt;
                stock._lockedDate  = _todayMkt;
                console.log(`[S314] 종가매매 잠금 시작 (${currentMarket}): ${stock.code} close=${_last.close.toLocaleString()} (마감 임박 첫 분석)`);
              }
            }
          } catch(_e314){
            console.warn('[S314] 종가매매 잠금 처리 실패 (무시하고 계속):', _e314);
          }
        }
        // ════════════════════════════════════════════════════════════
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    ...
    btStateBannerHTML = `<div style="padding:12px 14px;margin:0 0 8px;border-radius:12px;background:${_svVerdict.color}0D;border:1.5px solid ${_svVerdict.color}">
      ...
      <div style="font-size:9px;color:var(--text3);margin-top:3px">4축 합격 ${_svVerdict.passCount}/4 · 바닥 ${_scores4.readyScore} · 반등 ${_scores4.entryScore} · 추세 ${_scores4.trendScore} · 모멘텀 ${(_svMom && _svMom.direction === 'up') ? '↑' : (_svMom && _svMom.direction === 'down') ? '↓' : '→'}</div>
      ${_buildAEntryLine(stock, _svVerdict, indicators)}
    </div>`;
  }
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // [S313] 종가매매 잠금 상태면 현재가 대신 "🔒 종가매매 잠금" 박스로 교체
        //   판정: stock._lockedClose 존재 (= [S312] 잠금 메타가 박혀있음)
        //   효과: 사용자가 "지금 잠금된 상태"를 시각적으로 인지 + ↻ 갱신 버튼으로 강제 갱신 가능
        if(stock._lockedClose && stock._lockedClose > 0){
          const _lockedAtStr = stock._lockedAt
            ? new Date(stock._lockedAt).toLocaleString('ko-KR', { dateStyle:'short', timeStyle:'medium' })
            : (stock._lockedDate || '');
          return `<div class="anal-stock-price" style="margin-top:4px;padding:6px 10px;background:rgba(245,158,11,0.10);border:1px solid rgba(245,158,11,0.35);border-radius:8px;display:flex;align-items:center;gap:6px;flex-wrap:wrap">
            <span style="font-size:11px;font-weight:700;color:#d97706">🔒 종가매매 잠금</span>
            <span style="font-size:15px;font-weight:800;color:var(--text);letter-spacing:-.3px">${stock._lockedClose.toLocaleString()}원</span>
            <span style="font-size:9px;color:var(--text3);flex:1;min-width:0">${_lockedAtStr}</span>
            <button type="button" onclick="window._sxRefreshLockedBar('${stock.code}')" style="background:var(--surface2);border:1px solid var(--border);border-radius:6px;padding:3px 8px;font-size:10px;font-weight:700;color:var(--text2);cursor:pointer;flex-shrink:0" title="현재가 봉을 새로 불러와 잠금 갱신">↻ 갱신</button>
          </div>`;
        }
...
// [S313] 종가매매 잠금 갱신 핸들러
// ════════════════════════════════════════════════════════════
//   호출: 분석탭 헤더의 ↻ 갱신 버튼 onclick
//   동작: 1) localStorage SX_LOCKED_LASTBAR_v1 해당 종목 entry 제거
//         2) stock 내부 캔들/인디케이터 캐시 제거 → fetchCandles 강제 재호출
//         3) runAnalysis 재실행 → [S312] 코드가 새 close로 새 잠금 박음
//   주의: currentAnalStock(sx_screener.html 글로벌)가 현재 분석탭 종목.
//         이 종목과 일치할 때만 동작 (다른 종목 코드면 무시).
window._sxRefreshLockedBar = async function(code){
  try { if(typeof _sxVib === 'function') _sxVib(15); } catch(_){}
  if(typeof currentAnalStock === 'undefined' || !currentAnalStock || currentAnalStock.code !== code){
    console.warn(`[S313] 갱신 무시: 현재 분석탭 종목과 불일치 (요청=${code})`);
    return;
  }
  // 1) localStorage 잠금 캐시에서 해당 종목 제거
  try {
    const _key = 'SX_LOCKED_LASTBAR_v1';
    const _locks = JSON.parse(localStorage.getItem(_key) || '{}');
    if(_locks[code]){
      delete _locks[code];
      localStorage.setItem(_key, JSON.stringify(_locks));
      console.log(`[S313] 잠금 캐시 제거: ${code}`);
    }
  } catch(e){
    console.warn('[S313] 잠금 캐시 제거 실패 (무시):', e);
  }
  // 2) stock 내부 캐시 제거 → fetchCandles 재호출 강제 (runAnalysis 753줄 _lastAnalCandles 재사용 분기 우회)
  delete currentAnalStock._lastAnalCandles;
  delete currentAnalStock._indicators;
  delete currentAnalStock._lockedClose;
  delete currentAnalStock._lockedAt;
  delete currentAnalStock._lockedDate;
  // 3) 분석 재실행 — [S312] 코드가 새 close 값으로 새 잠금 박음
  if(typeof runAnalysis === 'function'){
    try { await runAnalysis(currentAnalStock); } catch(e){ console.error('[S313] 재분석 실패:', e); }
  }
  if(typeof toast === 'function') toast('🔒 종가매매 잠금 갱신됨');
};
