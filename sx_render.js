...
function calcBtScore(btData){
  ...
  return Math.round(Math.max(0, Math.min(core, tradeCap, overfitCap, 100)));
}
...
function _buildSimPositionLine(stock, btSt, svVerdict){
  ...
  if(btSt.state === 'holding'){
    ...
    if(svVerdict?.partialHint){
      ...
      // [S361] 안전필터 과열로 인한 익절 힌트면 사유(partialReason)를 클릭 토스트로 노출. ⓘ로 클릭 가능 표시.
      const _pr = (svVerdict.partialReason || '').replace(/['\\]/g, '');
      const _clk = _pr ? ` onclick="toast('과열 사유 · ${_pr}')"` : '';
      const _cur = _pr ? ';cursor:pointer' : '';
      partialBadge = `<span${_clk} title="${_pr}" style="display:inline-block;margin-left:6px;padding:2px 7px;font-size:10px;font-weight:700;color:${badgeColor};background:${badgeBg};border:1px solid ${badgeColor};border-radius:10px;letter-spacing:-.2px;vertical-align:middle${_cur}">${svVerdict.partialHint}${_pr?' ⓘ':''}</span>`;
    }
    ...
    let reasonBadge = '', reasonLine = '', _reasonTxtOut = '';
    {
      ...
      if(_actHold){
        ...
        _reasonTxtOut = _reasonTxt;
        // [S366] 배지 클릭 토스트 — 부분익절 ⓘ와 동일 UX. reasonLine(└)은 유지(A안: 비교용).
        const _rtClean = _reasonTxt.replace(/['\\]/g, '');
        const _rClk = _rtClean ? ` onclick="toast('${_actLabel} · ${_rtClean}')"` : '';
        const _rCur = _rtClean ? ';cursor:pointer' : '';
        reasonBadge = `<span${_rClk} style="display:inline-block;margin-left:6px;padding:2px 8px;font-size:10px;font-weight:800;color:${_actClr};background:${_actClr}1A;border:1px solid ${_actClr};border-radius:10px;letter-spacing:-.2px;vertical-align:middle${_rCur}">${_actLabel}${_rtClean?' ⓘ':''}</span>`;
        // [S361] 모멘텀 배지 삭제 — 전광판(▲N▼N + 부문 삼각형)에 흡수되어 중복.
        if(_reasonTxt) reasonLine = `<div style="${SUB_STYLE};color:${_actClr}">└ ${_reasonTxt}</div>`;
      }
    }

    // [S364] 배지 통합 — reasonBadge(보유유지)+partialBadge(부분익절)를 단일 statusBadge로.
    ...
    if(_curActB === '청산 준비' || _curActB === '청산 검토' || _curActB === '즉시 청산'){
      ...
      statusBadge = partialBadge;           // 부분 익절/손절/추가 매수 고려 (ⓘ 사유 토스트 포함)
    } else {
      const _hcClr = svVerdict?.color || 'var(--buy)';  // 보유 유지 색 재활용
      // [S366] 보유중 배지도 클릭 토스트 (사유 = verdict 사유). reasonLine(└)과 병행(A안: 비교용).
      const _bClean = (_reasonTxtOut || '').replace(/['\\]/g, '');
      const _bClk = _bClean ? ` onclick="toast('보유중 · ${_bClean}')"` : '';
      const _bCur = _bClean ? ';cursor:pointer' : '';
      statusBadge = `<span${_bClk} style="display:inline-block;margin-left:6px;padding:2px 8px;font-size:10px;font-weight:800;color:${_hcClr};background:${_hcClr}1A;border:1px solid ${_hcClr};border-radius:10px;letter-spacing:-.2px;vertical-align:middle${_bCur}">보유중${_bClean?' ⓘ':''}</span>`;
    }
...
async function _runEngineVerify(stock){
  ...
  try {
    ...
    // [S363] 단일소스화 가드 — sx_bt.js _mergeBtCandles와 동일 규칙 (부분봉수 퇴보 방지). 분석탭/단일검증 공유
...
function _sxbHTML(){
  ...
  if (window._sxBoard) window._sxBoard._total = total; // [S403] verdict 카드 1단계 제동용 종합점수 노출
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    + _sxbCircle(total, '', true, false, headColorDelta, false, false, null, false, bc)   // [S417] colorOverride=bc(dist5 색): 도넛=배지=TF점
    + `<div class="sxb-head-info">`
    + `<div class="sxb-head-tone" style="color:${tone.c}">${tone.t}</div>`
    + `<div class="sxb-head-badges">${transBadge}${whipBadge}${_dumpBadge}</div>`
    + `<div class="sxb-head-sub">상태 ${total} · ${_sxbGrade(total)}등급 · ${_cnt}개 지표 · <span style="color:#16a34a;font-weight:700">▲${greens}</span> <span style="color:#ef4444;font-weight:700">▼${reds}</span></div>`
...
function _computeBoardInputs(stock, indicators, scores, scoreMom, tf){
  ...
  // [S400] 거래대금 전이 — 최근 자금 유입/이탈 추세 (close×volume, 종목 규모 무관 점수화)
  //   최근 3봉 평균 거래대금 ÷ 이전 10봉 평균 → log2 스케일 0~100 (50=횡보 / >50 유입 / <50 이탈).
  //   투매 주의: 대금 급증(≥65) AND 가격 하락 AND OBV 이탈 → 헤더 배지(단순 조정 하락은 거름).
  try {
    const _cdls = stock._lastAnalCandles || [];
    if(_cdls.length >= 13){
      ...
      if(_avgP > 0){
        ...
        _boardExtras.tradeValTrend = { v: _tvScore };
        const _obvDown = indicators && indicators.obv && indicators.obv.trend === 'down';
        const _priceDown = (stock.changeRate || 0) < 0;
        if(_tvScore >= 65 && _priceDown && _obvDown){
          _boardExtras.dumpWarn = { on:true, reasons:[{t:'dump', l:`투매: 대금급증(${_tvScore})+가격하락+OBV이탈`}], tv:_tvScore };
        }
      }
    }
  } catch(_eTV){ /* 대금전이 실패 시 항목 없이 진행 (안전) */ }

  // [S419] 천정 되돌림 강한 위험 신호 → 투매주의 배지에 흡수(되돌림주의로 통합). indicators 직접(캔들 무관)이라 별도 try.
  //   배지 1개에 사유를 모으고 토스트에서 구분 — 화면 공간 절약 + 정말 위험한 신호만 경고.
  try {
    const _rr = (_boardExtras.dumpWarn && _boardExtras.dumpWarn.reasons) ? _boardExtras.dumpWarn.reasons : [];
    if(indicators && indicators.rsi && indicators.rsi.div === 'bearish') _rr.push({t:'rsiDiv', l:'RSI 약세 다이버전스: 가격↑ RSI↓ (상승 동력 소진)'});
    if(indicators && indicators.obv && indicators.obv.div === 'bearish') _rr.push({t:'obvDiv', l:'OBV 약세 다이버전스: 가격↑ 수급↓ (자금 이탈)'});
    const _vv = _boardExtras.vola && _boardExtras.vola.v, _bv = _boardExtras.bb && _boardExtras.bb.v;
    if(_vv!=null && _vv>=70 && _bv!=null && _bv>=100) _rr.push({t:'overheat', l:'과열 급등: 고변동성(ATR↑)+볼린저 상단 이탈 (추격 위험)'});
    if(_rr.length) _boardExtras.dumpWarn = { on:true, reasons:_rr, tv:(_boardExtras.dumpWarn && _boardExtras.dumpWarn.tv) || null };
  } catch(_eRisk){}
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  extras = extras || {};
  window._sxBoard = { verdict: verdict||null, lowConf: !!lowConf, lowConfReasons: (extras && extras._lowConfReasons) || [], dumpWarn: (extras && extras.dumpWarn) || null, groups: _buildBoardGroups(scores, sv4, structPos, pbScore, D, extras) };
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    ...
    const _scores4 = {
      ...
      safetyViol: qs && Array.isArray(qs._safetyViol) ? qs._safetyViol : [] // [S361] 안전필터 위반 → C 캡/익절힌트
    };
  ...
  (()=>{
    ...
    _pgShared = SXI.practicalGuide(_verdictAct, _as, _bs, _sum, indicators, _pg4, _pgMom);
  })();
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        ${(()=>{
          ...
          //   반등 경로(rebound): 반등신호+반등강도 활성 / 추세 경로(trend): 추세방향+추가상승 활성
          ...
          var _zoneStyle = function(zone){
            ...
            var active = (_passPath==='trend'   && (zone==='trend'||zone==='upside')) ||
                         (_passPath==='rebound' && (zone==='ready'||zone==='entry'));
        ...
        ${(()=>{
          ...
          if(_btSc != null){
            ...
            _reliabilityHTML = `<div style="margin-top:8px;padding:6px 8px;background:var(--surface);border-radius:6px">
              ...
              <div style="font-size:9px;color:var(--text3);margin-top:4px;letter-spacing:-.2px">${_metaParts.join(' · ')}</div>
            </div>`;
          }
