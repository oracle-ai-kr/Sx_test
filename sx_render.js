// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S839 · build 20260701-0250 cache-bust]
// ════════════════════════════════════════════════════════════
...
function _reboundCycleCalc(adv, stock, mkt){
  ...
  // ── X축: 추세 구조 (하락− ↔ 상승+) ──
  var x = 0, xParts = [];
  var lt = adv.maAlignLT;
  if(lt && lt.gateOn){ if(lt.bullish){ x += 60; xParts.push('장기 정배열 +60'); } else if(lt.bearish){ x -= 60; xParts.push('장기 역배열 −60'); } }
  var ma = adv.maAlign;
  if(ma){ if(ma.bullish){ x += 25; xParts.push('단기 정배열 +25'); } else if(ma.bearish){ x -= 25; xParts.push('단기 역배열 −25'); } }
  var pct = (adv.trend && typeof adv.trend.pct === 'number') ? adv.trend.pct : 0;
  var pctC = Math.max(-15, Math.min(15, pct));
  if(Math.abs(pctC) >= 1){ x += pctC; xParts.push('20일추세 ' + (pct>0?'+':'') + pct.toFixed(1) + '%'); }
  x = Math.max(-100, Math.min(100, Math.round(x)));
  // ── Y축: 반등 품질 (가짜·데드캣− ↔ 진짜반등+) — [S760] 측정 기반 전면 재설계 ──
  // 데드캣풀 N3/N5/N10 측정: 진짜반등=N10까지 살아남는 재료(거래량OSC 78%·ADX낮음 70%), 데드캣=N5 반짝 후 N10 추락(골든크로스 35%). 다이버전스·지지근접·과매도RSI는 무효/역효과라 제거. 골든크로스는 +가산→−감점으로 부호 반전.
  var y = 0, yPos = [], yNeg = [];
  // [S772] 측정 재검증 반영 — 진짜반등 핵심=과매도 바닥(ADX<14 + MA200이격도<-16 + RSI<45 = N10 92%/n72). 거래량OSC는 BT착시(순수데이터 +3~5%p)라 가산 약화. MA이격도(과대낙폭)=결정적 재료라 신규 추가·최대 비중. 골든크로스 단독=데드캣(N10 40%)이나 위 과대낙폭/과매도 동반 시 자연 상쇄(=진짜 골든크로스).
  ...
  // [과대낙폭 축] MA이격도 — 진짜 바닥의 결정적 재료. 깊을수록 반등 여력.
  var _dev200 = null, _dev60 = null;
  if(cl && typeof _smaLast733 === 'function'){
    var _m200 = _smaLast733(cl, 200), _m60 = _smaLast733(cl, 60), _cN = cl[cl.length-1];
    if(_m200 && _m200 !== 0 && typeof _cN === 'number') _dev200 = 100*(_cN-_m200)/_m200;
    if(_m60 && _m60 !== 0 && typeof _cN === 'number') _dev60 = 100*(_cN-_m60)/_m60;
  }
  if(_dev200 != null && _dev200 < -16){ y += 18; yPos.push('MA200이격도 ' + _dev200.toFixed(0) + '%(장기 과대낙폭)'); }
  else if(_dev60 != null && _dev60 < -10){ y += 12; yPos.push('MA60이격도 ' + _dev60.toFixed(0) + '%(중기 과대낙폭)'); }
  // [추세소진 축] ADX<14 — 하락 동력 소진
  var _adxV = (adv.adx && typeof adv.adx.adx === 'number') ? adv.adx.adx : null;
  if(_adxV != null && _adxV < 14){ y += 15; yPos.push('ADX ' + _adxV.toFixed(0) + '(추세 소진)'); }
  // [과매도 축] RSI<45 — 반등 여력
  if(adv.rsi && typeof adv.rsi.val === 'number' && adv.rsi.val < 45){ y += 12; yPos.push('RSI ' + adv.rsi.val.toFixed(0) + '(과매도)'); }
  // [S808 정합성 수선] 거래량OSC(+8)·CCI(+6) 제거 — S806 전수측정서 거래량OSC<-39.6은 진짜/가짜 못 가름(후반평균·절대 양쪽 역효과 -28%p). CCI도 동계열(과매도 보조·변별력 측정 lift만·전수 미검증)이라 함께 제거. 남은 양(+)=S772 검증조합(MA이격도+ADX+RSI, n72). ※양(+) 재료 전체 전수검증=다음 단계(배지 발동 vs Y 비교측정).
  // [데드캣 감점] 골든크로스 단독 = 측정상 데드캣 시그니처(N10 40%). 위 과대낙폭/과매도 가산 동반 시 상쇄(진짜 골든크로스).
  if(cl && typeof _goldenX733 === 'function'){
    if(_goldenX733(cl, 5, 20, 3)){ y -= 15; yNeg.push('골든크로스 5×20(데드캣 반짝)'); }
    if(_goldenX733(cl, 5, 9, 3)){ y -= 10; yNeg.push('골든크로스 5×9(단기 반짝)'); }
  }
  // [약세·되돌림 감점] 기존 유지 — 투매·수급이탈·약세다이버·고점권
  var dump = null;
  try { if(typeof SXE !== 'undefined' && SXE.calcDumpWarn) dump = SXE.calcDumpWarn(adv.rows, adv, (stock && typeof stock.changeRate === 'number') ? stock.changeRate : 0, mkt); } catch(_eDw){}
  if(dump && dump.on){ y -= 30; yNeg.push('되돌림주의(투매·약세)'); }
  if(adv.obv && adv.obv.trend === 'down'){ y -= 10; yNeg.push('OBV 하락(수급 이탈)'); }
  if(adv.rsi && adv.rsi.div === 'bearish'){ y -= 15; yNeg.push('RSI 약세다이버'); }
  if(adv.obv && adv.obv.div === 'bearish'){ y -= 12; yNeg.push('OBV 약세다이버'); }
  var nearRes = !!(adv.trend && adv.trend.struct && adv.trend.struct.nearResistance);
  if(nearRes){ y -= 10; yNeg.push('저항선 근접(고점권)'); }
  ...
  return { x:x, y:y, ang:ang, conf:conf, phase:phase, xParts:xParts, yPos:yPos, yNeg:yNeg };
}
function _rcDonutSvg(R, pts){
  ...
  var cur = _RC_PHASE[R.phase], weak = R.conf < 20;
...
function _buildReboundCycleCard(stock, indicators){
  ...
  var xRow = '<div style="font-size:10px;color:var(--text2);margin-bottom:3px"><b style="color:'+_rcNumCol(R.x)+'">추세 '+(R.x>0?'+':'')+R.x+'</b> <span style="color:var(--text3)">(하락◀▶상승)</span>'+(R.xParts.length?' · '+R.xParts.join(' · '):' · 중립')+'</div>';
  var yAll = R.yPos.concat(R.yNeg);
  var yRow = '<div style="font-size:10px;color:var(--text2);margin-bottom:3px"><b style="color:'+_rcNumCol(R.y)+'">품질 '+(R.y>0?'+':'')+R.y+'</b> <span style="color:var(--text3)">(가짜·실패◀▶진짜·건강)</span>'+(yAll.length?' · '+yAll.join(' · '):' · 중립')+'</div>';
  var cycle = '<div style="font-size:9px;color:var(--text3);text-align:center;margin:8px 0 0">↻ 바닥반등 → 눌림목 → 되돌림 → 데드캣 → 바닥반등</div>';
  var note = '<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px;line-height:1.55">추세구조(X)×반등품질(Y)을 도넛에 매핑한 <b>현위치</b> 시각화. 마커가 들어간 사분면 호 = 현재 단계. 숫자 색: <b style="color:#16a34a">+초록</b>·<b style="color:#dc2626">−빨강</b>·0근처 회색(=경계). 강도=단계가 얼마나 또렷한지(약하면 회색). <b style="color:#d97706">⚖️</b>=진짜/가짜 경계(품질 0 근처). 점·선 = 과거 여러 시점의 궤적(같은 데이터로 재계산, 저장 없음 → 깜빡임 없음): <b style="color:var(--text2)">순행 ↻</b>(사이클 정상 진행)은 회색, <b style="color:#dc2626">역행 ↺</b>(거꾸로) 구간은 빨강, 화살촉=진행방향. 반등 국면이 약해도 구조가 가리키는 위치를 유지합니다. C 판정·점수와 독립된 <b>실험 표시 카드</b>(매매 신호 아님).</div>';
...
function _sxBadgeRowInner(def, st){
  ...
  var inPool=!!st.inPool, count=st.count||0, oldest=(st.oldest==null?-1:st.oldest);
  var stage = (count<=0) ? null : (oldest<=0 ? 'now' : (oldest<=5 ? 'early' : 'late'));   // [S796] 0봉=발동 · 1-5봉=조짐 · 6-9봉=확정임박(N10 윈도우 형성중, 10봉째 확정)
  var chip;
  if(stage==='now')        chip='<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:'+def.color+';color:#fff;border:1px solid '+def.color+'">'+def.icon+' '+def.name+'</span>';
  else if(stage==='late')  chip='<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:'+def.color+';color:#fff;border:1px solid '+def.color+'">'+def.icon+' '+def.late+'</span>';
  else if(stage==='early') chip='<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:'+def.color+'22;color:'+def.color+';border:1px solid '+def.color+'">'+def.icon+' '+def.early+'</span>';
  else                     chip='<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">○ '+def.name+'</span>';
  var statusTxt, statusCol;
  if(count>0){ statusCol=def.color; statusTxt=(oldest<=0)?('✅ 방금 발동 '+count+'개'):('⏳ 확정임박 '+oldest+'봉전 · 관찰중 '+count+'개'); }
  ...
  var detail;
  if(count>0){
    detail='<div style="font-size:8.5px;color:'+def.color+';margin-top:3px">'+((oldest<=0)?('방금 '+count+'개 발동 → 결과 관찰 시작(~10봉 윈도우)'):(oldest+'봉전 발동분 결과 임박 · 관찰중 '+count+'개 (최근발동 0-9봉)'))+'</div>';
  } else {
    detail='<div style="font-size:8.5px;color:'+GR+';margin-top:3px">관찰중 레시피 없음 ('+total+'개 중 0)</div>';
  }
...
function _buildBadgeInventoryCard(stock, indicators){
  ...
  var desc = '<div style="font-size:8.5px;color:var(--text3);line-height:1.5;margin-bottom:4px">신호 배지 <b style="color:var(--text2)">시제품 보관소(SSOT)</b> — 다른 배지가 가져다 씁니다. <b>4종</b>: 역배열 바닥반등·데드캣 / 정배열 눌림목·되돌림. 카운트 = <b>레시피카드 \'관찰중\'(최근발동 0-9봉=N10 윈도우 미확정) 레시피 수</b>를 그대로 당겨옵니다(동일 소스라 카운터 일치). <b>확정임박 N봉</b> = 관찰중 중 가장 오래된 발동(10봉째 결과 확정). <b style="color:var(--text2)">회색=풀 불일치 또는 관찰중 0</b>.</div>';
...
async function _sxBadgeAsyncRender(token, mk, code){
  ...
  SX_BADGE_ORDER.forEach(function(key){
    ...
    var p=(pend && pend[def.pool+'|'+def.kind]) ? pend[def.pool+'|'+def.kind] : {count:0, oldest:-1};
    if(p.count>0) active++;
    el.innerHTML=_sxBadgeRowInner(def, { inPool:(ltNow===def.poolLt && !maBullNow), count:p.count, oldest:p.oldest });
  });
...
async function _btDiscrimBracket(mk, source, onProgress){
  ...
  else if(source==='all'){   // [S833] 확장 풀 — 측정 도구(base rate/겹침/손절)와 동일: 대표+관심+보강+top100, seen 중복제거. beam-search 풀 확장용.
...
// ════════ [S809] 레시피 겹침 측정 — real 레시피 동시발동 수↑면 N10 진짜비율↑? 게이트/변별력과 동일 풀(대표+관심 전수·CAP20)·동일 렌즈(N10 후반평균 [+6..+10] 방향). 봉별 겹침수=SXRecipeSignal.overlapScan(_scanStock 캐시 공유)에 위임. 단순 겹침수부터(deadcat-real 12/13이 거래량OSC>22.87 공통=비슷한 레시피라 '독립확인'보다 '과대낙폭 깊이'일 수 있음). 표본수 항상 병기(S806 교훈). 측정전용·엔진/C 무관. ════════
...
function _recipeOverlapRender(rep, watch, mk){
  ...
  var _poolTable=function(agg, poolKey, poolLabel, poolCol, kind){
    // kind='real'(적중=상승) | 'fake'(적중=하락=데드캣/되돌림 경고가 맞음). 겹침↑→적중비율↑ 단조면 그 신호=강.
    ...
    for(var i=0;i<ks.length;i++){
      ...
      prevRate=rate;
      rows+='<tr>'
        +'<td style="padding:5px 8px;font-weight:800;color:'+poolCol+';font-size:11px">겹침 '+kLabel+'</td>'
    ...
    var verdict = (totN<20) ? '<span style="color:'+T3+'">표본 부족 — 판단 보류</span>'
      : mono ? '<span style="color:'+GRN+'">겹침↑ → '+(fake?'하락적중↑ (단조) · 데드캣 경고 강':'진짜비율↑ (단조) · 진짜반등 강')+'</span>'
...
// ════════ [S831] 🔵 정배 base rate 분해 — pullback-real 겹침 진짜비율(57→82%)이 진짜 신호 힘인가 불장빨인가. 시장 base + 정배 레짐 base 대비 순수 lift. 통합 전 마지막 안전장치(거래량OSC n22 교훈). ════════
async function _pullbackBaseBracket(mk, onProgress, pool){
  ...
  var ov={1:{t:0,h:0},2:{t:0,h:0},3:{t:0,h:0},4:{t:0,h:0}}, stocksUsed=0;   // 선택 레짐 realK별 겹침 진짜비율
  for(var i=0;i<use.length;i++){
    var s=use[i];
    if(onProgress) onProgress(i+1, use.length, s.name);
    await _trendBatchSleep(0);
...
function _pullbackBaseRender(res, mk){
  ...
  // 겹침 3행 — lift는 정배 레짐 base 대비(겹침 순수 기여)
  ...
  var rows=baseRow('시장 전체 base', baseAll, res.allN, false)+baseRow(regNm+'열 레짐 base', baseBull, regN, true)
    +ovRow('겹침 1~2개', O12)+ovRow('겹침 3개', O3)+ovRow('겹침 4+개', O4);
  // 판정 — 4+ 겹침의 레짐 대비 lift
  var v='';
  if(O4.rate!=null && baseBull!=null){
    var l4=O4.rate-baseBull, lAll=(baseAll!=null)?(O4.rate-baseAll):null;
    if(l4>=10) v='<span style="color:'+GRN+'">✅ <b>겹침 4+가 '+regNm+' 레짐 base보다 +'+l4.toFixed(0)+'%p</b> — 겹침이 진짜 신호 힘(불장빨 너머). 하드코딩 안심'+(lAll!=null?' (시장 대비 +'+lAll.toFixed(0)+'%p)':'')+'</span>';
    else if(l4<=4) v='<span style="color:'+RED+'">⚠️ <b>겹침 4+가 '+regNm+' 레짐 base보다 +'+l4.toFixed(0)+'%p뿐</b> — 대부분 레짐+불장빨. '+(isPb?'하드코딩 수치는 <b>약세장에서 무너질 위험</b>':'<b>역배 겹침 무의미 재확인(확인사살)</b>')+'</span>';
    else v='<span style="color:'+AMB+'">겹침 4+의 '+regNm+' 레짐 대비 lift +'+l4.toFixed(0)+'%p — 중간. 겹침이 약간 기여하나 상당부분 레짐/불장</span>';
  }
  // 단조성 체크
  var mono='';
  if(O12.rate!=null&&O3.rate!=null&&O4.rate!=null){
    var up=(O3.rate>=O12.rate-1)&&(O4.rate>=O3.rate-1);
    mono='<div style="font-size:9px;margin-top:5px;color:'+(up?GRN:AMB)+'">'+(up?'✓ 겹침 늘수록 진짜비율 단조 상승 유지'+(isPb?'(정배의 핵심 패턴 재확인)':'(역배에서도?!)'):'⚠ 단조 상승 깨짐 — 겹침 수와 진짜비율 비례 약함'+(isPb?'':' (역배 겹침 무의미 재확인)'))+'</div>';
  }
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px"><b>'+regNm+' 겹침이 진짜 신호인가 불장빨인가</b>. <b>시장 base</b>=전체 봉 N10 상승% · <b style="color:var(--text2)">'+regNm+' 레짐 base</b>=발동 무관 기준선. <b>겹침</b>='+sigNm+' 동시발동 수별 진짜비율. <b style="color:'+AC+'">lift</b>=겹침 − '+regNm+' 레짐 base. lift 크면 진짜 신호, 작으면 레짐+불장빨. 합본 311개 기준·대표+관심+top100·'+(res.stocksUsed||0)+'종목.</div>';
  return head
    +'<table style="width:100%;border-collapse:collapse;background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">'
    +'<tr style="background:var(--surface2)"><td style="padding:5px 8px;font-size:8px;color:'+T3+'">기준 / 겹침</td><td style="padding:5px 6px;text-align:right;font-size:8px;color:'+T3+'">n</td><td style="padding:5px 8px;text-align:right;font-size:8px;color:'+T2+'">진짜비율</td><td style="padding:5px 8px;text-align:right;font-size:8px;color:'+BLU+'">lift</td></tr>'
    +rows+'</table>'
    +(v?'<div style="font-size:9.5px;margin-top:7px;line-height:1.45">'+v+'</div>':'')
    +mono;
}
// ════════ [S837] 🔴 역배 데드캣-real 겹침 × 다호라이즌(N10/N15/N20) — 최종 검증. "역배는 느리게 반응"(침체 종목 관심밖) 가설 → N10 단조 깨짐(62→61→71)이 긴 horizon서 살아나나? 각 horizon lift로 불장빨 배제. 엔진 무변경·event study. ════════
...
async function _deadcatHzRun(){
  ...
  var mk=(typeof currentMarket!=='undefined')?currentMarket:'kr';
  el.style.display='block';
  el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">🔴 역배 겹침 다호라이즌 측정 중...</div>';
  var res;
...
function _deadcatHzRender(res, mk){
  ...
  // 헤더 행 (N10/N15/N20)
  var thead='<tr style="background:var(--surface2)"><td style="padding:5px 7px;font-size:8px;color:'+T3+'">겹침 / hz</td>';
  ...
  rows+='</tr>';
  // 겹침 행 3개
  var grp=[['겹침 1~2', function(hi){return ov12(hi);}, function(hi){return ov12N(hi);}], ['겹침 3', function(hi){return ovRate(3,hi);}, function(hi){return res.ovHz[3][hi].t;}], ['겹침 4+', function(hi){return ovRate(4,hi);}, function(hi){return res.ovHz[4][hi].t;}]];
  ...
  for(var hv=0;hv<3;hv++){
    ...
    var ok=mono&&l4>=10;
    verdicts+='<div style="font-size:9px;margin-top:3px;color:'+(ok?GRN:(mono?AMB:T3))+'"><b>'+HZL[hv]+'</b>: '+(mono?'단조✓':'단조✗')+' · 4+ lift '+(l4>=0?'+':'')+l4.toFixed(0)+'%p '+(ok?'→ <b>역배 겹침 유효!</b>':(mono&&l4>=4?'(약함)':'→ 무의미'))+'</div>';
  }
  ...
  var concl='<div style="font-size:9.5px;margin-top:7px;padding:7px;border-radius:7px;background:'+(anyOk?GRN+'14':RED+'0d')+';color:'+(anyOk?GRN:RED)+';line-height:1.45">'
    +(anyOk?'✅ <b>긴 horizon에서 역배 겹침 단조+lift 살아남</b> — "역배는 느리게 반응"(침체 종목) 가설 데이터 확인. 역배 겹침 배지 가능(해당 horizon)':'⚠️ <b>어느 horizon에서도 단조+lift 동시 불충족</b> — 역배 겹침은 길게 봐도 무의미. N10 결론 유지·역배 겹침 배지 불가. 서사는 매력적이나 데이터가 기각')+'</div>';
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px"><b>역배 데드캣-real 겹침 × N10/N15/N20</b>. 셀=진짜비율(<b style="color:'+T2+'">작은수=lift</b>=그 horizon 역배 base 대비). <b>N10은 이미 단조 깨짐</b>(62→61→71) — 긴 horizon서 <b>단조(겹침↑=진짜↑) + 4+ lift≥10%p</b> 살아나면 "역배는 느림" 가설 확인. 합본 129개(deadcat-real)·대표+관심+top100·'+(res.stocksUsed||0)+'종목.</div>';
  return head
...
// ════════ [S838] 🔴 역배 겹침 3+ BT — N15/N20 유효 통계(68→72%)가 실전 총수익/MDD로 이어지나. "긴 호흡 단서" 검증: 2~3주 보유 수익·낙폭·시간당수익. 겹침3+ 진입→hz봉 보유 청산·복리 자산곡선. ════════
async function _deadcatHzBtBracket(mk, onProgress){
  ...
  var CAP=130, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
  var COMBOS=[[15,3],[15,4],[20,3],[20,4]];   // [S839] N15/N20 × 겹침3+/4+
  ...
  if((R['15_3']?R['15_3'].nT:0)<20) return { ok:false, reason:'겹침3+ 거래 표본 20 미만 (n='+(R['15_3']?R['15_3'].nT:0)+')' };
...
async function _deadcatHzBtRun(){
  ...
  var mk=(typeof currentMarket!=='undefined')?currentMarket:'kr';
  el.style.display='block';
  el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">🔴 역배 겹침 BT 실행 중...</div>';
  var res;
...
function _deadcatHzBtRender(res, mk){
  ...
  function tr(r){
    ...
    return '<tr'+(r.mk===4?' style="border-top:1px dashed var(--border)"':'')+'>'
      +'<td style="padding:6px 7px;font-weight:800;color:'+(small?T3:'var(--text)')+';font-size:10px">겹침'+r.mk+'+ N'+r.hz+warn+'</td>'
  ...
  if(best){
    ...
    var tag='겹침'+best.mk+'+ N'+best.hz;
    if(profitable && beatsPb && !deepMdd) v='<span style="color:'+GRN+'">✅ <b>'+tag+' 실전 수익 '+best.avg.toFixed(1)+'% · 시간당 +'+best.perbar.toFixed(2)+'%/봉(정배 N10 0.29% 이상)</b> — 긴 호흡 값어치. MDD '+best.mdd.toFixed(1)+'% 감당 가능. <b>역배 겹침 실전 유효</b></span>';
    ...
    if(r15_3.nT>=20 && r15_4.nT>=20 && r15_3.avg!=null && r15_4.avg!=null){
      ...
      v+='<div style="font-size:8.5px;color:'+T3+';margin-top:4px">겹침4+ vs 3+ (N15): 수익 '+(d>=0?'+':'')+d.toFixed(1)+'%p'+(Math.abs(d)<1?' — 4+ 더 짜도 개선 미미(겹침3+로 충분·표본 유지)':(d>0?' — 4+가 더 강(단 표본 '+r15_4.nT+'로 줆)':' — 4+가 오히려 낮음(표본 노이즈 의심)'))+'</div>';
    }
  }
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px"><b>역배 겹침 실전 BT</b> — N15/N20 × 겹침3+/4+. 겹침 진입→N봉 보유 청산·1포지션·복리. <b style="color:'+T2+'">평균</b>=거래당 수익 · <b>승률</b> · <b style="color:'+RED+'">MDD</b>=거래별 진입후 평균 최저 · <b style="color:'+BLU+'">시간당</b>=수익÷보유봉(<b>정배 N10 ≈0.29%/봉 기준선</b>). <b style="color:'+AMB+'">겹침4+는 표본 작을 수 있음(⚠)</b>. 대표+관심+top100·'+(res.stocksUsed||0)+'종목.</div>';
  return head
    +'<table style="width:100%;border-collapse:collapse;background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">'
...
// ════════ [S738] 🧬 OR 앙상블 시제품 — 고정 3경로(강 단독 OR 약 조합1 OR 약 조합2). entries 재사용. 각 경로 정밀도/커버리지 + 합집합 + 겹침분석 → OR이 AND와 달리 표본 유지하며 정밀도 올리는지 검증. ════════
var _ENS_PATHS = [
...
function _btEnsembleRender(entries){
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:6px">서로 다른 유형의 '+DP.good+'을 3경로로 포착 → OR 합집합. <b>AND와 달리 표본 유지하며 정밀도 오르나</b> 검증. 경로별 정밀도('+DP.goodShort+'%)·커버리지(전체 중 비율), 합집합, 겹침. <span style="color:'+AMB+'">※경로 정의는 가짜반등 기준 — 눌림목엔 탐색용</span>.</div>';
  // 경로 정의
  ...
  CR.forEach(function(C){
    ...
    out+='<div style="font-size:8px;color:'+T3+';margin-top:4px">겹침: 0경로 '+cnt[0]+' · 1경로 '+cnt[1]+' · 2경로 '+cnt[2]+' · 3경로 '+cnt[3]+' <span style="color:'+T3+'">(1경로多=경로 보완적·좋음 / 2~3경로多=겹침)</span></div>';
  });
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">판단: 합집합(∪) <b>커버리지가 경로A보다 크게 늘면서 정밀도가 기저율보다 충분히 높으면</b>(<b style="color:'+GRN+'">+10%p↑</b>) → OR 앙상블 효과 있음 = <b>게이트 B 교체 후보</b>(여러 유형 반등 통과). 합집합 정밀도가 기저율과 비슷하면 → 약한 경로가 노이즈만 추가(가장 강한 단일 경로 A가 나음). 겹침에서 1경로 多=경로들이 다른 자리 포착(보완적·좋음).</div>';
  out+='</div>';
  return out;
}
...
function _sxBuilderPreset(which){
  ...
  else if(which==='bbonly'){ setEn('bbPctB',true); setDir('bbPctB','gt'); setTh('bbPctB',1.1); }   // [S754] 눌림목 — BB%B 단독(겹침 적은 단일 신호)
