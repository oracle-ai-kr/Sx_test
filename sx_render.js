// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v4.4 <!-- S103-fix6 매도/진입타이밍 라벨 동기화(3단리포트 ready섹션+추이차트) + 가격UI 4줄→3줄 압축(pct+time 같은줄 · 구분자) + 공시배지 2열그리드(홀수마지막 중앙정렬) --> v4.3 <!-- S103-fix5 분석탭 TF칩/모드칩/연관키워드 진동 3종 추가 --> v4.2 <!-- S103 분석탭 동적 삼각형 토글 24개 + 차트전체화면 진동 추가 --> v4.1 <!-- S103 결과탭 관심목록(12)/초기화(15) 진동 추가 --> v4.0 <!-- S103 _stratDetailId 스코프버그 수정 (IIFE 안 선언→함수 본체 호이스팅) --> v3.9 <!-- S100 -->
...
function _renderDisclosureUI(stock, scores, indicators, qs){
  ...
  if(badgeArea && di && di.badges && di.badges.length){
    ...
    let html = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 8px;align-items:start">';
    catEntries.forEach(([cat, badges], idx) => {
      const isLast = idx === catEntries.length - 1;
      const fullWidth = isOdd && isLast;
      const cellStyle = fullWidth
