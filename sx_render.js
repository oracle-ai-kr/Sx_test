// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S818 · build 20260630-1530 cache-bust]
// ════════════════════════════════════════════════════════════
...
function _trendRenderInner(){
  ...
  const note=`<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · ${_effPredict?`🔮 예측 선행 ${_predLeadLbl}(kNN) → `:''}${cfg.s}MA×${cfg.l}MA 골든크로스${cfg.entrySlope?` +📈${cfg.s}MA기울기 조기진입${cfg.entryConfirm?'(확인봉)':''}${cfg.entryRsi?'+📊rsiGc OR':''}`:''} ${cfg.nextOpen?'→ 다음봉 시가 진입':'종가 진입'}${_buyN?` · 매수 ${_buyN}개 AND(최근${cfg.n}봉)`:' · MA 단독'} → 데드크로스${_sellN?` OR 매도 ${_sellN}개`:''} 종가청산${cfg.earlyMa5?` · ⚡종가<${cfg.s}MA 조기청산`:''}${cfg.earlySlope?` · 📉${cfg.s}MA기울기 청산`:''}${cfg.disSl?` · 🛡️${cfg.slAtr>0?cfg.slAtr:3}×ATR 재앙손절`:''}${_effPredict?` · 예측 진입+조기청산 · 실패 시 손절`:''}${cfg.reentry?' · 🔄정배열 재진입':''} · <b style="color:${_trBarsCol}">[${_trBars}봉 기준]</b></div>`;
...
function _trendBatchConfigs(market, mode){
  ...
  const base=function(){ return { s:sN, l:lN, n:3, aux:{}, sell:{}, reentry:false, predict:false, predLead:1, nextOpen:false, earlyMa5:false, earlySlope:false, disSl:false, slAtr:2, entrySlope:false, entryConfirm:true }; };
...
function _trendWaterAccount(bt, rows, fireMap, mode){
  ...
  for(let i=0;i<bt.trades.length;i++){
    ...
    let addShares=0, addCost=0, wtN=0;
    for(let b=t.entryIdx+WT_BLOCK+1; b<t.exitIdx; b++){ if(fireMap[b] && close[b]>0 && (!_belowOnly || close[b]<t.entry)){ addShares+=ADD/close[b]; addCost+=ADD; wtN++; break; } }   // [S815] 포지션당 1회 · [S818] 진입후 10봉 차단 + B모드 현재가<진입가
    injected+=addCost;
    ...
    tr.push({ entry:t.entry, exit:t.exit, bars:t.bars, entryDate:t.entryDate, exitDate:t.exitDate,
      wtN:wtN, addCost:addCost, avg:avg, wtPnl:+wtPnl.toFixed(2), origPnl:t.pnl, balAfter:Math.round(bal), absorbed:absorbedThis, absCost:absCost });
  }
  let openInfo=null;
  if(bt.open){
    const o=bt.open, baseShares=bal/o.entry, last=rows.length-1;
    let addShares=0, addCost=0, wtN=0;
    for(let b=o.entryIdx+WT_BLOCK+1; b<=last; b++){ if(fireMap[b] && close[b]>0 && (!_belowOnly || close[b]<o.entry)){ addShares+=ADD/close[b]; addCost+=ADD; wtN++; break; } }   // [S815] 포지션당 1회 · [S818] 진입후 10봉 차단 + B모드 현재가<진입가
    injected+=addCost;
    const totalShares=baseShares+addShares, invested=bal+addCost;
    const ev=totalShares*o.cur, avg=totalShares>0?invested/totalShares:o.entry;
    const wtPnl=avg>0?(o.cur/avg-1)*100:o.pnl;
    openInfo={ entry:o.entry, cur:o.cur, bars:o.bars, entryDate:o.entryDate, wtN:wtN, addCost:addCost, avg:avg, wtPnl:+wtPnl.toFixed(2), origPnl:o.pnl, evalAmt:Math.round(ev) };
  }
...
function _trendWtListHtml(acc, mode){
  ...
  const wrow=(bdg,bc,avg,entry,exit,pnl,origPnl,bars,d1,d2,wtN,bal,absCost,addCost)=>{
    ...
    let wtTag='';
    if(wtN>0){ let tagCol='#0891b2', tagTxt='물타기 '+wtN+'회'; if(mode==='B'){ if(addCost>0 && absCost>=addCost){ tagCol=RD; tagTxt='물타기 '+wtN+'회·전액흡수'; } else if(absCost>0){ tagCol='#d97706'; tagTxt='물타기 '+wtN+'회·일부흡수'; } else { tagCol='#0891b2'; tagTxt='물타기 '+wtN+'회·환수'; } } wtTag=`<span style="color:${tagCol};font-weight:700"> · ${tagTxt}</span>`; }
    return `<div style="border-bottom:1px solid var(--border);padding:8px 0"><div style="display:flex;align-items:center;gap:6px;font-size:12px"><span style="font-weight:800;color:${bc};min-width:42px">${bdg}</span><span style="color:${T2}">${avgTxt} → ${exit==null?'—':Math.round(exit).toLocaleString()}</span><span style="margin-left:auto;font-weight:800;color:${bc}">${pnl>=0?'+':''}${pnl}%</span><span style="color:${T3};font-size:10px;min-width:30px;text-align:right">${bars}봉</span></div><div style="display:flex;font-size:10px;color:${T3};margin-top:2px"><span>${d1} ~ ${d2}${wtN>0?` · 원${origPnl>=0?'+':''}${origPnl}%`:''}${wtTag}</span><span style="margin-left:auto">${bal.toLocaleString()}원</span></div></div>`;
  };
  let list='';
  if(acc.open){ const o=acc.open; list+=wrow('OPEN',BL,o.avg,o.entry,o.cur,o.wtPnl,o.origPnl,o.bars,o.entryDate||'','보유중',o.wtN,acc.curBal,(o.absCost||0),(o.addCost||0)); }
  for(let i=acc.tr.length-1;i>=0;i--){ const t=acc.tr[i]; const win=t.wtPnl>0; list+=wrow(win?'WIN':(t.wtPnl<0?'LOSS':'FLAT'),win?GR:(t.wtPnl<0?RD:T3),t.avg,t.entry,t.exit,t.wtPnl,t.origPnl,t.bars,t.entryDate||'',t.exitDate||'',t.wtN,t.balAfter,(t.absCost||0),(t.addCost||0)); }
  return list;
}
