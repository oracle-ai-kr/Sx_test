...
function _cvPoolRender(rep, watch, mk, abMode){
  ...
  if((rep&&rep.ok&&rep.gate)||(watch&&watch.ok&&watch.gate)){
    ...
    out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin:2px 0 5px">rebound경로를 ltAlign(🐻역배열·🔘중립·🔵정배열)로 분해. <b>어느 ltAlign이 등급역전(−)인지</b> + <b>두 브래킷 다 같은 부호</b>면 진짜 게이트 변수(종목성격 아님). 추상=추가상승IC(trend 재라우팅 가치). bull은 trend로 라우팅돼 rebound엔 거의 없음.</div>';
    ...
    var gateRow=function(P,lbl){
      ...
      var g=P.gate;
      return '<div style="font-size:9.5px;margin-top:5px;line-height:1.55"><b style="color:'+PUR+'">'+lbl+'</b><br>　🐻'+gcell('역배열',g.bear)+'<br>　🔘'+gcell('중립',g.off)+'<br>　🔵'+gcell('정배열',g.bull)+'</div>';
    };
    out+=gateRow(rep,'대표');
