...
function _mxRunStats(rows, cfg, dir, N){
  ...
  const stat = {};
  preds.concat(combos).forEach(p => { stat[p.key] = { n:0, hit:0, leadSum:0 }; });
  ...
  for(let i = warm; i <= len - 1 - N; i++){
    ...
    elig++; if(did) eligHit++;
    const pflag = {};
    preds.forEach(p => { const ok = !!p.test(ctx); pflag[p.key] = ok; if(ok){ stat[p.key].n++; if(did){ stat[p.key].hit++; stat[p.key].leadSum += lead; } } });
    combos.forEach(c => { const ok = !!c.test(ctx, pflag); if(ok){ stat[c.key].n++; if(did){ stat[c.key].hit++; stat[c.key].leadSum += lead; } } });
  }
  ...
  const rowsOut = preds.concat(combos).map(p => {
    ...
    return { key:p.key, label:p.label, tier:(p.tier || 0), n:s.n, hr:hr, lead:lead, lift:lift, isCombo: !p.tier };
  });
  return { baseRate:baseRate, elig:elig, eligHit:eligHit, rows:rowsOut, N:N };
}
...
function _buildMaCrossCard(stock, indicators){
  ...
  const _hdrRow = '<div style="display:flex;align-items:center;gap:8px;padding:0 0 3px"><span style="font-size:9px;color:var(--text3);flex:1">예측조건</span><span style="font-size:9px;color:var(--text3);width:40px;text-align:right">표본</span><span style="font-size:9px;color:var(--text3);width:40px;text-align:right">적중률</span><span style="font-size:9px;color:var(--text3);width:38px;text-align:right">리프트</span><span style="font-size:9px;color:var(--text3);width:36px;text-align:right">리드</span></div>';
  const _rowFn = (r) => {
    ...
    return '<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-top:1px solid var(--border)' + _op + '"><span style="font-size:10px;font-weight:700;color:var(--text);flex:1;min-width:0">' + r.label + _tag + '</span><span style="font-size:9px;color:var(--text3);width:40px;text-align:right">' + r.n + '</span><span style="font-size:11px;font-weight:800;color:' + _lc + ';width:40px;text-align:right">' + _hr + '</span><span style="font-size:10px;font-weight:700;color:' + _lc + ';width:38px;text-align:right">' + _lift + '</span><span style="font-size:9px;color:var(--text3);width:36px;text-align:right">' + _lead + '</span></div>';
  };
  const body = R.rows.map(_rowFn).join('');
  const note = '<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px;line-height:1.55"><b>리프트</b>=적중률÷기저율 (<span style="color:#16a34a">≥1.3 유의</span>·<span style="color:#ef4444">&lt;0.8 역효과</span>) · <b>리드</b>=조건→실제크로스 평균봉수 · 룩어헤드 차단(예측자 rows[≤t]·타깃 t+1~t+' + R.N + '봉) · MA쌍=단기추세 카드 동기화 · 🔮크로스예측(kNN) 보완 · BT 실행 시 600봉으로 표본↑</div>';
