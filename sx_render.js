// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S814 · build 20260630-1230 cache-bust]
// ════════════════════════════════════════════════════════════
...
function _trendWaterAccount(bt, rows, fireMap, mode){
  ...
  for(let i=0;i<bt.trades.length;i++){
    ...
    for(let b=t.entryIdx+1; b<t.exitIdx; b++){ if(fireMap[b] && close[b]>0){ addShares+=ADD/close[b]; addCost+=ADD; wtN++; } }
    injected+=addCost;
  ...
  if(bt.open){
    ...
    let addShares=0, addCost=0, wtN=0;
    for(let b=o.entryIdx+1; b<=last; b++){ if(fireMap[b] && close[b]>0){ addShares+=ADD/close[b]; addCost+=ADD; wtN++; } }
    injected+=addCost;
    const totalShares=baseShares+addShares, invested=bal+addCost;
...
function _trendDetailInner(bt,cfg,ctx){
  ...
  const _wtDesc={
    ...
    A:"물타기 돈이 청산 후에도 안 빠지고 본체에 계속 쌓임 → 시드가 눈덩이처럼 커짐. 진짜반등 신호 뜰 때마다 100만씩 '납입'하고 빼지 않음.",
  ...
  const wtChip=`<div style="margin:2px 0 10px">`
    +`<div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:5px"><span onclick="_sxVib(10);window._trendCycleWtMode&&_trendCycleWtMode()" title="보유 중 진짜반등 신호당 100만 추가입금. 탭으로 OFF→적금/DCA→물타기 동원 순환." style="font-size:10px;font-weight:800;padding:5px 11px;border-radius:14px;border:1px solid;cursor:pointer;${_trendWtMode!=='off'?'background:#0891b2;color:#fff;border-color:#0891b2':'background:var(--surface2);color:#0891b2;border-color:#0891b266'}">💧 ${_wtBtnLabel[_trendWtMode]}</span><span style="font-size:8.5px;color:var(--text3)">탭 전환 ↻</span>${_exclChk}</div>`
