...
function _buildAEntryLine(stock, svVerdict){
  const action = svVerdict?.action || '';
  const PURPLE = '#a855f7'; // 보라색 (차트 마커와 동일)
  const BOX_BASE = 'margin-top:8px;padding:8px 10px;border-radius:8px;border-left:3px solid ';
  const BOX_ACTIVE = BOX_BASE + PURPLE + ';background:#fff';
  const BOX_IDLE   = BOX_BASE + 'var(--border)' + ';background:#fff';

  const isBuyGroup = (action === '매수' || action === '관심');
  if(!isBuyGroup){
    const _stateMsg = {
      '관망':'추가 모멘텀 확인 후 진입 검토',
      '회피':'진입 부적합 · 다음 기회 대기',
      '보유 유지':'포지션 유지 중',
      '청산 준비':'일부 익절 / 손절선 상향 검토',
      '청산 검토':'비중 축소 검토',
      '즉시 청산':'손절 실행 검토',
      '매도 완료':'포지션 종료 완료'
    }[action] || '진입 신호 대기';
    return `<div style="${BOX_IDLE}">
      <div style="font-size:10px;color:var(--text3);font-weight:700;margin-bottom:2px">📍 진입분석</div>
      <div style="font-size:10.5px;color:var(--text2)">${_stateMsg}</div>
    </div>`;
  }
  ...
  const tpPct = (((calc.tp - calc.ep) / calc.ep) * 100).toFixed(1);
  const slPct = (((calc.sl - calc.ep) / calc.ep) * 100).toFixed(1);

  return `<div style="${BOX_ACTIVE}">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
      <span style="font-size:10px;font-weight:800;color:${PURPLE}">📍 진입분석</span>
      <span style="font-size:11px;font-weight:800;color:var(--text)">진입가 ${fmt(calc.ep)}</span>
    </div>
    <div style="display:flex;justify-content:space-between;gap:6px">
      <div style="flex:1;text-align:center;padding:4px 6px;background:rgba(34,197,94,0.08);border-radius:6px">
        <div style="font-size:9px;color:var(--text3);margin-bottom:1px">목표가</div>
        <div style="font-size:11px;font-weight:800;color:#22c55e">${fmt(calc.tp)}</div>
        <div style="font-size:9px;color:#22c55e">+${tpPct}%</div>
      </div>
      <div style="flex:1;text-align:center;padding:4px 6px;background:rgba(232,54,90,0.08);border-radius:6px">
        <div style="font-size:9px;color:var(--text3);margin-bottom:1px">손절가</div>
        <div style="font-size:11px;font-weight:800;color:#e8365a">${fmt(calc.sl)}</div>
...
function _buildSimPositionLine(stock, btSt, svVerdict){
  ...
  const BOX_STYLE = 'margin-top:8px;padding:8px 10px;background:#fff;border-radius:8px;border-left:3px solid ' + (svVerdict?.color || 'var(--text3)');
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    ...
    btStateBannerHTML = `<div style="padding:12px 14px;margin:0 0 8px;border-radius:12px;background:${_svVerdict.color}0D;border:1.5px solid ${_svVerdict.color}">
      ...
      ${_buildAEntryLine(stock, _svVerdict)}
    </div>`;
  }
    ...
    <div class="anal-header-card">
      ...
      <div style="margin-top:10px;padding:10px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid #3b82f6">
        ...
        ${(()=>{
          ...
          if(!_reliabilityHTML && !_bSimHTML) return '';
          return _reliabilityHTML + _bSimHTML;
        })()}
      </div>`;
      })()}
      ${(()=>{
