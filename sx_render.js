...
// [S794] 최근 maxBack봉 스캔 — rows(600봉 일봉)에서 봉별 정렬/약세/피처. 레시피카드와 동일 fetchRows600 소스라 barsAgo·발동판정 완전 일치.
function _sxBadgeScanRecent(rows, maxBack){
  if(!Array.isArray(rows) || rows.length<260 || typeof SXE==='undefined' || !SXE.calcAllScreener) return null;
  var n=rows.length, from=Math.max(250, n-1-maxBack), scan=[];
  for(var bi=from; bi<n; bi++){
    var slice=rows.slice(Math.max(0,bi-249), bi+1), ind=null;
    try { ind=SXE.calcAllScreener(slice,'day'); } catch(e){}
    if(!ind) continue;
    var f=null; try { f=_extractFeats733(ind, rows, bi); } catch(e){}
    scan.push({ barsAgo:(n-1-bi), lt:((typeof _ltStr733==='function')?_ltStr733(ind.maAlignLT):'off'), maBull:!!(ind.maAlign&&ind.maAlign.bullish), f:f });
  }
  return scan;   // 오래된→최신 순
}
// 배지 def의 최근 발동 {barsAgo, count, others} (없으면 null) — 결승선 임박(barsAgo 최대=오래된) 우선
function _sxBadgeLastFire(def, scan){
  ...
  if(!recs.length || !Array.isArray(scan)) return null;
  var fires=[];
  for(var i=0;i<scan.length;i++){ var s=scan[i];
    if(s.lt===def.poolLt && !s.maBull && s.f){ var m=recs.filter(function(r){return _condMatch733(s.f,r.conds,r.mode);}); if(m.length) fires.push({barsAgo:s.barsAgo, count:m.length}); }
  }
  if(!fires.length) return null;
  var prim=fires[0];
  for(var j=1;j<fires.length;j++){ if(fires[j].barsAgo>prim.barsAgo) prim=fires[j]; }   // [S793] 가장 오래된(결과 임박) 우선 — 끝나면 다음 신호로 승계
  return { barsAgo:prim.barsAgo, count:prim.count, others:(fires.length-1) };
}
// 배지 1행 내부 — scan(600봉 일봉)에서 현재봉(barsAgo0)+최근발동 전부 계산. scan=null→평가중, scan===false→데이터부족.
function _sxBadgeRowInner(def, scan){
  var GR='var(--text3)';
  var _e=function(t){ return String(t==null?'':t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); };
  var recs=(typeof window!=='undefined'&&window.SXRecipeSignal&&Array.isArray(window.SXRecipeSignal.RECIPES))?window.SXRecipeSignal.RECIPES.filter(function(r){return r.pool===def.pool&&r.kind===def.kind;}):[];
  var total=recs.length;
  var descLine='<div style="font-size:8px;color:'+GR+';margin-top:3px;font-style:italic">'+def.desc+'</div>';
  if(scan===null || scan===false){   // 로딩 or 데이터부족
    var msg = (scan===false) ? '600봉 미확보' : '평가 중…';
  ...
  // 현재봉(barsAgo 0)
  var cur=null; for(var i=0;i<scan.length;i++){ if(scan[i].barsAgo===0){ cur=scan[i]; break; } }
  var inPool = cur ? (cur.lt===def.poolLt && !cur.maBull) : false;
  var curMatched = (inPool && cur && cur.f) ? recs.filter(function(r){return _condMatch733(cur.f,r.conds,r.mode);}) : [];
  // 최근발동(오래된 우선)
  var lf=_sxBadgeLastFire(def, scan);
  var fire = lf ? lf.barsAgo : null;
  var cnt  = lf ? lf.count : 0;
  var stage = (fire==null)?null:(fire===0?'now':(fire<=5?'early':'late'));
  var chip;
  ...
  if(!inPool){ statusTxt='○ 풀 불일치'; statusCol=GR; }
  else if(stage==='now'){ statusTxt='✅ 발동 '+cnt+'/'+total; statusCol=def.color; }
  else if(stage){ statusTxt='⏳ '+fire+'봉전 '+cnt+'개 발동 · 관찰중'; statusCol=def.color; }
  ...
  var detail;
  if(stage==='now' && curMatched.length){
    var top=curMatched.slice(0,2).map(function(r){return _e(r.label);}).join('<br>');
    detail='<div style="font-size:9px;color:var(--text2);line-height:1.5;margin-top:3px">'+top+(curMatched.length>2?('<br><span style="color:'+GR+'">외 '+(curMatched.length-2)+'개 충족</span>'):'')+'</div>';
  } else if(stage==='early'||stage==='late'){
    var othTxt=(lf&&lf.others>0)?(' · 외 '+lf.others+'건 관찰중'):'';
    detail='<div style="font-size:8.5px;color:'+def.color+';margin-top:3px">'+fire+'봉전 발동 → 결과 관찰중 ('+(stage==='early'?'~5봉':'~10봉')+' 윈도우)'+othTxt+'</div>';
  } else {
    detail='<div style="font-size:8.5px;color:'+GR+';margin-top:3px">충족 레시피 없음 ('+total+'개 중 0)</div>';
  }
...
function _buildBadgeInventoryCard(stock, indicators){
  ...
  if(code && window.SXCandleBT && SXCandleBT.fetchRows600){   // 레시피카드와 동일 fetchRows600(캐시 공유) → 결과 일치
    setTimeout(function(){ try{ _sxBadgeAsyncRender(token, mk, code); }catch(e){} }, 40);
  }
  ...
  var desc = '<div style="font-size:8.5px;color:var(--text3);line-height:1.5;margin-bottom:4px">신호 배지 <b style="color:var(--text2)">시제품 보관소(SSOT)</b> — 다른 배지가 가져다 씁니다. <b>4종</b>: 역배열 바닥반등·데드캣 / 정배열 눌림목·되돌림. 각 배지 = <b>그 풀×종류 레시피가 1개라도 발동</b>하면 ON. 최근 발동은 <b>조짐/주의(~5봉)→유력/위험(~10봉)</b>으로 결과 관찰. <b>fetchRows600(레시피카드와 동일 일봉600)</b> 기준이라 봉수·발동이 일치. <b style="color:var(--text2)">회색=풀 불일치 또는 미발동</b>.</div>';
  return _sxExpCard('sxBadgeInvWrap', 'sxBadgeInvWrap_b', titleHtml, rightHtml, desc + rowsHtml);
}
// 비동기 렌더 — fetchRows600(레시피카드와 동일 캐시) → 최근 10봉 스캔 → 각 행 갱신 (토큰 가드)
async function _sxBadgeAsyncRender(token, mk, code){
  if(typeof document==='undefined' || window._sxBadgeTok!==token) return;
  var rows=null; try { rows=(window.SXCandleBT&&SXCandleBT.fetchRows600)? await SXCandleBT.fetchRows600(mk,'day',code) : null; } catch(e){}
  if(window._sxBadgeTok!==token) return;   // 네비게이션됨 → 폐기
  var scan=(Array.isArray(rows)&&rows.length>=260)?_sxBadgeScanRecent(rows,10):null;
  var arg = scan ? scan : false;   // scan 실패 → false(데이터부족)
  var active=0;
  SX_BADGE_ORDER.forEach(function(key){
    var def=SX_BADGE_DEFS[key];
    var el=document.getElementById('sxBadgeRow_'+key); if(!el) return;
    if(scan && _sxBadgeLastFire(def, scan)) active++;
    el.innerHTML=_sxBadgeRowInner(def, arg);
  });
  var cEl=document.getElementById('sxBadgeInvCount');
  if(cEl){ if(!scan){ cEl.textContent='데이터 부족'; cEl.style.color='var(--text3)'; } else { cEl.style.color=active?'#16a34a':'var(--text3)'; cEl.textContent=active?(active+' 활성'):(String(SX_BADGE_ORDER.length)+'종'); } }
}
if(typeof window!=='undefined'){ window._buildBadgeInventoryCard = _buildBadgeInventoryCard; window._sxBadgeAsyncRender = _sxBadgeAsyncRender; window._sxBadgeScanRecent = _sxBadgeScanRecent; }
...
async function _btGateAuditBracket(mk, source, onProgress){
  ...
  for(var i=0;i<use.length;i++){
    ...
    for(var bi=250; bi<rows.length-10; bi++){
      ...
      var rec={ n3:_nRet(3), n5:_n5, n10:_n10, hold:((_n5===null||_n10===null)?null:(_n5===true&&_n10===true)) };
...
async function _btGateAuditRun(){
  ...
  el.innerHTML=_btGateAuditRender(rep, watch, mk);
}
function _btGateAuditRender(rep, watch, mk){
  ...
  var keys=[{k:'n3',l:'N3'},{k:'n5',l:'N5'},{k:'n10',l:'N10'},{k:'hold',l:'유지'}];
  var _cell=function(r){ return r ? (r.rate+'%') : '—'; };
  var _grpRow=function(label, arr, col){
    var cells=keys.map(function(kk){ var r=_rate(arr,kk.k); return '<span style="width:48px;text-align:right;font-weight:800;color:'+col+'">'+_cell(r)+'</span>'; }).join('');
  ...
  var hdr='<div style="display:flex;font-size:8.5px;font-weight:700;color:'+T3+';padding-bottom:3px"><span style="flex:1">그룹</span>'+keys.map(function(kk){return '<span style="width:48px;text-align:right">'+kk.l+'</span>';}).join('')+'</div>';
  ...
  if(wN10 && sN10){
    ...
    var vcol, vtxt, vsum;
    if(dN10>=10){ vcol=GRN; vtxt='✅ 게이트 B 정당'; vsum='강세(통과)가 약세(차단)보다 N10 +'+dN10+'%p 높음 — maAlignBull이 가짜반등을 실제로 거름. 약세 차단·강세 통과가 맞음.'; }
    else if(dN10>=4){ vcol=AMB; vtxt='△ 게이트 B 약하게 정당'; vsum='강세가 약세보다 N10 +'+dN10+'%p — 방향은 맞으나 격차 작음. maAlignBull의 가짜반등 변별력이 약함.'; }
    else if(dN10>-4){ vcol=AMB; vtxt='⚠️ 게이트 B 근거 불충분'; vsum='강세·약세 N10 차이 '+(dN10>=0?'+':'')+dN10+'%p로 거의 없음 — maAlignBull이 가짜반등을 못 가름. 분기 근거 약함.'; }
    else { vcol=RED; vtxt='❌ 게이트 B 역효과 의심'; vsum='강세가 약세보다 N10 '+dN10+'%p 낮음(역전) — maAlignBull로 가르면 오히려 좋은 걸 차단할 수 있음. 게이트 재검토 필요.'; }
    verdict='<div style="margin-top:8px;padding:8px 10px;border-radius:8px;background:'+vcol+'14;border:1px solid '+vcol+'55"><div style="font-size:11px;font-weight:800;color:'+vcol+';margin-bottom:3px">'+vtxt+'</div><div style="font-size:9px;color:'+T2+';line-height:1.5">'+vsum+(dHold!=null?(' · 유지 격차 '+(dHold>=0?'+':'')+dHold+'%p'):'')+'</div></div>';
  }
  var desc='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:6px">C 게이트 B = <b>역배열+단기약세→반등차단</b>. 가정: <b>maAlignBull(단기정배열)이 가짜반등을 거른다</b>. 역배열 진입을 maAlignBull로 갈라 N10 비교 — <b style="color:'+GRN+'">강세가 약세보다 높으면 게이트 정당</b>. 대표+관심 합산 · <b style="color:'+GRN+'">BT 무관 순수 데이터</b>.</div>';
  var note='<div style="font-size:8px;color:'+T3+';margin-top:7px;line-height:1.45">약세=maAlignBull false(게이트가 <b>차단</b>) · 강세=maAlignBull true(게이트가 <b>통과</b>) · N10=진입 후 10봉 종가>진입가 비율 · 유지=N5·N10 둘다 위. 역배열 모든 봉 측정.</div>';
