...
// ════════════════════════════════════════════════════════════════════
function _candleTransitionScore(rows, indicators){
  ...
  // [S490] 2봉 전환 패턴 감지 (직전+최신) — 색이 바뀌는 전환봉이라 연속 카운트가 끊김. 별도 flip 모드로 포착.
  const _o1=O(last-1),_c1=C(last-1),_o0=O(last),_c0=C(last);
  const _up1=_c1>=_o1, _b1=Math.abs(_c1-_o1), _b0=Math.abs(_c0-_o0), _mid1=(_o1+_c1)/2;
  const engulfBull = !_up1 && _c0>=_o0 && _o0<=_c1 && _c0>=_o1 && _b0>_b1;   // 상승장악: 직전 음봉 몸통을 최신 양봉이 완전히 감쌈
  const engulfBear =  _up1 && _c0< _o0 && _o0>=_c1 && _c0<=_o1 && _b0>_b1;   // 하락장악: 직전 양봉 몸통을 최신 음봉이 완전히 감쌈
  const piercing   = !_up1 && _c0>=_o0 && _o0<_c1 && _c0>_mid1 && _c0<_o1;   // 관통: 직전 음봉 50%+ 회복한 양봉(완전감쌈 아님)
  const darkCloud  =  _up1 && _c0< _o0 && _o0>_c1 && _c0<_mid1 && _c0>_o1;   // 흑운: 직전 양봉 50%+ 잠식한 음봉(완전감쌈 아님)

  let mode = null;
  if(redCnt >= 2) mode='rebound';
  else if(greenCnt >= 2) mode='reversal';
  else if(engulfBull || piercing) mode='flipUp';
  else if(engulfBear || darkCloud) mode='flipDown';
  if(!mode) return { active:false, reason:'연속 음봉/양봉 2개 미만', redCnt, greenCnt };
  ...
  // [S490] flip 모드 — 2봉 전환 패턴(연속 끊긴 전환봉). 패턴 가점 + 추세 정합 + 거래량 신뢰도 → early return.
  if(mode === 'flipUp' || mode === 'flipDown'){
    const fup = mode === 'flipUp', fdir = fup ? 1 : -1;
    const trendDnF = slope < -0.005 && pos60 < 0.5;
    if(engulfBull){ score += 35; reasons.push('🔺 상승 장악형 — 직전 음봉 감싸는 양봉 (강세 전환)'); }
    else if(piercing){ score += 25; reasons.push('🔺 관통형 — 음봉 50%+ 회복 (강세 전환)'); }
    if(engulfBear){ score -= 35; reasons.push('🔻 하락 장악형 — 직전 양봉 감싸는 음봉 (약세 전환)'); }
    else if(darkCloud){ score -= 25; reasons.push('🔻 흑운형 — 양봉 50%+ 잠식 (약세 전환)'); }
    if(fup){
      if(trendDnF){ score += 12; reasons.push('하락추세 바닥 반전 신호 (신뢰도↑)'); }
    ...
    const label = score >= 30 ? '양봉 유망' : score <= -30 ? '음봉 유망' : '중립';
    return { active:true, mode, score, label, reasons, warn, redCnt, greenCnt, streak:0 };
  }
  ...
  score = Math.max(-100, Math.min(100, Math.round(score)));
  const label = score >= 30 ? '양봉 유망' : score <= -30 ? '음봉 유망' : '중립';
  return { active:true, mode, score, label, reasons, warn, redCnt, greenCnt, streak };
}
...
function _buildTransitionCard(stock, indicators){
  ...
  const r = _candleTransitionScore(rows, indicators);
  // [S486] 최신 봉 날짜 표기 — 오늘과 다르면 빨강(데이터 시점 주의). 종목별 최신봉 시차 혼동 방지.
  const _lastDate = rows.length ? rows[rows.length-1].date : null;
  ...
  const _dateLabel = _isToday ? `기준 ${_dateTxt}` : `⚠️ 기준 ${_dateTxt}`;
  const head = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:13px;font-weight:800;color:var(--text)">🧪 캔들 전이 점수</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span><span style="font-size:10px;font-weight:700;color:${_dateCol};margin-left:auto">${_dateLabel}</span></div>`;
  ...
  const top = `<div style="display:flex;align-items:baseline;gap:8px"><span style="font-size:26px;font-weight:800;color:${col}">${sign}${r.score}</span><span style="font-size:13px;font-weight:700;color:${col}">${r.label}</span><span style="font-size:10px;color:var(--text3);margin-left:auto">${r.mode==='rebound'?`음봉 ${r.streak}연속`:r.mode==='reversal'?`양봉 ${r.streak}연속`:r.mode==='flipUp'?'🔺 양전환 패턴':'🔻 음전환 패턴'}</span></div>`;
  ...
  const note = `<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · −100~+100 (양수=양봉 유망, 음수=음봉 유망)</div>`;
  ...
  return wrap(top + bar + _seqRow + warnHtml + reasonsHtml + note);
}
...
let _sxMacroTimer = null, _sxMacroIdx = 0;
...
function _sxMacroEvents(){
  ...
  const evs = [];
  const push = (key, emoji, label, next, prev, cls, curTxt, impl) => {
    const dn = next ? _sxMacroDday(next, t) : null;     // 다음 발표까지 남은 일
    const sp = prev ? -_sxMacroDday(prev, t) : null;    // 직전 발표 후 경과 일
    ...
    if(!mode) return;
    const when = mode==='soon' ? `${emoji} ${label} 곧 발표 (${_sxMacroMD(next)})` : `${emoji} ${label} 방금 발표`;
    const cur  = curTxt ? `\n직전: ${curTxt}` : '';
    const tail = mode==='soon' ? '\n발표 전후 변동성 확대에 유의하세요.' : '';
    // [S488] 다중 임박 시 한 토스트에 모을 간략 1줄 (영향 생략, 공통 변동성 문구는 하단 1회)
    const lineWhen = mode==='soon' ? `발표임박 ${_sxMacroMD(next)}` : '결과발표';
    const toastLine = `${emoji} ${label} · ${lineWhen}${curTxt?' · '+curTxt:''}`;
    evs.push({ key, mode, badgeText, cls, toastLine, toastMsg:`${when}${cur}\n\n${impl}${tail}` });
  };
  if(d.fomc) push('fomc','🏦','FOMC', d.fomc.next, d.fomc.prev, 'neu', d.fomc.rate?('현재 '+d.fomc.rate):'', 'FOMC 금리 결정 — 인하 신호면 호재, 매파적이면 부담이에요.');
  (d.indicators||[]).forEach(it=>{ const j=_sxMacroJudge(it); push(it.key, it.emoji, it.label, it.next, it.prev, j.cls, j.curTxt, j.impl); });
...
function _sxMacroBadgeHTML(ev){
  ...
  return `<span class="sxb-badge" style="color:${col};background:${col}1A;border:1px solid ${col};cursor:pointer" onclick="event.stopPropagation();_sxMacroWhy && _sxMacroWhy()">${ev.badgeText}</span>`;
}
// [S488] 클릭 시: 단일이면 상세, 다중이면 한 토스트에 임박/결과 전부 모아 표시
function _sxMacroWhy(){
...
function _sxMacroTick(){
  const slot = document.getElementById('sxMacroBadgeSlot');
  if(!slot) return;                       // 배지줄 미존재(다른 화면) → 다음 tick
  const evs = _sxMacroEvents();
  if(!evs.length){ slot.innerHTML=''; return; }
  if(_sxMacroIdx >= evs.length) _sxMacroIdx = 0;
  slot.innerHTML = _sxMacroBadgeHTML(evs[_sxMacroIdx]);
  _sxMacroIdx++;
}
function _sxMacroInit(){                   // _sxbHTML 호출마다 불려도 안전(fetch 캐시·타이머 중복가드)
  _sxMacroFetch().then(()=>{ _sxMacroIdx=0; _sxMacroTick(); });
  if(!_sxMacroTimer) _sxMacroTimer = setInterval(_sxMacroTick, 2000); // [S488] 4.5초→2초 회전 (색 교차로 주목도↑)
}
...
function _sxbHTML(){
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    ...
    + `<div class="sxb-head-badges">${transBadge}${whipBadge}${_dumpBadge}${_safetyBadge}${_mixBadge}${_mtfBadge}<span id="sxMacroBadgeSlot">${_sxMacroBadgeHTML(_sxMacroEvents()[0])}</span></div>`
