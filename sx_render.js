...
function _trendBt(rows,cfg,bbP){
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){
      ...
      if(dc || sellHit(i) || _predFail || _predExit){ const pnl=(close[i]/pos.entry-1)*100; trades.push({entry:pos.entry,exit:close[i],pnl:+pnl.toFixed(2),bars:i-pos.entryIdx,entryIdx:pos.entryIdx,exitIdx:i,re:!!pos.re,pred:!!pos.pred,predLead:pos.predLead||0,fail:!!_predFail,predExit:!!_predExit,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||'',exitDate:(rows[i]&&(rows[i].date||rows[i].t))||''}); pos=null; _lastExitIdx=i; }
    }
  }
...
function _trendRenderInner(){
  ...
  const note=`<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · ${_effPredict?`🔮 예측 선행 ${_predLeadLbl}(kNN) → `:''}${cfg.s}MA×${cfg.l}MA 골든크로스 ${cfg.nextOpen?'→ 다음봉 시가 진입':'종가 진입'}${_buyN?` · 매수 ${_buyN}개 AND(최근${cfg.n}봉)`:' · MA 단독'} → 데드크로스${_sellN?` OR 매도 ${_sellN}개`:''} 종가청산${_effPredict?` · 예측 진입+조기청산 · 실패 시 손절`:''}${cfg.reentry?' · 🔄정배열 재진입':''} · <b style="color:${_trBarsCol}">[${_trBars}봉 기준]</b></div>`;
...
function _buildChartPredictCard(stock, indicators){
  ...
  const _regimeOn = !!(typeof window!=='undefined' && window._sxCpRegimeOn);   // [S663] 레짐필터 토글(기본 ON — 전역 초기화)
  ...
  const stats = `<div style="display:flex;gap:6px;margin-top:10px;padding:8px 4px;background:var(--surface2);border-radius:8px">
    ${statBox('상승확률', r.upProb+'%', upC)}
    ${statBox('평균수익', sgn(r.avg)+r.avg+'%', avgC)}
    ${statBox('최대수익', sgn(r.max)+r.max+'%', RED)}
  </div>
  <div style="text-align:center;font-size:9.5px;color:var(--text3);margin-top:4px">중앙 ${sgn(r.median)}${r.median}% · 최저 ${sgn(r.min)}${r.min}% · 이후 ${r.h}봉 기준</div>`;
...
if(typeof window!=='undefined'){ if(window._sxCpRegimeOn===undefined) window._sxCpRegimeOn=true; if(window._sxCpHybridOn===undefined) window._sxCpHybridOn=false; window._cpRegimeToggle=_cpRegimeToggle; window._cpHybridToggle=_cpHybridToggle; window._cpRerenderCard=_cpRerenderCard; }
...
function _cpCloseBacktest(){ var ov=document.getElementById('sxCpBtOv'); if(ov) ov.remove(); }
...
function _svRunSignalVal(){
  try{
    var rows=window._sxCpRows;
    if(!rows||rows.length<175){ if(typeof alert==='function')alert('신호검증엔 데이터가 더 필요해요(175봉+).'); return; }
    if(!(window.SXVAL&&SXVAL.runSuite)){ if(typeof alert==='function')alert('SXVAL 미로드'); return; }
    _cpBtOverlay('<div style="text-align:center;padding:34px 10px;color:#475569;font-size:13px;font-weight:800">🔬 신호 검증 계산 중…<div style="font-size:11px;color:#94a3b8;font-weight:500;margin-top:8px">'+rows.length+'봉 · 5개 신호 워크포워드 · LR 학습 포함이라 십여 초 걸릴 수 있어요</div></div>');
    setTimeout(function(){ var r; try{ r=SXVAL.runSuite(rows); }catch(e){ r={ok:false,reason:String((e&&e.message)||e)}; } _cpBtOverlay(_svRenderSignalVal(r)); }, 40);
  }catch(_){ _cpCloseBacktest(); }
}
function _svRenderSignalVal(r){
  var RED='#e3493b',BLUE='#2563eb',GRN='#16a34a',AMB='#f59e0b',T3='#94a3b8';
  var close='<div style="text-align:right;margin-top:14px"><span onclick="window._cpCloseBacktest&&_cpCloseBacktest()" style="display:inline-block;font-size:12px;font-weight:700;color:#475569;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:8px;padding:7px 18px;cursor:pointer">닫기</span></div>';
  if(!r||!r.ok){ return '<div style="font-size:15px;font-weight:800;color:#0f172a;margin-bottom:8px">🔬 신호 검증</div><div style="font-size:12px;color:#64748b;line-height:1.6">실행할 수 없어요'+(r&&r.reason?' ('+r.reason+')':'')+'. 일봉 600봉처럼 이력이 충분할 때 시도해줘.</div>'+close; }
  function bcol(v){ return v==null?T3:v>=56?GRN:v>=52?AMB:v>=48?'#64748b':RED; }
  function trap(m){ if(!m.ok||m.contAcc==null||m.revAcc==null) return '<span style="color:'+T3+'">—</span>';
    var warn=(m.contAcc-m.revAcc)>25;
    return '<span style="color:'+(m.contAcc>=55?GRN:'#64748b')+'">'+m.contAcc+'</span><span style="color:'+T3+'">→</span><span style="color:'+(m.revAcc>=45?'#64748b':RED)+'">'+m.revAcc+'</span>'+(warn?' <span style="color:'+RED+'">⚠️</span>':''); }
  var h='<div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin-bottom:3px"><span style="font-size:15px;font-weight:800;color:#0f172a">🔬 신호 검증</span><span style="font-size:10px;color:'+T3+'">'+r.nPts+'시점 · 다음캔들</span></div>';
  h+='<div style="font-size:11px;color:#475569;margin-bottom:10px">기준선(항상 양봉 찍기) 실제 양봉률 <b>'+r.baseRate+'%</b> · <b>균형 50%</b>=찍기. 어느 신호가 다음 캔들 방향을 맞히나.</div>';
  h+='<div style="display:grid;grid-template-columns:1fr auto auto;gap:8px;padding:0 8px 3px;font-size:8.5px;color:'+T3+';font-weight:700"><span>신호</span><span style="text-align:right">균형정확도</span><span style="text-align:right">지속→전환</span></div>';
  r.signals.forEach(function(s,i){ var m=s.metrics, st=(i===0&&m.ok&&m.balAcc!=null);
    h+='<div style="display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;padding:6px 8px;border-radius:7px;'+(st?'background:#eff6ff':'')+'">'
      +'<span style="font-size:11px;font-weight:'+(st?800:600)+';color:#0f172a">'+(st?'★ ':'')+s.label+(m.ok?'':' <span style="color:'+T3+';font-weight:400">(표본부족)</span>')+'</span>'
      +'<span style="font-size:12px;font-weight:800;color:'+bcol(m.ok?m.balAcc:null)+';min-width:44px;text-align:right">'+(m.ok&&m.balAcc!=null?m.balAcc+'%':'—')+'</span>'
      +'<span style="font-size:10.5px;font-weight:700;min-width:62px;text-align:right">'+trap(m)+'</span></div>'; });
  var b=r.signals[0].metrics;
  if(b.ok){
    h+='<div style="margin-top:11px;padding:10px 11px;background:#f8fafc;border-radius:9px;border:1px solid #eef2f7">';
    h+='<div style="font-size:11px;font-weight:800;color:#0f172a;margin-bottom:5px">★ 최고: '+r.signals[0].label+'</div>';
    h+='<div style="font-size:10.5px;color:#475569;line-height:1.7">상승콜 <b>'+(b.upAcc!=null?b.upAcc+'%':'—')+'</b>('+b.upCalls+'건) · 하락콜 <b>'+(b.dnAcc!=null?b.dnAcc+'%':'—')+'</b>('+b.dnCalls+'건) · 무승부 '+b.tie+'건 제외</div>';
    var cal=b.calib.filter(function(c){return c.n>0;});
    if(cal.length) h+='<div style="font-size:9px;color:'+T3+';margin-top:7px;line-height:1.6">캘리브레이션(예측상승확률→실제양봉률): '+cal.map(function(c){var col=(c.actUp!=null&&c.actUp>=c.lo)?GRN:'#64748b';return '<span style="color:'+col+'">'+c.lo+'-'+c.hi+'→'+(c.actUp!=null?c.actUp+'%':'-')+'('+c.n+')</span>';}).join(' · ')+'</div>';
    h+='</div>';
  }
  var bv=b.ok?b.balAcc:null, verdict,vcol;
  if(bv==null){ verdict='결정 표본 부족'; vcol=T3; }
  else if(bv>=56){ verdict=r.signals[0].label+'가 방향 스킬 있음 (찍기 대비 +'+(Math.round((bv-50)*10)/10)+'p)'; vcol=GRN; }
  else if(bv>=52){ verdict=r.signals[0].label+'가 약한 스킬'; vcol=AMB; }
  else { verdict='전 신호 사실상 찍기 수준 — 다음캔들 예측 어려움'; vcol='#64748b'; }
  h+='<div style="margin-top:10px;font-size:10.5px;line-height:1.65;color:#334155"><b style="color:'+vcol+'">['+verdict+']</b>';
  if(b.ok&&b.contAcc!=null&&b.revAcc!=null&&(b.contAcc-b.revAcc)>25) h+=' '+r.signals[0].label+'은 지속 '+b.contAcc+'% vs 전환 '+b.revAcc+'% — <b style="color:'+RED+'">추세추종</b>(전환점서 약함).';
  h+='</div>';
  ...
  h+='<div style="margin-top:9px;font-size:9px;color:'+T3+';line-height:1.6;border-top:1px solid #eef2f7;padding-top:7px">다음 캔들(양봉/음봉) 예측 · 룩어헤드 0 · LR은 계산비용 때문에 '+r.step+'봉 간격 표본. ⚠️ 단일종목 '+r.nPts+'시점 — 노이즈 큼, 여러 종목 집계해야 신뢰.</div>';
  return h+close;
}
