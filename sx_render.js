...
function _trendCfg(market){
  ...
  let cfg={ s:d[0], l:d[1], n:3, aux:{}, sell:{} };
  try { const raw=localStorage.getItem('SX_TREND_'+market); if(raw){ const o=JSON.parse(raw); if(o){ cfg.s=+o.s||d[0]; cfg.l=+o.l||d[1]; cfg.n=+o.n||3; cfg.aux=(o.aux&&typeof o.aux==='object')?o.aux:{}; cfg.sell=(o.sell&&typeof o.sell==='object')?o.sell:{}; } } } catch(_){}
...
function _trendBt(rows,cfg){
  ...
  let rsi,rsiSig,mac,vMa,obv,obvSig,ma20,ma60;
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){ if(gc && auxOk(i)) pos={entry:close[i],entryIdx:i}; }
    else if(dc || sellHit(i)){ const pnl=(close[i]/pos.entry-1)*100; trades.push({entry:pos.entry,exit:close[i],pnl:+pnl.toFixed(2),bars:i-pos.entryIdx,entryIdx:pos.entryIdx,exitIdx:i,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||'',exitDate:(rows[i]&&(rows[i].date||rows[i].t))||''}); pos=null; }
  }
  let open=null;
...
function _trendRenderInner(){
  ...
  const maRow=`<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:8px;font-size:11px;color:var(--text2)">
    ...
    <span style="margin-left:auto;display:flex;gap:6px"><span onclick="window._trendReset&&_trendReset()" style="font-size:11px;font-weight:800;padding:5px 11px;border-radius:7px;background:var(--surface2);color:var(--text2);border:1px solid var(--border);cursor:pointer">리셋</span><span onclick="window._trendApply&&_trendApply()" style="font-size:11px;font-weight:800;padding:5px 12px;border-radius:7px;background:#2563eb;color:#fff;cursor:pointer">적용</span></span>
  </div>
  <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:8px;font-size:11px;color:var(--text2)">
    <span>보조 최근</span>${_inp('sxTrendN',cfg.n)}<span>봉</span>
  </div>`;
  // 매수/매도 토글 칩
  ...
  const buyChips=_TREND_AUX.map(a=>_chip(a,!!(cfg.aux&&cfg.aux[a.k]),'_trendToggleAux','#2563eb')).join('');
  const sellChips=_TREND_SELL.map(a=>_chip(a,!!(cfg.sell&&cfg.sell[a.k]),'_trendToggleSell','#e8365a')).join('');
  const _catLbl=(t,c)=>`<div style="font-size:9px;font-weight:800;color:${c};margin:0 0 5px;letter-spacing:.2px">${t}</div>`;
  const chipRow=`${_catLbl('🟢 매수 조건 · 모두 충족(AND)','#2563eb')}<div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:9px">${buyChips}</div>${_catLbl('🔴 매도 조건 · 하나라도(OR) · 데드크로스에 추가','#e8365a')}<div style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:9px">${sellChips}</div>`;
  // 상태
  ...
  const note=`<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · ${cfg.s}MA×${cfg.l}MA 골든크로스 진입${_buyN?` · 매수 ${_buyN}개 AND(최근${cfg.n}봉)`:' · MA 단독'} → 데드크로스${_sellN?` OR 매도 ${_sellN}개`:''} 청산</div>`;
...
function _trendReset(){ const ctx=window._sxTrendCtx; if(!ctx) return; const m=ctx.market; const d=_TREND_MKT_DEFAULT[m]||[5,9]; _trendSave(m,{ s:d[0], l:d[1], n:3, aux:{}, sell:{} }); _trendRerender(); }
if(typeof window!=='undefined'){ window._trendApply=_trendApply; window._trendToggleAux=_trendToggleAux; window._trendToggleSell=_trendToggleSell; window._trendReset=_trendReset; }
...
function _trendDetailInner(bt,cfg,ctx){
  ...
  const sub=`<div style="font-size:10px;color:var(--text3);margin-bottom:10px">${cfg.s}MA×${cfg.l}MA 골든→데드 · 매수: ${auxLbl}${auxOn.length?` (최근${cfg.n}봉)`:''}${sellLbl?` · 매도(OR): ${sellLbl}`:''}${nm}</div>`;
