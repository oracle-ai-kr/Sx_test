...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  body.innerHTML = `
    ...
    ${(()=>{
      ...
      if(!inner) return '';
      return `<div class="disc-card-rendered">${inner}</div>`;
    })()}
    ...
    ${(()=>{
      ...
      return `<div class="anal-section">
        ...
        ${fin.evEbitda!=null?`<div class="anal-row"><span class="al">EV/EBITDA${fin._ebitdaApprox?' <span style="font-size:8px;color:var(--text3);font-weight:normal">(근사)</span>':''}</span><span class="ar ${fin.evEbitda>0&&fin.evEbitda<5?'bullish':fin.evEbitda>15||fin.evEbitda<0?'bearish':'neutral'}">${fin.evEbitda.toFixed(1)}배</span></div>`:''}
  ...
  if(di && typeof SXI!=='undefined' && SXI.renderDisclosureCard){
    ...
    if(summaryEl && !existingCard){
      ...
      if(cardHTML){
        ...
        wrap.className = 'disc-card-rendered';
