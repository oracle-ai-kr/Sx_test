...
function _mxRunStats(rows, cfg, dir, N){
  ...
  const preds = [
    ...
    { key:'slope', label:(dir === 'gc' ? '단기선 기울기↑' : '단기선 기울기↓'), tier:1, test: c => (dir === 'gc' ? c.sUp === true : c.sUp === false) },
  ...
  for(let i = warm; i <= len - 1 - N; i++){
    ...
    const ctx = { gap, conv, btc, sUp, nearPct, vr, obvDir, macdDir, rsiS };
...
// ===== [S705] 범용 검증기 카드 (SXVAL UI) — 자(크로스/상승/MDD) × 신호 멀티선택 × 출처(현재종목/대표풀/관심목록) → 정밀·재현·리프트 리더보드 =====
//  엔진 = sx_validator.js (window.SXVAL). 순수수학형 신호만(평탄화 함정 회피). 점수조건 어댑터는 후속(SXVAL.registerPredictor 슬롯).
//  골격은 _buildMaCrossCard(MA크로스 전이) 복제 · 풀 순회는 _trendBatchRun 패턴(CAP20·fetchRows600·진행률 인라인).
var _sxvRuler = 'cross', _sxvDir = 'gc', _sxvN = 3, _sxvThr = 3, _sxvSrc = 'cur';   // [S705]
...
function _sxvToggleDir(){ _sxvDir = (_sxvDir === 'gc') ? 'dc' : 'gc'; _sxvRerender(); }
...
function _sxvSetSrc(s){ _sxvSrc = s; _sxvRerender(); }
...
// 검증 실행 — 현재종목=즉시(SXVAL.run) · 대표/관심=비동기 풀(fetchRows600 × CAP20 → SXVAL.runPool)
async function _sxvRun(){
  var L = window._sxvLast; if(!L) return;
  var resEl = document.getElementById('sxValResult'); if(!resEl) return;
  var preds = _sxvActivePreds();
  if(!preds.length){ resEl.innerHTML = '<div style="font-size:11px;color:#d97706;padding:8px 2px">신호를 1개 이상 선택해줘.</div>'; return; }
  if(typeof SXVAL === 'undefined'){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">검증 엔진(SXVAL) 미로드 — sx_validator.js 연결 확인.</div>'; return; }
  var mk = (L.stock && (L.stock._mkt || L.stock.market)) || (typeof currentMarket !== 'undefined' ? currentMarket : 'kr');
  var cfg = L.cfg || { s:5, l:9 };
  var opt = { s:cfg.s, l:cfg.l, N:_sxvN, dir:_sxvDir, thr:_sxvThr };
  if(_sxvSrc === 'cur'){
    var rows = (L.indicators && L.indicators._advanced && Array.isArray(L.indicators._advanced.rows)) ? L.indicators._advanced.rows
             : (L.stock && Array.isArray(L.stock._lastAnalCandles)) ? L.stock._lastAnalCandles : null;
    if(!rows || rows.length < 60){ resEl.innerHTML = '<div style="font-size:11px;color:var(--text3);padding:8px 2px">현재 종목 캔들 부족(≥60 필요).</div>'; return; }
    var R; try { R = SXVAL.run(rows, _sxvRuler, preds, opt); } catch(e){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">계산 오류: ' + String((e && e.message) || e) + '</div>'; return; }
    resEl.innerHTML = _sxvRenderResult(R, false);
    return;
  }
  // ── 풀(대표/관심) ──
  if(!(window.SXCandleBT && SXCandleBT.fetchRows600)){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">캔들 fetch 미연결(SXCandleBT).</div>'; return; }
  var list = _sxvPoolList(mk, _sxvSrc);
  if(!list.length){ resEl.innerHTML = '<div style="font-size:11px;color:var(--text3);padding:8px 2px">' + (_sxvSrc === 'watch' ? ('관심목록(' + mk + ') 비어있음 — 종목 추가') : ('대표목록 없음(' + mk + ')')) + '</div>'; return; }
  var CAP = 20; var capped = list.length > CAP; var use = list.slice(0, CAP);
  var rowsList = []; var skipped = 0;
  for(var i = 0; i < use.length; i++){
    var s = use[i];
    resEl.innerHTML = '<div style="text-align:center;padding:12px 2px;font-size:11px;color:var(--text3)">종목 ' + (i+1) + '/' + use.length + ' · ' + (s.name || '') + '<br><span style="font-size:9px">캔들 수집 중 — 수십 초</span></div>';
    await _trendBatchSleep(0);
  ...
  if(rowsList.length < 3){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">유효 종목 3개 미만 — 네트워크 확인(' + mk + ')</div>'; return; }
  var RP; try { RP = SXVAL.runPool(rowsList, _sxvRuler, preds, opt); } catch(e){ resEl.innerHTML = '<div style="font-size:11px;color:#dc2626;padding:8px 2px">풀 계산 오류: ' + String((e && e.message) || e) + '</div>'; return; }
  RP._capped = capped; RP._listTotal = list.length; RP._skipped = skipped;
  resEl.innerHTML = _sxvRenderResult(RP, true);
}

// 리더보드 — SXVAL 결과 rows를 리프트 내림차순. 자 type별 컬럼(event=리드 有 / return=리드 無), 풀이면 일관성 컬럼.
function _sxvRenderResult(R, isPool){
  ...
  var dirTxt = R.ruler === 'cross' ? (R.dir === 'gc' ? '골든' : '데드') : (R.ruler === 'upmove' ? '상승' : '하락');
  var dirCol = (R.ruler === 'drawdown' || (R.ruler === 'cross' && R.dir === 'dc')) ? '#ef4444' : '#16a34a';
  ...
  head += '<div style="font-size:10px;color:var(--text2);margin-bottom:8px">기저율 <b>' + br + '%</b> · 이벤트 ' + R.totalEvent + '개 <span style="color:' + T3 + '">— 정밀이 이걸 넘어야(리프트&gt;1) 의미</span></div>';
  ...
  var note = '<div style="font-size:9px;color:' + T3 + ';margin-top:9px;border-top:1px solid var(--border);padding-top:6px;line-height:1.55"><b>정밀</b>=신호→실제발생 적중률 · <b>재현</b>=전체 이벤트 중 이 신호가 잡은 비율 · <b>리프트</b>=정밀÷기저율(<span style="color:#16a34a">✅≥1.3 유의</span>·<span style="color:#ef4444">❌&lt;0.8 역효과</span>·△중간)' + (isEvent ? ' · <b>리드</b>=신호→발생 평균봉수' : '') + (isPool ? ' · <b>일관성</b>=종목 중 신호가 기저율 상회한 비율' : '') + ' · 룩어헤드 차단(신호 rows[≤t]·타깃 미래)</div>';
...
function _buildValidatorCard(stock, indicators){
  ...
  var titleHtml = '<span style="font-size:13px;font-weight:800;color:var(--text)">🔬 범용 검증</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span>';
  var rightHtml = '<span style="font-size:10px;font-weight:700;color:' + dateCol + ';margin-left:8px">' + dateTxt + ' 기준 · ' + cfg.s + '×' + cfg.l + '</span>';
  // 자 탭
  var rulerDefs = [ { id:'cross', label:'크로스', col:'#6366f1' }, { id:'upmove', label:'상승', col:'#16a34a' }, { id:'drawdown', label:'MDD', col:'#ef4444' } ];
  ...
  // 방향(cross) / N / thr(return)
  var ctrl2 = '';
  if(_sxvRuler === 'cross'){
    var dCol = _sxvDir === 'gc' ? '#e3493b' : '#2563eb';
    ctrl2 = '<span onclick="_sxVib(10);window._sxvToggleDir&&_sxvToggleDir()" style="font-size:10px;font-weight:800;padding:4px 10px;border-radius:13px;cursor:pointer;border:1px solid ' + dCol + ';background:' + dCol + '1a;color:' + dCol + '">' + (_sxvDir === 'gc' ? '🟢 골든' : '🔴 데드') + ' ⇄</span>';
  }
  ...
  var thrBtn = (_sxvRuler === 'cross') ? '' : '<span onclick="_sxVib(10);window._sxvCycleThr&&_sxvCycleThr()" title="목표 수익/낙폭 임계(2→3→5%)" style="font-size:10px;font-weight:800;padding:4px 9px;border-radius:13px;cursor:pointer;border:1px solid var(--border);background:var(--surface2);color:var(--text2)">' + _sxvThr + '% ⟳</span>';
  var ctrl2Row = '<div style="display:flex;align-items:center;gap:5px;margin-bottom:7px;flex-wrap:wrap">' + ctrl2 + nBtn + thrBtn + '</div>';
  // 신호 멀티칩
  var predList = (typeof SXVAL !== 'undefined' && SXVAL.PREDICTORS) ? Object.keys(SXVAL.PREDICTORS) : Object.keys(_sxvPredsOn);
  var predChips = predList.map(function(id){ var on = !!_sxvPredsOn[id]; var lab = (typeof SXVAL !== 'undefined' && SXVAL.PREDICTORS[id]) ? SXVAL.PREDICTORS[id].label : id; return '<span onclick="_sxVib(6);window._sxvTogglePred&&_sxvTogglePred(\'' + id + '\')" style="font-size:9px;font-weight:700;padding:3px 8px;border-radius:11px;cursor:pointer;border:1px solid ' + (on ? '#0891b2' : 'var(--border)') + ';background:' + (on ? '#0891b21a' : 'var(--surface2)') + ';color:' + (on ? '#0891b2' : 'var(--text3)') + (on ? '' : ';opacity:.55') + '">' + lab + '</span>'; }).join('');
  ...
  // 실행 버튼
  var runBtn = '<div onclick="_sxVib(12);window._sxvRun&&_sxvRun()" style="text-align:center;font-size:12px;font-weight:800;color:#fff;background:var(--accent);border-radius:9px;padding:9px;cursor:pointer;margin-bottom:9px">⚗️ 검증 실행</div>';
  // 결과 영역
  var resultDiv = '<div id="sxValResult"><div style="font-size:10px;color:var(--text3);line-height:1.6;padding:8px 10px;background:var(--surface2);border-radius:8px">자(예측 대상)·신호·출처를 고르고 <b>검증 실행</b>을 눌러줘. 각 신호의 정밀·재현·리프트로 그 신호가 예측에 값어치 하는지(✅합격/❌역효과) 가려줘. 합격 신호만 점수 구성에 남기면 돼.</div></div>';
  var inner = tabRow + ctrl2Row + predRow + srcRow + runBtn + resultDiv;
  return _sxExpCard('sxValCardWrap', 'sxValCardWrap_b', titleHtml, rightHtml, inner);
}
if(typeof window !== 'undefined'){ window._sxvSetRuler = _sxvSetRuler; window._sxvToggleDir = _sxvToggleDir; window._sxvCycleN = _sxvCycleN; window._sxvCycleThr = _sxvCycleThr; window._sxvTogglePred = _sxvTogglePred; window._sxvSetSrc = _sxvSetSrc; window._sxvRun = _sxvRun; window._sxvRerender = _sxvRerender; window._buildValidatorCard = _buildValidatorCard; }
