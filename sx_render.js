...
function _sxBadgeRowInner(def, st){
  ...
  if(overlap>0 && inPool){ statusCol=def.color; statusTxt='🔥 발동 '+overlap+'개'+(!isPull&&overlap>=3?' · 긴호흡':''); }
  ...
  if(overlap>0 && inPool){
    var vtxt = isPull
      ? ('정배 발동 '+overlap+'개 · 적중 ~'+((overlap>=4)?68:(overlap>=3?62:61))+'%'+(overlap>=4?'(강)':''))
...
async function _snapCreate(){
  ...
  if(el) el.innerHTML='<div style="border:1px solid var(--border);border-radius:10px;padding:10px;font-size:10.5px;color:var(--text2);line-height:1.6">📦 <b>'+fname+'</b> 생성 완료 — '+String(mk).toUpperCase()+' '+n+'종 · 기준일 '+(maxDate||'?')+' · 미수록(봉수미달) '+excluded.length+'종<br>→ repo 루트에 커밋 + <b>snap_manifest.json</b>의 latest.'+mk+'를 이 파일명으로 갱신</div>';
}
...
function _sxBuilderAutoScan(scanMode){
  ...
  var passGate = (scanMode==='deadcat')
    ? ok.filter(function(r){ var spark=((r.n3!=null&&allN3!=null&&r.n3>=allN3)||(r.n5!=null&&allN5!=null&&r.n5>=allN5)); var crash=((r.n10!=null&&allN10!=null&&r.n10<allN10)||(r.hold!=null&&r.hold<0.5)); return spark&&crash; })   // [S771] 데드캣 완화: (N3▲ or N5▲ 반짝) AND (N10▼ or 유지50%- 추락)
...
function _sxBeamRun(scanMode, out){
  ...
  var gatePass=function(s){
    if(_isDC){ var spark=((s.n3!=null&&allN3!=null&&s.n3>=allN3)||(s.n5!=null&&allN5!=null&&s.n5>=allN5)); var crash=((s.n10!=null&&allN10!=null&&s.n10<allN10)||(s.hold!=null&&s.hold<0.5)); return spark&&crash; }
    return (s.n10!=null&&allN10!=null&&s.n10>=allN10+0.15) && (s.hold!=null&&s.hold>=0.60) && (s.late!=null&&allLate!=null&&s.late>=allLate+0.03);   // [S789] 진짜 강티어: N10 기저+15%p · 유지60% · 후반 기저+3%p
  };
