...
function _trendRenderInner(){
  ...
  const head=`<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:13px;font-weight:800;color:var(--text)">📈 단기 추세 매매</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span><span onclick="window._trendOpenDetail&&_trendOpenDetail()" title="거래내역·자산흐름 보기" style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;background:#7c3aed22;color:#7c3aed;border:1px solid #7c3aed66;cursor:pointer">검증</span><span style="font-size:10px;font-weight:700;color:${_dCol};margin-left:auto">${_isToday?'':'⚠️ '}${_dTxt} 종가기준 매매</span></div>`;
  // MA 입력
...
if(typeof window!=='undefined'){ window._trendApply=_trendApply; window._trendToggleAux=_trendToggleAux; window._trendToggleSell=_trendToggleSell; window._trendToggleBuyOpen=_trendToggleBuyOpen; window._trendToggleSellOpen=_trendToggleSellOpen; window._trendReset=_trendReset; }
...
function _buildTransitionCard(stock, indicators){
  ...
  const _dateLabel = _isToday ? `${_dateTxt} 종가기준 예측` : `⚠️ ${_dateTxt} 종가기준 예측`;   // [S505] 확정봉 종가 기준 명시
