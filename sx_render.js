...
function _sxvRenderResult(R, isPool){
  ...
  var rows = R.rows.slice().sort(function(a, b){ var la = a.lift == null ? -1 : a.lift, lb = b.lift == null ? -1 : b.lift; return lb - la; });
  var body = rows.map(function(r){
    ...
    return '<div style="display:flex;align-items:center;gap:6px;padding:5px 0;border-top:1px solid var(--border)' + op + '">'
...
function _buildValidatorCard(stock, indicators){
  ...
  var ctrl2Row = '<div style="display:flex;align-items:center;gap:5px;margin-bottom:7px;flex-wrap:wrap">' + nBtn + thBtns + rgBn + mixBn + '</div>';
  // 신호 멀티칩
