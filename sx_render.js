...
function _mxRunStats(rows, cfg, dir, N){
  ...
  const preds = [
    { key:'btc',   label:'btc≤3 (투영 3봉내 교차)', tier:1, test: c => c.btc != null && c.btc <= 3 },
    ...
    { key:'vol',   label:'거래량 1.2배↑',            tier:2, test: c => c.vr != null && c.vr > 1.2 },
  ];
  const combos = [
    { key:'all1',    label:'① 1차 전체 AND',  test: (c, p) => p.btc && p.conv && p.near && p.slope },
    { key:'all1vol', label:'① 1차 + 거래량',  test: (c, p) => p.btc && p.conv && p.near && p.slope && p.vol },
    { key:'btcslp',  label:'btc≤3 & 기울기',  test: (c, p) => p.btc && p.slope },
  ];
  const stat = {};
  ...
  for(let i = warm; i <= len - 1 - N; i++){
    ...
    const ctx = { gap, conv, btc, sUp, nearPct, vr };
...
function _buildMaCrossCard(stock, indicators){
  ...
  const head = '<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:13px;font-weight:800;color:var(--text)">🧪 MA크로스 전이</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span>' + _dirBtn + '<span style="font-size:10px;font-weight:700;color:' + _dateCol + ';margin-left:auto">' + _dateTxt + ' 기준</span></div>';
  const wrap = inner => '<div id="sxMxCardWrap" style="margin:8px 10px;padding:12px 14px;border-radius:12px;background:var(--surface);border:1px solid var(--border)">' + head + inner + '</div>';
...
if(typeof window !== 'undefined'){ window._mxToggleDir = _mxToggleDir; window._mxRerenderCard = _mxRerenderCard; window._buildMaCrossCard = _buildMaCrossCard; }
