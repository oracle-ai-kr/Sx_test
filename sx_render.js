// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S805 · build 20260629-0438 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _btGateAuditBracket(mk, source, onProgress){
  ...
  for(var i=0;i<use.length;i++){
    ...
    for(var bi=250; bi<rows.length-10; bi++){
      ...
      var rec={ n3:_nRet(3), n5:_n5, n10:_n10, n15:_nRet(15), n20:_nRet(20), hold:((_n5===null||_n10===null)?null:(_n5===true&&_n10===true)), maBull:maBull, volOsc:volOsc };   // [S802]N15·N20 [S803]maBull·volOsc(2×2 교차)
...
// ════════ [S803] 🔬 maAlignBull × 거래량OSC 2×2 교차 — 방향(maAlignBull)과 변별(거래량OSC<-39.6 매도소진)을 동시에. 강세+OSC소진(좌상)=진짜반등 최강 후보. N10 종가>진입가 기준. ════════
function _btGateAuditComboRender(weak, strong, _rate, GRN, RED, AMB, T2, T3){
  ...
  var sbLo=_rate(grp.sb_lo,'n10'), sbHi=_rate(grp.sb_hi,'n10'), wkLo=_rate(grp.wk_lo,'n10'), wkHi=_rate(grp.wk_hi,'n10');
  var base=_rate(all.filter(function(r){return typeof r.volOsc==='number';}),'n10');
  var _c=function(r){ return r ? ('<b>'+r.rate+'%</b><br><span style="font-size:7.5px;color:'+T3+';font-weight:500">n'+r.n+'</span>') : '<span style="color:'+T3+'">—</span>'; };
  var hdr='<div style="display:flex;font-size:8.5px;font-weight:700;color:'+T3+';padding-bottom:3px"><span style="flex:1">N10 종가&gt;진입가</span><span style="width:72px;text-align:right">OSC&lt;-39.6<br><span style="font-weight:500;color:'+GRN+'">매도소진</span></span><span style="width:72px;text-align:right">OSC≥-39.6<br><span style="font-weight:500">약함</span></span></div>';
  var _row=function(lab, lo, hi, col){ return '<div style="display:flex;align-items:center;font-size:10px;padding:6px 0;border-top:1px solid var(--border)"><span style="flex:1;font-weight:800;color:'+col+'">'+lab+'</span><span style="width:72px;text-align:right;color:'+col+'">'+_c(lo)+'</span><span style="width:72px;text-align:right;color:'+col+'">'+_c(hi)+'</span></div>'; };
  var table = hdr + _row('🟢 강세(통과)', sbLo, sbHi, GRN) + _row('🔴 약세(차단)', wkLo, wkHi, RED);
  var _d=function(a,b){ return (a&&b)?(a.rate-b.rate):null; }, _sp=function(d){ return d==null?'—':((d>=0?'+':'')+d+'%p'); };
  var oscS=_d(sbLo,sbHi), oscW=_d(wkLo,wkHi);     // 거래량OSC 효과(소진−약함) 행별 = 변별력
  var maL=_d(sbLo,wkLo), maH=_d(sbHi,wkHi);       // maAlignBull 효과(강−약) 열별 = 방향
  var champ=sbLo, champLift=(champ&&base)?(champ.rate-base.rate):null;
  var vcol, vtxt, vsum;
  if(champ && champLift!=null && champLift>=10 && oscS!=null && oscS>=8){
    vcol=GRN; vtxt='✅ 강세+OSC소진 = 데드캣 회피 조합';
    vsum='최강셀 '+champ.rate+'% (기저 '+(base?base.rate:'?')+'% · +'+champLift+'%p). 거래량OSC 소진이 강세행 '+_sp(oscS)+'·약세행 '+_sp(oscW)+'로 양쪽 다 끌어올림 — maAlignBull 단독(±3%p)보다 강한 변별. 조합 게이트 후보.';
  } else if(champ && oscS!=null && oscS>=5){
    vcol=AMB; vtxt='△ 거래량OSC 변별력 있음 (조합 약하게 유효)';
    vsum='강세+OSC소진 '+champ.rate+'% (기저 +'+(champLift!=null?champLift:'?')+'%p). 거래량OSC 효과 강세행 '+_sp(oscS)+'·약세행 '+_sp(oscW)+' — maAlignBull보단 낫지만 격차 보통.';
  } else {
    vcol=AMB; vtxt='⚠️ 조합도 변별 약함';
    vsum='강세+OSC소진 '+(champ?champ.rate+'%':'—')+'. 거래량OSC 효과 강세행 '+_sp(oscS)+'·약세행 '+_sp(oscW)+' — 게이트 측정(종가>진입가 비율)에선 OSC도 크게 못 가름. 변별력 측정의 진짜/가짜 분류(후반평균 IC)와는 다른 지표라 결과 다를 수 있음.';
  }
  ...
  var desc='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:6px">역배열 진입을 <b>maAlignBull(방향)</b> × <b>거래량OSC&lt;-39.6(변별)</b> 2×2 교차 — <b style="color:'+GRN+'">행</b>=추세정렬 효과, <b style="color:'+GRN+'">열</b>=매도소진 효과. <b style="color:'+GRN+'">강세+OSC소진</b>(좌상)이 진짜반등 최강 후보.'+(noOsc?(' <span style="color:'+AMB+'">OSC미산출 '+noOsc+'개 제외</span>.'):'')+'</div>';
  var note='<div style="font-size:8px;color:'+T3+';margin-top:7px;line-height:1.45">OSC소진=거래량OSC&lt;-39.6(매도 소진=진짜반등 신호) · 기저율 '+(base?base.rate+'%':'—')+' · N10 종가&gt;진입가 비율 · BT무관. 거래량OSC&lt;-39.6은 변별력 측정서 진짜반등 77% 단일 최강 변별자였음.</div>';
