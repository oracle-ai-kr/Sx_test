...
function _trendRenderInner(){
  ...
  // [S750] 매수/매도 섹션 분리 — 실험 칩을 각 섹션 아래로 이동(매수=재진입·조기진입 / 매도=조기청산·기울기청산·재앙손절). 🔮크로스 예측 칩은 제거.
  const chipBuy=_section('🟢 매수 조건 · 모두 충족(AND)','#2563eb',_buyCnt,_trendBuyOpen,'_trendToggleBuyOpen',buyChips);
  const chipSell=_section('🔴 매도 조건 · 하나라도(OR) · 데드크로스에 추가','#e8365a',_sellCnt,_trendSellOpen,'_trendToggleSellOpen',sellChips);
  ...
  return head+`<div id="sxTrendBody" style="display:${_trOpen?'block':'none'}" data-loaded="1">`+maRow+chipBuy+reChip+entryRow+predRow+chipSell+exitRow+_guardBadge+stateRow+grid+note+batchSection+`</div>`;   // [S699] 본문 접기 래퍼(data-loaded=1) · [S750] 매수섹션+진입계열 / 매도섹션+청산계열 · [S751] 🔮크로스 예측 복구(진입계열 다음·매도섹션 앞 = 원위치)
}
