...
function _sxbCircle(score, label, big, neutral, delta, inverse){
  ...
  const fs = big?20:13;
  // [S362] 점수 숫자 색: inverse=값 기준(과열 빨/안정 녹), 일반=델타 기준(전이 색)
...
function _sxbHTML(){
  ...
  groups.forEach(g=>{
    ...
    g.items.forEach(it=>{ det += _sxbCircle(it.v, it.k, false, !!it.neutral, it.d, !!it.inverse); });
    det += `</div></div>`;
  });
  det += `</div>`;
  return h + mini + det;
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  ...
  if(extras.adx && extras.adx.v!=null) g1.push({k:'추세강도', v:extras.adx.v, d:extras.adx.d});           // [S362]
  ...
  if(extras.ad && extras.ad.v!=null) g3.push({k:'A/D', v:extras.ad.v, d:extras.ad.d});                   // [S362]
  if(extras.eom && extras.eom.v!=null) g3.push({k:'EOM', v:extras.eom.v, d:extras.eom.d});               // [S362]
  if(extras.chaikin && extras.chaikin.v!=null) g3.push({k:'Chaikin', v:extras.chaikin.v, d:extras.chaikin.d}); // [S362]
