...
function _cvPoolRender(rep, watch, mk, abMode){
  ...
  if((rep&&rep.ok&&rep.gate)||(watch&&watch.ok&&watch.gate)){
    ...
    var gateRow=function(P,lbl){
      ...
      var g=P.gate;
      return '<div style="font-size:9.5px;margin-top:5px;line-height:1.55"><b style="color:'+PUR+'">'+lbl+'</b><br>　🐻'+gcell('역배열',g.bear)+saSub(g.bearSaO,'🔼단기정배열5>20>60')+saSub(g.bearSaX,'🔽단기비정배열')+'<br>　🔘'+gcell('중립',g.off)+'<br>　🔵'+gcell('정배열',g.bull)+'</div>';
    };
    out+=gateRow(rep,'대표');
