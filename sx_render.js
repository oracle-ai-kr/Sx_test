// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S825 · build 20260630-1950 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _deadcatShortBracket(mk, onProgress){
  ...
  try{ watch=(typeof _getWatchlist==='function'?(_getWatchlist(mk)||[]):[]).filter(function(s){return s&&s.code;}).map(function(s){return {code:s.code,name:s.name||s.code};}); }catch(_){}
  var seen={}, list=[];
  rep.concat(watch).forEach(function(s){ if(s&&s.code&&!seen[s.code]){ seen[s.code]=1; list.push(s); } });   // 대표+관심(저변동보강 미사용=변동성 축 아님)
  if(!list.length) return { ok:false, reason:'종목 없음' };
  var CAP=45, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
