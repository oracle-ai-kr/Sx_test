...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  for(var i=0;i<use.length;i++){
    ...
    var R=null;
    try{ R=SXVVAL.run(rows,'day',mk,{raw:true, target:40, ablate:true}); }catch(e2){}   // target 40 · [S721] ablate=재료 분해 기록
...
async function _cvPoolRun(){
  ...
  el.innerHTML=_cvPoolRender(rep, watch, mk, _abMode, _gateMode);
}
function _cvPoolRender(rep, watch, mk, abMode, gateMode){
  ...
  out+='<div style="font-size:11px;font-weight:800;color:'+PUR+';margin-bottom:2px">🔬 점수·등급 풀링 · '+String(mk).toUpperCase()+(abMode?' · <span style="color:#e3493b">🧪 MACD전환 빼고</span>':'')+(gateMode?'':' · <span style="color:#e3493b">🚪 약세게이트 OFF(비교)</span>')+'</div>';
...
if(typeof window!=='undefined'){ window._cvToggle=_cvToggle; window._cvRunValidation=_cvRunValidation; window._cvRender=_cvRender; window._cvPoolRun=_cvPoolRun; window._cvMacdAbToggle=_cvMacdAbToggle; window._cvBearGateToggle=_cvBearGateToggle; }
