...
async function _btDiscrimBracket(mk, source, onProgress){
  ...
  if(!list.length) return { ok:false, reason:(source==='watch'?'관심목록 비어있음':(source==='all'?'확장 풀 비어있음':'대표목록 없음')) };
  var CAP=(source==='all')?130:20, use=list.slice(0,CAP);   // [S833] 확장 풀은 130종(측정 도구와 동일)
...
function _sxBeamRun(scanMode, out){
  ...
  if(typeof window!=='undefined'){
    window._sxAutoScanPass={ tool:'beam', scanMode:scanMode, pool:DP.key, poolName:DP.poolName, mode:'and', exhaustive:exhaustive,
...
function _sxAutoScanDownload(){
  ...
  try{
    ...
    a.href=url; a.download='sx_autoscan_'+(d.pool||'pool')+'_'+Date.now()+'.json';
