  ...
  if(btSt.state === 'holding'){
    ...
    let reasonBadge = '', reasonLine = '';
    {
      ...
      if(_actHold){
        ...
        // [S360] 모멘텀 배지 캡슐 — 배너에서 이관. 기존 label 텍스트 대신 캡슐(아이콘+색)로 표시.
        //   배너 _momBadgeHTML과 동일 형태(✨상승 시작/⚠하락 시작/➡횡보). 향후 위치는 지켜본 뒤 확정.
        const _mb = svVerdict.momBadge;
        let _momCapsule = '';
        if(_mb && _mb.icon && _mb.label){
          _momCapsule = `<span style="display:inline-flex;align-items:center;gap:2px;margin-right:5px;padding:1px 6px;font-size:9px;font-weight:700;color:${_mb.color};background:${_mb.color}1A;border:1px solid ${_mb.color};border-radius:9px;letter-spacing:-.2px;vertical-align:middle">${_mb.icon} ${_mb.label}</span>`;
        }
        if(_reasonTxt || _momCapsule) reasonLine = `<div style="${SUB_STYLE};color:${_actClr}">└ ${_momCapsule}${_reasonTxt}</div>`;
      }
    }
...
function _sxbBuildWhyMsg(it){
  ...
  if(it.signal === 'up'){
    if(v >= 65){
      const fired = (it.fired || []).filter(x=>x).join(', ');
      return fired ? `✨ ${k} ${v}점: ${fired}` : `✨ ${k} ${v}점 발동`;
    }
  ...
  // 인버스 (구조위치) — 값 자체 색
  if(it.inverse){
    if(v >= 70) return `🔴 ${k} ${v}% — 20봉 고가권 (과열 주의)`;
    if(v <= 30) return `🟢 ${k} ${v}% — 20봉 저가권 (안정/매수 우위)`;
    return `${k} ${v}% — 중립 구간`;
  }
...
function _sxbHTML(){
  ...
  // [S376] ▲▼ 카운터 기반 5단계 분류 (기존 3단계 → 강도까지 표현)
  let distDir, bi, bl, bc;
  if(diff >= 4){       distDir='strongUp';   bi='🚀'; bl='강한상승'; bc='var(--buy)'; }
  else if(diff >= 2){  distDir='up';         bi='✨'; bl='상승조짐'; bc='var(--buy)'; }
  else if(diff >= -1){ distDir='flat';       bi='➡'; bl='방향혼조'; bc='#94a3b8'; }
  else if(diff >= -3){ distDir='down';       bi='⚠'; bl='하락조짐'; bc='#f59e0b'; }
  else {               distDir='strongDown'; bi='🔻'; bl='강한하락'; bc='var(--sell)'; }
  const tone = _sxbTone(total);
  // [S376] 전이 방향 배지 (5단계 라벨)
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  ...
  if(extras.gc && extras.gc.v!=null) g5.push({k:'골든크로스', v:extras.gc.v, neutral:true, signal:'up', fired:extras.gc._fired||[]});
  if(extras.dc && extras.dc.v!=null) g5.push({k:'데드크로스', v:extras.dc.v, neutral:true, signal:'down', fired:extras.dc._fired||[]});
  window._sxBoard = { verdict: verdict||null, lowConf: !!lowConf, lowConfReasons: (extras && extras._lowConfReasons) || [], groups:[
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    ...
    // [S358] 진입분석 카드(entryCardHTML/_buildAEntryLine) 제거 — 엔진시뮬 카드와 정보 중복.
    //   R1/S1은 _buildR1S1Card로 분리되어 미니차트 아래 배치됨.
  }
  ...
  try {
    ...
    _boardExtras.gc = {v: _gcW === 0 ? 0 : Math.round(65 + (_gcW / 18) * 35), _fired: _gcFired.slice()};
    _boardExtras.dc = {v: _dcW === 0 ? 0 : Math.round(65 + (_dcW / 18) * 35), _fired: _dcFired.slice()};
