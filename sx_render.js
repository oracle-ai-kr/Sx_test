...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  if(SXVVAL.ablateIC){ try{
    ...
    P.ablateUp = SXVVAL.ablateIC(records.filter(function(r){ return r.path==='trend'; }), 'up'); // 추가상승 (추세경로 봉만 — upside는 trend경로서만 쓰임)
    P.ablateTr = SXVVAL.ablateIC(records, 'tr');                                           // 추세방향 (전체봉·양 경로서 쓰임)
  }catch(_e){} }   // [S721/722] 3축 재료 ablation IC
...
function _cvPoolRender(rep, watch, mk){
  ...
  function bracket(P, label){
    ...
    // [S721/722] 재료 ablation IC — 축 재료 빼면 그 축 IC ΔIC (전체/강세봉/약세봉). ready=반등신호·up=추가상승(추세경로)·tr=추세방향.
    ...
    function abTbl(ab, note){
      ...
      t+='<div style="font-size:8px;color:'+T3+';line-height:1.5;margin-bottom:3px">'+(note||'')+'재료 빼면 '+ab.label+' IC가 어떻게? baseline IC <b>'+sg(ab.all.base)+'</b> · <b style="color:'+RED+'">Δ&gt;0=빼면 IC↑(노이즈/해로움)</b> · <b style="color:'+GRN+'">Δ&lt;0=기여</b> · 익일</div>';
