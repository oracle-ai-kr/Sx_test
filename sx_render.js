// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S820 · build 20260630-1720 cache-bust]
// ════════════════════════════════════════════════════════════
...
function _trendDefaults(market){
  ...
  return { s:d[0], l:d[1], n:3,
    ...
    xCross:false, xs:d[0], xl:d[1],   // [S820] 청산 크로스 분리 — 진입(s×l) ≠ 청산(xs×xl). OFF면 청산도 진입 MA.
...
function _trendRenderInner(){
  ...
  const _section=(title,color,cnt,open,toggleFn,chips,extra)=>{   // [S752] extra = 섹션 body 안에서 함께 접히는 실험 칩
    const arrow=open?'▼':'▶';
  ...
  const chipBuy=_section('🟢 매수 조건 · 모두 충족(AND)','#2563eb',_buyCnt,_trendBuyOpen,'_trendToggleBuyOpen',buyChips,reChip+entryRow);
  const chipSell=_section('🔴 매도 조건 · 하나라도(OR) · 데드크로스에 추가','#e8365a',_sellCnt,_trendSellOpen,'_trendToggleSellOpen',sellChips,exitRow);
  // 상태
...
function _trendBatchConfigs(market, mode){
  ...
  const base=function(){ return { s:sN, l:lN, n:3, aux:{}, sell:{}, reentry:false, predict:false, predLead:1, nextOpen:false, earlyMa5:false, earlySlope:false, disSl:false, slAtr:2, entrySlope:false, entryConfirm:true, xCross:false, xs:sN, xl:lN }; };
