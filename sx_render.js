...
function _candleTransitionScore(rows, indicators){
  ...
  const mode = redCnt >= 2 ? 'rebound' : greenCnt >= 2 ? 'reversal' : null;
  ...
  // C1 추세 맥락 — 20MA 기울기 + 60일 고저권 (모드별 의미 반전)
...
function _buildTransitionCard(stock, indicators){
  ...
  const top = `<div style="display:flex;align-items:baseline;gap:8px"><span style="font-size:26px;font-weight:800;color:${col}">${sign}${r.score}</span><span style="font-size:13px;font-weight:700;color:${col}">${r.label}</span><span style="font-size:10px;color:var(--text3);margin-left:auto">${r.mode==='rebound'?'음봉':'양봉'} ${r.streak}연속</span></div>`;
