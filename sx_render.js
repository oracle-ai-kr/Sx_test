...
async function _btDiscrimBracket(mk, source, onProgress){
  ...
  var _OUT_HORIZON=Math.max(10, _LATE_TO, _SURV_TO);   // 루프 tail 가드 — 미래봉 부족 방지
  ...
  for(var i=0;i<use.length;i++){
    ...
    for(var bi=250; bi<rows.length-_OUT_HORIZON; bi+=_STEP){
      ...
      entries.push({ f:f, n3:_n3, n5:_n5, n10:_n10, hold:_hold, retN10:_retN10, retLate:_retLate, surv:_surv });
      _used=true;
...
function _sxBeamRun(scanMode, out){
  ...
  var statOf=function(conds){ var m=uni.filter(function(e){return _condMatch733(e.f, conds, 'and');}); var ag=_dcContAgg(m); return {n:m.length, late:ag.late, surv:ag.surv, n3:_rate(m,'n3'), n5:_rate(m,'n5'), n10:_rate(m,'n10'), hold:_rate(m,'hold')}; };
  var gatePass=function(s){
    if(_isDC){ var spark=((s.n3!=null&&allN3!=null&&s.n3>=allN3)||(s.n5!=null&&allN5!=null&&s.n5>=allN5)); var crash=((s.n10!=null&&allN10!=null&&s.n10<allN10)||(s.hold!=null&&s.hold<(allHold!=null?allHold+0.08:0.5))); return spark&&crash; }   // [S848] 유지 베이스+8%p 상대화
    return (s.n10!=null&&allN10!=null&&s.n10>=allN10+0.15) && (s.hold!=null&&s.hold>=(allHold!=null?allHold+0.15:0.60)) && (s.late!=null&&allLate!=null&&s.late>=allLate+0.03);   // [S789] 진짜 강티어 · [S848] 유지 절대치→베이스 상대(+15%p): kr 57.4%·us 54.2%(기존 60%와 동등=재실행 불필요) · coin 42~44%(도달 가능 — 절대 60%는 코인 유지베이스 27~29%에선 +31%p 요구=통과 불가였음)
  };
  var dirOK=function(s){ var d=_dcFitGrade(s.late, s.surv, s.n, allLate).dir; return _isDC ? d<0 : d>0; };
  var collect=function(conds, keys, depth, s){ if(depth>=2 && gatePass(s) && dirOK(s)){ results.push({conds:conds, keys:keys, depth:depth, n:s.n, late:s.late, surv:s.surv, n3:s.n3, n5:s.n5, n10:s.n10, hold:s.hold}); } };
  ...
  results.sort(function(a,b){ return _isDC ? ((a.late==null?9:a.late)-(b.late==null?9:b.late)) : ((b.late==null?-9:b.late)-(a.late==null?-9:a.late)); });

  // JSON 저장 — 각 조합의 전체 conds 포함(자체완결, bool→bin 변환). 💾 다운로드 그대로 사용.
  if(typeof window!=='undefined'){
    window._sxAutoScanPass={ tool:'beam', mkt:((typeof currentMarket!=='undefined')?currentMarket:'kr'), snap:((typeof _snapInfo==='function')?_snapInfo():null), scanMode:scanMode, pool:DP.key, poolName:DP.poolName, mode:'and', exhaustive:exhaustive,
      ...
      baseStats:{allN3:allN3,allN5:allN5,allN10:allN10,allHold:allHold,allLate:allLate, tests:tests, capped:capped},
      passed: results.map(function(r){ return { conds:r.conds.map(function(c){return {key:c.key,type:(c.type==='num'?'num':'bin'),dir:(c.type==='num'?c.dir:null),th:(c.type==='num'?c.th:null),label:c.label};}),
        n:r.n, n3:r.n3, n5:r.n5, n10:r.n10, hold:r.hold, late:r.late, surv:r.surv, fit:_dcFitGrade(r.late,r.surv,r.n,allLate).label, depth:r.depth }; }),
  ...
  var h='<div style="font-size:9px;color:'+T3+';margin-bottom:5px;line-height:1.4">🌳 <b style="color:#15803d">'+seedLbl+'</b> · 탐색 '+tests+'조합'+(capped?' <b style="color:'+RED+'">(상한 '+MAXTEST+' 도달 — 일부만)</b>':'')+' · 전체기저 후반 '+(allLate!=null?((allLate>=0?'+':'')+(100*allLate).toFixed(1)+'%'):'-')+'</div>';
  if(!results.length){ h+='<div style="font-size:9.5px;color:'+T3+';padding:8px 6px;line-height:1.5">'+passLbl+' 조합 없음 — '+(exhaustive?'이 풀에서 방향일치+게이트 통과 조합이 안 나왔어요.':'씨앗을 바꾸거나 무체크(전수)로 돌려보세요.')+'</div>'; }
    ...
    for(var k=0;k<DISP;k++){ var r=results[k]; var _fit=_dcFitGrade(r.late,r.surv,r.n,allLate);
      var firstTxt=(r.late!=null)?('후반 '+(r.late>=0?'+':'')+(100*r.late).toFixed(1)+'%'):'후반 —';
      var firstCol=(r.late==null)?T3:(r.late>0.005?GRN:r.late<-0.005?RED:T3);
...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  for(var i=0;i<use.length;i++){
    ...
    var R=null;
    try{ R=SXVVAL.run(rows,'day',mk,{raw:true, target:40, ablate:true, recipeSig:_rcpCb}); }catch(e2){}   // target 40 · [S721] ablate=재료 분해 기록
...
async function _cvPoolRun(){
  ...
  _sxMeasStash('pool_'+mk+'_'+(_rcpMode?'recipeON':'recipeOFF'), { mk:mk, rcpMode:_rcpMode, abMode:_abMode, gateMode:_gateMode, rep:rep, watch:watch });   // [S860]
...
function _cvPoolRender(rep, watch, mk, abMode, gateMode, rcpMode){
  ...
  out+='<div style="font-size:11px;font-weight:800;color:'+PUR+';margin-bottom:2px">🔬 점수·등급 풀링 · '+String(mk).toUpperCase()+(abMode?' · <span style="color:#e3493b">🧪 MACD전환 빼고</span>':'')+(rcpMode?' · <span style="color:#16a34a">🍳 레시피 반등표</span>':'')+(gateMode?'':' · <span style="color:#e3493b">🚪 약세게이트 OFF(비교)</span>')+'</div>';
