  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // S103-fix6c Phase2: 전이 확률 라벨 양방향 + 방향 전환 경보 배지
        //   Layer 1(정적): 라벨 자체를 모드에 맞춰 "강세 전이" ↔ "약세 전이"로 치환
        //   Layer 2(동적): 역방향 전이 확률 임계 넘으면 [하락전이]/[반등전이] 배지 추가
        //   임계값 r2e(ready→entry): 매수<35 / 매도>=45  (r2eBase=30, 분포 30~80 기준)
        //   임계값 e2t(entry→trend): 매수<25 / 매도>=55  (e2tBase=25, 기본값 낮아서 임계 하향 + 매도 오탐 차단)
        const _r2eLabel = _isSellMode ? '약세 전이' : '강세 전이';
        const _e2tLabel = '추세 전이'; // 양방향 중립 유지
        const _txBadge = function(prob, downTh, upTh){
          // prob: 정방향 전이 확률
          // downTh: 매수 모드에서 이 값 미만이면 [하락전이]
          // upTh: 매도 모드에서 이 값 이상이면 [반등전이]
          if(prob == null) return '';
          if(_isSellMode){
            return prob >= upTh ? '<span class="tx-badge up">반등전이</span>' : '';
          } else {
            return prob < downTh ? '<span class="tx-badge down">하락전이</span>' : '';
          }
        };
        ...
        ${(()=>{
          ...
          html += '<span style="font-weight:700;color:var(--accent)">'+_r2eLabel+' '+r2e.prob+'%</span>'+_txBadge(r2e.prob, 35, 45)+' · '+r2e.days;
          ...
          html += '<div style="margin-top:4px;padding:4px 8px;background:var(--surface);border-radius:6px;font-size:9px;line-height:1.5">';
          html += '<span style="font-weight:700;color:var(--accent)">'+_e2tLabel+' '+e2t.prob+'%</span>'+_txBadge(e2t.prob, 25, 55)+' · '+e2t.days;
