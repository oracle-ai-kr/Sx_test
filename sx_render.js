...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  // S96: 매매이력 가계부 형태 (매수행+매도행)
  let tradeHistHTML = '';
  const _btTrades = stock._btResult?.trades;
  if(_btTrades && _btTrades.length){
    const _fmtDKR = d => { if(!d) return '—'; const p=d.split(/[-T]/); if(p.length<3) return d.slice(0,10); return p[0]+'.'+parseInt(p[1])+'.'+parseInt(p[2]); };
    ...
    _recent.forEach(tr => {
      ...
      _listRows += `<div style="padding:5px 0;border-bottom:1px solid var(--border)">
    ...
    tradeHistHTML = `<div style="margin:0 0 8px;padding:10px 12px;background:var(--surface);border:1px solid var(--border);border-radius:10px">
      ...
      ${_listRows}
      ${_btTrades.length > 10 ? `<div style="font-size:9px;color:var(--text3);text-align:center;margin-top:6px">최근 10건 표시 (전체 ${_btTrades.length}건)</div>` : ''}
    </div>`;
  }
