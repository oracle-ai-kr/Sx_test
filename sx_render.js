...
function _dpMeta(){
  ...
  return { key:'deadcat', lt:'bear', poolName:'역배열+단기약세', short:'데드캣', good:'진짜반등', bad:'데드캣', goodShort:'진짜', badShort:'데드', icon:'🔴' };
}
...
function _btGateAuditRender(rep, watch, mk){
  ...
  if(wN10 && sN10){
    ...
    var vcol, vtxt, vsum;
    if(dN10>=10){ vcol=GRN; vtxt='✅ 게이트 B 정당'; vsum='강세(통과)가 약세(차단)보다 N10 +'+dN10+'%p 높음 — maAlignBull이 데드캣을 실제로 거름. 약세 차단·강세 통과가 맞음.'; }
    else if(dN10>=4){ vcol=AMB; vtxt='△ 게이트 B 약하게 정당'; vsum='강세가 약세보다 N10 +'+dN10+'%p — 방향은 맞으나 격차 작음. maAlignBull의 데드캣 변별력이 약함.'; }
    else if(dN10>-4){ vcol=AMB; vtxt='⚠️ 게이트 B 근거 불충분'; vsum='강세·약세 N10 차이 '+(dN10>=0?'+':'')+dN10+'%p로 거의 없음 — maAlignBull이 데드캣을 못 가름. 분기 근거 약함.'; }
  ...
  var desc='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:6px">C 게이트 B = <b>역배열+단기약세→반등차단</b>. 가정: <b>maAlignBull(단기정배열)이 데드캣을 거른다</b>. 역배열 진입을 maAlignBull로 갈라 N10 비교 — <b style="color:'+GRN+'">강세가 약세보다 높으면 게이트 정당</b>. 대표+관심 합산 · <b style="color:'+GRN+'">BT 무관 순수 데이터</b>.</div>';
...
function _dcFitGrade(late, surv, n, baseLate, isDeadcat){   // [S777] 레시피 적합도 등급 — 목표방향(반등=후반평균+/데드캣=후반평균−)으로 크기+생존+표본을 한 등급(강/중/약)으로. 스캔 트리아지·다운로드 판단용.
  if(late==null) return {g:'-', col:'var(--text3)', score:-9};
  var dir = isDeadcat ? -1 : 1;
  var goalAbs  = dir * late;                                       // 목표방향 절대(부호 맞아야 +)
  var goalLift = dir * (late - (baseLate!=null?baseLate:0));       // 기저 대비 목표방향 우위
  var survGoal = (surv==null) ? null : (isDeadcat ? (1-surv) : surv);   // 데드캣=생존↓ 좋음
  var nOk=(n>=30), nMid=(n>=15);
  var score = goalLift - (nOk?0:(nMid?0.01:0.03));                 // 트리아지 정렬용
  if(nOk && goalAbs>0 && goalLift>=0.02 && (survGoal==null||survGoal>=0.5)) return {g:'강', col:'#16a34a', score:score};
  if(nMid && goalAbs>0 && goalLift>0) return {g:'중', col:'#d97706', score:score};
  return {g:'약', col:'var(--text3)', score:score};
}
...
function _sxBuilderAutoScan(scanMode){
  ...
  if(scanMode==='deadcat'){ results.sort(function(a,b){ return ((b.n5||0)-(b.n10||0))-((a.n5||0)-(a.n10||0)); }); }   // [S770] 데드캣 = N5−N10 추락폭 큰 순
  else { results.sort(function(a,b){ return (b.n10||0)-(a.n10||0); }); }
  var ok=results.filter(function(r){return r.n>=30;}), low=results.filter(function(r){return r.n<30;});
  var pass = (scanMode==='deadcat')
  ...
  var _row=function(r, dim){
    var _fit=_dcFitGrade(r.late, r.surv, r.n, allLate, _isDC);   // [S777] 목표방향 적합도(전체기저 대비)
    var firstTxt, firstCol, n10c;
    if(_isDC){
      firstTxt='N5 '+(r.n5!=null?Math.round(100*r.n5)+'%':'-');
      firstCol=(r.n5!=null&&allN5!=null&&r.n5>allN5)?RED:T3;
      n10c=(r.n10!=null&&allN10!=null&&r.n10<allN10)?RED:'var(--text)';
    } else {
      firstTxt=(r.dN10!=null)?((r.dN10>=0?'▲+':'▼')+Math.round(100*r.dN10)+'p'):'';
      firstCol=(r.dN10==null)?T3:(r.dN10>0.02?GRN:r.dN10<-0.02?RED:T3);
      n10c=(r.n10!=null&&baseN10!=null&&r.n10>baseN10)?GRN:'var(--text)';
    }
    ...
    return '<div style="display:flex;align-items:center;gap:5px;padding:5px 7px;margin:2px 0;background:var(--surface2);border-radius:6px'+(dim?';opacity:.5':'')+'">'
      +'<span style="color:'+_fit.col+';font-size:11px;flex-shrink:0" title="적합도 '+_fit.g+'">●</span>'
      +'<span style="flex:1;font-size:10px;font-weight:700;color:var(--text);min-width:0">+ '+r.label+condTxt+'</span>'
      +'<span style="font-size:8px;color:'+firstCol+';font-weight:800;width:48px;text-align:right">'+firstTxt+'</span>'
  ...
  if(!results.length){ h+='<div style="font-size:9.5px;color:'+T3+';padding:6px">후보 없음 (모든 재료가 이미 체크됨).</div>'; }
  else if(!pass.length){ h+='<div style="font-size:9.5px;color:'+T3+';padding:8px 6px;line-height:1.5">'+passLbl+' 없음 <span style="font-size:8px">('+passCond+')</span><br>표본 30+ 중 미해당 '+okFail+'개 · 표본부족 '+low.length+'개.'+(ok.length?(_isDC?' 데드캣 패턴(반짝 후 추락)이 없어요 — 베이스를 바꿔보세요.':' 베이스보다 N10 올리는 후보가 없어요 — 베이스 조건을 바꿔보세요.'):'')+'</div>'; }
  else {
    ...
    h+='<div style="font-size:8px;color:'+T3+';margin:5px 0 0">그 외: 표본30+ 미해당 '+okFail+'개 · 표본부족 '+low.length+'개 (숨김)</div>';
  }
  h+='<div style="font-size:8px;color:'+T3+';margin-top:6px;line-height:1.4">'+(_isDC?('데드캣=N3나 N5는 기저↑(반짝)인데 N10은 기저↓ 또는 유지50%-(추락). <b style="color:'+RED+'">N5 빨강+N10 빨강</b>=전형적. 전체기저 N5 '+(allN5!=null?Math.round(100*allN5)+'%':'-')+'·N10 '+(allN10!=null?Math.round(100*allN10)+'%':'-')+'.'):('합격=베이스보다 N10 오르고(▲)+유지 50%+ · 표본 30+. 좋은 후보 <b>체크→베이스 추가→재탐색</b>.'))+' <b style="color:'+passCol+'">💾로 JSON 저장</b> <span style="font-size:8px">(후반평균·적합도 포함)</span>.</div>';
  h+='<div style="font-size:8px;color:'+T3+';margin-top:3px;line-height:1.45">● 적합도(목표방향 후반평균+생존): <b style="color:#16a34a">●강</b>=후반평균 기저+2%p↑·생존부합·표본30+ · <b style="color:#d97706">●중</b>=엣지 약함 · <b style="color:var(--text3)">●약</b>=엣지없음/방향반대. <b style="color:#16a34a">●강</b> 있는 스캔만 저장·전송해도 충분.</div>';
  out.innerHTML=h;
}
...
function _btEnsembleRender(entries){
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:6px">서로 다른 유형의 '+DP.good+'을 3경로로 포착 → OR 합집합. <b>AND와 달리 표본 유지하며 정밀도 오르나</b> 검증. 경로별 정밀도('+DP.goodShort+'%)·커버리지(전체 중 비율), 합집합, 겹침. <span style="color:'+AMB+'">※경로 정의는 데드캣 기준 — 눌림목엔 탐색용</span>.</div>';
  // 경로 정의
...
function _btBuilderUI(){
  ...
  h+='<div style="border-top:1px dashed var(--border);margin-top:10px;padding-top:8px"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:4px"><button onclick="_sxBuilderAutoScan(\'rebound\')" style="font-size:10px;font-weight:800;padding:6px 11px;border-radius:8px;border:1px solid #0ea5e9;background:#0ea5e9;color:#fff;cursor:pointer">🔍 진짜반등</button><button onclick="_sxBuilderAutoScan(\'deadcat\')" style="font-size:10px;font-weight:800;padding:6px 11px;border-radius:8px;border:1px solid #dc2626;background:#dc262614;color:#dc2626;cursor:pointer">🔴 데드캣</button><button onclick="_sxAutoScanDownload()" style="font-size:10px;font-weight:700;padding:6px 9px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text2);cursor:pointer">💾 JSON</button><button onclick="_sxYAxisValidate()" style="font-size:10px;font-weight:800;padding:6px 11px;border-radius:8px;border:1px solid #7c3aed;background:#7c3aed14;color:#7c3aed;cursor:pointer">🎯 Y축검증</button></div><div style="font-size:8.5px;color:var(--text3);line-height:1.4;margin-bottom:3px">체크=베이스 · 미체크 하나씩 추가 · <b style="color:#dc2626">데드캣=N5↑N10↓(거울상)</b></div><div id="sxbAutoOut" style="margin-top:4px"></div></div>';
  h+='</div>';
  return h;
}
...
function _sxBuilderEval(){
  ...
  var _bFit=_dcFitGrade(_bAg.late, _bAg.surv, _bM.length, _bBaseLate, false);   // [S777] 빌더는 진짜반등/눌림목 레시피 기준(후반평균+ = good)
  h+='<div style="margin-top:5px;padding:6px 8px;border-radius:6px;background:#7c3aed10;border:1px solid #7c3aed33;font-size:9.5px;color:var(--text2)"><span style="color:'+_bFit.col+';font-size:12px;margin-right:3px" title="적합도 '+_bFit.g+'">●</span>이 조합 '+_dcLatePill(_bM, _bBaseLate)+' <span style="font-size:8px;color:var(--text3)">(표본 '+_bM.length+' · 적합도 '+_bFit.g+')</span></div>';
  out.innerHTML=h;
}
