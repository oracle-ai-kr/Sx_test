...
//        분석탭 헤더 카드의 분석시간 아래에 1줄 표시 (희창 요청)
//  데이터: 232개 항목, ~14.5KB
//  형식: { "코드": { n: 업종명, s: 섹터약칭 } }
// ══════════════════════════════════════════════════════════════
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      <!-- [업종표시] DART/dart/company 응답으로 비동기 채움 (induty_code → KSIC 매핑). 국내 종목만 표시. -->
      <!--   [방어] stock에 industryCode가 이미 박혀있으면 (이전 진입에서 캐시) 동기 렌더 → 첫 진입 시 fetch 응답 도착 전 DOM 미존재로 누락되는 케이스 보완 -->
      ${(()=>{
        const _ic = stock && (stock.industryCode || stock._industryCode);
        const _ist = _ic ? _ksicSectorText(_ic) : '';
        if(_ist){
          return `<div id="analSectorText" style="font-size:11px;color:var(--text2);margin-top:6px;font-weight:600;letter-spacing:-.2px">${_ist}</div>`;
        }
        return `<div id="analSectorText" style="font-size:11px;color:var(--text2);margin-top:6px;font-weight:600;letter-spacing:-.2px;display:none"></div>`;
      })()}
      ${(()=>{
    ...
    ${(()=>{
      ...
      const srcBadge = fin._source ? `<span style="font-size:8px;color:var(--text3);float:right">${fin._source.toUpperCase()}${fin._reportLabel ? ' · ' + fin._reportLabel : ''}${sjBadge}</span>` : '';
      ...
      return `<div class="anal-section">
        ...
        ${(()=>{
          ...
          // [PATCH ebitda-fallback-v2] 마커 단계 표시 — null/false=정확, 'adjustment'=조정 추정, true=영업이익 추정
          //   조정 기반은 영업이익 단독보다 정확하므로 별도 라벨로 신뢰도 차별화
          const apxLbl = !fin._ebitdaApprox ? '' :
            ` <span style="font-size:8px;color:var(--text3);font-weight:normal">${fin._ebitdaApprox === 'adjustment' ? '(조정 추정)' : '(근사)'}</span>`;
          const cls = fin.evEbitda>0&&fin.evEbitda<5?'bullish':fin.evEbitda>15||fin.evEbitda<0?'bearish':'neutral';
          return `<div class="anal-row"><span class="al">EV/EBITDA${apxLbl}</span><span class="ar ${cls}">${fin.evEbitda.toFixed(1)}배</span></div>`;
        })()}
