    ...
    try{
      ...
      if(candles && candles.length >= 20){
        ...
        if(currentMarket === 'kr' && window._kisEnabled && _analTF === 'day' && candles.length > 0){
          // ──────────────────────────────────────────────────────────
          // [S312] 종가매매 일관성 — 3시 이후 마지막 봉 잠금 (sticky marker)
          // ──────────────────────────────────────────────────────────
          //   배경: KIS 일봉의 마지막 봉 close는 장중 실시간 현재가로 계속 변동.
          //         그 결과 분석탭 BT 마커(▲BT매수/▼BT매도)가 가격 변동에 따라 ON↔OFF 깜빡임.
          //         사용자는 종가매매 의도(3시 이후 매매 결정)인데 마커가 불안정해 결정 어려움.
          //   대안 검토: 시간봉(60m) 차트 — 데이터 부실로 작업량 과다 (사용자 결정).
          //   해결: KST 15:00 이후 첫 분석 시 candles[-1]을 localStorage에 잠금 →
          //         이후 같은 종목 재분석 시 잠긴 봉 복원 → BT/인디케이터/마커/카드 모두 일관 유지.
          //         다음 거래일이 되면 자동 만료 (캐시의 date 필드를 KST 오늘과 비교).
          //   사이드 이펙트: KIS-PRICE-FIX가 candles[-1].close로 stock.price 보정하므로,
          //                3:00~3:30 사이의 실시간 가격도 잠긴 값으로 표시 (마감 후엔 어차피 무변).
          //   범위: 국내 + KIS + 일봉만. 시간봉/주봉/월봉/해외/코인은 미적용.
          try {
            const _kstNow312 = new Date(Date.now() + 9*3600*1000);
            const _hKst312 = _kstNow312.getUTCHours(); // KST 시 (15시면 15 반환)
            const _todayKst312 = _kstNow312.toISOString().slice(0,10);
            if(_hKst312 >= 15){ // KST 15시 이후 (장 마감 30분 전부터 마감 후까지)
              const _lockKey312 = 'SX_LOCKED_LASTBAR_v1';
              let _locks312 = {};
              try { _locks312 = JSON.parse(localStorage.getItem(_lockKey312) || '{}'); } catch(_){}
              const _ex312 = _locks312[stock.code];
              const _last312 = candles[candles.length - 1];
              if(_ex312 && _ex312.date === _todayKst312 && _last312){
                // 잠긴 봉 복원 — close는 그대로, high/low는 외부값과 안전 병합 (확장만 허용)
                _last312.close = _ex312.close;
                _last312.open  = _ex312.open;
                _last312.high  = Math.max(_ex312.high || 0, _last312.high || 0);
                _last312.low   = (_last312.low > 0 && _ex312.low > 0)
                                 ? Math.min(_ex312.low, _last312.low) : (_ex312.low || _last312.low);
                // [S313] 헤더 분기 판정용 메타 (stock 객체에 박음)
                stock._lockedClose = _ex312.close;
                stock._lockedAt    = _ex312.lockedAt;
                stock._lockedDate  = _ex312.date;
                console.log(`[S312] 종가매매 잠금 복원: ${stock.code} close=${_ex312.close.toLocaleString()}원 (잠긴 시각: ${new Date(_ex312.lockedAt).toLocaleTimeString('ko-KR')})`);
              } else if(_last312 && _last312.close > 0){
                // 새로 잠금 — 종목별 1행
                const _newLockedAt312 = Date.now();
                _locks312[stock.code] = {
                  date: _todayKst312,
                  close: _last312.close,
                  high:  _last312.high,
                  low:   _last312.low,
                  open:  _last312.open,
                  lockedAt: _newLockedAt312
                };
                // 다른 날짜 캐시 자동 정리 (다음 거래일 시작 시 모두 만료)
                Object.keys(_locks312).forEach(k => {
                  if(_locks312[k] && _locks312[k].date !== _todayKst312) delete _locks312[k];
                });
                try { localStorage.setItem(_lockKey312, JSON.stringify(_locks312)); } catch(_){}
                // [S313] 헤더 분기 판정용 메타 (stock 객체에 박음)
                stock._lockedClose = _last312.close;
                stock._lockedAt    = _newLockedAt312;
                stock._lockedDate  = _todayKst312;
                console.log(`[S312] 종가매매 잠금 시작: ${stock.code} close=${_last312.close.toLocaleString()}원 (3시 이후 첫 분석)`);
              }
            }
          } catch(_e312){
            console.warn('[S312] 종가매매 잠금 처리 실패 (무시하고 계속):', _e312);
          }
          // ──────────────────────────────────────────────────────────
