  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        if(!_v) return '';
        // [S326] 부적합 분류 시: 적정/목표주가 둘 다 숨기고 안내 박스만 표시
        if(_v.unsuitable){
          return `<div class="anal-fair-price" style="margin-top:4px;letter-spacing:-.2px;line-height:1.4">`
            + `<div style="font-size:10px;font-weight:700;color:var(--text2);background:rgba(255,180,0,.08);border-left:2px solid #ffb400;padding:5px 8px;border-radius:3px;text-align:left">`
            + `📌 ${_v.unsuitCategory}`
            + `<div style="font-size:9px;font-weight:500;color:var(--text3);margin-top:2px">${_v.unsuitTip}</div>`
            + `</div>`
            + `</div>`;
        }
        ...
        if(_v.targetPrice != null && _v.targetDiffPct != null){
          ...
          _tpHTML = `<div class="anal-target-price" style="margin-top:4px;letter-spacing:-.2px;line-height:1.3">`
            ...
            + `PER ${_v.targetPer}배 · ${_growthNote} · `
