// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v2.3 <!-- S82 -->
...
function renderResults(){
  ...
  // S81: 배지 필터 적용 + S82: 배지 내 전이확률 순 정렬
  let displayed = _stageBadgeFilter ? filtered.filter(s => _getStockStage(s) === _stageBadgeFilter) : filtered;
  if (_stageBadgeFilter && displayed.length > 1) {
    displayed = displayed.slice().sort((a, b) => {
      const aProb = _getTransitionProb(a, _stageBadgeFilter);
      const bProb = _getTransitionProb(b, _stageBadgeFilter);
      return bProb - aProb; // 높은 전이확률 우선
    });
  }
