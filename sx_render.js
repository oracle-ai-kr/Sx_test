...
var _sxvAxisLabel = { ready:'반등신호', entry:'반등강도', upside:'추가상승', trend:'추세방향' };
...
function _buildValidatorCard(stock, indicators){
  ...
  var _matSec = _gMat.length
    ...
    : '<div style="font-size:9px;color:var(--text3);margin:7px 0 2px;font-style:italic">재료 — 레시피에서 🔬축(반등신호 등)을 켜면 그 축 재료가 자동으로 나타나요</div>';
...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  if(SXVVAL.ablateIC){ try{
    P.ablate   = SXVVAL.ablateIC(records, 'rd');                                            // 반등신호 (전체봉)
    P.ablateEn = SXVVAL.ablateIC(records.filter(function(r){ return r.path==='rebound'; }), 'en'); // 반등강도 (반등경로 봉만)
...
function _cvPoolRender(rep, watch, mk){
  var GRN='#16a34a',RED='#e3493b',MUT='#64748b',T3='var(--text3)',PUR='#a855f7',AXN={rd:'반등신호',en:'반등강도',tr:'추세방향',up:'추가상승'}; // [S690] 경로분할 렌더
  ...
  function bracket(P, label){
    ...
    // [S721-723] 재료 ablation IC — 축 재료 빼면 그 축 IC ΔIC (전체/강세봉/약세봉). 4축: 반등신호·반등강도(반등경로)·추가상승(추세경로)·추세방향.
  ...
  if(rep&&rep.ok&&watch&&watch.ok){
    out+='<div style="margin-top:9px;font-size:9px;color:var(--text2);line-height:1.55;border-top:1px solid var(--border);padding-top:6px">경로별로 그 경로의 <b>활성 축만</b> 격리 평가 — rebound경로는 <b>반등강도</b>가, trend경로는 <b>추가상승</b>이 핵심 축. 같은 경로 축이 두 브래킷 다 같은 부호 +IC면 그 경로 로직 범용. <b>경로수익 부호</b>=그 경로 탄 시점이 실제 먹혔는지 · <b>등급격차</b>=경로 내 등급 변별.</div>';
  }
  out+='</div>';
  return out;
}
