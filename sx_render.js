  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        //   · C패널 "최종 판정" 자리 → "현재 지표 상태 요약"(pg.extras) 렌더
        //   · BT패널 활용토글 내부 "현재 지표 상태 요약" 자리 → "최종 판정"(보유유지 등 9종) 카드 렌더
        //   취지: 보유유지/매도완료 등 판정 = 시뮬(BT) 성격 → BT쪽 / RSI·MA·OBV·레짐 안내 = 지표 성격 → C쪽
        //   배선: pg를 outer에서 1회 계산(_pgShared) → 양쪽 공유. C IIFE가 BT IIFE보다 먼저 실행되므로
        //         verdict HTML은 C IIFE가 _swapVerdictHTML에 stash → BT IIFE 활용토글이 소비.
        //   폴백: pg가 null(verdictAction 없음=BT 거래수 부족/미지원 TF)이면 swap 안 함 → C에 verdict 원래대로 유지
        //         (sx_render.js 단독 변경, BT 계산 미러와 무관)
      ...
      ${(()=>{
        ...
        if(_swapVerdictHTML){
          pgHTML += `<div style="margin-top:6px;padding-top:6px;border-top:1px solid var(--border)"><div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:4px">최종 판정</div>${_swapVerdictHTML}</div>`;
        }
        pgHTML += `</div></div>`;
        return pgHTML;
      })()}
