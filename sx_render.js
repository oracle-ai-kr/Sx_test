// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S838 · build 20260701-0220 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _deadcatHzBtBracket(mk, onProgress){
  ...
  var CAP=130, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
  var R={15:{nT:0,sum:0,wins:0,mddSum:0,eqSum:0,stocks:0}, 20:{nT:0,sum:0,wins:0,mddSum:0,eqSum:0,stocks:0}}, stocksUsed=0;
  for(var i=0;i<use.length;i++){
    var s=use[i];
    if(onProgress) onProgress(i+1, use.length, s.name);
    await _trendBatchSleep(0);
    ...
    var _u=false;
    var hzs=[15,20];
    for(var hi=0;hi<hzs.length;hi++){ var hz=hzs[hi];
      var r=null; try{ r=await SXRecipeSignal.deadcatHzBtScan(s.code, rows, hz, 3); }catch(e2){}
      if(r&&r.nT>0){ R[hz].nT+=r.nT; R[hz].sum+=r.sumPnl; R[hz].wins+=r.wins; R[hz].mddSum+=r.mddSum; R[hz].eqSum+=(r.eqEnd-1); R[hz].stocks++; _u=true; }
    }
    if(_u) stocksUsed++;
    await _trendBatchSleep(10);
  }
  if(R[15].nT<20) return { ok:false, reason:'겹침3+ 거래 표본 20 미만 (n='+R[15].nT+')' };
...
function _deadcatHzBtRender(res, mk){
  ...
  function rowFor(hz){
    var g=res.R[hz];
    return { hz:hz, nT:g.nT, avg:(g.nT?g.sum/g.nT*100:null), wr:(g.nT?g.wins/g.nT*100:null), mdd:(g.nT?g.mddSum/g.nT*100:null), stotal:(g.stocks?g.eqSum/g.stocks*100:null), perbar:(g.nT?(g.sum/g.nT*100)/hz:null) };
  }
  var r15=rowFor(15), r20=rowFor(20);
  function tr(r){
    var acol=(r.avg==null)?T3:(r.avg>=0?GRN:RED), pcol=(r.perbar==null)?T3:(r.perbar>=PB_N10_PERBAR?GRN:(r.perbar>=0?AMB:RED));
    return '<tr>'
      +'<td style="padding:6px 7px;font-weight:800;color:var(--text);font-size:10px">겹침3+ N'+r.hz+'</td>'
      ...
      +'<td style="padding:6px 4px;text-align:right;color:'+RED+';font-size:9.5px">'+(r.mdd!=null?r.mdd.toFixed(1)+'%':'—')+'</td>'
  ...
  // 판정 — 수익 + 시간당(정배 대비) + MDD
  var best=(r15.perbar!=null&&r20.perbar!=null)?(r15.perbar>=r20.perbar?r15:r20):(r15.perbar!=null?r15:r20);
  var v='';
  if(best&&best.avg!=null){
    var profitable=best.avg>0, beatsPb=(best.perbar!=null&&best.perbar>=PB_N10_PERBAR), deepMdd=(best.mdd!=null&&best.mdd<=-8);
    if(profitable && beatsPb && !deepMdd) v='<span style="color:'+GRN+'">✅ <b>N'+best.hz+' 겹침3+ 실전 수익 '+best.avg.toFixed(1)+'% · 시간당 +'+best.perbar.toFixed(2)+'%/봉(정배 N10 0.29% 이상)</b> — 긴 호흡 값어치. MDD '+best.mdd.toFixed(1)+'% 감당 가능. <b>역배 겹침 실전 유효</b></span>';
    else if(!profitable) v='<span style="color:'+RED+'">⚠️ N'+best.hz+' 겹침3+ 실전 <b>수익 '+best.avg.toFixed(1)+'%</b> — 통계 72%가 실전 수익으로 안 이어짐. 진짜비율↑여도 <b>손실/횡보</b>. 긴 호흡 단서 기각</span>';
    else if(!beatsPb) v='<span style="color:'+AMB+'">△ N'+best.hz+' 겹침3+ 수익 '+best.avg.toFixed(1)+'%(양수)지만 <b>시간당 +'+best.perbar.toFixed(2)+'%/봉 &lt; 정배 N10 0.29%</b> — 2~3주 묶는 값 못함. 정배가 시간대비 나음</span>';
    else if(deepMdd) v='<span style="color:'+AMB+'">△ N'+best.hz+' 수익 '+best.avg.toFixed(1)+'%지만 <b>MDD '+best.mdd.toFixed(1)+'% 깊음</b> — 통계상 유효해도 실전 낙폭 못 버팀(손절당함). 긴 호흡 어려움</span>';
  }
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px"><b>역배 겹침3+ 실전 BT</b> — N15/N20 유효 통계가 실제 수익/낙폭으로? 겹침3+ 진입→N봉 보유 청산·1포지션·복리. <b style="color:'+T2+'">평균</b>=거래당 수익 · <b>승률</b> · <b style="color:'+RED+'">MDD</b>=거래별 진입후 평균 최저 · <b style="color:'+BLU+'">시간당</b>=수익÷보유봉(<b>정배 N10 ≈0.29%/봉이 기준선</b>). 겹침3+·대표+관심+top100·'+(res.stocksUsed||0)+'종목.</div>';
  return head
    +'<table style="width:100%;border-collapse:collapse;background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">'
