...
function _trendBt(rows,cfg,bbP){
  ...
  const _predGc=(i)=>{
    ...
    const prob = wA>0?wU/wA:0;   // 비슷한 과거 모양이 3봉내 교차로 이어진 비율
...
function _trendRenderInner(){
  ...
  // [S628] 선행 가드 — 이 종목에서 예측 ON이 OFF보다 손해/위험이면 경고(종목 과거 실측 기반, 룩어헤드 아님)
  let _guardBadge='';
  if(cfg.predict){
    try{
      const _btOff=_trendBt(rows,Object.assign({},cfg,{predict:false}),_trendBbParams(market));
      if(_btOff){
        const dE=+(bt.expectancy-_btOff.expectancy).toFixed(2);
        const rM=_btOff.mdd>0?bt.mdd/_btOff.mdd:1, dM=bt.mdd-_btOff.mdd;
        let col,txt;
        if((rM>=1.5&&dM>=5)||dE<=-0.3){ col='#e8365a'; txt=`⚠️ 선행 손해 · OFF 권장 (MDD ${_btOff.mdd}%→${bt.mdd}%${dE<0?` · 기댓값 ${dE}`:''})`; }
