...
//   Y(품질): 진짜/건강(+: 상승다이버·지지근접·거래량OSC<−39.6[S738]·신규 GC 5×20·과매도) − 가짜/실패(−: 되돌림 dumpWarn·OBV하락·약세다이버·과열·저항근접)
...
function _reboundCycleCalc(adv, stock, mkt){
  ...
  // ── Y축: 반등 품질 (가짜·실패− ↔ 진짜·건강+) ──
  var y = 0, yPos = [], yNeg = [];
  if(adv.rsi && adv.rsi.div === 'bullish'){ y += 20; yPos.push('RSI 상승다이버'); }
  if(adv.obv && adv.obv.div === 'bullish'){ y += 15; yPos.push('OBV 상승다이버'); }
  var nearSup = !!(adv.trend && adv.trend.struct && adv.trend.struct.nearSupport);
  if(nearSup){ y += 15; yPos.push('지지선 근접'); }
  if(typeof adv.volOsc === 'number' && adv.volOsc < -39.6){ y += 20; yPos.push('거래량OSC ' + adv.volOsc.toFixed(0) + '(조용한 매집)'); }
  ...
  var freshGc = (cl && typeof _goldenX733 === 'function') ? _goldenX733(cl, 5, 20, 3) : false;
  if(freshGc){ y += 15; yPos.push('신규 골든크로스 5×20'); }
  if(adv.rsi && typeof adv.rsi.val === 'number' && adv.rsi.val < 35){ y += 10; yPos.push('과매도 RSI ' + adv.rsi.val.toFixed(0)); }
  var dump = null;
