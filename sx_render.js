...
var _sxvPredsOn = { slope:1, slopeTurn:1, rsiGc:1, obv:1, macd:1, slopeOrRsi:1, slopeAndRsi:1 };   // [S705] 신호 on/off (기본 전체 on)
...
function _sxvResultHtml(){
  ...
  var opt = { s:cfg.s, l:cfg.l, N:_sxvN, thr:_sxvThr, tp:_sxvTp, sl:_sxvSl, regime:_sxvRegime };   // [S705-3] dir 제거 · [S706] 배리어 tp/sl · [S707] 레짐 필터
  if(_sxvSrc === 'cur'){
    var rows = (L.indicators && L.indicators._advanced && Array.isArray(L.indicators._advanced.rows)) ? L.indicators._advanced.rows
             : (L.stock && Array.isArray(L.stock._lastAnalCandles)) ? L.stock._lastAnalCandles : null;
...
function _buildValidatorCard(stock, indicators){
  ...
  var ctrl2Row = '<div style="display:flex;align-items:center;gap:5px;margin-bottom:7px;flex-wrap:wrap">' + nBtn + thBtns + rgBn + '</div>';
  // 신호 멀티칩
