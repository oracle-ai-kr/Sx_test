...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  var _rcpCb=(_rcpOn && window.SXRecipeSignal && SXRecipeSignal.evalBar) ? function(qs, slice, e){
    var r=SXRecipeSignal.evalBar(qs && qs.ind, slice, e, mk);
    if(!r) return null;
    var v=0;
    if(r.pure){
      if(mk==='kr') v = r.realK>=4?4 : (r.realK>=3?3 : (r.realK>=2?2 : 1));
      else if(mk==='us') v = (r.lt==='bull') ? (r.realK>=3?4:3) : 2;
      else v = 1;   // coin — real 약함(신호 표시만), fake 감점(혼재=0)이 주 가치
    }
