...
function _trendCfg(market){
  ...
  try { const raw=localStorage.getItem('SX_TREND_'+market); if(raw){ const o=JSON.parse(raw); if(o){ cfg.s=+o.s||d[0]; cfg.l=+o.l||d[1]; cfg.n=+o.n||3; cfg.aux=(o.aux&&typeof o.aux==='object')?o.aux:{}; cfg.sell=(o.sell&&typeof o.sell==='object')?o.sell:{}; cfg.reentry=!!o.reentry; cfg.nextOpen=!!o.nextOpen; } } } catch(_){}
...
function _trendBt(rows,cfg,bbP){
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){
      if(gc && auxOk(i)){ const p=_mkPos(i,false); if(p) pos=p; } // [S581] 진입가/진입idx는 _mkPos가 결정
...
function _trendRenderInner(){
  ...
  const note=`<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · ${cfg.s}MA×${cfg.l}MA 골든크로스 ${cfg.nextOpen?'→ 다음봉 시가 진입':'종가 진입'}${_buyN?` · 매수 ${_buyN}개 AND(최근${cfg.n}봉)`:' · MA 단독'} → 데드크로스${_sellN?` OR 매도 ${_sellN}개`:''} 종가청산${cfg.reentry?' · 🔄정배열 재진입':''}</div>`;
