...
// ══════════════════════════════════════════════════════════════
// S103-fix7 Phase3-B-4f: 상단 배너 내부 "시뮬 포지션 라인" 생성 (프로젝트 C v2.0)
//   🎯 v2.0 철학: 시스템 = 가상 트레이더, 사용자 = 미러링
//   이 함수는 "시뮬 트레이더의 현재 상태"를 내러티브로 표출
//
//   v5.4 _buildBtSimLine(롤백됨)과의 차이점:
//     · v5.4: BT 데이터 리포트 어조 ("BT 매수 4/6 @ 317,317")
//     · v2.0: 시뮬 내러티브 어조 ("▲ 4/6 진입 @ 317,317" = 시뮬의 발자취)
//     · v5.4: empty 상태에서 점수/부족분 표시 (기계적)
//     · v2.0: empty 상태에서 관찰자 모드 안내 ("시뮬 포지션 없음, 타이밍 대기")
//     · v5.4: 회피 상태 미표시 (공백)
//     · v2.0: 회피 상태에서 "시뮬 진입 안 함" 명시 (투명성)
//     · v5.4: C 판정과 독립 (혼란 유발)
//     · v2.0: C 판정(svVerdict)에 맞춰 어조/포커스 적응 (정합)
//
//   상태별 분기 (5종):
//     · holding + _isBuySignal=true  → 🟢 매수 신호 방금 (시뮬 진입 중)
//     · holding + _isBuySignal=false → 🟢 시뮬 보유 중 (날짜/가격/pnl/보유일)
//     · sell_signal                  → 🔴 시뮬 청산 완료 (익절/손절 + pnl + 청산일)
//     · waiting + C='관심'/'관망'     → 🔵 시뮬 관찰 중 (매수 타이밍 대기)
//     · waiting + C='회피' or no_data→ ⚪ 시뮬 포지션 없음 (진입 부적합 or BT 미실행)
//
//   차트 마커(▲/▼)와 시각적 일관성: svVerdict.chartMarker와 동일 기호 사용
//   → 차트의 ▲와 배너의 ▲가 같은 "시뮬의 매수 행적"을 나타냄
// ══════════════════════════════════════════════════════════════
function _buildSimPositionLine(stock, btSt, svVerdict){
  ...
  // ── no_data: BT 미실행 (시뮬 트레이더 자체가 없음) ──
  if(!btSt || btSt.state === 'no_data'){
    return `<div style="${BOX_STYLE}">
      <div style="${TITLE_STYLE}">📊 시뮬 상태</div>
      <div style="${MAIN_STYLE}">시뮬 미실행</div>
      <div style="${SUB_STYLE}">BT 백테스트 실행 시 시뮬 포지션 활성화</div>
    </div>`;
  }

  // ── holding + _isBuySignal=true: 방금 매수 신호 (시뮬 진입 순간) ──
  if(btSt.state === 'holding' && btSt._isBuySignal){
    ...
    return `<div style="${BOX_STYLE}">
      <div style="${TITLE_STYLE}">📊 시뮬 신호</div>
  ...
  // ── holding + _isBuySignal=false: 시뮬 보유 중 (▲ 있고 ▼ 없음, 2봉 초과) ──
  if(btSt.state === 'holding'){
    ...
    // TP/SL 라인 (BT 기준 = 시뮬 포지션의 실제 목표/손절)
    let tpslLine = '';
    if(btSt.tp && btSt.sl){
    ...
    return `<div style="${BOX_STYLE}">
      <div style="${TITLE_STYLE}">📊 시뮬 포지션</div>
  ...
  if(btSt.state === 'sell_signal'){
    ...
    return `<div style="${BOX_STYLE}">
      <div style="${TITLE_STYLE}">📊 시뮬 완결</div>
  ...
  if(btSt.state === 'waiting'){
    ...
    if(verdictAction === '회피'){
      return `<div style="${BOX_STYLE}">
        <div style="${TITLE_STYLE}">📊 시뮬 상태</div>
        <div style="${MAIN_STYLE}">시뮬 포지션 없음</div>
    ...
    if(verdictAction === '관심'){
      return `<div style="${BOX_STYLE}">
        <div style="${TITLE_STYLE}">📊 시뮬 관찰</div>
        <div style="${MAIN_STYLE}">현재 시뮬 포지션 없음</div>
    ...
    if(verdictAction === '관망'){
      return `<div style="${BOX_STYLE}">
        <div style="${TITLE_STYLE}">📊 시뮬 보류</div>
        <div style="${MAIN_STYLE}">현재 시뮬 포지션 없음</div>
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    ...
    // S103-fix7 Phase3-B-4f: 시뮬 포지션 라인 추가 (프로젝트 C v2.0 시뮬 내러티브)
    //   _buildSimPositionLine: "시뮬 트레이더의 현재 상태" 표출 (5가지 상태별 분기)
    //   v5.4 _buildBtSimLine의 롤백 이유(BT↔C 시점 혼란)를 다음 방식으로 해결:
    //     ① C 판정(svVerdict)에 맞춰 어조/포커스 적응 (정합)
    //     ② "📊 시뮬 포지션/신호/완결/관찰/보류/상태" 타이틀로 의미 명확화
    //     ③ 차트 ▲/▼ 기호와 시각 일관성 (시뮬 트레이더의 발자취)
