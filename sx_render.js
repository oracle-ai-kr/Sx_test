...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  var records=[], stocksUsed=0, skipped=0;
  for(var i=0;i<use.length;i++){
    var s=use[i];
    if(onProgress) onProgress(i+1, use.length, s.name);
    await _trendBatchSleep(0);
    ...
    if(!Array.isArray(rows)||rows.length<_floor){ skipped++; await _trendBatchSleep(8); continue; }
    var R=null;
  ...
  if(stocksUsed<3) return { ok:false, reason:'유효 종목 3개 미만('+mk+' '+(source==='watch'?'관심':'대표')+') — 네트워크 확인' };
  var P=SXVVAL.pool(records);
  if(!P||!P.ok) return P||{ok:false,reason:'집계 실패'};
  P.source=source; P.market=mk; P.stocks=stocksUsed; P.capped=capped; P.total=list.length;
  return P;
}
...
function _cvPoolRender(rep, watch, mk){
  ...
  function bracket(P, label){
    if(!P||!P.ok) return '<div style="margin-top:10px;font-size:10.5px;font-weight:800;color:'+PUR+'">'+label+'</div><div style="font-size:10px;color:'+T3+';margin-top:3px">집계 불가'+(P&&P.reason?' ('+P.reason+')':'')+'</div>';
    var h='<div style="margin-top:12px;font-size:11px;font-weight:800;color:'+PUR+'">'+label+' · '+P.stocks+'종목·'+P.n+'시점'+(P.capped?'(상한20)':'')+'</div>';
    var BP=P.byPath||{};
