...
function _sxvResultHtml(){
  ...
  var opt = { s:cfg.s, l:cfg.l, N:_sxvN, thr:_sxvThr };   // [S705-3] dir 제거 — 자 polarity가 방향 결정
  if(_sxvSrc === 'cur'){
    var rows = (L.indicators && L.indicators._advanced && Array.isArray(L.indicators._advanced.rows)) ? L.indicators._advanced.rows
             : (L.stock && Array.isArray(L.stock._lastAnalCandles)) ? L.stock._lastAnalCandles : null;
...
function _sxvRenderResult(R, isPool){
  ...
  var dirTxt = R.ruler === 'upmove' ? '상승' : (R.ruler === 'peakdraw' ? '되돌림' : '하락');
  var dirCol = R.ruler === 'upmove' ? '#16a34a' : (R.ruler === 'peakdraw' ? '#f59e0b' : '#ef4444');
...
function _buildValidatorCard(stock, indicators){
  ...
  // 자 탭
  var rulerDefs = [ { id:'upmove', label:'상승', col:'#16a34a' }, { id:'drawdown', label:'하락', col:'#ef4444' }, { id:'peakdraw', label:'되돌림', col:'#f59e0b' } ];   // [S705-3] 상승·하락·고점되돌림(청산)
  ...
  // N / thr (상승·하락 둘 다 return형) [S705-3]
  ...
  var thrBtn = '<span onclick="_sxVib(10);window._sxvCycleThr&&_sxvCycleThr()" title="목표 수익/낙폭 임계(2→3→5%)" style="font-size:10px;font-weight:800;padding:4px 9px;border-radius:13px;cursor:pointer;border:1px solid var(--border);background:var(--surface2);color:var(--text2)">' + _sxvThr + '% ⟳</span>';
  var ctrl2Row = '<div style="display:flex;align-items:center;gap:5px;margin-bottom:7px;flex-wrap:wrap">' + nBtn + thrBtn + '</div>';
  // 신호 멀티칩
...
if(typeof window !== 'undefined'){ window._sxvSetRuler = _sxvSetRuler; window._sxvCycleN = _sxvCycleN; window._sxvCycleThr = _sxvCycleThr; window._sxvTogglePred = _sxvTogglePred; window._sxvSetSrc = _sxvSetSrc; window._sxvRerender = _sxvRerender; window._buildValidatorCard = _buildValidatorCard; }
