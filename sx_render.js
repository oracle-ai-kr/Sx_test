...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  ...
  // ⑤ 기술지표 [S367/S369] — 이평선 배열(7단 정밀) + 골든크로스(단방향 ▲) + 데드크로스(단방향 ▼) · GC/DC는 전환 신호라 종합 평균에서 제외(neutral)
  if(extras.maAlign && extras.maAlign.v!=null) g5.push({k:'이평선 배열', v:extras.maAlign.v, byValue:true});
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  try {
    ...
    if(indicators){
      ...
      // [S367/S368] ⑤ 기술지표 — 이평선 배열 (7단 정밀 우선, 필드 누락 시 3단 fallback)
      const _maA = indicators.maAlign;
      if(_maA){
        if(_maA.short!=null && _maA.mid!=null && _maA.long!=null){
