 ...
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  if(!indicators){
    try{
      ...
      let candles;
      // [S228] 캐시 재사용 시 무결성 재검증 — 옛 비정상 봉이 stock._lastAnalCandles에 박힌 경우 차단
      //   증상: SK하이닉스/삼성전기/현대차 등에서 마지막 봉이 {open:0, low:0, close:정상} 형태로 캐시됨
      //         → 차트 wick 0까지 뻗어 빨간 세로선 + 거래대금 close*volume = 526억/1억 등
      //   원인: [S220] 패치 이전에 캐시된 비정상 봉이 메모리에 살아남아 fetch skip 시 재사용됨
      //   해결: 재사용 전 sx_screener.html의 _sxIsValidCandle로 검증 → 비정상 발견 시 캐시 폐기 + 재fetch
      const _isCacheClean = (rows) => {
        if(!rows || !rows.length) return false;
        if(typeof _sxIsValidCandle !== 'function') return true; // 함수 없으면 통과 (안전 기본값)
        for(let i = 0; i < rows.length; i++){
          if(!_sxIsValidCandle(rows[i])) return false;
        }
        return true;
      };
      if(stock._lastAnalCandles && stock._lastAnalCandles.length > _analCount){
        if(_isCacheClean(stock._lastAnalCandles)){
          candles = stock._lastAnalCandles;
          console.log(`[runAnalysis] 확장 캔들 재사용: ${candles.length}봉 (fetch skip)`);
        } else {
          // 비정상 봉 발견 → 캐시 폐기 + 단계 리셋 + 재fetch
          console.warn(`[S228] ⚠️ 비정상 봉 캐시 발견 → 폐기 후 재fetch (${stock._lastAnalCandles.length}봉)`);
          stock._lastAnalCandles = null;
          stock._analCandlesExtended = false;
          stock._analCandlesExtendedStage = 0;
