...
// [S615] 자기-kNN 약함 판정 — 풀링 추천 넛지 트리거 (표본 충분 + 적중률 50% 미만 = 자기 닮은꼴 부족)
function _ctKnnIsWeak(res){ return !!(res && res.hitRate!=null && res.pred>=8 && res.hitRate < 50); }
...
function _ensurePoolNudgeCss(){
  ...
  st.textContent='@keyframes sxPoolPing{0%{box-shadow:0 0 0 0 rgba(14,165,233,.55)}70%{box-shadow:0 0 0 9px rgba(14,165,233,0)}100%{box-shadow:0 0 0 0 rgba(14,165,233,0)}}@keyframes sxPoolBeat{0%,32%,100%{transform:scale(1)}13%{transform:scale(1.22)}}@keyframes sxPoolGlow{0%,100%{border-color:#0ea5e955}50%{border-color:#0ea5e9bb}}';
...
function _ctPoolNudge(){
  return '<div style="margin-top:8px;padding:8px 11px;border-radius:10px;background:linear-gradient(90deg,#0ea5e914,#7c3aed0e);border:1px solid #0ea5e955;display:flex;align-items:center;gap:10px;animation:sxPoolGlow 2.4s ease-in-out infinite">'
    ...
    + '<div style="flex:1;min-width:0"><div style="font-size:10.5px;font-weight:800;color:#0ea5e9">자기 패턴이 약해요 — 풀링 추천</div>'
    + '<div style="font-size:9px;color:var(--text3);margin-top:1px;line-height:1.45">자기 과거 닮은꼴이 부족한 종목이에요. <b style="color:#7c3aed">검증</b> → 🌐 다종목 풀링 비교를 눌러보세요.</div></div>'
    + '</div>';
}
  ...
  if(_useKnn){
    ...
    if(Object.prototype.hasOwnProperty.call(_ctKnnHitCache, _hk)){
      ...
      (function(_r,_key,_hi,_ni){ setTimeout(function(){
        ...
        var ne=document.getElementById(_ni); if(ne && _ctKnnIsWeak(res)){ _ensurePoolNudgeCss(); ne.outerHTML=_ctPoolNudge(); }
