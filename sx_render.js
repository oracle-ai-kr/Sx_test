...
//   재검색마다 coingecko/바이낸스를 때리면 rate limit(429)에 걸려 도넛이 빠지므로 7분 TTL 캐시.
...
// [S357→S358] 김치프리미엄 — 업비트 KRW-BTC vs 바이낸스 BTCUSDT × USDKRW 환율 (= 업비트 프리미엄)
//   〔이력〕 S357은 글로벌가를 coingecko로 받았으나 도미넌스와 같은 도메인이라 rate limit 동시 누락 →
//          S358에서 바이낸스(분당 1200 weight, 여유)로 분산 + 7분 캐시. coingecko 부담 도미넌스 1개로 감소.
...
async function _computeKimchiScore(){
  ...
  try{
    const base = (typeof WORKER_BASE!=='undefined') ? WORKER_BASE : 'https://stock-signal-proxy.cheaheechang.workers.dev';
    const bTarget = 'https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT';
    const [upJ, bJ, fxJ] = await Promise.all([
      fetch(`${base}/upbit/ticker?markets=KRW-BTC`, {signal:AbortSignal.timeout(8000)}).then(r=>r.ok?r.json():null).catch(()=>null),
      fetch(`${base}/proxy?url=${encodeURIComponent(bTarget)}`, {signal:AbortSignal.timeout(8000)}).then(r=>r.ok?r.json():null).catch(()=>null),
      fetch(`${base}/naver/exchange?sym=USDKRW`, {signal:AbortSignal.timeout(8000)}).then(r=>r.ok?r.json():null).catch(()=>null),
    ]);
    const upbit = Array.isArray(upJ) && upJ[0] && upJ[0].trade_price;
    const usd = bJ && parseFloat(bJ.price);
    const fx = fxJ && fxJ.price;
    if(upbit && usd && fx && isFinite(upbit) && isFinite(usd) && isFinite(fx)){
      const km = (upbit / (usd * fx) - 1) * 100;
