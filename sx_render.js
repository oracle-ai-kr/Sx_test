      ...
      if(candles && candles.length >= 20){
        ...
        if(currentMarket === 'kr' && window._kisEnabled && _analTF === 'day' && candles.length > 0){
          ...
          if(_lastBar && _lastBar.close > 0){
            ...
            // [S219] 거래대금/거래량 비정상값 보강 — KRX market-cap이 장외/일요일에 부분값 반환하는 경우
            ...
            if(_lastBar.volume > 0){
              ...
              if(_stockVol === 0 || (_kisVol > 0 && _stockVol > 0 && _kisVol / _stockVol > 10)){
                ...
                console.log(`[S219 VOL-FIX] ${stock.code} 거래량/거래대금 보강: vol ${_prevVol.toLocaleString()} → ${_kisVol.toLocaleString()}주, 거래대금 ${_prevTA.toLocaleString()} → ${stock.tradeAmount.toLocaleString()}백만원`);
              }
            }
          }
        }
