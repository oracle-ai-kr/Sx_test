// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S810 · build 20260629-2330 cache-bust]
// ════════════════════════════════════════════════════════════
...
// ════════ [S810] 종목 변동성 × 진짜반등 — 변동성 낮을수록 N10 진짜비율↑? (사용자 가설: 관심풀 약세→고변동 변경이 역배 84%vs33% 격차 유발=종목 성격이 데드캣 변별자일 수). 역배 deadcat-real 발동봉을 '종목 일간변동성'(최근120봉 수익률 표준편차)으로 3분위. 변동성↓→진짜비율↑ 단조면 종목성격=강 변별자. 대표+관심 합산(변동성 스펙트럼·풀구분 무의미). S731 BT bear-gate(우량 역배=진짜/약세 역배=데드캣)와 일관 가설. 측정전용. ════════
...
async function _volRegimeBracket(mk, onProgress){
  ...
  var seen={}, list=[];
  rep.concat(watch).forEach(function(s){ if(s&&s.code&&!seen[s.code]){ seen[s.code]=1; list.push(s); } });   // 대표+관심 합치고 중복제거
  if(!list.length) return { ok:false, reason:'종목 없음' };
  var CAP=40, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
...
function _volRegimeRender(res, mk){
  ...
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px">역배열 deadcat-real 발동봉을 <b>종목 일간변동성</b>(최근120봉 수익률 표준편차)으로 3분위 → N10 진짜비율. 대표+관심 합산(변동성 스펙트럼). <b style="color:'+GRN+'">변동성 낮을수록 진짜비율↑</b>이면 종목 성격=변별자(=관심풀 84%vs33% 격차의 정체). 표본='+res.bars.length+'봉·'+(res.stocksUsed||0)+'종목.</div>';
  return head
    +'<table style="width:100%;border-collapse:collapse;background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">'+rows+'</table>'
