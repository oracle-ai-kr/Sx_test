...
async function runAnalysis(stock){
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  // [S393] 코인 시장심리(공포탐욕) 비동기 — '시장·리스크' 그룹
  if(currentMarket === 'coin'){
    _computeFearGreedScore().then(fg=>{
      if(fg!=null) _sxBoardSetAsync('fund','시장심리',fg.v,{byValue:true});
    }).catch(()=>{});
    // [S357] BTC 도미넌스 — '시장·리스크' (BTC 정방향 / 알트 뒤집기, byValue)
    _computeBtcDominanceScore(stock).then(dm=>{
      if(dm!=null) _sxBoardSetAsync('fund','도미넌스',dm.v,{byValue:true,_dom:dm._dom,_isBtc:dm._isBtc});
    }).catch(()=>{});
    // [S357] 김치프리미엄 — '시장·리스크' (과열↑ → 점수↓, byValue)
    _computeKimchiScore().then(km=>{
      if(km!=null) _sxBoardSetAsync('fund','김프',km.v,{byValue:true,_km:km._km});
    }).catch(()=>{});
    // [S357] 코인 공시(상폐) — '시장·리스크' 전광판 + 부문별 점수 카드 동시 갱신
...
function calcBtScore(btData){
  ...
  // ⑥ 거래수 신뢰도 (0~10점) — 통계 충분할수록 가점, 부족하면 페널티
  //   trades 0은 상단에서 이미 null 리턴 처리됨
  let tradesScore;
  if(trades >= 20)      tradesScore = 10;
  else if(trades >= 10) tradesScore = 7;
  else if(trades >= 5)  tradesScore = 3;
  else                  tradesScore = 0;

  return Math.round(Math.min(100, Math.max(0, pnlScore + wrScore + pfScore + mddScore + evScore + tradesScore)));
}
...
function _buildR1S1Card(stock, indicators){
  ...
  const _r1 = _pivot?.R1 ? Math.round(_pivot.R1) : null;
  const _s1 = _pivot?.S1 ? Math.round(_pivot.S1) : null;
  if(!_r1 && !_s1) return '';
  return `<div style="margin:0 0 8px;padding:8px 12px;background:#fff;border:1px solid var(--border);border-radius:10px;display:flex;justify-content:space-between;align-items:center">
    ...
    <span style="font-size:10px;color:var(--text3);font-weight:700;letter-spacing:.3px">R1 · S1</span>
...
function _buildAEntryLine(stock, svVerdict, indicators){
  ...
  // 피봇 저항/지지 — isBuyGroup 여부 무관하게 항상 계산
  const _pivot = indicators?.pivot || indicators?._advanced?.pivot || null;
  const _r1 = _pivot?.R1 ? Math.round(_pivot.R1) : null;
  const _s1 = _pivot?.S1 ? Math.round(_pivot.S1) : null;
  const pivotRow = (_r1 || _s1) ? `
    <div style="display:flex;justify-content:space-between;margin-top:4px;padding:3px 4px;background:var(--surface2);border-radius:5px">
      ...
      <span style="font-size:9px;color:var(--text3)">R1 · S1</span>
  ...
  if(btSt.state === 'holding'){
    ...
    let reasonBadge = '', reasonLine = '';
    {
      ...
      if(_actHold){
        ...
        if(svVerdict.passCount != null) _reasonTxt += ` (4축 ${svVerdict.passCount}/4)`;
        // [S361] 모멘텀 배지 삭제 — 전광판(▲N▼N + 부문 삼각형)에 흡수되어 중복.
        if(_reasonTxt) reasonLine = `<div style="${SUB_STYLE};color:${_actClr}">└ ${_reasonTxt}</div>`;
      }
    }

    return `<div style="${BOX_STYLE}">
      <div style="${TITLE_STYLE}">📊 엔진시뮬 보유중${reasonBadge}${partialBadge}</div>
...
// [S357] 전이 삼각형 — Δ>+2 상승(녹)▲ / Δ<-2 하락(빨)▼ / 그외 생략. neutral=위치값은 회색 방향만
function _sxbTri(d, neutral){
  if(d==null || isNaN(d) || Math.abs(d)<=2) return '';
  const up = d>0;
  const col = neutral ? 'var(--text3)' : (up?'var(--buy)':'var(--sell)');
...
const _SXB_PARAM = new Set(['추세방향','추세신호','변동성','가격모멘텀','반등신호','반등강도','눌림목','볼린저%B','골든크로스','데드크로스','MTF']);
function _sxbCircle(score, label, big, neutral, delta, inverse, byValue, signal, param){
  const r = big?32:18, sw = big?7:5, box=(r+sw)*2;
  ...
  let col;
  if(signal === 'up')        col = sc >= 65 ? 'var(--buy)'  : 'var(--text3)';
  ...
  const fs = big?20:13;
  // [S369] 점수 숫자 색: signal=단방향(≥65만) · inverse=값반전 · byValue=값정방향 · 일반=델타(전이)
  let numColor = 'var(--text)';
  if(signal === 'up'){
    if(sc >= 65) numColor = 'var(--buy)';
  } else if(signal === 'down'){
    if(sc >= 65) numColor = 'var(--sell)';
  } else if(inverse){
    if(sc>=70) numColor='var(--sell)'; else if(sc<=30) numColor='var(--buy)';
  } else if(byValue){
    if(sc>=65) numColor='var(--buy)'; else if(sc<=35) numColor='var(--sell)';
  } else if(!neutral && typeof delta==='number' && Math.abs(delta)>2){
    numColor = delta>0 ? 'var(--buy)' : 'var(--sell)';
  }
  return `<div class="sxb-circle${big?' sxb-big':''}"${label && !big ? ` onclick="_sxbWhy && _sxbWhy('${label}')" style="cursor:pointer"` : ''}>`
    ...
    + `</svg>`
    + (label?`<div class="sxb-circle-label"${param?' style="color:#a855f7"':''}>${label}</div>`:'')
    + `</div>`;
}
...
function _sxbWhipWhy(){
  ...
  // 엔진방향 vs 지표분포 방향 표기 (B 객체엔 distDir 미저장이므로 verdict.momBadge로 추정)
  //   현실에서 휩소 배지는 momDir이 'up' or 'down'일 때만 뜨므로 안전
  const momTxt = momDir === 'up' ? '엔진(▲상승)' : (momDir === 'down' ? '엔진(▼하락)' : '엔진');
  const distTxt = momDir === 'up' ? '지표(▼하락 우세)' : (momDir === 'down' ? '지표(▲상승 우세)' : '지표');
  const reasonTxt = reasons.length ? `\n사유: ${reasons.join(', ')}` : '';
  const msg = `⚡ 휩소주의: ${momTxt} ↔ ${distTxt} 엇갈림${reasonTxt}`;
  if(typeof toast === 'function') toast(msg);
  if(typeof _sxVib === 'function') _sxVib(8);
}
...
function _sxbBuildWhyMsg(it){
  ...
  if(k === '공시'){
    const lv = v<=10?'⛔ 거래지원 종료·상폐':v<=30?'⚠️ 투자 유의 지정':'중립';
    return `📑 공시 ${v}점 — ${lv}`;
  }
  // 단방향 GC (signal='up')
  if(it.signal === 'up'){
  ...
  if(typeof it.d === 'number'){
    if(it.d > 2){
      const prev = Math.round(v - it.d);
      return `🟢 ${k}: 어제 ${prev} → 오늘 ${v} (Δ +${Math.round(it.d)})`;
    }
    if(it.d < -2){
      const prev = Math.round(v - it.d);
      return `🔴 ${k}: 어제 ${prev} → 오늘 ${v} (Δ ${Math.round(it.d)})`;
    }
    return `${k} ${v}점 — 어제 대비 큰 변화 없음`;
  }
  // 그 외 (펀더멘털 등 델타 없는 항목)
  return `${k} ${v}점`;
}
...
async function _computeFearGreedScore(){
  try{
    const base = (typeof WORKER_BASE!=='undefined') ? WORKER_BASE : 'https://stock-signal-proxy.cheaheechang.workers.dev';
    const target = 'https://api.alternative.me/fng/?limit=1';
    const res = await fetch(`${base}/proxy?url=${encodeURIComponent(target)}`, {signal:AbortSignal.timeout(8000)});
    if(!res.ok) return null;
    const j = await res.json();
    const d = j && j.data && j.data[0];
    if(!d) return null;
    return { v: Math.max(0,Math.min(100, parseInt(d.value,10)||50)), cls: d.value_classification };
  }catch(e){ return null; }
}
// [S358] 코인 거시지표 캐시 — 도미넌스(BTC.D%)·김프(%)는 시장 전체값이라 종목 무관.
//   재검색마다 coingecko/coinbase를 때리면 rate limit(429)에 걸려 도넛이 빠지므로 7분 TTL 캐시.
//   실패 시에도 stale 캐시가 있으면 그대로 사용(stale-while-error) → 누락 방지.
let _coinMacroCache = { domPct:null, domTs:0, kmPct:null, kmTs:0 };
const _COIN_MACRO_TTL = 7*60*1000;
// [S357→S358] BTC 도미넌스 — coingecko /global (market_cap_percentage.btc) · 7분 캐시
//   점수화: BTC.D 38~62% → 0~100. 방향 분기(byValue 통일):
//     · BTC 종목 : 도미넌스↑ = 강세 → score=norm    · 알트 종목 : 도미넌스↑ = 약세 → score=100-norm
async function _computeBtcDominanceScore(stock){
  ...
  const now = Date.now();
  if(_coinMacroCache.domPct!=null && (now - _coinMacroCache.domTs) < _COIN_MACRO_TTL)
    return _toScore(_coinMacroCache.domPct);
  try{
    const base = (typeof WORKER_BASE!=='undefined') ? WORKER_BASE : 'https://stock-signal-proxy.cheaheechang.workers.dev';
    ...
    if(res.ok){
      const j = await res.json();
      const dom = j && j.data && j.data.market_cap_percentage && j.data.market_cap_percentage.btc;
      if(dom!=null && isFinite(dom)){
        _coinMacroCache.domPct = dom; _coinMacroCache.domTs = now;
        return _toScore(dom);
      }
    }
  }catch(e){}
  // 실패: stale 캐시라도 있으면 사용(TTL 무시) — 재검색 시 누락 방지
  if(_coinMacroCache.domPct!=null) return _toScore(_coinMacroCache.domPct);
  return null;
}
// [S357→S358→S359] 김치프리미엄 — 업비트 KRW-BTC vs 코인베이스 글로벌 KRW환산가 (= 업비트 프리미엄)
//   〔이력〕 S357 coingecko(rate limit) → S358 바이낸스(워커 IP 'restricted location' 차단) →
//          S359 코인베이스 exchange-rates(currency=BTC). 미국 거래소라 워커 차단 없음 + KRW 환산값 직접 제공
//          → 별도 환율 호출 불필요(네이버 환율 장애도 회피). 호출 2개(업비트+코인베이스). 7분 캐시 + stale 폴백.
//   검증: 업비트/coinbase KRW 비교 = 업비트 앱 "업비트 프리미엄"과 거의 일치(±0.1%).
//   점수화: 김프 -3%~+5% → 0~100 후 뒤집기(score=100-norm). 과열(고김프)↑ → 점수↓ = 빨강/▼
async function _computeKimchiScore(){
  const _toScore = (km)=>{
    const norm = Math.max(0, Math.min(100, Math.round((km + 3) / (5 + 3) * 100)));
    return { v: 100 - norm, _km: km };
  };
  const now = Date.now();
  if(_coinMacroCache.kmPct!=null && (now - _coinMacroCache.kmTs) < _COIN_MACRO_TTL)
    return _toScore(_coinMacroCache.kmPct);
  try{
    const base = (typeof WORKER_BASE!=='undefined') ? WORKER_BASE : 'https://stock-signal-proxy.cheaheechang.workers.dev';
    const cbTarget = 'https://api.coinbase.com/v2/exchange-rates?currency=BTC';
    const [upJ, cbJ] = await Promise.all([
      fetch(`${base}/upbit/ticker?markets=KRW-BTC`, {signal:AbortSignal.timeout(8000)}).then(r=>r.ok?r.json():null).catch(()=>null),
      fetch(`${base}/proxy?url=${encodeURIComponent(cbTarget)}`, {signal:AbortSignal.timeout(8000)}).then(r=>r.ok?r.json():null).catch(()=>null),
    ]);
    const upbit = Array.isArray(upJ) && upJ[0] && upJ[0].trade_price;
    const globalKrw = cbJ && cbJ.data && cbJ.data.rates && parseFloat(cbJ.data.rates.KRW);
    if(upbit && globalKrw && isFinite(upbit) && isFinite(globalKrw)){
      const km = (upbit / globalKrw - 1) * 100;
      _coinMacroCache.kmPct = km; _coinMacroCache.kmTs = now;
      return _toScore(km);
    }
  }catch(e){}
  if(_coinMacroCache.kmPct!=null) return _toScore(_coinMacroCache.kmPct);
  return null;
}
...
function _computeMtfScore(){
  ...
  const _W = {day:1.0, week:1.3, month:1.6};
  const _DIR = {strongUp:2, up:1, flat:0, down:-1, strongDown:-2};
  let sum=0, wsum=0, n=0;
  ['day','week','month'].forEach(tf=>{
    const c = _analTFCache[tf];
    if(c && c.dist5 && _DIR[c.dist5.distDir]!=null){
      sum += _DIR[c.dist5.distDir]*_W[tf]; wsum += _W[tf]; n++;
    }
  });
  if(n===0 || wsum<=0) return null;
  const v = Math.round((sum + 2*wsum)/(4*wsum)*100);
  return { v: Math.max(0,Math.min(100,v)), _n:n };
}
function _sxbHTML(){
  ...
  // [S393] MTF (멀티TF 추세 정합) — dist/분포(greens−reds) 산출 후 모멘텀 그룹에 편입.
  //   dist는 MTF 제외(MTF가 dist 종합이라 순환 방지) · total/개수엔 기여(byValue, _noCount 없음)
  {
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    + _sxbCircle(total, '', true, false, headColorDelta)
    + `<div class="sxb-head-info">`
    + `<div class="sxb-head-tone" style="color:${tone.c}">${tone.t}</div>`
    + `<div class="sxb-head-badges">${transBadge}${whipBadge}</div>`
    + `<div class="sxb-head-sub">종합 ${total} · ${_sxbGrade(total)}등급 · ${_cnt}개 지표 · <span style="color:#16a34a;font-weight:700">▲${greens}</span> <span style="color:#ef4444;font-weight:700">▼${reds}</span></div>`
    + `</div>`
...
function _buildBoardGroups(scores, sv4, structPos, pbScore, D, extras){
  ...
  if(extras.gc && extras.gc.v!=null) g5.push({k:'골든크로스', v:extras.gc.v, signal:'up', fired:extras.gc._fired||[]});
  if(extras.dc && extras.dc.v!=null) g5.push({k:'데드크로스', v:extras.dc.v, inverse:true, signal:'down', fired:extras.dc._fired||[]});
  return [
...
function _classifyBoardDist(groups){
  ...
  (groups||[]).forEach(g=>g.items.forEach(it=>{
    ...
    } else if(!it.neutral && typeof it.d==='number'){
      if(it.d > 2) greens++;
      else if(it.d < -2) reds++;
    }
  }));
...
function _computeBoardInputs(stock, indicators, scores, scoreMom, tf){
  ...
  try {
    ...
    if(indicators){
      ...
      // [S367/S375] 상태형 골든크로스
      if(indicators.obv && indicators.obv.div === 'bullish'){ _gcW += 2; _gcFired.push('OBV 상승다이버전스'); }
      if(indicators.rsi && indicators.rsi.div === 'bullish'){ _gcW += 2; _gcFired.push('RSI 상승다이버전스'); }
      if(indicators.squeeze && indicators.squeeze.squeeze && indicators.maAlign && indicators.maAlign.bullish){ _gcW += 1; _gcFired.push('스퀴즈+정배열'); }
      // [S369/S375] 상태형 데드크로스
      if(indicators.obv && indicators.obv.div === 'bearish'){ _dcW += 2; _dcFired.push('OBV 하락다이버전스'); }
      if(indicators.rsi && indicators.rsi.div === 'bearish'){ _dcW += 2; _dcFired.push('RSI 하락다이버전스'); }
      if(indicators.squeeze && indicators.squeeze.squeeze && indicators.maAlign && indicators.maAlign.bearish){ _dcW += 1; _dcFired.push('스퀴즈+역배열'); }
    }
    ...
    if(_rowsB357 && _rowsB357.length>=61 && typeof calcIndicators==='function'){
      ...
      if(indicators.rsi && _prevInd357.rsi && indicators.rsi.val!=null && _prevInd357.rsi.val!=null
         && indicators.rsi.val >= 30 && _prevInd357.rsi.val < 30){ _gcW += 3; _gcFired.push('RSI 과매도 반등 ★'); }
      // ★★ 중요 (×2)
      ...
      if(indicators.rsi && _prevInd357.rsi && indicators.rsi.val!=null && _prevInd357.rsi.val!=null
         && indicators.rsi.val < 70 && _prevInd357.rsi.val >= 70){ _dcW += 3; _dcFired.push('RSI 과매수 이탈 ★'); }
      // ★★ 중요 (×2)
      if(indicators.stoch && _prevInd357.stoch
         && indicators.stoch.k!=null && indicators.stoch.d!=null
         && _prevInd357.stoch.k!=null && _prevInd357.stoch.d!=null
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  extras = extras || {};
  window._sxBoard = { verdict: verdict||null, lowConf: !!lowConf, lowConfReasons: (extras && extras._lowConfReasons) || [], groups: _buildBoardGroups(scores, sv4, structPos, pbScore, D, extras) };
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        if(_v.targetPrice != null && _v.targetDiffPct != null){
          ...
          _tpHTML = `<div class="anal-target-price" style="margin-top:4px;letter-spacing:-.2px;line-height:1.3">`
            + `<div style="font-size:11px;font-weight:700;color:${_tpColor}">목표주가 ${_tpStr}</div>`
      ...
      <div style="margin-top:10px;padding:10px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid ${
        (()=>{ const sc=calcBtScore(_btR); return sc==null?'#94a3b8':sc>=70?'#22c55e':sc>=50?'#3b82f6':sc>=30?'#f97316':'#e8365a'; })()
      }">
        ...
        ${(()=>{
          ...
          if(_btSc != null){
            ...
            _reliabilityHTML = `<div style="display:flex;align-items:center;gap:6px;margin-top:8px;padding:6px 8px;background:var(--surface);border-radius:6px">
              <span style="font-size:10px;color:var(--text3);font-weight:700;white-space:nowrap">BT신뢰도</span>
...
function _renderFundSectorRow(stock){
  ...
  try{
    ...
    // [S365] PER 밸류(적정가) — 적정주가 괴리율 기반 가치매력도. 저평가=높은점수(녹)/고평가=낮은점수(빨)
    //   _sxBoardSetAsync가 inverse 미지원 → 점수를 (50 - 괴리율%)로 정의해 정방향 색 활용
    //   괴리율(diffPct): 양수=고평가 → 낮은 점수 / 음수=저평가 → 높은 점수
    try {
      if(typeof SXI.valuationJudge === 'function' && stock._financial.eps > 0){
        let _hp = null;
        if(typeof SXI.calcHistoricalPer === 'function' && stock._financial._historicalEps){
          const _cdl = stock._lastAnalCandles || [];
          if(_cdl.length > 0) _hp = SXI.calcHistoricalPer(stock._financial._historicalEps, _cdl);
        }
        let _mp = null;
        ...
        const _vv = SXI.valuationJudge(stock._financial, stock.price, stock, _hp, _mp);
        if(_vv && !_vv.unsuitable && _vv.diffPct != null){
          _sxBoardSetAsync('fund','적정가', Math.max(0, Math.min(100, Math.round(50 - _vv.diffPct))));
        }
      }
    } catch(_){}
...
function _renderAnalTfChips(){
  ...
  tfs.forEach(t => {
    ...
    let icon = '';
    if(cacheHit && cacheHit.dist5) icon = `<span class="atf-icon" title="${cacheHit.dist5.label}">${cacheHit.dist5.dot}</span>`;
