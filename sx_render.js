...
function _sxbCircle(score, label, big, neutral){
...
function _sxbHTML(){
  ...
  let all=[]; groups.forEach(g=>g.items.forEach(it=>{ if(!it.neutral&&typeof it.v==='number') all.push(it.v); }));
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    ...
    + `<div class="sxb-head-info"><div class="sxb-head-tone" style="color:${tone.c}">${tone.t}</div>`
    + `<div class="sxb-head-sub">종합 ${total} · ${_sxbGrade(total)}등급 · ${all.length}개 지표</div></div>`
    + `<span class="sxb-arrow">▶</span></div>`;
  // 미니: 섹션 평균 (접힘 상태에서 노출)
  let mini = `<div class="sxb-mini">`;
  groups.forEach(g=>{ const a=_sxbAvg(g.items); mini += _sxbCircle(a==null?0:a, g.title, false, a==null); });
  mini += `</div>`;
  // 상세: 섹션 카드 (펼침 상태에서 노출)
  ...
  groups.forEach(g=>{
    ...
    g.items.forEach(it=>{ det += _sxbCircle(it.v, it.k, false, !!it.neutral); });
    det += `</div></div>`;
  });
  det += `</div>`;
  return h + mini + det;
...
function _buildScoreBoard(scores, sv4, structPos, pbScore){
  const g1=[], g2=[], g3=[], g4=[];
  // ① 추세·구조
  if(sv4 && sv4.trendScore!=null) g1.push({k:'추세방향', v:sv4.trendScore});
  if(scores && scores.trend!=null) g1.push({k:'추세신호', v:scores.trend});
  if(structPos!=null) g1.push({k:'구조위치', v:structPos, neutral:true});
  // ② 모멘텀·진입
  if(scores && scores.momentum!=null) g2.push({k:'가격모멘텀', v:scores.momentum});
  if(sv4 && sv4.readyScore!=null) g2.push({k:'반등신호', v:sv4.readyScore});
  if(sv4 && sv4.entryScore!=null) g2.push({k:'반등강도', v:sv4.entryScore});
  if(pbScore!=null) g2.push({k:'눌림목', v:pbScore});
  // ③ 수급·거래량
  if(scores && scores.volume!=null) g3.push({k:'거래량', v:scores.volume});
  // ④ 펀더멘털 (밸류=동기 / 재무·공시=비동기 확장)
  if(scores && scores.value!=null) g4.push({k:'밸류', v:scores.value});
  window._sxBoard = { groups:[
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  body.innerHTML = `
    ${chartHTML}
    ${btStateBannerHTML}
    ${_buildScoreBoard(scores, stock._svScores4, _boardStruct, _boardPb)}
    ${tradeHistHTML}
