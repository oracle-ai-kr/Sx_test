// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S829 · build 20260630-2150 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _deadcatConfirmBracket(mk, onProgress){
  ...
  var CAP=130, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
  var imm={r:0,t:0,real:0}, c1={r:0,t:0,real:0}, c2={r:0,t:0,real:0}, stocksUsed=0;
  for(var i=0;i<use.length;i++){
    var s=use[i];
    if(onProgress) onProgress(i+1, use.length, s.name);
    await _trendBatchSleep(0);
    ...
    for(var j=0;j<cs.length;j++){ var rec=cs[j];
      if(rec.imm){ imm.t++; imm.r+=rec.imm.ret; if(rec.imm.real)imm.real++; _u=true; }
      if(rec.c1){ c1.t++; c1.r+=rec.c1.ret; if(rec.c1.real)c1.real++; }
      if(rec.c2){ c2.t++; c2.r+=rec.c2.ret; if(rec.c2.real)c2.real++; }
    }
    if(_u) stocksUsed++;
    await _trendBatchSleep(10);
  }
  if(imm.t<20) return { ok:false, reason:'deadcat-real 확정 표본 20 미만 (n='+imm.t+')' };
  return { ok:true, imm:imm, c1:c1, c2:c2, stocksUsed:stocksUsed };
}
...
function _deadcatConfirmRender(res, mk){
  ...
  if(!res) return '<div style="font-size:9px;color:'+T3+'">—</div>';
  if(!res.ok) return '<div style="font-size:9.5px;color:'+AMB+';padding:6px">'+(res.reason||'측정 실패')+'</div>';
  function stat(g){ return {n:g.t, rate:g.t?g.real/g.t*100:null, avg:g.t?g.r/g.t*100:null}; }
  var I=stat(res.imm), C1=stat(res.c1), C2=stat(res.c2);
  var grpDef=[ ['즉시 진입 (신호봉)', I, false], ['t+1 확인 진입', C1, true], ['t+2 확인 진입', C2, true] ];
  var rows='';
  for(var g=0;g<grpDef.length;g++){
    var nm=grpDef[g][0], st=grpDef[g][1], conf=grpDef[g][2];
    var small=st.n<15;
    var rcol=(st.rate==null||small)?T3:(st.rate>=60?GRN:(st.rate>=45?AMB:RED));
    var acol=(st.avg==null)?T3:(st.avg>=0?GRN:RED);
    var avgStr=(st.avg!=null)?((st.avg>=0?'+':'')+st.avg.toFixed(1)+'%'):'—';
    rows+='<tr>'
      +'<td style="padding:6px 8px;font-weight:'+(conf?'700':'800')+';color:var(--text);font-size:10.5px">'+nm+'</td>'
      +'<td style="padding:6px 8px;text-align:right;color:'+T3+';font-size:9.5px">n='+st.n+(small?'⚠':'')+'</td>'
      +'<td style="padding:6px 8px;text-align:right;font-weight:700;color:'+rcol+';font-size:11px">'+(st.rate!=null?st.rate.toFixed(0)+'%':'—')+'</td>'
      +'<td style="padding:6px 8px;text-align:right;font-weight:800;color:'+acol+';font-size:12px">'+avgStr+'</td>'
      +'</tr>';
  }
  // 판정: 확인 진입 평균N10 vs 즉시 (진입가 상승 비용 반영됨)
  var v='';
  if(I.avg!=null && C1.avg!=null){
    var d1=C1.avg-I.avg, d2=(C2.avg!=null)?C2.avg-I.avg:null;
    var best='t+1', bestD=d1;
    if(d2!=null && d2>bestD){ best='t+2'; bestD=d2; }
    if(bestD>=0.5) v='<span style="color:'+GRN+'">✅ <b>'+best+' 확인 진입이 즉시보다 평균 N10 +'+bestD.toFixed(1)+'%p 높음</b> — 진입가 올라간 비용보다 가짜 거름 이득이 큼. <b>확인 후 진입 우세</b> → BT로 총수익 확인 단계</span>';
    else if(d1<=-0.5) v='<span style="color:'+RED+'">⚠️ 확인 진입이 즉시보다 평균 N10 '+d1.toFixed(1)+'%p <b>낮음</b> — 진입가 상승이 가짜 거름 이득을 까먹음. <b>즉시 진입이 나음</b></span>';
    else v='<span style="color:'+AMB+'">확인 진입과 즉시 진입 평균 N10 차이 미미(Δ'+d1.toFixed(1)+'%p) — 진짜비율↑가 진입가↑로 상쇄. 수익상 큰 차이 없음</span>';
  }
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px">deadcat-real 신호 후 <b>확인 진입</b> 비교: <b>즉시</b>=신호봉 종가 진입 · <b>t+1/t+2 확인</b>=그 봉 종가가 신호봉보다 <b>위(살아남음)</b>일 때만 그 종가에 진입. <b style="color:var(--text2)">평균 N10</b>이 핵심(진입가 상승 비용 반영). 진짜비율↑여도 진입가↑면 수익은 상쇄될 수 있음. <b style="color:'+AMB+'">⚠한계</b>: 확인 진입은 빠진 봉(일부 진짜)을 포기 → 진입당 수익↑여도 전체 기회는 줄 수 있어 BT 필요. 대표+관심+top100·'+(res.stocksUsed||0)+'종목.</div>';
  return head
    +'<table style="width:100%;border-collapse:collapse;background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden">'
    +'<tr style="background:var(--surface2)"><td style="padding:5px 8px;font-size:8px;color:'+T3+'">진입 방식</td><td style="padding:5px 8px;text-align:right;font-size:8px;color:'+T3+'">진입수</td><td style="padding:5px 8px;text-align:right;font-size:8px;color:'+T3+'">진짜비율</td><td style="padding:5px 8px;text-align:right;font-size:8px;color:'+T2+'">평균N10</td></tr>'
    +rows+'</table>'
