...
function _trendCfg(market){
  ...
  let cfg={ s:d[0], l:d[1], n:3, aux:{}, sell:{}, reentry:false };
  try { const raw=localStorage.getItem('SX_TREND_'+market); if(raw){ const o=JSON.parse(raw); if(o){ cfg.s=+o.s||d[0]; cfg.l=+o.l||d[1]; cfg.n=+o.n||3; cfg.aux=(o.aux&&typeof o.aux==='object')?o.aux:{}; cfg.sell=(o.sell&&typeof o.sell==='object')?o.sell:{}; cfg.reentry=!!o.reentry; } } } catch(_){}
...
function _trendBt(rows,cfg,bbP){
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){
      if(gc && auxOk(i)) pos={entry:close[i],entryIdx:i,re:false};
      // [S570] 재진입 — 실제 청산 이력(_lastExitIdx>=0) + 정배열 유지(maS>maL) + 청산 후 1봉 대기 + 매도조건 해제 + 매수조건 재충족
      //   _lastExitIdx>=0 가드: "청산 후 재진입"이 본질 → 콜드스타트(이미 정배열인 추세에 첫 봉부터 진입) 방지.
      else if(_reOn && _lastExitIdx>=0 && maS[i]>maL[i] && (i-_lastExitIdx>1) && !sellHit(i) && auxOk(i)) pos={entry:close[i],entryIdx:i,re:true};
    }
    else if(dc || sellHit(i)){ const pnl=(close[i]/pos.entry-1)*100; trades.push({entry:pos.entry,exit:close[i],pnl:+pnl.toFixed(2),bars:i-pos.entryIdx,entryIdx:pos.entryIdx,exitIdx:i,re:!!pos.re,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||'',exitDate:(rows[i]&&(rows[i].date||rows[i].t))||''}); pos=null; _lastExitIdx=i; }
  }
  let open=null;
  if(pos){ const last=close[n-1]; open={entry:pos.entry,cur:last,pnl:+((last/pos.entry-1)*100).toFixed(2),bars:(n-1)-pos.entryIdx,entryIdx:pos.entryIdx,re:!!pos.re,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||''}; }
  // 통계
...
function _trendState(rows,cfg,bt){
  ...
  if(lastGc>=0 && lastGc>lastDc){ const ago=(n-1)-lastGc; if(ago<=N) return { label:`🟢 진입 신호 (골든크로스 ${ago}봉 전 매수)`, color:'#22c55e' }; return { label:`상승 추세 (골든크로스 ${ago}봉 전 매수, 신호창 지남)`, color:'var(--text2)' }; }
...
function _trendRenderInner(){
  ...
  const bt=_trendBt(rows,cfg,_trendBbParams(market));
  ...
  const note=`<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · ${cfg.s}MA×${cfg.l}MA 골든크로스 진입${_buyN?` · 매수 ${_buyN}개 AND(최근${cfg.n}봉)`:' · MA 단독'} → 데드크로스${_sellN?` OR 매도 ${_sellN}개`:''} 청산${cfg.reentry?' · 🔄정배열 재진입':''}</div>`;
  return head+maRow+chipRow+reChip+stateRow+grid+note;
}
...
function _trendRedrawChart(){
  ...
  try{
    ...
    var _sv=(typeof currentAnalStock!=='undefined' && currentAnalStock) ? (currentAnalStock._svChartMarker||currentAnalStock._svVerdict) : null;
...
function _trendReset(){ const ctx=window._sxTrendCtx; if(!ctx) return; const m=ctx.market; const d=_TREND_MKT_DEFAULT[m]||[5,9]; _trendSave(m,{ s:d[0], l:d[1], n:3, aux:{}, sell:{}, reentry:false }); _trendRerender(); _trendRedrawChart(); }
if(typeof window!=='undefined'){ window._trendApply=_trendApply; window._trendToggleAux=_trendToggleAux; window._trendToggleSell=_trendToggleSell; window._trendToggleReentry=_trendToggleReentry; window._trendToggleBuyOpen=_trendToggleBuyOpen; window._trendToggleSellOpen=_trendToggleSellOpen; window._trendReset=_trendReset; }
...
function _trendOpenDetail(){
  ...
  const cfg=_trendCfg(ctx.market); const bt=_trendBt(ctx.rows,cfg,_trendBbParams(ctx.market));
  _trendCloseDetail();
