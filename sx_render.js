// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S830 · build 20260630-2220 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _deadcatTrajBracket(mk, onProgress){
  if(!(window.SXCandleBT&&SXCandleBT.fetchRows600)) return { ok:false, reason:'캔들 fetch 미연결' };
  ...
  for(var i=0;i<use.length;i++){
    ...
    var rows=null; try{ rows=await SXCandleBT.fetchRows600(mk,'day',s.code); }catch(e){}
    if(!Array.isArray(rows)||rows.length<_floor){ await _trendBatchSleep(6); continue; }
    var tr=null; try{ tr=await SXRecipeSignal.deadcatTrajScan(s.code, rows); }catch(e2){}
...
async function _deadcatTrajRun(){
  var el=document.getElementById('deadcatTrajResult'); if(!el) return;
  var mk=(typeof currentMarket!=='undefined')?currentMarket:'kr';
  el.style.display='block';
  el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">📉 진입 후 궤적 측정 중...</div>';
  var res;
  try{ res=await _deadcatTrajBracket(mk,function(i,t,n){ el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">📉 스캔 '+i+'/'+t+' · '+(n||'')+'</div>'; }); }catch(e){ res={ok:false,reason:String(e&&e.message||e)}; }
  el.innerHTML=_deadcatTrajRender(res, mk);
}
function _deadcatTrajRender(res, mk){
  var GRN='#16a34a', RED='#dc2626', AMB='#d97706', T2='var(--text2)', T3='var(--text3)';
  if(!res) return '<div style="font-size:9px;color:'+T3+'">—</div>';
  if(!res.ok) return '<div style="font-size:9.5px;color:'+AMB+';padding:6px">'+(res.reason||'측정 실패')+'</div>';
  var rA=[], fA=[], gap=[];
  ...
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px">deadcat-real 발동(진입) 후 <b>봉별 누적수익</b>을 N10 결과로 <b style="color:'+GRN+'">진짜</b>/<b style="color:'+RED+'">가짜</b> 분리. <b>언제 갈라지나</b>=조기 손절 지점. <b>t+1~5=라벨 독립</b>(진짜 조기감지)·t+6~10=라벨이 이 구간 결과라 순환(흐림·반투명). 진짜 n='+res.rN+'·가짜 n='+res.fN+'·'+(res.stocksUsed||0)+'종목.</div>';
...
async function _deadcatConfirmBracket(mk, onProgress){
  if(!(window.SXCandleBT&&SXCandleBT.fetchRows600)) return { ok:false, reason:'캔들 fetch 미연결' };
  ...
  for(var i=0;i<use.length;i++){
    ...
    var rows=null; try{ rows=await SXCandleBT.fetchRows600(mk,'day',s.code); }catch(e){}
    if(!Array.isArray(rows)||rows.length<_floor){ await _trendBatchSleep(6); continue; }
    var cs=null; try{ cs=await SXRecipeSignal.deadcatConfirmScan(s.code, rows); }catch(e2){}
...
async function _deadcatConfirmRun(){
  var el=document.getElementById('deadcatConfirmResult'); if(!el) return;
  var mk=(typeof currentMarket!=='undefined')?currentMarket:'kr';
  el.style.display='block';
  el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">🎯 확인 진입 측정 중...</div>';
  var res;
  try{ res=await _deadcatConfirmBracket(mk,function(i,t,n){ el.innerHTML='<div style="padding:10px;font-size:11px;color:var(--text2)">🎯 스캔 '+i+'/'+t+' · '+(n||'')+'</div>'; }); }catch(e){ res={ok:false,reason:String(e&&e.message||e)}; }
  el.innerHTML=_deadcatConfirmRender(res, mk);
}
function _deadcatConfirmRender(res, mk){
