 ...
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  if (currentMarket === 'us' && stock.code) {
    ...
    if (_nd) {
      ...
      //   stock._tradeAmountDisplay에 원본 보관 (분석탭에서 그대로 표시 가능, [S269]에서 활용)
      if (_nd.tradingValueDisplay && !stock._tradeAmountDisplay) stock._tradeAmountDisplay = _nd.tradingValueDisplay;
      // 메타 정보 — 한글명/로고/시총 원화환산/거래소 등 ([S269] 분석탭 렌더링 차별화용)
...
async function _runEngineVerify(stock){
  ...
  try {
    ...
    try{
      ...
      console.log(`[S245] [분석탭] ${stock.name||stock.code} BT호출옵션: applyRegimeAdjust=${opts.applyRegimeAdjust} · slippage=${opts.slippage} · nextBarEntry=${opts.nextBarEntry}`);
      console.log(`[S245] [분석탭] ${stock.name||stock.code} 전역상태: regimeAdaptEnabled=${_raEn} · applySafetyToBt=${!!SXE._applySafetyToBt} · btEntryGates=${!!SXE._btEntryGates} · btEarlyExit=${!!(SXE._btEarlyExit && SXE._btEarlyExit.enabled)}`);
      console.log(`[S245] [분석탭] ${stock.name||stock.code} BT파라미터: ${JSON.stringify(params)}`);
      console.log(`[S245] [분석탭] ${stock.name||stock.code} _loadAnalParams: ${JSON.stringify(_ap)}`);
    ...
    try{
      ...
      if(_idx530 >= 0){
        ...
        console.log(`[S249] [분석탭] ${stock.name||stock.code} 20250530+30봉 close (idx ${_idx530}~${_idx530+_slice.length-1}/${rows.length}): ${_closes}`);
      } else {
        console.log(`[S249] [분석탭] ${stock.name||stock.code} 20250530 봉 인덱스 없음 (rows ${rows.length}봉)`);
      }
    ...
    try{
      ...
      console.log(`[S250] [분석탭] ${stock.name||stock.code} close 비정상 봉: ${_badRows.length}개 / 전체 ${rows.length}봉`);
      if(_badRows.length > 0){
        ...
        console.log(`[S250] [분석탭] ${stock.name||stock.code} 손상샘플(앞 5개): ${_samples}`);
        // 인덱스 패턴 (연속/산발 확인)
        const _allIdx = _badRows.map(({i}) => i);
        console.log(`[S250] [분석탭] ${stock.name||stock.code} 손상 인덱스 전체: [${_allIdx.join(',')}]`);
      }
    ...
    if(r && !r.error){
      console.log(`[S245] [분석탭] ${stock.name||stock.code} raw결과: rowsLength=${r.rowsLength||rows.length} 거래=${r.totalTrades} 승률=${(r.winRate||0).toFixed(1)}% 수익=${(r.totalPnl||0).toFixed(2)}% MDD=${r.mdd||0}% PF=${r.profitFactor||0} 게이트차단=${r.gateBlocks||0}`);
      ...
      try{
        ...
        console.log(`[S246] [분석탭] ${stock.name||stock.code} 진입목록(${(r.trades||[]).length}건): ${_trades}`);
      ...
      try{
        ...
        console.log(`[S247] [분석탭] ${stock.name||stock.code} 거래상세: ${_details}`);
      ...
      try{
        ...
        console.log(`[S248] [분석탭] ${stock.name||stock.code} 청산상세: ${_exits}`);
