// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v3.1 <!-- S95 -->
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  // S95: 매매이력 HTML 생성 (최근 10건)
  let tradeHistHTML = '';
  const _btTrades = stock._btResult?.trades;
  if(_btTrades && _btTrades.length){
    // 날짜 포맷 헬퍼
    const _fmtD = d => { if(!d) return '—'; const p=d.split(/[-T]/); return p.length>=3 ? p[1]+'/'+p[2] : d.slice(0,10); };
    const _recent = _btTrades.slice(-10).reverse();
    let _listRows = '';
    _recent.forEach(tr => {
      const pnlC = tr.pnl >= 0 ? 'var(--buy)' : 'var(--sell)';
      const typeLbl = tr.type==='WIN'?'익절':tr.type==='LOSS'?'손절':tr.type==='OPEN'?'보유중':'—';
      const typeBg = tr.type==='WIN'?'rgba(34,197,94,.1)':tr.type==='LOSS'?'rgba(232,54,90,.1)':'rgba(59,130,246,.1)';
      _listRows += `<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:2px;padding:5px 0;border-bottom:1px solid var(--border);font-size:10px;align-items:center">
        <div><span style="display:inline-block;padding:1px 4px;border-radius:3px;background:${typeBg};font-size:9px;font-weight:700;color:${pnlC}">${typeLbl}</span></div>
        <div style="color:var(--text2)">${_fmtD(tr.entryDate)}→${_fmtD(tr.exitDate)}</div>
        <div style="text-align:right;color:var(--text2)">${Math.round(tr.entry).toLocaleString()}→${Math.round(tr.exit).toLocaleString()}</div>
        <div style="text-align:right;font-weight:700;color:${pnlC}">${tr.pnl>=0?'+':''}${tr.pnl}% <span style="font-weight:400;color:var(--text3);font-size:8px">${tr.bars}봉</span></div>
      </div>`;
    });
    // 통계 요약
    ...
    tradeHistHTML = `<div style="margin:0 0 8px;padding:10px 12px;background:var(--surface);border:1px solid var(--border);border-radius:10px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:2px;padding:0 0 4px;font-size:8px;color:var(--text3);font-weight:600">
        <div>결과</div><div>기간</div><div style="text-align:right">가격</div><div style="text-align:right">수익률</div>
      </div>
      ${_listRows}
