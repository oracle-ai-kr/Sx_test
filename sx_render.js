...
var SX_BADGE_DEFS = {
  bottom:   { id:'bottom',   pool:'deadcat',  poolLt:'bear', kind:'real', poolName:'역배열+단기약세', icon:'🟢', name:'바닥반등', color:'#16a34a', desc:'역배열·단기약세에서 진짜반등 레시피가 1개라도 발동 = 바닥에서 진짜 돌아섬' },
  deadcat:  { id:'deadcat',  pool:'deadcat',  poolLt:'bear', kind:'fake', poolName:'역배열+단기약세', icon:'🔴', name:'데드캣',   color:'#dc2626', desc:'역배열·단기약세에서 가짜반등(함정) 레시피가 1개라도 발동 = 반짝 후 재하락 경고' },
  pullback: { id:'pullback', pool:'pullback', poolLt:'bull', kind:'real', poolName:'정배열+단기조정', icon:'🔵', name:'눌림목',   color:'#2563eb', desc:'정배열·단기조정에서 진짜눌림목 레시피가 1개라도 발동 = 건강한 조정 후 재상승' },
  retrace:  { id:'retrace',  pool:'pullback', poolLt:'bull', kind:'fake', poolName:'정배열+단기조정', icon:'🟠', name:'되돌림',   color:'#d97706', desc:'정배열·단기조정에서 가짜되돌림(함정) 레시피가 1개라도 발동 = 추세 약화 경고' }
};
...
function _buildBadgeInventoryCard(stock, indicators){
  ...
  var rowsHtml = SX_BADGE_ORDER.map(function(key){
    ...
    if(ev.fired) firedCount++;
    var chip = ev.fired
      ? '<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:'+def.color+';color:#fff;border:1px solid '+def.color+'">'+def.icon+' '+def.name+'</span>'
      : '<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">\u25cb '+def.name+'</span>';
    var statusTxt, statusCol;
    if(!ev.inPool){ statusTxt = ev.matchCount>0 ? ('\uc7ac\ub8cc\ucda9\uc871 '+ev.matchCount+' \u00b7 \ud480 \ubd88\uc77c\uce58') : '\u25cb \ud480 \ubd88\uc77c\uce58'; statusCol=GR; }
    else if(ev.fired){ statusTxt='\u2705 \ubc1c\ub3d9 '+ev.matchCount+'/'+ev.total; statusCol=def.color; }
    else { statusTxt='\u25cf \uad00\ub9dd 0/'+ev.total; statusCol=GR; }
    var recLine;
    if(ev.matchCount>0){
      var top=ev.matched.slice(0,2).map(function(r){ return _e(r.label); }).join('<br>');
      recLine = '<div style="font-size:9px;color:var(--text2);line-height:1.5;margin-top:3px">'+top+(ev.matchCount>2?('<br><span style="color:'+GR+'">\uc678 '+(ev.matchCount-2)+'\uac1c \ucda9\uc871</span>'):'')+'</div>';
    } else {
      recLine = '<div style="font-size:8.5px;color:'+GR+';margin-top:3px">\ucda9\uc871 \ub808\uc2dc\ud53c \uc5c6\uc74c ('+ev.total+'\uac1c \uc911 0)</div>';
    }
    var borderC = ev.fired ? def.color+'66' : 'var(--border)';
    return '<div style="padding:9px 10px;margin:6px 0;border-radius:9px;background:var(--surface2);border:1px solid '+borderC+'">'
      +   '<div style="display:flex;align-items:center;gap:7px;margin-bottom:4px">'+chip
  ...
  var rightHtml = '<span style="font-size:9.5px;font-weight:700;color:'+(firedCount?'#16a34a':'var(--text3)')+'">'+(firedCount? (firedCount+' \ubc1c\ub3d9'):(String(SX_BADGE_ORDER.length)+'\uc885'))+'</span>';
  var desc = '<div style="font-size:8.5px;color:var(--text3);line-height:1.5;margin-bottom:4px">\uc2e0\ud638 \ubc30\uc9c0 <b style="color:var(--text2)">\uc2dc\uc81c\ud488 \ubcf4\uad00\uc18c(SSOT)</b> \u2014 \ub2e4\ub978 \ubc30\uc9c0\uac00 \uac00\uc838\ub2e4 \uc501\ub2c8\ub2e4. <b>4\uc885</b>: \uc5ed\ubc30\uc5f4 \ubc14\ub2e5\ubc18\ub4f1\u00b7\ub370\ub4dc\uce21 / \uc815\ubc30\uc5f4 \ub20c\ub9bc\ubaa9\u00b7\ub418\ub3cc\ub9bc. \uac01 \ubc30\uc9c0 = <b>\uadf8 \ud480\u00d7\uc885\ub958 \ub808\uc2dc\ud53c\uac00 1\uac1c\ub77c\ub3c4 \ubc1c\ub3d9</b>\ud558\uba74 ON. <b style="color:var(--text2)">\ud68c\uc0c9=\ud480 \ubd88\uc77c\uce58 \ub610\ub294 \ubbf8\ubc1c\ub3d9</b>.</div>';
