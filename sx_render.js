...
function _trendRenderInner(){
  ...
  const head=`<div class="cv-toggle" onclick="_sxVib(8);window._cvToggle&&_cvToggle(this,'sxTrendBody')" style="display:flex;align-items:center;gap:6px;cursor:pointer;margin-bottom:${_trOpen?'8px':'0'}"><span class="cv-arrow" style="color:var(--accent);font-size:11px">${_trOpen?'▼':'▶'}</span><span style="font-size:13px;font-weight:800;color:var(--text)">📈 단기 추세 매매</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span><span onclick="event.stopPropagation();_sxVib(12);window._trendOpenDetail&&_trendOpenDetail()" title="거래내역·자산흐름 보기" style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;background:#7c3aed22;color:#7c3aed;border:1px solid #7c3aed66;cursor:pointer">검증</span><span class="cv-hint" style="font-size:9px;color:var(--text3);font-weight:600;margin-left:auto">${_trOpen?'접기':'펼치기'}</span><span style="font-size:10px;font-weight:700;color:${_dCol};margin-left:8px">${_isToday?'':'⚠️ '}${_dTxt} 종가기준 매매</span></div>`;
  // MA 입력
  ...
  return head+`<div id="sxTrendBody" style="display:${_trOpen?'block':'none'}">`+maRow+chipRow+reChip+predRow+entryRow+exitRow+_guardBadge+stateRow+grid+note+batchSection+`</div>`;   // [S699] 본문 접기 래퍼
}
...
function _sxExpCard(outerId, bodyId, titleHtml, rightHtml, bodyHtml){
  ...
  return '<div' + (outerId ? (' id="' + outerId + '"') : '') + ' style="margin:0 0 10px;padding:12px 14px;border-radius:12px;background:var(--surface);border:1px solid var(--border)">'
    ...
    + titleHtml
    + '<span class="cv-hint" style="font-size:9px;color:var(--text3);font-weight:600;margin-left:auto">' + (open ? '접기' : '펼치기') + '</span>'
    + (rightHtml || '')
    + '</div>'
