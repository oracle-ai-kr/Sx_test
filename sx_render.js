...
function _sxbCircle(score, label, big, neutral, delta, inverse, byValue){
  ...
  // [S362] stroke 색: inverse=값 기준 반전(과열 빨/안정 녹) · neutral=회색 · 기본=점수 기반
  const col = inverse ? _sxbColorInv(sc) : (neutral ? 'var(--text3)' : _sxbColor(sc));
  const fs = big?20:13;
  // [S366] 점수 숫자 색: inverse=값 반전(과열/안정) · byValue=값 정방향(≥65 녹/≤35 빨, 시각·분포 일치) · 일반=델타(전이)
...
function _sxbHTML(){
  ...
  // [S364] 평균(종합점수)=neutral 제외 / 분포(▲▼)=시각 일관성 통합:
  //   · 인버스(구조위치): 값 기준 반전 — 과열≥70→▼ · 안정≤30→▲
  //   · byValue(추세강도/A/D/EOM/Chaikin): 값 기준 정방향 — ≥65→▲ · ≤35→▼ (상태 지표라 델타가 0이어도 시각 색 반영)
  //   · 일반(4축/부문): 델타(전이) 기준 — 어제 대비 변화
  let all=[], greens=0, reds=0;
  groups.forEach(g=>g.items.forEach(it=>{
    if(!it.neutral && typeof it.v==='number') all.push(it.v);
    if(it.inverse && typeof it.v==='number'){
      if(it.v >= 70) reds++;
  ...
  groups.forEach(g=>{
    ...
    g.items.forEach(it=>{ det += _sxbCircle(it.v, it.k, false, !!it.neutral, it.d, !!it.inverse, !!it.byValue); });
    det += `</div></div>`;
  });
  det += `</div>`;
  return h + mini + det;
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  ...
  // ⑤ 기술지표 [S367] — 이평선 배열(7단 정밀 상태점수) + 골든크로스(8종 가중치 카운터)
  if(extras.maAlign && extras.maAlign.v!=null) g5.push({k:'이평선 배열', v:extras.maAlign.v, byValue:true});
  if(extras.gc && extras.gc.v!=null) g5.push({k:'골든크로스', v:extras.gc.v, byValue:true});
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  try {
    ...
    let _gcW = 0; // [S367] 골든크로스 가중치 합 (만점 18)
    if(indicators){
      ...
      // [S367] ⑤ 기술지표 — 이평선 배열 7단 점수 (MA5/20/60 정밀 분류)
      ...
      // [S367] 상태형 골든크로스 (★★ OBV/RSI 다이버전스, ★ 스퀴즈+정배열) — 1봉 전 불필요
    ...
    // [S367] ⑤ 기술지표 — 골든크로스 최종 점수 (가중치 합 / 만점 18 × 100)
    _boardExtras.gc = {v: Math.round((_gcW / 18) * 100)};
