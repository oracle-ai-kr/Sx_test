...
function _trendBt(rows,cfg,bbP){
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){
      ...
      if(dc || sellHit(i) || _predFail || _predExit || _earlyMa5 || _earlySlope || _disSl){ const pnl=(close[i]/pos.entry-1)*100; trades.push({entry:pos.entry,exit:close[i],pnl:+pnl.toFixed(2),bars:i-pos.entryIdx,entryIdx:pos.entryIdx,exitIdx:i,re:!!pos.re,pred:!!pos.pred,predLead:pos.predLead||0,fail:!!_predFail,predExit:!!_predExit,early:!!_earlyMa5,slope:!!_earlySlope,disSl:!!_disSl,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||'',exitDate:(rows[i]&&(rows[i].date||rows[i].t))||''}); pos=null; _lastExitIdx=i; }
    }
  }
...
function _trendRenderInner(){
  ...
  const batchSection=`<div style="margin-top:10px;border-top:1px dashed var(--border);padding-top:9px">`
    +`<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span style="font-size:10px;font-weight:800;color:var(--text2);min-width:52px">🧪 배치</span>`+_bsChip(_bsRep,'대표풀','rep')+_bsChip(!_bsRep,'관심목록','watch')+`</div>`
    +`<div style="display:flex;align-items:center;gap:6px;margin-bottom:7px"><span style="font-size:9px;font-weight:700;color:var(--text3);min-width:52px">조기청산</span>`+_bmChip(_bmClose,'종가<MA5','close')+_bmChip(!_bmClose,'MA5기울기','slope')+`</div>`
...
function _trendBatchConfigs(market, mode){
  ...
  const base=function(){ return { s:d[0], l:d[1], n:3, aux:{}, sell:{}, reentry:false, predict:false, predLead:1, nextOpen:false, earlyMa5:false, earlySlope:false, disSl:false, slAtr:2 }; };
