...
var _sxvPredsOn = { slope:1, slopeTurn:1, rsiGc:1, obv:1, macd:1, closeVsMa:1, rsi50:1, macdCross:1, slopeOrRsi:1, slopeAndRsi:1, resBreak:1, hhBreak:1, volSurge:1, align3:1, adx:1, bandWalk:1, andCore:1, andStrong:1, upsideMix:1 };   // [S709→S710] 신호 on/off (기본 전체 on — 19종: +정배열/ADX/밴드워킹/AND2)
...
function _buildValidatorCard(stock, indicators){
  ...
  var predChips = predList.map(function(id){ var on = !!_sxvPredsOn[id]; var lab = (typeof SXUV !== 'undefined' && SXUV.PREDICTORS && SXUV.PREDICTORS[id]) ? SXUV.PREDICTORS[id].label : id; return '<span onclick="_sxVib(6);window._sxvTogglePred&&_sxvTogglePred(\'' + id + '\')" style="font-size:9px;font-weight:700;padding:3px 8px;border-radius:11px;cursor:pointer;border:1px solid ' + (on ? '#0891b2' : 'var(--border)') + ';background:' + (on ? '#0891b21a' : 'var(--surface2)') + ';color:' + (on ? '#0891b2' : 'var(--text3)') + (on ? '' : ';opacity:.55') + '">' + lab + '</span>'; }).join('');
