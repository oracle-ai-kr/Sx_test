...
function _buildAEntryLine(stock, svVerdict, indicators){
  ...
  const isBuyGroup = (action === '매수' || action === '관심' || action === '보유 유지');
  if(!isBuyGroup){
  ...
  // [S294] 보유 유지 상태 + BT tp/sl 있으면 BT 값 우선 사용
  const _btSt = stock._btState;
  const isBtHolding = (action === '보유 유지') && _btSt && _btSt.tp && _btSt.sl;
  const dispTp = isBtHolding ? _btSt.tp : calc.tp;
  const dispSl = isBtHolding ? _btSt.sl : calc.sl;
  const dispEp = calc.ep;
  ...
  return `<div style="${BOX_ACTIVE}">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
      ...
      <span style="font-size:11px;font-weight:800;color:var(--text)">진입가 ${fmt(dispEp)}</span>
    </div>
...
function _sxbCircle(score, label, big, neutral, delta, inverse, byValue, signal){
  ...
  return `<div class="sxb-circle${big?' sxb-big':''}">`
    ...
    + `</svg>`
    + (label?`<div class="sxb-circle-label">${label}</div>`:'')
    + `</div>`;
}
// 점수성 항목만 평균 (neutral=위치값은 제외)
function _sxbAvg(items){
  const vs = items.filter(it=>!it.neutral && typeof it.v==='number').map(it=>it.v);
...
function _sxbHTML(){
  ...
  //   · 일반(4축/부문): 델타 기준
  let all=[], greens=0, reds=0;
  groups.forEach(g=>g.items.forEach(it=>{
    if(!it.neutral && typeof it.v==='number') all.push(it.v);
  ...
  const total = all.length?Math.round(all.reduce((a,b)=>a+b,0)/all.length):0;
  const diff = greens - reds;                                              // 분포 차이
  const distDir = diff>=2 ? 'up' : (diff<=-2 ? 'down' : 'flat');           // 5:2 이상 우세부터 방향 판정
  const tone = _sxbTone(total);
  // [S361] 전이 방향 배지 — 녹/빨 분포 기준 (평균이 0이어도 양극화면 횡보, 분포 우세면 방향)
  let bi, bl, bc;
  if(distDir==='up'){ bi='✨'; bl='상승 시작'; bc='var(--buy)'; }
  else if(distDir==='down'){ bi='⚠'; bl='하락 시작'; bc='#f59e0b'; }
  else { bi='➡'; bl='횡보 구간'; bc='#94a3b8'; }
  const transBadge = `<span class="sxb-badge" style="color:${bc};background:${bc}1A;border:1px solid ${bc}">${bi} ${bl}</span>`;
  // [S361] 휩소주의 — momBadge ↔ 분포 방향 엇갈림 AND 신뢰도 낮음(lowConf)
  let whipBadge = '';
  const _momDir = B.verdict && B.verdict.momBadge && B.verdict.momBadge.direction;
  if(_momDir && (_momDir==='up'||_momDir==='down') && _momDir!==distDir && B.lowConf){
    const _P='#a855f7';
    whipBadge = `<span class="sxb-badge" style="color:${_P};background:${_P}1A;border:1px solid ${_P}">⚡ 휩소주의</span>`;
  }
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    ...
    + `<div class="sxb-head-sub">종합 ${total} · ${_sxbGrade(total)}등급 · ${all.length}개 지표 · ▲${greens} ▼${reds}</div>`
    + `</div>`
  ...
  groups.forEach(g=>{
    ...
    g.items.forEach(it=>{ det += _sxbCircle(it.v, it.k, false, !!it.neutral, it.d, !!it.inverse, !!it.byValue, it.signal); });
    det += `</div></div>`;
  });
  det += `</div>`;
  return h + mini + det;
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  ...
  if(scores && scores.value!=null) g4.push({k:'밸류', v:scores.value});
  ...
  if(extras.gc && extras.gc.v!=null) g5.push({k:'골든크로스', v:extras.gc.v, neutral:true, signal:'up'});
  if(extras.dc && extras.dc.v!=null) g5.push({k:'데드크로스', v:extras.dc.v, neutral:true, signal:'down'});
  window._sxBoard = { verdict: verdict||null, lowConf: !!lowConf, groups:[
    {id:'trend', title:'추세·구조', items:g1},
    {id:'mom',   title:'모멘텀·진입', items:g2},
    {id:'flow',  title:'수급·거래량', items:g3},
    {id:'fund',  title:'펀더멘털', items:g4},
    {id:'tech',  title:'기술지표', items:g5},
  ]};
  return `<div class="sxb" id="sxScoreBoard">${_sxbHTML()}</div>`;
}
// 비동기 점수 도착 시 그리드 확장 (재무·공시) — 펼침 상태 보존
function _sxBoardSetAsync(groupId, key, score){
  ...
  if(ex) ex.v = score; else g.items.push({k:key, v:score});
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  // [S372] indicators에 maAlign 누락 보강 — 모든 진입 경로 커버 (runAnalysis 신규 / cached 재진입 / saveAnalResult 등)
  //   [S371]은 runAnalysis 단일 경로만 fix했으나 cached 경로 등에서 maAlign 없는 indicators가 그대로 들어와
  //   "이평선 (?)" 표시되는 문제 → 여기서 캔들로 calcIndicators 재호출해 maAlign만 보강
  if(indicators && !indicators.maAlign && typeof calcIndicators === 'function'){
    const _candlesFix = stock._lastAnalCandles || _currentAnalRows;
    if(_candlesFix && _candlesFix.length >= 20){
      try {
        const _fresh = calcIndicators(_candlesFix, _analTF);
        if(_fresh && _fresh.maAlign){
          indicators.maAlign = _fresh.maAlign;
          console.log('[S372] maAlign 보강 완료:', _fresh.maAlign);
        }
      } catch(e){ console.warn('[S372] maAlign 보강 실패:', e); }
    }
  }
  ...
  let entryCardHTML = ''; // [S365] 진입가/목표가 카드 — 배너에서 분리 (2단)
  const _btSt = stock._btState || null;
  {
    ...
    // [S365] 배너 해체 — 3단 분리: ①BT 상태 카드(verdict+momBadge만) / ②진입가 카드(별도) / ③전광판
    //   4축 줄("합격 2/4 · 바닥 · 반등 · 추세 · 모멘텀")은 전광판에 모두 흡수돼 있어 제거 (정보 중복 해소)
    btStateBannerHTML = `<div style="padding:10px 14px;margin:0 0 6px;border-radius:12px;background:${_svVerdict.color}0D;border:1.5px solid ${_svVerdict.color}">
    ...
    // [S365] 2단 — 진입가/목표가/저항/지지 (기존 _buildAEntryLine 자체 카드 wrapping 있음)
    entryCardHTML = `<div style="margin:0 0 8px">${_buildAEntryLine(stock, _svVerdict, indicators)}</div>`;
  }
  ...
  try {
    const _mom357 = stock._scoreMomentum;
    ...
    const _scoreDir362 = (d)=>d==='up'?70:(d==='down'?30:50);
    const _scoreVal362 = (v)=>v>0?70:(v<0?30:50);
    ...
    if(indicators){
      ...
      if(indicators.ad && indicators.ad.trend) _boardExtras.ad = {v:_scoreDir362(indicators.ad.trend)};
      if(indicators.eom && indicators.eom.trend) _boardExtras.eom = {v:_scoreDir362(indicators.eom.trend)};
      if(indicators.chaikinOsc && indicators.chaikinOsc.val!=null) _boardExtras.chaikin = {v:_scoreVal362(indicators.chaikinOsc.val)};
      ...
      // [S367] 상태형 골든크로스 (★★ OBV/RSI 다이버전스, ★ 스퀴즈+정배열)
      if(indicators.obv && indicators.obv.div === 'bullish') _gcW += 2;
      if(indicators.rsi && indicators.rsi.div === 'bullish') _gcW += 2;
      if(indicators.squeeze && indicators.squeeze.squeeze && indicators.maAlign && indicators.maAlign.bullish) _gcW += 1;
      // [S369] 상태형 데드크로스 (★★ OBV/RSI 하락 다이버전스, ★ 스퀴즈+역배열)
      if(indicators.obv && indicators.obv.div === 'bearish') _dcW += 2;
      if(indicators.rsi && indicators.rsi.div === 'bearish') _dcW += 2;
      if(indicators.squeeze && indicators.squeeze.squeeze && indicators.maAlign && indicators.maAlign.bearish) _dcW += 1;
    }
    ...
    if(_rowsB357 && _rowsB357.length>=61 && typeof calcIndicators==='function'){
      ...
      if(_boardExtras.ad && _prevInd357.ad && _prevInd357.ad.trend)
        _boardExtras.ad.d = _scoreDir362(indicators.ad.trend) - _scoreDir362(_prevInd357.ad.trend);
      if(_boardExtras.eom && _prevInd357.eom && _prevInd357.eom.trend)
        _boardExtras.eom.d = _scoreDir362(indicators.eom.trend) - _scoreDir362(_prevInd357.eom.trend);
      if(_boardExtras.chaikin && _prevInd357.chaikinOsc && _prevInd357.chaikinOsc.val!=null)
        _boardExtras.chaikin.d = _scoreVal362(indicators.chaikinOsc.val) - _scoreVal362(_prevInd357.chaikinOsc.val);
      // [S367] 전이형 골든크로스 (1봉 전 vs 현재 비교)
      // ★★★ 핵심 GC (×3)
      if(indicators.macd && _prevInd357.macd && indicators.macd.hist >= 0 && _prevInd357.macd.hist < 0) _gcW += 3; // MACD GC
      if(indicators.maAlign && _prevInd357.maAlign
         ...
         && _prevInd357.maAlign.short <= _prevInd357.maAlign.mid) _gcW += 3; // 이평선 GC (MA5>MA20 전환)
      if(indicators.rsi && _prevInd357.rsi && indicators.rsi.val!=null && _prevInd357.rsi.val!=null
         && indicators.rsi.val >= 30 && _prevInd357.rsi.val < 30) _gcW += 3; // RSI 과매도 반등
      // ★★ 중요 (×2)
      if(indicators.stoch && _prevInd357.stoch
         ...
         && _prevInd357.stoch.k <= _prevInd357.stoch.d) _gcW += 2; // Stoch GC
      if(indicators.bb && _prevInd357.bb
         && indicators.bb.pctB!=null && _prevInd357.bb.pctB!=null
         && _prevInd357.bb.pctB < 0.2 && indicators.bb.pctB > _prevInd357.bb.pctB) _gcW += 2; // BB 하단 반등
      // [S369] 전이형 데드크로스 (GC 거울 대칭)
      // ★★★ 핵심 DC (×3)
      if(indicators.macd && _prevInd357.macd && indicators.macd.hist <= 0 && _prevInd357.macd.hist > 0) _dcW += 3; // MACD DC
      if(indicators.maAlign && _prevInd357.maAlign
         && indicators.maAlign.short!=null && indicators.maAlign.mid!=null
         && _prevInd357.maAlign.short!=null && _prevInd357.maAlign.mid!=null
         && indicators.maAlign.short < indicators.maAlign.mid
         && _prevInd357.maAlign.short >= _prevInd357.maAlign.mid) _dcW += 3; // 이평선 DC (MA5<MA20 전환)
      if(indicators.rsi && _prevInd357.rsi && indicators.rsi.val!=null && _prevInd357.rsi.val!=null
         && indicators.rsi.val < 70 && _prevInd357.rsi.val >= 70) _dcW += 3; // RSI 과매수 이탈
      // ★★ 중요 (×2)
      if(indicators.stoch && _prevInd357.stoch
         && indicators.stoch.k!=null && indicators.stoch.d!=null
         && _prevInd357.stoch.k!=null && _prevInd357.stoch.d!=null
         && indicators.stoch.k < indicators.stoch.d
         && _prevInd357.stoch.k >= _prevInd357.stoch.d) _dcW += 2; // Stoch DC
      if(indicators.bb && _prevInd357.bb
         && indicators.bb.pctB!=null && _prevInd357.bb.pctB!=null
         && _prevInd357.bb.pctB > 0.8 && indicators.bb.pctB < _prevInd357.bb.pctB) _dcW += 2; // BB 상단 하락
    }
    // [S367/S369] ⑤ 기술지표 — GC/DC 점수 매핑:
    //   · 발동 없음(w=0) → 0점 (회색 도넛, 분포 카운트 없음)
    //   · 1개라도 발동(w≥1) → 65~100점 (단방향 임계 65 통과로 ▲/▼ 카운트)
    //   · 가중치 클수록 점수도 높음 (변별력은 좁지만 의도 우선: "1개라도 있으면 신호")
    _boardExtras.gc = {v: _gcW === 0 ? 0 : Math.round(65 + (_gcW / 18) * 35)};
    _boardExtras.dc = {v: _dcW === 0 ? 0 : Math.round(65 + (_dcW / 18) * 35)};
  } catch(_e357){ /* 전이 계산 실패 시 삼각형 없이 진행 (안전) */ }

  // [S359] 휩소 신뢰도 — 추세 약함(ADX<25) or 변동성 큼(ATR%≥5) or 횡보(VHF ranging)
  let _lowConf = false;
  try {
    ...
    _lowConf = (_adxV!=null && _adxV<25) || (_atrR!=null && _atrR>=5) || (_vhfT==='ranging');
  } catch(_eLC){}
