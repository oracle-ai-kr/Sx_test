...
// ════════ [S756] 📋 검증 배지 인벤토리 = 검증 신호 시제품 SSOT. 풀 1개가 재료 충족 여부로 두 결과로 갈림(충족=진짜·검증됨 / 미충족=가짜·미검증). window.SX_BADGE_DEFS + _sxBadgeEval(key,adv)로 노출 → 점수판·조건검색·알림 등 다른 위치가 같은 정의를 가져다 씀(이벤트=데이터 공급은 호출측, 판정=여기 단일). 경계=풀+재료(이벤트 미포함·추후 확장). 표시 전용·엔진 무변경. ════════
var SX_BADGE_DEFS = {
  rebound_pool: {
    id:'rebound_pool', poolLt:'bear', poolName:'역배열+단기약세', date:'2026-06-27',
    signal:[{feat:'adx', dir:'lt', th:14.18, label:'ADX < 14.18'}, {feat:'dev200', dir:'lt', th:-16.46, label:'MA200이격도 < −16.46'}, {feat:'rsi', dir:'lt', th:45.07, label:'RSI < 45.07'}],
    support:[],
    yes:{ icon:'🟢', name:'바닥반등', color:'#16a34a', verified:true,  n10:92, n5:81, base:51, sample:'72' },
    no: { icon:'🔴', name:'데드캣',   color:'#dc2626', verified:false },
    note:'추세소진(ADX↓)+장기바닥(MA200이격도 깊음)+과매도(RSI) 동시충족 = 진짜 반등 92%(n72). 미충족쪽(데드캣)은 아직 미검증 — 거울상(N3·N5↑ N10·유지↓) 측정대상.'
  },
  pullback_pool: {
    id:'pullback_pool', poolLt:'bull', poolName:'정배열+단기조정', date:'2026-06-26',
    signal:[{feat:'bbPctB', dir:'gt', th:1.0, label:'BB%B > 1.0'}],
    support:[{feat:'dev20', dir:'gt', th:8, label:'MA20이격도 > 8'}],
    yes:{ icon:'🔵', name:'눌림목', color:'#2563eb', verified:true,  n10:74, n5:63, base:52, sample:'19~27' },
    no: { icon:'🟠', name:'되돌림', color:'#d97706', verified:false },
    note:'BB%B 밴드상단 돌파면 건강한 눌림목 74%. 미충족쪽(되돌림)은 미검증·측정대상.'
  }
};
var SX_BADGE_ORDER = ['rebound_pool','pullback_pool'];
...
// ★시제품 API — 순수 함수. 다른 위치가 adv만 자기 방식으로 공급하면 동일 판정. 반환: {inPool, signalMet, result:'yes'|'no'|null, meta, def}
function _sxBadgeEval(defKey, adv){
  var def = SX_BADGE_DEFS[defKey];
  if(!def) return { inPool:false, signalMet:false, result:null, def:null };
  if(!adv)  return { inPool:false, signalMet:false, result:null, def:def };
  ...
  var signalMet = def.signal.every(function(c){ return _vbCondMet(c, adv); });
  var result = inPool ? (signalMet ? 'yes' : 'no') : null;
  return { inPool:inPool, signalMet:signalMet, result:result, meta:(result?def[result]:null), def:def };
}
...
function _buildBadgeInventoryCard(stock, indicators){
  ...
  var rowsHtml = SX_BADGE_ORDER.map(function(key){
    ...
    var ev = _sxBadgeEval(key, adv);
    var cur = ev.meta;   // yes/no meta or null(풀밖)
    var chip = cur
      ? '<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:'+cur.color+';color:#fff;border:1px solid '+cur.color+'">'+cur.icon+' '+cur.name+'</span>'
      : '<span style="font-size:11px;font-weight:800;padding:3px 9px;border-radius:12px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">○ 비활성</span>';
    var statusTxt, statusCol;
    if(!ev.inPool){ statusTxt='○ 풀 불일치'; statusCol=GR; }
    else if(ev.result==='yes'){ statusTxt='✅ '+def.yes.name+' 감지'; statusCol=def.yes.color; }
    else { statusTxt='● '+def.no.name+' 분류'; statusCol=def.no.color; }
    var sigTxt = def.signal.map(function(c){ var v=_vbFeatVal(c.feat, adv); var ok=_vbCondMet(c, adv); return '<span style="color:'+(ok?def.yes.color:GR)+'">'+(ok?'✓ ':'')+c.label+(v!=null?' <span style="color:'+GR+'">('+v.toFixed(1)+')</span>':'')+'</span>'; }).join(' · ');
    var supTxt = def.support.length ? ' <span style="color:'+GR+'">+보조 '+def.support.map(function(c){ var ok=_vbCondMet(c, adv); return (ok?'✓':'')+c.label; }).join(', ')+'</span>' : '';
    var Y=def.yes, verLine;
    if(ev.result==='yes'){
      verLine='<span style="color:'+Y.color+';font-weight:700">✅ 검증</span> N10 '+Y.n10+'% · N5 '+Y.n5+'% (기저 '+Y.base+'%) · 표본 '+Y.sample+' · '+def.date;
    } else if(ev.result==='no'){
      verLine='<span style="color:'+GR+'">'+def.no.name+'쪽은 <b>미검증</b>(측정 대상) · 진짜('+Y.name+')쪽 조건 미충족</span>';
    } else {
      verLine='<span style="color:'+GR+'">풀 들어오면 판정 · 충족 시 '+Y.name+' (검증 N10 '+Y.n10+'%)</span>';
    }
    var borderC = (ev.inPool && cur) ? cur.color+'66' : 'var(--border)';
    return '<div style="padding:9px 10px;margin:6px 0;border-radius:9px;background:var(--surface2);border:1px solid '+borderC+'">'
      +   '<div style="display:flex;align-items:center;gap:7px;margin-bottom:5px">'+chip
      +     '<span style="margin-left:auto;font-size:9.5px;font-weight:700;color:'+statusCol+'">'+statusTxt+'</span></div>'
      +   '<div style="font-size:9px;color:'+GR+';line-height:1.5">풀: <b style="color:var(--text2)">'+def.poolName+'</b> '+(ev.inPool?('<span style="color:'+(cur?cur.color:GR)+'">●일치</span>'):'<span>●불일치</span>')+'</div>'
      +   '<div style="font-size:9.5px;line-height:1.6;margin-top:2px">신호: '+sigTxt+supTxt+'</div>'
      +   '<div style="font-size:8.5px;margin-top:4px;line-height:1.5">'+verLine+'</div>'
      +   (def.note?'<div style="font-size:8px;color:'+GR+';margin-top:2px;font-style:italic">'+def.note+'</div>':'')
      + '</div>';
  }).join('');
  var detCount = SX_BADGE_ORDER.map(function(k){ return _sxBadgeEval(k, adv); }).filter(function(e){ return e.inPool; }).length;
  var titleHtml = '<span style="font-size:13px;font-weight:800;color:var(--text)">📋 검증 배지 인벤토리</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span>';
  var rightHtml = '<span style="font-size:9.5px;font-weight:700;color:'+(detCount?'#16a34a':'var(--text3)')+'">'+(detCount? (detCount+' 판정'):(String(SX_BADGE_ORDER.length)+'종'))+'</span>';
  var desc = '<div style="font-size:8.5px;color:var(--text3);line-height:1.5;margin-bottom:4px">검증된 신호 <b style="color:var(--text2)">시제품 보관소(SSOT)</b> — 다른 배지가 가져다 씁니다. 풀 1개가 재료 충족으로 갈림: <b>충족=진짜</b>(검증) / <b>미충족=가짜</b>(미검증). <b style="color:var(--text2)">회색=풀 불일치</b>, 풀 들어오면 컬러로 현재 판정.</div>';
  return _sxExpCard('sxBadgeInvWrap', 'sxBadgeInvWrap_b', titleHtml, rightHtml, desc + rowsHtml);
}
if(typeof window!=='undefined'){ window._buildBadgeInventoryCard = _buildBadgeInventoryCard; }
...
function _sxBeamRun(scanMode, out){
  var MAXDEPTH=4, MAXTEST=30000;   // 깊이 4재료 캡 · 안전 상한
  ...
  var gatePass=function(s){
    ...
    return (s.n10!=null&&allN10!=null&&s.n10>allN10) && (s.hold!=null&&s.hold>=0.5);
  };
  ...
  if(!exhaustive){
    ...
    if(bs.n>=30 && base.length<MAXDEPTH) queue.push({conds:base, keys:bk, depth:base.length});
  } else {
    allCands.forEach(function(c){ visited[c.key]=1; var s=statOf([c]); if(s.n>=30 && 1<MAXDEPTH) queue.push({conds:[c], keys:[c.key], depth:1}); });
  }
  ...
  while(queue.length){
    ...
    for(var i=0;i<allCands.length;i++){
      ...
      if(s.n<30) continue;   // 가지치기 — 표본 부족이면 더 깊이 가도 소용없음
  ...
  h+='<div style="font-size:8px;color:'+T3+';margin-top:6px;line-height:1.45">씨앗(체크) 있으면 <b>그 재료 포함 조합만</b>, 없으면 <b>전수</b>(2~4재료). n≥30 조합만 더 깊이(자동 가지치기) · 게이트+<b>실제 후반평균 방향</b> 통과만 수집 · MACD 쌍둥이 제외. <b style="color:'+passCol+'">💾 JSON</b>으로 전체 저장 → 업로드.</div>';
  out.innerHTML=h;
}
