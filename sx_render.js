...
async function _btDiscrimBracket(mk, source, onProgress){
  ...
  for(var i=0;i<use.length;i++){
    ...
    for(var bi=250; bi<rows.length-10; bi+=_STEP){
      var slice, ind;
      ...
      entries.push({ f:f, n3:_n3, n5:_n5, n10:_n10, hold:_hold });
      _used=true;
...
function _btDiscrimRender(rep, watch, mk){
  ...
  out+='<div style="font-size:11px;font-weight:800;color:#0891b2;margin-bottom:4px">🧪 재료 변별력 · '+DP.icon+DP.short+'풀 · 분류기준 4종 · '+String(mk).toUpperCase()+'</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:5px">'+DP.poolName+' 진입을 4가지로 가름: <b>N3/N5/N10</b>(진입 후 그 봉수 뒤 종가>진입가)·<b>유지</b>(N5·N10 둘다 위=반짝 '+DP.bad+' 제외). 각 재료가 어느 기준에서 '+DP.good+'을 가르나. '+DP.bad+'은 짧게 꺾이니 <b>N별 차이·유지 여부가 정보</b>. <b style="color:'+GRN+'">BT 무관 — 풀 조건 만족 모든 봉 측정(순수 데이터)</b>.</div>';
  out+='<div style="font-size:8px;color:'+T3+';margin-bottom:6px">표본('+DP.goodShort+'/'+DP.badShort+'): '+CRIT.map(function(c){ return c.label+' '+PC[c.key].win+'/'+PC[c.key].loss; }).join(' · ')+'</div>';
  ...
  out+='</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">읽기: <b style="color:'+GRN+'">강</b>=그 기준에서 '+DP.good+' 잘 가름·<b style="color:'+AMB+'">중</b>=약하게·<b>약</b>=못 가름·<b>-</b>=표본부족. 한 재료가 특정 N에서만 강하면 그 시간축이 핵심. <b>유지(N5·N10)</b>가 진짜반등의 핵심 기준('+DP.bad+' 반짝 제외). ※강만 신뢰. <b style="color:'+GRN+'">BT·조기청산과 무관한 순수 데이터.</b></div>';
  out+='</div>';
  return out;
}
