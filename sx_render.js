  ...
  body.innerHTML = `
    ...
    ${(()=>{
      ...
      return `<div class="anal-section">
        ...
        ${fin.evEbitda!=null?`<div class="anal-row"><span class="al">EV/EBITDA${(()=>{
          // [PATCH ebitda-fallback-v2] 마커 단계 표시 — null/false=정확, 'adjustment'=조정 추정, true=영업이익 추정
          //   조정 기반은 영업이익 단독보다 정확하므로 별도 라벨로 신뢰도 차별화
          if(!fin._ebitdaApprox) return '';
          const lbl = fin._ebitdaApprox === 'adjustment' ? '(조정 추정)' : '(근사)';
          return ` <span style="font-size:8px;color:var(--text3);font-weight:normal">${lbl}</span>`;
        })()}</span><span class="ar ${fin.evEbitda>0&&fin.evEbitda<5?'bullish':fin.evEbitda>15||fin.evEbitda<0?'bearish':'neutral'}">${fin.evEbitda.toFixed(1)}배</span></div>`:''}
