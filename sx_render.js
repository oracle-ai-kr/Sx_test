...
var _RC_PHASE = {
  bottom:  { ko:'바닥반등', emoji:'🟢', col:'#16a34a', tip:'하락 구조에서 나온 진짜 반등 — 바닥 다지고 상승 시작' },
  pullback:{ ko:'눌림목',   emoji:'🔵', col:'#2563eb', tip:'상승 구조의 건강한 조정 — 매수 자리' },
  retrace: { ko:'되돌림',   emoji:'🟠', col:'#d97706', tip:'상승 동력 약화 — 눌림목 회복 또는 데드캣 추락의 갈림길' },
  deadcat: { ko:'데드캣',   emoji:'🔴', col:'#dc2626', tip:'하락 구조의 가짜 반등 — 곧 재하락 위험' }
};
...
function _rcDonutSvg(R, pts){
  ...
  if(pts && pts.length >= 2){
    ...
    for(i=1;i<pts.length;i++){
      ...
      var dx = B.x - A.x, dy = B.y - A.y, L = Math.sqrt(dx*dx + dy*dy) || 1, ex = B.x, ey = B.y;
      if(last){ ex = B.x - (dx/L)*12; ey = B.y - (dy/L)*12; }
      trail += '<line x1="'+A.x.toFixed(1)+'" y1="'+A.y.toFixed(1)+'" x2="'+ex.toFixed(1)+'" y2="'+ey.toFixed(1)+'" stroke="'+segCol+'" stroke-width="'+(rev?2.5:2)+'" stroke-dasharray="3 3" opacity="'+segOp.toFixed(2)+'"/>';
      if(last){
        var aA = Math.atan2(dy, dx);
        var h2x = ex - 7*Math.cos(aA - 0.45), h2y = ey - 7*Math.sin(aA - 0.45);
        var h3x = ex - 7*Math.cos(aA + 0.45), h3y = ey - 7*Math.sin(aA + 0.45);
        trail += '<polygon points="'+ex.toFixed(1)+','+ey.toFixed(1)+' '+h2x.toFixed(1)+','+h2y.toFixed(1)+' '+h3x.toFixed(1)+','+h3y.toFixed(1)+'" fill="'+segCol+'" opacity="0.85"/>';
      }
    }
  ...
  var center = '<text x="'+cx+'" y="'+(cy-7)+'" text-anchor="middle" dominant-baseline="central" font-size="11" font-weight="800" fill="'+cur.col+'">'+cur.emoji+' '+cur.ko+'</text>'
             + '<text x="'+cx+'" y="'+(cy+10)+'" text-anchor="middle" dominant-baseline="central" font-size="9" font-weight="700" fill="var(--text3)">강도 '+R.conf+(weak?' · 약함':'')+'</text>';
...
function _buildReboundCycleCard(stock, indicators){
  ...
  var tip = '<div style="font-size:11px;color:'+cur.col+';font-weight:700;text-align:center;margin:2px 0 10px">'+cur.tip+'</div>';
  var xRow = '<div style="font-size:10px;color:var(--text2);margin-bottom:3px"><b style="color:var(--text)">추세 '+(R.x>0?'+':'')+R.x+'</b> <span style="color:var(--text3)">(하락◀▶상승)</span>'+(R.xParts.length?' · '+R.xParts.join(' · '):' · 중립')+'</div>';
  var yAll = R.yPos.concat(R.yNeg);
  var yRow = '<div style="font-size:10px;color:var(--text2);margin-bottom:3px"><b style="color:var(--text)">품질 '+(R.y>0?'+':'')+R.y+'</b> <span style="color:var(--text3)">(가짜·실패◀▶진짜·건강)</span>'+(yAll.length?' · '+yAll.join(' · '):' · 중립')+'</div>';
  var cycle = '<div style="font-size:9px;color:var(--text3);text-align:center;margin:8px 0 0">↻ 바닥반등 → 눌림목 → 되돌림 → 데드캣 → 바닥반등</div>';
  var note = '<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px;line-height:1.55">추세구조(X)×반등품질(Y)을 도넛에 매핑한 <b>현위치</b> 시각화. 마커가 들어간 사분면 호 = 현재 단계. 강도=중앙에서 얼마나 또렷한지. 점·선 = 과거 여러 시점의 궤적(같은 데이터로 재계산, 저장 없음 → 깜빡임 없음): <b style="color:var(--text2)">순행 ↻</b>(사이클 진행)은 회색, <b style="color:#dc2626">역행 ↺</b> 구간은 빨강, 화살촉=진행방향. 반등 국면이 약해도 구조가 가리키는 위치를 유지합니다. C 판정·점수와 독립된 <b>실험 표시 카드</b>(매매 신호 아님).</div>';
  return _sxExpCard('sxRcCardWrap', 'sxRcCardWrap_b', _titleHtml, _rightHtml, donut + dirRow + tip + xRow + yRow + cycle + note);
}
