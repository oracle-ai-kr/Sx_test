// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v3.2 <!-- S96 -->
//  S96: 매매이력 2행 한국식 날짜/가격 포맷 (2026.2.24 / 66,877원)
//  S95: BT시그널통합 — BT상태배너+매매이력메인+점수소형배지+drawMiniWithTrades+btGetParams정합
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  if(_btSt && _btSt.state !== 'no_data'){
    ...
    btStateBannerHTML = `<div style="padding:10px 14px;margin:0 0 6px;border-radius:10px;background:${_btSt.state==='holding'?(_btSt.pnl>=0?'rgba(34,197,94,.08)':'rgba(232,54,90,.08)'):_btSt.state==='sell_signal'?'rgba(59,130,246,.08)':'var(--surface2)'};border:1px solid ${_btSt.state==='waiting'?'var(--border)':(_btSt.color||'var(--border)')}">
      <div style="display:flex;align-items:center;gap:8px">
        <span style="font-size:14px;color:${_btSt.color||'var(--text3)'}">${_bsIcon}</span>
        <span style="font-size:14px;font-weight:800;color:${_btSt.color||'var(--text)'}">${_btSt.text}</span>
      </div>
  ...
  if(_btTrades && _btTrades.length){
    ...
    _recent.forEach((tr, idx) => {
      ...
      const row = `<div style="padding:5px 0;border-bottom:1px solid var(--border)">
        <div style="display:flex;align-items:center;gap:6px;font-size:10px"><span style="display:inline-block;width:28px;text-align:center;padding:1px 0;border-radius:3px;background:rgba(34,197,94,.1);color:var(--buy);font-size:9px;font-weight:700">매수</span><span style="color:var(--text2)">${_fmtDKR(tr.entryDate)}</span><span style="color:var(--text);margin-left:auto">${_entryP}</span></div>
        <div style="display:flex;align-items:center;gap:6px;font-size:10px;margin-top:2px"><span style="display:inline-block;width:28px;text-align:center;padding:1px 0;border-radius:3px;background:${tr.type==='WIN'?'rgba(34,197,94,.1)':tr.type==='LOSS'?'rgba(232,54,90,.1)':'rgba(59,130,246,.1)'};color:${pnlC};font-size:9px;font-weight:700">${exitLbl}</span><span style="color:var(--text2)">${_fmtDKR(tr.exitDate)}</span><span style="color:var(--text);margin-left:auto">${_exitP}</span>${_pnlTxt ? `<span style="margin-left:6px">${_pnlTxt}</span>` : ''}</div>
      </div>`;
    ...
    tradeHistHTML = `<div style="margin:0 0 8px;padding:10px 12px;background:var(--surface);border:1px solid var(--border);border-radius:10px">
      ...
      <div style="text-align:center;margin-top:6px;cursor:pointer" onclick="var el=document.getElementById('${_thId}');var open=el.style.display!=='none';el.style.display=open?'none':'block';this.innerHTML=open?'▼ 이전 매매 ${_recent.length-1}건 더보기':'▲ 접기'">
        <span style="font-size:10px;color:var(--accent);font-weight:600">▼ 이전 매매 ${_recent.length-1}건 더보기</span>
      </div>` : ''}
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        return `<div style="display:flex;align-items:center;gap:6px;margin-top:6px;flex-wrap:wrap">
          <div style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1px solid var(--border);cursor:pointer" onclick="var el=document.getElementById('${_3stageId}');el.style.display=el.style.display==='none'?'block':'none'">
          ...
          <div style="display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1px solid var(--border)">
