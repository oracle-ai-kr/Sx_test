...
function _candleTransitionScore(rows, indicators, market, tf){
  ...
  if(_rDiv === undefined && _oDiv === undefined && _vRes === undefined && typeof SXE !== 'undefined' && SXE.calcAllScreener){
    try { const _f = SXE.calcAllScreener(rows, tf || 'day'); _rDiv = _f.rsi && _f.rsi.div; _oDiv = _f.obv && _f.obv.div; _vRes = _f._volResist; } catch(_eF){}
  }
  const _applyExtra = () => {
...
function _buildTransitionCard(stock, indicators){
  ...
  return wrap(top + bar + _seqRow + _hitRow + warnHtml + reasonsHtml + note + btnBT);
}
