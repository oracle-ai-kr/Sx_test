  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        if(typeof SXI.calcHistoricalPer === 'function' && _fin._historicalEps){
          ...
          try{
            window._sxValDebug = {
              ...
              epsSource: _fin._epsSource || 'n/a',
              eps: _fin.eps != null ? _fin.eps : null,
              hasFinObj: true
            };
            console.log('[S327 valuation debug]', window._sxValDebug);
          }catch(_){}
        }
        const _v = SXI.valuationJudge(_fin, stock.price, stock, _histPer);
        if(!_v) return '';
        ...
        // [S327 Phase 2] 적정주가 라벨 — historical 평균 PER 사용 여부에 따라 표시 분기
        //   historical 사용: "3년 평균 PER 13.2배 기준"
        //   시장평균 폴백: "시장평균 PER 11배 기준"
        const _perLabelStr = _v.historicalPer && _v.historicalPer.avgPer != null && _v.historicalPer.validYears >= 2
          ? `${_v.historicalPer.validYears}년 평균PER ${_v.benchPer.toFixed(1)}배 기준`
          : `시장평균PER ${Math.round(_v.benchPer)}배 기준`;
    ...
    ${(()=>{
      ...
      return `<div class="anal-section">
        ...
        ${(()=>{
          ...
          const _v = SXI.valuationJudge(fin, stock.price, stock, _histPer2);
          if(!_v) return '';
