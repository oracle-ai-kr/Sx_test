...
async function runAnalysis(stock){
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  try{
    ...
    //   엔진 scoreMomentum._normV와 아래 기대버전을 함께 올려야 함 (현재 2).
    var _momStale = stock._scoreMomentum && stock._scoreMomentum.history && stock._scoreMomentum.history.length && (stock._scoreMomentum._normV || 0) < 2;
      ...
      ${(()=>{
        ...
        ${(()=>{
          ...
          var _zoneStyle = function(zone){
            ...
            if(active) return 'padding:8px 10px 4px;border-left:4px solid #f5b301;background:rgba(245,197,24,.16);border-radius:0 6px 6px 0;'; // [S357] 활성 경로 연노랑 강조
            return 'opacity:.55;'; // 비활성 경로 — 정보 보존 위해 과하게 흐리지 않게(판정엔 4축 모두 반영됨)
          };
