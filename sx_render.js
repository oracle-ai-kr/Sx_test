  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // S103-fix6c Phase2: 전이 확률 라벨 양방향 + 행동 신호 2단계 배지
        //   Layer 1(정적): 라벨 자체를 모드에 맞춰 "강세 전이" ↔ "약세 전이"로 치환
        //   Layer 2(동적): r2e(진입타이밍/매도타이밍 섹션)에만 배지 — 4분기 완전 매트릭스
        //     조짐(예고, 연한색): 현재 모드와 반대 신호 축적 중
        //       매수 + r2e<35 → [하락조짐] (강세 피로, 전환 예고)
        //       매도 + r2e>=65 → [반등조짐] (바닥 다지기, 전환 예고)
        //     유력(확정, 진한색): 현재 모드와 같은 방향 신호 확정
        //       매수 + r2e>=65 → [반등유력] (반등 모멘텀 확정)
        //       매도 + r2e<35 → [하락유력] (하락 모멘텀 확정)
        //     중립(35~65): 배지 없음 — 판단 유보 구간
        const _r2eLabel = _isSellMode ? '약세 전이' : '강세 전이';
        const _e2tLabel = '추세 전이'; // 양방향 중립 유지
        const _txBadge = function(prob){
          // prob: r2e 정방향 전이 확률 (ready→entry)
          if(prob == null) return '';
          if(_isSellMode){
            // 매도 모드
            if(prob >= 65) return '<span class="tx-badge up-soft">반등조짐</span>';
            if(prob < 35)  return '<span class="tx-badge down">하락유력</span>';
          } else {
            // 매수 모드
            if(prob >= 65) return '<span class="tx-badge up">반등유력</span>';
            if(prob < 35)  return '<span class="tx-badge down-soft">하락조짐</span>';
          }
          return '';
        };
        ...
        ${(()=>{
          ...
          html += '<span style="font-weight:700;color:var(--accent)">'+_r2eLabel+' '+r2e.prob+'%</span>'+_txBadge(r2e.prob)+' · '+r2e.days;
