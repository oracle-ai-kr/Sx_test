...
function _reboundCycleCalc(adv, stock, mkt){
  ...
  var y = 0, yPos = [], yNeg = [];
  // [소진 축] 거래량OSC·ADX는 같은 현상(매도소진=추세강도소진) — 앙상블 검증상 거의 중복(둘다 76% vs 단독 78%)이라 max로 1회만 가산(중복 가산 방지)
  var _exh = 0, _exhP = '';
  if(typeof adv.volOsc === 'number' && adv.volOsc < -40){ _exh = 25; _exhP = '거래량OSC ' + adv.volOsc.toFixed(0) + '(매도 소진)'; }
  var _adxV = (adv.adx && typeof adv.adx.adx === 'number') ? adv.adx.adx : null;
  if(_adxV != null && _adxV < 19 && _exh < 18){ _exh = 18; _exhP = 'ADX ' + _adxV.toFixed(0) + '(추세강도 소진)'; }
  if(_exh > 0){ y += _exh; yPos.push(_exhP); }
  // [위치 축] 과매도 위치 — 보조(60~61%), 약하게
  if(typeof adv.cci === 'number' && adv.cci < 80){ y += 8; yPos.push('CCI ' + adv.cci.toFixed(0) + '(과매도권)'); }
  if(adv.rsi && typeof adv.rsi.val === 'number' && adv.rsi.val < 57){ y += 8; yPos.push('RSI ' + adv.rsi.val.toFixed(0) + '(반등 여력)'); }
  // [데드캣 감점] N5 반짝 후 N10 추락 = 잠깐 튀는 가짜. 골든크로스는 측정상 데드캣 시그니처(5×20=35%·전구간 기저 이하)
...
function _sxBuilderAutoScan(scanMode){
  ...
  var pass = (scanMode==='deadcat')
    ? ok.filter(function(r){return r.n3!=null&&allN3!=null&&r.n3>=allN3 && r.n5!=null&&allN5!=null&&r.n5>=allN5 && r.n10!=null&&allN10!=null&&r.n10<allN10 && r.hold!=null&&allHold!=null&&r.hold<allHold;})   // [S770] 데드캣: N3·N5 기저↑ + N10·유지 기저↓ (반짝 후 추락)
  ...
  var passLbl=_isDC?'🔴 데드캣 후보':'✅ 진짜반등 합격', passCond=_isDC?'N3·N5 기저↑ + N10·유지 기저↓':'표본30+ · N10▲ · 유지50%+', passCol=_isDC?RED:GRN;
  ...
  h+='<div style="font-size:8px;color:'+T3+';margin-top:6px;line-height:1.4">'+(_isDC?('데드캣=N3·N5는 기저↑(반짝)인데 N10·유지는 기저↓(추락). <b style="color:'+RED+'">N5 빨강+N10 빨강</b>=전형적 데드캣. 전체기저 N5 '+(allN5!=null?Math.round(100*allN5)+'%':'-')+'·N10 '+(allN10!=null?Math.round(100*allN10)+'%':'-')+'.'):('합격=베이스보다 N10 오르고(▲)+유지 50%+ · 표본 30+. 좋은 후보 <b>체크→베이스 추가→재탐색</b>.'))+' <b style="color:'+passCol+'">💾로 JSON 저장</b>.</div>';
  out.innerHTML=h;
}
