...
// ═══════════════════════════════════════════════════════════════
// S114/S115: 엔진판단 검증 실행 함수 (_runEngineVerify)
//   역할: 분석탭의 "엔진판단 검증" 버튼 클릭 시 단일검증 [▶ 백테스트 실행]과 동등한 BT 실행
//
//   S115 hotfix2: 탭 전환 방식 폐기 → 백그라운드 실행으로 재설계
//     [이전 문제] switchAnalTab(1) → btRunBasic() → switchAnalTab(원래탭) 방식은
//                사용자 화면이 단일검증 탭으로 튀었다가 돌아오는 깜빡임 발생 + 중간 종료 버그
//     [채희창 요청] "단일검증탭으로 안넘어갈 방법. 현재탭 유지되고 결과만 버튼아래에 보여주면 됨"
//     [S115 해결] btRunBasic의 DOM 의존 (btnBtBasic/btBasicProg/btBasicProgFill/btBasicProgText/btBasicResult)
//                을 제거한 간소 버전 직접 실행. stock._btResult만 갱신하고 분석탭 재렌더로 결과 표시.
//
//   흐름:
//     1. 사용자가 "엔진판단 검증 ▶" 클릭
//     2. 진행 토스트 표시 (탭 전환 없음)
//     3. sx_bt.js의 경로 A/B/C 로직을 인라인으로 실행 (DOM 진행바 참조 제거)
//     4. sxRunBtEngine 실행 → stock._btResult 갱신
//     5. stock._needsAnalRerender=true 설정 + 현재 분석탭이 이미 활성이면 즉시 runAnalysis 재호출
//     6. 분석탭 내 "엔진판단 검증 결과 6칸 카드"가 새 결과로 자동 갱신됨
//
//   프로젝트 C v3.0 원칙:
//     - 원칙 ⑤ "정합 우선": stock._lastAnalCandles 업데이트로 분석탭 재진입 시에도 600봉 유지
//     - 원칙 ⑦ "UI 역할 분리": 엔진판단 검증 버튼 = 분석탭 내에서만 동작 (탭 전환 없음)
//     - 원칙 ⑧ "검증가능성": 실제 BT 엔진 결과를 그대로 표시
// ═══════════════════════════════════════════════════════════════
async function _runEngineVerify(stock){
  ...
  try {
    ...
    // _btSaveBtResult 있으면 localStorage 저장
    ...
    // ─── 분석탭 재렌더 (결과 카드 갱신) ───
    // 현재 분석탭(0)에 있을 때만 즉시 재렌더, 다른 탭에 있다면 _needsAnalRerender 플래그로 탭 복귀 시 처리
    if(typeof _analTabIdx !== 'undefined' && _analTabIdx === 0 && typeof currentAnalStock !== 'undefined' && currentAnalStock === stock){
      if(typeof runAnalysis === 'function'){
        stock._needsAnalRerender = false;  // 플래그 클리어 (중복 방지)
        await runAnalysis(stock);
        console.log('[S115] 분석탭 즉시 재렌더 완료');
      }
    }
  } catch(e) {
    ...
    <div class="anal-header-card">
      ...
      <div style="margin-top:10px;padding:10px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid #3b82f6">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
          <div style="font-size:11px;color:var(--text2);font-weight:700;letter-spacing:-.3px">📊 엔진판단 검증 결과</div>
