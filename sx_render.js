...
function _sxBuilderAutoScan(scanMode){
  ...
  var passLbl=_isDC?'🔴 가짜반등 후보':'✅ 진짜반등 합격', passCond=_isDC?'N3▲or N5▲ + N10▼or 유지50%-':'표본30+ · N10▲ · 유지50%+', passCol=_isDC?RED:GRN;
  ...
  else if(!pass.length){ h+='<div style="font-size:9.5px;color:'+T3+';padding:8px 6px;line-height:1.5">'+passLbl+' 없음 <span style="font-size:8px">('+passCond+')</span><br>표본 30+ 중 미해당 '+okFail+'개 · 표본부족 '+low.length+'개.'+(ok.length?(_isDC?' 가짜반등 패턴(반짝 후 추락)이 없어요 — 베이스를 바꿔보세요.':' 베이스보다 N10 올리는 후보가 없어요 — 베이스 조건을 바꿔보세요.'):'')+'</div>'; }
  else {
  ...
  h+='<div style="font-size:8px;color:'+T3+';margin-top:3px;line-height:1.45">● picks 실제 후반평균 방향: <b style="color:#16a34a">●오름</b>=진짜반등 · <b style="color:#dc2626">●추락</b>=가짜반등(데드캣) · <b style="color:var(--text3)">●보합</b>=애매. 강/중=크기. <b>진짜반등 모드는 ●오름, 가짜반등 모드는 ●추락 찾기</b>. (가짜반등 후보인데 ●오름이면 = 실제론 안 떨어짐 = 게이트 헛짚음)</div>';
  out.innerHTML=h;
}
...
function _btBuilderUI(){
  ...
  h+='<div style="border-top:1px dashed var(--border);margin-top:10px;padding-top:8px"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:4px"><button onclick="_sxBuilderAutoScan(\'rebound\')" style="font-size:10px;font-weight:800;padding:6px 11px;border-radius:8px;border:1px solid #0ea5e9;background:#0ea5e9;color:#fff;cursor:pointer">🔍 진짜반등</button><button onclick="_sxBuilderAutoScan(\'deadcat\')" style="font-size:10px;font-weight:800;padding:6px 11px;border-radius:8px;border:1px solid #dc2626;background:#dc262614;color:#dc2626;cursor:pointer">🔴 가짜반등</button><button onclick="_sxAutoScanDownload()" style="font-size:10px;font-weight:700;padding:6px 9px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text2);cursor:pointer">💾 JSON</button><button onclick="_sxYAxisValidate()" style="font-size:10px;font-weight:800;padding:6px 11px;border-radius:8px;border:1px solid #7c3aed;background:#7c3aed14;color:#7c3aed;cursor:pointer">🎯 Y축검증</button></div><div style="font-size:8.5px;color:var(--text3);line-height:1.4;margin-bottom:3px">체크=베이스 · 미체크 하나씩 추가 · <b style="color:#dc2626">가짜반등=N5↑N10↓(거울상)</b></div><div id="sxbAutoOut" style="margin-top:4px"></div></div>';
  h+='</div>';
  return h;
}
