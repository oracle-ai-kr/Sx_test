// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v3.3 <!-- S97 -->
...
function renderResults(){
  ...
  // S81: 3단 배지 카운트, S84: 평균 점수 통계, S85: 전이확률평균+상위종목
  const _badgeData = _countStageBadges(filtered);
  const _badgeCnt = _badgeData.count;
  const _badgeAvg = _badgeData.avg;
  const _badgeTransAvg = _badgeData.transAvg;
  const _badgeTops = _badgeData.tops;
  const _hasBadges = _badgeCnt.ready + _badgeCnt.entry + _badgeCnt.trend + _badgeCnt.caution + _badgeCnt.wait > 0;

  // S83: 배지 필터 적용 (복수 선택) + S82: 배지 내 전이확률 순 정렬
  const _hasFilter = _stageBadgeFilter.size > 0;
  let displayed = _hasFilter ? filtered.filter(s => _stageBadgeFilter.has(_getStockStage(s))) : filtered;
  if (_hasFilter && displayed.length > 1) {
    displayed = displayed.slice().sort((a, b) => {
      const aStg = _getStockStage(a), bStg = _getStockStage(b);
      const aProb = _getTransitionProb(a, aStg);
      const bProb = _getTransitionProb(b, bStg);
      return bProb - aProb;
    });
  }

  // S85: 배지 통계 tooltip (평균 점수 + 전이확률)
  function _badgeStatTip(key) {
    var a = _badgeAvg[key];
    if (!a) return '';
    var tip = '준비 '+a.r+' / 진입 '+a.e+' / 추세 '+a.t;
    if (_badgeTransAvg[key]) tip += ' | 전이 '+_badgeTransAvg[key]+'%';
    return ' title="'+tip+'"';
  }
  ...
  area.innerHTML = `
    ...
    ${_hasBadges ? `<div style="display:flex;gap:6px;padding:4px 12px 6px;flex-wrap:wrap">
      <span onclick="_setStageBadgeFilter('ready')"${_badgeStatTip('ready')} style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter.has('ready')?'var(--accent)':'var(--border)'};background:${_stageBadgeFilter.has('ready')?'var(--accent)':'var(--surface)'};color:${_stageBadgeFilter.has('ready')?'#fff':'var(--text2)'}">관심 ${_badgeCnt.ready}</span>
      <span onclick="_setStageBadgeFilter('entry')"${_badgeStatTip('entry')} style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter.has('entry')?'var(--buy)':'var(--border)'};background:${_stageBadgeFilter.has('entry')?'var(--buy)':'var(--surface)'};color:${_stageBadgeFilter.has('entry')?'#fff':'var(--text2)'}">진입검토 ${_badgeCnt.entry}</span>
      <span onclick="_setStageBadgeFilter('trend')"${_badgeStatTip('trend')} style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter.has('trend')?'var(--buy)':'var(--border)'};background:${_stageBadgeFilter.has('trend')?'var(--buy)':'var(--surface)'};color:${_stageBadgeFilter.has('trend')?'#fff':'var(--text2)'}">보유확인 ${_badgeCnt.trend}</span>
      <span onclick="_setStageBadgeFilter('caution')"${_badgeStatTip('caution')} style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter.has('caution')?'var(--sell)':'var(--border)'};background:${_stageBadgeFilter.has('caution')?'var(--sell)':'var(--surface)'};color:${_stageBadgeFilter.has('caution')?'#fff':'var(--text2)'}">과열주의 ${_badgeCnt.caution}</span>
      <span onclick="_setStageBadgeFilter('wait')"${_badgeStatTip('wait')} style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter.has('wait')?'var(--text3)':'var(--border)'};background:${_stageBadgeFilter.has('wait')?'var(--text3)':'var(--surface)'};color:${_stageBadgeFilter.has('wait')?'#fff':'var(--text2)'}">관망 ${_badgeCnt.wait}</span>
    ...
    ${_hasFilter && _stageBadgeFilter.size === 1 ? (()=>{
      const selKey = Array.from(_stageBadgeFilter)[0];
      const a = _badgeAvg[selKey];
      if (!a) return '';
      var line = '<div style="padding:0 12px 4px;font-size:8px;color:var(--text3);line-height:1.6">';
      line += '평균 — 준비 '+a.r+' / 진입 '+a.e+' / 추세 '+a.t;
      if (_badgeTransAvg[selKey]) line += ' · 전이 '+_badgeTransAvg[selKey]+'%';
      // S85: 상위 종목 미리보기
      var tp = _badgeTops[selKey];
      if (tp && tp.length) {
        line += '<br>상위: ';
        for (var tpi = 0; tpi < tp.length; tpi++) {
          if (tpi > 0) line += ', ';
          line += tp[tpi].name + '(' + tp[tpi].score + ')';
        }
      }
      line += '</div>';
      return line;
          ...
          <div class="sr-body">
            ...
            <div class="sr-row2">
              ...
              ${(()=>{
                ...
                // S81: 이모지 배지 라벨
                let _badgeEmoji='', _badgeLabel='', _badgeColor='var(--text3)';
                if(t>=60&&e>=50){ _badgeLabel='보유확인'; _badgeColor='var(--buy)'; }
                else if(e>=60&&r>=50){ _badgeLabel='진입검토'; _badgeColor='var(--buy)'; }
                else if(r>=60){ _badgeLabel='관심'; _badgeColor='var(--accent)'; }
                else if(t>=70&&r<30){ _badgeLabel='과열주의'; _badgeColor='var(--sell)'; }
                else { _badgeLabel='관망'; }
                return '<span class="sr-score-chip" style="font-size:9px;color:'+_badgeColor+'">'+_badgeLabel+'</span>';
              })()}
              ${(()=>{
 ...
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  try{
    ...
    if(btCandles && btCandles.length >= 60 && typeof SXE!=='undefined' && SXE.runBtEngine){
      ...
      const btR = SXE.runBtEngine(rawRows, currentTF, _btParams);
      if(btR && !btR.error){
        _analBtResult = btR;
  ...
  try{
    ...
    if(momCandles && momCandles.length >= 80 && typeof SXE!=='undefined' && SXE.scoreMomentum){
      ...
      stock._scoreMomentum = SXE.scoreMomentum(rawRows, currentTF, 5);
    }
...
function _drawMiniCandleChart(rows, trades){
  if(typeof SXChart==='undefined') return;
  if(trades && trades.length && SXChart.drawMiniWithTrades){
    SXChart.drawMiniWithTrades('miniCandleChart', rows, trades);
  } else {
...
function _buildBtBanner(stock, qs){
  ...
  if(!_isBtSupportedTF(_btMkt, currentTF)){
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  // S95: BT 상태 배너 HTML 생성
  const _btSt = stock._btState || null;
  let btStateBannerHTML = '';
  if(_btSt && _btSt.state !== 'no_data'){
    let _bsIcon = '', _bsDetail = '';
    if(_btSt.state === 'holding'){
      // S97: 현재봉 매수 신호 vs 보유중 분기
      const _isBuySignal = _btSt.entryIdx != null && _btSt.totalBars > 0 && _btSt.entryIdx >= _btSt.totalBars - 2;
      if(_isBuySignal){
        _bsIcon = '▲';
        _bsDetail = _btSt.tp && _btSt.sl ? `진입가 ${Math.round(_btSt.entry).toLocaleString()} · TP ${Math.round(_btSt.tp).toLocaleString()} / SL ${Math.round(_btSt.sl).toLocaleString()}` : `진입가 ${Math.round(_btSt.entry).toLocaleString()}`;
        _btSt._isBuySignal = true;
        _btSt.text = 'BT 매수 신호 — 진입가 ' + Math.round(_btSt.entry).toLocaleString();
      } else {
        _bsIcon = '●';
        const _curP = stock.price || stock.close || 0;
        const _pnlAmt = _curP > 0 && _btSt.entry > 0 ? Math.round(_curP - _btSt.entry).toLocaleString() : '';
        let _detailParts = `진입가 ${Math.round(_btSt.entry).toLocaleString()}`;
        if(_curP > 0) _detailParts += ` → 현재가 ${Math.round(_curP).toLocaleString()}`;
        if(_pnlAmt) _detailParts += ` (${_btSt.pnl>=0?'+':''}${_pnlAmt}원)`;
        if(_btSt.tp && _btSt.sl) _detailParts += ` · TP ${Math.round(_btSt.tp).toLocaleString()} / SL ${Math.round(_btSt.sl).toLocaleString()}`;
        _bsDetail = _detailParts;
      }
    } else if(_btSt.state === 'sell_signal'){
      _bsIcon = '◆';
      _bsDetail = `청산가 ${Math.round(_btSt.exitPrice).toLocaleString()} (${_btSt.pnl>=0?'+':''}${_btSt.pnl}%)`;
    } else if(_btSt.state === 'waiting'){
      _bsIcon = '○';
      _bsDetail = _btSt.currentScore != null ? `매수 기준 ${_btSt.buyTh}` : '';
    }
    // S97: 배너 색상 통일 — 매수/보유=주황, 익절/수익=녹색, 손절/손실=빨강 + 펄스 테두리
    const _bnPnlPos = _btSt.pnl >= 0;
    let _bnBg, _bnBorder, _bnIconClr, _bnTextClr;
    if(_btSt.state === 'holding'){
      if(_btSt._isBuySignal){
        // 현재봉 매수 신호 — 녹색
        _bnBg = 'rgba(34,197,94,.08)';
        _bnBorder = '#22c55e';
        _bnIconClr = '#22c55e';
        _bnTextClr = '#22c55e';
      } else {
        // 보유중 — 수익=주황, 손실=파랑
        _bnBg = _bnPnlPos ? 'rgba(255,153,0,.08)' : 'rgba(59,130,246,.08)';
        _bnBorder = _bnPnlPos ? '#ff9900' : '#3b82f6';
        _bnIconClr = _bnPnlPos ? '#ff9900' : '#3b82f6';
        _bnTextClr = _bnPnlPos ? '#ff9900' : '#3b82f6';
      }
    } else if(_btSt.state === 'sell_signal'){
      // 매도 신호 — 익절=주황, 손절=파랑
      const _isWin = _btSt.isWin;
      _bnBg = _isWin ? 'rgba(255,153,0,.08)' : 'rgba(59,130,246,.08)';
      _bnBorder = _isWin ? '#ff9900' : '#3b82f6';
      _bnIconClr = _isWin ? '#ff9900' : '#3b82f6';
      _bnTextClr = _isWin ? '#ff9900' : '#3b82f6';
    } else {
      _bnBg = 'var(--surface2)'; _bnBorder = 'var(--border)'; _bnIconClr = 'var(--text3)'; _bnTextClr = 'var(--text3)';
    }
    const _bnPulseId = '_bnPulse_' + Math.random().toString(36).slice(2,6);
    const _bnActive = _btSt.state === 'holding' || _btSt.state === 'sell_signal';
    const _bnPulse = _btSt.state === 'holding' && !_btSt._isBuySignal; // 보유중만 펄스 (매수신호 제외)
    btStateBannerHTML = `<style>@keyframes ${_bnPulseId}{0%,100%{border-color:${_bnBorder}}50%{border-color:transparent}}</style>
    <div style="padding:10px 14px;margin:0 0 6px;border-radius:10px;background:${_bnBg};border:1.5px solid ${_bnActive?_bnBorder:'var(--border)'};${_bnPulse?`animation:${_bnPulseId} 1.2s ease-in-out infinite`:''}">
      <div style="display:flex;align-items:center;gap:8px">
        <span style="font-size:14px;color:${_bnIconClr}">${_bsIcon}</span>
        <span style="font-size:14px;font-weight:800;color:${_bnTextClr}">${_btSt.text}</span>
      </div>
      ${_bsDetail?`<div style="font-size:10px;color:var(--text3);margin-top:4px;padding-left:22px">${_bsDetail}</div>`:''}
    </div>`;
  }
  ...
  if(indicators?._advanced?.rows){
    chartHTML = `<div class="mini-chart-wrap" onclick="if(typeof SXChart!=='undefined'&&_currentAnalRows)SXChart.openFull(_currentAnalRows,_currentAnalName,_currentAnalTrades)" style="cursor:pointer"><div class="mini-chart-title">최근 ${Math.min(indicators._advanced.rows.length, 60)}봉 <span style="font-size:8px;color:var(--accent);font-weight:400">(탭하면 상세)</span></div><canvas id="miniCandleChart" width="400" height="180"></canvas></div>`;
  }

  // S96: 매매이력 가계부 형태 (매수행+매도행) + 접기/펼치기
  let tradeHistHTML = '';
  const _btTrades = stock._btResult?.trades;
  if(_btTrades && _btTrades.length){
    const _fmtDKR = d => { if(!d) return '—'; d=String(d); if(d.length===8&&d.indexOf('-')<0) return d.slice(0,4)+'.'+parseInt(d.slice(4,6))+'.'+parseInt(d.slice(6,8)); const p=d.split(/[-T]/); if(p.length<3) return d.slice(0,10); return p[0]+'.'+parseInt(p[1])+'.'+parseInt(p[2]); };
    const _recent = _btTrades.slice(-10).reverse();
    const _thId = '_tradeHist_' + Math.random().toString(36).slice(2,8);
    let _firstRow = '';
    let _restRows = '';
    _recent.forEach((tr, idx) => {
      const pnlC = tr.pnl >= 0 ? 'var(--buy)' : 'var(--sell)';
      const exitLbl = tr.type==='WIN'?'익절':tr.type==='LOSS'?'손절':tr.type==='OPEN'?'보유중':'매도';
      const _entryP = Math.round(tr.entry).toLocaleString() + '원';
      const _exitP = tr.type==='OPEN' ? '—' : Math.round(tr.exit).toLocaleString() + '원';
      const _pnlTxt = tr.type==='OPEN' ? '' : `<span style="font-weight:700;color:${pnlC};font-size:10px">${tr.pnl>=0?'+':''}${tr.pnl}%</span> <span style="color:var(--text3);font-size:8px">${tr.bars}봉</span>`;
      const _hlIdx = tr.entryIdx != null ? tr.entryIdx : -1;
      const row = `<div style="padding:5px 0;border-bottom:1px solid var(--border);cursor:pointer" onclick="if(typeof SXChart!=='undefined'&&_currentAnalRows){SXChart.highlightBar('miniCandleChart',${_hlIdx},_currentAnalRows,_currentAnalTrades);var _mc=document.getElementById('miniCandleChart');if(_mc)_mc.scrollIntoView({behavior:'smooth',block:'center'})}">
        <div style="display:flex;align-items:center;gap:6px;font-size:10px"><span style="display:inline-block;width:28px;text-align:center;padding:1px 0;border-radius:3px;background:rgba(34,197,94,.15);color:#22c55e;font-size:9px;font-weight:700">매수</span><span style="color:var(--text2)">${_fmtDKR(tr.entryDate)}</span><span style="color:var(--text);margin-left:auto">${_entryP}</span></div>
        <div style="display:flex;align-items:center;gap:6px;font-size:10px;margin-top:2px"><span style="display:inline-block;width:28px;text-align:center;padding:1px 0;border-radius:3px;background:${tr.type==='WIN'?'rgba(34,197,94,.1)':tr.type==='LOSS'?'rgba(232,54,90,.1)':'rgba(59,130,246,.1)'};color:${tr.type==='WIN'?'var(--buy)':tr.type==='LOSS'?'var(--sell)':'var(--accent)'};font-size:9px;font-weight:700">${exitLbl}</span><span style="color:var(--text2)">${_fmtDKR(tr.exitDate)}</span><span style="margin-left:auto;display:flex;align-items:center;gap:6px">${_pnlTxt || ''}<span style="color:var(--text)">${_exitP}</span></span></div>
      </div>`;
      if(idx === 0) _firstRow = row; else _restRows += row;
    });
    const _closedTr = _btTrades.filter(t=>t.type!=='OPEN');
    const _wins = _closedTr.filter(t=>t.type==='WIN').length;
    const _total = _closedTr.length;
    const _wr = _total > 0 ? Math.round(_wins/_total*100) : 0;
    const _totalPnl = stock._btResult.totalPnl || 0;
    const _hasMore = _recent.length > 1;
    tradeHistHTML = `<div style="margin:0 0 8px;padding:10px 12px;background:var(--surface);border:1px solid var(--border);border-radius:10px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        <span style="font-size:12px;font-weight:800;color:var(--text)">매매 이력</span>
        <span style="font-size:10px;color:var(--text3)">${_total}건 · 승률 ${_wr}% · 수익 <span style="color:${_totalPnl>=0?'var(--buy)':'var(--sell)'}">${_totalPnl>=0?'+':''}${_totalPnl.toFixed(1)}%</span></span>
      </div>
      ${_firstRow}
      ${_hasMore ? `<div id="${_thId}" style="display:none">${_restRows}</div>
      <div style="text-align:right;margin-top:4px;cursor:pointer;padding:2px 0" onclick="var el=document.getElementById('${_thId}');var open=el.style.display!=='none';el.style.display=open?'none':'block';this.querySelector('span').textContent=open?'▼':'▲'">
        <span style="font-size:11px;color:var(--accent)">▼</span>
      </div>` : ''}
      ${_btTrades.length > 10 ? `<div style="font-size:9px;color:var(--text3);text-align:center;margin-top:4px">최근 10건 표시 (전체 ${_btTrades.length}건)</div>` : ''}
    </div>`;
  }
  ...
  const _curTf = typeof currentTF!=='undefined' ? currentTF : 'day';
  ...
  body.innerHTML = `
    ...
    ${btStateBannerHTML}
    ${tradeHistHTML}
    <div style="padding:4px 10px;margin:0 0 4px;text-align:center"><span style="display:inline-block;font-size:9px;padding:3px 8px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">${_apStr}</span></div>
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _btMkt = stock._mkt || stock.market || currentMarket;
        const _btTfOk = _isBtSupportedTF(_btMkt, currentTF);
        ...
        // S95: 소형 인라인 배지 — 근거용 (탭→전이상세)
        return `<div style="display:flex;align-items:center;gap:6px;margin-top:6px">
          <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:4px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1.5px solid ${_timingBtnClr};cursor:pointer" onclick="var el=document.getElementById('${_3stageId}');el.style.display=el.style.display==='none'?'block':'none'">
          ...
          <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:4px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1.5px solid ${_stratBtnClr}">
        ...
        ${(()=>{
          ...
          const _report = SXI.threeStageReport(qs, _mom, stock._btResult, _btSc, _lastDate, stock._btTransitionStats);
          ...
          html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:11px;font-weight:800;color:var(--text)">진입타이밍</span><span style="font-size:14px;font-weight:800;color:var(--'+rCls+')">'+_report.ready.score+_deltaTag(_readyDelta)+'</span><span style="font-size:9px;color:var(--text3)">'+_report.ready.level+' · '+_report.ready.progress+'</span></div>';
          ...
          if(_report.ready.met.length || _report.ready.unmet.length) {
            ...
            if(_hasRU) {
              html += '<div onclick="var u=document.getElementById(\''+_rUnmetId+'\');u.style.display=u.style.display===\'none\'?\'flex\':\'none\';this.querySelector(\'span\').textContent=u.style.display===\'none\'?\'▶\':\'▼\'" style="font-size:8px;color:var(--text3);cursor:pointer;margin-bottom:3px"><span>▶</span> 미충족 조건 '+_hasRU+'개</div>';
          ...
          html += '<span style="font-weight:700;color:var(--accent)">강세 전이 '+r2e.prob+'%</span> · '+r2e.days;
          ...
          html += '<div style="margin-bottom:10px">';
          html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:11px;font-weight:800;color:var(--text)">강세</span><span style="font-size:14px;font-weight:800;color:var(--'+eCls+')">'+_report.entry.score+_deltaTag(_entryDelta)+'</span><span style="font-size:9px;color:var(--text3)">'+_report.entry.level+'</span></div>';
          ...
          if((_report.entry.met && _report.entry.met.length) || (_report.entry.unmet && _report.entry.unmet.length)) {
            ...
            if(_hasEU) {
              html += '<div onclick="var u=document.getElementById(\''+_eUnmetId+'\');u.style.display=u.style.display===\'none\'?\'flex\':\'none\';this.querySelector(\'span\').textContent=u.style.display===\'none\'?\'▶\':\'▼\'" style="font-size:8px;color:var(--text3);cursor:pointer;margin-bottom:3px"><span>▶</span> 미충족 조건 '+_hasEU+'개</div>';
          ...
          html += '<div style="margin-top:4px;padding:4px 8px;background:var(--surface);border-radius:6px;font-size:9px;line-height:1.5">';
          html += '<span style="font-weight:700;color:var(--accent)">추세 전이 '+e2t.prob+'%</span> · '+e2t.days;
          ...
          if(_mom && _mom.history.length >= 3){
            ...
            html += '<div style="margin-bottom:10px">';
            html += '<div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:4px">강세 추이 (최근 '+_mom.lookback+'봉)</div>';
          ...
          if((_report.trend.metW && _report.trend.metW.length) || (_report.trend.unmetW && _report.trend.unmetW.length)) {
            ...
            html += '</div>';
            if(_tuw.length) {
              html += '<div onclick="var u=document.getElementById(\''+_tUnmetId+'\');u.style.display=u.style.display===\'none\'?\'flex\':\'none\';this.querySelector(\'span\').textContent=u.style.display===\'none\'?\'▶\':\'▼\'" style="font-size:8px;color:var(--text3);cursor:pointer;margin-bottom:3px"><span>▶</span> 미충족 조건 '+_tuw.length+'개</div>';
          ...
          if(_mom && _mom.history.length >= 3){
            ...
            html += '<div style="margin-bottom:10px">';
            html += '<div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:4px">진입타이밍 추이 (최근 '+_mom.lookback+'봉)</div>';
      ...
      ${(()=>{
        ...
        const _pgTfOk = _isBtSupportedTF(_pgMkt, currentTF);
    ...
    ${(()=>{
      ...
      return `<div class="anal-section">
        ...
        <div class="itp-toggle-inline" onclick="const el=document.getElementById('${fiId}');el.classList.toggle('show');this.querySelector('.sb-arrow').textContent=el.classList.contains('show')?'▼':'▶'" style="font-size:10px;color:var(--accent);cursor:pointer;font-weight:600"><span class="sb-arrow">▶</span> ${fi.label}</div>
  ...
  // S31: 미니 캔들차트 그리기
  if(indicators?._advanced?.rows){
    setTimeout(()=>_drawMiniCandleChart(indicators._advanced.rows, stock._btResult?.trades), 50);
  }
  // S57: 재무 바차트 그리기
