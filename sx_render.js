...
function _trendBt(rows,cfg,bbP){
  ...
  const _lead = _auto ? 2 : ((cfg.predLead===2)?2:1);   // 게이트/라벨 최대 창
  ...
  const _gcLabelAt=(e)=>{ for(let j=e+1;j<=e+_lead;j++){ if(j<1||j>=n) break; if(maS[j]!=null&&maL[j]!=null&&maS[j-1]!=null&&maL[j-1]!=null&&maS[j]>maL[j]&&maS[j-1]<=maL[j-1]) return true; } return false; };
  ...
  const _predGc=(i)=>{
    ...
    if(!(btc <= _lead+0.5)) return 0;               // 최대 선행창 밖 (해석적 게이트)
    const qv=_pPre.vecs[i-_pPre.from]; if(!qv) return 0;
    const cand=[];
    for(let e=_pPre.from; e<=i-_lead; e++){ const cv=_pPre.vecs[e-_pPre.from]; if(!cv) continue; let d=0; for(let t=0;t<qv.length;t++){ const df=(qv[t]-cv[t])*_pW[t]; d+=df*df; } cand.push({d:d,up:_gcLabelAt(e)}); }
    if(cand.length<_MINB) return 0;
    ...
    const prob = wA>0?wU/wA:0;   // 비슷한 과거 모양이 최대창 내 교차로 이어진 비율
    if(_auto){
      // [S625] 레짐 기반 — 강추세(bull/up) 레짐이면 2봉 선행, 아니면 1봉. (S624 per-signal prob 자동은 실측상 1봉만도 못해 폐기)
      if(prob < _PTHR) return 0;                       // kNN 헤드페이크 필터는 유지
      const wantLead = _isStrong(i) ? 2 : 1;           // 강추세 → 2봉, 그 외 → 1봉
      return (btc <= wantLead + 0.5) ? wantLead : 0;
    }
    return (prob>=_PTHR) ? _lead : 0;                // 수동 고정(1/2)
  };
  for(let i=1;i<n;i++){
    ...
    if(pos==null){
      ...
      const _predFail = pos.pred && (i-pos.sigIdx>=(pos.predLead||_lead)) && !(maS[i]>maL[i]);
  ...
  if(_predOn && _pPre){
    for(let i=_pPre.from; i<n; i++){
      ...
      if(i+L > n-1) continue;                 // 미래 미확정 → 평가 제외
      _predFires++;
      let hit=false; for(let j=i+1;j<=i+L;j++){ if(maS[j]!=null&&maL[j]!=null&&maS[j-1]!=null&&maL[j-1]!=null&&maS[j]>maL[j]&&maS[j-1]<=maL[j-1]){ hit=true; break; } }
      if(hit) _predHits++;
    }
  }
  ...
  return { winRate, pf, totalPnl, totalTrades:nT, avgWin, avgLoss, mdd:+(mdd*100).toFixed(2), expectancy, trades, open, lastClose:close[n-1], predHit, predFires:_predFires };
}
...
function _trendState(rows,cfg,bt){
  ...
  if(bt.open){
    ...
    if(bt.open.pred && !bt.open.confirmed){
      return { label:`🔮 예측 진입 · ${bt.open.bars===0?'오늘':bt.open.bars+'봉 전'} 매수 · 골든크로스 미확정 (${bt.open.predLead||1}봉내 미발생 시 손절)`, color:'#7c3aed' };
    }
...
function _trendRenderInner(){
  ...
  const _predDesc=cfg.predLead==='auto'?'추세 강하면 2봉·약하면 1봉 선행 진입':('kNN이 '+_predLeadLbl+' 내 골든크로스 예측 시 선행 진입');
  ...
  return head+maRow+chipRow+reChip+predRow+stateRow+grid+note;
}
