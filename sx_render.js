// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v2.6 <!-- S85 -->
...
function _showTlDetail(el, detailId) {
  ...
  var txt = '<b>봉 #' + bar + '</b> — ';
  txt += '<span style="color:var(--accent)">준비 ' + r + '%</span> · ';
  txt += '<span style="color:var(--buy);opacity:.7">진입 ' + e + '%</span> · ';
...
function _buildBtBanner(stock, qs){
  ...
  // S79: 거래 10회 미만 → 데이터 부족
  if(trades < BT_MIN_TRADES){
    return `<div class="bt-banner info">
      <div class="bt-banner-header"><div class="bt-banner-title info">매매전략 — 데이터 부족</div>${helpBtn}</div>
      <div class="bt-action-line neutral">거래 ${trades}회 / 최소 ${BT_MIN_TRADES}회 필요</div>
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _btTrades = _getBtTotalTrades(stock);
        const _btMkt = stock._mkt || stock.market || currentMarket;
        const _btTfOk = _isBtSupportedTF(_btMkt, currentTF);
        const _btInsufficient = !_btTfOk || (_btScRaw != null && _btTrades < BT_MIN_TRADES);
        const _btSc = _btInsufficient ? null : _btScRaw;

        // S82: 이모지 제거 — verdict 변수 (현재 미사용, 향후 확장 대비 유지)
        const _readySc = qs?.readyScore ?? 0;
        const _entrySc = qs?.entryScore ?? 0;
        const _trendSc = _analSc;
        let _verdictLabel='관망', _verdictColor='var(--text3)';
        if(_trendSc>=60&&_entrySc>=50){ _verdictLabel='보유 확인'; _verdictColor='var(--buy)'; }
        else if(_entrySc>=60&&_readySc>=50){ _verdictLabel='진입 검토'; _verdictColor='var(--buy)'; }
        else if(_readySc>=60){ _verdictLabel='관심'; _verdictColor='var(--accent)'; }
        else if(_trendSc>=70&&_readySc<30){ _verdictLabel='과열 주의'; _verdictColor='var(--sell)'; }

        // S79: 모멘텀 화살표
        const _mom = stock._scoreMomentum;
        let _momArrow = '';
        if(_mom && _mom.history.length >= 2){
          const arrow = _mom.direction==='up' ? '▲' : _mom.direction==='down' ? '▼' : '—';
          const sign = _mom.delta > 0 ? '+' : '';
          const color = _mom.direction==='up' ? 'var(--buy)' : _mom.direction==='down' ? 'var(--sell)' : 'var(--text3)';
          _momArrow = `<span style="font-size:10px;color:${color};font-weight:700;margin-left:4px">${arrow}${sign}${_mom.delta}</span>`;
        }
        ...
        return `<div class="anal-score-wrap" style="flex-direction:column;gap:8px">
          <div style="display:flex;justify-content:center;gap:8px;width:100%">
            <div class="anal-score-box" style="flex:1;cursor:pointer" onclick="var el=document.getElementById('${_3stageId}');el.style.display=el.style.display==='none'?'block':'none'">
              <div class="score-num" style="font-size:22px;color:var(--${_analCls})">${_analSc}${_momArrow}</div>
              <div class="score-label" style="font-size:9px">분석 점수</div>
              <div style="font-size:7px;color:var(--accent);margin-top:2px">▶ 3단 상세</div>
            </div>
            <div class="anal-score-box" style="flex:1">
              <div class="score-num" style="font-size:22px;color:var(--${_btCls})">${_btSc!=null?_btSc:'—'}</div>
              <div class="score-label" style="font-size:9px">매매전략 (BT)</div>
              ${_btInsufficient?'<div style="font-size:7px;color:var(--text3);margin-top:2px">데이터 부족</div>':''}
            </div>
          </div>
        </div>
        ...
        ${(()=>{
          ...
          html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:11px;font-weight:800;color:var(--text)">준비</span><span style="font-size:14px;font-weight:800;color:var(--'+rCls+')">'+_report.ready.score+'</span><span style="font-size:9px;color:var(--text3)">'+_report.ready.level+' · '+_report.ready.progress+'</span></div>';
          html += '<div style="font-size:10px;color:var(--text2);line-height:1.6;margin-bottom:4px">'+_report.ready.summary+'</div>';
          // S82: 조건 체크박스 시각화 + S83: 조건별 가중치 표시
          ...
          html += '<span style="font-weight:700;color:var(--accent)">진입 전이 '+r2e.prob+'%</span> · '+r2e.days;
          ...
          html += '<div style="margin-bottom:10px">';
          html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:11px;font-weight:800;color:var(--text)">진입</span><span style="font-size:14px;font-weight:800;color:var(--'+eCls+')">'+_report.entry.score+'</span><span style="font-size:9px;color:var(--text3)">'+_report.entry.level+'</span></div>';
          html += '<div style="font-size:10px;color:var(--text2);line-height:1.6;margin-bottom:4px">'+_report.entry.summary+'</div>';
          // S84: 진입 조건별 가중치 체크박스 시각화
          ...
          html += '<div style="margin-bottom:10px">';
          html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:11px;font-weight:800;color:var(--text)">추세</span><span style="font-size:14px;font-weight:800;color:var(--'+tCls+')">'+_report.trend.score+'</span><span style="font-size:9px;color:var(--text3)">'+_report.trend.level+'</span></div>';
          html += '<div style="font-size:10px;color:var(--text2);line-height:1.6;margin-bottom:4px">'+_report.trend.position+'</div>';
          // S85: 추세 조건별 가중치 체크박스 (Ready/Entry와 동일 패턴)
          ...
          // 모멘텀 바차트 (기존 유지)
          // S83: 전이확률 추이 스파크라인, S84: 전이 마커 표시, S85: 클릭 인터랙티브
          if(stock._btTransitionStats && stock._btTransitionStats.timeline && stock._btTransitionStats.timeline.length >= 3) {
            ...
            html += '<div style="display:flex;align-items:flex-end;gap:1px;height:40px;position:relative;padding-top:10px">';
            for(var ti=0;ti<tl.length;ti++){
              var pt = tl[ti];
              var hasEvt = pt.transEvt;
              // S85: 클릭 데이터를 data 속성으로 저장
              var clickData = 'data-bar="'+pt.bar+'" data-r="'+pt.readyPct+'" data-e="'+pt.entryPct+'" data-t="'+pt.trendPct+'"';
              if(hasEvt) clickData += ' data-from="'+hasEvt.from+'" data-to="'+hasEvt.to+'" data-cnt="'+hasEvt.count+'"';
              html += '<div '+clickData+' onclick="_showTlDetail(this,\''+_tlDetailId+'\')" style="cursor:pointer;flex:1;display:flex;flex-direction:column;height:100%;position:relative'+(hasEvt?';border-left:1px solid rgba(255,255,255,.4)':'')+'">';
              html += '<div style="flex:'+pt.readyPct+';background:var(--accent,#7c5cff);opacity:.5;min-height:'+(pt.readyPct>0?'1px':'0')+';pointer-events:none"></div>';
              html += '<div style="flex:'+pt.entryPct+';background:var(--buy,#e8365a);opacity:.7;min-height:'+(pt.entryPct>0?'1px':'0')+';pointer-events:none"></div>';
              html += '<div style="flex:'+pt.trendPct+';background:var(--buy,#e8365a);min-height:'+(pt.trendPct>0?'1px':'0')+';pointer-events:none"></div>';
              // S84: 전이 마커
              if(hasEvt) {
                var mClr = hasEvt.to==='trend'?'var(--buy,#e8365a)':hasEvt.to==='entry'?'var(--accent,#7c5cff)':'var(--text3,#999)';
                var mSym = hasEvt.to==='trend'?'^':hasEvt.to==='entry'?'>':'v';
                html += '<div style="position:absolute;top:-8px;left:50%;transform:translateX(-50%);font-size:7px;font-weight:800;color:'+mClr+';line-height:1;pointer-events:none" title="'+hasEvt.from+'->'+hasEvt.to+(hasEvt.count>1?' x'+hasEvt.count:'')+'">'+mSym+'</div>';
              }
              html += '</div>';
            }
            html += '</div>';
            ...
            html += '<div style="display:flex;justify-content:space-between;font-size:7px;color:var(--text3);margin-top:2px"><span>과거</span><span>현재</span></div>';
            html += '<div style="display:flex;gap:8px;font-size:7px;color:var(--text3);margin-top:2px">';
            html += '<span style="color:var(--accent,#7c5cff)">■ 준비</span>';
            html += '<span style="color:var(--buy,#e8365a);opacity:.7">■ 진입</span>';
            html += '<span style="color:var(--buy,#e8365a)">■ 추세</span>';
            html += '<span style="margin-left:4px">^ 전이</span>';
            html += '</div>';
            html += '</div>';
          }
          ...
          if(_mom && _mom.history.length >= 3){
            html += '<div style="margin-bottom:10px">';
            html += '<div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:4px">점수 추이 (최근 '+_mom.lookback+'봉)</div>';
            html += '<div style="display:flex;align-items:flex-end;gap:2px;height:36px;margin-bottom:4px">';
            const rev = _mom.history.slice().reverse();
            for(var bi=0;bi<rev.length;bi++){
              const pct = Math.max(4, rev[bi].score);
              const prev = bi > 0 ? rev[bi-1].score : rev[bi].score;
              const barColor = rev[bi].score > prev ? 'var(--buy,#e8365a)' : rev[bi].score < prev ? 'var(--sell,#3b82f6)' : 'var(--text3,#999)';
              html += '<div style="flex:1;display:flex;flex-direction:column;align-items:center"><div style="font-size:7px;color:var(--text3);margin-bottom:1px">'+rev[bi].score+'</div><div style="width:100%;max-width:24px;height:'+pct*0.35+'px;background:'+barColor+';border-radius:2px"></div></div>';
            }
            html += '</div>';
            html += '<div style="display:flex;justify-content:space-between;font-size:8px;color:var(--text3)"><span>'+(_mom.lookback-1)+'봉전</span><span>현재</span></div>';
            html += '</div>';
          }
