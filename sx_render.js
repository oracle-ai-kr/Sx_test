...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  if(_btSt && _btSt.state !== 'no_data'){
    ...
    if(_btSt.state === 'holding'){
      _bnBg = _bnPnlPos ? 'rgba(34,197,94,.08)' : 'rgba(232,54,90,.08)';
      _bnBorder = _bnPnlPos ? '#22c55e' : '#e8365a';
      _bnIconClr = '#ff9900'; // 주황 (보유=매수)
      _bnTextClr = _bnPnlPos ? '#22c55e' : '#e8365a';
    } else if(_btSt.state === 'sell_signal'){
      const _isWin = _btSt.isWin;
      _bnBg = _isWin ? 'rgba(34,197,94,.08)' : 'rgba(232,54,90,.08)';
      _bnBorder = _isWin ? '#22c55e' : '#e8365a';
      _bnIconClr = _isWin ? '#22c55e' : '#e8365a';
      _bnTextClr = _isWin ? '#22c55e' : '#e8365a';
    } else {
    ...
    <div style="padding:10px 14px;margin:0 0 6px;border-radius:10px;background:${_bnBg};border:1.5px solid ${_bnBorder};${_btSt.state!=='waiting'?`animation:${_bnPulseId} 1.2s ease-in-out infinite`:''}">
  ...
  if(_btTrades && _btTrades.length){
    ...
    _recent.forEach((tr, idx) => {
      ...
      const row = `<div style="padding:5px 0;border-bottom:1px solid var(--border)">
        <div style="display:flex;align-items:center;gap:6px;font-size:10px"><span style="display:inline-block;width:28px;text-align:center;padding:1px 0;border-radius:3px;background:rgba(255,153,0,.15);color:#f90;font-size:9px;font-weight:700">매수</span><span style="color:var(--text2)">${_fmtDKR(tr.entryDate)}</span><span style="color:var(--text);margin-left:auto">${_entryP}</span></div>
