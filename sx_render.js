...
function _trendRenderInner(){
  ...
  const exitRow=`<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">`
    ...
    +(cfg.disSl?`<span onclick="_sxVib(10);window._trendCycleSlAtr&&_trendCycleSlAtr()" title="손절 폭(ATR 배수) 순환: 2.5→3→4→5. 넓을수록 폭락만 컷." style="font-size:10px;font-weight:800;padding:5px 10px;border-radius:14px;border:1px solid #dc262666;background:#dc262618;color:#dc2626;cursor:pointer">${_slAtrLbl}</span>`:'')
  ...
  return head+maRow+chipRow+reChip+predRow+exitRow+_guardBadge+stateRow+grid+note;
}
...
function _trendCycleSlAtr(){ const ctx=window._sxTrendCtx; if(!ctx) return; const m=ctx.market; const cfg=_trendCfg(m); const seq=[2.5,3,4,5]; const cur=cfg.slAtr>0?cfg.slAtr:3; const idx=seq.indexOf(cur); cfg.slAtr=seq[(idx+1)%seq.length]; _trendSave(m,cfg); _trendRerender(); } // [S675] 손절폭 순환 2.5→3→4→5
...
if(typeof window!=='undefined'){ window._trendApply=_trendApply; window._trendToggleAux=_trendToggleAux; window._trendToggleSell=_trendToggleSell; window._trendToggleReentry=_trendToggleReentry; window._trendToggleNextOpen=_trendToggleNextOpen; window._trendTogglePredict=_trendTogglePredict; window._trendCycleLead=_trendCycleLead; window._trendToggleEarlyMa5=_trendToggleEarlyMa5; window._trendToggleDisasterSl=_trendToggleDisasterSl; window._trendCycleSlAtr=_trendCycleSlAtr; window._trendToggleBuyOpen=_trendToggleBuyOpen; window._trendToggleSellOpen=_trendToggleSellOpen; window._trendReset=_trendReset; }
