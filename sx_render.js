...
function _buildTransitionCard(stock, indicators){
  ...
  const r = _candleTransitionScore(rows, indicators, (stock && (stock._mkt || stock.market)) || (typeof currentMarket !== 'undefined' ? currentMarket : 'kr'));
  ...
  return wrap(top + bar + _seqRow + _hitRow + warnHtml + reasonsHtml + note);
}
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _aTimingBadge = (_aTimingOn === true)  ? '<span class="tx-badge up">진입 신호</span>'
                            ...
                            : '';
        // [S515][S516][S517] 상단 배지 행 — [진입대기(A)][하락유력(C)]를 A 버튼 왼쪽 끝에 함께 묶음.
        //   〔이유〕 진입대기의 사유가 곧 하락유력(C)이라, 한곳에 모으면 인과로 읽혀 깔끔(S516 좌우 분리 → 좌측 그룹으로 변경).
        //   각 배지 margin-left:4px라 사이 간격 자동. 둘 중 하나만 있어도 그대로 노출, 둘 다 없으면 행 생략.
        const _topBadgeRow = (_aTimingBadge || _verdictBadgeTop)
          ? `<div style="display:flex;justify-content:flex-start;align-items:center;margin-top:8px;margin-bottom:-2px">${_aTimingBadge}${_verdictBadgeTop}</div>`
          : '';
