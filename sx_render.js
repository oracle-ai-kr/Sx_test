...
function _sxBuilderAutoScan(){
  ...
  var results=cands.map(function(D){
    ...
    return { label:D.label, type:D.type, cond:c, n10:_rate(m,'n10'), hold:_rate(m,'hold'), n:m.length };
  ...
  if(!ok.length && !low.length){ h+='<div style="font-size:9.5px;color:'+T3+';padding:6px">후보 없음 (모든 재료가 이미 체크됨).</div>'; }
  else {
    if(ok.length){ h+='<div style="font-size:8.5px;font-weight:800;color:'+SKY+';margin:6px 0 2px">✅ 표본 30+ · N10 높은 순 ('+ok.length+'개)</div>'; h+=ok.map(function(r){return _row(r,false);}).join(''); }
    else { h+='<div style="font-size:9px;color:'+RED+';padding:4px">표본 30+ 후보 없음 — 베이스가 너무 좁아요(조건 완화/제거).</div>'; }
    if(low.length){ h+='<div style="font-size:8px;color:'+T3+';margin:7px 0 2px">⚠️ 표본 30 미만 (과적합 위험 · 상위 '+Math.min(6,low.length)+')</div>'; h+=low.slice(0,6).map(function(r){return _row(r,true);}).join(''); }
  }
  h+='<div style="font-size:8px;color:'+T3+';margin-top:6px;line-height:1.4">▲/▼=베이스 대비 N10 변화. 좋은 후보를 위에서 <b>체크→베이스에 추가→다시 자동탐색</b>(forward greedy). <b style="color:'+GRN+'">N10·유지 둘 다 오르는 게 진짜 반등</b>. BT 무관 순수 데이터.</div>';
  out.innerHTML=h;
}
