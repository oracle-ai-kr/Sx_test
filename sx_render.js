...
function _buildTransitionCard(stock, indicators){
  ...
  const _knNbrBar = _useKnn ? `<div style="display:flex;align-items:center;gap:9px;margin:10px 0 2px">
    ...
    <div style="flex:1;display:flex;height:18px;border-radius:9px;overflow:hidden;background:var(--surface2);box-shadow:inset 0 0 0 1px var(--border)">
      ${_kn.nUp>0?`<div style="width:${Math.round(_kn.nUp/_kn.k*100)}%;background:#e3493b;display:flex;align-items:center;justify-content:center"><span style="font-size:10.5px;font-weight:800;color:#fff">🔺${_kn.nUp}</span></div>`:''}
      ${_kn.nDn>0?`<div style="width:${Math.round(_kn.nDn/_kn.k*100)}%;background:#2563eb;display:flex;align-items:center;justify-content:center"><span style="font-size:10.5px;font-weight:800;color:#fff">🔻${_kn.nDn}</span></div>`:''}
    </div>
  </div>` : '';
  // [S611] kNN 적중률 인라인 — 종목 캐시 적중 시 즉시, 없으면 비동기 측정 후 span 교체(렌더 블로킹 방지). 모달 [검증]과 동일 수치.
  let _hitHtml = '';
  if(_useKnn){
    const _hk = ((stock&&(stock.code||stock.name))||'') + '|' + _tfCT + '|' + (_lastDate||'');
    if(Object.prototype.hasOwnProperty.call(_ctKnnHitCache, _hk)){
      _hitHtml = _ctKnnHitInline(_ctKnnHitCache[_hk]);
    } else {
      ...
      (function(_r,_key,_id){ setTimeout(function(){
        ...
        var el=document.getElementById(_id); if(el) el.outerHTML=_ctKnnHitInline(res);
      }, 30); })(rows, _hk, _hid);
    }
  }
  ...
  return wrap(top + _knNbrBar + confHtml + bar + _seqRow + _hitRow + warnHtml + reasonsHtml + note);
}
