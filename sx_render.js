    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _aTimingBadge = (_aTimingOn === true)  ? '<span class="tx-badge up">진입 신호</span>'
                            ...
                            : '';
        // [S515][S516] 상단 배지 행: 좌=A 타이밍 / 우=C 판정. 버튼 행과 동일하게 flex:1 두 칸 → 각 배지가 자기 버튼 왼쪽 끝에 정렬(하락유력=B버튼 왼쪽끝, 진입대기처럼). 둘 다 없으면 행 생략.
        const _topBadgeRow = (_aTimingBadge || _verdictBadgeTop)
          ? `<div style="display:flex;align-items:center;gap:6px;margin-top:8px;margin-bottom:-2px"><div style="flex:1">${_aTimingBadge}</div><div style="flex:1">${_verdictBadgeTop}</div></div>`
          : '';
