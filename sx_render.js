...
function _btDiscrimRender(rep, watch, mk){
  ...
  // ===== 참고: 기존 단일봉 이진 (비교용·노이즈) =====
  out+='<div style="margin-top:10px;padding-top:8px;border-top:1px dashed var(--border);opacity:.62">';
  out+='<div style="font-size:9.5px;font-weight:800;color:var(--text2);margin-bottom:3px">참고 — 기존 단일봉 이진(N3/N5/N10/유지) · 노이즈 비교용</div>';
...
function _sxBuilderAutoScan(scanMode){
  ...
  var results=cands.map(function(D){
    ...
    return { label:D.label, key:D.key, type:D.type, cond:c, n3:_rate(m,'n3'), n5:_rate(m,'n5'), n10:_rate(m,'n10'), hold:_rate(m,'hold'), n:m.length };
