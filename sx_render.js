...
async function runAnalysis(stock){
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  try{
    // [S357] 구버전 캐시(upsideScore 미포함) 감지 → 재계산하여 추가상승 추이 정상화
    //   스캔워커가 넘긴 _scoreMomentum이 upside 추가 이전 버전이면 history에 upsideScore가 없어
    //   추가상승 추이바가 0으로 표시됨 → 키 부재 감지 시 강제 재계산
    var _momStale = stock._scoreMomentum && stock._scoreMomentum.history && stock._scoreMomentum.history.length && !('upsideScore' in stock._scoreMomentum.history[0]);
      ...
      ${(()=>{
        ...
        ${(()=>{
          ...
          var _zoneStyle = function(zone){
            ...
            if(active) return 'padding:8px 10px 4px;border-left:4px solid var(--accent);background:var(--accent-glow);border-radius:0 6px 6px 0;';
            return 'opacity:.42;'; // 비활성 경로 → 차분하게
          };
          ...
          if(_report.upside){
            ...
            html += '<div style="font-size:10px;color:var(--text2);line-height:1.6;margin-bottom:4px">'+_report.upside.summary+'</div>';
