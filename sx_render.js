// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v2.2 <!-- S81 -->
...
// S81: 3단 배지 필터 상태
let _stageBadgeFilter = null; // null | 'ready' | 'entry' | 'trend'

function _setStageBadgeFilter(stage) {
  _stageBadgeFilter = (_stageBadgeFilter === stage) ? null : stage;
  renderResults();
}

// S81: 3단 배지 판정 — 종목의 현재 단계 판별
...
function _countStageBadges(stocks) {
  var cnt = { ready: 0, entry: 0, trend: 0 };
  for (var i = 0; i < stocks.length; i++) {
    var st = _getStockStage(stocks[i]);
    if (st === 'ready') cnt.ready++;
    else if (st === 'entry') cnt.entry++;
    else if (st === 'trend') cnt.trend++;
  }
  return cnt;
}
...
function renderResults(){
  ...
  // S81: 3단 배지 카운트
  const _badgeCnt = _countStageBadges(filtered);
  const _hasBadges = _badgeCnt.ready + _badgeCnt.entry + _badgeCnt.trend > 0;

  // S81: 배지 필터 적용
  const displayed = _stageBadgeFilter ? filtered.filter(s => _getStockStage(s) === _stageBadgeFilter) : filtered;
  ...
  area.innerHTML = `
    ...
    ${_hasBadges ? `<div style="display:flex;gap:6px;padding:4px 12px 6px;flex-wrap:wrap">
      <span onclick="_setStageBadgeFilter('ready')" style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter==='ready'?'var(--accent)':'var(--border)'};background:${_stageBadgeFilter==='ready'?'var(--accent)':'var(--surface)'};color:${_stageBadgeFilter==='ready'?'#fff':'var(--text2)'}">👀 관심 ${_badgeCnt.ready}</span>
      <span onclick="_setStageBadgeFilter('entry')" style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter==='entry'?'var(--buy)':'var(--border)'};background:${_stageBadgeFilter==='entry'?'var(--buy)':'var(--surface)'};color:${_stageBadgeFilter==='entry'?'#fff':'var(--text2)'}">🔥 진입검토 ${_badgeCnt.entry}</span>
      <span onclick="_setStageBadgeFilter('trend')" style="cursor:pointer;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:700;border:1px solid ${_stageBadgeFilter==='trend'?'var(--buy)':'var(--border)'};background:${_stageBadgeFilter==='trend'?'var(--buy)':'var(--surface)'};color:${_stageBadgeFilter==='trend'?'#fff':'var(--text2)'}">✅ 보유확인 ${_badgeCnt.trend}</span>
      ${_stageBadgeFilter ? '<span onclick="_setStageBadgeFilter(null)" style="cursor:pointer;padding:3px 8px;border-radius:12px;font-size:9px;border:1px solid var(--border);color:var(--text3)">전체</span>' : ''}
    </div>` : ''}
          ...
          <div class="sr-body">
            ...
            <div class="sr-row2">
              ...
              ${(()=>{
                ...
                if(t>=60&&e>=50){ _badgeEmoji='✅'; _badgeLabel='보유확인'; _badgeColor='var(--buy)'; }
                else if(e>=60&&r>=50){ _badgeEmoji='🔥'; _badgeLabel='진입검토'; _badgeColor='var(--buy)'; }
                else if(r>=60){ _badgeEmoji='👀'; _badgeLabel='관심'; _badgeColor='var(--accent)'; }
                else if(t>=70&&r<30){ _badgeEmoji='⚠️'; _badgeLabel='과열주의'; _badgeColor='var(--sell)'; }
                else { _badgeEmoji='⏸️'; _badgeLabel='관망'; }
                return '<span class="sr-score-chip" style="font-size:9px;color:'+_badgeColor+'">'+_badgeEmoji+' '+_badgeLabel+'</span>';
              })()}
              ${(()=>{
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _apStr = `📐 RSI${_ap.rsiLen} BB${_ap.bbLen}×${_ap.bbMult} ATR${_ap.atrLen} MA${_effMaS}/${_effMaM}/${_effMaL} B${_effBuy} S${_effSell} TP${_effTp} SL${_effSl} · 레짐${_raOn?'ON':'OFF'}`;
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        let _verdictEmoji='⏸️', _verdictLabel='관망', _verdictColor='var(--text3)';
        if(_trendSc>=60&&_entrySc>=50){ _verdictEmoji='✅'; _verdictLabel='보유 확인'; _verdictColor='var(--buy)'; }
        else if(_entrySc>=60&&_readySc>=50){ _verdictEmoji='🔥'; _verdictLabel='진입 검토'; _verdictColor='var(--buy)'; }
        else if(_readySc>=60){ _verdictEmoji='👀'; _verdictLabel='관심'; _verdictColor='var(--accent)'; }
        else if(_trendSc>=70&&_readySc<30){ _verdictEmoji='⚠️'; _verdictLabel='과열 주의'; _verdictColor='var(--sell)'; }
        ...
        return `<div class="anal-score-wrap" style="flex-direction:column;gap:8px">
          <div style="text-align:center;margin-bottom:4px">
            <span style="font-size:16px">${_verdictEmoji}</span>
            <span style="font-size:12px;font-weight:800;color:${_verdictColor};margin-left:4px">${_verdictLabel}</span>
          </div>
        ...
        ${(()=>{
          ...
          const _report = SXI.threeStageReport(qs, _mom, stock._btResult, _btSc, _lastDate);
          ...
          if(_report.ready.notes.length) html += '<div style="font-size:9px;color:var(--text3);line-height:1.5">'+_report.ready.notes.map(function(n){return '· '+n}).join('<br>')+'</div>';
          // 전이 확률
          ...
          html += '<span style="font-weight:700;color:var(--accent)">→ Entry 전이 '+r2e.prob+'%</span> · '+r2e.days;
          ...
          html += '<div style="margin-top:4px;padding:4px 8px;background:var(--surface);border-radius:6px;font-size:9px;line-height:1.5">';
          html += '<span style="font-weight:700;color:var(--accent)">→ Trend 전이 '+e2t.prob+'%</span> · '+e2t.days;
          ...
          html += '<div style="font-size:10px;color:var(--text2);line-height:1.6">'+_report.trend.position+'</div>';
          html += '</div>';
          ...
          if(_report.staleness.warning){
            ...
            html += '<span style="font-weight:700;color:#ff8c00">⏰ '+_report.staleness.label+'</span><br>'+_report.staleness.warning;
            html += '</div>';
          }

          // 최종 판정
          html += '<div style="margin-top:8px;padding:8px 10px;background:var(--surface);border-radius:8px;border:1px solid var(--border)">';
          html += '<div style="font-size:11px;font-weight:800;color:var(--text);margin-bottom:4px">'+_report.verdict.emoji+' '+_report.verdict.label+'</div>';
