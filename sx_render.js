  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // S93: BT 점수 항상 표시 + 데이터 신뢰도 3색 텍스트
        const _btNoData = !_btTfOk || _btScRaw == null; // BT 미실행 or TF 미지원
        const _btSc = _btNoData ? null : _btScRaw;
        // 신뢰도: 부족(<10, 빨강) / 충족(10~29, 파랑) / 충분(30+, 녹색)
        let _btReliLabel = '', _btReliColor = '';
        if(!_btNoData){
          if(_btTrades < BT_MIN_TRADES){ _btReliLabel='데이터 부족'; _btReliColor='var(--sell)'; }
          else if(_btTrades < 30){ _btReliLabel='데이터 충족'; _btReliColor='var(--accent)'; }
          else { _btReliLabel='데이터 충분'; _btReliColor='var(--buy)'; }
        }
        ...
        return `<div class="anal-score-wrap" style="flex-direction:column;gap:8px">
          <div style="display:flex;justify-content:center;gap:8px;width:100%">
            ...
            <div class="anal-score-box" style="flex:1">
              ...
              ${_btReliLabel?`<div style="font-size:7px;color:${_btReliColor};font-weight:700;margin-top:2px">${_btReliLabel}</div>`:''}
              ${_btNoData?'<div style="font-size:7px;color:var(--text3);margin-top:2px">데이터 없음</div>':''}
            </div>
          </div>
        </div>
