...
function _sxBuilderAutoScan(){
  ...
  var _row=function(r, dim){
    var dCol=(r.dN10==null)?T3:(r.dN10>0.02?GRN:r.dN10<-0.02?RED:T3);
    var dTxt=(r.dN10!=null)?((r.dN10>=0?'▲+':'▼')+Math.round(100*r.dN10)+'p'):'';
    var n10c=(r.n10!=null&&baseN10!=null&&r.n10>baseN10)?GRN:'var(--text)';
    ...
    return '<div style="display:flex;align-items:center;gap:5px;padding:5px 7px;margin:2px 0;background:var(--surface2);border-radius:6px'+(dim?';opacity:.5':'')+'">'
      ...
      +'<span style="font-size:8px;color:'+dCol+';font-weight:800;width:40px;text-align:right">'+dTxt+'</span>'
  ...
  else if(!pass.length){ h+='<div style="font-size:9.5px;color:'+T3+';padding:8px 6px;line-height:1.5">합격 후보 없음 <span style="font-size:8px">(표본30+ · N10▲ · 유지50%+)</span><br>표본 30+ 중 미합격 '+okFail+'개 · 표본부족 '+low.length+'개.'+(ok.length?' 베이스보다 N10 올리는 후보가 없어요 — 베이스 조건을 바꿔보세요.':'')+'</div>'; }
  else {
    h+='<div style="font-size:8.5px;font-weight:800;color:'+GRN+';margin:6px 0 3px">✅ 합격 '+pass.length+'개 <span style="color:'+T3+';font-weight:500">· 표본30+ · N10▲ · 유지50%+</span></div>';
    h+=pass.map(function(r){return _row(r,false);}).join('');
    h+='<div style="font-size:8px;color:'+T3+';margin:5px 0 0">그 외: 표본30+ 미합격 '+okFail+'개 · 표본부족 '+low.length+'개 (숨김)</div>';
  }
  h+='<div style="font-size:8px;color:'+T3+';margin-top:6px;line-height:1.4">합격=베이스보다 N10 오르고(▲)+유지 50%+ · 표본 30+. 좋은 후보 <b>체크→베이스 추가→재탐색</b>. <b style="color:'+GRN+'">💾로 합격만 JSON 저장</b>.</div>';
  out.innerHTML=h;
}
...
function _btBuilderUI(){
  ...
  h+='<div style="border-top:1px dashed var(--border);margin-top:10px;padding-top:8px"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:4px"><button onclick="_sxBuilderAutoScan()" style="font-size:10px;font-weight:800;padding:6px 12px;border-radius:8px;border:1px solid #0ea5e9;background:#0ea5e9;color:#fff;cursor:pointer">🔍 자동탐색</button><button onclick="_sxAutoScanDownload()" style="font-size:10px;font-weight:700;padding:6px 10px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text2);cursor:pointer">💾 합격 JSON</button><span style="font-size:8.5px;color:var(--text3);line-height:1.4;flex:1;min-width:90px">체크=베이스 · 미체크 하나씩 추가</span></div><div id="sxbAutoOut" style="margin-top:4px"></div></div>';
  h+='</div>';
  return h;
}
