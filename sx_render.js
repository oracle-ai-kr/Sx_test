...
async function _cvPoolBracket(mk, source, onProgress){
  ...
  if(SXVVAL.ablateIC){ try{ P.ablate=SXVVAL.ablateIC(records); }catch(_e){} }   // [S721] 재료 ablation IC(전체/강세봉/약세봉)
...
function _cvPoolRender(rep, watch, mk){
  ...
  function bracket(P, label){
    ...
    // [S721] 재료 ablation IC — ready 재료 빼면 readyScore IC ΔIC (전체/강세봉/약세봉)
    if(P.ablate && P.ablate.ok && P.ablate.all){
      var ab=P.ablate, dcol=function(v){ return v==null?MUT:(v>0.02?RED:v<-0.02?GRN:MUT); };
      h+='<div style="margin-top:11px;padding-top:8px;border-top:1px dashed var(--border)"><div style="font-size:10px;font-weight:800;color:#d97706;margin-bottom:1px">🧪 반등신호 재료 ablation IC</div>';
      h+='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:3px">재료 빼면 readyScore IC가 어떻게? baseline IC <b>'+sg(ab.all.base)+'</b> · <b style="color:'+RED+'">Δ&gt;0=빼면 IC↑(노이즈/해로움)</b> · <b style="color:'+GRN+'">Δ&lt;0=기여</b> · 익일 기준</div>';
      ...
      h+='<table style="width:100%;border-collapse:collapse;font-size:8.5px"><tr style="color:'+T3+'"><td style="text-align:left">재료</td><td style="text-align:right">전체</td><td style="text-align:right">강세봉('+((ab.bull&&ab.bull.n)||0)+')</td><td style="text-align:right">약세봉('+((ab.bear&&ab.bear.n)||0)+')</td></tr>';
      ord.forEach(function(r){ h+='<tr><td style="text-align:left;padding:1px 0;color:var(--text2)">'+r.key+'</td><td style="text-align:right;font-weight:800;color:'+dcol(r.a)+'">'+sg(r.a)+'</td><td style="text-align:right;color:'+dcol(r.b)+'">'+sg(r.b)+'</td><td style="text-align:right;color:'+dcol(r.c)+'">'+sg(r.c)+'</td></tr>'; });
      h+='</table></div>';
    } else if(P.ablate && !P.ablate.ok){
      h+='<div style="margin-top:8px;font-size:8.5px;color:#d97706">🧪 재료 ablation: '+(P.ablate.reason||'불가')+'</div>';
    }
    return h;
  }
