...
function _candleTransitionScore(rows, indicators){
  ...
  if(slope > 0.005 && pos60 > 0.5){ score += 20; reasons.push('상승추세 눌림목 (20MA↑ · 60일 고점권)'); }
  else if(slope < -0.005 && pos60 < 0.4){ score -= 25; warn = '하락추세 진행 중 — 연속 음봉이 추세 하락의 일부일 수 있음'; reasons.push('⚠️ 하락추세 (20MA↓ · 60일 저점권)'); }
  else { score += 5; reasons.push('추세 횡보'); }

  // C2 매도압력 고갈 — 거래량 위축 + 몸통 축소
  let vol5 = 0, vc = 0; for(let i=Math.max(0,last-5); i<last; i++){ vol5 += V(i); vc++; } vol5 = vc ? vol5/vc : 0;
  if(vol5 > 0 && V(last) < vol5 * 0.7){ score += 10; reasons.push('거래량 위축 (매도압력 고갈)'); }
  const body = Math.abs(C(last)-O(last)), bodyPrev = Math.abs(C(last-1)-O(last-1));
  if(bodyPrev > 0 && body < bodyPrev){ score += 8; reasons.push('음봉 몸통 축소'); }

  // C3 지지선 겹침 — 200MA · 직전 전고점 · 볼린저 하단 (현재가 ±3%)
  ...
  if(prevSeg.length && near(Math.max(...prevSeg.map(r=>+r.high)))){ confl++; reasons.push('직전 전고점 매물대'); }
  const bbLow = (indicators && indicators._advanced && indicators._advanced.bb && indicators._advanced.bb.lower)
              ? indicators._advanced.bb.lower
              : (indicators && indicators.bb && indicators.bb.lower) ? indicators.bb.lower : null;
  if(near(bbLow)){ confl++; reasons.push('볼린저 하단'); }
  if(confl > 0){ score += confl * 8; if(confl >= 2) reasons.push(`지지선 ${confl}겹 — 매수세 대기 자리`); }

  // C4 낙폭 / 이격 — 20일 고점 대비 조정폭
  ...
  // C5 마지막 음봉 꼬리
  const rng = H(last) - L(last), lowerWick = Math.min(O(last),C(last)) - L(last);
  const wickRatio = rng>0 ? lowerWick/rng : 0, bodyRatio = rng>0 ? body/rng : 0;
  if(wickRatio >= 0.30){ score += 8; reasons.push('아래꼬리 — 저가 매수세 유입'); }
  else if(wickRatio < 0.05 && bodyRatio > 0.7){ score -= 12; reasons.push('⚠️ 장대음봉 (꼬리 없음 — 추가 하락 여지)'); }

  score = Math.max(0, Math.min(100, Math.round(score)));
  const label = score >= 70 ? '반등 유망' : score >= 50 ? '중립' : '약함';
  return { active:true, score, label, reasons, warn, redCnt };
}
...
function _buildTransitionCard(stock, indicators){
  ...
  const head = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:13px;font-weight:800;color:var(--text)">🧪 캔들 전이 점수</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span></div>`;
  ...
  if(!r.active){
    return wrap(`<div style="font-size:12px;color:var(--text3)">전이 대기 — ${r.reason}${r.redCnt!=null?` (연속 음봉 ${r.redCnt})`:''}</div>`);
  }
  const col = r.score>=70 ? '#16a34a' : r.score>=50 ? '#f59e0b' : '#ef4444';
  const top = `<div style="display:flex;align-items:baseline;gap:8px"><span style="font-size:26px;font-weight:800;color:${col}">${r.score}</span><span style="font-size:13px;font-weight:700;color:${col}">${r.label}</span><span style="font-size:10px;color:var(--text3);margin-left:auto">음봉 ${r.redCnt}연속</span></div>`;
  const bar = `<div style="height:7px;border-radius:4px;background:var(--surface2);overflow:hidden;margin:7px 0"><div style="height:100%;width:${r.score}%;background:${col};transition:width .4s"></div></div>`;
  ...
  const note = `<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · 음봉→양봉 전이 유망도(휴리스틱)</div>`;
  return wrap(top + bar + warnHtml + reasonsHtml + note);
}
...
function _sxbHTML(){
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    ...
    + `<div class="sxb-head-badges">${transBadge}${whipBadge}${_dumpBadge}${_safetyBadge}${_mixBadge}${_mtfBadge}</div>`
