...
function _rcDonutSvg(R){
  var SZ = 176, cx = SZ/2, cy = SZ/2, RAD = 60, SW = 15, C = 2 * Math.PI * RAD, seg = C / 4;
  // 4 부채꼴 = dasharray(seg gap)+rotate. SVG 시계방향(3시→6시). 회전으로 사분면 배치:
  //   되돌림 우하(0) · 데드캣 좌하(90) · 바닥반등 좌상(180) · 눌림목 우상(270)
  var arcs = [ {k:'retrace',rot:0},{k:'deadcat',rot:90},{k:'bottom',rot:180},{k:'pullback',rot:270} ];
  ...
  var rad = R.ang * Math.PI / 180, mx = cx + RAD*Math.cos(rad), my = cy - RAD*Math.sin(rad);
  ...
  var labels = lbl(135,'bottom') + lbl(45,'pullback') + lbl(315,'retrace') + lbl(225,'deadcat');
  ...
  return '<svg viewBox="0 0 '+SZ+' '+SZ+'" width="'+SZ+'" height="'+SZ+'" style="display:block;margin:0 auto">'+ring+labels+marker+center+'</svg>';
}
function _buildReboundCycleCard(stock, indicators){
  ...
  var note = '<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px;line-height:1.55">추세구조(X)×반등품질(Y)을 도넛에 매핑한 <b>현위치</b> 시각화. 마커가 들어간 사분면 호 = 현재 단계. 강도=중앙에서 얼마나 또렷한지. 반등 국면이 약해도 구조가 가리키는 위치를 유지합니다. C 판정·점수와 독립된 <b>실험 표시 카드</b>(매매 신호 아님).</div>';
  return _sxExpCard('sxRcCardWrap', 'sxRcCardWrap_b', _titleHtml, _rightHtml, donut + tip + xRow + yRow + cycle + note);
}
