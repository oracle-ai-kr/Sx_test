...
function _cpBtOverlay(inner){
  ...
  if(!ov){ ov=document.createElement('div'); ov.id='sxCpBtOv';
    ...
    document.body.appendChild(ov);
    // [S670] 안드로이드 뒤로가기로 모달 닫기 — 열 때 히스토리 1개 push, popstate에서 닫기
    try{
      if(!window._cpModalPopBound){ window._cpModalPopBound=true;
        window.addEventListener('popstate', function(){ var o=document.getElementById('sxCpBtOv'); if(o){ window._cpModalBack=true; _cpCloseBacktest(); window._cpModalBack=false; } });
      }
      window._cpModalPushed=true; history.pushState({_sxCpModal:1},'');
    }catch(_){}
  }
...
function _cpCloseBacktest(){ var ov=document.getElementById('sxCpBtOv'); if(ov) ov.remove();
  try{ if(window._cpModalPushed){ window._cpModalPushed=false; if(!window._cpModalBack) history.back(); } }catch(_){}   // [S670] 수동 닫기 땐 push한 히스토리 정리
}
...
function _cvRunValidation(cid){
  ...
  if(!(window.SXE&&SXE.scrQuickScore)||!(window.SXC&&SXC.unifiedVerdictV2)){ body.innerHTML='<div style="font-size:11px;color:#f59e0b">분석/C 엔진 미로드</div>'; return; }
