...
function _cpDrawCanvas(id, qPct, paths, medPath){
  try{
    ...
    ctx.strokeStyle=mfin>=0?'#e3493b':'#2563eb'; ctx.lineWidth=2.4; ctx.beginPath();
...
function _buildChartPredictCard(stock, indicators){
  ...
  let r; try { r = SXCP.predict(rows); } catch(_e){ return ''; }
  if(!r || !r.active) return '';   // 표본부족/평탄/단기이력 → 숨김(캔들전이와 동일 정책)
  ...
  const head = `<div style="display:flex;justify-content:space-between;align-items:center;gap:6px;flex-wrap:wrap">
    <div style="display:flex;align-items:center;gap:6px">
      ...
      <span style="font-size:9px;font-weight:800;color:${PUR};padding:2px 6px;border-radius:6px;background:${PUR}1a;border:1px solid ${PUR}55">실험</span>
    </div>
  ...
  const binRow = (i) => {
    ...
    return `<div style="display:flex;align-items:center;gap:6px;margin:2px 0">
      ...
      <span style="width:30px;font-size:9px;font-weight:700;color:${r.binsPct[i]?bc:'var(--text3)'};text-align:left;flex-shrink:0">${r.binsPct[i]}%</span>
    </div>`;
  };
  ...
  return `<div style="margin:0 0 10px;padding:12px 14px;background:#fff;border:1px solid var(--border);border-radius:12px">${head}${stats}${canvas}${dist}${note}</div>`;
}
