...
function _candleTransitionScore(rows, indicators, market, tf){
  ...
  const _applyExtra = () => {
    ...
    // [S536] 볼린저밴드 몸통(종가) 이탈 → 다음봉 되돌림 (mean-reversion). 밴드 밖 종가 = 과확장 → 반대 방향 가점. 전 모드 공통·대칭. 바스켓 검증대상.
    //   C3(밴드 '근처'=지지/저항)와 별개 개념(밴드 '밖'=과확장 되돌림). bb.upper/lower는 평탄·풀 ind 둘 다 보유.
    ...
    if(_bbX && _bbX.upper > 0 && _bbX.lower > 0){
      const _clX = C(last);
      if(_clX > _bbX.upper){ score -= 10; reasons.push('⚠️ 볼린저 상단 이탈 (종가 밴드 밖 — 다음봉 되돌림 여지)'); }
      else if(_clX < _bbX.lower){ score += 10; reasons.push('볼린저 하단 이탈 (종가 밴드 밖 — 다음봉 반등 여지)'); }
    }
    // [S537] 장대봉(꼬리 거의 없는 큰 몸통) — 단일봉 과확장 → 다음봉 되돌림. streak 무관·전 모드 공통(BB이탈과 동일 성격). S535에서 C5(연속 모드 한정)에 있던 걸 단일봉 신호라 이리로 이동.
    ...
    if(_rngL > 0){
      ...
      if(_bRL > 0.7){
        ...
        if(_bullL && _upWL < 0.05){ score -= 10; reasons.push('⚠️ 장대양봉 (단일봉 과확장 — 다음봉 되돌림 여지)'); }
        else if(!_bullL && _dnWL < 0.05){ score += 10; reasons.push('장대음봉 (단일봉 과확장 — 다음봉 반등 여지)'); }
      }
    }
  };
  ...
  // C5 마지막 봉 꼬리 — 반등: 아래꼬리(매수세) / 반전: 윗꼬리(매도세). [S537] 장대봉(단일봉 과확장)은 _applyExtra로 이동.
...
function _ctConfidence(score, market){
  var mk = _ctNormMkt(market), a = Math.abs(score);
  if(a < 30)        return { label:'예측 임계 미만', color:'#6b7280', why:'|점수| 30 미만 — 방향 예측 보류 구간' };
  if(mk === 'kr')   return { label:'예측력 약함 (KR)', color:'#6b7280', why:'KR 바스켓은 전 임계에서 과거 적중이 baseline 미달 — 이 시장은 다음봉 색 예측이 잘 통하지 않음' };
  if(score > 0)     return { label:'신뢰 낮음 (양봉)', color:'#dc2626', why:'양봉 전이 예측은 3시장 바스켓 어디서도 baseline 미달 — 상승 전이는 노이즈가 커 신뢰 낮음' };
  if(a >= 40 && a <= 44) return { label:'신뢰 높음 ⭐', color:'#16a34a', why:'음봉·40~44·' + mk + ' = 과거 바스켓 sweet spot (baseline +7~10%p · 상회 10~11/12종목)' };
  if(a >= 45)       return { label:'신뢰 보통 (과열)', color:'#f59e0b', why:'|점수|≥45는 과열·극단권 — 강할수록 되돌림 위험(과거 45+ edge 급락). 40~44가 최적 구간' };
  return { label:'신뢰 보통', color:'#f59e0b', why:'음봉 30~39 — 약한 신호. sweet spot은 40~44' };
}
...
function _buildTransitionCard(stock, indicators){
  ...
  const _conf = _ctConfidence(r.score, _mktCT);
