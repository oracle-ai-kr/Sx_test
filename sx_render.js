...
function _buildR1S1Card(stock, indicators){
  ...
  const _px = _pivot?.price ?? indicators?.price ?? stock.price ?? null;
...
function _buildAEntryLine(stock, svVerdict, indicators){
  ...
  const _pivot = indicators?.pivot || indicators?._advanced?.pivot || null;
  const _pxA = _pivot?.price ?? indicators?.price ?? stock.price ?? null;
...
function _sxbHTML(){
  ...
  // [S476] 강건성 불안·과최적 의심 중복 차감 — calcBtScore와 동일 조건/수치(−8/−12)를 전광판 종합점수에도 적용(의도된 중복, 변별력 확보).
  {
    const _dg = B._diag;
    if(_dg){
      let _dp = 0;
      if(_dg.robFragile) _dp += 8;
      if(_dg.overfit)    _dp += 12;
      if(_dp > 0){
        _displayTotal = Math.max(0, _displayTotal - _dp);
        if(window._sxBoard){
          if(!window._sxBoard._riskInfo) window._sxBoard._riskInfo = { orig: total, display: _displayTotal, penalty: 0, viol: [], capApplied: false, cap: 0 };
          window._sxBoard._riskInfo.diagPenalty = _dp;          // 강건성/과최적 감점분(토스트 표기용)
          window._sxBoard._riskInfo.display = _displayTotal;     // 최종 표시값 갱신
        }
      }
    }
  }
  ...
  let _safetyBadge = '';
  {
    ...
    if(_svB.length){
      const _capped = !!(B.verdict && B.verdict.capReason);
      // [S476] 안전필터 계열 색 통일 — 강등/위반 무관 보라(#a855f7) 고정.
      //   사유: 위반만일 때 주황(#f59e0b)이 매수신호 혼조 배지와 같은 색이라 "안전필터 ↔ 혼조"가 UI에서 혼동됨.
      //   강등 여부는 라벨 텍스트('안전필터감지' vs '위험 N건')로만 구분하고 색은 안전필터 계열로 일관 유지.
      const _SC = '#a855f7';                                             // 보라 통일
      const _label = _capped ? '🔒 안전필터감지' : `🔒 위험 ${_svB.length}건`;
...
function _computeBoardInputs(stock, indicators, scores, scoreMom, tf){
  ...
  // [S476] 강건성/과최적 플래그 — 전광판 종합점수 중복 차감용(calcBtScore와 동일 조건). stock에서 추출해 extras로 전달.
  _boardExtras._diag = {
    robFragile: !!((stock._robustness && stock._robustness.label === 'fragile') || (stock._coreDiag && stock._coreDiag.rob === 'fragile')),
    overfit:    !!(stock._coreDiag && stock._coreDiag.overfit)
  };
...
function _buildScoreBoard(scores, sv4, structPos, pbScore, D, verdict, lowConf, extras){
  extras = extras || {};
  window._sxBoard = { verdict: verdict||null, lowConf: !!lowConf, lowConfReasons: (extras && extras._lowConfReasons) || [], dumpWarn: (extras && extras.dumpWarn) || null, mixWarn: (extras && extras.mixWarn) || null, _diag: (extras && extras._diag) || null, groups: _buildBoardGroups(scores, sv4, structPos, pbScore, D, extras) };
