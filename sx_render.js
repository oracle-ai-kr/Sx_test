...
function _buildTransitionCard(stock, indicators){
  ...
  const _pRight = _useKnn
    ? `닮은꼴 ${_kn.k}개 중 양${_kn.nUp}·음${_kn.nDn}`
  ...
  const confHtml = _useKnn
    ? `<div style="margin-top:8px"><span style="font-size:11px;font-weight:800;color:#7c3aed;padding:3px 9px;border-radius:7px;background:#7c3aed1a;border:1px solid #7c3aed55">🧬 자기유사도 kNN · 검증 신호</span><div style="font-size:10px;color:var(--text3);line-height:1.55;margin-top:5px">최근 16봉 패턴과 가장 닮은 과거 ${_kn.k}봉의 다음봉 색으로 예측. 백테스트에서 룰 상회 확인(7종목 평균). |점수|≥30만 방향 신뢰, 이 종목 상세 적중률은 <b style="color:#7c3aed">검증</b> 버튼.</div></div>`
