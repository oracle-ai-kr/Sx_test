...
var _DISCRIM_FEATS = [
  { key:'rsi',     label:'RSI',              type:'num', get:function(ind){ return (ind.rsi && typeof ind.rsi.val==='number') ? ind.rsi.val : null; } },
  { key:'stochK',  label:'Stoch %K',         type:'num', get:function(ind){ return (ind.stoch && typeof ind.stoch.k==='number') ? ind.stoch.k : null; } },
  { key:'cci',     label:'CCI',              type:'num', get:function(ind){ return (typeof ind.cci==='number') ? ind.cci : null; } },
  { key:'bbPctB',  label:'BB %B',            type:'num', get:function(ind){ return (ind.bb && typeof ind.bb.pctB==='number') ? ind.bb.pctB : null; } },
  { key:'volOsc',  label:'거래량 OSC',        type:'num', get:function(ind){ return (typeof ind.volOsc==='number') ? ind.volOsc : null; } },
  { key:'adx',     label:'ADX',              type:'num', get:function(ind){ return (typeof ind.adx==='number') ? ind.adx : null; } },
  { key:'rsiDiv',  label:'RSI 상승다이버전스', type:'bool', get:function(ind){ return (ind.rsi && ind.rsi.div==='bullish') ? 1 : 0; } },
  { key:'obvDiv',  label:'OBV 상승다이버전스', type:'bool', get:function(ind){ return (ind.obv && ind.obv.div==='bullish') ? 1 : 0; } },
  { key:'nearSup', label:'지지선 근접',        type:'bool', get:function(ind){ return (ind.trend && ind.trend.struct && ind.trend.struct.nearSupport) ? 1 : 0; } },
  { key:'squeeze', label:'BB 스퀴즈',          type:'bool', get:function(ind){ return (ind.squeeze && ind.squeeze.squeeze) ? 1 : 0; } }
];
async function _btDiscrimBracket(mk, source, onProgress){
  ...
  var params=(typeof btGetParams==='function')?btGetParams():{};
  var _prevGate = (typeof SXE!=='undefined') ? SXE._btBearGate : undefined;
  var winF=[], lossF=[], stocksUsed=0, excluded=[];
  try {
    ...
    for(var i=0;i<use.length;i++){
      ...
      (function(rws){
        res.trades.forEach(function(t){
          ...
          var f={}; _DISCRIM_FEATS.forEach(function(D){ f[D.key]=D.get(ind); });
          if(t.pnl>0) winF.push(f); else lossF.push(f);
        });
      })(rows);
      await _trendBatchSleep(10);
    }
  ...
  if(stocksUsed<3) return { ok:false, reason:'유효 종목 3개 미만 — 네트워크 확인', excluded:excluded };
  return { ok:true, winF:winF, lossF:lossF, excluded:excluded };
}
...
function _btDiscrimRender(rep, watch, mk){
  ...
  // 대표+관심 bear-deadcat 진입 합산(표본 확보) — 진짜반등 가르는 재료 탐색
  var winF=[], lossF=[];
  if(rep&&rep.ok){ winF=winF.concat(rep.winF); lossF=lossF.concat(rep.lossF); }
  if(watch&&watch.ok){ winF=winF.concat(watch.winF); lossF=lossF.concat(watch.lossF); }
  var nW=winF.length, nL=lossF.length;
  if(nW<5 || nL<5){
    var rsn=(rep&&!rep.ok&&rep.reason)||(watch&&!watch.ok&&watch.reason)||('표본 부족(진짜반등 '+nW+'·데드캣 '+nL+')');
  ...
  // 재료별 변별력 산출
  var rows=_DISCRIM_FEATS.map(function(D){
    var wv=winF.map(function(f){return f[D.key];}), lv=lossF.map(function(f){return f[D.key];});
    if(D.type==='num'){
      var mw=_mean(wv), ml=_mean(lv);
      if(mw==null||ml==null) return null;
      var sw=_std(wv,mw), sl=_std(lv,ml), pooled=Math.sqrt((sw*sw+sl*sl)/2)||1e-9;
      var d=Math.abs(mw-ml)/pooled;   // 효과크기(Cohen's d 유사)
      return { label:D.label, type:'num', win:+mw.toFixed(1), loss:+ml.toFixed(1), diff:+(mw-ml).toFixed(1), power:d, str:(d>=0.35?'강':d>=0.2?'중':'약') };
    } else {
      var rw=100*(_mean(wv)||0), rl=100*(_mean(lv)||0);
      var rd=Math.abs(rw-rl)/100;
      return { label:D.label, type:'bool', win:+rw.toFixed(0), loss:+rl.toFixed(0), diff:+(rw-rl).toFixed(0), power:rd, str:(rd>=0.15?'강':rd>=0.08?'중':'약') };
    }
  }).filter(Boolean).sort(function(a,b){return b.power-a.power;});
  var out='<div style="border:1px solid var(--border);border-radius:10px;padding:10px;margin-top:8px">';
  out+='<div style="font-size:11px;font-weight:800;color:#0891b2;margin-bottom:4px">🧪 재료 변별력 · 진짜반등 vs 데드캣 · '+String(mk).toUpperCase()+'</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:6px">bear-deadcat 진입(역배열+단기약세)을 결과로 가름: <b style="color:'+GRN+'">진짜반등 '+nW+'건</b>(이후 수익) vs <b style="color:'+RED+'">데드캣 '+nL+'건</b>(이후 손실). 각 재료의 두 그룹 평균 차이 = 변별력. <b>강</b>=둘을 잘 가름(레시피 후보)·<b>약</b>=못 가름. 대표+관심 합산.</div>';
  out+='<div style="display:grid;grid-template-columns:1.4fr 1fr 1fr 0.7fr;gap:3px 6px;font-size:9px;align-items:center">';
  out+='<div style="font-weight:800;color:var(--text2)">재료</div><div style="font-weight:800;color:'+GRN+';text-align:right">진짜반등</div><div style="font-weight:800;color:'+RED+';text-align:right">데드캣</div><div style="font-weight:800;color:var(--text2);text-align:center">변별력</div>';
  rows.forEach(function(r){
    var sc=(r.str==='강')?GRN:(r.str==='중')?AMB:T3;
    var unit=(r.type==='bool')?'%':'';
    out+='<div style="color:var(--text)">'+r.label+'</div>';
    out+='<div style="text-align:right;font-weight:700;color:var(--text)">'+r.win+unit+'</div>';
    out+='<div style="text-align:right;font-weight:700;color:var(--text)">'+r.loss+unit+'</div>';
    out+='<div style="text-align:center;font-weight:800;color:'+sc+'">'+r.str+'</div>';
  });
  out+='</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">읽기: 진짜반등과 데드캣의 평균이 크게 다른 재료(<b style="color:'+GRN+'">강</b>)가 둘을 가르는 신호 → 레시피에 넣을 후보. 차이 없으면(<b>약</b>) 진입 시점엔 구분 불가. ※표본 적으면 노이즈 — 강만 신뢰. kNN은 STEP 2(별도 모듈).</div>';
  out+='</div>';
  return out;
}
