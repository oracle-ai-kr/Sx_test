...
async function runAnalysis(stock){
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  // [S393] 코인 시장심리(공포탐욕) 비동기 — 펀더 '시장심리' 도넛
  ...
  if(currentMarket === 'kr' && stock.code){
    ...
    if(stock.sector){
      _computeSectorScore(stock.sector).then(sec=>{
        if(sec!=null) _sxBoardSetAsync('fund','섹터',sec.v,{byValue:true});
      }).catch(()=>{});
    }
...
async function _computeSectorScore(sectorName){
  if(!sectorName) return null;
  try{
    const base = (typeof WORKER_BASE!=='undefined') ? WORKER_BASE : 'https://stock-signal-proxy.cheaheechang.workers.dev';
    const res = await fetch(`${base}/naver/sector?type=upjong`, {signal:AbortSignal.timeout(8000)});
    if(!res.ok) return null;
    const raw = await res.json();
    if(typeof SXE==='undefined' || !SXE.sectorRadar) return null;
    const radar = SXE.sectorRadar(raw, [{code:'_self', name:'_self', sector:sectorName}]);
    const m = radar.watchlistMatch && radar.watchlistMatch[0];
    if(!m) return null;
    const total = m.totalSectors || 1, rank = m.sectorRank || total;
    const v = total>1 ? Math.round((total - rank)/(total - 1)*100) : 50;
    return { v: Math.max(0,Math.min(100,v)), rank, total, change: m.sectorChange };
  }catch(e){ return null; }
}
