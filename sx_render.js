...
function _cvPoolRender(rep, watch, mk, abMode, gateMode){
  ...
  if((rep&&rep.ok&&rep.gate)||(watch&&watch.ok&&watch.gate)){
    ...
    var saSub=function(c,lbl){ return (c&&c.n)? '<br>　　└'+lbl+' n='+c.n+' 격차<b style="color:'+gCol(c.gap)+'">'+gv(c.gap,'p')+'</b>(진'+c.enN+'/회'+c.avN+') 수익<b style="color:'+(c.retO>0?GRN:c.retO<0?RED:MUT)+'">'+gv(c.retO,'%')+'</b>' : ''; };
    var simC=function(c){ return c?('격차<b style="color:'+gCol(c.gap)+'">'+gv(c.gap,'p')+'</b>·수익<b style="color:'+(c.retO>0?GRN:c.retO<0?RED:MUT)+'">'+gv(c.retO,'%')+'</b>(n'+c.n+')'):'—'; };
