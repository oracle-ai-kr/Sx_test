// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S816 · build 20260630-1430 cache-bust]
// ════════════════════════════════════════════════════════════
...
function _trendRenderInner(){
  ...
  return head+`<div id="sxTrendBody" style="display:${_trOpen?'block':'none'}" data-loaded="1">`+engRow+maRow+chipBuy+chipSell+predRow+_guardBadge+stateRow+grid+note+batchSection+`</div>`;   // [S699] 접기 래퍼 · [S752] 실험 칩을 매수/매도 섹션 body 안으로(접으면 같이 감춤) · 🔮크로스 예측만 매도 섹션 다음 별도(항상 보임) · [S816] engRow=엔진 토글
}
...
function _trendWaterAccount(bt, rows, fireMap, mode){
  ...
  for(let i=0;i<bt.trades.length;i++){
    ...
    for(let b=t.entryIdx+1; b<t.exitIdx; b++){ if(fireMap[b] && close[b]>0){ addShares+=ADD/close[b]; addCost+=ADD; wtN++; break; } }   // [S815] 포지션당 1회(첫 진짜반등만)
    injected+=addCost;
  ...
  if(bt.open){
    ...
    let addShares=0, addCost=0, wtN=0;
    for(let b=o.entryIdx+1; b<=last; b++){ if(fireMap[b] && close[b]>0){ addShares+=ADD/close[b]; addCost+=ADD; wtN++; break; } }   // [S815] 포지션당 1회
    injected+=addCost;
    const totalShares=baseShares+addShares, invested=bal+addCost;
...
function _trendDetailInner(bt,cfg,ctx){
  ...
  const wtChip=`<div style="margin:2px 0 10px">`
    +`<div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:5px"><span onclick="_sxVib(10);window._trendCycleWtMode&&_trendCycleWtMode()" title="보유 중 첫 진짜반등 1회 100만 추가입금(포지션당 1회). 탭으로 OFF→적금/DCA→물타기 동원 순환." style="font-size:10px;font-weight:800;padding:5px 11px;border-radius:14px;border:1px solid;cursor:pointer;${_trendWtMode!=='off'?'background:#0891b2;color:#fff;border-color:#0891b2':'background:var(--surface2);color:#0891b2;border-color:#0891b266'}">💧 ${_wtBtnLabel[_trendWtMode]}</span><span style="font-size:8.5px;color:var(--text3)">탭 전환 ↻</span>${_exclChk}</div>`
...
if(typeof window!=='undefined'){ window._trendOpenDetail=_trendOpenDetail; window._trendDetailClose=_trendDetailClose; window._trendCycleWtMode=_trendCycleWtMode; window._trendToggleWtExclBear=_trendToggleWtExclBear; window._trendToggleEngine=_trendToggleEngine; }
