...
function _sxbHTML(){
  ...
  groups.forEach(g=>g.items.forEach(it=>{
    // ▲▼ 분포 카운트 (종합점수 기여와 별개 — 값/델타 기준 방향성)
  ...
  // [S378] 종합점수 = 카테고리 가중평균 (시장별 가중치)
  //   · _sxbAvg = 카테고리 내 점수 평균(inverse 역산 반영, [S367])
  //   · 카테고리 단위 가중 → 항목 수 불균형(추세5 vs 펀더1~4) 해소
  //   · 핵심(추세·수급)에 큰 가중 / 코인은 펀더(밸류 1개뿐) 거의 무시
  const _SXB_MKT_W = {
    kr:   { trend:0.25, mom:0.20, flow:0.25, fund:0.15, tech:0.15 },
    us:   { trend:0.25, mom:0.20, flow:0.20, fund:0.20, tech:0.15 },
    coin: { trend:0.30, mom:0.25, flow:0.20, fund:0.05, tech:0.20 },
  };
  const _mkt = (typeof currentMarket !== 'undefined' && _SXB_MKT_W[currentMarket]) ? currentMarket : 'kr';
  const _W = _SXB_MKT_W[_mkt];
  ...
  let total = _wDen > 0 ? Math.round(_wNum / _wDen) : 0;
  // [S381] ▲▼ 카운터 배지 보정 — 지표 분포 우열(net=▲−▼)을 종합점수에 반영해 변별력 확보.
  //   코인은 모멘텀 시장 특성상 분포 신뢰도가 높아 계수를 키움.
  const _netW = ({ kr:1.5, us:1.5, coin:1.8 })[_mkt] || 1.5;
  total = Math.max(0, Math.min(100, total + Math.round((greens - reds) * _netW)));
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    ...
    + `<div class="sxb-head-sub">종합 ${total} · ${_sxbGrade(total)}등급 · ${_cnt}개 지표 · ▲${greens} ▼${reds}</div>`
    + `</div>`
