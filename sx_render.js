...
function _buildTransitionCard(stock, indicators){
  ...
  let _poolApplied = false, _poolDelta = null;
  if (_useKnn && window.SXCandleBT && SXCandleBT.poolAutoOn && SXCandleBT.poolAutoOn()) {
    ...
    if (Object.prototype.hasOwnProperty.call(_ctPoolAutoCache, _pak)) {
      ...
      if (_pv && _pv.applies && _pv.poolCur && _pv.poolCur.active) { _kn = _pv.poolCur; _poolApplied = true; _poolDelta = _pv.delta; }
    } else {
      ...
      }, 50); })(rows, _pak, _mktCT, _tfCT, (stock&&(stock.code||stock.name))||'', (stock&&(stock.name||stock.code))||'');
    }
  }
