  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        return `${_verdictBadgeTop ? `<div style="display:flex;justify-content:flex-start;margin-top:8px;margin-bottom:-2px">${_verdictBadgeTop}</div>` : ''}<div style="display:flex;align-items:center;gap:6px;margin-top:6px">
          ...
          <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1.5px solid ${_entryEvalClr};cursor:pointer" onclick="_sxVib(10);var el=document.getElementById('${_stratDetailId}'),t=document.getElementById('${_3stageId}');if(t&&t.style.display!=='none'){t.style.display='none';if(currentAnalStock)currentAnalStock._3stageOpen=false;}var willOpen=el.style.display==='none';el.style.display=willOpen?'block':'none';if(currentAnalStock){currentAnalStock._engineVerifyOpen=willOpen;}if(willOpen)_runEngineVerify(currentAnalStock);">
            ...
            <span style="font-size:11px;font-weight:800;color:${_entryEvalClr};line-height:1.2">${stock._btResult && stock._btResult.totalTrades ? `거래 ${stock._btResult.totalTrades} 승률 ${(stock._btResult.winRate||0).toFixed(0)}%` : '백테스트 실행'}</span>
          </div>
        </div>
      ...
      <div style="margin-top:10px;padding:14px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid var(--text3);text-align:center">
        ...
        // BT 결과 6칸: 승률/손익비/수익률 / 거래수/MDD/평균이익
        ...
        // 평균이익 (거래당 평균 수익률, %)
        const _avgProfitStr = _tr > 0 ? ((_pnl / _tr).toFixed(1) + '%') : '—';
        // 색상 분기
        const _wrClr = _wr >= 60 ? '#22c55e' : _wr >= 50 ? '#3b82f6' : _wr >= 40 ? '#eab308' : '#e8365a';
        const _pnlClr = _pnl >= 20 ? '#22c55e' : _pnl >= 0 ? '#3b82f6' : '#e8365a';
        const _rrClr = _rr >= 1.5 ? '#22c55e' : _rr >= 1 ? '#3b82f6' : '#eab308';
        const _mddClr = _mdd <= 10 ? '#22c55e' : _mdd <= 20 ? '#eab308' : '#e8365a';
        const _trClr = _tr >= 30 ? '#22c55e' : _tr >= 10 ? '#3b82f6' : '#e8365a';
        // 봉수 표시 (투명성)
      ...
      <div style="margin-top:10px;padding:10px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid #3b82f6">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px">
          ...
          <div style="text-align:center;padding:6px 2px;background:var(--surface);border-radius:6px">
            <div style="font-size:14px;font-weight:800;color:var(--text);line-height:1.2">${_avgProfitStr}</div>
            <div style="font-size:9px;color:var(--text3);margin-top:1px">평균이익</div>
          </div>
        </div>
      </div>`;
      })()}
