  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        //     > +30%: 고평가 (빨강)
        if(!stock.price || stock.price <= 0) return '';
        if(currentMarket === 'coin') return '';
        const _fin = stock._financial;
        if(!_fin || _fin.eps == null || _fin.eps <= 0) return '';
        if(typeof SXI === 'undefined' || typeof SXI.valuationJudge !== 'function') return '';
        const _v = SXI.valuationJudge(_fin, stock.price, stock);
        if(!_v || _v.fairPrice == null || _v.diffPct == null) return '';
        const _fpStr = _isUsStock(stock)
          ? `$${_v.fairPrice.toFixed(2)}`
          : `${Math.round(_v.fairPrice).toLocaleString()}원`;
        const _sign = _v.diffPct >= 0 ? '+' : '';
        return `<div class="anal-fair-price" style="margin-top:4px;letter-spacing:-.2px;line-height:1.3">`
    ...
    ${(()=>{
      ...
      return `<div class="anal-section">
        ...
        ${(()=>{
          ...
          if(typeof SXI === 'undefined' || typeof SXI.valuationJudge !== 'function') return '';
          const _v = SXI.valuationJudge(fin, stock.price, stock);
          if(!_v) return '';
        ...
        ${fin.per!=null?`<div class="anal-row"><span class="al">PER${fin._epsSource==='dart'?' <span style="font-size:8px;color:var(--text3);font-weight:normal">DART 기반</span>':fin._epsSource==='sec'?' <span style="font-size:8px;color:var(--text3);font-weight:normal">SEC 기반</span>':''}</span><span class="ar ${fin.per>0&&fin.per<=15?'bullish':fin.per>50||fin.per<0?'bearish':'neutral'}">${fin.per.toFixed(1)}배</span></div>`:''}
        ${(()=>{
        ...
        ${fin.eps!=null?`<div class="anal-row"><span class="al">EPS${fin._epsSource==='dart'?' <span style="font-size:8px;color:var(--text3);font-weight:normal">DART 기반</span>':fin._epsSource==='sec'?' <span style="font-size:8px;color:var(--text3);font-weight:normal">SEC 기반</span>':''}</span><span class="ar">${_isUsStock(stock) ? `$${fin.eps.toFixed(2)}` : `${fin.eps.toLocaleString()}원`}</span></div>`:''}
