// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.10  [S812 · build 20260630-0039 cache-bust]
// ════════════════════════════════════════════════════════════
...
async function _recipeOverlapBracket(mk, source, onProgress){
  ...
  var CAP=20, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
  var agg={ bear:{}, bull:{} }, stocksUsed=0, excluded=[];
  for(var i=0;i<use.length;i++){
    var s=use[i];
    if(onProgress) onProgress(i+1, use.length, s.name);
    await _trendBatchSleep(0);
    ...
    for(var j=0;j<ov.length;j++){
      var o=ov[j];
      if(!o.complete) continue;                          // N10 확정분만
      var pool=(o.lt==='bear')?'bear':(o.lt==='bull'?'bull':null); if(!pool) continue;
      var kb=Math.min(o.k,4);                            // 4개 이상 합침(4+)
      var b=agg[pool][kb]||(agg[pool][kb]={n:0,h:0});
      b.n++; if(o.hit) b.h++; _used=true;
    }
    if(_used) stocksUsed++;
    await _trendBatchSleep(10);
  }
...
function _recipeOverlapRender(rep, watch, mk){
  var GRN='#16a34a', RED='#dc2626', AMB='#d97706', T2='var(--text2)', T3='var(--text3)';
  var _poolTable=function(agg, poolKey, poolLabel, poolCol){
    ...
    if(!any) return '<div style="font-size:9px;color:'+T3+';margin:3px 0 8px">'+poolLabel+': 발동·확정 표본 없음</div>';
    var verdict = (totN<20) ? '<span style="color:'+T3+'">표본 부족 — 판단 보류</span>'
      : mono ? '<span style="color:'+GRN+'">겹침↑ → 진짜비율↑ (단조) · 겹침=강 신호</span>'
      : '<span style="color:'+AMB+'">단조 아님 — 겹쳐도 진짜비율 안 오름</span>';
    return '<div style="margin:2px 0 10px">'
      +'<div style="font-size:10px;font-weight:800;color:'+poolCol+';margin-bottom:3px">'+poolLabel+'</div>'
      ...
      +'<div style="font-size:9px;margin-top:4px;line-height:1.4">'+verdict+'</div>'
      +'</div>';
  };
  var _block=function(res, label){
    ...
    var dead=_poolTable(res.agg,'bear','🔴 역배열 (deadcat-real)','#dc2626');
    var pull=_poolTable(res.agg,'bull','🔵 정배열 (pullback-real)','#2563eb');
    return '<div style="margin-bottom:10px"><div style="font-size:10.5px;font-weight:800;color:'+T2+';margin-bottom:5px;border-bottom:1px solid var(--border);padding-bottom:3px">'+label+' <span style="font-weight:500;color:'+T3+';font-size:8.5px">'+(res.stocksUsed||0)+'종목</span></div>'+dead+pull+'</div>';
  };
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:8px">real 레시피 <b>동시발동 수</b>별 N10 진짜비율(후반평균 [+6..+10] 상승). 겹침↑면 비율↑(단조)= 겹침이 데드캣 변별 "강". <b style="color:'+AMB+'">⚠ n&lt;15=표본 작음(과적합 주의)</b> · BT무관 순수데이터. ※deadcat-real 13개 중 12개가 거래량OSC&gt;22.87 공통=비슷한 레시피라 단순 겹침은 \'독립확인\'보다 \'과대낙폭 깊이\'에 가까울 수 있음.</div>';
