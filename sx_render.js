  ...
  try{
    ...
    if(btCandles && btCandles.length >= 60 && typeof SXE!=='undefined' && SXE.runBtEngine){
      ...
      if(btR && !btR.error){
        ...
        //   무한 루프 방지: stock._analCandlesExtended 플래그
        ...
        console.log(`[S107-9a DBG] 확장 감지 — trades=${_btTradesNow}, min=${BT_MIN_TRADES}, extended=${!!stock._analCandlesExtended}, market=${_extMkt}, supported=${_extSupported}, fetchFn=${typeof fetchCandlesExtended}, rowsLen=${rawRows.length}`);
        if(_btTradesNow < BT_MIN_TRADES && !stock._analCandlesExtended && _extSupported
           && typeof fetchCandlesExtended === 'function' && rawRows.length > 0){
          console.log(`[S107-9a] ★ 확장 시도 시작: BT 거래 ${_btTradesNow}회 < ${BT_MIN_TRADES}, 시장=${_extMkt}`);
          stock._analCandlesExtended = true; // 무한 루프 방지
          ...
          console.log(`[S107-9a DBG] 확장 건너뜀 — 조건 미충족`);
        }
      }
    }
  ...
  const helpHTML = `
    <div class="bt-banner-help" id="${helpId}">
      <div class="bt-help-section">
        ...
        <b>진입 검토</b> — 최신 봉 기준 순수 기술적 상태 평가 (0~100)<br>
        <b>매매전략</b> — 과거 백테스트 성과 종합 (0~100)<br>
        시점이 다릅니다. 진입 검토는 "지금 이 순간의 기술적 조건", 매매전략은 "이 전략의 과거 성적표"입니다.<br>
        재무·매크로·공시 등 비기술 요소는 별도 해석 카드에서 참고하세요.
      </div>
      <div class="bt-help-section">
        <div class="bt-help-title">왜 둘 다 필요한가</div>
        진입 검토 80점 = "기술적으로 반등 조건이 갖춰졌다"<br>
        매매전략 30점 = "하지만 같은 전략으로 반복 매매하면 손실이 컸다"<br><br>
        비유하면 진입 검토는 "오늘 날씨가 맑다", 매매전략은 "이 지역은 1년 중 비가 200일 온다" 입니다. 오늘 맑다고 해서 우산이 필요 없는 건 아닙니다.
      </div>
      <div class="bt-help-section">
        <div class="bt-help-title">교차 판단 기준</div>
        <table class="bt-guide-table">
          <tr><th>진입 검토</th><th>매매전략</th><th>행동지침</th></tr>
      ...
      <div class="bt-help-section" style="margin-top:10px">
        <b>매매전략</b>이 기준선이고, <b>진입 검토</b>가 실행 신호입니다.<br>매매전략이 높은 종목에서 진입 검토 점수가 올라올 때가 가장 좋은 진입 시점입니다.
      </div>
    </div>`;
