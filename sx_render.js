...
function _mxRunTournament(rows, cfg, dir, N){
  ...
  var St = { slope:{n:0,hit:0,c:{}}, knn:{n:0,hit:0,c:{}} }, allCross = {}, eligHit = 0, knnFail = 0;
  ...
  return { slope:mk(St.slope), knn:mk(St.knn), baseRate:baseRate, totalCross:totalCross, nTests:nT, eligTotal:elig.length, knnFail:knnFail };
}
...
function _mxRenderTournament(R, dir, N){
  ...
  var h = '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;margin-bottom:4px"><span style="font-size:15px;font-weight:800;color:#0f172a">⚔️ kNN vs 기울기</span><span style="font-size:10px;color:'+T3+'">'+dk+' · N='+N+' · '+R.nTests+'시점</span></div>';
  ...
  h += rowH('단기선 기울기','📈',R.slope,'#16a34a');
  h += rowH('kNN 차트예측','🔮',R.knn,'#7c3aed');
  var verdict='';
  if(R.slope.prec!=null && R.knn.prec!=null && R.knn.n>=5){
    var ds=(R.slope.prec-R.knn.prec)*100, dr=(R.slope.rec-R.knn.rec)*100;
    var pWin = Math.abs(ds)<3 ? '정밀 호각' : (ds>0?'정밀 <b>기울기</b> 우세':'정밀 <b>kNN</b> 우세');
    var rWin = Math.abs(dr)<3 ? '재현 호각' : (dr>0?'재현 <b>기울기</b> 우세':'재현 <b>kNN</b> 우세');
    verdict = '<div style="font-size:11px;color:#334155;line-height:1.5;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:8px 10px;margin-top:2px">🏁 '+pWin+' ('+(ds>=0?'+':'')+ds.toFixed(0)+'pp) · '+rWin+' ('+(dr>=0?'+':'')+dr.toFixed(0)+'pp)</div>';
  } else if(R.knn.n<5){
    verdict = '<div style="font-size:11px;color:#64748b;line-height:1.5;margin-top:2px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:8px 10px">kNN이 이 표본서 크로스를 거의 예측 안 함(신호 '+R.knn.n+'건). 패턴예측은 크로스 <i>기하</i>보다 방향 예측에 가까움 — 명시조건(기울기)의 보완재이지 대체재가 아님을 시사.</div>';
  }
  h += verdict;
  h += '<div style="font-size:9px;color:'+T3+';margin-top:9px;line-height:1.5">동일 표본·워크포워드(각 시점 rows[≤t]). 기울기=단기MA 상향. kNN='+(dir==='gc'?'medPath 중앙경로 투영 시 5MA가 9MA 추월':'medPath 투영 시 5MA가 9MA 하향돌파')+'. kNN 지평(medPath 길이)이 N보다 짧으면 그만큼만 평가.</div>';
  return h + closeBtn;
}
