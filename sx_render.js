...
var SX_BADGE_DEFS = {
  rebound_pool: {
    id:'rebound_pool', poolLt:'bear', poolName:'역배열+단기약세', date:'2026-06-26',
    signal:[{feat:'volOsc', dir:'lt', th:-39.6, label:'거래량OSC < −39.6'}],
    support:[],
    yes:{ icon:'🟢', name:'바닥반등', color:'#16a34a', verified:true,  n10:78, n5:74, base:50, sample:'23' },
    no: { icon:'🔴', name:'데드캣',   color:'#dc2626', verified:false },
    note:'거래량OSC 깊으면(투매·워시아웃) 진짜 반등 78%. 미충족쪽(데드캣)은 미검증·측정대상.'
  },
  pullback_pool: {
...
// ════════ [S728] 🐻 BT 약세게이트 풀링 — 대표/관심 풀 각 종목 fetchRows600 봉으로 sxRunBtEngine을 게이트 OFF·ON 2회 → 집계 비교. 캐시 무관(C판단검증과 동일 풀 조달). 교차검증 봉수 부족 우회. ════════
async function _btPoolBracket(mk, source, onProgress){
  if(!(window.SXCandleBT&&SXCandleBT.fetchRows600)) return { ok:false, reason:'캔들 fetch 미연결' };
  if(!(typeof SXE!=='undefined'&&SXE.runBtEngine)) return { ok:false, reason:'BT 엔진 미로드' };
  var list;
  ...
  var _prevGate = (typeof SXE!=='undefined') ? SXE._btBearGate : undefined;   // ★측정 후 원복용
  var off=[], on=[], excluded=[];
  try {
    for(var i=0;i<use.length;i++){
      var s=use[i];
      ...
      if(!Array.isArray(rows)||rows.length<_floor){ excluded.push({name:s.name, reason:(!Array.isArray(rows)||!rows.length)?'페치 실패':('이력 '+(rows?rows.length:0)+'봉')}); await _trendBatchSleep(6); continue; }
      var rOff=null, rOn=null;
      try{ SXE._btBearGate=false; rOff=SXE.runBtEngine(rows,'day',params,{applyRegimeAdjust:true}); }catch(e2){}
      try{ SXE._btBearGate=true;  rOn =SXE.runBtEngine(rows,'day',params,{applyRegimeAdjust:true}); }catch(e3){}
      if(rOff&&!rOff.error&&rOff.totalTrades>0) off.push(rOff);
      if(rOn&&!rOn.error&&rOn.totalTrades>0) on.push(rOn);
      await _trendBatchSleep(10);
    }
  } finally { if(typeof SXE!=='undefined') SXE._btBearGate = _prevGate; }   // ★게이트 원복(측정이 사용자 설정 안 바꾸게)
  if(off.length<3 && on.length<3) return { ok:false, reason:'유효 종목 3개 미만 — 네트워크 확인', excluded:excluded };
  var _agg=function(arr){ if(!arr.length) return null; var n=arr.length; return {
    n:n,
    wr:+(arr.reduce(function(a,r){return a+(r.winRate||0);},0)/n).toFixed(1),
    pnl:+(arr.reduce(function(a,r){return a+(r.totalPnl||0);},0)/n).toFixed(2),
    mdd:+(arr.reduce(function(a,r){return a+(r.mdd||0);},0)/n).toFixed(2),
    pf:+(arr.reduce(function(a,r){return a+(r.profitFactor||0);},0)/n).toFixed(2),
    tr:arr.reduce(function(a,r){return a+(r.totalTrades||0);},0)
  }; };
  return { ok:true, off:_agg(off), on:_agg(on), excluded:excluded };
}
async function _btPoolRun(){
  var el=document.getElementById('btPoolResult'); if(!el) return;
  var mk=(typeof currentMarket!=='undefined')?currentMarket:'kr';
  el.style.display='block';
  var _prog=function(stage,i,t,n){ el.innerHTML='<div style="text-align:center;padding:14px;color:#0d9488;font-size:12px;font-weight:800">🐻 '+stage+' BT 풀링 '+i+'/'+t+'<div style="font-size:10px;color:#94a3b8;font-weight:500;margin-top:4px">'+(n||'')+' · 종목당 OFF/ON 2회</div></div>'; };
  el.innerHTML='<div style="text-align:center;padding:14px;color:#0d9488;font-size:12px;font-weight:800">🐻 BT 풀링 시작…</div>';
  await _trendBatchSleep(30);
  var rep=null, watch=null;
  try{ rep=await _btPoolBracket(mk,'rep',function(i,t,n){_prog('대표',i,t,n);}); }catch(e){ rep={ok:false,reason:String(e&&e.message||e)}; }
  await _trendBatchSleep(20);
  try{ watch=await _btPoolBracket(mk,'watch',function(i,t,n){_prog('관심',i,t,n);}); }catch(e2){ watch={ok:false,reason:String(e2&&e2.message||e2)}; }
  el.innerHTML=_btPoolRender(rep, watch, mk);
}
function _btPoolRender(rep, watch, mk){
  var GRN='#16a34a',RED='#dc2626',MUT='#64748b',T3='var(--text3)';
  var _row=function(lbl, off, on){
    if(!off||!on) return '<div style="font-size:10px;color:'+T3+';padding:4px 0">'+lbl+': 데이터 부족(OFF '+(off?off.n:0)+'·ON '+(on?on.n:0)+')</div>';
    var _d=function(o,n,goodDown){ var diff=+(n-o).toFixed(2); var good=goodDown?diff<0:diff>0; var c=(diff===0)?MUT:(good?GRN:RED); return '<span style="color:'+c+';font-weight:700">'+(diff>0?'+':'')+diff+'</span>'; };
    var h='<div style="margin:6px 0;padding:7px 9px;background:var(--surface2);border-radius:7px">';
    h+='<div style="font-size:11px;font-weight:800;color:var(--text);margin-bottom:5px">'+lbl+' <span style="font-size:9px;color:'+T3+';font-weight:500">OFF n'+off.n+' → ON n'+on.n+'</span></div>';
    h+='<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:4px;font-size:9px;text-align:center">';
    h+='<div><div style="color:'+T3+'">수익률</div><div style="font-weight:700">'+off.pnl+'→'+on.pnl+'</div>'+_d(off.pnl,on.pnl,false)+'</div>';
    h+='<div><div style="color:'+T3+'">MDD</div><div style="font-weight:700">'+off.mdd+'→'+on.mdd+'</div>'+_d(off.mdd,on.mdd,true)+'</div>';
    h+='<div><div style="color:'+T3+'">PF</div><div style="font-weight:700">'+off.pf+'→'+on.pf+'</div>'+_d(off.pf,on.pf,false)+'</div>';
    h+='<div><div style="color:'+T3+'">승률</div><div style="font-weight:700">'+off.wr+'→'+on.wr+'</div>'+_d(off.wr,on.wr,false)+'</div>';
    h+='<div><div style="color:'+T3+'">거래</div><div style="font-weight:700">'+off.tr+'→'+on.tr+'</div>'+_d(off.tr,on.tr,true)+'</div>';
    h+='</div></div>';
    return h;
  };
  var out='<div style="border:1px solid var(--border);border-radius:10px;padding:10px;margin-top:8px">';
  out+='<div style="font-size:11px;font-weight:800;color:#0d9488;margin-bottom:4px">🐻 BT 약세게이트 OFF→ON 풀링 · '+String(mk).toUpperCase()+'</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:6px">대표/관심 풀 각 종목 fetchRows600 봉으로 BT를 게이트 OFF·ON 2회 → 집계. <b style="color:#16a34a">녹색</b>=게이트가 개선(MDD↓·거래↓·수익↑·PF↑), <b style="color:#dc2626">빨강</b>=악화. 캐시 무관·C판단검증과 동일 풀 조달. 측정 후 게이트는 사용자 설정대로 원복.</div>';
  out+= (rep&&rep.ok)? _row('🔵 대표(되돌릴 우량주)', rep.off, rep.on) : '<div style="font-size:10px;color:'+RED+';padding:4px 0">대표: '+((rep&&rep.reason)||'실패')+'</div>';
  out+= (watch&&watch.ok)? _row('🐻 관심(순수 약세)', watch.off, watch.on) : '<div style="font-size:10px;color:'+RED+';padding:4px 0">관심: '+((watch&&watch.reason)||'실패')+'</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:6px;line-height:1.4">판단: <b>대표 MDD↓·수익 유지/↑·PF↑</b>면 커밋 정당. 거래수 감소=제거된 데드캣 진입. 관심은 최악풀이라 참고용.</div>';
  out+='</div>';
  return out;
}
if(typeof window!=='undefined'){ window._btPoolRun=_btPoolRun; }
// ════════ [S733] 🔬 막힌 거래 분석 — 게이트 OFF 거래목록을 진입봉 레짐별로 분류해 'bear-deadcat(역배열+단기약세)' 진입의 실제 손익 분포 + 종목별 분해. 게이트가 막은 게 이득(손실거래)인지 손해(수익거래=진짜반등)인지 직접 판별. ════════
...
function _classifyEntry733(rows, idx, tf){   // 진입봉 idx의 ltAlign·maAlignBull 재계산 → 게이트와 동일 분류. 250봉(maAlignLT 200 충족)
  try {
    if(!(typeof SXE!=='undefined' && SXE.calcAllScreener)) return null;
    var slice = rows.slice(Math.max(0, idx-249), idx+1);
    var ind = SXE.calcAllScreener(slice, tf);
    if(!ind) return null;
    var ltAlign = _ltStr733(ind.maAlignLT);
    var maBull = !!(ind.maAlign && ind.maAlign.bullish);
    return { ltAlign:ltAlign, maBull:maBull, bearDeadcat:(ltAlign==='bear' && !maBull) };
  } catch(_){ return null; }
}
async function _btBlockedBracket(mk, source, onProgress){
  if(!(window.SXCandleBT&&SXCandleBT.fetchRows600)) return { ok:false, reason:'캔들 fetch 미연결' };
  if(!(typeof SXE!=='undefined'&&SXE.runBtEngine&&SXE.calcAllScreener)) return { ok:false, reason:'BT 엔진 미로드' };
  var list;
  if(source==='watch'){ try{ list=(typeof _getWatchlist==='function'?(_getWatchlist(mk)||[]):[]).filter(function(s){return s&&s.code;}).map(function(s){return {code:s.code,name:s.name||s.code};}); }catch(_){ list=[]; } }
  else { try{ var p=(window.SXCandleBT&&SXCandleBT.getRepPool)?SXCandleBT.getRepPool(mk):[]; list=(p||[]).map(function(x){return {code:x[0],name:x[1]};}); }catch(_){ list=[]; } }
  ...
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
  var params=(typeof btGetParams==='function')?btGetParams():{};
  var _prevGate = (typeof SXE!=='undefined') ? SXE._btBearGate : undefined;
  var gated=[], kept=[], perStock={}, excluded=[], stocksUsed=0;
  try {
    if(typeof SXE!=='undefined') SXE._btBearGate=false;   // ★OFF 거래목록 기준(막힐 진입이 실제로 어떤 손익이었나)
    for(var i=0;i<use.length;i++){
      var s=use[i];
      if(onProgress) onProgress(i+1, use.length, s.name);
      await _trendBatchSleep(0);
      ...
      var res=null;
      try{ res=SXE.runBtEngine(rows,'day',params,{applyRegimeAdjust:true}); }catch(e2){}
      if(!res||!Array.isArray(res.trades)){ excluded.push({name:s.name, reason:'BT 실패'}); await _trendBatchSleep(8); continue; }
      stocksUsed++;
      (function(rws, nm){
        res.trades.forEach(function(t){
          if(t.type!=='WIN' && t.type!=='LOSS') return;   // 청산 거래만(실현손익)
          var cls=_classifyEntry733(rws, t.entryIdx, 'day');
          if(!cls) return;
          if(cls.bearDeadcat){
            gated.push({pnl:t.pnl});
            if(!perStock[nm]) perStock[nm]={n:0,sum:0,win:0};
            perStock[nm].n++; perStock[nm].sum+=t.pnl; if(t.pnl>0) perStock[nm].win++;
          } else { kept.push({pnl:t.pnl}); }
        });
      })(rows, s.name);
      await _trendBatchSleep(10);
    }
  } finally { if(typeof SXE!=='undefined') SXE._btBearGate=_prevGate; }   // ★게이트 원복
  if(stocksUsed<3) return { ok:false, reason:'유효 종목 3개 미만 — 네트워크 확인', excluded:excluded };
  var _agg=function(arr){ if(!arr.length) return {n:0,avg:0,wr:0}; var n=arr.length, sum=arr.reduce(function(a,r){return a+r.pnl;},0), win=arr.filter(function(r){return r.pnl>0;}).length; return {n:n, avg:+(sum/n).toFixed(2), wr:+(100*win/n).toFixed(1)}; };
  var ps=Object.keys(perStock).map(function(k){ var v=perStock[k]; return {name:k, n:v.n, avg:+(v.sum/v.n).toFixed(2), wr:+(100*v.win/v.n).toFixed(1)}; }).sort(function(a,b){return b.n-a.n;});
  return { ok:true, gated:_agg(gated), kept:_agg(kept), perStock:ps, excluded:excluded };
}
async function _btBlockedRun(){
  var el=document.getElementById('btBlockedResult'); if(!el) return;
  var mk=(typeof currentMarket!=='undefined')?currentMarket:'kr';
  el.style.display='block';
  var _prog=function(stage,i,t,n){ el.innerHTML='<div style="text-align:center;padding:14px;color:#a855f7;font-size:12px;font-weight:800">🔬 '+stage+' 막힌거래 분석 '+i+'/'+t+'<div style="font-size:10px;color:#94a3b8;font-weight:500;margin-top:4px">'+(n||'')+'</div></div>'; };
  el.innerHTML='<div style="text-align:center;padding:14px;color:#a855f7;font-size:12px;font-weight:800">🔬 분석 시작…</div>';
  await _trendBatchSleep(30);
  var rep=null, watch=null;
  try{ rep=await _btBlockedBracket(mk,'rep',function(i,t,n){_prog('대표',i,t,n);}); }catch(e){ rep={ok:false,reason:String(e&&e.message||e)}; }
  await _trendBatchSleep(20);
  try{ watch=await _btBlockedBracket(mk,'watch',function(i,t,n){_prog('관심',i,t,n);}); }catch(e2){ watch={ok:false,reason:String(e2&&e2.message||e2)}; }
  el.innerHTML=_btBlockedRender(rep, watch, mk);
}
function _btBlockedRender(rep, watch, mk){
  var GRN='#16a34a',RED='#dc2626',T3='var(--text3)';
  var _block=function(lbl, R){
    if(!R||!R.ok) return '<div style="font-size:10px;color:'+RED+';padding:4px 0">'+lbl+': '+((R&&R.reason)||'실패')+'</div>';
    var g=R.gated, k=R.kept;
    // 판정: 막힌(gated) 진입 평균손익 — <0이면 막은 게 이득(손실거래 차단), >0이면 손해(수익거래=진짜반등 차단)
    var verdict, vCol;
    if(g.n===0){ verdict='막힌 진입 없음'; vCol=T3; }
    else if(g.avg < 0){ verdict='✅ 막은 게 이득 — 평균 '+g.avg+'% 손실 거래 차단'; vCol=GRN; }
    else { verdict='⚠️ 막은 게 손해 — 평균 +'+g.avg+'% 수익 거래(진짜 반등) 차단'; vCol=RED; }
    var h='<div style="margin:7px 0;padding:8px 10px;background:var(--surface2);border-radius:8px">';
    h+='<div style="font-size:11px;font-weight:800;color:var(--text);margin-bottom:6px">'+lbl+'</div>';
    h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:6px">';
    h+='<div style="padding:5px 7px;background:var(--surface);border-radius:6px"><div style="font-size:8.5px;color:'+T3+'">🐻 막힌 진입(역배열+단기약세)</div><div style="font-size:10px;font-weight:700;color:'+(g.avg<0?GRN:RED)+'">'+g.n+'건 · 평균 '+(g.avg>0?'+':'')+g.avg+'% · 승률 '+g.wr+'%</div></div>';
    h+='<div style="padding:5px 7px;background:var(--surface);border-radius:6px"><div style="font-size:8.5px;color:'+T3+'">통과 진입(나머지)</div><div style="font-size:10px;font-weight:700;color:var(--text)">'+k.n+'건 · 평균 '+(k.avg>0?'+':'')+k.avg+'% · 승률 '+k.wr+'%</div></div>';
    h+='</div>';
    h+='<div style="font-size:9.5px;font-weight:700;color:'+vCol+';margin-bottom:6px">'+verdict+'</div>';
    // 종목별 분해(막힌 진입 기여 상위) — 하락주 집중 + 우량주 반등 진짜 여부
    if(R.perStock && R.perStock.length){
      h+='<div style="font-size:8.5px;color:'+T3+';margin-bottom:3px">종목별 막힌 진입(상위) — 평균손익 <b style="color:'+RED+'">빨강=수익(막으면 손해)</b>·<b style="color:'+GRN+'">녹색=손실(막으면 이득)</b></div>';
      h+='<div style="display:flex;flex-direction:column;gap:2px">';
      R.perStock.slice(0,8).forEach(function(p){
        var c=p.avg>0?RED:GRN;
        h+='<div style="display:flex;justify-content:space-between;font-size:9px"><span style="color:var(--text2)">'+p.name+'</span><span style="color:'+c+';font-weight:700">'+p.n+'건 · '+(p.avg>0?'+':'')+p.avg+'% · '+p.wr+'%</span></div>';
      });
      h+='</div>';
    }
    h+='</div>';
  ...
  var out='<div style="border:1px solid var(--border);border-radius:10px;padding:10px;margin-top:8px">';
  out+='<div style="font-size:11px;font-weight:800;color:#a855f7;margin-bottom:4px">🔬 막힌 거래 분석 · '+String(mk).toUpperCase()+'</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:6px">게이트 OFF 거래목록을 진입봉 레짐별로 분류. <b>막힌 진입</b>=역배열+단기약세(게이트 타겟). 그 평균손익이 <b style="color:'+GRN+'">음수면 막은 게 이득</b>(데드캣), <b style="color:'+RED+'">양수면 손해</b>(진짜 반등을 막음). 종목별로 보면 하락 우량주(카카오·NAVER 등) 반등이 진짜였는지 드러남.</div>';
  out+=_block('🔵 대표(되돌릴 우량주)', rep);
  out+=_block('🐻 관심(순수 약세)', watch);
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:6px;line-height:1.4">해석: 관심 막힌진입 <b>음수</b>(데드캣) + 대표 막힌진입 <b>양수</b>(진짜반등)면 → <b>종목 품질별로 게이트 효과 반대</b> = 단순 ltAlign 게이트론 부족, 우량도 차원 필요. 둘 다 음수면 → 게이트 커밋 정당.</div>';
  out+='</div>';
  return out;
}
if(typeof window!=='undefined'){ window._btBlockedRun=_btBlockedRun; }
...
function _sxSetDiscrimPool(mode){
  ...
  ['btDiscrimResult','btThreshResult','btEnsembleResult','btBuilderResult','btPoolResult','btBlockedResult'].forEach(function(id){ var e=document.getElementById(id); if(e){ e.innerHTML=''; e.style.display='none'; } });
...
async function _btDiscrimBracket(mk, source, onProgress){
  ...
  var _poolLt=_dpMeta().lt;   // [S753] 'bear'(데드캣=역배열) | 'bull'(눌림목=정배열)
  try {
    if(typeof SXE!=='undefined') SXE._btBearGate=false;   // OFF 거래목록(막힐 진입 포함)
    for(var i=0;i<use.length;i++){
      var s=use[i];
      if(onProgress) onProgress(i+1, use.length, s.name);
      await _trendBatchSleep(0);
      ...
      (function(rws){
        res.trades.forEach(function(t){
          if(t.type!=='WIN' && t.type!=='LOSS') return;
          var slice, ind;
          try { slice=rws.slice(Math.max(0, t.entryIdx-249), t.entryIdx+1); ind=SXE.calcAllScreener(slice,'day'); } catch(_){ return; }
          if(!ind) return;
          var ltAlign=_ltStr733(ind.maAlignLT), maBull=!!(ind.maAlign && ind.maAlign.bullish);
          if(!(ltAlign===_poolLt && !maBull)) return;   // [S753] ★선택 풀만 — 데드캣=bear(역배열)·눌림목=bull(정배열), 공통 단기약세(!maBull)
          var f=_extractFeats733(ind, rws, t.entryIdx);
          var ep=t.entry, ei=t.entryIdx;
          var _nRet=function(N){ var j=ei+N; if(j>=rws.length || !rws[j] || typeof rws[j].close!=='number' || !ep) return null; return rws[j].close > ep; };   // 진입 후 N봉 종가 > 진입가
          var _n3=_nRet(3), _n5=_nRet(5), _n10=_nRet(10);
          var _hold=(_n5===null||_n10===null)?null:(_n5===true&&_n10===true);   // [S761] 유지=N5·N10 둘다 위(데드캣=N5 반짝 후 N10 추락은 false). 진짜반등의 '추이' 정의 — 단일시점 N10의 한계 보완.
          entries.push({ f:f, pnl:(t.pnl>0), n3:_n3, n5:_n5, n10:_n10, hold:_hold });
        });
      })(rows);
      await _trendBatchSleep(10);
    }
  } finally { if(typeof SXE!=='undefined') SXE._btBearGate=_prevGate; }
  if(stocksUsed<3) return { ok:false, reason:'유효 종목 3개 미만 — 네트워크 확인', excluded:excluded };
...
function _btDiscrimRender(rep, watch, mk){
  ...
  var CRIT=[{key:'pnl',label:'손익'},{key:'n3',label:'N3'},{key:'n5',label:'N5'},{key:'n10',label:'N10'}];
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';line-height:1.45;margin-bottom:5px">'+DP.poolName+' 진입을 4가지로 가름: <b>손익</b>(거래 청산 손익)·<b>N3/N5/N10</b>(진입 후 그 봉수 뒤 종가>진입가). 각 재료가 어느 기준에서 '+DP.good+'을 가르나. '+DP.bad+'은 짧게 꺾이니 <b>N별 차이 자체가 정보</b>. 대표+관심 합산.</div>';
  ...
  out+='</div>';
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">읽기: <b style="color:'+GRN+'">강</b>=그 기준에서 '+DP.good+' 잘 가름·<b style="color:'+AMB+'">중</b>=약하게·<b>약</b>=못 가름·<b>-</b>=표본부족. 한 재료가 특정 N에서만 강하면 그 시간축이 핵심. <b>손익 기준은 BT 청산 로직이 껴 노이즈</b>일 수 있어 N 기준이 더 순수. ※강만 신뢰. kNN=STEP 2.</div>';
  out+='</div>';
  return out;
}
if(typeof window!=='undefined'){ window._btDiscrimRun=_btDiscrimRun; }
...
var _ENS_PATHS = [
  { id:'A', label:'거래량OSC &lt; -39.6', sub:'강 단독', test:function(f){ return f.volOsc!=null && f.volOsc < -39.6; } },
  { id:'B', label:'CCI&lt;143 AND BB%B&lt;1', sub:'약 조합1', test:function(f){ return f.cci!=null && f.cci<143 && f.bbPctB!=null && f.bbPctB<1; } },
  { id:'C', label:'RSI&lt;61 AND Stoch&lt;83', sub:'약 조합2', test:function(f){ return f.rsi!=null && f.rsi<61 && f.stochK!=null && f.stochK<83; } }
];
...
function _btEnsembleRender(entries){
  ...
  out+='<div style="font-size:8.5px;color:'+T3+';margin-top:7px;line-height:1.4">판단: 합집합(∪) <b>커버리지가 경로A보다 크게 늘면서 정밀도가 기저율보다 충분히 높으면</b>(<b style="color:'+GRN+'">+10%p↑</b>) → OR 앙상블 효과 있음(자유 빌더로 확장). 합집합 정밀도가 기저율과 비슷하면 → 약경로가 노이즈만 추가(거래량OSC 단독이 나음). 겹침에서 1경로 多=경로들이 다른 자리 포착(보완적).</div>';
  out+='</div>';
  return out;
}
...
var _BUILDER_DEFAULTS_DEADCAT  = { volOsc:{dir:'lt',th:-39.6}, cci:{dir:'lt',th:143}, rsi:{dir:'lt',th:61},  bbPctB:{dir:'lt',th:1},   stochK:{dir:'lt',th:83}, adx:{dir:'gt',th:20}, dev20:{dir:'lt',th:0}, dev60:{dir:'lt',th:0}, dev120:{dir:'lt',th:0}, dev200:{dir:'lt',th:0}, ma5slope:{dir:'gt',th:0} };
var _BUILDER_DEFAULTS_PULLBACK = { volOsc:{dir:'gt',th:8},     cci:{dir:'gt',th:158}, rsi:{dir:'gt',th:59},  bbPctB:{dir:'gt',th:1.1}, stochK:{dir:'gt',th:67}, adx:{dir:'gt',th:20}, dev20:{dir:'gt',th:8}, dev60:{dir:'gt',th:0}, dev120:{dir:'gt',th:0}, dev200:{dir:'gt',th:0}, ma5slope:{dir:'gt',th:0} };
