...
function _trendBt(rows,cfg,bbP){
  ...
  const _mkPos=(i,re)=>{
    if(_nextOpen){
      ...
      return { entry:o, entryIdx:j, re:!!re };
    }
    return { entry:close[i], entryIdx:i, re:!!re };
  };
  ...
  if(_predOn && _KN && _KN.buildVecs && n>=16+_MINB){ try { _pPre=_KN.buildVecs(rows,16); _pW=_pPre.W; } catch(_eP){ _pPre=null; } }
  ...
  const _knnProb=(i)=>{
    ...
    const cand=[];
    for(let e=_pPre.from; e<=i-_LBL; e++){ const cv=_pPre.vecs[e-_pPre.from]; if(!cv) continue; let d=0; for(let t=0;t<qv.length;t++){ const df=(qv[t]-cv[t])*_pW[t]; d+=df*df; } cand.push({d:d,up:_gcLabelAt(e)}); }
  ...
  for(let i=1;i<n;i++){
    ...
    if(pos==null){
      ...
      if(dc || sellHit(i) || _predFail){ const pnl=(close[i]/pos.entry-1)*100; trades.push({entry:pos.entry,exit:close[i],pnl:+pnl.toFixed(2),bars:i-pos.entryIdx,entryIdx:pos.entryIdx,exitIdx:i,re:!!pos.re,pred:!!pos.pred,predLead:pos.predLead||0,fail:!!_predFail,entryDate:(rows[pos.entryIdx]&&(rows[pos.entryIdx].date||rows[pos.entryIdx].t))||'',exitDate:(rows[i]&&(rows[i].date||rows[i].t))||''}); pos=null; _lastExitIdx=i; }
    }
  }
  ...
  // [S629] 라이브 D-day 골든크로스 예보 — 최신봉 기준, 엔트리 모드와 무관한 순수 예보(룩어헤드 아님)
  let predDday=null, predDdayProb=null;
  if(_predOn && _pPre && n>=2){
    const li=n-1;
    if(maS[li]!=null&&maL[li]!=null&&maS[li-1]!=null&&maL[li-1]!=null){
      const gp=maS[li]-maL[li], cv=gp-(maS[li-1]-maL[li-1]);
      if(gp<0 && cv>0){ const btc=-gp/cv; if(btc<=3.5){ const pr=_knnProb(li); if(pr>=_PTHR){ predDday=Math.max(1,Math.round(btc)); predDdayProb=Math.round(pr*100); } } }
    }
  }
  let open=null;
  ...
  return { winRate, pf, totalPnl, totalTrades:nT, avgWin, avgLoss, mdd:+(mdd*100).toFixed(2), expectancy, trades, open, lastClose:close[n-1], predHit, predFires:_predFires, predDday, predDdayProb };
}
...
function _trendRenderInner(){
  ...
  // [S628/S629] 선행 가드 + 자동전환 — ON이 OFF보다 손해/위험이면 라이브 상태를 OFF 동작으로(종목 과거 실측 기반, 룩어헤드 아님)
  let _guardBadge='', _guardHurt=false, _btOff=null;
  if(cfg.predict){
    try{
      _btOff=_trendBt(rows,Object.assign({},cfg,{predict:false}),_trendBbParams(market));
      if(_btOff){
        const dE=+(bt.expectancy-_btOff.expectancy).toFixed(2);
        const rM=_btOff.mdd>0?bt.mdd/_btOff.mdd:1, dM=bt.mdd-_btOff.mdd;
        let col,txt;
        if((rM>=1.5&&dM>=5)||dE<=-0.3){ _guardHurt=true; col='#e8365a'; txt=`⚠️ 선행 손해 · OFF 자동전환 (MDD ${_btOff.mdd}%→${bt.mdd}%${dE<0?` · 기댓값 ${dE}`:''})`; }
        else if(dE>=0.3&&bt.mdd<=_btOff.mdd*1.3){ col='#22c55e'; txt=`✓ 선행 유효 (기댓값 +${dE} vs OFF · MDD ${bt.mdd}%)`; }
        else { col='var(--text3)'; txt=`선행 중립 · OFF와 비슷 (기댓값 ${dE>=0?'+':''}${dE} · MDD ${bt.mdd}%)`; }
  ...
  // [S629] 가드 손해 종목은 라이브 상태를 OFF 동작으로 전환(백테스트 그리드는 선택 모드 유지 — 비교용)
  let st;
  if(_guardHurt && _btOff){ const _o=_trendState(rows,Object.assign({},cfg,{predict:false}),_btOff); st={ label:`🛡️ ${_o.label}`, color:_o.color }; }
  else st=_trendState(rows,cfg,bt);
  ...
  const maRow=`<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:8px;font-size:11px;color:var(--text2)">
    ...
    ${bt.predDday?`<span title="kNN+MA수렴 기준 골든크로스 임박 예보(적중률 동반)" style="margin-left:auto;font-size:10px;font-weight:800;color:#7c3aed;background:#7c3aed1a;border:1px solid #7c3aed55;border-radius:12px;padding:3px 9px">🔮 골든크로스 D-${bt.predDday}${bt.predDdayProb!=null?` · ${bt.predDdayProb}%`:''}</span>`:''}
  </div>
  ...
  const _predDesc=cfg.predLead==='auto'?'정배열 강추세면 2봉·아니면 1봉 선행 (kNN 3봉내)':(cfg.predLead===2?'정배열 강추세 + kNN(3봉내) → 2봉전 진입':'kNN(3봉내) 임박 시 1봉전 진입');
  const _phc=(bt&&bt.predHit!=null)?(bt.predHit>=65?'#22c55e':bt.predHit>=50?'#f59e0b':'#ef4444'):'var(--text3)';
  const _predAcc=(cfg.predict&&bt&&bt.predFires>0)
    ...
    : (cfg.predict?`<span style="font-size:9px;color:var(--text3)">적중 데이터 부족</span>`:'');
  const predRow=`<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">`
    +`<span onclick="_sxVib(10);window._trendTogglePredict&&_trendTogglePredict()" title="실제 골든크로스를 기다리지 않고, kNN이 '곧 교차할 모양'이라 보면 lead봉 먼저 진입. 예측 실패(교차 미확정) 시 손절." style="font-size:10px;font-weight:800;padding:5px 11px;border-radius:14px;border:1px solid;cursor:pointer;${cfg.predict?'background:#7c3aed;color:#fff;border-color:#7c3aed':'background:var(--surface2);color:#7c3aed;border-color:#7c3aed66'}">🔮 크로스 예측 진입 ${cfg.predict?'ON':'OFF'}</span>`
    +_predAcc
    +(cfg.predict?`<span onclick="_sxVib(10);window._trendCycleLead&&_trendCycleLead()" title="1봉→2봉→자동 순환. 자동: 추세 강하면 2봉, 약하면 1봉 선행." style="font-size:10px;font-weight:800;padding:5px 10px;border-radius:14px;border:1px solid #7c3aed66;background:#7c3aed18;color:#7c3aed;cursor:pointer">선행 ${_predLeadLbl}</span>`:'')
  ...
  const note=`<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · ${cfg.predict?`🔮 예측 선행 ${_predLeadLbl}(kNN) → `:''}${cfg.s}MA×${cfg.l}MA 골든크로스 ${cfg.nextOpen?'→ 다음봉 시가 진입':'종가 진입'}${_buyN?` · 매수 ${_buyN}개 AND(최근${cfg.n}봉)`:' · MA 단독'} → 데드크로스${_sellN?` OR 매도 ${_sellN}개`:''} 종가청산${cfg.predict?` · 예측실패 시 손절`:''}${cfg.reentry?' · 🔄정배열 재진입':''}</div>`;
  return head+maRow+chipRow+reChip+predRow+_guardBadge+stateRow+grid+note;
}
...
function _trendTogglePredict(){ const ctx=window._sxTrendCtx; if(!ctx) return; const m=ctx.market; const cfg=_trendCfg(m); cfg.predict=!cfg.predict; _trendSave(m,cfg); _trendRerender(); } // [S623] 🔮 크로스 예측 진입 토글
...
function _trendDetailInner(bt,cfg,ctx){
  ...
  return head+sub+flow+regimeHtml+listHead+list;
}
...
function _buildTransitionCard(stock, indicators){
  ...
  let _kn = null;
  try { if(window.SXKNN && SXKNN.score) _kn = SXKNN.score(rows, { win:16, k:10 }); } catch(_ekn){ _kn = null; }
  ...
  const head = `<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="font-size:13px;font-weight:800;color:var(--text)">🧪 캔들 전이 예측</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span><span onclick="_sxVib(12);window.SXCandleBT&&SXCandleBT.open()" title="과거 데이터로 이 예측의 적중률 검증" style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;background:#7c3aed22;color:#7c3aed;border:1px solid #7c3aed66;cursor:pointer">검증</span><span style="font-size:10px;font-weight:700;color:${_dateCol};margin-left:auto">${_dateLabel}</span></div>`;
  ...
  const col = _pScore>=30 ? '#e3493b' : _pScore<=-30 ? '#2563eb' : '#f59e0b';   // [S489] 양봉유망=빨강 / 음봉유망=파랑 (미니캔들 색과 동일)
  const sign = _pScore > 0 ? '+' : '';
  const _pLabel = _useKnn
    ? (_pScore>=30 ? '양봉 전이 유망' : _pScore<=-30 ? '음봉 전이 유망' : '중립')
    : r.label;
  ...
  if(_useKnn){
    const _hk = ((stock&&(stock.code||stock.name))||'') + '|' + _tfCT + '|' + (_lastDate||'') + '|' + rows.length;
  ...
  const confHtml = _useKnn
    ? `<div style="margin-top:8px"><span style="font-size:11px;font-weight:800;color:#7c3aed;padding:3px 9px;border-radius:7px;background:#7c3aed1a;border:1px solid #7c3aed55">🧬 자기유사도 kNN</span> ${_hitHtml}<div style="font-size:10px;color:var(--text3);line-height:1.55;margin-top:5px">최근 16봉 패턴과 가장 닮은 과거 ${_kn.k}봉의 다음봉 색으로 예측. 적중률은 이 종목 과거 150봉 검증치(모달 [검증]과 동일). |점수|≥30만 방향 신뢰.</div></div>`
  ...
  const note = `<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">${_useKnn?'메인 = 자기유사도 kNN(검증) · 아래 캔들/해설은 보조 · ':''}실험 지표 · 정식 판정과 무관 · −100~+100 (양수=양봉 전이 유망, 음수=음봉 전이 유망)</div>`;
  ...
  return wrap(top + _knNbrBar + confHtml + _nudgeHtml + bar + _seqRow + _hitRow + warnHtml + reasonsHtml + note);
}
