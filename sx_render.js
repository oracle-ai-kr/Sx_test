...
function _computeBoardInputs(stock, indicators, scores, scoreMom, tf){
  ...
  if(indicators && indicators.rs && indicators.rs.rs20 != null){
    _boardExtras.rs = { v: Math.max(0, Math.min(100, Math.round(50 + indicators.rs.rs20 * 2.5))) };
  }
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _btnTrendLevel = _btnTrendScore >= 70 ? '강추세'
                             ...
                             : '하락추세';
        // [S444] 분석신호검토 버튼 텍스트 = C verdict 판단(색은 이미 _timingBtnClr=_svVerdict.color).
        //   label이 action을 포함하면 풍부하게(예: '관망 — 진입 보류'), 아니면 action만(예: '매수'). verdict 없으면 추세 폴백.
        const _btnVd = stock._svVerdict;
        const _btnVerdictTxt = (_btnVd && _btnVd.action)
          ? ((_btnVd.label && _btnVd.label.indexOf(_btnVd.action) >= 0) ? _btnVd.label : _btnVd.action)
          : `추세 ${_btnTrendScore} · ${_btnTrendLevel}`;
