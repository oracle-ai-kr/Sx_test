// ════════════════════════════════════════════════════════════
//  SIGNAL X — Render Engine v7.8 <!-- S127 후속 fix1 배너 C 판정 배지 "청산 → 매도" UI 치환 (render 출력 레이어 한정). [배경] v7.7에서 상단 엔진시뮬 포지션 라인의 "▼ 청산"만 "▼ 매도"로 고쳤는데, 그 위쪽 C 판정 배지(_svVerdict.action 직접 출력)는 그대로 "🟠 청산 검토" / "🟡 청산 준비" / "🔴 즉시 청산"으로 표시되는 비일관 발견(LS ELECTRIC 스크린샷). [변경 1건] btStateBannerHTML 배지 출력 L1854 — `${_svVerdict.action}` → `${_svVerdict.action.replace('청산', '매도')}` 치환. 결과: 배너 3종 배지가 "🟡 매도 준비 / 🟠 매도 검토 / 🔴 즉시 매도"로 표시됨(색상·아이콘·로직은 동일). [원칙 유지] SXC.unifiedVerdict 내부 return label(L155 'label:\"청산 검토\"' 등) 및 action 값('청산 준비'/'즉시 청산'/'청산 검토') 모두 원본 그대로 — switch case, includes 배열, verdictAction === '청산 준비' 비교 로직 전체 영향 없음. 치환은 **render.js 출력 직전 1회**로 한정, SXC 소스 불변. [검증] 전 파일 스캔 결과 `${...action}` 템플릿 출력은 L1854가 유일(interpret/project_c에는 없음) → 한 줄 수정으로 완결. [대상] sx_render.js 1854줄. [효과] v7.7 _buildSimPositionLine "▼ 매도" + v7.8 배지 "매도 검토" → 배너 전체가 "매도" 톤으로 완전 통일, interpret.js v5.3 본문(summary/practicalGuide)과도 정합. --> v7.7 <!-- S127 후속 "청산 → 매도" UI 용어 치환 (본문 텍스트 한정). [배경] v7.6 4층 구조 리팩토링 완료 후, 상단 배너 sell_signal 라인이 여전히 "▼ 4/6 청산"으로 렌더되어 interpret.js의 summary/practicalGuide 톤("매도 완료")과 어긋남 → 시각 일관성 확보 필요. [변경 3건] (1) _buildSimPositionLine sell_signal 분기 mainText — "▼ ${dateStr} 청산" → "▼ ${dateStr} 매도" (992줄). (2) 상단 플로우 주석 "청산일" → "매도일" (881줄). (3) 블록 주석 "엔진시뮬 청산 완료" → "엔진시뮬 매도 완료" (979줄). [원칙] verdictAction 값('청산 준비'/'즉시 청산')과 `case '청산'` 등 로직 심볼은 건드리지 않음 — interpret.js 사전 치환과 동일 스코프(UI 본문 텍스트만). "매도" = 포지션 종료의 중립 용어로 통일, 초보자 혼동 최소화. [대상] sx_render.js _buildSimPositionLine 본체 + 헤더 주석. --> v7.6 <!-- S127 분석탭 "진입 검토" 펼침 영역 4층 구조 리팩토링 (C→A→A→C): [목표] 버튼(C) → Layer 2(A, 검토) → Layer 3(A, 강세/약세) → Layer 4(C 시각테마, 추세) → 종합평(C+B, 별도섹션). [변경 5건] (1) 버튼 색상 — polarity+_transition.value 구간 분기 → SXC.getVerdictBadge tx-badge 클래스 매칭(매수 녹색 / 관심 하늘색 / 청산준비·검토 주황 / 즉시청산·매도완료 빨강 / 그 외 회색) → 상단 배너 배지와 시각 통일. (2) 버튼 문구 — "${_transition.label} ${_transition.value}%" (예: 관망 강등 70%) → "추세 ${score} · ${level}" (예: 추세 47 · 약세) → 펼침 Layer 4 요약 미리보기 관계 성립. (3) ready 섹션 내부 전이박스(_r2eLabel) 제거 → 버튼·종합평 중복 해소. (4) entry 섹션 내부 전이박스(_e2tLabel) 제거 → 동일 논리. (5) 진입 검토 추이 차트 위치 이동 — 구간분포 추이 다음(맨 아래) → ready 섹션 직후 (Layer 2 차트로 재편성). (6) trend 섹션 C 시각테마 — 좌측 4px accent border + accent-glow 배경 + "추세" 라벨 accent 색상 → A/C 레이어 시각 구분. [부수 정리] _transition/_svRr4f/_r2eLabel/_e2tLabel 계산 및 변수 제거(죽은 코드 정리). SXC.calcVerdictTransition 함수 자체는 보존(향후 복원 대비). [대상] sx_render.js 2138~2420줄 (_3stageId 내부 + 버튼 영역). --> v7.5 <!-- S120-2 강건성 배지 (🌱 신뢰 / ⚠️ 불안): "BT 봉수 늘어 거래수 늘면 양질일까?"(채희창) 통찰 → 200봉 vs 600봉 수익률 편차로 구간 의존성(과적합) 탐지. [계산] _runEngineVerify(rows 600봉+)에서 rows.slice(-200)으로 200봉 재BT → stock._btResult_200 저장 → 편차=|pnl200-pnl600|/max(|pnl600|,1) → 20% 미만=🌱신뢰, 20% 이상=⚠️불안. 거래수 200/600 둘 다 3건 이상일 때만 배지 표시 (표본 부족 시 숨김). [렌더] 분석탭 엔진판단 검증 카드 헤더 + 단일검증 탭 bt-card-title에 배지 삽입. hover tooltip으로 "200봉 X% vs 600봉 Y% · 편차 Z%" 상세. 색상: 🌱 녹색(#22c55e) / ⚠️ 주황(#f59e0b). [저장] stock._robustness = { label, deviation, pnl200, pnl600, trades200, trades600, show }. [리셋] openAnalysis에서 stock._btResult_200, stock._robustness도 초기화 (이전 종목 잔존 방지). [철학] 한국어 2글자 미학 - "과적합" 전문용어 대신 "불안" 감정 단어로 비개발자 친화. [sx_bt.js 동기화] btRunBasic에서도 동일 로직 추가 - 단일검증 [▶ 백테스트 실행] 경로에서도 배지 표시. --> v7.4 <!-- S120-1 판정 근거 해체 (채희창 정리 철학): 분석탭 배너의 "▶ 판정 근거" 펼침 영역 완전 제거. [배경] 펼침 내용에 3개 엔진(SXC/SXE/BT) 데이터가 혼재하여 사용자 혼란(같은 라벨 다른 의미: "손익비 0.29"=_svRr vs "손익비 1.70"=profitFactor / 같은 목적 다른 시점: 배너 목표 211,666=BT 진입가 기준 vs 판정근거 TP 188,984=분석엔진 현재가 기준). [삭제 6건] ①분석엔진 TP/SL(배너 상단 _buildSimPositionLine과 중복) ②BT 기준 TP/SL(배너 상단과 중복) ③ATR/피봇 reasons(지표 분석 섹션에 이미 있음) ④BT 요약 21거래...(엔진판단 검증 6칸 카드에 이미 있음) ⑤전이 62→66 5봉(진입 검토 카드에 더 상세) ⑥손익비 _svRr(미러 트레이딩 철학: 엔진시뮬 기준만 노출). [코드 삭제] 1733~1775줄 _svTpsl/_svTp/_svSl/_svTpPct/_svSlPct/_btSummary/_rrWarn 계산 + _detailHTML 조립 + 1796~1799줄 판정 근거 버튼/펼침 영역 완전 제거. [유지 변수] _svRr(supervisorJudge+calcVerdictTransition 입력), _svMom(supervisorJudge+_dirText 입력). [UI 결과] 배너는 C 판정 배지 + _dirText + _buildSimPositionLine만 표시 - 대폭 간결화. [철학] 채희창: "C는 현시점 매수/매도, BT는 시뮬레이션, 기본(분석엔진)은 C가 대처하니 전부 감추자". 프로젝트 C v3.0 원칙 ⑦ "UI 역할 분리" 강화 — 측정값=지표 분석/판정=C 배너/검증=BT 6칸 카드/조건=진입 검토. --> v7.3 <!-- S119 fix2 버튼 재클릭 시 중복 실행 방지: 자동 실행(S119 본체)으로 BT 완료된 상태에서 사용자가 엔진판단 검증 버튼을 재클릭하면 카드 토글만 하고 _runEngineVerify 재호출은 skip. 기존 버튼 onclick은 willOpen(카드 열림)일 때 무조건 _runEngineVerify 호출 → 이미 결과 있는데도 재실행 + "✅ 엔진판단 검증 완료" 토스트 반복 문제. 해결: onclick에 `!currentAnalStock._btResult` 체크 추가 — BT 결과가 이미 있으면 일반 accordion 버튼처럼 펼침/접힘 토글만 수행. 채희창 원칙: "자동완료되고 토스트에서 완료 표시 이후 일반버튼처럼 세부내역 펼치고 접는용도". 분석탭 = 결과 보기, 단일검증 탭 = 세부 조정 + 재실행(옵션 변경 시) 역할 분리. --> S119 fix1 미국 시장 무한 루프 방지: _isExtSupported=false인 미국은 _runEngineVerify 경로 D(300봉 단일 fetch, _lastAnalCandles 갱신 안 함)로 _curBars=500 그대로 유지 → _curBars<_targetBars 영원히 true → 🔁 무한 루프 위험. 해결: 미국(us)은 _targetBars=0으로 설정하여 자동 실행 조건 원천 차단. 미국은 S114에서 이미 range=2y로 400~500봉 기반 BT 실행 완료 상태이므로 자동 재실행 불필요. --> S119 본체 엔진판단 검증 자동 실행 (분석탭 진입 시 백그라운드): runAnalysis 완료(renderAnalysisResult 직후) 시점에 500ms 지연 후 _runEngineVerify(stock) 자동 호출. [목적] 모든 종목 600봉 기준 통일 — "공평성을 따지면 다 같이 600봉이 맞아"(채희창). 사용자가 엔진판단 검증 버튼을 수동 클릭한 것과 동일한 효과를 분석탭 진입 시 자동으로 제공. [플로우] 1)200봉 fetch → 2)자동 400봉 확장 → 3)화면 표시(renderAnalysisResult) → 4)★500ms 후 _runEngineVerify 자동 호출(백그라운드) → 5)400→600봉 확장 + BT + runAnalysis 재귀 → 6)재귀 시 봉수>=600이면 자동 실행 SKIP(무한 루프 방지). [자동 실행 조건] stock._btResult 없음(처음 진입/종목 전환 후) 또는 봉수<목표봉수(일봉 600, 주/월봉 400). [중복 방지] _engineVerifyRunning 플래그 + 500ms 지연 후 재확인(사용자가 이미 클릭했거나 다른 종목 이동한 경우 skip) + currentAnalStock === stock 체크. [이슈 B 자연 해소] S118 미해결 "분석탭 400봉 vs 단일검증 600봉 거래수 불일치" → 이제 분석탭 진입 시 자동 600봉 확장되므로 두 탭 항상 동일. [UX] 방법 B(백그라운드) — 화면 먼저 표시 → 뒤에서 BT 실행 → 완료 시 runAnalysis 재귀로 화면 자동 갱신(엔진판단 검증 카드 펼침 포함). --> v7.2 <!-- S118 fix2+fix3 분석탭/단일검증 탭 색상 규칙 통일 + 핑크 톤 조정: 손익비 색상을 검정(var(--text))에서 핑크(#f472b6, fix3에서 #ec4899→#f472b6로 변경, 빨강과 구분 강화)로, MDD 색상을 검정에서 보라(#8b5cf6)로 변경. [원칙] 부틱님 정의: "빨강=부정 / 녹색=긍정 / 핑크=보조긍정 / 보라=보조부정". 주 지표(승률/수익률/평균소득)는 녹색/빨강으로 방향성 강조, 보조 지표(손익비/MDD)는 핑크/보라로 정보 톤 차별화 → "온통 녹색과 빨강뿐"이라는 애매함 해소. [4색 체계] #22c55e 녹색(기본 긍정) / #e8365a 빨강(기본 부정) / #f472b6 핑크(손익비 고정, 소프트 분홍) / #8b5cf6 보라(MDD 고정). [단일검증 탭 쪽은 sx_bt.js btRenderBasicResult 별도 수정]. --> v7.1 <!-- S118 hotfix 종목 전환 시 단일검증 탭 결과 완전 초기화. [버그] S115 hotfix3에서 "단일검증 탭 동기화" 기능(btRenderBasicResult 호출) 추가 시 저장 경로만 만들고 종목 전환 시 리셋 경로 누락. 이전 종목(예: 알테오젠)에서 엔진판단 검증 실행 → #btBasicResult DOM에 결과 렌더됨 → 새 종목(예: LG화학) 선택해도 #btBasicResult는 이전 종목 결과 그대로 유지 → 단일검증 탭 클릭 시 헤더는 LG화학인데 내부엔 알테오젠 결과 보이는 현상. [해결] openAnalysis에서 종목 전환 시 (1) stock._btResult = null, stock._btScore = null, stock._btState = null로 리셋 (2) document.getElementById('btBasicResult').innerHTML = '' + display:none으로 DOM 초기화. [영향] 단일검증 탭 진입 시 "백테스트 실행 버튼만 있는 초기 상태"로 돌아감 → 사용자가 다시 [▶ 백테스트 실행] 클릭하거나 분석탭에서 엔진판단 검증 클릭으로 갱신. 채희창 발견: "BT 단일검증이 다른 종목 들어가도 리셋이 안 돼" — 스크린샷으로 LG화학 종목 단일검증 탭에 알테오젠(196170) 결과 잔존 확인. 프로젝트 C 원칙 ⑤(정합 우선) 강화. [남은 이슈 B] 분석탭 400봉 vs 단일검증 600봉 봉수 차이로 인한 거래수 불일치 — S118 이후 별도 세션에서 처리 예정. --> v7.0 <!-- S117 엔진판단 검증 결과 카드 색상 규칙 재정의 + 평균소득 라벨 도입 + 버튼 색상 BT 평균소득 기반으로 전환: (1) 승률/수익률/평균소득 — 음수 빨강(#e8365a) / 양수 녹색(#22c55e) 2단계 단순화. (2) 손익비/MDD — 기본 검정(var(--text))로 중립화. (3) 거래수 — 데이터 충족 라벨과 동일 색(부족=빨강/충족=파랑/충분=녹색) 연동. (4) "평균이익" → "평균소득" 이름 변경. 계산식은 동일(totalPnl / trades = 거래당 순 평균). 채희창 정의: "실제 매매를 17번 했다면 결과적으로 남은 거래당 소득". 단일검증 탭의 "평균이득"(avgWin, 이긴 거래 평균)과 구분 — 라벨 분리로 혼동 방지. (5) 엔진판단 검증 버튼 색상 — SXC.calcEntryEvaluation 점수 기반(S114에서 카드는 삭제했으나 버튼 색만 잔존한 역행 이슈 잔재) → 실제 BT 평균소득 기반 5단계로 전환. 기준: BT 미실행 회색 #94a3b8 / 평균소득 ≥+5% 녹색 #22c55e / 0~+4% 하늘색 #3b82f6 / -4~-1% 주황 #f97316 / ≤-5% 빨강 #e8365a. 소수점 무시(Math.trunc). 버튼 하단 문구("거래 N 승률 N%")와 같은 BT 결과 출처라 시각 일관성 완성. (6) _entryEval 변수 참조 완전 제거 (S114 삭제 이후 잔존 코드 정리). 프로젝트 C 원칙 ①②⑤⑦⑧ 준수 강화. --> v6.9 <!-- S116 매매 근거 상세 카드 완전 제거 + 하단 "매매전략" 배너 제거 + 데이터 충족 라벨 이관. 버튼 onclick에서 willOpen 판정 후 플래그 저장 → 카드 렌더 시 display:${flag ? 'block' : 'none'}로 조건부 렌더 → runAnalysis 재호출 시 열림 유지. (2) 단일검증 탭 결과 동기화 (부틱님 제안): _runEngineVerify 완료 시 btRenderBasicResult(stock, r) 호출 → 단일검증 탭 #btBasicResult DOM에도 결과 렌더 → 나중에 단일검증 탭 클릭 시 바로 결과 확인 가능. (3) _runEngineVerify 전면 재작성: 탭 전환 방식(switchAnalTab) 완전 폐기 → sx_bt.js의 BT 실행 로직(경로 A/B/C/D)을 인라인으로 실행 → DOM 의존성 제거 → 현재 분석탭 유지하며 백그라운드 실행. (4) willOpen 판정 후 열릴 때만 _runEngineVerify 실행 (닫힐 때는 BT 재실행 안 함 → 불필요한 호출 방지). (5) 종목 전환 시 openAnalysis에서 _engineVerifyOpen / _3stageOpen 플래그 초기화 (이전 종목 상태 잔존 방지). (6) 상단 "매도 검토" 버튼 onclick도 _3stageOpen 플래그 관리 추가 → 양쪽 카드 상호 배타적 토글 + 각자 상태 독립 유지. 채희창 통찰: "같은 버튼인데 단일검증탭도 백테스트 완료가 되어야" → 분석탭/단일검증 완전 양방향 동기화 달성. --> v6.7 <!-- S114 엔진판단 검증 전환 + 400봉 기본화 + 역행 이슈 해결 + 정밀검사 fix1: (1) 분석탭 진입 시 무조건 400봉 확장 (기존 "거래수<10" 조건 제거), 실제 2초 대기 추가 (기존엔 주석만 있고 미적용). (2) 미국 시장 확장 지원 — sx_screener.html range=2y로 일봉 500봉+ 확보, stage=1 즉시 세팅 (한국/코인은 기존 fetchCandlesExtended + 2초 대기 방식 유지). (3) "엔진판단 근거" → "엔진판단 검증" 이름 변경. 버튼 클릭 시 단순 토글이 아닌 신규 함수 _runEngineVerify(stock) 호출 → 단일검증 [▶ 백테스트 실행]과 동일 동작 (btRunBasic 호출, 400→600봉 확장, BT 재실행, 분석탭 자동 재렌더). (4) 매수 근거 평가 카드 (calcEntryEvaluation 60점 양호한 매수) 완전 제거 → BT 결과 6칸 카드로 교체 (승률/손익비/수익률/거래수/MDD/평균이익). [역행 이슈 해결]: calcEntryEvaluation은 C 판정과 독립 계산되어 즉시청산 상태에서도 "양호한 매수 60점" 표시되는 문제 (S112 미래에셋증권 사례). 이제 실제 BT 결과 표시로 전환 — 검증된 데이터라 C와 독립이어도 사실이므로 모순 없음. 프로젝트 C v3.0 원칙 ⑤(정합 우선) + ⑧(검증가능성) 준수. (5) 매매전략 배너 ? 버튼 UI 숨김. --> v6.6 <!-- S112 방향 3 (정합성 보장): 자동 확장 후 "runAnalysis 재귀 호출"로 분석+BT 완전 재계산. 기존 문제: 확장 시 BT만 400봉으로 재실행, 분석 점수는 200봉 기준 그대로 → 한 화면에 기준 다른 숫자 공존 (LG에너지솔루션 분석 12회 vs BT 23회 유사 현상). 해결: _extraCandles 병합 후 stock._lastAnalCandles 저장 → return runAnalysis(stock) 재귀. 재귀 진입 시 383줄 재사용 로직(_lastAnalCandles.length > _analCount)이 fetch skip하고 전체 계산(indicators, scores, sectorItp, maAlignItp, qs, _btResult, 모멘텀 등) 확장된 봉수 기준으로 재수행. 무한 루프 방지: _analCandlesExtendedStage=1 이미 설정(483줄)되어 재귀 진입 시 _curStage<1 조건 불만족 → 재확장 안 함. 채희창 통찰: "같은 시각 데이터를 분석 로직과 BT가 사용해야 정합이 된다" — 프로젝트 C 원칙 ⑤(정합 우선)와 일관. --> v6.5 <!-- S109 bugfix + Phase 3-B-9a-ext-fix6: (1) 결과탭 시장별 저장 버그 수정: KEYS.SEARCH_RESULTS_KR/US/COIN 3개 키로 분리. (2) fix6 배너 stage 분기 안내: stage 0~1 (확장 가능) → "거래 N회 / 최소 10회 필요". stage 2 (600봉) → "💡 [권장 TF] 전환 권장". --> v6.4 <!-- S108 Phase 3-B-9a-ext 수동 데이터 확장 + fix (3차): (1) "?" 버튼을 수동 확장 버튼(+200봉)으로 교체. (2) stage 플래그 0/1/2 전환. (3) _loadMoreCandles 함수 신규. (4) fix1: _lastAnalCandles 재사용. (5) fix2: 재렌더 전후 펼침 상태 캡처/복원. --> v6.2 <!-- S107 Phase 3-B-9a 분석탭 BT 자동 확장 (Reactive Loading): runAnalysis에서 BT 1차 실행 후 totalTrades < BT_MIN_TRADES(10) 감지 시 자동으로 fetchCandlesExtended 호출 → +200봉 → BT 2차 실행. --> v6.1 <!-- S105 Phase3-B-4f.5 엔진판단 근거 버튼 C 기반 실시간화 --> v6.0 <!-- S103-fix7 Phase3-B-4f.4 진입/매도 검토 C 전이 확률화 --> v5.9 <!-- S103-fix7 Phase3-B-4f.3 용어 통일 추가: ①"진입타이밍"→"진입 검토" 전체 교체(22곳). ②"매도타이밍"→"매도 검토" 전체 교체. ③한국어 조사 자동 교정(받침 없어져 '은/이'→'는/가'): 4곳 수동 수정(859/866 "은"→"는", 935 "이"→"가", 879 HTML태그 뒤 "이"→"가"). UI 라벨/도움말HTML/detailText/코드주석/차트라벨 등 전 영역에 적용. 헤더 주석(이력 추적)은 제외. 용어 철학: "타이밍"은 추상적/불확정, "검토"는 구체적 행동 지칭 + 사용자 행동 유도 강화. --> v5.8 <!-- S103-fix7 Phase3-B-4f.2 후속개선2 (W-6 TP/SL 이중표시 해결): ①엔진판단 근거 펼침의 5개 가격카드(현재가/진입가/목표가/손절가/손익비) 완전 제거. 이유: 상단 배너(엔진시뮬 기준 BT TP/SL)와 하단 카드(분석엔진 기준 TP/SL)가 충돌 → 사용자 혼란. v2.0 원칙 ⑤/⑥에 따라 엔진시뮬 단일 관점으로 통합. ②버튼명 "매매전략" → "엔진판단 근거"로 변경 (카드 없으므로 "판단 근거"로 의미 명확화). ③매매 근거 상세 텍스트(ATR/피봇/채널 분석)는 유지 (엔진의 판단 근거로 가치 있음). --> v5.7 <!-- S103-fix7 Phase3-B-4f.1 후속개선: ①종목헤더에 현재가+전일대비 추가(배너 pnl% 비교기준점 제공, 종목명 밑 배치로 중복 제거). ②배너 타이틀 용어 초보자 친화 교체: "시뮬"→"엔진시뮬"(분석엔진+BT엔진이 돌리는 시뮬레이션 명확화), "포지션"→"보유중"(금융용어 대체), "신호"→"신호포착"(이벤트 명확화). 용어 체계: 엔진시뮬 보유중/신호포착/완결/관찰/보류/상태. --> v5.6 <!-- S103-fix7 Phase3-B-4f 상단 배너(btStateBannerHTML)에 "시뮬 포지션 라인" 추가 (프로젝트 C v2.0 시뮬 내러티브). _buildSimPositionLine 헬퍼 신규(5가지 상태 분기 holding/buy_signal/sell_signal/waiting/no_data). v5.4 _buildBtSimLine 롤백 교훈 반영 — C 판정(svVerdict)에 맞춰 어조/포커스 적응, 차트 ▲/▼ 기호와 시각 일관성, "📊 시뮬 포지션/신호/완결/관찰/보류/상태" 타이틀로 의미 명확화. v2.0 철학: 시스템 = 가상 트레이더, 사용자 = 미러링. 배너는 이제 판정 요약 + 시뮬 포지션 상태 + 판정 근거 접힘. --> v5.5 <!-- S103-fix7 Phase3-B-4a 상단 배너(btStateBannerHTML)에서 _buildBtSimLine 호출+함수 정의 완전 제거 (v5.4의 BT 시뮬 라인이 C 판정과 시점 불일치 혼란 유발로 롤백). _rrWarn/BT요약을 판정근거 접힘 영역으로 이관한 구조는 유지(v5.4 개선점). 상단 배너는 이제 순수 C 판정 + 방향 텍스트 + 접힘 영역의 판정 근거만 표시. 채희창 통찰: BT trades(과거 매수/매도 시점) vs C 판정(현재 상태)은 다른 시간축이라 한 배너에 섞으면 혼란. --> v5.4 <!-- S103-fix7 Phase3-B-4 상단 배너(btStateBannerHTML) BT 시뮬 포지션 전환 - 목표/손절 이론값 라인 제거(섹션7 매매전략 클릭 영역과 중복)+_rrWarn/BT요약 라인 제거(판정근거 접힘 영역으로 이관)+BT 시뮬 포지션 라인 추가(_buildBtSimLine 헬퍼 신규). 상태별 5분기: holding(▲매수 날짜+가격+현재수익률+보유일), buy_signal(▲매수신호+진입제안), sell_signal(▼매도완료+익절/손절 pnl), waiting(점수/임계값+부족분), no_data(BT 미실행 안내). --> v5.3 <!-- S103-fix7 Phase3-B-7 C 로직 5종(supervisorJudge/unifiedVerdict/_mapVerdictToBtAction/_verdictMap/_verdictBadgeTop) 모두 sx_project_c.js로 분리 (SXC 네임스페이스), render.js는 SXC.xxx 호출로 전환, 호출부 13곳 교체, 기능 변경 없음 --> v5.2 <!-- S103-fix7 Phase3-B-3 SXI.summary 호출부 3곳에 verdictAction 5번째 파라미터 전달 — 종합평/행동가이드/무효화/강화조건이 보유중 5종일 때 보유자 맥락으로 재작성됨 --> v5.1 <!-- S103-fix7 Phase3-B-1 practicalGuide 호출부 _btAction → _svVerdict.action 교체(C 직접 소비, 프로젝트 C 원칙 ② 준수) --> v5.0 <!-- S103-fix7 Phase3-B-2 _btAction C 기반 매핑 교체(14종목 진단 결과 14/14 불일치 확인 → 점수 기반 es×bs 4분류 폐기, _svVerdict.action 9종 → _btAction 4종 매핑 함수 도입. 분석탭 3곳(522,2543,2699 → 범위)에 적용. 스캔루프(screener.html/scan_worker)는 Phase 3-B-2b로 연기) --> v4.9 <!-- S103-fix7 Phase3-A-3 unifiedVerdict 4-arg(btStateObj 추가)로 매도완료 익절/손절+pnl% 라벨 강화, threeStageReport 호출에 btStateObj 전달 --> v4.8 <!-- S103-fix7 Phase3-A-2 unifiedVerdict 재설계(채희창 매트릭스: BT×분석 2×2, empty Stage btScore≥60→관심/<60+분석○→관망/<60+분석✗→회피), "대기" 폐지→"회피" 통합, _verdictMap/_verdictBadgeTop 매도완료 추가/회피제거, btStateKey 판정 경로 3곳 btScore 인자 추가+2497줄 _isBuySignal 체크 보강 --> v4.7 <!-- S103-fix6c Phase2 후속개선 감독관기반 행동배지 위치이동(통합배너 내부 → 진입타이밍 버튼 위 좌측정렬, 가시성 개선) + 배너 내부 배지 제거(중복방지) --> v4.6 <!-- S103-fix6c Phase2 양방향3단 전이확률라벨(강세전이↔약세전이) + 감독관통합판정기반 행동배지 + .tx-badge CSS 4종(screener.html) --> v4.5 <!-- S103-fix6c Phase1 양방향3단 entry섹션타이틀+강세추이차트 라벨동기화(강세↔약세) --> v4.4 <!-- S103-fix6 매도/진입타이밍 라벨 동기화(3단리포트 ready섹션+추이차트) + 가격UI 4줄→3줄 압축(pct+time 같은줄 · 구분자) + 공시배지 2열그리드(홀수마지막 중앙정렬) + fix6b 공시배지 좌우쌍 가운데수렴(좌=우측정렬+우=좌측정렬) --> v4.3 <!-- S103-fix5 분석탭 TF칩/모드칩/연관키워드 진동 3종 추가 --> v4.2 <!-- S103 분석탭 동적 삼각형 토글 24개 + 차트전체화면 진동 추가 --> v4.1 <!-- S103 결과탭 관심목록(12)/초기화(15) 진동 추가 --> v4.0 <!-- S103 _stratDetailId 스코프버그 수정 (IIFE 안 선언→함수 본체 호이스팅) --> v3.9 <!-- S100 -->
...
function saveSearchResults(){
  try{
    ...
    localStorage.setItem(key, JSON.stringify({results:slim, sortKey, sortDir, ts:Date.now()}));
...
function openAnalysis(idx){
  ...
  //   채희창 발견: "BT 단일검증이 다른 종목 들어가도 리셋이 안 돼"
  // ═══════════════════════════════════════════════════════════════════
...
async function _runEngineVerify(stock){
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════
    // [목적] "BT 봉수가 늘어서 거래수가 늘면 양질의 매매일까?"(채희창)
    ...
    // ─── 단일검증 탭 결과 동기화 (부틱님 제안: "같은 버튼인데 단일검증탭도 완료돼야") ───
...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  const _btSt = stock._btState || null;
  {
    ...
    // ═══════════════════════════════════════════════════════════════
    // S120-1 판정 근거 해체 (2026-04-20 오후 세션, 채희창 정리 철학)
    // ═══════════════════════════════════════════════════════════════
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // S127: 진입 검토 버튼 색상 — 상단 배너(_verdictBadgeTop)의 tx-badge 색상과 통일
        //   [이전] polarity + _transition.value 구간별 분기 (배너와 다른 기준 → 시각 분리)
        //   [신규] SXC.getVerdictBadge()의 클래스 매핑과 동일 색상 채용 → 버튼과 배너가 한 묶음으로 보임
        //     매수(반등유력)          → #22c55e (up)
        //     관심(반등조짐)          → #0ea5e9 (up-soft)
        //     청산 준비/검토(하락조짐) → #e67e00 (down-soft)
        //     즉시 청산/매도 완료(하락유력) → #e8365a (down)
        //     그 외(보유 유지/관망/회피) → #94a3b8 (중립 회색, 배지 없음 케이스)
        //   버튼 내부 문구도 함께 교체: 전이 확률 → 추세 점수/레벨 미리보기 (S127 ④)
        let _timingBtnClr;
        switch(_svAction4f){
          case '매수':          _timingBtnClr = '#22c55e'; break;
          case '관심':          _timingBtnClr = '#0ea5e9'; break;
          case '청산 준비':
          case '청산 검토':     _timingBtnClr = '#e67e00'; break;
          case '즉시 청산':
          case '매도 완료':     _timingBtnClr = '#e8365a'; break;
          default:              _timingBtnClr = '#94a3b8'; // 보유 유지/관망/회피/미판정
        }
        ...
        const _btnTrendLevel = _btnTrendScore >= 70 ? '강세'
                             : _btnTrendScore >= 60 ? '양호'
                             : _btnTrendScore >= 50 ? '중립'
                             : _btnTrendScore >= 40 ? '약세'
                             : '하락';
        ...
        ${(()=>{
          ...
          html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px"><span style="font-size:11px;font-weight:800;color:var(--accent)">추세</span><span style="font-size:14px;font-weight:800;color:var(--'+tCls+')">'+_report.trend.score+_deltaTag(_trendDelta)+'</span><span style="font-size:9px;color:var(--text3)">'+_report.trend.level+'</span></div>';
      ...
      ${(()=>{
        ...
        //     · 부틱님 설계: "엔진판단 근거 → 엔진판단 검증" (실제 백테스트로 검증)
      ...
      <div style="margin-top:10px;padding:14px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid var(--text3);text-align:center">
        ...
        //   부틱님 정의: "실제 매매를 17번 했다면 결과적으로 남은 거래당 소득"
      ...
      ${(()=>{
        ...
        //   [이유] 채희창 통찰: "ATR/피봇은 누구 판단이야?" → Layer 1 독자 판단 확인 → 삭제
