...
function _buildChartPredictCard(stock, indicators){
  ...
  const _regimeOn = !!(typeof window!=='undefined' && window._sxCpRegimeOn);   // [S663] 레짐필터 토글(기본 OFF)
  let r; try { r = SXCP.predict(rows, { regime:_regimeOn }); } catch(_e){ return ''; }
  ...
  // [S663] 레짐 토글 + 현재 레짐 — 카드가 보이면 항상 접근 가능(끄기 가능)
  const _toggle = `<span onclick="_sxVib(8);window._cpRegimeToggle&&_cpRegimeToggle()" title="ON: 현재 레짐(상승추세/횡보/하락추세)과 같은 구간만 집계해 장분위기를 맞춤. 표본이 줄어 ≥10개 안 되면 비활성. 기본 OFF." style="font-size:9.5px;font-weight:800;padding:3px 9px;border-radius:12px;border:1px solid;cursor:pointer;${_regimeOn?`background:${_regCol};color:#fff;border-color:${_regCol}`:`background:var(--surface2);color:${_regCol};border-color:${_regCol}66`}">⚡ 레짐필터 ${_regimeOn?'ON':'OFF'}</span>`;
  const _regBadge = `<span style="font-size:9.5px;font-weight:700;color:${_regCol}">현재 ${_regKr}${(_regimeOn&&r&&r.active)?` · ${r.n}개`:''}</span>`;
  const _toggleRow = `<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:9px">${_toggle}${_regBadge}</div>`;

  // 비활성: 레짐 ON 표본부족이면 토글 유지한 최소 카드(끄기 가능). 그 외(데이터부족/평탄)는 숨김.
  if(!r || !r.active){
    if(_regimeOn && r && r.regimeOn){
      const _hMin = `<div style="display:flex;justify-content:space-between;align-items:center;gap:6px;flex-wrap:wrap"><div style="display:flex;align-items:center;gap:6px"><span style="font-size:13px;font-weight:800;color:var(--text)">🔮 차트예측</span><span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--surface2);color:var(--text3);border:1px solid var(--border)">실험</span></div><span style="font-size:10px;color:var(--text3);font-weight:700">${_regKr} 표본부족</span></div>`;
      return `<div id="sxCpCardWrap" style="margin:0 0 10px;padding:12px 14px;background:#fff;border:1px solid var(--border);border-radius:12px">${_hMin}${_toggleRow}<div style="font-size:10px;color:var(--text3);line-height:1.6;margin-top:8px;padding:8px 10px;background:var(--surface2);border-radius:8px">⚡ <b style="color:${_regCol}">${_regKr}</b> 레짐과 같은 유사 표본이 ${r.regimeMatched||0}개로 10개 미만이에요. 필터를 끄면 전체 표본으로 표시합니다.</div></div>`;
    }
    return '';
  }
  ...
  const head = `<div style="display:flex;justify-content:space-between;align-items:center;gap:6px;flex-wrap:wrap">
    <div style="display:flex;align-items:center;gap:6px">
      <span style="font-size:13px;font-weight:800;color:var(--text)">🔮 차트예측</span>
      ...
      <span onclick="_sxVib(12);if(typeof SXChart!=='undefined'&&_currentAnalRows)SXChart.openFull(_currentAnalRows,_currentAnalName,_currentAnalTrades,(typeof _resolvePurpleSv==='function'?_resolvePurpleSv(currentAnalStock):(currentAnalStock&&(currentAnalStock._svChartMarker||currentAnalStock._svVerdict))))" title="기존 풀차트(캔들·MA·BB·마커) 열기" style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;background:#7c3aed22;color:#7c3aed;border:1px solid #7c3aed66;cursor:pointer">📈 차트확인</span>
    </div>
    <span style="font-size:10px;color:var(--text3);font-weight:700">유사 ${r.n}개 · 최고 ${r.nearSim}%</span>
  </div>`;
  ...
  const note = `<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px;line-height:1.5">최근 ${r.win}봉 종가형태와 닮은 과거 ${r.n}개 구간의 이후 ${r.h}봉 분포 · 예측 아님 · BT/판정 미반영 · <b style="color:${rows.length>=400?'#16a34a':'#d97706'}">[기준 ${rows.length}봉]</b></div>`;
...
if(typeof window!=='undefined'){ window._cpRegimeToggle=_cpRegimeToggle; window._cpRerenderCard=_cpRerenderCard; }
