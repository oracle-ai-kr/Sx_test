...
async function _deadcatShortBracket(mk, onProgress){
  ...
  var seen={}, list=[];
  rep.concat(watch).concat(aug).concat(dca).forEach(function(s){ if(s&&s.code&&!seen[s.code]){ seen[s.code]=1; list.push(s); } });   // [S826] 대표+관심+보강+top100 합산·코드 중복 자동제거(겹침은 한 번만)
  if(!list.length) return { ok:false, reason:'종목 없음' };
  var CAP=130, use=list.slice(0,CAP);   // [S826] (하,하)/(하,혼조) 표본 확보 위해 45→130
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
...
async function _deadcatTrajBracket(mk, onProgress, pool){
  ...
  var seen={}, list=[];
  rep.concat(watch).concat(aug).concat(dca).forEach(function(s){ if(s&&s.code&&!seen[s.code]){ seen[s.code]=1; list.push(s); } });   // [S827] 대표+관심+보강+top100(=단기쪼개기와 동일 풀)
  if(!list.length) return { ok:false, reason:'종목 없음' };
  var CAP=130, use=list.slice(0,CAP);
  var _tgt=(typeof _btTargetBars==='function')?_btTargetBars(mk,'day'):600, _floor=Math.floor(_tgt*0.95);
...
function _deadcatHzRender(res, mk){
  ...
  var head='<div style="font-size:8.5px;color:'+T3+';line-height:1.5;margin-bottom:7px"><b>역배 데드캣-real 발동 × N10/N15/N20</b>. 셀=진짜비율(<b style="color:'+T2+'">작은수=lift</b>=그 horizon 역배 base 대비). <b>N10은 이미 단조 깨짐</b>(62→61→71) — 긴 horizon서 <b>단조(발동↑=진짜↑) + 4+ lift≥10%p</b> 살아나면 "역배는 느림" 가설 확인. 합본 129개(deadcat-real)·대표+관심+top100·'+(res.stocksUsed||0)+'종목.</div>';
  return head
