...
async function _runEngineVerify(stock){
  if(!stock){
    console.warn('[S114/_runEngineVerify] stock 없음');
    return;
  }
  console.log(`[S114/_runEngineVerify] 엔진판단 검증 시작: ${stock.name||stock.code}`);

  // 현재 탭 저장 (복귀용) — S114 정밀검사 fix: _analTabIdx가 실제 변수 (sx_screener.html 7127줄 전역 선언)
  const _origTab = (typeof _analTabIdx !== 'undefined') ? _analTabIdx : 0;

  try {
    // 단일검증 탭 활성화 (btRunBasic이 탭 내부 DOM — 버튼/진행바/결과 영역 참조)
    if(typeof switchAnalTab === 'function'){
      switchAnalTab(1);
      // DOM 안정화 대기 (탭 전환 직후 렌더링 완료 보장)
      await new Promise(r => setTimeout(r, 50));
    }

    // btRunBasic 호출 (단일검증의 [▶ 백테스트 실행] 버튼과 완전 동일 동작)
    //   - 400봉 → +200봉 확장 (경로 B, 2초 대기 포함)
    //   - BT 실행 → stock._btResult 갱신
    //   - stock._needsAnalRerender = true 설정 (S112-fix2)
    if(typeof btRunBasic === 'function'){
      await btRunBasic();
    } else {
      console.error('[S114/_runEngineVerify] btRunBasic 함수 없음');
    }
  } catch(e) {
    console.error('[S114/_runEngineVerify] 실행 오류:', e);
  } finally {
    // 원래 탭으로 복귀 (분석탭 재진입 → _needsAnalRerender 감지 → 자동 재렌더)
    if(typeof switchAnalTab === 'function'){
      switchAnalTab(_origTab);
    }
    console.log(`[S114/_runEngineVerify] 완료 (탭 ${_origTab} 복귀)`);
  }
}
