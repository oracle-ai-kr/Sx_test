...
async function runAnalysis(stock){
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  try{
    ...
    var _momStale = stock._scoreMomentum && stock._scoreMomentum.history && stock._scoreMomentum.history.length && (stock._scoreMomentum._normV || 0) < 6;
...
const _SXB_PARAM = new Set(['추세방향','추세신호','변동성','가격모멘텀','반등신호','반등강도','눌림목','볼린저%B','크로스신호','추가상승','MTF']);
...
// [S400] 투매 주의 배지 클릭 → 사유 토스트
function _sxbDumpWhy(){
  const B = window._sxBoard;
  if(!B || !B.dumpWarn) return;
  const msg = `⚠️ 투매 주의: 거래대금 급증(전이 ${B.dumpWarn.tv}) + 가격 하락 + OBV 이탈 — 매도 우위 자금이 유입되는 구간입니다. 단순 조정과 달리 수급이 빠지므로 손절 기준을 점검하세요.`;
  if(typeof toast === 'function') toast(msg);
  if(typeof _sxVib === 'function') _sxVib(8);
}
...
function _sxbHTML(){
  ...
  if(B.dumpWarn && B.dumpWarn.on){
    const _DC='#e8365a';
    _dumpBadge = `<span class="sxb-badge" style="color:${_DC};background:${_DC}1A;border:1px solid ${_DC};cursor:pointer" onclick="event.stopPropagation();_sxbDumpWhy && _sxbDumpWhy()">⚠️ 투매 주의</span>`;
  }
    ...
    if(_cdls.length >= 13){
      ...
      if(_avgP > 0){
        ...
        if(_tvScore >= 65 && _priceDown && _obvDown){
          _boardExtras.dumpWarn = { on:true, tv:_tvScore };
        }
      }
    }
      ...
      ${(()=>{
        ...
        ${(()=>{
          ...
          if(_bTot != null && (_vLbl==='매수'||_vLbl==='관심'||_vLbl==='보유 유지') && _bTot < 50){
            html += '<div style="font-size:9px;color:var(--sell);margin-top:6px;line-height:1.5;padding-top:6px;border-top:1px dashed var(--border)">⚠️ 종합점수 '+_bTot+'('+_sxbGrade(_bTot)+'등급) — 종목 상태가 약화 구간입니다. 판정보다 한 단계 신중하게(손절·익절 기준 점검) 접근하세요.</div>';
          } else if(_bTot != null && (_vLbl==='청산 검토'||_vLbl==='즉시 청산'||_vLbl==='회피'||_vLbl==='매도 완료') && _bTot >= 65){
            html += '<div style="font-size:9px;color:var(--buy);margin-top:6px;line-height:1.5;padding-top:6px;border-top:1px dashed var(--border)">ℹ️ 종합점수 '+_bTot+'('+_sxbGrade(_bTot)+'등급) — 상태는 견조한 편이니 과도한 비관은 경계하세요.</div>';
          }
          html += '</div>';
