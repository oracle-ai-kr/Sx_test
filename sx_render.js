...
function _renderAnalTfChips(){
  ...
  tfs.forEach(t => {
    ...
    if(_loading) icon = `<span class="atf-icon" title="상위 TF 로딩 중" style="display:inline-block;animation:spin .8s linear infinite">↻</span>`;
    else if(cacheHit && cacheHit.dist5) icon = `<span class="atf-icon" title="${cacheHit.dist5.label}">${cacheHit.dist5.dot}</span>`;
