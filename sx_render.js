    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        const _aTimingBadge = (_aTimingOn === true)  ? '<span class="tx-badge up">진입 신호</span>'
                            ...
                            : '';
        // [S515] 상단 배지 행: 좌=A 타이밍 / 우=C 판정(하락유력 등). 둘 다 없으면 행 자체 생략.
        const _topBadgeRow = (_aTimingBadge || _verdictBadgeTop)
          ? `<div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;margin-bottom:-2px"><span>${_aTimingBadge}</span><span>${_verdictBadgeTop}</span></div>`
          : '';
