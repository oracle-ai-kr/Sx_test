...
function _candleTransitionScore(rows, indicators){
  ...
  let score = 0; const reasons = []; let warn = null;   // [S482] base 0 — 양방향(-100~+100)으로 가/감점 작용 가능하게
  ...
  const label = score >= 30 ? '반등 유망' : score <= -30 ? '하락 우위' : '중립';
...
function _buildTransitionCard(stock, indicators){
  ...
  const top = `<div style="display:flex;align-items:baseline;gap:8px"><span style="font-size:26px;font-weight:800;color:${col}">${sign}${r.score}</span><span style="font-size:13px;font-weight:700;color:${col}">${r.label}</span><span style="font-size:10px;color:var(--text3);margin-left:auto">음봉 ${r.redCnt}연속</span></div>`;
  ...
  const note = `<div style="font-size:9px;color:var(--text3);margin-top:9px;border-top:1px solid var(--border);padding-top:6px">실험 지표 · 정식 판정과 무관 · −100~+100 (양수=반등 우위, 음수=하락 우위)</div>`;
