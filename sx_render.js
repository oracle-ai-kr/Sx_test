...
function _sxbSafetyWhy(){
  const B = window._sxBoard;
  if(!B || !B.verdict || !B.verdict.capReason) return;
  const viol = (B.verdict.safetyViol && B.verdict.safetyViol.length) ? B.verdict.safetyViol : String(B.verdict.capReason).split(' · ');
  const lines = viol.map(v => `• ${String(v).replace(/^🔒/, '')}`).join('\n');
  const msg = `🔒 안전필터감지 — 진입보류 (${viol.length}건)\n${lines}\n\n기술적 매수 신호가 있어도 위험 징후가 감지돼 진입을 보류합니다. 설정 탭에서 개별 안전필터를 끌 수 있습니다.`;
  if(typeof toast === 'function') toast(msg);
  if(typeof _sxVib === 'function') _sxVib(8);
}
...
function _sxbHTML(){
  ...
  // [S434] 안전필터 감지 배지 — 비보유 안전필터 강등(verdict.capReason 존재 = 진입보류). 보라색, 클릭 시 위반 사유 토스트.
  let _safetyBadge = '';
  if(B.verdict && B.verdict.capReason){
    const _SC='#a855f7';
    _safetyBadge = `<span class="sxb-badge" style="color:${_SC};background:${_SC}1A;border:1px solid ${_SC};cursor:pointer" onclick="event.stopPropagation();_sxbSafetyWhy && _sxbSafetyWhy()">🔒 안전필터감지</span>`;
  }
  ...
  let h = `<div class="sxb-head" onclick="_sxVib(8);this.parentElement.classList.toggle('sxb-open')">`
    ...
    + `<div class="sxb-head-badges">${transBadge}${whipBadge}${_dumpBadge}${_safetyBadge}</div>`
  ...
  body.innerHTML = `
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        // [S450] 분석신호검토 버튼 = A(진입타이밍 = scrQuickScore.score) 전용. 라벨·색 모두 A 점수 기준.
        ...
        return `${_verdictBadgeTop ? `<div style="display:flex;justify-content:flex-start;margin-top:8px;margin-bottom:-2px">${_verdictBadgeTop}</div>` : ''}<div style="display:flex;align-items:center;gap:6px;margin-top:6px">
          <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1.5px solid ${_timingBtnClr};cursor:pointer" onclick="_sxVib(10);var el=document.getElementById('${_3stageId}'),s=document.getElementById('${_stratDetailId}');if(s&&s.style.display!=='none'){s.style.display='none';if(currentAnalStock)currentAnalStock._engineVerifyOpen=false;}var willOpen=el.style.display==='none';el.style.display=willOpen?'block':'none';if(currentAnalStock){currentAnalStock._3stageOpen=willOpen;}">
            <span style="font-size:9px;color:${_timingBtnClr};font-weight:700"><span style="display:inline-block;width:13px;height:13px;line-height:13px;text-align:center;border-radius:50%;background:#3b82f6;color:#fff;font-size:8px;font-weight:800;margin-right:3px">A</span>분석신호 검토 <span style="font-size:7px">▶</span></span>
          ...
          <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1.5px solid ${_entryEvalClr};cursor:pointer" onclick="_sxVib(10);var el=document.getElementById('${_stratDetailId}'),t=document.getElementById('${_3stageId}');if(t&&t.style.display!=='none'){t.style.display='none';if(currentAnalStock)currentAnalStock._3stageOpen=false;}var willOpen=el.style.display==='none';el.style.display=willOpen?'block':'none';if(currentAnalStock){currentAnalStock._engineVerifyOpen=willOpen;}/* S119 fix2: 자동 실행 완료 후 재클릭 시 재실행 방지 — 이미 BT 결과 있으면 토글만 */if(willOpen&&currentAnalStock&&!currentAnalStock._btResult)_runEngineVerify(currentAnalStock);">
            <span style="font-size:9px;color:${_entryEvalClr};font-weight:700"><span style="display:inline-block;width:13px;height:13px;line-height:13px;text-align:center;border-radius:50%;background:#22c55e;color:#fff;font-size:8px;font-weight:800;margin-right:3px">B</span>엔진판단 검증 <span style="font-size:7px">▶</span></span>
