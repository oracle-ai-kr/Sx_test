...
// [v3.13 ENV-BADGE] 시장 환경 배지 추가 — 결과 해석 시 시장 흐름 컨텍스트 제공
//   배경: 내 필터 탭에는 envBadge 있지만 결과 탭 진입 시 시장 흐름 정보 없음
//        예: "체결강도 100%↑ 208종목"이 약세장 결과인지 강세장 결과인지 판단 어려움
//   동작: MarketEnv.getSummary 호출하여 state/indices/ageStr 표시 (조건검색 envBadge와 동일)
function _buildResultHeaderHTML(cntText){
  let envHTML = '';
  try{
    if(typeof MarketEnv !== 'undefined' && typeof MarketEnv.getSummary === 'function'){
      const _es = MarketEnv.getSummary(currentMarket);
      if(_es && _es.state !== 'unknown'){
        const stateColor = _es.state.includes('bull') ? '#dc2626' : _es.state.includes('bear') ? '#2563eb' : 'var(--text2)';
        const stateDot = _es.state.includes('bull') ? '🔴' : _es.state.includes('bear') ? '🔵' : '⚪';
        envHTML = `<div class="result-env-badge" style="display:flex;align-items:center;gap:6px;font-size:10px;color:var(--text2);padding:4px 12px;background:var(--surface2);border-bottom:1px solid var(--border);margin:0 -12px 8px">
          <span style="color:${stateColor};font-weight:700">${stateDot} ${_es.stateLabel}</span>
...
async function runAnalysis(stock){
 try{ // S63: 전체 try-catch — 에러 시 무한로딩 방지
  ...
  try {
    ...
    const _targetBars = (_tfLocal === 'week' || _tfLocal === 'month') ? 400 : 600;
