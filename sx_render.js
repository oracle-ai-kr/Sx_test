...
function renderAnalysisResult(stock, scores, indicators, qs, analTime, sectorItp, maAlignItp, basicItp){
  ...
  if(_btSt && _btSt.state !== 'no_data'){
    ...
    if(_btSt.state === 'holding'){
      if(_btSt._isBuySignal){
        ...
        // 보유중 — 수익=주황, 손실=빨강
        _bnBg = _bnPnlPos ? 'rgba(255,153,0,.08)' : 'rgba(232,54,90,.08)';
        _bnBorder = _bnPnlPos ? '#ff9900' : '#e8365a';
        _bnIconClr = _bnPnlPos ? '#ff9900' : '#e8365a';
        _bnTextClr = _bnPnlPos ? '#ff9900' : '#e8365a';
      }
    } else if(_btSt.state === 'sell_signal'){
      // 매도 신호 — 파랑
      _bnBg = 'rgba(59,130,246,.08)';
      _bnBorder = '#3b82f6';
      _bnIconClr = '#3b82f6';
      _bnTextClr = '#3b82f6';
    } else {
    ...
    const _bnPulse = _btSt.state === 'holding'; // 매수신호+보유중 = 펄스
  ...
  if(_btTrades && _btTrades.length){
    ...
    _recent.forEach((tr, idx) => {
      ...
      const row = `<div style="padding:5px 0;border-bottom:1px solid var(--border)">
    ...
    <div class="anal-header-card">
      ...
      ${(()=>{
        ...
        return `<div style="display:flex;align-items:center;gap:6px;margin-top:6px">
          <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:4px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1px solid var(--border);cursor:pointer" onclick="var el=document.getElementById('${_3stageId}');el.style.display=el.style.display==='none'?'block':'none'">
            <span style="font-size:9px;color:var(--text3)">진입타이밍</span>
            <span style="font-size:13px;font-weight:800;color:var(--${_analCls})">${_analSc}${_momArrow}</span>
            <span style="font-size:7px;color:var(--accent)">▶</span>
          </div>
          <div style="flex:1;display:flex;align-items:center;justify-content:center;gap:4px;padding:4px 10px;border-radius:6px;background:var(--surface2);border:1px solid var(--border)">
            <span style="font-size:9px;color:var(--text3)">매매전략</span>
