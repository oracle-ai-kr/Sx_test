...
function _candleTransitionScore(rows, indicators, market, tf){
  ...
  if(mode === 'count'){
    ...
    return { active:true, mode:'count', score, label, reasons, warn:null, redCnt, greenCnt, streak:0, red8:_red8, green8:_green8, counter8:_counter8, asOf: rows[last].date, nextBar:_nextBar, sbFired:_sbFired };
  }
  ...
  if(mode === 'flipUp' || mode === 'flipDown'){
    ...
    score = Math.max(-100, Math.min(100, Math.round(score)));
    const label = _ctLabel(); // [S539] 단일봉 신호 시 ±20 약확신
    return { active:true, mode, score, label, reasons, warn, redCnt, greenCnt, streak:0, patName: pat.name, red8:_red8, green8:_green8, counter8:_counter8, asOf: rows[last].date, nextBar:_nextBar, sbFired:_sbFired };
  }
  ...
  score = Math.max(-100, Math.min(100, Math.round(score)));
  const label = _ctLabel(); // [S539] 단일봉 신호 시 ±20 약확신
  return { active:true, mode, score, label, reasons, warn, redCnt, greenCnt, streak, red8:_red8, green8:_green8, counter8:_counter8, asOf: rows[last].date, nextBar:_nextBar, sbFired:_sbFired };
}
...
function _ctConfidence(score, market, sbFired){
  var mk = _ctNormMkt(market), a = Math.abs(score);
  // [S541] 3시장 바스켓 실측(S540 시점·≥20 포함) 재조정. 카드가 방향 잡는 구간만 표시: |s|≥30(유망) 또는 단일봉+|s|≥20(약확신).
  if(a < 30 && !(sbFired && a >= 20)) return { label:'예측 임계 미만', color:'#6b7280', why:'방향 예측 보류 구간 (|점수|<30, 단일봉 신호도 없음)' };
  if(mk === 'kr')   return { label:'예측력 약함 (KR)', color:'#6b7280', why:'KR 바스켓은 전 임계에서 baseline 미달 — 이 시장은 다음봉 색 예측이 잘 통하지 않음 (차후 전용 로직 예정)' };
  if(score > 0)     return { label:'신뢰 낮음 (양봉)', color:'#dc2626', why:'양봉 전이 예측은 3시장 바스켓에서 대체로 baseline 미달 — 상승 전이는 노이즈가 커 신뢰 낮음' };
  // 음봉 (coin/us)
  if(mk === 'coin'){
    if(a >= 30) return { label:'신뢰 높음 ⭐', color:'#16a34a', why:'코인 음봉·30↑ = 바스켓 +6~11%p · 상회 9~11/12 (45·50도 유지, 과열 절벽 없음)' };
    return { label:'신뢰 보통 (단일봉)', color:'#f59e0b', why:'코인 음봉·20~30 = +6.3%p · 상회 10/12 — 단일봉 신호 기반 약확신' };
  }
  // us 음봉
  if(a >= 30) return { label:'신뢰 높음', color:'#16a34a', why:'US 음봉·30↑ = +5~8%p · 상회 10/12 (정점 ≈40)' };
  return { label:'신뢰 보통 (약)', color:'#f59e0b', why:'US 음봉·20~30 = +1.3%p · 상회 5/12 — 약함, 보강 필요' };
}
...
function _buildTransitionCard(stock, indicators){
  ...
  const _conf = _ctConfidence(r.score, _mktCT, r.sbFired);
  ...
  // [S525] 미니 백테스트 버튼 — 과거 데이터로 이 예측의 적중률을 검증 (sx_candle_bt.js 모달)
  const btnBT = `<div style="margin-top:9px;text-align:center"><button onclick="window.SXCandleBT&&SXCandleBT.open()" style="font-size:11px;font-weight:700;color:#7c3aed;background:#7c3aed12;border:1px solid #7c3aed44;border-radius:8px;padding:7px 14px;cursor:pointer">🧪 미니 백테스트 — 적중률 검증 ▶</button></div>`;
  ...
  return wrap(top + confHtml + bar + _seqRow + _hitRow + warnHtml + reasonsHtml + note + btnBT);
}
