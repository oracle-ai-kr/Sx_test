...
function mtRender(){
  ...
  if(closed.length >= 1){
    ...
    // [S316-fix4] 표시용 거래수 = closed.length (FLAT 포함) — 청산한 모든 거래
    //   배경: 기존 `total = wins + losses`는 FLAT 제외 → "총매매수"가 청산 1건(FLAT)일 때 0으로 표기
    //         메인 카드의 "총 거래" 카운트와 자산 흐름의 청산 N건 기준과 불일치
    //   변경: UI 표시는 closedCount (FLAT 포함), 승률·기댓값 분모는 total (FLAT 제외) 유지
    //         → 승률 의미 보존(FLAT은 진정한 승도 패도 아님) + 표시 일관성 확보
