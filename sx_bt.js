...
//   기존 인덱스 기반 판정(entryIdx >= rowsLen - 2)은 어제 진입까지 잡아버리는 버그.
//   해결: 봉 날짜를 KST 오늘 날짜와 직접 비교 → nextBar 모드/봉 인덱스 무관하게 정확.
...
function btGetCurrentState(btResult, currentPrice){
  ...
  if(lastTrade.type === 'OPEN'){
    ...
    //   기존 버그: entryIdx >= _rowsLen - 2 → 어제 진입(rowsLen-2)도 "방금 신호"로 잘못 잡힘
    //   예: SK스퀘어 4/29 진입(어제) → bars=1 → entryIdx=rowsLen-2 → 잘못 true
    //       기업은행 4/28 진입 → bars=2 → entryIdx=rowsLen-3 → 정상 false
    //   수정: 진입일이 "오늘 KST"인지 날짜로 직접 판정 (timezone-safe, nextBar 모드 무관)
...
function btGetOpts(){
  ...
  //   배경: 기존에는 BT가 슬롯 base 값만 사용 → 실시간 분석(scrQuickScore)은 봉별 보정 적용 →
  //         차트의 매매 마크가 분석탭 현재 판정과 어긋나는 비대칭 발생.
  //   해결: BT도 동일하게 봉별 보정 적용 → 차트 미러링 = 실시간 시뮬레이션으로 사용 가능.
  //   조건: SXE.regimeAdaptEnabled() (사용자 토글) 기준 → OFF면 보정 없음 (기존 동작 유지).
  //   sx_bt.js 모든 경로(단일검증/관심종목 BT/교차검증/워크포워드/대시보드)에 일괄 적용.
