...
async function _runBtWithExtension(stock, tf, quiet){
  try{
    ...
    // 경로 C: 캐시 없음 → 새로 3단계 (200 → 400 → 600)
      ...
      if(_targetCount > 600 && rows.length >= 600){
        await _btSleep(2000);
        try{
...
function btRenderBasicResult(stock, r){
  ...
  //     - #f472b6 핑크: 보조 긍정 (손익비 — 항상 고정, 정보 톤)
    ...
    html += `<div class="bt-card">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px">
        ...
        <div style="background:var(--surface2);padding:8px 10px;border-radius:6px">
          ...
          <div style="font-weight:700;font-size:13px;color:${_realizedColor}">${_realizedDelta>=0?'+':''}${_fmtKrw(_realizedDelta)}원</div>
        ...
        ${_hasOpen ? `<div style="background:var(--surface2);padding:8px 10px;border-radius:6px">
          ...
          <div style="font-weight:700;font-size:13px;color:${_openColor}">${_openDelta>=0?'+':''}${_fmtKrw(_openDelta)}원</div>
        ...
        <div style="background:var(--surface2);padding:8px 10px;border-radius:6px;${_hasOpen?'':'grid-column:span 2;'}border:1.5px solid ${_totalColor}">
          <div style="color:var(--text3);font-size:9px;margin-bottom:3px">현재 Equity <span style="opacity:0.7">(총수익률 · 복리)</span></div>
          <div style="font-weight:800;font-size:14px;color:${_totalColor}">${_fmtKrw(_equityFinal)}원</div>
  ...
  if(r.trades && r.trades.length){
    ...
    for(let i = _startIdx; i >= _endIdx; i--){
      ...
      const _dateLine = _entryD
        ...
        : '';
      // [S240] 거래별 잔고 — 수익률 아래에 작게 표시 (시간순 누적, 사전 계산된 _balances[i] 사용)
      //   OPEN 포지션은 평가 잔고(미실현 포함), 청산은 실현 잔고
      const _balAtI = _balances[i];
      const _balLine = (typeof _balAtI === 'number')
        ? `<span style="font-size:9px;color:var(--text3);font-weight:600;line-height:1;margin-top:2px">${_fmtKrw(_balAtI)}원</span>`
        : '';
