// ════════════════════════════════════════════════════════════
//  SIGNAL X — BT Module v1.11-fix6 <!-- S112-fix2 (분석탭 자동 재렌더): btRunBasic 완료 시 stock._needsAnalRerender=true 플래그 설정. 배경: S112-fix1로 _lastAnalCandles는 600봉으로 업데이트되지만 분석탭 화면은 이전 렌더 상태(400봉, 21회) 그대로 → 단일검증(28회)과 화면상 불일치. 해결: 플래그 + sx_screener.html switchAnalTab(0) 전환 시 자동 runAnalysis 재실행 (캐시 재사용으로 즉시). 진단 결과 정합성은 이미 완벽(진단탭 28회 확인), 이건 순수 UI 동기화 버그. 채희창 콘솔 로그 스크린샷으로 완벽 진단. --> v1.11-fix5 <!-- S112-fix1 (양방향 공유 완성): btRunBasic 경로 B _lastAnalCandles 저장 누락 버그 수정. --> v1.11-fix4 <!-- S110 fix4 (봉수 투명성): rowsLength 필드 + 🟢/🔵/🔴 봉수 배지 + 교차검증 ☆ 관심종목만. --> v1.11-fix3 <!-- S110 fix3: 0.5초 스태거 + 분석탭 로그 초기화 + ☆ 관심종목만 --> v1.11-fix <!-- S110 fix: 리스트 갱신 + ☆ 마크 --> v1.11 <!-- S110 Phase C+D 통합 --> v1.10 <!-- 매매이력 날짜 --> v1.9 <!-- BT 600봉 --> v1.8
