// ════════════════════════════════════════════════════════════
//  SIGNAL X — BT Module v1.11-fix4 <!-- S110 fix4 (봉수 투명성, 채희창 피드백): 실제 사용된 봉수를 🟢/🔵/🔴 동그라미 이모지로 표시. (1) _runBtWithExtension/btRunBasic 반환 결과에 rowsLength 포함. (2) _watchBtSet 캐시에도 rowsLength 저장 (기존 캐시는 다음 갱신 시 반영). (3) 교차검증 결과 테이블 — 매매 열 옆에 봉수 배지 (🟢 600봉 이상 / 🔵 400봉 이상 / 🔴 200봉 이하). 결과 테이블 위에 범례 한 줄 표시. (4) 대시보드 — 상단 요약에 봉수 분포 (🟢 N개 · 🔵 N개 · 🔴 N개). (5) 배경: 관심 등록 시 분석탭의 200봉 캐시를 활용하여 _runBtWithExtension 경로 B로 진입 → 한 번만 추가 확장(+200) → 400봉 상한. 해외 종목은 경로 D (300봉). 네이버/Upbit 봇 감지 방지를 위해 반복 확장 호출은 안 하고 대신 "실제 사용 봉수"를 투명하게 표시하여 사용자가 신뢰도를 판단할 수 있게 함. --> v1.11-fix3 <!-- S110 fix3: 0.5초 스태거 + 분석탭 로그 초기화 + ☆ 관심종목만 --> v1.11-fix <!-- S110 fix: 리스트 갱신 + ☆ 마크 --> v1.11 <!-- S110 Phase C+D 통합 --> v1.10 <!-- 매매이력 날짜 --> v1.9 <!-- BT 600봉 --> v1.8
