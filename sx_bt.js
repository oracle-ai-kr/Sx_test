...
// S161: 캐시 미스 이유 확인 (통계용 — 교차검증 재계산 안내에 사용)
//   반환: 'hit' | 'no_cache' | 'tf_mismatch' | 'expired' | 'gate_changed'
function _watchBtGetReason(market, code, tf){
  const all = _watchBtLoadAll();
  const key = _watchBtKey(market, code);
  const entry = all[key];
...
async function btRunCross(){
  ...
  const _cacheMissReasons = { no_cache:0, tf_mismatch:0, expired:0, gate_changed:0 };
  ...
  for(const t of targets){
    ...
    let missReason = 'hit';
    try{
      cached = _watchBtGet(t.market, t.code, _tf);
      if(!cached) missReason = _watchBtGetReason(t.market, t.code, _tf);
  ...
  btRenderCrossResult(results, _cacheMissReasons);
  _btSaveBtCross(results);
  // 대상 리스트 재갱신 (캐시 상태 반영)
  btCrossRefreshTargetList();
}

function btRenderCrossResult(results, cacheMissReasons){
  const result = document.getElementById('btCrossResult');
  result.style.display='block';
  ...
  // S161: 게이트 재계산/활성 안내 (결과 카드 직후)
  html += gateInfoHtml + gateSummaryHtml;
