// ════════════════════════════════════════════════════════════
//  SIGNAL X — BT Module v1.11 <!-- S110 Phase C+D 통합 — 관심종목 기반 교차/대시보드 + 자동 BT 캐싱: (1) 관심 BT 캐시 인프라: SX_WATCH_BT_CACHE 키 (시장_코드 복합키, 7일 TTL). _watchBtKey/LoadAll/SaveAll/Get/Set/Delete 6개 헬퍼. (2) _runBtWithExtension 헬퍼: btRunBasic 4경로 확장 로직 추출 — 백그라운드/교차/대시보드 공용, UI 없음, 반환 {ok, result, rows} or {ok:false, error}. (3) _autoBtOnWatchlistAdd: 관심 ☆ 등록 시 백그라운드 자동 600봉 BT (silent), 중복 방지, 최근 캐시 있으면 skip. toggleWatchlist/removeWlItem 연결. (4) 교차검증 재설계 btRunCross: searchResults 기반 → _btGetCrossTargets (관심+현재분석, 중복제거). 대상 0개 → 버튼 비활성 + 안내, 1개 → 비활성(무의미), 2개+ → 실행 가능. 캐시 히트 종목 즉시 활용, 미스만 _runBtWithExtension 실행 후 캐시 저장. (5) 대시보드도 동일 패턴: btRunDashboard + btDashRefreshTargetList. 워크포워드는 첫 대상 종목에 대해 새 BT 실행. (6) 대상 리스트 UI: 각 종목 옆에 ✓캐시(N일 전) 또는 ⟲새 BT 배지. 총 N종목 · 캐시 X개 · 새로 Y개 요약. 예상 시간(Y*6초). (7) 단일BT 실행 시 관심종목이면 캐시도 자동 갱신 (일봉 조건). (8) 버튼 텍스트 "검증결과누적" → "검증결과갱신". switchAnalTab에서 교차/대시보드 진입 시 자동 리스트 갱신 훅. --> v1.10 <!-- S110 단일검증 매매목록에 진입~청산 날짜 표시 --> v1.9 <!-- S109 Phase 3-B-9b 단일검증 + 학습검증 자동 3단계 확장 (200 → 400 → 600봉) --> v1.8 <!-- S103-fix7 Phase3-A-1 _isBuySignal/sell_signal 판정 기준 교정 --> v1.7 <!-- S103-fix5 -->
...
function _watchBtSet(stock, btResult, tf){
  ...
  _watchBtSaveAll(all);
  console.log(`[S110-watchBt] ✅ 캐시 저장: ${stock.name||stock.code} (${tf||_btTF()}, 거래 ${btResult.totalTrades})`);
}
...
function btCrossRefreshTargetList(){
  ...
  const cachedList = targets.map(t => {
    ...
    return `<div style="display:flex;align-items:center;gap:6px;padding:4px 0">
      <span style="flex:1">${t.name}</span>${badge}
    </div>`;
  }).join('');
  ...
  listEl.innerHTML = `<div style="padding:8px 10px;background:var(--surface2);border-radius:6px">
    <div style="font-size:10px;color:var(--text3);margin-bottom:6px">
      ...
      ${newCount > 0 ? `<span style="color:var(--accent)"> (${newCount * 6}초 예상)</span>` : `<span style="color:var(--buy)"> (즉시)</span>`}
    </div>
    ${cachedList}
  </div>`;
}
...
async function btRunCross(){
  ...
  for(let i=0;i<targets.length;i++){
    const t = targets[i];
    progFill.style.width = `${Math.round((i/targets.length)*100)}%`;
    ...
    if(cached && cached.btResult){
      // 캐시 활용 — 즉시
      progText.textContent = `${i+1}/${targets.length} — ${t.name} (캐시)`;
      ...
      console.log(`[S110-cross] ✅ 캐시 활용: ${t.name}`);
      await _btSleep(30);
    } else {
      // 캐시 미스 → 새로 BT (600봉 확장)
      progText.textContent = `${i+1}/${targets.length} — ${t.name} (BT 중, 6초 예상)`;
      console.log(`[S110-cross] ⟲ 새 BT: ${t.name}`);
      const btOut = await _runBtWithExtension(t, _tf, false);
...
function btDashRefreshTargetList(){
  ...
  listEl.innerHTML = `<div style="padding:8px 10px;background:var(--surface2);border-radius:6px">
    <div style="font-size:10px;color:var(--text3);margin-bottom:6px">
      <b>대상 ${n}종목</b> · 캐시 ${cachedCount}개 · 새로 실행 ${newCount}개
      ${newCount > 0 ? `<span style="color:var(--accent)"> (${newCount * 6 + 3}초 예상)</span>` : `<span style="color:var(--buy)"> (즉시 + 워크포워드)</span>`}
    </div>
    <div style="font-size:10px;color:var(--text3)">워크포워드는 ${targets[0].name}로 실행</div>
  </div>`;
}
...
async function btRunDashboard(){
  ...
  for(let i=0;i<targets.length;i++){
    const t = targets[i];
    progFill.style.width = `${10+Math.round((i/targets.length)*40)}%`;

    let cached = null;
    try{ cached = _watchBtGet(t.market, t.code, _tf); }catch(e){}

    if(cached && cached.btResult){
      progText.textContent = `교차 ${i+1}/${targets.length} — ${t.name} (캐시)`;
      const r = cached.btResult;
      crossResults.push({
        name:t.name, code:t.code,
        winRate:r.winRate, profitFactor:r.profitFactor, totalPnl:r.totalPnl,
      ...
      progText.textContent = `교차 ${i+1}/${targets.length} — ${t.name} (BT 중)`;
      const btOut = await _runBtWithExtension(t, _tf, false);
      if(btOut.ok && btOut.result && btOut.result.totalTrades > 0){
        const r = btOut.result;
        crossResults.push({
