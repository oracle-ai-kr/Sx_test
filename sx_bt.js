...
async function _runBtWithExtension(stock, tf, quiet){
  try{
    ...
    // 경로 C: 캐시 없음 → 새로 3단계 (200 → 400 → 600)
      ...
      // [S293] >= 400 → < _targetCount: Yahoo 부분 반환(197봉 등)으로 397봉 시 조건 실패 버그 수정
      if(_targetCount > 400 && rows.length < _targetCount){
        await _btSleep(2000);
        try{
      ...
      // [S217] 4단계: 700봉 (KIS 활성 시) — 분석탭과 정합
      // [S293] >= 600 → < _targetCount 동일 수정
      if(_targetCount > 600 && rows.length < _targetCount){
        await _btSleep(2000);
        try{
          const _extra3 = await fetchCandlesExtended(stock.code, _tf, rows[0].date || rows[0].t, _targetCount - rows.length);
          if(_extra3 && _extra3.length > 0){
            rows = [..._extra3, ...rows];
            console.log(`[S110-runBt][S217] ${stock.name||stock.code} 4단계 완료: +${_extra3.length}봉 → ${rows.length}봉 (KIS 700봉)`);
          } else {
            console.warn(`[S110-runBt][S217] ${stock.name||stock.code} 4단계 실패 — 600봉으로 진행`);
          }
        }catch(e){ console.warn(`[S110-runBt][S217] ${stock.name||stock.code} 4단계 예외: ${e.message||e}`); }
      }

      // stock에 확장 캐시 보존 (다음 재사용용)
      stock._lastAnalCandles = rows.slice();
      // [S217+B3] _analCandlesExtendedStage 4단계: 0/1/2/3 (200/400/600/700봉)
      //   stage=3 = KIS 700봉 모드 (KIS 활성 시 한국 시장 7개월 추가 fetch)
      //   동일 패턴 6곳에 반복 적용 (364/479/517/1150/1217/2243행 + 1578행 단순 복사)
      //   〔이력〕 이전: 0/1/2만 정의 → KIS 700봉 달성해도 stage=2로 표시 (수정됨)
...
async function _fetchExtCandles(stock, tf, quiet){
  try{
    ...
    // 경로 C: 캐시 없음 → 새로 3단계 확장 (200 → 400 → 600)
      ...
      const _first = await btFetchCandles(stock.code, _isCoin, _tf, 200);
      if(!_first || _first.length === 0) return { ok:false, error:'초기 데이터 로드 실패' };
      // [S251] 형식 통일 — btFetchCandles는 _btNormalizeRows로 BT 형식({t,o,h,l,c,v}) 반환,
      //        fetchCandlesExtended는 분석탭 형식({date,open,...,close}) 반환 → 혼합 시 옵티마이저 캐시 손상.
      //        〔이력〕 옵티마이저 BT 거래 6건 vs 분석탭 BT 18건 차이의 근본 원인.
      //               idx 400~599(=_first 200봉)의 r.close=undefined → sxRunBtEngine 청산 비교 영구 false → OPEN.
      //        해결: _first를 분석탭 형식으로 변환해서 _extra1/_extra2와 형식 통일.
      //        영향 범위: 옵티마이저 _optPreloadCandles만 경로 C 사용 (분석탭은 stock._lastAnalCandles로 경로 B).
      rows = _first.map(r => ({
        date: r.date || r.t || '',
        open: typeof r.open === 'number' ? r.open : (typeof r.o === 'number' ? r.o : 0),
        high: typeof r.high === 'number' ? r.high : (typeof r.h === 'number' ? r.h : 0),
        low: typeof r.low === 'number' ? r.low : (typeof r.l === 'number' ? r.l : 0),
        close: typeof r.close === 'number' ? r.close : (typeof r.c === 'number' ? r.c : 0),
        volume: typeof r.volume === 'number' ? r.volume : (typeof r.v === 'number' ? r.v : 0)
      }));
      if(!quiet) console.log(`[S251] _first 형식 통일: ${_first.length}봉 (BT형식 → 분석탭형식)`);
      ...
      // [S293] >= 400 조건 → < _targetCount 로 변경: Yahoo가 197봉 등 부분 반환 시 397봉으로
      //        기존 조건 실패하던 버그 수정 (해외 일봉 397봉에서 멈추는 문제)
      if(_targetCount > 400 && rows.length < _targetCount){
        await _btSleep(2000);
        try{
          const _extra2 = await fetchCandlesExtended(stock.code, _tf, rows[0].date || rows[0].t, 200);
      ...
      if(_targetCount > 600 && rows.length < _targetCount){
        ...
        try{
          const _extra3 = await fetchCandlesExtended(stock.code, _tf, rows[0].date || rows[0].t, _targetCount - rows.length);
          if(_extra3 && _extra3.length > 0) rows = [..._extra3, ...rows];
        }catch(e){}
      }
...
function btGetOpts(){
  ...
  //   〔이력〕 이전: BT가 슬롯 base 값만 사용 → 실시간 분석(scrQuickScore)은 봉별 보정 적용 →
  //     차트의 매매 마크가 분석탭 현재 판정과 어긋나는 비대칭 발생 (수정됨)
  return { slippage:slip, nextBarEntry:nextBar, applyRegimeAdjust:true };
}
...
async function btRunBasic(){
  ...
  try{
    ...
    const _canReuse = stock._btResult
      ...
      && stock._btResultOpts.nextBarEntry === _curOpts.nextBarEntry
    ...
    // 경로 C: 캐시 없음 + 시장 지원 → 새로 3단계 확장
      ...
      // [S293] >= 400 → < _targetCount: Yahoo 부분 반환 버그 수정
      if(_targetCount > 400 && rows.length < _targetCount){
        // 3단계: 600봉
      ...
      // [S293] >= 600 → < _targetCount 동일 수정
      if(_targetCount > 600 && rows.length < _targetCount){
        progFill.style.width='65%'; progText.textContent=`4단계: 700봉 확장 중... (2초 대기)`;
        await _btSleep(2000);
        try{
          const _oldestDate3 = rows[0].date;
          const _extra3 = await fetchCandlesExtended(stock.code, _btTFVal, _oldestDate3, _targetCount - rows.length);
          if(_extra3 && _extra3.length > 0){
            rows = [..._extra3, ...rows];
            console.log(`[S109-9b][S217] 4단계 완료: ${rows.length}봉 (KIS 700봉)`);
          } else {
            console.warn(`[S109-9b][S217] 4단계 실패 — ${rows.length}봉으로 진행`);
          }
        }catch(e3){ console.warn('[S109-9b][S217] 4단계 예외:', e3); }
      }
...
async function btRunWf(){
  ...
  try{
    ...
    // 경로 C: 캐시 없음 → 새로 3단계 확장
      ...
      // [S217] 4단계: 700봉 (KIS 활성 시)
      // [S293] >= 600 → < _targetCount 동일 수정
      if(_targetCount > 600 && rows.length < _targetCount){
        progFill.style.width='55%'; progText.textContent=`4단계: 700봉 확장 중... (2초 대기)`;
        await _btSleep(2000);
        try{
          const _extra3 = await fetchCandlesExtended(stock.code, _btTFVal, rows[0].date, _targetCount - rows.length);
          if(_extra3 && _extra3.length > 0){
            rows = [..._extra3, ...rows];
            console.log(`[S109-9b][S217] 4단계 완료: ${rows.length}봉 (KIS 700봉)`);
          }
...
function mtRender(){
  ...
  // [S316] 자세히 보기 — 단일검증탭 스타일 9개 통계 + 자산 흐름
  //   〔조건〕 CLOSED ≥ 1건일 때만 표시 (OPEN만 있으면 통계 의미 없음)
  ...
  if(closed.length >= 1){
    ...
    const winColor = hitRate >= 60 ? COLOR_POS : hitRate >= 40 ? COLOR_BLUE : hitRate >= 20 ? COLOR_ORANGE : COLOR_NEG;
    const pfColor = profitFactor >= 2.0 ? COLOR_POS : profitFactor >= 1.5 ? COLOR_BLUE : profitFactor >= 1.0 ? COLOR_ORANGE : COLOR_NEG;
    ...
    let tradesColor;
    if(total < 10) tradesColor = COLOR_NEG;
    else if(total < 30) tradesColor = COLOR_BLUE;
    else tradesColor = COLOR_POS;
    // 자산 흐름 — 가상 초기자본 100만원 (cumMul 재활용, 위에서 이미 계산됨)
    const INIT_CAP = 1_000_000;
    const finalEq = INIT_CAP * cumMul;
    const realizedDelta = finalEq - INIT_CAP;
    const realizedColor = totalPnl >= 0 ? COLOR_POS : COLOR_NEG;
    const fmtKrw = (v) => Math.round(v).toLocaleString();
    const pfDisplay = profitFactor >= 99 ? '∞' : profitFactor;
    // 토글 헤더 + 본문 (흰 바탕 · 텍스트 간격만으로 구분 — mt-total-summary 톤 통일)
    const _arrow = _mtStatsOpen ? '▼' : '▶';
    const _bodyStyle = _mtStatsOpen ? '' : 'display:none';
    detailHtml = `<div style="margin-bottom:8px">
      <div onclick="mtToggleStats()" style="padding:8px 4px;cursor:pointer;display:flex;align-items:center;gap:6px;font-size:12px;font-weight:700;color:var(--text);border-top:1px solid var(--border)">
        <span style="font-size:10px">${_arrow}</span>
        <span>📊 자세히 보기</span>
        <span style="margin-left:auto;font-size:10px;font-weight:500;color:var(--text3)">청산 ${closed.length}건 기준</span>
      </div>
      <div style="${_bodyStyle};padding:8px 0 4px">
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px 6px">
          <div class="mt-total-item"><div class="mt-total-num" style="color:${winColor}">${hitRate}%</div><div class="mt-total-lbl">승률</div></div>
          <div class="mt-total-item"><div class="mt-total-num" style="color:${pfColor}">${pfDisplay}</div><div class="mt-total-lbl">손익비(PF)</div></div>
          ...
          <div class="mt-total-item"><div class="mt-total-num" style="color:${tradesColor}">${total}</div><div class="mt-total-lbl">총매매수</div></div>
          <div class="mt-total-item"><div class="mt-total-num" style="color:${COLOR_POS}">+${avgWin}%</div><div class="mt-total-lbl">평균이익</div></div>
          <div class="mt-total-item"><div class="mt-total-num" style="color:${COLOR_NEG}">-${avgLoss}%</div><div class="mt-total-lbl">평균손실</div></div>
        ...
        <div style="margin-top:14px;padding-top:10px;border-top:1px solid var(--border)">
          <div style="font-size:11px;font-weight:700;color:var(--text);text-align:center;margin-bottom:10px">💰 자산 흐름 <span style="font-size:10px;font-weight:500;color:var(--text3)">(가상 초기자본 100만원 · 복리)</span></div>
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px 6px">
          ...
          ${opens.length ? `<div style="margin-top:10px;font-size:10px;color:var(--text3);text-align:center">💡 보유 ${opens.length}건의 미실현 손익은 통계 제외 (청산 후 반영)</div>` : ''}
        </div>
      </div>
    </div>`;
  }
