...
function btCrossRefreshTargetList(){
  ...
  listEl.innerHTML = `<div style="padding:8px 10px;background:var(--surface2);border-radius:6px">
    <div class="cv-toggle" onclick="_sxVib(8);window._cvToggle&&_cvToggle(this,'btCrossListBody')" style="font-size:10px;color:var(--text3);margin-bottom:6px;cursor:pointer">
      ...
      <span class="cv-arrow" style="color:var(--accent);font-size:11px;margin-left:5px">▶</span>
    </div>
    <!-- [S748] 종목리스트 접이식(기본접힘) -->
    <div id="btCrossListBody" style="display:none" data-loaded="1">
      ${listHtml}
      ${excludedNote}
    </div>
  </div>`;
...
async function btCrossReinforce(){
  ...
  for(let i = 0; i < candidates.length; i++){
    ...
    try{
      ...
      // [S732] ★자동보강 봉 조달을 풀링과 동일한 fetchRows600(견고한 2-tier)로 선조달 → _runBtWithExtension 경로A(_lastAnalCandles 재사용)로 처리.
      //   기존 fetchCandlesExtended는 currentMarket 의존 + API 일시실패로 '됐다 안됐다' → 풀링이 쓰는 동일 경로로 통일(안정화).
      try {
        if(window.SXCandleBT && SXCandleBT.fetchRows600){
          const _r600rf = await SXCandleBT.fetchRows600(_stockMarket, _tf, t.code);
          if(Array.isArray(_r600rf) && _r600rf.length >= 200){ t._lastAnalCandles = _r600rf; }
        }
      } catch(_rf){}
...
function btRenderCrossResult(results, cacheMissReasons, excludedList){
  ...
  // [S728] 평균 MDD·PF·총거래 — bear-gate 측정용(게이트 OFF/ON 비교 시 MDD↓·총거래↓·PF↑ 확인)
  const avgMdd = valid.length ? +(valid.reduce((s,r)=>s+(r.mdd||0),0)/valid.length).toFixed(2) : 0;
  const avgPf = valid.length ? +(valid.reduce((s,r)=>s+(r.profitFactor||0),0)/valid.length).toFixed(2) : 0;
  const sumTrades = valid.reduce((s,r)=>s+(r.totalTrades||0),0);
  ...
  let html = `<div class="bt-card">
    ...
    <div class="bt-stat-grid">
      ...
      <div class="bt-stat-item"><div class="bt-stat-num" style="color:var(--sell)">${avgMdd}%</div><div class="bt-stat-label">평균 MDD</div></div>
      <div class="bt-stat-item"><div class="bt-stat-num" style="color:${avgPf>=1?'var(--buy)':'var(--sell)'}">${avgPf}</div><div class="bt-stat-label">평균 PF</div></div>
      <div class="bt-stat-item"><div class="bt-stat-num">${sumTrades}</div><div class="bt-stat-label">총 거래</div></div>
    </div>
  </div>`;
