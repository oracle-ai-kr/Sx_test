...
// ────────────────────────────────────────────────────────────
// [S363] BT 캔들 단일소스화 — 봉수 흔들림(400↔600) 방지
// ────────────────────────────────────────────────────────────
...
function _mergeBtCandles(stock, rows, targetCount, tag){
  try{
    ...
    if(rows.length < tc && prev && prev.length > rows.length){
      ...
      if(_nd <= _pd){
        console.log(`[S363] ${(stock&&stock.code)||''} 봉수 퇴보 방지: 새 ${rows.length} < 기존 ${prev.length}봉 (끝봉 ${_nd}≤${_pd}) → 기존 재사용 (${tag})`);
    ...
    console.warn('[S363] _mergeBtCandles 예외:', e && (e.message||e));
    return rows;
  }
}
...
async function _runBtWithExtension(stock, tf, quiet){
  try{
    ...
    // [S363] 단일소스화 가드 — 부분봉수 퇴보 방지 (세 경로 공유: runBt/fetchExt/engineVerify)
