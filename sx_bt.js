...
function _crossMinBars(market){
  return CROSS_MIN_BARS[market || 'kr'] || 400;
}
...
async function btCrossReinforce(){
  ...
  const startMsg = `[S162-reinforce] ⚡ 자동 보강 시작: ${candidates.length}종목`;
  ...
  for(let i = 0; i < candidates.length; i++){
    ...
    try{
      const r = await _runBtWithExtension(t, _tf, true);
      if(r.ok && r.result && r.result.totalTrades >= 0){
        // 봉수 확인 — 시장별 최소 기준 충족 여부
        ...
        if(bars >= minBars){
          ...
          console.warn(`[S162-reinforce] ⚠ ${t.name} 봉수 부족 (${bars}/${minBars}봉) — API 제한 가능성`);
        }
      } else {
        failed++;
        failedNames.push(t.name);
        console.warn(`[S162-reinforce] ❌ ${t.name} 실패: ${r.error || 'unknown'}`);
      }
    }catch(e){
      failed++;
      failedNames.push(t.name);
      console.error(`[S162-reinforce] 예외 ${t.name}:`, e);
    }
