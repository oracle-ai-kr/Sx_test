...
function mtRender(){
  ...
  if(closed.length >= 1){
    ...
    // 자산 흐름 — 가상 초기자본 100만원 (cumMul 재활용)
    ...
    // 메인 카드에 추가될 자산 흐름 — 상단 4개 그리드와 구분선으로만 분리
    ...
    expandedHtml = `<div style="${_bodyStyle};margin-bottom:8px;padding:10px 4px 4px;display:grid;grid-template-columns:repeat(3,1fr);gap:14px 6px">
  ...
  <div style="text-align:center;font-size:10px;color:var(--text3);margin-bottom:8px">적중률 ${hitRate}% · 복리 기준${toggleIndicator}</div>
  ${expandedHtml}`;
