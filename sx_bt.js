// ════════════════════════════════════════════════════════════
//  SIGNAL X — BT Module v1.11-fix3 <!-- S110 Phase C+D fix3 (병렬+안전, 채희창 피드백): (1) 교차/대시 병렬 배치에 0.5초 종목 간 스태거 추가 — Promise.all 내부에서 각 종목 시작을 idx*500ms 지연하여 계단식 실행. 초당 호출 수를 10건 → 2건으로 분산 (Upbit 공식 초당 10건 제한 대비 80% 여유). 5종목 배치 = 2초(스태거) + 6초(블록 3개) = 약 8초 소요 (기존 6초 대비 +2초). 분석탭/단일검증/학습검증은 기존 2초 대기 그대로 유지 (사용자 직접 대기 UX, 빠르게). (2) 분석탭 진입 시 window._s107Logs 초기화 — 종목별 깨끗한 로그 확인. (3) ☆ 마크는 관심종목만 (현재 분석 종목은 공백) — 사용자가 관심 vs 임시 구분 가능. --> v1.11-fix <!-- S110 Phase C+D fix (2건): (1) 관심 해제 시 교차/대시 리스트 즉시 갱신. (2) 관심종목 ☆ 시각 구분. --> v1.11 <!-- S110 Phase C+D 통합: 관심 기반 교차/대시 + 자동 BT 캐싱 (SX_WATCH_BT_CACHE 7일 TTL). --> v1.10 <!-- S110 매매이력 날짜 표시 --> v1.9 <!-- S109 BT 탭 600봉 확장 --> v1.8
    ...
    for(const batch of batches){
      ...
      for(const { t, btOut } of batchResults){
        if(btOut.ok && btOut.result && btOut.result.totalTrades > 0){
          ...
          console.log(`[S110-cross] ✅ 병렬 BT 완료: ${t.name} (거래 ${r.totalTrades})`);
        } else {
          results.push({
            name:t.name, code:t.code,
