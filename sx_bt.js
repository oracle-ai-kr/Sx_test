// ════════════════════════════════════════════════════════════
//  SIGNAL X — BT Module v1.9 <!-- S109 Phase 3-B-9b 단일검증 자동 3단계 확장 (200 → 400 → 600봉): btRunBasic에 4경로 확장 로직 추가. 경로 A: 분석탭 확장 캐시(_lastAnalCandles ≥ 600봉) 즉시 재사용. 경로 B: 부분 캐시(400봉 등) 추가 확장. 경로 C: 캐시 없음 + 시장 지원(coin/kr) → 새로 3단계 (200→[2초]→400→[2초]→600). 경로 D: 시장 미지원(us) → 기존 300~400봉 단일 fetch. 주봉/월봉은 기존 400봉 유지(충분), 분봉/일봉은 600봉 목표. 각 단계 2초 대기 (봇 감지 회피). 확장 결과를 stock._lastAnalCandles에 저장하여 분석탭 재진입 시 재사용. stage 플래그도 업데이트 (600→2, 400→1). 실패 시 부분 데이터로 폴백 (에러 X). --> v1.8 <!-- S103-fix7 Phase3-A-1 _isBuySignal/sell_signal 판정 기준 교정(entryIdx vs _totalBars 기준축 불일치 근본버그 수정, _btGetRowsLen 헬퍼+fallback) --> v1.7 <!-- S103-fix5 -->
