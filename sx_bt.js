// ════════════════════════════════════════════════════════════
//  SIGNAL X — BT Module v1.9 <!-- S109 Phase 3-B-9b 단일검증 + 학습검증 자동 3단계 확장 (200 → 400 → 600봉): (1) btRunBasic 4경로 확장 로직 (경로A: 분석탭 확장 캐시 재사용, 경로B: 부분 캐시 추가 확장, 경로C: 새 3단계, 경로D: 시장 미지원 단일 fetch). (2) btRunWf에 동일 4경로 적용 — 학습 70% × 600봉 = 420봉 / 검증 180봉 → 통계적 유의성 확보 (300봉 대비 2배). (3) btRenderWfResult에 trainBars/testBars 파라미터 추가 — 학습/검증 구간 헤더에 "70% · 420봉" 형태로 봉 수 표시 (신뢰도 근거 제공). 전체 봉 수도 타이틀에 "총 600봉"으로 표기. 주봉/월봉은 400봉 유지, 분봉/일봉은 600봉 목표. 각 단계 2초 대기 (봇 감지 회피). 분석탭/단일검증/학습검증 간 stock._lastAnalCandles 공유로 캐시 재사용. stage 플래그 자동 갱신 (600→2, 400→1). 실패 시 부분 데이터 폴백. --> v1.8 <!-- S103-fix7 Phase3-A-1 _isBuySignal/sell_signal 판정 기준 교정(entryIdx vs _totalBars 기준축 불일치 근본버그 수정, _btGetRowsLen 헬퍼+fallback) --> v1.7 <!-- S103-fix5 -->
