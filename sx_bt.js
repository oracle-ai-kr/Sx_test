// ════════════════════════════════════════════════════════════
//  SIGNAL X — BT Module v1.11-fix <!-- S110 Phase C+D fix (2건, 채희창 피드백): (1) 관심 해제 시 교차/대시 리스트 즉시 갱신 — toggleWatchlist(분석탭 ☆)와 removeWlItem(관심패널 ✕) 양쪽에 btCrossRefreshTargetList/btDashRefreshTargetList 호출 추가. 이전엔 switchAnalTab 훅만 있어 이미 열린 탭에서는 stale 상태 유지됨. (2) 관심종목 ☆ 시각 구분 — 교차/대시 대상 리스트 각 종목 앞에 ☆ 마크 추가 (전부 관심목록이므로 모두 표시, "관심 기반"임을 시각적 명시). 대시보드도 종목별 리스트 표시로 일관성 확보 (이전엔 요약만). 결과 테이블 종목명에도 ☆ prefix. --> v1.11 <!-- S110 Phase C+D 통합: (1) 관심 BT 캐시 인프라 SX_WATCH_BT_CACHE (7일 TTL). (2) _runBtWithExtension 헬퍼 (4경로 공용). (3) _autoBtOnWatchlistAdd 백그라운드 자동 BT. (4) 교차검증/대시보드 관심목록 기반 재설계. (5) 캐시 히트/미스 배지. (6) 단일BT → 관심 캐시 자동 갱신. (7) "검증결과누적" → "검증결과갱신". --> v1.10 <!-- S110 매매이력 날짜 표시 --> v1.9 <!-- S109 BT 탭 600봉 확장 --> v1.8
...
function btCrossRefreshTargetList(){
  ...
  // S110 fix: 캐시 상태 + ☆ 관심종목 마크
  //   대상은 전부 관심목록이므로 모두 ☆ 표시 (시각적으로 "관심 기반"임을 명시)
  const cachedList = targets.map(t => {
    ...
    const starMark = `<span style="color:var(--accent);font-size:11px;width:14px;text-align:center;flex-shrink:0">☆</span>`;
...
function btDashRefreshTargetList(){
  ...
  // S110 fix: 대시보드도 종목별 리스트 + ☆ 마크 (교차와 일관성)
