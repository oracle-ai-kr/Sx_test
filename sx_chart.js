// ════════════════════════════════════════════════════════════
//  SX Chart Module v1.6 — S97: 색상통일(매수주황/익절녹/손절빨)+글로우테두리
...
function openFull(rows, stockName, trades){
  ...
  setTimeout(function(){
    _drawFullCandle('sxFullCandle', data, closes, bb, 240, rows.length, trades);
...
/* ── 풀차트: 캔들 + MA + BB ── */
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades){
  ...
  // S96: BT 마커 오버레이 — 마지막 거래만, 2배 크기 + 테두리
  if(trades && trades.length){
    var offset = (fullLen||data.length) - data.length;
    var BUY_C = '#ff9900', WIN_C = '#22c55e', LOSS_C = '#e8365a';
    ...
    function _glowStroke(ctx,c,lw){ ctx.save(); ctx.shadowColor=c; ctx.shadowBlur=6; ctx.strokeStyle=c; ctx.lineWidth=lw; ctx.stroke(); ctx.shadowBlur=0; ctx.restore(); }
    ...
    if(lastTr){
      ...
      if(lastTr.type!=='OPEN'&&lastTr.exitIdx!=null){
        ...
        if(xL>=0&&xL<data.length){
          ...
          var clr=lastTr.type==='WIN'?WIN_C:LOSS_C;
          ctx.fillStyle=clr;
          ctx.beginPath(); ctx.moveTo(xx,xy+ms); ctx.lineTo(xx-ms*0.7,xy-ms*0.3); ctx.lineTo(xx+ms*0.7,xy-ms*0.3); ctx.closePath(); ctx.fill();
          _glowStroke(ctx,'#fff',2);
          ctx.fillStyle=clr; ctx.font='bold 8px Outfit,sans-serif'; ctx.textAlign='center'; ctx.fillText(fmtSh(lastTr.exitDate),xx,xy-ms*0.3-4);
        }
      }
    }
    // 범례
    ...
    ctx.fillStyle=WIN_C; ctx.fillText('▼매도', W-pad.r-2, 10);
  }
...
/* ════════════════════════════════════════════════════════════
   S95: 미니 차트 + BT 매수/매도 마커 오버레이
   trades: [{entryIdx, exitIdx, type('WIN'/'LOSS'/'OPEN'), entryDate, exitDate, entry, exit}]
   offset: rows.length - displayedCount (슬라이스 오프셋)
   ════════════════════════════════════════════════════════════ */
function drawMiniWithTrades(canvasId, rows, trades){
  ...
  // BT 마커 오버레이 — 마지막 거래만 표시
  var BUY_CLR = '#ff9900';  // 노랑 (매수)
  var WIN_CLR = '#22c55e';  // 녹색 (익절)
  var LOSS_CLR = '#e8365a'; // 빨강 (손절)
  var markerSize = Math.max(cw * 1.0, 8);

  // 날짜 포맷 헬퍼: '2025-03-15' → '3/15'
  function fmtShort(d){
    if(!d) return '';
    d=String(d);
  ...
  function _glowStrokeMini(ctx,c,lw){ ctx.save(); ctx.shadowColor=c; ctx.shadowBlur=5; ctx.strokeStyle=c; ctx.lineWidth=lw; ctx.stroke(); ctx.shadowBlur=0; ctx.restore(); }
  ...
  if(lastTr){
    ...
    if(lastTr.type !== 'OPEN' && lastTr.exitIdx != null){
      ...
      if(xLocal >= 0 && xLocal < data.length){
        ...
        var clr = lastTr.type === 'WIN' ? WIN_CLR : LOSS_CLR;
        ctx.fillStyle = clr;
        ctx.beginPath();
  ...
  ctx.fillStyle = WIN_CLR; ctx.fillText('▼매도', W-pad.r-2, 10);
}

return { drawMini: drawMini, drawMiniWithTrades: drawMiniWithTrades, openFull: openFull, closeFull: closeFull, drawFinBar: drawFinBar, drawFinTrend: drawFinTrend, drawScoreSpark: drawScoreSpark, drawDeltaBar: drawDeltaBar, drawScoreGauge: drawScoreGauge };
})();
