...
// [BUGFIX] EMA 분석엔진과 일관성 — 첫 p봉의 SMA를 시드로 사용 (이전: i=0에서 첫 값으로 시작)
//   분석엔진 emaArray와 동일한 구현 → 차트 표시값과 분석탭 값의 미세한 차이 제거
//   차트 MACD/시그널 라인의 정확도 향상
function ema(arr,p){
  const r=[],k=2/(p+1);
  let e=null;
...
// [BUGFIX] calcMACD가 null EMA 값을 안전하게 처리 — fast/slow 어느 쪽이라도 null이면 macd line도 null
//   기존: null - null = NaN → drawLine의 null 가드 우회 → NaN 좌표로 그려짐
...
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades, svVerdict){
  ...
  if(trades && trades.length){
    ...
    if(lastTr){
      // [POLICY 2026-04 USER REQUEST] 마커 정책 단순화 (drawMiniWithTrades와 동일)
      //   사용자 정책:
      //     1) lastTr.type === 'OPEN' (보유중) — ▲ entry만 (showSellMarker면 ▼ 마지막봉 + 진입▲ 짝)
      //     2) lastTr.type === 'WIN'/'LOSS' (청산됨) — showSellMarker일 때만 ▲entry+▼exit 쌍
      //        showBuyMarker (= 엔진시뮬 포지션 없음 + 새 매수 신호) — 마커 표시 안 함 ★
      //        그 외(관심/관망/회피) — 마커 표시 안 함
      if(lastTr.type === 'OPEN' && lastTr.entryIdx != null){
        ...
        } else if(!(svVerdict && (svVerdict.action === '회피' || svVerdict.action === '관망' || svVerdict.action === '관심'))){
        ...
        // showBuy (포지션 없음 + 새 매수 신호) / 그 외 — 마커 표시 안 함
      }
      // svVerdict 없으면 fallback (하위 호환) — 마지막 거래 entry+exit 쌍
      if(!svVerdict){
...
function drawMiniWithTrades(canvasId, rows, trades, svVerdict){
  ...
  // [POLICY 2026-04 USER REQUEST] 마커 정책 단순화
  //   사용자 의도: "마지막 BT 거래 자체를 깨끗하게 표시 — 짝 마커로 직전 거래 그리지 않음"
  //   기존 (PATCH-15 / S128 시기): 통합판정 매수/매도일 때 "직전 청산 거래의 exitIdx"를 짝 ▼로 추가 표시
  //     → 마지막 거래 청산 + 새 매수 신호 케이스에서 ▲(현재 매수)+▼(이전 거래 청산)이 섞여서
  //        "최근 매도 마크가 갱신 안 됨"처럼 보이고 마커가 너무 많아 시각 혼란
  //   사용자 정책:
  //     1) lastTr.type === 'OPEN' (보유중) — ▲ entry만 (▼ 없음, 아직 청산 안 함)
  //     2) lastTr.type === 'WIN'/'LOSS' (청산됨) — ▲ entry + ▼ exit (같은 거래의 쌍, 짝 보조 마커 없음)
  //     3) "엔진시뮬 포지션 없음" + chartMarker='buy' (= 신규 신호지만 BT 미진입) — 마커 표시 안 함
  //        ※ "포지션 없음"의 식별 = lastTr가 청산된 거래 + showBuyMarker 일 때
  //            (BT 마지막이 청산이므로 현재 BT는 무포지션 상태)
  //   showSellMarker (lastTr 청산 + 통합판정 매도 = 가장 최근 거래의 종료를 보여줌) — 정상 표시
  if(lastTr){
    if(lastTr.type === 'OPEN' && lastTr.entryIdx != null){
      // 케이스 1: 보유중 — ▲만 표시 (showBuyMarker 또는 일반 보유 맥락)
      //   showSellMarker (보유중 + sell_ready) 케이스는 아래 별도 분기에서 처리
      if(showSellMarker){
        // 보유중 + 매도 신호 → 마지막 봉에 ▼ + 진입 봉에 ▲ 짝 (실제 청산 전 시그널 시각화)
        _drawSellMini((offset + data.length - 1), null, false);
        _drawBuyMini(lastTr.entryIdx, lastTr.entryDate, true);
      } else if(!(svVerdict && (svVerdict.action === '회피' || svVerdict.action === '관망' || svVerdict.action === '관심'))){
        // 일반 보유중 (매수 신호 또는 단순 보유 맥락) — ▲ 진입가
        _drawBuyMini(lastTr.entryIdx, lastTr.entryDate, false);
      }
      // 회피/관망/관심 액션이면 ▲도 안 그림 (배너 v2.0과 일관성)
    }
    else if(lastTr.type !== 'OPEN' && lastTr.exitIdx != null && lastTr.entryIdx != null){
      // 케이스 2: 마지막 거래가 청산됨 (WIN/LOSS)
      //   "엔진시뮬 포지션 없음" 상태 — BT는 이미 청산을 마쳤고 현재 미보유.
      //   showBuyMarker (새 매수 신호)일 때 마커를 표시 안 함이 사용자 정책 ★
      //   showSellMarker (이미 청산됨이 통합판정 매도와 일치) — 마지막 거래의 entry+exit 쌍 표시
      //   둘 다 false (관심/관망/회피 등)일 때도 마지막 거래는 청산 끝 — 마커 표시 안 함
      if(showSellMarker){
        _drawBuyMini(lastTr.entryIdx, lastTr.entryDate, false);
        _drawSellMini(lastTr.exitIdx, lastTr.exitDate, false);
      }
      // showBuyMarker (포지션 없음 + 새 매수 신호) — 마커 표시 안 함 ← 사용자 정책
      // 그 외 — 마커 표시 안 함
    }
  }
...
function highlightBar(canvasId, barIdx, rows, trades, svVerdict){
  ...
  // [BUGFIX] H를 동적으로 결정 — drawMini=160, drawMiniWithTrades=180
  //   기존: 180으로 하드코딩 → trades 없을 때 차트 영역(160) 밖에 하이라이트 그려짐
  ...
  // [BUGFIX] 흰 배경에 흰색 반투명(rgba(255,255,255,0.15))은 거의 안 보임 → 노란색 반투명으로 변경
  ctx.save();
