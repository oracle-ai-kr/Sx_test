// ════════════════════════════════════════════════════════════
//  SX Chart Module v1.7 — S99: 차트마커 통합판정기준(▲매수=BT+buy_ready/▼매도=BT매도|sell_ready)
    ...
    if(lastTr){
      ...
      if(showBuy && lastTr.entryIdx!=null){
        ...
        if(eL>=0&&eL<data.length){
          ...
          ctx.fillStyle=BUY_C; ctx.font='bold 8px Outfit,sans-serif'; ctx.textAlign='center'; ctx.fillText(fmtSh(lastTr.entryDate),ex,ey+ms*0.3+10);
        }
      }
      // 매도 ▼ — 통합판정 매도일 때만
      if(showSell){
        if(lastTr.type!=='OPEN'&&lastTr.exitIdx!=null){
          ...
          if(xL>=0&&xL<data.length){
            ...
            ctx.fillStyle=SELL_C; ctx.font='bold 8px Outfit,sans-serif'; ctx.textAlign='center'; ctx.fillText(fmtSh(lastTr.exitDate),xx,xy-ms*0.3-4);
          }
        }
