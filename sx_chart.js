// ════════════════════════════════════════════════════════════
//  SX Chart Module v1.9 — S103 차트전체화면 닫기버튼 진동15 추가
//  v1.8 — S99: 차트마커 통합판정기준(▲매수=BT+buy_ready/▼매도=BT매도|sell_ready)
//  S97: 색상통일(매수주황/익절녹/손절빨)+글로우테두리
//  S96: 풀차트BT마커+미니/풀 마커 봉밀착
//  S58: drawFinTrend 분기별 추이 차트 추가
  ...
  if(trades && trades.length){
    ...
    if(lastTr){
      ...
      if(showBuy && lastTr.entryIdx!=null){
        var eL=lastTr.entryIdx-offset;
        if(eL>=0&&eL<data.length){
          ...
          ctx.fillStyle=BUY_C; ctx.font='bold 16px Outfit,sans-serif'; ctx.textAlign='center'; ctx.fillText(fmtSh(lastTr.entryDate),ex,ey+ms*0.3+14);
        }
      }
      // 매도 ▼ — 통합판정 매도일 때만
      if(showSell){
        if(lastTr.type!=='OPEN'&&lastTr.exitIdx!=null){
          var xL=lastTr.exitIdx-offset;
          if(xL>=0&&xL<data.length){
            ...
            ctx.fillStyle=SELL_C; ctx.font='bold 16px Outfit,sans-serif'; ctx.textAlign='center'; ctx.fillText(fmtSh(lastTr.exitDate),xx,xy-ms*0.3-8);
          }
        }
        // 보유중 + sell_ready → 마지막 봉
        else if(lastTr.type==='OPEN'){
          var sIdx=data.length-1;
          var sx=pad.l+sIdx*cw+cw/2, sy=yFn(data[sIdx].high)-1;
          ctx.fillStyle=SELL_C;
          ctx.beginPath(); ctx.moveTo(sx,sy+ms); ctx.lineTo(sx-ms*0.7,sy-ms*0.3); ctx.lineTo(sx+ms*0.7,sy-ms*0.3); ctx.closePath(); ctx.fill();
          _markerStroke(ctx,SELL_C);
        }
      }
      // svVerdict 없으면 fallback
      if(!svVerdict){
        if(lastTr.entryIdx!=null){
          var eL2=lastTr.entryIdx-offset;
          if(eL2>=0&&eL2<data.length){
            var ex2=pad.l+eL2*cw+cw/2, ey2=yFn(data[eL2].low)+1;
            ctx.fillStyle=BUY_C; ctx.beginPath(); ctx.moveTo(ex2,ey2-ms); ctx.lineTo(ex2-ms*0.7,ey2+ms*0.3); ctx.lineTo(ex2+ms*0.7,ey2+ms*0.3); ctx.closePath(); ctx.fill(); _markerStroke(ctx,BUY_C);
          }
        }
        if(lastTr.type!=='OPEN'&&lastTr.exitIdx!=null){
          var xL2=lastTr.exitIdx-offset;
          if(xL2>=0&&xL2<data.length){
            var xx2=pad.l+xL2*cw+cw/2, xy2=yFn(data[xL2].high)-1;
            ctx.fillStyle=SELL_C; ctx.beginPath(); ctx.moveTo(xx2,xy2+ms); ctx.lineTo(xx2-ms*0.7,xy2-ms*0.3); ctx.lineTo(xx2+ms*0.7,xy2-ms*0.3); ctx.closePath(); ctx.fill(); _markerStroke(ctx,SELL_C);
          }
        }
      }
    }
...
function drawMiniWithTrades(canvasId, rows, trades, svVerdict){
  ...
  // S99: 통합판정 기준 마커 — svVerdict.chartMarker로 판단
  ...
  if(showBuyMarker && lastTr && lastTr.entryIdx != null){
    var eLocal = lastTr.entryIdx - offset;
    if(eLocal >= 0 && eLocal < data.length){
      var ex = pad.l + eLocal * cw + cw/2;
      var ey = yFn(data[eLocal].low) + 1;
      ...
      ctx.textAlign = 'center';
      ctx.fillText(fmtShort(lastTr.entryDate), ex, ey + markerSize*0.3 + 9);
    }
  }

  // 매도 ▼ — 통합판정 "매도"일 때만 (BT 매도 or 보유중+sell_ready)
  if(showSellMarker && lastTr){
    // BT 매도 신호인 경우
    if(lastTr.type !== 'OPEN' && lastTr.exitIdx != null){
      var xLocal = lastTr.exitIdx - offset;
      if(xLocal >= 0 && xLocal < data.length){
        var xx = pad.l + xLocal * cw + cw/2;
        var xy = yFn(data[xLocal].high) - 1;
        ...
        ctx.font = 'bold 14px Outfit,sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(fmtShort(lastTr.exitDate), xx, xy - markerSize*0.3 - 4);
      }
    }
    // 보유중 + sell_ready인 경우 → 마지막 봉에 ▼ 표시
    else if(lastTr.type === 'OPEN'){
      var sellLocal = data.length - 1;
      var sx = pad.l + sellLocal * cw + cw/2;
      var sy = yFn(data[sellLocal].high) - 1;
      ctx.fillStyle = SELL_CLR;
      ctx.beginPath();
      ctx.moveTo(sx, sy + markerSize);
      ctx.lineTo(sx - markerSize*0.7, sy - markerSize*0.3);
      ctx.lineTo(sx + markerSize*0.7, sy - markerSize*0.3);
      ctx.closePath(); ctx.fill();
      _markerStrokeMini(ctx,SELL_CLR);
    }
  ...
  if(!svVerdict && lastTr){
    if(lastTr.entryIdx != null){
      var eLocal2 = lastTr.entryIdx - offset;
      if(eLocal2 >= 0 && eLocal2 < data.length){
        var ex2 = pad.l + eLocal2 * cw + cw/2;
        var ey2 = yFn(data[eLocal2].low) + 1;
        ctx.fillStyle = BUY_CLR;
        ctx.beginPath(); ctx.moveTo(ex2, ey2-markerSize); ctx.lineTo(ex2-markerSize*0.7, ey2+markerSize*0.3); ctx.lineTo(ex2+markerSize*0.7, ey2+markerSize*0.3); ctx.closePath(); ctx.fill();
        _markerStrokeMini(ctx,BUY_CLR);
      }
    }
    if(lastTr.type !== 'OPEN' && lastTr.exitIdx != null){
      var xLocal2 = lastTr.exitIdx - offset;
      if(xLocal2 >= 0 && xLocal2 < data.length){
        var xx2 = pad.l + xLocal2 * cw + cw/2;
        var xy2 = yFn(data[xLocal2].high) - 1;
        ctx.fillStyle = SELL_CLR;
        ctx.beginPath(); ctx.moveTo(xx2, xy2+markerSize); ctx.lineTo(xx2-markerSize*0.7, xy2-markerSize*0.3); ctx.lineTo(xx2+markerSize*0.7, xy2-markerSize*0.3); ctx.closePath(); ctx.fill();
        _markerStrokeMini(ctx,SELL_CLR);
      }
    }
  }
