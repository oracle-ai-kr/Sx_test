...
function drawMini(canvasId, rows, svVerdict){
  ...
  if(svVerdict && (svVerdict.chartMarker === 'buy' || svVerdict.chartMarker === 'sell')){
    ...
    var _holdC2 = !!svVerdict.chartMarkerHold; // [S361] 보유중 → 채움없는 실선 굵은 테두리
...
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades, svVerdict){
  ...
  if(trades && trades.length){
    ...
    var _holdC = !!(svVerdict && svVerdict.chartMarkerHold); // [S361] 보유중 마커 → 채움없는 보라 실선 굵은 테두리
  ...
  _drawTrendMarkers(ctx, data, pad, cw, yFn, closes, W, 8);
...
/* ════════════════════════════════════════════════════════════
   ...
   마커 없음 = 그 외
   ════════════════════════════════════════════════════════════ */
// [S562] 단기추세 MA 크로스 마커 — 매수(골든) 하늘색▲ / 매도(데드) 핑크색▼ + 날짜 라벨.
//   기간은 단기추세매매 카드 설정(SX_TREND_{market}) 따라감(기본 5×9). 화면 내 가장 최근 골든/데드 1개씩.
//   sma()는 이 IIFE 내 배열반환 헬퍼. BT 녹/적 마커와 색으로 구분, 별도 호출이라 BT 거래 유무와 무관.
function _drawTrendMarkers(ctx, data, pad, cw, yFn, closes, W, legendFont){
  ...
  _mk(gIdx, TBUY, true);
  _mk(dIdx, TSELL, false);
  // 범례 (좌상단, 실제로 그린 것만) — 우상단 BT 범례와 충돌 안 함
  if(gIdx>=0 || dIdx>=0){
    ctx.save();
    ctx.font='bold '+(legendFont||8)+'px Outfit,sans-serif'; ctx.textAlign='left';
    var lx=pad.l+2;
    if(gIdx>=0){ ctx.fillStyle=TBUY; ctx.fillText('\u25B2\uCD94\uC138\uB9E4\uC218', lx, 10); lx+=46; }
    if(dIdx>=0){ ctx.fillStyle=TSELL; ctx.fillText('\u25BC\uCD94\uC138\uB9E4\uB3C4', lx, 10); }
    ctx.restore();
  }
}
...
function drawMiniWithTrades(canvasId, rows, trades, svVerdict){
  ...
  var _holdMini = !!(svVerdict && svVerdict.chartMarkerHold); // [S361] 보유중 → 채움없는 실선 굵은 테두리
  ...
  _drawTrendMarkers(ctx, data, pad, cw, yFn, closes, W, 7);
  drawMALegend(ctx, closes, pad, H, 8);
