...
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades, svVerdict){
  ...
  if(trades && trades.length){
    ...
    if(lastTr){
      // 매수 ▲ — 통합판정 매수일 때 + 짝 ▼
      // [BUGFIX 2026-04] lastTr 타입에 따라 짝 마커 위치 분기 (drawMiniWithTrades와 동일)
      //   기존: 항상 lastTr.entryIdx로 ▲ 표시 + trades.length-2부터 짝 ▼ 찾기
      //         → 마지막 거래가 청산된 WIN/LOSS인데 chartMarker='buy'면 가장 최근 청산 마커가 누락됨
      //   수정: lastTr가 OPEN(보유중)이 아니라 청산 완료면 lastTr.exitIdx를 메인 ▼로 표시
      if(showBuy && lastTr){
        ...
        } else if(lastTr.type !== 'OPEN' && lastTr.exitIdx != null){
          // 케이스 B: 마지막 거래 청산 완료 + 새 매수 신호
          //   → 가장 최근 청산(lastTr.exitIdx) 메인 ▼ + 신호 봉(마지막 봉)에 ▲
          _drawSellMarker(lastTr.exitIdx, lastTr.exitDate, false);
          _drawBuyMarker((offset + data.length - 1), null, false);
        } else if(lastTr.entryIdx != null){
          // 폴백: type 불명
          _drawBuyMarker(lastTr.entryIdx, lastTr.entryDate, false);
        }
      }
      ...
      //   [v2.0] 배너 _buildSimPositionLine과 일관성:
      //     최종 C 판정이 '매수'가 아니면 "포지션 취소"로 표시되므로
      //     차트의 ▲ 맥락 마커도 함께 숨겨야 함 (마커만 떠있으면 모순)
      //     - 관심/관망/회피 모두 차단 (점수 일부 부족 또는 모멘텀 승급 케이스 포함)
      //   [폐기] v1.x [강등] 라벨 분기는 v2.0 모멘텀 보정으로 대체되어 더 이상 발생 안 함
      if(!showBuy && !showSell && lastTr.type === 'OPEN' && lastTr.entryIdx != null
         && !(svVerdict && (svVerdict.action === '회피' || svVerdict.action === '관망' || svVerdict.action === '관심'))){
...
function drawMiniWithTrades(canvasId, rows, trades, svVerdict){
  ...
  // 매수 ▲ — 통합판정 "매수"일 때 + 짝 ▼
  // [BUGFIX 2026-04] lastTr 타입에 따라 짝 마커 위치 분기
  //   기존: 항상 lastTr.entryIdx로 ▲ 표시 + trades.length-2부터 짝 ▼ 찾기
  //         → 마지막 거래가 이미 청산된 WIN/LOSS인 경우 lastTr.exitIdx(가장 최근 청산)을 무시하고
  //            그 이전 거래의 exitIdx로 ▼ 표시 → 사용자 혼란
  //         예: 마지막 LOSS(2/27 진입 → 3/3 청산), chartMarker='buy'일 때
  //             기존: ▲2/27(LOSS 진입가) + ▼2/20(이전 WIN 청산)  ← 3/3 청산 마커 누락!
  //             수정: ▼3/3(가장 최근 청산, 메인) + ▲ 마지막 봉(새 신호 위치)
  //   수정 분기:
  //     - lastTr.type === 'OPEN' (보유중 + 매수 강화): 기존 동작 유지
  //     - lastTr 청산됨 + 새 매수 신호: lastTr.exitIdx 메인 ▼ + 마지막 봉에 ▲
  if(showBuyMarker && lastTr){
    if(lastTr.type === 'OPEN' && lastTr.entryIdx != null){
      // 케이스 A: 보유중 + 매수 신호 강화 → 현재 진입가 ▲ + 직전 청산 ▼ (반투명 짝)
      ...
      // 케이스 B: 마지막 거래 청산 완료 + 새 매수 신호
      //   가장 최근 청산(lastTr.exitIdx)을 메인 ▼로, 신호 봉(마지막 봉)에 ▲
      _drawSellMini(lastTr.exitIdx, lastTr.exitDate, false);
      // 신호 봉 = 마지막 봉 (현재가). 새 진입 가격은 아직 없으므로 마지막 봉 위치에 ▲
      _drawBuyMini((offset + data.length - 1), null, false);
    } else if(lastTr.entryIdx != null){
      // 폴백: type 불명이면 기존 동작
      _drawBuyMini(lastTr.entryIdx, lastTr.entryDate, false);
    }
