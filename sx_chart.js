...
function drawMini(canvasId, rows, svVerdict){
  ...
  if(svVerdict && (svVerdict.chartMarker === 'buy' || svVerdict.chartMarker === 'sell')){
    ...
    var _holdC2 = !!svVerdict.chartMarkerHold; // [S359] 보유중 → 점선
    var ms = Math.max(cw * 1.2, 10);
    var li = data.length - 1;
    if(li >= 0){
      var mx = pad.l + li*cw + cw/2;
      ctx.save();
      if(_holdC2) ctx.globalAlpha = 0.5;
      ...
      if(_holdC2) ctx.setLineDash([2.5,2]);
      ctx.shadowColor = CURRENT_C; ctx.shadowBlur = _holdC2 ? 4 : 6; ctx.strokeStyle = _holdC2 ? CURRENT_C : '#000'; ctx.lineWidth = 1.4; ctx.stroke(); ctx.shadowBlur = 0;
      ctx.restore();
    }
...
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades, svVerdict){
  ...
  if(trades && trades.length){
    ...
    function _drawCurrentBuyMarkerFull(){
      ...
      ctx.lineTo(ex + ms*0.7, ey + ms*0.3);
      ctx.closePath(); ctx.fill();
      if(_holdC){ ctx.setLineDash([2.5,2]); ctx.shadowColor=CURRENT_C; ctx.shadowBlur=4; ctx.strokeStyle=CURRENT_C; ctx.lineWidth=1.4; ctx.stroke(); }
      else _markerStroke(ctx, CURRENT_C);
      ctx.restore();
    }
...
function drawMiniWithTrades(canvasId, rows, trades, svVerdict){
  ...
  var _holdMini = !!(svVerdict && svVerdict.chartMarkerHold); // [S359] 보유중 마커 → 점선
  function _drawCurrentBuyMini(){
    var lastIdx = data.length - 1;
    if(lastIdx < 0) return;
    var ex = pad.l + lastIdx*cw + cw/2, ey = yFn(data[lastIdx].low) + 1;
    ctx.save();
    if(_holdMini) ctx.globalAlpha = 0.5;
    ...
    ctx.lineTo(ex + markerSize*0.7, ey + markerSize*0.3);
    ctx.closePath(); ctx.fill();
    if(_holdMini){ ctx.setLineDash([2.5,2]); ctx.shadowColor=CURRENT_CLR; ctx.shadowBlur=4; ctx.strokeStyle=CURRENT_CLR; ctx.lineWidth=1.4; ctx.stroke(); }
    else _markerStrokeMini(ctx, CURRENT_CLR);
    ctx.restore();
  }
