// ════════════════════════════════════════════════════════════
//  SX Chart Module v1.5 — S96: 풀차트BT마커+미니/풀 마커 봉밀착
//  S95: drawMiniWithTrades(BT마커+날짜오버레이) 추가
...
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades){
  ...
  if(trades && trades.length){
    ...
    var BUY_C = '#22c55e', WIN_C = '#3b82f6', LOSS_C = '#e8365a';
    var ms = Math.max(cw * 0.6, 5);
    function fmtSh(d){ if(!d) return ''; var p=d.split(/[-T]/); return p.length>=3?parseInt(p[1],10)+'/'+parseInt(p[2],10):d.slice(5,10); }
    for(var ti=0;ti<trades.length;ti++){
      var tr=trades[ti];
      // 매수 ▲
      if(tr.entryIdx!=null){
        var eL=tr.entryIdx-offset;
        if(eL>=0&&eL<data.length){
          var ex=pad.l+eL*cw+cw/2, ey=yFn(data[eL].low)+1;
          ctx.fillStyle=BUY_C;
          ctx.beginPath(); ctx.moveTo(ex,ey); ctx.lineTo(ex-ms*0.7,ey+ms); ctx.lineTo(ex+ms*0.7,ey+ms); ctx.closePath(); ctx.fill();
          ctx.font='7px Outfit,sans-serif'; ctx.textAlign='center'; ctx.fillText(fmtSh(tr.entryDate),ex,ey+ms+9);
        }
      }
      // 매도 ▼
      if(tr.type!=='OPEN'&&tr.exitIdx!=null){
        var xL=tr.exitIdx-offset;
        if(xL>=0&&xL<data.length){
          var xx=pad.l+xL*cw+cw/2, xy=yFn(data[xL].high)-1;
          var clr=tr.type==='WIN'?WIN_C:LOSS_C;
          ctx.fillStyle=clr;
          ctx.beginPath(); ctx.moveTo(xx,xy); ctx.lineTo(xx-ms*0.7,xy-ms); ctx.lineTo(xx+ms*0.7,xy-ms); ctx.closePath(); ctx.fill();
          ctx.font='7px Outfit,sans-serif'; ctx.textAlign='center'; ctx.fillText(fmtSh(tr.exitDate),xx,xy-ms-3);
        }
      }
    }
    // 범례
    ctx.font='8px Outfit,sans-serif'; ctx.textAlign='right';
    ctx.fillStyle=LOSS_C; ctx.fillText('▼매도', W-pad.r-2, 10);
...
function drawMiniWithTrades(canvasId, rows, trades){
  ...
  // BT 마커 오버레이
  var BUY_CLR = '#22c55e';  // 초록
  var WIN_CLR = '#3b82f6';  // 파랑 (익절)
  var LOSS_CLR = '#e8365a'; // 빨강 (손절)
  var markerSize = Math.max(cw * 0.5, 4);
  ...
  var lastTradeIdx = trades.length - 1; // 가장 최근 거래만 날짜 표기

  for(var ti=0;ti<trades.length;ti++){
    var tr = trades[ti];
    var isLast = (ti === lastTradeIdx);
    // 진입 마커 ▲
    if(tr.entryIdx != null){
      var eLocal = tr.entryIdx - offset;
      if(eLocal >= 0 && eLocal < data.length){
        ...
        if(isLast){
          ...
          ctx.font = '6px Outfit,sans-serif';
          ctx.textAlign = 'center';
          ctx.fillText(fmtShort(tr.entryDate), ex, ey + markerSize*0.3 + 8);
        }
      }
    }
    // 청산 마커 ▼ (OPEN은 제외)
    if(tr.type !== 'OPEN' && tr.exitIdx != null){
      var xLocal = tr.exitIdx - offset;
      if(xLocal >= 0 && xLocal < data.length){
        var xx = pad.l + xLocal * cw + cw/2;
        var xy = yFn(data[xLocal].high) - 1;
        var clr = tr.type === 'WIN' ? WIN_CLR : LOSS_CLR;
        ctx.fillStyle = clr;
        ctx.beginPath();
        ...
        if(isLast){
          ...
          ctx.font = '6px Outfit,sans-serif';
          ctx.textAlign = 'center';
          ctx.fillText(fmtShort(tr.exitDate), xx, xy - markerSize*0.3 - 3);
        }
      }
    }
  }
  ...
  ctx.font = '7px Outfit,sans-serif';
  ctx.textAlign = 'right';
  ctx.fillStyle = BUY_CLR; ctx.fillText('▲매수', W-pad.r-2, 10);
  ctx.fillStyle = LOSS_CLR; ctx.fillText('▼매도', W-pad.r-32, 10);
}
