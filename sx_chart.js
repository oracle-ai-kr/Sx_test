...
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades, svVerdict){
  ...
  if(trades && trades.length){
    ...
    function _drawBuyMarker(idxGlobal, dateStr, isPair){
      ...
      if(dateStr){
        ctx.fillStyle = BUY_C; ctx.font = 'bold 16px Outfit,sans-serif'; ctx.textAlign = 'center';
        ctx.fillText(fmtSh(dateStr), ex, ey + ms*0.3 + 14);
      }
      ctx.restore();
    }
    function _drawSellMarker(idxGlobal, dateStr, isPair){
      ...
      if(dateStr){
        ctx.fillStyle = SELL_C; ctx.font = 'bold 16px Outfit,sans-serif'; ctx.textAlign = 'center';
        ctx.fillText(fmtSh(dateStr), xx, xy - ms*0.3 - 8);
      }
      ctx.restore();
    }
...
function drawMiniWithTrades(canvasId, rows, trades, svVerdict){
  ...
  function _drawBuyMini(idxGlobal, dateStr, isPair){
    ...
    if(dateStr){
      ctx.fillStyle = BUY_CLR; ctx.font = 'bold 14px Outfit,sans-serif'; ctx.textAlign = 'center';
      ctx.fillText(fmtShort(dateStr), ex, ey + markerSize*0.3 + 9);
    }
    ctx.restore();
  }
  function _drawSellMini(idxGlobal, dateStr, isPair){
    ...
    if(dateStr){
      ctx.fillStyle = SELL_CLR; ctx.font = 'bold 14px Outfit,sans-serif'; ctx.textAlign = 'center';
      ctx.fillText(fmtShort(dateStr), xx, xy - markerSize*0.3 - 4);
    }
    ctx.restore();
  }
