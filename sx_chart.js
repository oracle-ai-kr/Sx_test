...
function _drawFullCandle(id, data, closes, bb, H, fullLen, trades, svVerdict){
  ...
  if(trades && trades.length){
    ...
    // [POLICY C — 하이브리드 (사용자 요청 2026-04-29)] 풀차트 동일 적용
    //   ① BT 마커 (녹/적, 날짜) — 마지막 BT 거래 entry+exit
    //   ② 현재 판정 마커 (보라, 마지막 봉, 날짜 없음) — 통합판정
...
function drawMiniWithTrades(canvasId, rows, trades, svVerdict){
  ...
  // [POLICY 2026-04-29 정책 C — 하이브리드] 현재 판정 마커 (보라색, 날짜 라벨 없음)
  //   BT 거래 마커(녹/적 + 날짜)와 색상으로 구분. 마지막 봉 위치에 항상 그려짐.
  //   사용자 의도: "BT 이력은 그대로 두고, 배너의 현재 판정도 차트에 표시"
  ...
  // [POLICY C — 하이브리드 (사용자 요청 2026-04-29)]
  //   ① BT 마커 (기존 색상 + 날짜) — 마지막 BT 거래의 entry/exit를 그대로 표시
  //   ② 현재 판정 마커 (보라색 + 날짜 없음) — 통합판정의 매수/매도를 마지막 봉에 추가 표시
  //   장점: BT 이력 + 현재 신호 둘 다 보임. 색상으로 구분 → 시각 혼란 방지
  if(lastTr){
    // ── ① BT 마커 (기존 녹/적, 날짜 라벨 포함) ──
    //   lastTr가 OPEN(보유중): ▲ entry만
    //   lastTr가 WIN/LOSS(청산됨): ▲ entry + ▼ exit 쌍
    //   회피/관망/관심 액션이어도 BT 이력은 표시 (정보 손실 방지)
    if(lastTr.type === 'OPEN' && lastTr.entryIdx != null){
    ...
    // ── ② 현재 판정 마커 (보라색, 마지막 봉, 날짜 없음) ──
    //   통합판정의 chartMarker에 따라 ▲ 또는 ▼ 표시
    //   "관심/관망/회피" 액션은 매수/매도 신호 아님 → 표시 안 함
    if(showBuyMarker){
      _drawCurrentBuyMini();
  ...
  if(!svVerdict && lastTr && lastTr.type === 'OPEN' && lastTr.entryIdx != null){
    // OPEN인 경우는 위 BT 마커 분기에서 이미 처리됨 (svVerdict 있어도/없어도 동일)
    // 폴백이 따로 필요 없음 (회피/관망/관심 가드는 svVerdict 있을 때만)
  }

  drawMALegend(ctx, closes, pad, H, 8);
  // 마커 범례 — BT 마커(녹/적 + 날짜) + 현재 판정 마커(보라)
  ctx.font = '7px Outfit,sans-serif';
  ctx.textAlign = 'right';
