...
const _OPT_MODES = {
  profit:   { label:'🔥 수익형', desc:'종합 > 수익률 > 거래수 > MDD > 승률' },
  balanced: { label:'⚖️ 안정형', desc:'종합 > 거래수 > 수익률 > MDD > 승률' },
  safe:     { label:'🛡️ 보수형', desc:'종합 > MDD > 수익률 > 거래수 > 승률' },
};
// S91: 모드별 5단계 순차비교 정렬 (종합점수 1순위, 5% 허용 대역)
// 종합점수 차이 5% 이내 → 같은 그룹 → 2순위로 모드별 성격 결정
// profit:   종합점수 → 수익률 → 거래수 → MDD낮음 → 승률
// safe:     종합점수 → MDD낮음 → 수익률 → 거래수 → 승률
// balanced: 종합점수 → 거래수 → 수익률 → MDD낮음 → 승률
function _optSortCompare(a, b, mode){
  const scoreA = _optSortScore(a,'balanced');
  const scoreB = _optSortScore(b,'balanced');
  const maxScore = Math.max(Math.abs(scoreA), Math.abs(scoreB), 0.01);
  const scoreDiff = Math.abs(scoreA - scoreB) / maxScore; // 상대적 차이
  if(scoreDiff > 0.05) return scoreA - scoreB; // 1순위: 종합점수 (5% 초과 시)
  // 5% 이내 → 같은 그룹 → 모드별 2순위
  ...
  if(mode==='profit'){
    if(Math.abs(pnl) > 0.01) return pnl;            // 2: 수익률
  ...
  if(mode==='safe'){
    ...
    if(Math.abs(pnl) > 0.01) return pnl;            // 3: 수익률
    if(tr !== 0) return tr;                          // 4: 거래수
    return wr;                                        // 5: 승률
  }
  // balanced
...
function _optRenderModeGate(){
  ...
  if(!gate) return;
  const modeDescs = {
    profit: '종합 > 수익률 > 거래수 > MDD > 승률',
    balanced: '종합 > 거래수 > 수익률 > MDD > 승률',
    safe: '종합 > MDD > 수익률 > 거래수 > 승률'
  };
...
function _buildMergedCards(tfResOff, tfResOn, tfLabels, code, label){
  ...
  for(const tf of allTfs){
    ...
    const _modeFormula = {profit:'종합 > 수익률 > 거래수 > MDD > 승률', balanced:'종합 > 거래수 > 수익률 > MDD > 승률', safe:'종합 > MDD > 수익률 > 거래수 > 승률'};
