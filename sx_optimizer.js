...
async function _optRun(){
  ...
  _running = true;
  // [S563] 옵티마이저 실행 중 정배열 게이트 모드 = 탐색 모드(_optSortMode). 종료 시 해제.
  try { if(typeof SXE !== 'undefined') SXE._btModeOverride = _optSortMode; } catch(_){}
  _cancelled = false;
...
async function _optToggleRepresentCore(market, entryId){
  ...
  if(bucket[repField] === entryId){
    ...
    if(entry.params){
      // [S563] 대표 프리셋이 발견된 탐색 모드를 슬롯에 동봉 → 분석탭/스캐너/실시간 BT 정배열 게이트가
      //   대표와 동일하게 따라감(A 일관성). sxRunBtEngine이 활성 슬롯 _mode를 읽어 게이트 적용.
      //   _getSlotParams는 {...SCR_ANAL_DEFAULTS, ...slot} 스프레드라 _mode 보존됨(DEFAULTS엔 _mode 없음).
      if(entry.mode === 'profit' || entry.mode === 'balanced' || entry.mode === 'safe'){
