...
function _optSortScore(bt, mode){
  // 정규화 (0~100 스케일)
  const pnl = Math.max(bt.totalPnl || 0, 0);         // 수익률 (%)
  const trades = Math.max(bt.totalTrades || 0, 0);    // 거래수
  const wr = Math.max(bt.winRate || 0, 0);            // 승률 (%)
  const mddSafe = 100 - Math.min(bt.mdd || 0, 100);  // MDD 안전도 (100=MDD 0%, 0=MDD 100%)

  if(mode === 'profit'){
    // 수익형: 수익률 > 거래수 > MDD
    return pnl * 0.50 + trades * 0.30 + mddSafe * 0.20;
  }
  if(mode === 'safe'){
    // 보수형: MDD안전 > 승률 > 거래수
    return mddSafe * 0.45 + wr * 0.35 + trades * 0.20;
  }
  // balanced (안정형): 중간 균형
  return pnl * 0.30 + wr * 0.25 + trades * 0.25 + mddSafe * 0.20;
}
...
function _optRenderModeGate(){
  ...
  if(!gate) return;
  const modeDescs = {
    profit: '수익률 50% + 거래수 30% + MDD 20% — 거래 많고 수익 최고',
    balanced: '수익률 30% + 승률 25% + 거래수 25% + MDD 20% — 중간 균형',
    safe: 'MDD 45% + 승률 35% + 거래수 20% — 안전 승률 우선'
  };
