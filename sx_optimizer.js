  ...
  overlay.innerHTML = `<div class="opt-panel">
    ...
    <div class="opt-body">
      ...
      <div class="opt-section" id="optControlSection">
        ...
        <div id="optModeDesc" style="font-size:9px;color:var(--text3,#999);margin-bottom:10px;text-align:center"></div>
        <!-- S90: 커스텀 가중치 (접기 카드) -->
        ...
        <!-- [S587→S588] MA크로스 게이트 — 종합점수 비중 카드 아래로 이동(커스텀↔비중표 인접 유지). 모드와 독립 on/off. -->
...
async function _optClearBestCore(){
  // [S588] 현재 화면 시장(_optBestViewMarket)의 기록 전체 삭제 — 대표·슬롯까지 클린(기본값 복원).
  //   〔변경〕 이전엔 ★대표를 보존하고 나머지만 삭제했으나, 사용자 요청으로 기록 초기화 = 시장 통째 삭제로 통일.
  //          (리스트는 유지하고 대표만 해제하려면 '프리셋 초기화'를 사용 — 역할 분리.)
  const mktNames = {kr:'국내', us:'해외', crypto:'코인'};
  const mktIcons = {kr:'🇰🇷', us:'🇺🇸', crypto:'🪙'};
  const all = _loadOptBest();
  ...
  const delCount = bucket.ranks.length;
  const hadRep = !!(bucket.representOnId || bucket.representOffId);

  const _conf = (typeof window !== 'undefined' && window.sxConfirm) ? window.sxConfirm : (m=>Promise.resolve(confirm(m)));
  if(!await _conf(`[${mktLabel}] 최고기록 ${delCount}개를 모두 삭제합니다.\n${hadRep ? '★ 대표도 함께 해제되고 ' : ''}시장 슬롯이 기본값으로 복원됩니다.\n\n계속하시겠습니까?`)) return;

  // 1) 기록 버킷 통째 삭제 (대표 id 포함)
  delete all[targetMkt];
  _saveOptBest(all);
  // 2) 매트릭스 슬롯 비움 (양 레짐) → _getSlotParams 하드코딩 폴백(순수)
  if(typeof SXE !== 'undefined' && SXE._loadParamsMatrix && SXE._saveParamsMatrix){
    const matrix = SXE._loadParamsMatrix();
    if(matrix[targetMkt]){ delete matrix[targetMkt]; SXE._saveParamsMatrix(matrix); }
  }
  if(typeof loadAnalParamsUI === 'function') loadAnalParamsUI();
  _optRenderBestCards();
  toast(`${mktLabel} 기록 초기화 · 대표/슬롯 복원`);
}
