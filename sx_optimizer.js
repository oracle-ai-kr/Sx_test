...
function _renderDualResults(tfResultsOff, tfResultsOn, code, r2Off, r2On){
  ...
  // TF별로 OFF+ON 합산 순위
  const allTfs = new Set([...Object.keys(tfResultsOff), ...Object.keys(tfResultsOn)]);
  for(const tf of allTfs){
    const tfLabel = tfLabels[tf] || tf;
    html += `<div class="opt-tf-header">📊 ${tfLabel}</div>`;

    // OFF/ON 결과 합치기 (regime 태그 부착)
    const merged = [];
    const addResults = (results, regime) => {
      if(!results || !Array.isArray(results)) return;
      if(results.length===1 && results[0].error) return;
      results.filter(r=>r.bt && r.bt.totalTrades>=3).forEach(r=>{
        merged.push({ params:r.params, bt:r.bt, regime });
      });
    };
    addResults(tfResultsOff[tf], 'OFF');
    addResults(tfResultsOn[tf], 'ON');

    if(merged.length===0){
      html += `<div class="opt-result-card"><div style="color:var(--text3);font-size:11px">유효한 결과 없음</div></div>`;
      continue;
    }
    ...
    if(merged.length > 10){
      html += `<div class="opt-info" style="text-align:center;margin-top:4px">전체 유효 조합: ${merged.length}개 (상위 10개 표시)</div>`;
    }
  }
  ...
  if(r2Off && r2On){
    ...
    const allTfs2 = new Set([...Object.keys(r2Off), ...Object.keys(r2On)]);
    for(const tf of allTfs2){
      const tfLabel = tfLabels[tf] || tf;
      html += `<div class="opt-tf-header">📊 ${tfLabel} (2차)</div>`;
      const merged2 = [];
      const add2 = (results, regime) => {
        if(!results || !Array.isArray(results)) return;
        if(results.length===1 && results[0].error) return;
        results.filter(r=>r.bt && r.bt.totalTrades>=3).forEach(r=>{
          merged2.push({ params:r.params, bt:r.bt, regime });
        });
      };
      add2(r2Off[tf], 'OFF');
      add2(r2On[tf], 'ON');
      ...
      top2.forEach((r,i)=>{
        ...
        const regTag = r.regime==='ON'
          ? '<span style="font-size:9px;padding:2px 5px;border-radius:3px;background:rgba(37,99,235,0.1);color:var(--accent,#2563eb);font-weight:600;margin-left:6px">⚡ON</span>'
          : '<span style="font-size:9px;padding:2px 5px;border-radius:3px;background:var(--surface2,#eee);color:var(--text3,#999);font-weight:600;margin-left:6px">🔒OFF</span>';
        html += `<div class="${cardClass}"><div style="display:flex;align-items:center"><span class="opt-rank ${rankClass}">${i+1}</span><span style="font-size:12px;font-weight:700">TOP ${i+1}</span>${regTag}</div><div class="opt-result-params">${paramStr}</div><div class="opt-result-stats"><div><div class="stat-val">${b.winRate.toFixed(1)}%</div><div class="stat-lbl">승률</div></div><div><div class="stat-val" style="color:${pnlColor}">${b.totalPnl>=0?'+':''}${b.totalPnl.toFixed(1)}%</div><div class="stat-lbl">수익률</div></div><div><div class="stat-val">${b.totalTrades}</div><div class="stat-lbl">거래수</div></div><div><div class="stat-val">${b.mdd}%</div><div class="stat-lbl">MDD</div></div></div><div class="opt-result-actions"><button class="opt-btn-primary" style="padding:6px;font-size:10px;border-radius:5px" onclick="_optApply(${JSON.stringify(p).replace(/"/g,'&quot;')})">슬라이더 적용</button><button class="opt-btn-secondary" style="padding:6px;font-size:10px;border-radius:5px" onclick="_optSaveSlot(${JSON.stringify(p).replace(/"/g,'&quot;')},'${tf}','${_esc(code)}${suffix}')">프리셋 저장</button></div></div>`;
      });
      if(merged2.length>10) html += `<div class="opt-info" style="text-align:center;margin-top:4px">전체: ${merged2.length}개 (상위 10개)</div>`;
    }
  }
