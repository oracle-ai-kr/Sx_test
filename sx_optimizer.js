...
const _OPT_MODES = {
  profit:   { label:'🔥 수익형', desc:'수익률 중심 종합 > 수익률 > 거래수 > MDD > 승률' },
  balanced: { label:'⚖️ 안정형', desc:'거래수 중심 종합 > 거래수 > 수익률 > MDD > 승률' },
  safe:     { label:'🛡️ 보수형', desc:'MDD 중심 종합 > MDD > 수익률 > 거래수 > 승률' },
};
// S91: 모드별 종합점수 + 5% 허용 대역 + 4단계 타이브레이커
// 수익형: 수익률^1.5 × log(거래수+1) × (1-MDD/100)^0.5 × 승률^0.5
// 보수형: 수익률^0.5 × log(거래수+1) × (1-MDD/100)^2.0 × 승률
// 안정형: 수익률 × log(거래수+1)^1.5 × (1-MDD/100) × 승률^0.7
function _optSortScore(bt, mode){
  const w = Math.max(bt.winRate||0, 0.01) / 100;
  const p = Math.max(bt.totalPnl||0, 0.01);
  const t = Math.log((bt.totalTrades||0) + 1);
  const m = Math.max(1 - (bt.mdd||0) / 100, 0.01);
  if(mode==='profit') return Math.pow(p, 1.5) * t * Math.pow(m, 0.5) * Math.pow(w, 0.5);
  if(mode==='safe')   return Math.pow(p, 0.5) * t * Math.pow(m, 2.0) * w;
  /* balanced */       return p * Math.pow(t, 1.5) * m * Math.pow(w, 0.7);
}

// 정렬: 모드별 종합점수 단일 비교 (가중치가 이미 모드 성격을 반영)
function _optSortCompare(a, b, mode){
  return _optSortScore(a, mode) - _optSortScore(b, mode);
}
...
function _optRenderModeGate(){
  ...
  if(!gate) return;
  const modeDescs = {
    profit: '수익률 중심 종합 > 수익률 > 거래수 > MDD > 승률',
    balanced: '거래수 중심 종합 > 거래수 > 수익률 > MDD > 승률',
    safe: 'MDD 중심 종합 > MDD > 수익률 > 거래수 > 승률'
  };
...
function _buildMergedCards(tfResOff, tfResOn, tfLabels, code, label){
  ...
  for(const tf of allTfs){
    ...
    const _scoreFormula = {profit:'p^1.5×t×m^0.5×w^0.5', balanced:'p×t^1.5×m×w^0.7', safe:'p^0.5×t×m^2×w'};
    html += `<div style="font-size:9px;color:var(--text3,#999);text-align:center;margin-top:2px;margin-bottom:6px">정렬: ${_modeFormula[_optSortMode]||''} | 종합(${_scoreFormula[_optSortMode]||''})</div>`;
