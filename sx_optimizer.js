...
// [v3.18 PF-AXIS-INPUT] 기본값 3 → 0 (OFF 기본 명확화 — 입력 박스 빈칸 = OFF = 0)
//   이전: 3 (사용자가 명시 ON 안 해도 거래 3건 미만은 자동 제외)
//   변경: 0 (입력 박스 비우면 모든 결과 통과 → 사용자 능동 설정 강조)
// [v3.19 DEFAULT-20] 기본값 20 (일봉 추천값) — 사용자가 필요시 빈칸으로 비우거나 다른 값 입력
...
//   사유: 사용자 요청 — PF 중시 정렬을 원하지만 PF 단독 정렬은 거래수 적은 우연한 결과가
//        1위로 올라오는 위험 존재 (예: 거래 3회 PF 10.0 vs 거래 50회 PF 2.5)
//   설계:
...
//   가중치 변경 (이전 → 현재):
//     수익형: pnl 45→30, wr 35→35, mdd 15→15, trades 5→5, +pf 15
//     안정형: pnl 35→25, wr 45→40, mdd 15→15, trades 5→5, +pf 15
//     보수형: pnl 15→10, wr 35→30, mdd 45→40, trades 5→5, +pf 15
...
function _loadOptBest(){
  try{
    ...
    // [v3.21 REP-SPLIT] representId 단일 → representOnId/representOffId 분리 마이그레이션
    //   사유: 분석엔진 매트릭스(SX_SCR_ANAL_PARAMS_MATRIX_V2)는 시장×레짐 6슬롯 구조
    //         (kr.on/kr.off/us.on/us.off/crypto.on/crypto.off)
    //         그런데 옵티마이저는 시장당 1대표만 허용 → ON 슬롯과 OFF 슬롯을 동시에
    //         사용자 의도대로 채울 방법이 없었음.
    //   변경: 시장당 ON 대표 + OFF 대표 = 2대표 허용
    //         → 분석엔진 6슬롯 구조와 1:1 매칭됨
    //   마이그레이션: 기존 representId가 가리키던 entry의 regime을 보고
    //                ON이면 representOnId, OFF면 representOffId로 이동
    Object.keys(data).forEach(mktKey => {
      const bucket = data[mktKey];
      if(!bucket) return;
      // 단일 representId가 있으면 분리
      if(bucket.representId !== undefined){
        if(bucket.representId && bucket.ranks){
          const ent = bucket.ranks.find(r => r.id === bucket.representId);
          if(ent){
            const isOn = (ent.regime === 'ON' || ent.regime === true);
            if(isOn) bucket.representOnId = bucket.representId;
            else bucket.representOffId = bucket.representId;
          }
        }
        delete bucket.representId; // 정리
      }
      // 누락 필드 초기화 (구버전 데이터 호환)
  ...
  if(bucket[repField] === entryId){
    ...
    if(entry.params){
      if(typeof SXE !== 'undefined' && SXE._saveSlotParams){
        SXE._saveSlotParams(mktKey, isOn, entry.params);
