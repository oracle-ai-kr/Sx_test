...
function _optRenderBestCards(){
  ...
  sorted.forEach(tf=>{
    ...
    const modeLabels = {profit:'🔥수익', balanced:'⚖️안정', safe:'🛡️보수'};
    const modeColors = {profit:'var(--sell,#e74c3c)', balanced:'var(--accent,#2563eb)', safe:'var(--buy,#27ae60)'};

    // 호환: 구버전 데이터(모드 없는 flat 구조) 처리
    const entries = {};
    if(tfData.params){
      // 구버전: flat → balanced로 취급
    ...
    html += `<div style="background:var(--surface2,#f8f8f8);border-radius:8px;padding:8px 10px;margin-bottom:4px;border:1px solid var(--border,#e0e0e0)">
      <div style="font-size:11px;font-weight:700;margin-bottom:4px">📊 ${tfLabel}</div>`;
    ...
    Object.entries(entries).forEach(([mode, e])=>{
      ...
      html += `<div style="display:flex;align-items:center;gap:6px;padding:3px 0;border-top:1px solid var(--border,#eee)">
        <span style="font-size:9px;font-weight:700;color:${modeColors[mode]||'#666'};min-width:42px">${modeLabels[mode]||mode}</span>
        <span style="font-size:8px;color:var(--text3,#999);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${paramStr} ${regTag}</span>
        <span style="font-size:9px;color:var(--text2,#666);white-space:nowrap">${(e.winRate||0).toFixed(0)}% <span style="color:${pnlColor}">${(e.totalPnl||0)>=0?'+':''}${(e.totalPnl||0).toFixed(0)}%</span> MDD${e.mdd||0}%</span>
        <span style="font-size:9px;font-weight:700;color:${modeColors[mode]||'#666'}">${(e.score||0).toFixed(0)}pt</span>
      </div>`;
    });
