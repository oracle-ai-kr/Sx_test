...
const OPT_DEFAULTS = {
  ...
  buyTh:   { min:30,  max:50,  step:1,   enabled:false },  // 1. 진입 점수 임계
  bbLen:   { min:14,   max:14,  step:0,   enabled:false },  // 2. BB 기간 (신호 빈도)
  bbMult:  { min:1.9, max:1.9, step:0, enabled:false },  // 3. BB 배수 (진입 폭)
  // [청산 결정] — 진입 안정화 후 최적화
  tpMult:  { min:1.0, max:15.0, step:0.5, enabled:false },  // 4. 익절 폭
  slMult:  { min:0.5, max:5.0, step:0.5, enabled:false },  // 5. 손절 폭
  sellTh:  { min:20,  max:40,  step:1,   enabled:false },  // 6. 청산 점수
  // [지표 기간] — 표준값 고정 (step=0)
...
const OPT_MARKET_MODE_RANGES = {
  ...
  profit: {
    kr:   { bbLen:{min:14, max:14, step:0}, bbMult:{min:1.9, max:1.9, step:0}, buyTh:{min:25, max:40, step:1}, sellTh:{min:20, max:35, step:1}, tpMult:{min:1.0, max:1.5, step:0.1}, slMult:{min:0.5, max:0.8, step:0.1} },
    us:   { bbLen:{min:20, max:20, step:0}, bbMult:{min:2.0, max:2.0, step:0}, buyTh:{min:20, max:35, step:1}, sellTh:{min:15, max:30, step:1}, tpMult:{min:0.7, max:1.0, step:0.1}, slMult:{min:0.3, max:0.5, step:0.1} },
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:30, max:45, step:1}, sellTh:{min:25, max:40, step:1}, tpMult:{min:1.5, max:2.5, step:0.1}, slMult:{min:0.8, max:1.2, step:0.1} },
  },
  // ⚖️ 스윙 (balanced): 중간 여유
  balanced: {
    kr:   { bbLen:{min:14, max:14, step:0}, bbMult:{min:1.9, max:1.9, step:0}, buyTh:{min:35, max:50, step:1}, sellTh:{min:30, max:45, step:1}, tpMult:{min:4.0, max:6.0, step:0.1}, slMult:{min:2.0, max:3.0, step:0.1} },
    us:   { bbLen:{min:20, max:20, step:0}, bbMult:{min:2.0, max:2.0, step:0}, buyTh:{min:40, max:55, step:1}, sellTh:{min:35, max:45, step:1}, tpMult:{min:5.0, max:8.0, step:0.1}, slMult:{min:2.5, max:4.0, step:0.1} },
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:35, max:45, step:1}, sellTh:{min:30, max:45, step:1}, tpMult:{min:8.0, max:12.0, step:0.1}, slMult:{min:4.0, max:6.0, step:0.1} },
  },
  // 🛡️ 중기 (safe): 넓은 여유, 국내/해외 동일 · 코인만 별도
  safe: {
    kr:   { bbLen:{min:14, max:14, step:0}, bbMult:{min:1.9, max:1.9, step:0}, buyTh:{min:50, max:65, step:1}, sellTh:{min:45, max:60, step:1}, tpMult:{min:6.0,  max:9.0,  step:0.1}, slMult:{min:2.0, max:3.0, step:0.1} },
    us:   { bbLen:{min:20, max:20, step:0}, bbMult:{min:2.0, max:2.0, step:0}, buyTh:{min:50, max:65, step:1}, sellTh:{min:45, max:60, step:1}, tpMult:{min:6.0,  max:9.0,  step:0.1}, slMult:{min:2.0, max:3.0, step:0.1} },
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:55, max:70, step:1}, sellTh:{min:50, max:65, step:1}, tpMult:{min:12.0, max:18.0, step:0.1}, slMult:{min:4.0, max:6.0, step:0.1} },
  },
};
...
const _OPT_MODE_WEIGHTS = {
  profit:   { pnl:80,  trades:5, mdd:5, wr:5, pf:5  },  // [S292] 수익률 100% 집중
  balanced: { pnl:80,  trades:5, mdd:5, wr:5, pf:5  },  // [S292] 승률·수익률·MDD 균형
  safe:     { pnl:80,  trades:5, mdd:5, wr:5, pf:5  }   // [S292] MDD 단독 1위, 승률 2위
};
...
const _OPT_MODE_TIEBREAK_DEFAULTS = {
  profit:   { pnl:1, trades:4, mdd:2, wr:3, pf:5 },
  balanced: { pnl:1, trades:5, mdd:2, wr:3, pf:4 },
  safe:     { pnl:1, trades:5, mdd:3, wr:2, pf:4 }
};
...
const _OPT_MODES = {
  profit:   { label:'🔥 단타', desc:'수익률 > PF > 승률 > MDD' },
  balanced: { label:'⚖️ 스윙', desc:'MDD↓ ≈ 승률 > 수익률 ≈ PF (균형)' },
  safe:     { label:'🛡️ 중기', desc:'MDD↓ > PF ≈ 승률 > 수익률' },
};
...
// ════════════════════════════════════════════════════════════
// [S292] 탐색 모드 → 조기청산 설정 동기화
//   수익형: 조기청산 ON + 트레일(진입즉시)  + BT 종가/진입고정
//   안정형: 조기청산 ON + 트레일(수익권후)  + BT 종가/진입고정
//   보수형: 조기청산 OFF                   + BT 종가/진입고정
//   * 나머지(점수/본전/시간청산/각 수치)는 현재 설정값 그대로 유지
// ════════════════════════════════════════════════════════════
function _optSyncEarlyExitToMode(mode){
  if(typeof SXE === 'undefined') return;

  // BT 청산 동작 모드 — 3모드 공통: 종가(close) + 진입시 고정(entry)
  if(typeof SXE.setBtExitMode     === 'function') SXE.setBtExitMode('close');
  if(typeof SXE.setBtTrailAtrMode === 'function') SXE.setBtTrailAtrMode('entry');

  var ee = SXE._btEarlyExit;
  if(!ee || typeof ee !== 'object') return;

  // [S302] 사용자 결정 (3모드별 사진 캡처 기준) — 검증 마친 추가 청산 룰까지 전체 자동 매핑
  //   설계 원칙:
  //     · enabled / breakeven / trailing : 3모드 공통 ON (마스터·본전·트레일은 항상 사용)
  //     · score : profit만 ON (스윙·중기는 매도 점수 변동에 흔들리지 않게 OFF)
  //     · 추가 청산 룰(bbUpper/maBreak/nHigh): 모드별 차별 (사진 기준)
  //     · 슬라이더 파라미터(beTrigger/trailMult/trailTrigger/maxBars/timeMinPnl): 3모드 동일
  //   〔이력〕
  //     [S292] enabled+trailing+trailMode 3개만 매핑, safe는 enabled=false
  //     → [S300] 추가 청산 룰(bbUpper/maBreak/nHigh) 신설, 자동 연동은 의도적으로 제외 (사용자 수동 검증)
  //     → [S302] 사용자 검증 완료 후 사진 기준으로 전체 자동 매핑 활성화 (safe도 enabled=true)
  //   주의: maBreakLen=120, nHighBars=60은 사용자 결정값. 권장 텍스트(스윙=20)와 다름.
  ...
  if(mode === 'profit'){
    // 🔥 단타: 점수 ON + 트레일 진입즉시 + 추가 룰 3개 모두 ON (가장 공격적)
    ee.score      = true;
    ee.trailMode  = 'immediate';
    ee.bbUpper    = true;
    ee.maBreak    = true;
    ee.maBreakLen = 20;
    ee.nHigh      = true;
    ...
    // ⚖️ 스윙: 점수 OFF + 트레일 수익권 후 + MA120 이탈 + N봉 60 신고점 (휩소 방지 보수적)
    ee.score      = false;
    ee.trailMode  = 'profit';
    ee.bbUpper    = false;
    ee.maBreak    = true;
    ...
    // 🛡️ 중기: 점수 OFF + 트레일 수익권 후 + MA120 이탈만 (가장 보수)
    ee.score      = false;
    ee.trailMode  = 'profit';
    ee.bbUpper    = false;
    ee.maBreak    = true;
  ...
  console.log(`[S302] 조기청산 동기화: mode=${mode} → enabled=${ee.enabled}, score=${ee.score}, trail=${ee.trailMode}, bb=${ee.bbUpper}, ma=${ee.maBreak}(${ee.maBreakLen}), nHi=${ee.nHigh}(${ee.nHighBars})`);
}
...
// [S301] defaults를 _OPT_MODE_WEIGHTS (L778~) 기본값과 동기화 — 3모드 모두 80/5/5/5/5
//   배경: 이전엔 _optResetWeights의 defaults가 _OPT_MODE_WEIGHTS와 어긋나 있어서
//        초기화 버튼 누르면 balanced 30/40 / safe 20/40 같은 옛 값으로 되돌아갔음.
//   현재: 3모드 동일하게 "수익률 80% + 4축 각 5%" — 순위(타이브레이커)만 모드별로 다름.
//   주의: _OPT_MODE_WEIGHTS 변경 시 반드시 여기 defaults도 같이 수정.
function _optResetWeights(){
  _optVib(15);
  const defaults = {
    profit:   { pnl:80, trades:5, mdd:5, wr:5, pf:5 },
    balanced: { pnl:80, trades:5, mdd:5, wr:5, pf:5 },
    safe:     { pnl:80, trades:5, mdd:5, wr:5, pf:5 }
  };
...
async function _optToggleRepresentCore(market, entryId){
  ...
  if(bucket[repField] === entryId){
    ...
    _optRenderBestCards();
    // [S293] 대표 선정 → 탐색 모드 버튼 + 조기청산 3-way 동기화
    //   entry.mode = 카드 발견 당시 사용된 정렬 가중치 모드 ('profit'/'balanced'/'safe')
    //   chain: ★대표 → _optSortMode 갱신 → _optRenderModeGate → _optSyncEarlyExitToMode
    if(entry.mode && ['profit','balanced','safe'].includes(entry.mode)){
      _optSortMode = entry.mode;
      // [S304] window._optSortMode 동기화 — _optSetModeCore와 동일 패턴
      //   배경: _eeRenderModeRuleCard()가 window._optSortMode를 읽어서 청산룰 카드 텍스트를 그림.
      //        이 동기화 누락 시 SXE._btEarlyExit는 정확히 갱신되지만 (BT는 적용됨),
      //        조건검색 탭/모달의 청산룰 카드는 옛 모드 그대로 → 사용자가 "연동 안 됨"이라고 느낌.
      //   주의: _optSortMode = ... 변경 지점 모두 window 동기화 필수 (현재 _optSetModeCore, 여기 2곳)
      if(typeof window !== 'undefined') window._optSortMode = entry.mode;
      _optCustomWeights = null; // 커스텀 가중치 초기화 (모드 기준으로 정렬 일관성 보장)
      _optRenderModeGate();
      _optSyncEarlyExitToMode(entry.mode);
      const _mSyncLabels = {profit:'🔥 단타', balanced:'⚖️ 스윙', safe:'🛡️ 중기'};
      console.log(`[S293] 대표 선정 동기화: mode=${entry.mode} → ${_mSyncLabels[entry.mode]} 모드 + 조기청산 적용`);
    }
    // [S294] 대표 선정 → 레짐 ON/OFF 버튼 동기화
    //   isOn = 선정된 카드의 레짐 방향 (ON레짐 대표 → ON, OFF레짐 대표 → OFF)
    //   _optSetRegime 내부에서 SXE.setRegimeAdapt + 설정탭 토글 + 분석탭 재분석까지 일괄 처리
    _optSetRegime(isOn ? 'on' : 'off');
    console.log(`[S294] 레짐 동기화: ${isOn ? '⚡ON' : '⚡OFF'} 대표 선정 → 레짐 버튼 ${isOn ? 'ON' : 'OFF'}`);
