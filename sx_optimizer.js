// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v3.13 — 파라미터 자동 최적화 모듈
//  S113-f: BUY도 동반자로 체크 가능하도록 개선
//    채희창 피드백: "BUY도 체크가 되야 할거같아"
//    [변경]
//      - UI: BUY 체크박스 disabled 제거 (활성화)
//      - 로직: BUY 체크 거부 코드 제거
//      - 필터: BUY 제외 filter 제거
//      - useCompanion 조건: (paramKey !== 'buyTh') → (paramKey !== _companionKey)
//      - UI 안내: "BUY 이후 Step" → "모든 Step"
//    [BUY 체크 시 동작]
//      Step 1: BUY 단계 SKIP (동반 중이라)
...
//      총 BT: ~1,519회 (기존 순차 80회 + BUY와 모든 파라미터 상호작용)
//  v3.12 <!-- S113-e: 동반자 탐색 (라디오 + BUY 제외) -->
...
window._optUpdateCount = function(){
  ...
  if(_optCheckedParams && _optCheckedParams.size > 0){
    ...
    if(checkedArr.length > 0){
      ...
      const compSteps = (!compR || compR.step <= 0 || compR.max <= compR.min) ? 1 : (Math.round((compR.max - compR.min) / compR.step) + 1);
      ...
      Object.keys(ranges).forEach(k => {
        ...
        const rk = ranges[k];
        const kSteps = (rk.step <= 0 || rk.max <= rk.min) ? 1 : (Math.round((rk.max - rk.min) / rk.step) + 1);
