  ...
  overlay.innerHTML = `<div class="opt-panel">
    ...
    <div class="opt-body">
      ...
      <div class="opt-section">
        ...
        <div class="opt-info">시장: ${_PARAM_MARKET_LABELS_SHORT[market]} · <span id="optStockCount">${stocks.length}</span>/${OPT_MAX_STOCKS}개 · 칩 탭하면 삭제</div>
      </div>
...
async function _optRun(){
  ...
  const market = typeof currentMarket !== 'undefined' ? currentMarket : 'kr';
  const stocks = _getOptStocks(market);
  const code = stocks.length > 0 ? stocks[0].code : '';
  if(!code){ toast('대표 종목을 추가하세요'); return; }
...
// ═══════════════════════════════════════════
//  S77: 종목 검색 드롭다운 + 종목 탐색 패널 + 칩 관리
// ═══════════════════════════════════════════

// 드롭다운 검색 (내 필터탭 onSxStockInput과 동일 패턴)
function _optOnStockInput(){
  ...
  const pool = (typeof _getSxUniverse==='function') ? _getSxUniverse() : [];
...
// 종목 탐색 패널(▼ 버튼) — 기존 sxAzPanel 재사용
let _optPickerCallback = null;
function _optToggleAzPanel(){
  const market = typeof currentMarket !== 'undefined' ? currentMarket : 'kr';
  const stocks = _getOptStocks(market);
  if(stocks.length >= OPT_MAX_STOCKS){ toast(`최대 ${OPT_MAX_STOCKS}개`); return; }
  _optPickerCallback = function(code, name){
    _optPickStock(code, name);
    _optPickerCallback = null;
  };
  if(typeof toggleSxAzPanel === 'function') toggleSxAzPanel();
  else { toast('종목 탐색 패널을 사용할 수 없습니다'); _optPickerCallback = null; }
}
...
function _optPickStock(code, name){
  ...
  const market = typeof currentMarket !== 'undefined' ? currentMarket : 'kr';
  const ok = _addOptStock(market, code, name || code);
  if(ok){
    _optRenderChips();
    toast(`✅ ${name||code} 추가됨`);
  } else {
    toast('이미 등록됨 또는 최대 초과');
  }
}
...
function _optRenderChips(){
  ...
  area.innerHTML = stocks.map(s =>
    `<span style="display:inline-flex;align-items:center;gap:4px;padding:4px 8px;border-radius:12px;font-size:10px;background:var(--surface2,#f0f0f0);color:var(--text,#222);border:1px solid var(--border,#ddd);cursor:pointer" onclick="_optRemoveChip('${s.code}')" title="탭하면 삭제">${s.name} <span style="color:var(--text3)">${s.code}</span> ✕</span>`
  ).join('');
}
