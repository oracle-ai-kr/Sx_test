...
// ── 기본 설정 ──
const OPT_DEFAULTS = {
  rsiLen:  { min:5,  max:30, step:5,   enabled:true },
  bbLen:   { min:10, max:40, step:10,  enabled:true },
  ...
  buyTh:   { min:40, max:80, step:10,  enabled:true },
...
function _optSetMode(mode, btn){
  ...
  else { document.querySelectorAll('.opt-mode-btn').forEach(b=>{ if(mode==='quick'&&b.textContent.includes('빠른')) b.classList.add('active'); if(mode==='full'&&b.textContent.includes('정밀')) b.classList.add('active'); }); }
  const desc = document.getElementById('optModeDesc');
  if(mode==='quick'){
    if(desc) desc.textContent = '핵심 3개 파라미터만 탐색합니다 (RSI, BB기간, BUY임계)';
    ['rsiLen','bbLen','buyTh'].forEach(k=>{ const el=document.getElementById('optEn_'+k); if(el) el.checked=true; });
    ['bbMult','atrLen','maShort','maMid','maLong','sellTh','tpMult','slMult'].forEach(k=>{ const el=document.getElementById('optEn_'+k); if(el) el.checked=false; });
  } else {
    if(desc) desc.textContent = '전체 11개 파라미터를 조합 탐색합니다 (시간이 오래 걸릴 수 있음)';
