// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v3.10 — 파라미터 자동 최적화 모듈
//  S113-c: 체크박스 하이브리드 탐색 — 각 파라미터 행 왼쪽에 체크박스 추가
//    - 체크됨 = 전체 조합 그리드 서치 (Phase 1)
//    - 체크 안됨 = 기존 순차 탐색 (Phase 2)
//    - 🔒 잠김 = 탐색 제외 (고정값 유지)
//    - Phase 1 완료 후 최고 조합을 baseParams에 반영 → Phase 2 진행
//    - 예상 시간 표시: "☑️ 전체탐색: BUY × TP × SL = 3,069 조합 (약 30초)"
//    - 채희창 설계: "체크된 것만 전체 조합, 나머지는 순차"
//    - 효과: 핵심 파라미터 상호작용 완전 탐색 + 나머지는 빠른 순차
//  v3.9 <!-- S113-b: 스톱워치 추가 -->
...
function _saveStockSets(market, sets){
  ...
  all[market] = sets;
  localStorage.setItem(OPT_STOCK_SETS_KEY, JSON.stringify(all));
}
...
function _saveActiveSetIdx(market, idx){
  ...
  all[market] = idx;
  localStorage.setItem(OPT_STOCK_ACTIVE_SET_KEY, JSON.stringify(all));
}
...
function _optLoadStockSet(idx){
  ...
  if(typeof _renderStockChips === 'function') _renderStockChips();
  _renderStockSetBar();
...
function openOptimizer(){
  ...
  const paramRows = Object.entries(OPT_DEFAULTS).map(([k,d])=>{
    ...
    // S113-c: 체크박스 (전체 탐색 대상 선택)
    //   체크됨 = 전체 조합 그리드 서치 대상 (하이브리드)
    //   체크 안됨 = 기존 순차 탐색
    ...
    return `<div class="opt-row" id="optRow_${k}" style="${rowOpacity}">
      <input type="checkbox" id="optCheck_${k}" ${checked} ${locked?'disabled':''} onchange="_optToggleCheck('${k}')" style="margin-right:4px;width:14px;height:14px;cursor:pointer;${checkedStyle}" title="체크 = 이 파라미터 전체 조합 탐색">
...
function _optToggleCheck(key){
  ...
  if(_optCheckedParams.has(key)){
    ...
    toast(`${labels[key]} 전체탐색 해제`);
  } else {
    _optCheckedParams.add(key);
    toast(`☑️ ${labels[key]} 전체탐색 추가`);
  }
  _optUpdateCount();
}
// 전체 체크/해제 유틸
function _optCheckAll(){
  ...
  _OPT_PARAM_ORDER.forEach(k => {
    if(!_optLocked.hasOwnProperty(k) && !_optIsSingleValueStatic(k)){
      _optCheckedParams.add(k);
    }
  });
  _OPT_PARAM_ORDER.forEach(k => _optRenderParamRow(k));
  _optUpdateCount();
  toast(`☑️ ${_optCheckedParams.size}개 파라미터 전체탐색 선택`);
}
...
window._optUpdateCount = function(){
  ...
  // S113-c: 체크된 파라미터 전체 조합 수 계산 (하이브리드 탐색)
  let gridInfo = '';
  if(_optCheckedParams && _optCheckedParams.size > 0){
    let gridCombos = 1;
    const gridNames = [];
    _optCheckedParams.forEach(k => {
      if(_optLocked.hasOwnProperty(k)) return; // 잠긴 것 무시
      const r = ranges[k];
      ...
      if(steps > 1){
        ...
        gridNames.push(shortLabels[k]||k);
      }
    });
    if(gridCombos > 1){
      const gridTotal = gridCombos * tfs.length * regimeRounds * stockCount * repeatCount;
      const gridSec = Math.ceil(gridTotal * 0.01); // S113 실측: 100회/1초
      const gridTime = gridSec >= 60 ? `약 ${Math.ceil(gridSec/60)}분` : `약 ${gridSec}초`;
      gridInfo = `<br><span style="color:var(--accent,#2563eb);font-size:11px">☑️ 전체탐색: ${gridNames.join(' × ')} = <b>${gridCombos.toLocaleString()}</b> 조합 (${gridTime})</span>`;
    }
  }
  
  const el = document.getElementById('optComboInfo');
  if(el) el.innerHTML = `파라미터 <b>${paramCount}</b>개 × 범위합 <b>${totalSteps}</b> × TF <b>${tfs.length}</b>개 × ${regimeLabel}${stockCount>1?' × <b>'+stockCount+'</b>종목':''}${repeatCount>1?' × <b>'+repeatCount+'</b>회':''} = <b>${total}</b>회 BT (${estMin})${gridInfo}`;
};
...
async function _optRunNonstop(){
  ...
  // ═══════════════════════════════════════════════════════════
  // S113-c: Phase 1 — 체크된 파라미터 전체 조합 그리드 서치
  //   체크된 파라미터들만 모든 조합 테스트 → 최고 조합을 baseParams에 반영
  //   이후 Phase 2(순차 탐색)가 이 최적 조합 기준으로 진행
  //   채희창 설계: "체크된 것만 전체 조합, 나머지는 순차"
  // ═══════════════════════════════════════════════════════════
  const gridKeys = Array.from(_optCheckedParams || []).filter(k => 
    !_optLocked.hasOwnProperty(k) && !_optIsSingleValue(k)
  );
  if(gridKeys.length > 0){
    progText.textContent = `🎯 전체탐색 Phase 1: ${gridKeys.length}개 파라미터 조합 생성 중...`;
    
    // 각 파라미터의 값 배열 생성
    const gridValues = {};
    gridKeys.forEach(k => {
      const isFloat = ['bbMult','tpMult','slMult'].includes(k);
      const min = isFloat ? parseFloat(document.getElementById('optMin_'+k)?.value) : parseInt(document.getElementById('optMin_'+k)?.value);
      const max = isFloat ? parseFloat(document.getElementById('optMax_'+k)?.value) : parseInt(document.getElementById('optMax_'+k)?.value);
      const step = isFloat ? parseFloat(document.getElementById('optStep_'+k)?.value) : parseInt(document.getElementById('optStep_'+k)?.value);
      const arr = [];
      for(let v = min; v <= max + 0.0001; v += step){
        arr.push(isFloat ? Math.round(v * 1000) / 1000 : Math.round(v));
      }
      gridValues[k] = arr;
    });
    
    // 모든 조합 생성 (카테시안 곱)
    const _cartesian = (keys, idx, current, result) => {
      if(idx === keys.length){ result.push({...current}); return; }
      const k = keys[idx];
      for(const v of gridValues[k]){
        current[k] = v;
        _cartesian(keys, idx+1, current, result);
      }
    };
    const gridCombos = [];
    _cartesian(gridKeys, 0, {}, gridCombos);
    
    console.log(`[S113-c] Phase 1 시작: ${gridKeys.length}개 파라미터 × ${gridCombos.length} 조합`);
    ...
    let gridDone = 0;
    const gridTotal = gridCombos.length * tfs.length * codes.length;
    
    for(let ci = 0; ci < gridCombos.length; ci++){
      if(_cancelled) break;
      const gridP = { ...baseParams, ..._optLocked, ...gridCombos[ci] };
      _saveAnalParams(gridP);
      const btP = { 
        buyTh: gridP.buyTh>0?gridP.buyTh:62, 
        sellTh: gridP.sellTh>0?gridP.sellTh:38, 
        tpMult: gridP.tpMult>0?gridP.tpMult:2.5, 
        slMult: gridP.slMult>0?gridP.slMult:1.5 
      };
      // 각 TF × 종목 평균 점수 계산
      let aggScore = 0, aggCount = 0;
      for(const tf of tfs){
        let sW=0,sP=0,sT=0,sM=0,vc=0;
        for(const code of codes){
          const rows = candleCache[`${code}_${tf}`];
          if(!rows) continue;
          try{
            let r = sxRunBtEngine(rows, tf, btP, {slippage:0.001, nextBarEntry:false});
            if(_optPerTradeFilter && r && r.trades) r = _optFilterBtResult(r);
            if(!r.error && r.totalTrades >= 1){
              sW += r.winRate; sP += r.totalPnl; sT += r.totalTrades; sM += (r.mdd||0); vc++;
            }
          }catch(_){}
          gridDone++;
        }
        if(vc > 0){
          const avg = { winRate:sW/vc, totalPnl:sP/vc, totalTrades:sT/vc, mdd:sM/vc };
          const sc = _optSortScore(avg, _optSortMode);
          aggScore += sc; aggCount++;
        }
      }
      const finalScore = aggCount > 0 ? aggScore / aggCount : -Infinity;
      if(finalScore > bestGridScore){
        bestGridScore = finalScore;
        bestGridParams = { ...gridCombos[ci] };
      }
      // 진행 표시 (100개마다)
      if(ci % 50 === 0){
        progFill.style.width = `${(ci/gridCombos.length*20).toFixed(1)}%`; // Phase 1은 전체의 20%
        progText.textContent = `🎯 Phase 1: ${ci+1}/${gridCombos.length} 조합 (최고 ${bestGridScore.toFixed(1)}pt)`;
        await _sleep(0);
      }
    }
    
    // Phase 1 결과를 baseParams에 반영
    if(bestGridParams && !_cancelled){
      Object.keys(bestGridParams).forEach(k => {
        baseParams[k] = bestGridParams[k];
        // UI 상태 표시
        const se = document.getElementById('optStatus_'+k);
        if(se){ se.textContent = `✅${bestGridParams[k]}`; se.style.color = 'var(--buy,#27ae60)'; }
      });
      console.log(`[S113-c] Phase 1 완료: 최고 조합`, bestGridParams, `점수 ${bestGridScore.toFixed(2)}`);
      progText.textContent = `✅ Phase 1 완료 (${gridCombos.length} 조합) → Phase 2 순차 탐색 시작`;
      await _sleep(500);
    }
  }
  // ═══════════════════════════════════════════════════════════
  ...
  try {
    ...
    for(let round = 0; round <= repeatCount; round++){
      ...
      for(let si = 0; si < unlocked.length; si++){
        ...
        progText.textContent = `${roundLabel} ${paramLabel} 탐색 중... (${si+1}/${totalSteps})`;
        ...
        _OPT_PARAM_ORDER.forEach(k => {
          ...
          const max = isFloat ? parseFloat(document.getElementById('optMax_'+k)?.value||OPT_DEFAULTS[k].max) : parseInt(document.getElementById('optMax_'+k)?.value||OPT_DEFAULTS[k].max);
          const step = isFloat ? parseFloat(document.getElementById('optStep_'+k)?.value||OPT_DEFAULTS[k].step) : parseInt(document.getElementById('optStep_'+k)?.value||OPT_DEFAULTS[k].step);
          ranges[k] = { min, max, step: Math.max(step, 0), enabled: (k === paramKey) };
        });
        ...
        if(_optRegimeMode !== 'on'){
          ...
          for(const tf of tfs){
            ...
            for(let ci=0; ci<combos.length; ci++){
              ...
              if(done%5===0){ progText.textContent=`${roundLabel} ${paramLabel} [OFF][${tf}] ${done}/${total}`; await _sleep(0); }
            }
          }
        }
        ...
        if(_optRegimeMode !== 'off'){
          ...
          for(const tf of tfs){
            ...
            for(let ci=0; ci<combos.length; ci++){
              ...
              if(done%5===0){ progText.textContent=`${roundLabel} ${paramLabel} [ON][${tf}] ${done}/${total}`; await _sleep(0); }
            }
          }
        }
...
function _saveOptBest(d){ localStorage.setItem(OPT_BEST_KEY_V3, JSON.stringify(d)); }
