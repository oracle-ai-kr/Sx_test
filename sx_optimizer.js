...
function openOptimizer(){
    ...
    <div class="opt-body">
      ...
      <div class="opt-section">
        ...
        <div style="display:flex;gap:6px;align-items:center">
          <select id="optStockSel" class="opt-stock-input" style="flex:1">
            ${stocks.map((s,i)=>`<option value="${s.code}"${i===0?' selected':''}>${s.name} (${s.code})</option>`).join('')}
          </select>
          <span style="font-size:18px;cursor:pointer;color:var(--accent,#2563eb)" title="종목 추가" onclick="_optOpenStockPicker()">＋</span>
        </div>
        <div class="opt-info">시장: ${_PARAM_MARKET_LABELS_SHORT[market]} · <span id="optStockCount">${stocks.length}</span>/${OPT_MAX_STOCKS}개 · 길게 누르면 삭제</div>
      </div>
  ...
  if(sel){
    let _lpt = 0;
    sel.addEventListener('touchstart',()=>{ _lpt = Date.now(); });
    sel.addEventListener('touchend',(e)=>{
      if(Date.now()-_lpt > 600){
        e.preventDefault();
        const code = sel.value;
        const opt = sel.options[sel.selectedIndex];
        const name = opt ? opt.textContent : code;
        if(confirm(`"${name}" 을(를) 대표종목에서 삭제하시겠습니까?`)){
...
// ═══════════════════════════════════════════
//  S77: 종목 탐색 패널 연동 (내 필터탭 종목 탐색 재사용)
// ═══════════════════════════════════════════
let _optPickerCallback = null;

function _optOpenStockPicker(){
  const market = typeof currentMarket !== 'undefined' ? currentMarket : 'kr';
  const stocks = _getOptStocks(market);
  if(stocks.length >= OPT_MAX_STOCKS){ toast(`최대 ${OPT_MAX_STOCKS}개 — 기존 종목 삭제 후 추가하세요`); return; }

  // 기존 sxAzPanel 종목 탐색 활용
  // 콜백 등록: 종목 선택 시 대표 종목에 추가
  ...
  // sxAzPanel 열기
  if(typeof toggleSxAzPanel === 'function'){
    toggleSxAzPanel();
  } else {
    toast('종목 탐색 패널을 사용할 수 없습니다');
    _optPickerCallback = null;
  }
}
...
window._optOpenStockPicker = _optOpenStockPicker;
