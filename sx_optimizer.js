// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v2.7 — 파라미터 자동 최적화 모듈
...
let _optMaxLossPnl = 20;  // S97: 손절 최대 손실률 필터 (기본 20%, OFF 시 999)
...
function _optFilterBtResult(r){
  ...
  if(closed.length === 0) return { ...r, totalTrades:0, winRate:0, totalPnl:0, trades:valid };
      ...
      <div class="opt-section" id="optControlSection">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:6px 10px;background:var(--surface2,#f5f5f5);border-radius:6px;border:1px solid var(--border,#e0e0e0)">
          ...
          <div style="flex:1">
            ...
            <span style="font-size:9px;color:var(--text3,#999)">익절 최소 <input type="number" id="optMinWinPnl" value="2" min="0" max="50" step="0.5" style="width:40px;padding:1px 3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center" onchange="_optUpdatePerTradeFilter()">% · 손절 최대 <input type="number" id="optMaxLossPnl" value="20" min="1" max="100" step="1" style="width:40px;padding:1px 3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center" onchange="_optUpdatePerTradeFilter()">%</span>
          </div>
        </div>
        <!-- S90: 레짐 탐색 선택 -->
