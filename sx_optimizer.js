// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v2.0 — 파라미터 자동 최적화 모듈
...
// ── 기본 설정 ──
const OPT_DEFAULTS = {
  rsiLen:  { min:2,   max:50,  step:1,   enabled:false },
  bbLen:   { min:5,   max:60,  step:1,   enabled:false },
  bbMult:  { min:0.5, max:5.0, step:0.1, enabled:false },
  atrLen:  { min:2,   max:50,  step:1,   enabled:false },
  maShort: { min:2,   max:30,  step:1,   enabled:false },
  maMid:   { min:5,   max:80,  step:1,   enabled:false },
  maLong:  { min:10,  max:200, step:1,   enabled:false },
  buyTh:   { min:10,  max:95,  step:1,   enabled:false },
  sellTh:  { min:5,   max:90,  step:1,   enabled:false },
  tpMult:  { min:0.1, max:10.0,step:0.1, enabled:false },
  slMult:  { min:0.1, max:5.0, step:0.1, enabled:false },
};
...
function _updateOptBest(market, tf, entry){
  // entry: {params, score, winRate, totalPnl, totalTrades, mdd, regime, code, ts}
  const all = _loadOptBest();
  if(!all[market]) all[market] = {};
  const existing = all[market][tf];
  // 종합점수가 더 높으면 갱신
  ...
  try {
    ...
    for(const tf of tfs){
      ...
      for(const code of codes){
        if(_cancelled) break;
        const cacheKey = `${code}_${tf}`;
        progText.textContent = `[레짐OFF] [${tf}] ${code} 캔들 수집 중...`;
        const count = (tf==='week'||tf==='month') ? 400 : 300;
        try {
          if(candleCache[cacheKey]){ codeRows[code] = candleCache[cacheKey]; }
          else { const rows = await btFetchCandles(code, isCoin, tf, count); candleCache[cacheKey] = rows; codeRows[code] = rows; }
        } catch(e){
          // 해당 종목 스킵
          done += combos.length;
    ...
    for(const tf of tfs){
      ...
      for(const code of codes){
        ...
        try {
          ...
          else { const rows = await btFetchCandles(code, isCoin, tf, count); candleCache[ck]=rows; codeRows[code]=rows; }
        } catch(_){ done+=combos.length; }
      }
      const validCodes = Object.keys(codeRows);
      if(validCodes.length===0){ done+=combos.length*codes.length; continue; }
      for(let ci=0; ci<combos.length; ci++){
...
function _buildMergedCards(tfResOff, tfResOn, tfLabels, code, label){
  ...
  for(const tf of allTfs){
    ...
    top.forEach((r,i)=>{
      ...
      // S78: 종합점수 계산 (정렬 공식과 동일)
      const cardScore = ((b.winRate/100)*b.totalPnl*Math.log(b.totalTrades+1)*(1-(b.mdd||0)/100)).toFixed(1);
...
function _optApplySelected(){
  ...
  _optRenderLockedBar();
  _optUpdateCount();
  // S77: 선택된 카드로 시장×TF 1등 갱신
  if(r && r.bt){
    const b = r.bt;
    const score = parseFloat(((b.winRate/100)*b.totalPnl*Math.log(b.totalTrades+1)*(1-(b.mdd||0)/100)).toFixed(1));
  ...
  sorted.forEach(tf=>{
    ...
    html += `<div style="background:var(--surface2,#f8f8f8);border-radius:8px;padding:8px 10px;margin-bottom:4px;border:1px solid var(--border,#e0e0e0)">
      <div style="display:flex;align-items:center;justify-content:space-between">
        <span style="font-size:11px;font-weight:700">📊 ${tfLabel} ${regTag}</span>
...
// 1등 기록을 시장별 프리셋 슬롯에 저장
function _optSaveBestPreset(){
  const bests = _getOptBestForMarket(_optMarket);
  const keys = Object.keys(bests);
  if(keys.length === 0){ toast('저장할 기록이 없습니다'); return; }
  // 가장 높은 점수의 TF 찾기
  let bestTf = keys[0], bestScore = bests[keys[0]].score || 0;
  keys.forEach(k=>{ if((bests[k].score||0) > bestScore){ bestScore = bests[k].score; bestTf = k; } });
  const entry = bests[bestTf];
  if(!entry || !entry.params){ toast('기록 데이터 오류'); return; }
  const market = _optMarket;
  const tfLabels = {};
  if(typeof TF_MAP !== 'undefined') (TF_MAP[market]||[]).forEach(t=>{ tfLabels[t.k]=t.l; });
  const tfLabel = tfLabels[bestTf] || bestTf;
  const name = `${entry.code||''}_${tfLabel}_1등_${bestScore.toFixed(0)}pt`;
  const slots = _loadMarketSlots(market);
