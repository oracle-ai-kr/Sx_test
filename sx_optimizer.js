// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v2.4 — 파라미터 자동 최적화 모듈
...
let _optRegimeMode = 'both'; // 'off' | 'on' | 'both'
const _OPT_MODES = {
  profit:   { label:'🔥 수익형', desc:'수익률 > 거래수 > 승률 > MDD > 종합' },
  balanced: { label:'⚖️ 안정형', desc:'수익률 > 거래수 > MDD > 승률 > 종합' },
  safe:     { label:'🛡️ 보수형', desc:'수익률 > MDD > 거래수 > 승률 > 종합' },
};
// S91: 모드별 5단계 순차비교 정렬 (수익률 1순위 공통, 종합점수 최종)
// profit:   수익률 → 거래수 → 승률 → MDD낮음 → 종합점수
// safe:     수익률 → MDD낮음 → 거래수 → 승률 → 종합점수
// balanced: 수익률 → 거래수 → MDD낮음 → 승률 → 종합점수
function _optSortCompare(a, b, mode){
  const pnl = (a.totalPnl||0) - (b.totalPnl||0);
  if(Math.abs(pnl) > 0.01) return pnl; // 1순위: 수익률
  const tr = (a.totalTrades||0) - (b.totalTrades||0);
  const wr = (a.winRate||0) - (b.winRate||0);
  const mdd = (b.mdd||0) - (a.mdd||0); // 낮을수록 좋음 → b-a
  const cs = _optSortScore(a,'balanced') - _optSortScore(b,'balanced'); // 종합점수
  if(mode==='profit'){
    if(tr !== 0) return tr;                        // 2: 거래수
    if(Math.abs(wr) > 0.01) return wr;             // 3: 승률
    if(Math.abs(mdd) > 0.01) return mdd;           // 4: MDD
    return cs;                                      // 5: 종합점수
  }
  if(mode==='safe'){
    if(Math.abs(mdd) > 0.01) return mdd;           // 2: MDD
    if(tr !== 0) return tr;                        // 3: 거래수
    if(Math.abs(wr) > 0.01) return wr;             // 4: 승률
    return cs;                                      // 5: 종합점수
  }
  // balanced
  if(tr !== 0) return tr;                          // 2: 거래수
  if(Math.abs(mdd) > 0.01) return mdd;             // 3: MDD
  if(Math.abs(wr) > 0.01) return wr;               // 4: 승률
  return cs;                                        // 5: 종합점수
}
// 하위호환: 단일 점수가 필요한 곳용 (표시/기록)
  ...
  overlay.innerHTML = `<div class="opt-panel">
    ...
    <div class="opt-body">
      ...
      <div class="opt-section" id="optControlSection">
        ...
        <div id="optModeDesc" style="font-size:9px;color:var(--text3,#999);margin-bottom:8px;text-align:center"></div>
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
          ...
          <span style="font-size:9px;color:var(--text3,#999);margin-left:auto">파라미터 위→아래 논스톱 순차탐색</span>
        </div>
...
window._optUpdateCount = function(){
  ...
  const total = cnt * tfs.length * regimeRounds * stockCount;
  ...
  if(el) el.innerHTML = `조합: <b>${cnt}</b>개 × TF <b>${tfs.length}</b>개 × ${regimeLabel}${stockCount>1?' × <b>'+stockCount+'</b>종목':''} = <b>${total}</b>회 BT (${estMin})`;
};
  ...
  try {
    for(let round = 1; round <= repeatCount; round++){
      ...
      for(let si = 0; si < unlocked.length; si++){
        ...
        const combos = _generateCombinations(ranges);
        if(combos.length <= 1) continue; // 범위 없으면 스킵
        ...
        // 이 파라미터의 최적값 추출 (TOP1)
        const merged = [];
        const addM = (res, regime) => { if(!res) return; Object.values(res).forEach(arr=>{ if(Array.isArray(arr)) arr.filter(r=>r.bt&&r.bt.totalTrades>=_optMinTrades&&r.bt.winRate>=_optMinWinRate).forEach(r=>merged.push({...r,regime})); }); };
...
function _optRenderModeGate(){
  ...
  if(!gate) return;
  const modeDescs = {
    profit: '수익률 > 거래수 > 승률 > MDD > 종합점수',
    balanced: '수익률 > 거래수 > MDD > 승률 > 종합점수',
    safe: '수익률 > MDD > 거래수 > 승률 > 종합점수'
  };
  ...
  if(desc) desc.textContent = modeDescs[_optSortMode] || '';
}
...
function _buildMergedCards(tfResOff, tfResOn, tfLabels, code, label){
  ...
  for(const tf of allTfs){
    ...
    const _modeFormula = {profit:'수익률 > 거래수 > 승률 > MDD > 종합', balanced:'수익률 > 거래수 > MDD > 승률 > 종합', safe:'수익률 > MDD > 거래수 > 승률 > 종합'};
    html += `<div style="font-size:9px;color:var(--text3,#999);text-align:center;margin-top:2px;margin-bottom:6px">정렬: ${_modeFormula[_optSortMode]||''} | 종합 = 승률×수익률×log(거래수+1)×(1−MDD/100)</div>`;
...
function _updateOptBest(market, tf, mode, regime, entry){
  ...
  // S91: 무조건 덮어쓰기 — 사용자가 선택한 카드가 최고기록
...
function _optRenderBestCards(){
  ...
  html+=`<div style="display:flex;justify-content:space-between;margin-top:4px"><span style="font-size:9px;color:var(--text3,#999);cursor:pointer;text-decoration:underline" onclick="_optClearBest()">기록 초기화</span><span style="font-size:9px;color:var(--accent,#2563eb);cursor:pointer;text-decoration:underline" onclick="_optSaveBestAll()">프리셋 저장</span></div>`;
  area.innerHTML=html;
}

function _optSaveBestPreset(tf, mode, regime){
  const all=_loadOptBest();
  const entry=all[_optMarket]?.[tf]?.[mode]?.[regime];
  if(!entry||!entry.params){ toast('기록 없음'); return; }
  const market=_optMarket;
  const tfLabels={};
  if(typeof TF_MAP!=='undefined')(TF_MAP[market]||[]).forEach(t=>{tfLabels[t.k]=t.l;});
  const mNames={profit:'수익형',balanced:'안정형',safe:'보수형'};
  const tfLabel=tfLabels[tf]||tf;
  const name=`${tfLabel}_${mNames[mode]||mode}_${regime}_${(entry.score||0).toFixed(0)}pt`;
  const slots=_loadMarketSlots(market);
  if(slots.length>=SCR_ANAL_MAX_SLOTS){ toast('슬롯 가득참'); return; }
  const ok=_saveSlot(market,name,entry.params,-1);
...
// 최고기록 전체를 프리셋으로 일괄 저장
function _optSaveBestAll(){
  const all=_loadOptBest();
  const mkt=all[_optMarket];
  if(!mkt||Object.keys(mkt).length===0){ toast('저장할 기록이 없습니다'); return; }
  const market=_optMarket;
  const tfLabels={};
  if(typeof TF_MAP!=='undefined')(TF_MAP[market]||[]).forEach(t=>{tfLabels[t.k]=t.l;});
  const mNames={profit:'수익형',balanced:'안정형',safe:'보수형'};
  let saved=0;
  for(const[tf,modes] of Object.entries(mkt)){
    for(const[mode,regimes] of Object.entries(modes)){
      for(const[regime,e] of Object.entries(regimes)){
        if(!e||!e.params) continue;
        const tfLabel=tfLabels[tf]||tf;
        const name=`${tfLabel}_${mNames[mode]||mode}_${regime}_${(e.score||0).toFixed(0)}pt`;
        const slots=_loadMarketSlots(market);
        if(slots.length>=SCR_ANAL_MAX_SLOTS) break;
        if(_saveSlot(market,name,e.params,-1)) saved++;
      }
    }
  }
  if(typeof updateParamMarketStatus==='function') updateParamMarketStatus();
  toast(saved>0?`✅ ${saved}개 프리셋 저장됨`:'저장할 기록이 없습니다');
}
...
window._optSaveBestAll = _optSaveBestAll;
