// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v3.2 — 파라미터 자동 최적화 모듈
//  S101: OPT_DEFAULTS 단일값+step0 / step=0 보존(Math.max 가드 제거) / 가중치 초기화값 사진2 기준
//  S101-fix: float step 부동소수 오차 수정 (BB/TP/SL 배수 max값 누락 버그 제거) + 단일값 파라미터 재탐색 제거(✅값 즉시 표시)
...
const OPT_DEFAULTS = {
  atrLen:  { min:14,  max:14,  step:0,   enabled:false },  // 1. 변동성 기반 (고정 14, step=0)
  ...
  rsiLen:  { min:14,  max:14,  step:0,   enabled:false },   // 5. 시그널 감도 (고정 14, step=0)
  buyTh:   { min:25,  max:55,  step:1,   enabled:false },   // 6. 진입 점수
  sellTh:  { min:5,   max:5,   step:0,   enabled:false },   // 7. 청산 점수 (고정 5, step=0)
...
async function _optRunNonstop(){
  ...
  const getUnlockedParams = () => _OPT_PARAM_ORDER.filter(k => !_optLocked.hasOwnProperty(k) && !_optIsSingleValue(k));
