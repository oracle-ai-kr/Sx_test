...
// 로컬 전종목 로드 (_loadSxAzItems 동일 로직)
function _optLoadAllStocks(){
  const m = typeof currentMarket!=='undefined' ? currentMarket : 'kr';
  let items = [];
  if(m==='kr'){
...
function _optToggleAzPanel(){
  ...
  panel.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px 8px;flex-shrink:0;border-bottom:1px solid var(--border,#e0e0e0)">
      <span style="font-size:14px;color:var(--text,#222);font-weight:700">종목 탐색</span>
...
function _optRenderAzList(items){
  ...
  if(!list) return;
  if(!items.length){ list.innerHTML='<div style="padding:12px;text-align:center;font-size:11px;color:var(--text3,#999)">데이터 없음 — 설정 탭에서 종목 마스터를 새로고침하세요</div>'; return; }
