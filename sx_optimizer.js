// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v2.8 — 파라미터 자동 최적화 모듈
...
const _OPT_MODE_WEIGHTS = {
  profit:   { pnl:50, wr:25, trades:20, mdd:5 },
  balanced: { pnl:40, trades:20, mdd:20, wr:20 },
  safe:     { pnl:30, mdd:30, wr:30, trades:10 }
};
let _optCustomWeights = null; // null이면 모드 기본값 사용
// S98: 모드별 타이브레이커 순위 (1=최우선, 4=최하위, 음수=낮을수록 좋음 e.g. MDD)
// 키: pnl, wr, trades, mdd  값: [순위, 방향(1=높을수록/-1=낮을수록)]
const _OPT_MODE_TIEBREAK_DEFAULTS = {
  profit:   { pnl:1, wr:2, trades:3, mdd:4 },
  balanced: { pnl:1, trades:2, mdd:3, wr:4 },
  safe:     { pnl:1, mdd:2, wr:3, trades:4 }
};
...
const _OPT_MODES = {
  profit:   { label:'🔥 수익형', desc:'수익률 > 승률 > 거래수 > MDD' },
  balanced: { label:'⚖️ 안정형', desc:'수익률 > 거래수 > MDD > 승률' },
  safe:     { label:'🛡️ 보수형', desc:'수익률 > MDD↓ > 승률 > 거래수' },
};
      ...
      <div class="opt-section" id="optControlSection">
        ...
        <div id="optWeightArea" style="display:none;padding:8px 10px;margin-bottom:8px;background:var(--surface2,#f5f5f5);border-radius:8px;border:1px solid var(--border,#e0e0e0)">
          <details style="margin-bottom:8px">
            <summary style="font-size:10px;font-weight:600;color:var(--accent,#2563eb);cursor:pointer">ℹ️ 가중치 · 타이브레이커란?</summary>
            <div style="font-size:9px;color:var(--text2,#666);margin-top:6px;line-height:1.6;padding:6px 8px;background:var(--surface,#fff);border-radius:6px;border:1px solid var(--border,#e8e8e8)">
              <b>가중치 (%)</b> — 종합점수 계산용<br>
              점수 = 수익률×W₁ + 승률×W₂ + 거래수×W₃ + MDD×W₄<br>
              모든 파라미터 조합에 점수를 매겨 순위를 정합니다.<br><br>
              <b>타이브레이커 (순위)</b> — 점수가 비슷할 때(10% 이내) 우선순위 결정<br>
              예: 수익형 1→수익률, 2→승률, 3→거래수, 4→MDD<br>
              동점일 때 1순위 지표부터 비교하여 최종 순위를 결정합니다.
            </div>
          </details>
          <div style="font-size:10px;font-weight:600;margin-bottom:6px;color:var(--text2,#666)">동점 타이브레이커 순위 (1=최우선, 4=최하위)</div>
          <div style="display:grid;grid-template-columns:50px 1fr 1fr 1fr 1fr;gap:3px;align-items:center;font-size:9px;text-align:center;margin-bottom:10px">
            <span></span><span style="font-weight:600">수익률</span><span style="font-weight:600">거래수</span><span style="font-weight:600">MDD↓</span><span style="font-weight:600">승률</span>
            <span style="font-weight:600;text-align:left">🔥수익</span>
            <input type="number" id="optTB_profit_pnl" value="1" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_profit_trades" value="3" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_profit_mdd" value="4" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_profit_wr" value="2" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <span style="font-weight:600;text-align:left">⚖️안정</span>
            <input type="number" id="optTB_balanced_pnl" value="1" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_balanced_trades" value="2" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_balanced_mdd" value="3" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_balanced_wr" value="4" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <span style="font-weight:600;text-align:left">🛡보수</span>
            <input type="number" id="optTB_safe_pnl" value="1" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_safe_trades" value="4" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_safe_mdd" value="2" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optTB_safe_wr" value="3" min="1" max="4" step="1" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
          </div>
          ...
          <div style="display:grid;grid-template-columns:50px 1fr 1fr 1fr 1fr 36px;gap:3px;align-items:center;font-size:9px;text-align:center">
            ...
            <span style="font-weight:600;text-align:left">🔥수익</span>
            <input type="number" id="optCW_profit_pnl" value="50" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_profit_trades" value="20" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <input type="number" id="optCW_profit_wr" value="25" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <span id="optCWS_profit" style="font-size:9px;font-weight:600">100</span>
            <span style="font-weight:600;text-align:left">⚖️안정</span>
            <input type="number" id="optCW_balanced_pnl" value="40" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_balanced_trades" value="20" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_balanced_mdd" value="20" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_balanced_wr" value="20" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            ...
            <input type="number" id="optCW_safe_trades" value="10" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_safe_mdd" value="30" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
            <input type="number" id="optCW_safe_wr" value="30" min="0" max="100" step="5" style="width:100%;padding:3px;border:1px solid var(--border);border-radius:3px;font-size:10px;text-align:center">
        ...
        if(_optRegimeMode !== 'on'){
          ...
          for(const tf of tfs){
            ...
            for(let ci=0; ci<combos.length; ci++){
              ...
              for(const code of validCodes){
                try{ let r=sxRunBtEngine(candleCache[`${code}_${tf}`],tf,btP,{slippage:0.001,nextBarEntry:false}); if(_optPerTradeFilter&&r&&r.trades)r=_optFilterBtResult(r); if(!r.error&&r.totalTrades>=1){sW+=r.winRate;sP+=r.totalPnl;sT+=r.totalTrades;sM+=(r.mdd||0);vc++;} }catch(_){}
                done++;
              }
...
function _optSaveTiebreakLocal(d){ localStorage.setItem(OPT_TIEBREAK_KEY, JSON.stringify(d)); }
...
function _optResetWeights(){
  ...
  localStorage.removeItem(OPT_TIEBREAK_KEY);
  _optCustomWeights = null;
        ...
        html+=`<div style="background:var(--surface2,#f8f8f8);border-radius:8px;padding:8px 10px;margin-bottom:4px;border:1px solid var(--border,#e0e0e0)">
          <div style="display:flex;align-items:center;justify-content:space-between">
            ...
            <span style="display:flex;align-items:center;gap:6px">
              ...
              <span style="font-size:11px;color:var(--text3,#999);cursor:pointer;line-height:1" onclick="_optDeleteBestOne('${tf}','${mode}','${regime}','${tfLabel}','${mNames[mode]||mode}')">✕</span>
            </span>
          </div>
          ...
          <div style="display:flex;gap:8px;margin-top:3px;font-size:9px;color:var(--text3,#999)">
            ...
            <span style="margin-left:auto">${e.filter?`🔧${e.filter.minWin}/${e.filter.maxLoss}%`:''}${dateStr?(' '+dateStr):''}</span>
          </div>
        </div>`;
      }
    }
...
function _optClearBest(){
  const mktLabel = _PARAM_MARKET_LABELS_SHORT[_optMarket]||_optMarket;
  if(!confirm(`[${mktLabel}] 전체 최적 파라미터 프리셋을 삭제하시겠습니까?`)) return;
  const all=_loadOptBest();
...
// S98: 개별 최고기록 삭제
function _optDeleteBestOne(tf, mode, regime, tfLabel, modeName){
  const mktLabel = _PARAM_MARKET_LABELS_SHORT[_optMarket]||_optMarket;
  const regLabel = regime==='ON'?'레짐ON':'레짐OFF';
  if(!confirm(`[${mktLabel}/${tfLabel}/${modeName}/${regLabel}]\n최적 파라미터 프리셋을 삭제하시겠습니까?`)) return;
  const all=_loadOptBest();
  try{ delete all[_optMarket][tf][mode][regime]; }catch(_){}
  // 빈 분기 정리
  try{
    if(all[_optMarket]?.[tf]?.[mode] && Object.keys(all[_optMarket][tf][mode]).length===0) delete all[_optMarket][tf][mode];
    if(all[_optMarket]?.[tf] && Object.keys(all[_optMarket][tf]).length===0) delete all[_optMarket][tf];
    if(all[_optMarket] && Object.keys(all[_optMarket]).length===0) delete all[_optMarket];
  }catch(_){}
  _saveOptBest(all);
  _optRenderBestCards();
  toast('프리셋 삭제됨');
