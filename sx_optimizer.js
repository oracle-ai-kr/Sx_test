// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v3.1 — 파라미터 자동 최적화 모듈
...
async function _optRefreshCandles(){
  ...
  if(!confirm(`선택된 ${selCodes.length}종목의 캔들을 다시 불러옵니다.\n기존 캐시는 전부 삭제됩니다.\n\n계속하시겠습니까?`)) return;

  const isCoin = _optMarket === 'coin';
  const total = selCodes.length * tfs.length;
  let done = 0;

  // S100: 전체 캐시 초기화 (메모리 + localStorage 모든 _optCandle_* 키)
  _memCandleCache = {};
  try {
    const toRemove = [];
    for(let i=0; i<localStorage.length; i++){
      const k = localStorage.key(i);
      if(k && k.startsWith(CANDLE_CACHE_PREFIX)) toRemove.push(k);
    }
  ...
  _optUpdateCandleStatus();
  toast(`✅ 전체 캐시 초기화 · 선택 ${selCodes.length}종목 갱신 완료`);
}
...
function _optResetPreset(){
  ...
  if(!confirm(`[${mIcons[targetMode]} ${mNames[targetMode]}] 프리셋 초기화\n\n최고기록 리스트는 유지하고\n대표 해제 + 파라미터 잠금 해제 + 기본값으로 복원합니다.\n\n계속하시겠습니까?`)) return;

  const all = _loadOptBest();
  if(all[targetMode]) all[targetMode].representId = null;
  _saveOptBest(all);
  // S100: 11개 파라미터 잠금 전체 해제 (기본값 복원 전에 실행 — 재잠김 방지)
  const lockedKeys = Object.keys(_optLocked);
  ...
  _optUpdateCount();
  // 기본 하드코딩 복원 (모드별 우선)
  const _modeDef = (typeof SCR_ANAL_MODE_DEFAULTS !== 'undefined' && SCR_ANAL_MODE_DEFAULTS[targetMode])
                    ? SCR_ANAL_MODE_DEFAULTS[targetMode]
                    : (typeof SCR_ANAL_DEFAULTS !== 'undefined' ? SCR_ANAL_DEFAULTS : null);
  if(_modeDef) _saveAnalParams({..._modeDef});
  if(typeof loadAnalParamsUI === 'function') loadAnalParamsUI();
  _optRenderBestCards();
  toast(`${mNames[targetMode]} 프리셋 초기화 · 잠금 해제 · 기본값 복원`);
}
