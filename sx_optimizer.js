...
//   사진2 기준 범위:
//     상단 5개: BUY 20~60, BB기간 10~30, BB배수 1.0~3.0, TP 1.0~4.0, SL 0.5~2.0
//     하단 6개: SELL 1~10, RSI 5~25, ATR 10~20, MA단 1~15, MA중 10~60, MA장 50~250
const OPT_DEFAULTS = {
  // [진입 결정] — 거래 발생 여부를 좌우하는 게이트 최우선
  buyTh:   { min:20,  max:60,  step:1,   enabled:false },  // 1. 진입 점수 임계
  bbLen:   { min:10,  max:30,  step:1,   enabled:false },  // 2. BB 기간 (신호 빈도)
  bbMult:  { min:1.0, max:3.0, step:0.1, enabled:false },  // 3. BB 배수 (진입 폭)
  // [청산 결정] — 진입 안정화 후 최적화
  tpMult:  { min:1.0, max:4.0, step:0.1, enabled:false },  // 4. 익절 폭
  slMult:  { min:0.5, max:2.0, step:0.1, enabled:false },  // 5. 손절 폭
  sellTh:  { min:1,   max:10,  step:1,   enabled:false },  // 6. 청산 점수
  // [지표 기간]
  rsiLen:  { min:5,   max:25,  step:1,   enabled:false },  // 7. RSI 기간
  atrLen:  { min:10,  max:20,  step:1,   enabled:false },  // 8. ATR 기간
  // [이동평균]
  maShort: { min:1,   max:15,  step:1,   enabled:false },  // 9. MA 단기
  maMid:   { min:10,  max:60,  step:1,   enabled:false },  // 10. MA 중기
  maLong:  { min:50,  max:250, step:1,   enabled:false },  // 11. MA 장기
};
...
// 모든 파라미터를 OPT_DEFAULTS_BASE(사진2 기준) 범위로 초기화
function _optResetRanges(){
  ...
  overlay.innerHTML = `<div class="opt-panel">
    ...
    <div class="opt-body">
      ...
      <div class="opt-section" id="optControlSection">
        ...
        <!-- [S173] 둘째 줄: 📐 최소값 정렬 / ▶ 결과보기 / 🔓 전체 해제 (3개 균등 한 줄) -->
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
          <span style="flex:1;font-size:10px;padding:6px 4px;border-radius:5px;cursor:pointer;background:var(--surface2,#f0f0f0);color:var(--text2,#666);border:1px solid var(--border,#ddd);text-align:center;white-space:nowrap" onclick="_optMinValueAlign()" title="11개 파라미터 max를 min으로 자동 정렬 (단일값)">📐 최소값 정렬</span>
          <span style="flex:1;font-size:10px;padding:6px 4px;border-radius:5px;cursor:pointer;background:var(--accent,#2563eb);color:#fff;border:1px solid var(--accent,#2563eb);text-align:center;white-space:nowrap;font-weight:600" onclick="_optShowMinValueResult()" title="최소값 정렬 + 즉시 BT 실행 (수동탐색)">▶ 결과보기</span>
        ...
        if(isWarmup){
          ...
          if(_optRegimeMode !== 'on'){
            ...
            for(const tf of tfs){
              ...
              let sW=0, sP=0, sT=0, sM=0, vc=0, _zeroTrades=0, _errors=0;
              ...
              if(vc > 0){
                const _entry = {
                  params: {..._svParams},
                  bt: { winRate: sW/vc, totalPnl: sP/vc, totalTrades: Math.round(sT/vc), mdd: parseFloat((sM/vc).toFixed(2)), error: null },
                  _stockCount: vc
                };
                _svResOff[tf].push(_entry);
                console.log(`[S173] [${tf}] OFF 결과: 승률 ${_entry.bt.winRate.toFixed(1)}%, 수익 ${_entry.bt.totalPnl.toFixed(2)}%, 거래 ${_entry.bt.totalTrades}회, MDD ${_entry.bt.mdd}%`);
              } else {
          ...
          if(_optRegimeMode !== 'off'){
            ...
            for(const tf of tfs){
              ...
              if(vc > 0){
                ...
                console.log(`[S173] [${tf}] ON 결과: 승률 ${_entry.bt.winRate.toFixed(1)}%, 수익 ${_entry.bt.totalPnl.toFixed(2)}%, 거래 ${_entry.bt.totalTrades}회, MDD ${_entry.bt.mdd}%`);
              } else {
          ...
          for(const tf of tfs){
            ...
            for(let ci=0; ci<combos.length; ci++){
              ...
              for(const code of validCodes){
                ...
                try{ let r=sxRunBtEngine(slicedRows,tf,btP,{slippage:0.001,nextBarEntry:false,applyRegimeAdjust:true}); if(_optPerTradeFilter&&r&&r.trades)r=_optFilterBtResult(r); if(!r.error&&r.totalTrades>=1){sW+=r.winRate;sP+=r.totalPnl;sT+=r.totalTrades;sM+=(r.mdd||0);vc++;} }catch(_){}
                done++;
              }
...
// ════════════════════════════════════════════════════════════
// [S169] 결과보기 — 최소값 정렬된 단일값 BT 즉시 실행
//   _optMinValueAlign() 호출 후 _optRunNonstop() 실행
//   11개 모두 단일값이라 1회 BT만 돌고 끝 (수동탐색 = 빠른 결과 확인)
// ════════════════════════════════════════════════════════════
async function _optShowMinValueResult(){
  _optVib(20);
  // [S173-log] 진단 로그
  console.log('[S173] ▶ 결과보기 클릭');
