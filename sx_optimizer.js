// ════════════════════════════════════════════════════════════
//  sx_optimizer.js v3.0 — 파라미터 자동 최적화 모듈
//  S99-5: OPT_BEST V3(모드→ranks[6]+representId) + ★대표선정모달 + V2→V3마이그레이션
...
// ── 기본 설정 (최적화 우선순위 순) ──
const OPT_DEFAULTS = {
  atrLen:  { min:14,  max:14,  step:1,   enabled:false },  // 1. 변동성 기반 (고정 14)
  bbMult:  { min:1.8, max:2.5, step:0.1, enabled:false },  // 2. 진입 기준 폭
  tpMult:  { min:2.5, max:3.5, step:0.1, enabled:false },  // 3. 익절폭
  slMult:  { min:1.0, max:1.8, step:0.1, enabled:false },  // 4. 손절폭
  rsiLen:  { min:14,  max:14,  step:1,   enabled:false },   // 5. 시그널 감도 (고정 14)
  buyTh:   { min:25,  max:55,  step:1,   enabled:false },   // 6. 진입 점수
  sellTh:  { min:5,   max:5,   step:1,   enabled:false },   // 7. 청산 점수 (고정 5)
  bbLen:   { min:10,  max:30,  step:1,   enabled:false },   // 8. 추세 필터
  maShort: { min:2,   max:10,  step:1,   enabled:false },   // 9. MA 단기
  maMid:   { min:5,   max:30,  step:1,   enabled:false },   // 10. MA 중기
  maLong:  { min:20,  max:120, step:1,   enabled:false },   // 11. MA 장기
};
    ...
    for(let round = 1; round <= repeatCount; round++){
      ...
      for(let si = 0; si < unlocked.length; si++){
        ...
        if(combos.length <= 1){
          ...
          progText.textContent = `[${round}/${repeatCount}회] ${paramLabel} — 고정 ${fixedVal}, 스킵`;
          await _sleep(300);
          continue;
        }
...
function _optRenderBestCards(){
  ...
  modes.forEach(mode=>{
    const bucket = all[mode];
    if(!bucket || !bucket.ranks || bucket.ranks.length === 0) return;
    hasAny = true;
    const repId = bucket.representId;

    html += `<div style="font-size:12px;font-weight:700;color:var(--text,#222);margin:10px 0 6px;border-bottom:2px solid var(--border,#e0e0e0);padding-bottom:4px">${mIcons[mode]} ${mNames[mode]} <span style="font-size:9px;font-weight:400;color:var(--text3)">${bucket.ranks.length}/${OPT_BEST_MAX_RANKS}</span></div>`;
    ...
    bucket.ranks.forEach((e, idx) => {
      ...
      const mktTag = (mktIcons[e.market]||'') + ' ' + (mktNames[e.market]||e.market||'');
      // S99-5: 다중TF 표기 — tfs 배열 우선, 없으면 단일 tf
      let tfTag = '';
      ...
      html+=`<div style="background:var(--surface2,#f8f8f8);border-radius:8px;padding:8px 10px;margin-bottom:4px;${repBorder}">
        <div style="display:flex;align-items:center;justify-content:space-between">
          ...
          <span style="display:flex;align-items:center;gap:6px">
            ...
            <span style="font-size:11px;cursor:pointer;line-height:1" onclick="_optToggleRepresent('${mode}','${eId}')" title="${isRep?'대표 해제':'대표 선정'}">${isRep?'★':'☆'}</span>
            <span style="font-size:11px;color:var(--text3,#999);cursor:pointer;line-height:1" onclick="_optDeleteBestOne('${mode}','${eId}')">✕</span>
          </span>
        </div>
  ...
  if(!hasAny){
    html+='<div style="font-size:10px;color:var(--text3,#999);padding:6px 0;text-align:center">최고기록 없음 — 고정 적용 시 자동 기록</div>';
  } else {
    html = `<div style="font-size:11px;font-weight:700;color:var(--text,#222);margin-bottom:6px">🏆 최고기록 (모드별 최대 ${OPT_BEST_MAX_RANKS}개 · ★대표 = 분석/BT 적용)</div>` + html;
  }
...
function _optToggleRepresent(mode, entryId){
  ...
  if(bucket.representId === entryId){
    // 해제 — 리스트 유지, 대표만 해제, params 변경 없음 (탐색 이어가기용)
    if(!confirm(`★ 대표 해제\n\n${mIcons[mode]} ${mNames[mode]} 대표를 해제합니다.\n현재 파라미터는 유지됩니다.\n\n해제하시겠습니까?`)) return;
    bucket.representId = null;
    _saveOptBest(all);
    _optRenderBestCards();
    toast(`${mNames[mode]} 대표 해제됨`);
  } else {
    // 선정 — params 즉시 적용
    ...
    _optRenderBestCards();
    toast(`★ ${mNames[mode]} 대표 선정 · 파라미터 적용됨`);
  }
}
...
function _optClearBest(){
  ...
  if(!confirm(`[${mIcons[targetMode]} ${mNames[targetMode]}] 최고기록 ${bucket.ranks.length}개를 삭제합니다.\n(리스트 삭제 + 대표 해제)\n\n현재 파라미터는 유지됩니다.\n계속하시겠습니까?`)) return;

  delete all[targetMode];
  _saveOptBest(all);
  _optRenderBestCards();
  toast(`${mNames[targetMode]} 기록 초기화됨`);
}
...
function _optDeleteBestOne(mode, entryId){
  ...
  // 대표였으면 해제
  if(all[mode].representId === entryId) all[mode].representId = null;
