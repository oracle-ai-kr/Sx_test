          ...
          for(const tf of tfs){
            ...
            for(let ci=0; ci<combos.length; ci++){
              ...
              for(const code of validCodes){
                ...
                try{ let r=sxRunBtEngine(slicedRows,tf,btP,{slippage:0.001,applyRegimeAdjust:true}); if(_optPerTradeFilter&&r&&r.trades)r=_optFilterBtResult(r); if(!r.error&&r.totalTrades>=1){sW+=r.winRate;sP+=r.totalPnl;sT+=r.totalTrades;sM+=(r.mdd||0);sPF+=Math.min(r.profitFactor||0,10);vc++;} }catch(_){}
                done++;
              }
  ...
  try {
    ...
    for(const tf of tfs){
      ...
      for(let ci=0; ci<combos.length; ci++){
        ...
        for(const code of validCodes){
          try { let r=sxRunBtEngine(codeRows[code],tf,btP,{slippage:0.001,applyRegimeAdjust:true}); if(_optPerTradeFilter&&r&&r.trades)r=_optFilterBtResult(r); if(!r.error&&r.totalTrades>=1){sumWR+=r.winRate;sumPnl+=r.totalPnl;sumTr+=r.totalTrades;sumMdd+=(r.mdd||0);vc++;} } catch(_){}
          done++;
        }
...
async function _optSetModeCore(mode, needsConfirm){
  ...
  // [S292] 탐색 모드 변경 → 조기청산/BT 청산 동작 모드 자동 동기화
  // [S332] 시장 인수 추가 — 9칸 매트릭스 적용
  _optSyncEarlyExitToMode(mode, _optMarket);
...
// ════════════════════════════════════════════════════════════
// [S332] 탐색 모드 × 시장 → 조기청산 설정 동기화 (9칸 매트릭스 박제)
// ════════════════════════════════════════════════════════════
//   설계 원칙:
//     · 9칸 (3시장 × 3모드)을 사진 캡처 그대로 코드에 박제.
//     · 시장(_optMarket) 또는 모드(_optSortMode) 변경 시 매트릭스 조회 → 일괄 적용.
//     · BT 청산 동작 모드(btExitMode/btTrailAtrMode)까지 9칸 별로 다르게 적용.
//     · 슬라이더 파라미터(beTrigger/trailMult/trailTrigger/maxBars/timeMinPnl/
//       maBreakLen/nHighBars)는 9칸 공통.
//
//   〔이력〕
//     [S292] enabled+trailing+trailMode 3개만 매핑, safe는 enabled=false
//     [S300] 추가 청산 룰(bbUpper/maBreak/nHigh) 신설, 자동 연동 제외
//     [S302] 검증 완료 후 모드 기준 전체 자동 매핑 활성화 (3칸)
//     [S307] bbUpper true→false 통일
//     [S311] score/maBreakLen/nHigh 사진 기준 재조정
//     [S332] 시장 인수 추가 → 9칸 매트릭스 박제. 코인 단타=immediate, 해외 단타=nHigh ON,
//            국내 단타=maBreak ON, 코인 중기=본전/트레일/BT트레일ATR=entry, 그 외는 dynamic.
//
//   비고:
//     · 사진 슬라이더 위치 그대로: maBreakLen=60, nHighBars=60, trailMult=1.5, beTrigger=2.0,
//       trailTrigger=2.0, maxBars=20, timeMinPnl=0.5
//     · trailMode는 trailing=false인 경우에도 일관성을 위해 'profit'으로 디폴트 박음
//     · time(시간 청산), bbUpper(BB 상단 익절)는 9칸 모두 OFF

// 9칸 매트릭스 — 사진 캡처 기준
// [S467] 시장별 비대칭 폐기 → 3시장(kr/us/coin) 전부 모드별로 동일. 사용자 사진 3장(단타/스윙/중기) 그대로 박제.
//   〔변경 요약 vs S332〕
//     · score      : 9칸 false → 모드 전부 true (점수청산 ON, 분석엔진 매도 동기화)
//     · 단타 trailMode : (kr/us)profit·coin immediate → 3시장 전부 immediate(진입즉시)
//     · 단타 bbUpper   : 전부 false → 단타만 true(BB 상단 익절 ON)
//     · 단타 maBreak   : kr만 true → 단타 3시장 true, maBreakLen=20(셀별로 이동)
//     · 단타 nHigh     : us만 true → 단타 전부 false
//     · 단타 btTrailAtrMode : dynamic → entry(진입시고정)
//     · 중기 breakeven/trailing : false → true (본전·트레일 ON, 수익권 진입 후)
//     · 중기 regimeAdaptive : 신규 true(추세 적용 — 하락전환에만)
//     · 코인 중기 btTrailAtrMode : entry → dynamic (3시장 중기 통일)
//     · deadCross/dumpExit : 신규, 모드 전부 true (데드크로스·되돌림주의 ON)
//     · beEscape   : 신규, 모드 전부 true. 단타/스윙=pct(arm5/exit1), 중기=atr(arm2/exit0.5)
//     · maBreakLen : 공통 → 셀별 (단타20·스윙60·중기60)
//   주의: 시장 비대칭이 사라졌어도 9칸 구조는 유지(레짐/시장 라우팅 호환). kr=us=coin.
const _EE_PROFIT = {
  // 🔥 단타: 점수+본전+트레일(진입즉시)+BB상단+MA이탈(20)+데드/되돌림+본전탈출(%5/1), BT=종가+진입고정
  score:true, breakeven:true, trailing:true, trailMode:'immediate',
  bbUpper:true, maBreak:true, maBreakLen:20, nHigh:false,
  deadCross:true, dumpExit:true, regimeAdaptive:false,
  supportBreakExit:true, sbMa20:true, sbMa60:true, sbMa120:true,
  beEscape:true, beEscapeBasis:'pct', beEscapeArm:5, beEscapeExit:1,
  btExitMode:'close', btTrailAtrMode:'entry'
};
const _EE_BALANCED = {
  // ⚖️ 스윙: 점수+본전+트레일(수익권후)+데드/되돌림+본전탈출(%5/1), BB/MA OFF, BT=종가+매봉재계산
  score:true, breakeven:true, trailing:true, trailMode:'profit',
  bbUpper:false, maBreak:false, maBreakLen:60, nHigh:false,
  deadCross:true, dumpExit:true, regimeAdaptive:false,
  supportBreakExit:true, sbMa20:false, sbMa60:true, sbMa120:true,
  beEscape:true, beEscapeBasis:'pct', beEscapeArm:5, beEscapeExit:1,
  btExitMode:'close', btTrailAtrMode:'dynamic'
};
const _EE_SAFE = {
  // 🛡️ 중기: 점수+본전+트레일(수익권후)+추세적용+데드/되돌림+본전탈출(ATR 2/0.5), BB/MA OFF, BT=종가+매봉재계산
  score:true, breakeven:true, trailing:true, trailMode:'profit',
  bbUpper:false, maBreak:false, maBreakLen:60, nHigh:false,
  deadCross:true, dumpExit:true, regimeAdaptive:true,
  supportBreakExit:true, sbMa20:false, sbMa60:false, sbMa120:true,
  beEscape:true, beEscapeBasis:'atr', beEscapeArm:2, beEscapeExit:0.5,
  btExitMode:'close', btTrailAtrMode:'dynamic'
};
const _EARLY_EXIT_MATRIX = Object.freeze({
  kr:   { profit:_EE_PROFIT, balanced:_EE_BALANCED, safe:_EE_SAFE },
  us:   { profit:_EE_PROFIT, balanced:_EE_BALANCED, safe:_EE_SAFE },
  coin: { profit:_EE_PROFIT, balanced:_EE_BALANCED, safe:_EE_SAFE }
});

// 9칸 공통 슬라이더/숫자 파라미터
// [S467] maBreakLen은 셀별(단타20/스윙·중기60)로 이동 → 공통에서 제거
const _EARLY_EXIT_COMMON = Object.freeze({
  beTrigger:    2.0,
  trailMult:    1.5,
  trailTrigger: 2.0,
  maxBars:      20,
...
// [S332] 시장 정규화 — _optMarket/window.currentMarket이 'crypto'/'KR' 등으로 들어와도 안전
function _eeNormalizeMarket(m){
  if(typeof m !== 'string') return 'kr';
  var s = m.toLowerCase();
  if(s === 'kr' || s === 'us' || s === 'coin') return s;
  if(s === 'crypto') return 'coin';
  return 'kr';
}

// [S332] 모드 + 시장 → 매트릭스 한 칸 조회 (안전 폴백)
function _eeResolveCell(mode, market){
  var _mkt = _eeNormalizeMarket(market);
  var _md  = (mode === 'profit' || mode === 'balanced' || mode === 'safe') ? mode : 'balanced';
  var cell = _EARLY_EXIT_MATRIX[_mkt] && _EARLY_EXIT_MATRIX[_mkt][_md];
  if(!cell) cell = _EARLY_EXIT_MATRIX.kr.balanced; // 안전 폴백
  return { cell: cell, mkt: _mkt, md: _md };
}

// [S332] 핵심 동기화 — market 인수 추가. 미지정 시 _optMarket → currentMarket → 'kr' 폴백
//   배경: 페이지 새로고침 시점에는 _optMarket='kr' 기본값이지만 currentMarket은 이미 localStorage에서
//         복원돼 사용자가 마지막으로 보던 시장(예: 'coin')이 들어있음. _optMarket 단독 폴백이면
//         새로고침 직후 [S307] 시작 동기화가 항상 국내 룰로 잘못 적용되는 문제 → currentMarket 차폴백.
function _optSyncEarlyExitToMode(mode, market){
  if(typeof SXE === 'undefined') return;

  // 시장 폴백: 인수 → _optMarket(옵티마이저 시장) → currentMarket(조건검색 시장) → 'kr'
  if(typeof market !== 'string'){
    if(typeof _optMarket !== 'undefined' && _optMarket){
      market = _optMarket;
    } else if(typeof currentMarket !== 'undefined' && currentMarket){
      market = currentMarket;
    } else if(typeof window !== 'undefined' && typeof window.currentMarket === 'string'){
      market = window.currentMarket;
    } else {
      market = 'kr';
    }
  }
  var resolved = _eeResolveCell(mode, market);
  var cell = resolved.cell;

  // BT 청산 동작 모드 — 매트릭스 셀별 (코인 중기만 entry, 나머지 dynamic)
  if(typeof SXE.setBtExitMode     === 'function') SXE.setBtExitMode(cell.btExitMode);
  if(typeof SXE.setBtTrailAtrMode === 'function') SXE.setBtTrailAtrMode(cell.btTrailAtrMode);

  var ee = SXE._btEarlyExit;
  if(!ee || typeof ee !== 'object'){
    if(typeof SXE === 'object') SXE._btEarlyExit = ee = {};
    else return;
  }

  // 9칸 공통 파라미터
  // [S467] maBreakLen은 공통 제거 → 셀별(단타20/스윙·중기60)로 이동
  ee.enabled      = _EARLY_EXIT_COMMON.enabled;
  ee.time         = _EARLY_EXIT_COMMON.time;
  ee.beTrigger    = _EARLY_EXIT_COMMON.beTrigger;
  ee.trailMult    = _EARLY_EXIT_COMMON.trailMult;
  ee.trailTrigger = _EARLY_EXIT_COMMON.trailTrigger;
  ee.maxBars      = _EARLY_EXIT_COMMON.maxBars;
  ee.timeMinPnl   = _EARLY_EXIT_COMMON.timeMinPnl;
  ee.nHighBars    = _EARLY_EXIT_COMMON.nHighBars;

  // 셀별 토글
  ee.score      = cell.score;
  ee.breakeven  = cell.breakeven;
  ee.trailing   = cell.trailing;
  ee.trailMode  = cell.trailMode;
  ee.bbUpper    = cell.bbUpper;
  ee.maBreak    = cell.maBreak;
  ee.maBreakLen = cell.maBreakLen;   // [S467] 셀별 (단타20/스윙·중기60)
  ee.nHigh      = cell.nHigh;
  // [S467] 데드크로스·되돌림주의·추세적응·본전탈출도 매트릭스 셀에서 직접 적용
  //   (이전엔 매트릭스 미포함이라 resetEarlyExit가 강제 OFF로 박았음 → 이제 셀별 권위)
  ee.deadCross      = cell.deadCross;
  ee.dumpExit       = cell.dumpExit;
  ee.regimeAdaptive = cell.regimeAdaptive;
  // [S470] 지지선 이탈 청산 — 마스터 + MA20/60/120 (모드별: 단타20·60·120/스윙60·120/중기120)
  ee.supportBreakExit = cell.supportBreakExit;
  ee.sbMa20  = cell.sbMa20;
  ee.sbMa60  = cell.sbMa60;
  ee.sbMa120 = cell.sbMa120;
  ee.beEscape       = cell.beEscape;
  ee.beEscapeBasis  = cell.beEscapeBasis;
  ee.beEscapeArm    = cell.beEscapeArm;
  ee.beEscapeExit   = cell.beEscapeExit;

  if(typeof SXE._btEarlyExitSave === 'function') SXE._btEarlyExitSave();

  // 설정탭 조기청산 카드 라벨 동기화
  if(typeof _eeUpdateStatusLabel === 'function') _eeUpdateStatusLabel();
  // [S299] 조기청산 모달 청산 룰 카드 동기화 — 모달이 열려 있으면 즉시 반영, 닫혀 있으면 무해
  if(typeof window._eeRenderModeRuleCard === 'function') window._eeRenderModeRuleCard();
  // [S302] 조기청산 모달이 열려 있으면 토글/슬라이더 UI도 즉시 반영 (DOM null 안전)
  if(typeof window._eeRenderModalFromState === 'function') window._eeRenderModalFromState();

  console.log(`[S332] 조기청산 동기화: market=${resolved.mkt} × mode=${resolved.md} → BE=${ee.breakeven}, Trail=${ee.trailing}(${ee.trailMode}), MA=${ee.maBreak}, nHi=${ee.nHigh}, BT=${cell.btExitMode}/${cell.btTrailAtrMode}`);
}
...
function _optSwitchMarket(m){
  ...
  // [S332] 시장 변경 → 조기청산 9칸 매트릭스 재동기화
  //   배경: 이전엔 _optSyncEarlyExitToMode가 모드 변경 시에만 호출됐음.
  //         9칸 매트릭스가 도입되면서 시장만 바뀌어도 조기청산 셀이 달라지므로 (예: kr→coin 단타: profit→immediate)
  //         시장 변경 시점에도 동기화가 필요.
  //   동작: 현재 _optSortMode + 새 시장 m으로 매트릭스 재조회 → ee 상태/BT 청산 모드 갱신.
  if(typeof _optSyncEarlyExitToMode === 'function'){
    _optSyncEarlyExitToMode(_optSortMode, m);
  }
...
function _optSyncFromMarket(m){
  try {
    ...
    if(!document.getElementById('optStockChips')) {
      ...
      // [S335] 모달 닫힘 상태에서도 9칸 매트릭스 동기화 필수
      //   배경: 사용자가 옵티마이저 모달을 열지 않고 스크리너 메인의 시장 탭바만 눌러도
      //         조기청산/BT 청산 모드는 9칸 매트릭스의 새 시장 셀로 즉시 갱신되어야 함.
      //         _optSwitchMarket 분기(아래)에는 [S332]에서 이미 호출이 있지만,
      //         이 "모달 닫힘" 분기는 _optSwitchMarket을 건너뛰므로 별도로 호출해야 함.
      //   효과: 코인 탭바 클릭 → 코인 중기/스윙/단타 셀로 즉시 적용 (사진 9칸 정확 일치)
      if(typeof _optSyncEarlyExitToMode === 'function'){
        _optSyncEarlyExitToMode(_optSortMode, m);
      }
      return;
