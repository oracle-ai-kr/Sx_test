...
function _optSortScore(bt, mode){
  ...
  if(!_optCustomWeights && typeof calcBtScore === 'function'){
    const _sc = calcBtScore(bt);
    if(_sc != null) return _sc;
  }
...
const _EARLY_EXIT_MATRIX = Object.freeze({
  kr: {
    // 🛡️ 국내 중기: 본전·트레일 OFF, 추가룰 OFF, BT=종가+매봉재계산
    safe:     { score:false, breakeven:false, trailing:false, trailMode:'profit',
                bbUpper:false, maBreak:false, nHigh:false,
                btExitMode:'close', btTrailAtrMode:'dynamic' },
    // ⚖️ 국내 스윙: 본전·트레일 ON(수익권), 추가룰 OFF, BT=종가+매봉재계산
    balanced: { score:false, breakeven:true,  trailing:true,  trailMode:'profit',
                bbUpper:false, maBreak:false, nHigh:false,
                btExitMode:'close', btTrailAtrMode:'dynamic' },
    // 🔥 국내 단타: 본전·트레일 ON(수익권), MA 이탈 ON, BT=종가+매봉재계산
    profit:   { score:false, breakeven:true,  trailing:true,  trailMode:'profit',
                bbUpper:false, maBreak:true,  nHigh:false,
                btExitMode:'close', btTrailAtrMode:'dynamic' },
  },
  us: {
    // 🛡️ 해외 중기: 국내 중기와 동일
    ...
    balanced: { score:false, breakeven:true,  trailing:true,  trailMode:'profit',
                bbUpper:false, maBreak:false, nHigh:false,
                btExitMode:'close', btTrailAtrMode:'dynamic' },
    // 🔥 해외 단타: MA 이탈 OFF, N봉 신고점 ON (국내와 비대칭 — 미국 추세 강함, 전고점 익절)
    profit:   { score:false, breakeven:true,  trailing:true,  trailMode:'profit',
                bbUpper:false, maBreak:false, nHigh:true,
                btExitMode:'close', btTrailAtrMode:'dynamic' },
  },
  coin: {
    // 🛡️ 코인 중기: 본전·트레일 ON, BT 트레일ATR=진입고정 (코인 ATR 변동성 큼 → 매봉 재계산 시 트레일링 폭 출렁임)
    safe:     { score:false, breakeven:true,  trailing:true,  trailMode:'profit',
                bbUpper:false, maBreak:false, nHigh:false,
                btExitMode:'close', btTrailAtrMode:'entry' },     // ⚠️ 9칸 중 유일하게 entry
    // ⚖️ 코인 스윙: 국내/해외 스윙과 동일
    balanced: { score:false, breakeven:true,  trailing:true,  trailMode:'profit',
                bbUpper:false, maBreak:false, nHigh:false,
                btExitMode:'close', btTrailAtrMode:'dynamic' },
    // 🔥 코인 단타: 트레일 발동시점=진입즉시 (코인 24h 변동 — 첫 봉부터 따라가며 손실 차단)
    profit:   { score:false, breakeven:true,  trailing:true,  trailMode:'immediate',  // ⚠️ 9칸 중 유일하게 immediate
                bbUpper:false, maBreak:false, nHigh:false,
                btExitMode:'close', btTrailAtrMode:'dynamic' },
  }
});
...
function _optSyncEarlyExitToMode(mode, market){
  ...
  ee.maBreakLen   = _EARLY_EXIT_COMMON.maBreakLen;
