    ...
    for(const[mode, regimes] of Object.entries(modes)){
      for(const[regime, e] of Object.entries(regimes)){
        ...
        html+=`<div style="background:var(--surface2,#f8f8f8);border-radius:8px;padding:8px 10px;margin-bottom:4px;border:1px solid var(--border,#e0e0e0)">
          ...
          <div style="text-align:right;margin-top:3px"><span style="font-size:9px;color:var(--accent,#2563eb);cursor:pointer;text-decoration:underline" onclick="_optSaveBestPreset('${tf}','${mode}','${regime}')">프리셋 저장</span></div>
        </div>`;
      }
    }
  });

  html+=`<div style="display:flex;justify-content:space-between;margin-top:4px"><span style="font-size:9px;color:var(--text3,#999);cursor:pointer;text-decoration:underline" onclick="_optClearBest()">기록 초기화</span></div>`;
  area.innerHTML=html;
}
