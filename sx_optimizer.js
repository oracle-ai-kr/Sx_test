...
let _optSortMode = 'profit'; // 'profit' | 'balanced' | 'safe'
let _optRegimeMode = 'on'; // 'off' | 'on'
...
function _optSwitchMarket(m){
  ...
  if(typeof window.switchMarket === 'function' &&
    ...
    console.log(`[S256] _optSwitchMarket → switchMarket(${m}) 호출`);
    try { window.switchMarket(m); } catch(e){ console.warn(`[S256] switchMarket 예외: ${e.message}`); }
  } else {
    console.log(`[S256] _optSwitchMarket → switchMarket 호출 SKIP (이유: switchMarket=${typeof window.switchMarket}, syncFlag=${!!window._optMarketSyncFromScreener})`);
  }
}
