...
const OPT_DEFAULTS = {
  ...
  buyTh:   { min:30,  max:70,  step:2,   enabled:false },  // 1. 진입 점수 임계
  ...
  sellTh:  { min:20,  max:50,  step:2,   enabled:false },  // 6. 청산 점수
  // [지표 기간] — 표준값 고정 (step=0)
...
// [S293] 탐색 모드별 TP/SL 기본범위
//   모드 전환(수익형↔안정형↔보수형) 시 tpMult/slMult 범위를 자동으로 덮어씀
//   수익형(단타): 빠른 익절/손절 → TP 1~1.5 / SL 0.5~0.8
//   안정형(스윙): 중간 여유      → TP 3~4   / SL 1.5~2
//   보수형(중기): 넓은 여유      → TP 6~9   / SL 2~3
const OPT_MODE_TP_SL = {
  profit:   { tpMult:{min:1.0, max:1.5, step:0.1}, slMult:{min:0.5, max:0.8, step:0.1} },
  balanced: { tpMult:{min:3.0, max:4.0, step:0.1}, slMult:{min:1.5, max:2.0, step:0.1} },
  safe:     { tpMult:{min:6.0, max:9.0, step:0.1}, slMult:{min:2.0, max:3.0, step:0.1} },
};
...
async function _optResetRanges(opts){
  ...
  if(_showConfirm){
    const _conf = (typeof window !== 'undefined' && window.sxConfirm) ? window.sxConfirm : (m=>Promise.resolve(confirm(m)));
    // [S293] confirm 메시지에 현재 모드의 TP/SL 기본범위 동적 표시
    const _curMode = (typeof _optSortMode !== 'undefined') ? _optSortMode : 'balanced';
    const _tpsl = OPT_MODE_TP_SL[_curMode] || OPT_MODE_TP_SL.balanced;
    const _mLabel = {profit:'🔥 수익형', balanced:'⚖️ 안정형', safe:'🛡️ 보수형'}[_curMode] || '';
    const _tpLine = `TP ${_tpsl.tpMult.min}~${_tpsl.tpMult.max} / SL ${_tpsl.slMult.min}~${_tpsl.slMult.max}  (${_mLabel} 기준)`;
    if(!await _conf(`파라미터 범위를 기본값으로 초기화합니다.\n\nBUY 30~70 / BB 9~20 / BB배수 1.5~2.5\n${_tpLine}\nSELL 30~50 / RSI·ATR 14 고정 / MA 5/20/60 고정\n\n계속하시겠습니까?`)) return;
  }
...
// [S293] 탐색 모드에 맞는 TP/SL 범위를 OPT_DEFAULTS + UI 입력에 즉시 적용
//   호출 시점: 모드 전환(_optSetModeCore) / 기본범위설정(_optResetRanges)
//   적용 대상: tpMult, slMult (min/max/step)
//   부수 효과: _optSaveRanges() → localStorage 저장, _optUpdateCount() → 조합 수 UI 갱신
function _optApplyModeRanges(mode){
  const preset = OPT_MODE_TP_SL[mode];
  if(!preset) return;
  ['tpMult','slMult'].forEach(k => {
    const p = preset[k];
...
const _OPT_MODES = {
  profit:   { label:'🔥 수익형', desc:'수익률 > PF > 승률 > MDD' },
  balanced: { label:'⚖️ 안정형', desc:'MDD↓ ≈ 승률 > 수익률 ≈ PF (균형)' },
  safe:     { label:'🛡️ 보수형', desc:'MDD↓ > PF ≈ 승률 > 수익률' },
};
...
async function _optSetModeCore(mode, needsConfirm){
  ...
  // [S293] 탐색 모드 변경 → TP/SL 기본범위 자동 적용
  _optApplyModeRanges(mode);
  const mLabels = {profit:'🔥 수익형(단타)', balanced:'⚖️ 안정형(스윙)', safe:'🛡️ 보수형(중기)'};
...
async function _optToggleRepresentCore(market, entryId){
  ...
  if(bucket[repField] === entryId){
    ...
    if(entry.mode && ['profit','balanced','safe'].includes(entry.mode)){
      ...
      const _mSyncLabels = {profit:'🔥 수익형', balanced:'⚖️ 안정형', safe:'🛡️ 보수형'};
