...
function _optToggleRepresentCore(mode, entryId){
  ...
  if(bucket.representId === entryId){
    ...
    _saveOptBest(all);
    // S125: 대표 프리셋의 레짐 상태를 먼저 전역에 반영
    //   (이 순서가 중요 — 아래 _saveSlotParams가 현재 레짐 좌표를 참조하기 때문.
    //    단, 우리는 entry.regime을 명시적으로 슬롯 저장 시 전달하므로 순서 의존성은 없음.)
    if(entry.regime){
      const regOn = (entry.regime === 'ON' || entry.regime === true);
      if(typeof SXE !== 'undefined' && SXE.setRegimeAdapt) SXE.setRegimeAdapt(regOn);
      // S124 bugfix: 잘못된 '1'/'0' 저장 라인 제거 (SXE.setRegimeAdapt이 'on'/'off'로 저장).
      if(typeof _syncRegimeToggleUI === 'function') _syncRegimeToggleUI();
    }
    // S125: entry.params를 (mode, entry.regime) 좌표의 슬롯에 저장
    //   현재 화면 모드/레짐과 무관하게 대표 선정된 좌표 슬롯을 채움.
    //   현재 화면 좌표와 일치하면 _loadAnalParams()가 이 슬롯을 자동으로 반환.
    if(entry.params){
      const entryRegOn = (entry.regime === 'ON' || entry.regime === true);
      if(typeof SXE !== 'undefined' && SXE._saveSlotParams){
        SXE._saveSlotParams(mode, entryRegOn, entry.params);
      }
      // 현재 화면 모드와 일치하면 UI 즉시 갱신
      if(typeof _analMode !== 'undefined' && _analMode === mode){
        if(typeof _saveAnalParams === 'function') _saveAnalParams(entry.params); // 레거시 단일 키 미러링
        if(typeof loadAnalParamsUI === 'function') loadAnalParamsUI();
      }
    }
    _optRenderBestCards();
    toast(`★ ${mNames[mode]} 대표 선정 · 슬롯[${mode}.${entry.regime==='ON'||entry.regime===true?'on':'off'}] 저장됨`);
  }
}
