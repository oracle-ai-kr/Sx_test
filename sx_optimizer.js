...
function _optShowParamHelp(){
  ...
  const tables = {
    kr: {
      ...
      rows: [
        ...
        ['SELL 임계', '8',   '6~12, step 1'],
        ['BB 기간',   '11',  '10~14, step 1'],
        ['BB 배수',   '1.9', '1.8~2.1, step 0.1'],
        ['TP 배수',   '2.5', '2.0~3.0, step 0.25'],
        ['SL 배수',   '1.5', '1.3~1.8, step 0.1']
      ],
      note: 'BUY는 코드 기본값 41 근처로 신호 품질 유지. SELL은 5보다 살짝 높여서 약세 전환 빨리 감지. TP/SL도 너무 길게 잡지 않아 회전율 확보.'
    },
    us: {
      title: '🇺🇸 미국 주식 (일봉)',
      rows: [
    ...
    crypto: {
      ...
      note: '코인은 변동폭 자체가 커서 TP/SL은 크게 유지. 대신 단기형이니까 SELL 임계 8로 약세 빨리 잡고 빠지기.'
    }
  };
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface,#fff);border-radius:10px;width:100%;max-width:520px;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 4px 20px rgba(0,0,0,0.3)">
      ...
      <div id="optParamHelpBody" style="flex:1;overflow-y:auto;padding:12px 14px"></div>
      <div style="padding:10px 14px;border-top:1px solid var(--border,#ddd);background:var(--surface2,#f5f5f5)">
