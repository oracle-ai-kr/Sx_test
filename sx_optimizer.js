...
const OPT_MARKET_MODE_RANGES = {
  ...
  // ⚖️ 스윙 (balanced): 중간 여유
  balanced: {
    kr:   { bbLen:{min:14, max:14, step:0}, bbMult:{min:1.9, max:1.9, step:0}, buyTh:{min:35, max:50, step:1}, sellTh:{min:30, max:45, step:1}, tpMult:{min:3.0, max:4.0, step:0.1}, slMult:{min:1.5, max:2.0, step:0.1} },
    us:   { bbLen:{min:20, max:20, step:0}, bbMult:{min:2.0, max:2.0, step:0}, buyTh:{min:40, max:55, step:1}, sellTh:{min:35, max:45, step:1}, tpMult:{min:2.5, max:4.0, step:0.1}, slMult:{min:1.5, max:2.0, step:0.1} },
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:35, max:45, step:1}, sellTh:{min:30, max:45, step:1}, tpMult:{min:4.0, max:6.0, step:0.1}, slMult:{min:1.5, max:2.5, step:0.1} },
  },
  ...
  safe: {
    ...
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:55, max:70, step:1}, sellTh:{min:50, max:65, step:1}, tpMult:{min:12.0, max:18.0, step:0.1}, slMult:{min:3.0, max:4.5, step:0.1} },
  },
};
...
function _optSyncEarlyExitToMode(mode){
  ...
  if(mode === 'profit'){
    // 수익형: 조기청산 ON + 트레일링 진입즉시
    ee.enabled   = true;
    ee.trailing  = true;
    ee.trailMode = 'immediate';
  } else if(mode === 'balanced'){
    // 안정형: 조기청산 ON + 트레일링 수익권 진입 후
    ee.enabled   = true;
    ee.trailing  = true;
    ee.trailMode = 'profit';
  } else if(mode === 'safe'){
    // 보수형: 조기청산 OFF (TP/SL만 사용)
    ee.enabled   = false;
  }
  ...
  // [S299] 조기청산 모달 청산 룰 카드 동기화 — 모달이 열려 있으면 즉시 반영, 닫혀 있으면 무해 (DOM null 가드)
  if(typeof window._eeRenderModeRuleCard === 'function') window._eeRenderModeRuleCard();

  console.log(`[S292] 조기청산 동기화: mode=${mode} → enabled=${ee.enabled}, trailMode=${ee.trailMode||'-'}, btExitMode=close, btTrailAtrMode=entry`);
}
