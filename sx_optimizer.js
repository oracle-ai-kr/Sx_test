...
function _buildResultHTML(tfResults, code, regimeLabel){
  ...
  for(const[tf, results] of Object.entries(tfResults)){
    ...
    const sorted = results
      ...
      .sort((a,b)=>{
        const scoreA = (a.bt.winRate/100) * a.bt.totalPnl * Math.log(a.bt.totalTrades+1) * (1 - (a.bt.mdd||0)/100);
        const scoreB = (b.bt.winRate/100) * b.bt.totalPnl * Math.log(b.bt.totalTrades+1) * (1 - (b.bt.mdd||0)/100);
    ...
    top.forEach((r,i)=>{
      ...
      html += `<div class="${cardClass}">
        ...
        <div class="opt-result-actions">
          ...
          <button class="opt-btn-secondary" style="padding:6px;font-size:10px;border-radius:5px" onclick="_optSaveSlot(${JSON.stringify(p).replace(/"/g,'&quot;')},'${tf}','${_esc(code)}${suffix}')">💾 고정값 저장</button>
        </div>
      </div>`;
    });
...
function _renderDualResults(tfResultsOff, tfResultsOn, code, r2Off, r2On){
  ...
  let html = '';
  // 1차
  html += _buildMergedCards(tfResultsOff, tfResultsOn, tfLabels, code, '');
  // 2차
  if(r2Off && r2On){
    ...
    html += _buildMergedCards(r2Off, r2On, tfLabels, code, '2차');
  }

  if(_optResultList.length > 0) _lastTopParams = {..._optResultList[0].params};
  // 순위 공식 표기
  html += `<div style="margin-top:12px;padding:8px 10px;background:var(--surface2,#f5f5f5);border-radius:6px;font-size:9px;color:var(--text3,#999);text-align:center;line-height:1.5">순위산정: 승률 × 수익률 × log(거래수+1) × (1 − MDD/100)</div>`;
...
function _buildMergedCards(tfResOff, tfResOn, tfLabels, code, label){
  ...
  for(const tf of allTfs){
    const tfLabel = tfLabels[tf] || tf;
    // S77: 현재 탐색 파라미터의 엔진 기본값 표시
    const _enabledKeys = Object.keys(OPT_DEFAULTS).filter(k=>{ const el=document.getElementById('optEn_'+k); return el&&el.checked; });
    ...
    const _tfTh = (typeof SCR_TF_THRESHOLD!=='undefined'&&SCR_TF_THRESHOLD[tf]) ? SCR_TF_THRESHOLD[tf] : {buy:62,sell:38};
    const _tfMa = (typeof SCR_TF_MA!=='undefined'&&SCR_TF_MA[tf]) ? SCR_TF_MA[tf] : {short:5,mid:20,long:60};
    const _defMap = {rsiLen:14,bbLen:20,bbMult:2.0,atrLen:14,maShort:_tfMa.short,maMid:_tfMa.mid,maLong:_tfMa.long,buyTh:_tfTh.buy,sellTh:_tfTh.sell,tpMult:2.5,slMult:1.5};
    const _defStr = _enabledKeys.map(k=>`${_defLabels[k]||k}=${_defMap[k]||'?'}`).join(' ');
    const _defHint = _defStr ? ` <span style="font-size:9px;color:var(--text3);font-weight:400">기본값: ${_defStr}</span>` : '';
    html += `<div class="opt-tf-header">📊 ${tfLabel}${label?' ('+label+')':''}${_defHint}</div>`;
    const merged = [];
    const add = (res, regime) => {
      if(!res||!Array.isArray(res)) return;
      if(res.length===1&&res[0].error) return;
      res.filter(r=>r.bt&&r.bt.totalTrades>=3).forEach(r=>merged.push({params:r.params,bt:r.bt,regime,tf,code}));
    };
    add((tfResOff||{})[tf], 'OFF');
    add((tfResOn||{})[tf], 'ON');
    ...
    html += `<div id="optActionBar" style="display:flex;gap:6px;margin-bottom:6px">
      <button class="opt-btn opt-btn-primary" style="flex:1;padding:8px;font-size:11px" onclick="_optApplySelected()">🔒 고정 적용</button>
      <button class="opt-btn opt-btn-secondary" style="flex:1;padding:8px;font-size:11px" onclick="_optSaveSelected()">💾 고정값 저장</button>
    </div>`;
    ...
    top.forEach((r,i)=>{
      ...
      html += `<div class="opt-result-card" id="optCard_${gi}" onclick="_optSelectCard(${gi})" style="cursor:pointer;${borderStyle}">
        <div style="display:flex;align-items:center"><span class="opt-rank ${rankClass}">${i+1}</span><span style="font-size:12px;font-weight:700">TOP ${i+1}</span>${regTag}</div>
        <div class="opt-result-params">${paramStr}</div>
        <div class="opt-result-stats">
          <div><div class="stat-val">${b.winRate.toFixed(1)}%</div><div class="stat-lbl">승률</div></div>
          <div><div class="stat-val" style="color:${pnlColor}">${b.totalPnl>=0?'+':''}${b.totalPnl.toFixed(1)}%</div><div class="stat-lbl">수익률</div></div>
...
function _optSaveSelected(){
  const keys = Object.keys(_optLocked);
  if(keys.length === 0){ toast('고정된 파라미터가 없습니다. 먼저 🔒 고정하세요'); return; }
  const market = _optMarket || 'kr';
  // 현재 활성 파라미터에 고정값 오버라이드
  const merged = { ..._loadAnalParams(), ..._optLocked };
  // TF 라벨
  const tfLabels = {};
  if(typeof TF_MAP !== 'undefined') (TF_MAP[market]||[]).forEach(t=>{ tfLabels[t.k]=t.l; });
  const selTfs = _getSelectedTFs();
  const tfLabel = selTfs.length > 0 ? (tfLabels[selTfs[0]] || selTfs[0]) : '';
  const code = _optSelectedCode || '';
  const name = `${code}_${tfLabel}_최적(${keys.length}개)`;
  const slots = _loadMarketSlots(market);
  if(slots.length >= SCR_ANAL_MAX_SLOTS){ toast(`슬롯 가득참 (최대 ${SCR_ANAL_MAX_SLOTS}개)`); return; }
  const ok = _saveSlot(market, name, merged, -1);
  if(ok){
    if(typeof updateParamMarketStatus === 'function') updateParamMarketStatus();
    toast(`✅ "${name}" 저장됨 (${keys.length}개 고정값 포함)`);
  } else toast('저장 실패');
}
