...
const _OPT_MODE_WEIGHTS = {
  profit:   { pnl:40, trades:5, mdd:20, wr:30, pf:5 },  // [S311] 사용자 사진: 수익률40·MDD30·승률20 강조
  balanced: { pnl:40, trades:5, mdd:20, wr:30, pf:5 },  // [S311] 3모드 동일 가중치
  safe:     { pnl:40, trades:5, mdd:20, wr:30, pf:5 }   // [S311] 모드 차별성은 타이브레이커(순위)로만
};
...
const _OPT_MODE_TIEBREAK_DEFAULTS = {
  ...
  balanced: { pnl:1, trades:4, mdd:3, wr:2, pf:5 },
  safe:     { pnl:1, trades:4, mdd:3, wr:2, pf:5 }
};
...
window._optUpdateCount = function(){
  ...
  // [S315] BT당 추정 시간 0.3 → 0.15초 — 실측 기반 보정 (10회 ≈ 1.5초)
  const estSec = Math.ceil(total * 0.15);
  ...
  if(_optCheckedParams && _optCheckedParams.size > 0){
    ...
    if(checkedArr.length > 0){
      ...
      // [S315] BT당 추정 시간 0.01 → 0.15초 — 실측 기반 보정
      //   이전 0.01은 명백한 오타로 보임(10배 단위 오류). 메인과 동일 0.15 적용
      const compSec = Math.ceil(compBT * 0.15);
...
function _optSyncEarlyExitToMode(mode){
  ...
  if(mode === 'profit'){
    // 🔥 단타: 점수 ON + 트레일 진입즉시 + MA20 이탈 + N봉 60 신고점 (가장 공격적)
    // [S307] bbUpper true→false — 단타에서도 BB 상단 익절 룰은 OFF 유지
    // [S311] nHighBars 20→60 — 사용자 사진 기준 (단타도 직전 60봉 신고점 익절)
    ee.score      = true;
    ...
    ee.maBreakLen = 20;
    ee.nHigh      = true;
    ee.nHighBars  = 60; // [S311] 20→60
  } else if(mode === 'balanced'){
    // ⚖️ 스윙: 점수 ON + 트레일 수익권 후 + MA60 이탈 (휩소 방지)
    // [S311] score false→true, maBreakLen 120→60, nHigh true→false — 사용자 사진 기준
    //   변경 의미: 스윙도 점수 청산 활용, MA 기간 단축(60), 직전 고점 익절은 미사용
    ee.score      = true;  // [S311] false→true
    ee.trailMode  = 'profit';
    ee.bbUpper    = false;
    ee.maBreak    = true;
    ee.maBreakLen = 60;    // [S311] 120→60
    ee.nHigh      = false; // [S311] true→false
    ee.nHighBars  = 60;
  } else if(mode === 'safe'){
    // 🛡️ 중기: 점수 OFF + 트레일 수익권 후 + 보조 룰 최소화 (가장 보수)
    // [S311] maBreak true→false — 사용자 사진 기준 (중기는 MA 이탈 손절도 끔)
    //   설계 의도: 중기는 TP/SL과 트레일/본전만으로 잡고 보조 룰은 비활성화
    ee.score      = false;
    ee.trailMode  = 'profit';
    ee.bbUpper    = false;
    ee.maBreak    = false; // [S311] true→false
    ee.maBreakLen = 60;
    ee.nHigh      = false;
    ee.nHighBars  = 60;    // [S311] 20→60 (UI 슬라이더 일관성)
  }
...
function _optResetWeights(){
  _optVib(15);
  const defaults = {
    profit:   { pnl:40, trades:5, mdd:20, wr:30, pf:5 },
    balanced: { pnl:40, trades:5, mdd:20, wr:30, pf:5 },
    safe:     { pnl:40, trades:5, mdd:20, wr:30, pf:5 }
  };
