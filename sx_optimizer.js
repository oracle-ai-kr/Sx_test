...
const OPT_DEFAULTS = {
  ...
  buyTh:   { min:30,  max:65,  step:5,   enabled:false },  // 1. 진입 점수 임계
  bbLen:   { min:14,   max:14,  step:0,   enabled:false },  // 2. BB 기간 (신호 빈도)
  bbMult:  { min:1.9, max:1.9, step:0, enabled:false },  // 3. BB 배수 (진입 폭)
  // [청산 결정] — 진입 안정화 후 최적화
  tpMult:  { min:1.0, max:9.0, step:0.5, enabled:false },  // 4. 익절 폭
  slMult:  { min:0.5, max:4.5, step:0.5, enabled:false },  // 5. 손절 폭
  sellTh:  { min:20,  max:50,  step:5,   enabled:false },  // 6. 청산 점수
  // [지표 기간] — 표준값 고정 (step=0)
...
const OPT_MARKET_MODE_RANGES = {
  ...
  profit: {
    ...
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:40, max:55, step:1}, sellTh:{min:25, max:40, step:1}, tpMult:{min:1.5, max:2.2, step:0.1}, slMult:{min:0.7, max:1.1, step:0.1} },
  },
  // ⚖️ 스윙 (balanced): 중간 여유
  balanced: {
    ...
    us:   { bbLen:{min:20, max:20, step:0}, bbMult:{min:2.0, max:2.0, step:0}, buyTh:{min:40, max:55, step:1}, sellTh:{min:25, max:40, step:1}, tpMult:{min:2.8, max:4.0, step:0.1}, slMult:{min:1.4, max:2.0, step:0.1} },
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:50, max:65, step:1}, sellTh:{min:35, max:50, step:1}, tpMult:{min:6.0, max:9.0, step:0.1}, slMult:{min:3.0, max:4.5, step:0.1} },
  },
  // 🛡️ 중기 (safe): 넓은 여유
  safe: {
    kr:   { bbLen:{min:14, max:14, step:0}, bbMult:{min:1.9, max:1.9, step:0}, buyTh:{min:50, max:65, step:1}, sellTh:{min:35, max:50, step:1}, tpMult:{min:6.0,  max:9.0,  step:0.1}, slMult:{min:3.0, max:4.5, step:0.1} },
    us:   { bbLen:{min:20, max:20, step:0}, bbMult:{min:2.0, max:2.0, step:0}, buyTh:{min:45, max:60, step:1}, sellTh:{min:30, max:45, step:1}, tpMult:{min:4.0,  max:6.0,  step:0.1}, slMult:{min:2.0, max:3.0, step:0.1} },
    coin: { bbLen:{min: 9, max: 9, step:0}, bbMult:{min:2.1, max:2.1, step:0}, buyTh:{min:55, max:70, step:1}, sellTh:{min:40, max:55, step:1}, tpMult:{min:9.0, max:13.5, step:0.1}, slMult:{min:4.5, max:6.7, step:0.1} },
  },
};
...
const _OPT_MODE_WEIGHTS = {
  profit:   { pnl:5,  trades:5, mdd:80, wr:5, pf:5  },  // [S292] 수익률 100% 집중
  balanced: { pnl:5,  trades:5, mdd:80, wr:5, pf:5  },  // [S292] 승률·수익률·MDD 균형
  safe:     { pnl:5,  trades:5, mdd:80, wr:5, pf:5  }   // [S292] MDD 단독 1위, 승률 2위
};
...
const _OPT_MODE_TIEBREAK_DEFAULTS = {
  profit:   { pnl:2, trades:4, mdd:1, wr:3, pf:5 },
  balanced: { pnl:2, trades:4, mdd:1, wr:3, pf:5 },
  safe:     { pnl:2, trades:4, mdd:1, wr:3, pf:5 }
};
...
function _optSyncEarlyExitToMode(mode){
  ...
  if(mode === 'profit'){
    // 🔥 단타: 점수 ON + 트레일 진입즉시 + MA20 이탈 + N봉 20 신고점 (가장 공격적)
    // [S307] bbUpper true→false 사용자 요청 — 단타에서도 BB 상단 익절 룰은 OFF 유지
    ee.score      = true;
    ee.trailMode  = 'immediate';
    ee.bbUpper    = false; // [S307] true→false
    ee.maBreak    = true;
    ee.maBreakLen = 20;
    ee.nHigh      = true;
    ee.nHighBars  = 20;
...
function _optResetWeights(){
  _optVib(15);
  const defaults = {
    profit:   { pnl:5, trades:5, mdd:80, wr:5, pf:5 },
    balanced: { pnl:5, trades:5, mdd:80, wr:5, pf:5 },
    safe:     { pnl:5, trades:5, mdd:80, wr:5, pf:5 }
  };
