...
function _optSortScore(bt, mode){
  ...
  const t = Math.log(Math.max((bt.totalTrades||0), 1) + 1);
  const m = Math.max(1 - (bt.mdd||0) / 100, 0.01);
  if(mode==='profit') return Math.pow(p,1.3) * Math.pow(t,0.7) * Math.pow(m,0.5) * Math.pow(w,0.5);
  if(mode==='safe')   return Math.pow(p,0.7) * t * Math.pow(m,2.0) * w;
  /* balanced */       return p * Math.pow(t,1.3) * m * Math.pow(w,0.7);
}
...
function _optSortCompare(a, b, mode){
  ...
  else if(mode==='safe')    order = [[mddA,mddB,-1],[pnlA,pnlB,1],[trdA,trdB,1],[wrA,wrB,1]];
  else /* balanced */        order = [[trdA,trdB,1],[pnlA,pnlB,1],[mddA,mddB,-1],[wrA,wrB,1]];
...
function _optRenderModeGate(){
  ...
  if(!gate) return;
  const modeDescs = {
    profit: '수익률 가중 종합(10%대역) → 수익률 > 거래수 > MDD↓ > 승률',
    balanced: '균형 가중 종합(10%대역) → 거래수 > 수익률 > MDD↓ > 승률',
    safe: 'MDD 가중 종합(10%대역) → MDD↓ > 수익률 > 거래수 > 승률'
  };
