...
const _OPT_MODES = {
  profit:   { label:'🔥 수익형', desc:'수익률 > 거래수 > 승률' },
  balanced: { label:'⚖️ 안정형', desc:'수익률 > 거래수 > MDD > 승률' },
  safe:     { label:'🛡️ 보수형', desc:'수익률 > MDD > 거래수' },
};
// S91: 모드별 다중필드 순차비교 정렬 (수익률 1순위 공통)
// profit:   수익률 → 거래수 → 승률
// safe:     수익률 → MDD낮음 → 거래수
// balanced: 수익률 → 거래수 → MDD낮음 → 승률
function _optSortCompare(a, b, mode){
  const da = (a.totalPnl||0) - (b.totalPnl||0);
  if(Math.abs(da) > 0.01) return da; // 1순위: 수익률 (높을수록)
  if(mode==='profit'){
    const dt = (a.totalTrades||0) - (b.totalTrades||0);
    if(dt !== 0) return dt; // 2순위: 거래수 (많을수록)
    return (a.winRate||0) - (b.winRate||0); // 3순위: 승률
  }
  if(mode==='safe'){
    const dm = (b.mdd||0) - (a.mdd||0); // 2순위: MDD (낮을수록 = b-a)
    if(Math.abs(dm) > 0.01) return dm;
    return (a.totalTrades||0) - (b.totalTrades||0); // 3순위: 거래수
  }
  // balanced
  const dt = (a.totalTrades||0) - (b.totalTrades||0); // 2순위: 거래수
  if(dt !== 0) return dt;
  const dm = (b.mdd||0) - (a.mdd||0); // 3순위: MDD (낮을수록)
  if(Math.abs(dm) > 0.01) return dm;
  return (a.winRate||0) - (b.winRate||0); // 4순위: 승률
}
...
function _optRenderModeGate(){
  ...
  if(!gate) return;
  const modeDescs = {
    profit: '수익률 > 거래수 > 승률 — 공격적 수익 추구',
    balanced: '수익률 > 거래수 > MDD > 승률 — 균형 잡힌 분석',
    safe: '수익률 > MDD > 거래수 — 안정적 수익 우선'
  };
...
function _buildMergedCards(tfResOff, tfResOn, tfLabels, code, label){
  ...
  for(const tf of allTfs){
    ...
    const _modeFormula = {profit:'수익률 > 거래수 > 승률', balanced:'수익률 > 거래수 > MDD > 승률', safe:'수익률 > MDD > 거래수'};
    html += `<div style="font-size:9px;color:var(--text3,#999);text-align:center;margin-top:2px;margin-bottom:6px">종합 = 승률 × 수익률 × log(거래수+1) × (1−MDD/100) | 정렬: ${_modeFormula[_optSortMode]||''}</div>`;
