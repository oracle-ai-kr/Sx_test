            ...
            for(const tf of tfs){
              ...
              for(const code of _svValid){
                try{
                  ...
                  console.log(`[S241] [${tf}/OFF] ${code} 호출: rows.length=${fullRows?.length||0} · 레짐ON=${SXE.regimeAdaptEnabled()}`);
                  ...
                  try{
                    if(fullRows && fullRows.length > 0){
                      ...
                      console.log(`[S242] [${tf}/OFF] ${code} 입력데이터: ${fullRows.length}봉 · 첫=${_fD}(C${_fC}) · 끝=${_lD}(C${_lC})`);
                    }
                    ...
                    if(typeof _loadAnalParams === 'function'){
                      ...
                      console.log(`[S242] [${tf}/OFF] _loadAnalParams: ${JSON.stringify(_ap)}`);
                    }
                    console.log(`[S242] [${tf}/OFF] 전역플래그: applySafetyToBt=${!!SXE._applySafetyToBt} · btEntryGates=${!!SXE._btEntryGates} · btEarlyExit=${!!(SXE._btEarlyExit && SXE._btEarlyExit.enabled)}`);
                    ...
                    try{
                      ...
                      if(_idx530 >= 0){
                        ...
                        console.log(`[S249] [${tf}/OFF] ${code} 20250530+30봉 close (idx ${_idx530}~${_idx530+_slice.length-1}/${fullRows.length}): ${_closes}`);
                      } else {
                        console.log(`[S249] [${tf}/OFF] ${code} 20250530 봉 인덱스 없음 (rows ${(fullRows||[]).length}봉)`);
                      }
                    ...
                    try{
                      ...
                      console.log(`[S250] [${tf}/OFF] ${code} close 비정상 봉: ${_badRows.length}개 / 전체 ${(fullRows||[]).length}봉`);
                      if(_badRows.length > 0){
                        ...
                        console.log(`[S250] [${tf}/OFF] ${code} 손상샘플(앞 5개): ${_samples}`);
                        const _allIdx = _badRows.map(({i}) => i);
                        console.log(`[S250] [${tf}/OFF] ${code} 손상 인덱스 전체: [${_allIdx.join(',')}]`);
                      }
                    ...
                    try{
                      if(typeof currentAnalStock !== 'undefined' && currentAnalStock && currentAnalStock.code === code && Array.isArray(currentAnalStock._lastAnalCandles) && currentAnalStock._lastAnalCandles.length > 0){
                        ...
                        console.log(`[S243] [${tf}/OFF] ${code} 데이터비교: 분석탭(${_ana.length}봉) 첫=${_aFD}(C${_aFC})/끝=${_aLD}(C${_aLC}) vs 옵티(${fullRows.length}봉) 첫=${_fD}(C${_fC})/끝=${_lD}(C${_lC}) → 봉수${_sameLen?'✓':'✗'} 첫봉${_sameFirst?'✓':'✗'} 끝봉${_sameLast?'✓':'✗'}`);
                      } else {
                        console.log(`[S243] [${tf}/OFF] ${code} 데이터비교: 분석탭 데이터 없음 (currentAnalStock 미일치 또는 _lastAnalCandles 비어있음) → 비교 스킵`);
                      }
                  ...
                  if(r && !r.error){
                    console.log(`[S241] [${tf}/OFF] ${code} raw결과: rowsLength=${r.rowsLength} 거래=${r.totalTrades} 승률=${(r.winRate||0).toFixed(1)}% 수익=${(r.totalPnl||0).toFixed(2)}% MDD=${r.mdd||0}% PF=${r.profitFactor||0} 게이트차단=${r.gateBlocks||0}`);
                    // [S246] 진입 날짜 리스트 — 분석탭 [S246]과 직접 비교용
                    try{
                      const _trades = (r.trades || []).map(t => `${t.entryDate||'?'}/${t.type||'?'}/${(t.pnl||0).toFixed(1)}`).join(' | ');
                      console.log(`[S246] [${tf}/OFF] ${code} 진입목록(${(r.trades||[]).length}건): ${_trades}`);
                    ...
                    try{
                      ...
                      console.log(`[S247] [${tf}/OFF] ${code} 거래상세: ${_details}`);
                    ...
                    try{
                      ...
                      console.log(`[S248] [${tf}/OFF] ${code} 청산상세: ${_exits}`);
                    }catch(_e){ console.warn(`[S248] 청산상세 로그 예외: ${_e.message}`); }
                  } else if(r && r.error){
                    console.log(`[S241] [${tf}/OFF] ${code} raw에러: ${r.error}`);
                  }
                  ...
                  if(_optPerTradeFilter && r && r.trades){
                    ...
                    console.log(`[S241] [${tf}/OFF] ${code} 건별필터 적용: 거래 ${_rawTrades}→${r.totalTrades} · 수익 ${(_rawPnl||0).toFixed(2)}%→${(r.totalPnl||0).toFixed(2)}% (minWin=${_optMinWinPnl}% maxLoss=${_optMaxLossPnl}%)`);
                  }
            ...
            for(const tf of tfs){
              ...
              for(const code of _svValid){
                try{
                  ...
                  console.log(`[S241] [${tf}/ON] ${code} 호출: rows.length=${fullRows?.length||0} · 레짐ON=${SXE.regimeAdaptEnabled()}`);
                  ...
                  try{
                    if(fullRows && fullRows.length > 0){
                      ...
                      const _fC = _f?.close ?? _f?.c ?? '?';
                      const _lC = _l?.close ?? _l?.c ?? '?';
                      console.log(`[S242] [${tf}/ON] ${code} 입력데이터: ${fullRows.length}봉 · 첫=${_fD}(C${_fC}) · 끝=${_lD}(C${_lC})`);
                    }
                    if(typeof _loadAnalParams === 'function'){
                      const _ap = _loadAnalParams();
                      console.log(`[S242] [${tf}/ON] _loadAnalParams: ${JSON.stringify(_ap)}`);
                    }
                    console.log(`[S242] [${tf}/ON] 전역플래그: applySafetyToBt=${!!SXE._applySafetyToBt} · btEntryGates=${!!SXE._btEntryGates} · btEarlyExit=${!!(SXE._btEarlyExit && SXE._btEarlyExit.enabled)}`);
                    ...
                    try{
                      ...
                      if(_idx530 >= 0){
                        const _slice = fullRows.slice(_idx530, Math.min(_idx530 + 30, fullRows.length));
                        const _closes = _slice.map(r => `${r.date||r.t}:${r.close}`).join(' | ');
                        console.log(`[S249] [${tf}/ON] ${code} 20250530+30봉 close (idx ${_idx530}~${_idx530+_slice.length-1}/${fullRows.length}): ${_closes}`);
                      } else {
                        console.log(`[S249] [${tf}/ON] ${code} 20250530 봉 인덱스 없음 (rows ${(fullRows||[]).length}봉)`);
                      }
                    }catch(_e){ console.warn(`[S249] close 비교 로그 예외: ${_e.message}`); }
                    // [S250] close 비정상 봉 전수조사
                    try{
                      const _badRows = (fullRows||[]).map((r, i) => ({i, r}))
                        .filter(({r}) => r.close == null || isNaN(r.close) || r.close === 0);
                      console.log(`[S250] [${tf}/ON] ${code} close 비정상 봉: ${_badRows.length}개 / 전체 ${(fullRows||[]).length}봉`);
                      if(_badRows.length > 0){
                        const _samples = _badRows.slice(0, 5).map(({i, r}) =>
                          `idx=${i}/date=${r.date||r.t}/o=${r.open}/h=${r.high}/l=${r.low}/c=${r.close}/v=${r.volume}`
                        ).join(' | ');
                        console.log(`[S250] [${tf}/ON] ${code} 손상샘플(앞 5개): ${_samples}`);
                        const _allIdx = _badRows.map(({i}) => i);
                        console.log(`[S250] [${tf}/ON] ${code} 손상 인덱스 전체: [${_allIdx.join(',')}]`);
                      }
                    }catch(_e){ console.warn(`[S250] 손상조사 로그 예외: ${_e.message}`); }
                    ...
                    try{
                      if(typeof currentAnalStock !== 'undefined' && currentAnalStock && currentAnalStock.code === code && Array.isArray(currentAnalStock._lastAnalCandles) && currentAnalStock._lastAnalCandles.length > 0){
                        ...
                        const _sameFirst = _fD === _aFD && _fC === _aFC;
                        const _sameLast = _lD === _aLD && _lC === _aLC;
                        console.log(`[S243] [${tf}/ON] ${code} 데이터비교: 분석탭(${_ana.length}봉) 첫=${_aFD}(C${_aFC})/끝=${_aLD}(C${_aLC}) vs 옵티(${fullRows.length}봉) 첫=${_fD}(C${_fC})/끝=${_lD}(C${_lC}) → 봉수${_sameLen?'✓':'✗'} 첫봉${_sameFirst?'✓':'✗'} 끝봉${_sameLast?'✓':'✗'}`);
                      } else {
                        console.log(`[S243] [${tf}/ON] ${code} 데이터비교: 분석탭 데이터 없음 → 비교 스킵`);
                      }
                    }catch(_e){ console.warn(`[S243] 데이터비교 예외: ${_e.message}`); }
                  }catch(_e){ console.warn(`[S242] 진단 로그 예외: ${_e.message}`); }
                  let r = sxRunBtEngine(fullRows, tf, _svBtP, {slippage:0.001, nextBarEntry:false, applyRegimeAdjust:true});
                  // [S241] raw 결과
                  if(r && !r.error){
                    console.log(`[S241] [${tf}/ON] ${code} raw결과: rowsLength=${r.rowsLength} 거래=${r.totalTrades} 승률=${(r.winRate||0).toFixed(1)}% 수익=${(r.totalPnl||0).toFixed(2)}% MDD=${r.mdd||0}% PF=${r.profitFactor||0} 게이트차단=${r.gateBlocks||0}`);
                    // [S246] 진입 날짜 리스트 — 분석탭과 직접 비교용
                    try{
                      const _trades = (r.trades || []).map(t => `${t.entryDate||'?'}/${t.type||'?'}/${(t.pnl||0).toFixed(1)}`).join(' | ');
                      console.log(`[S246] [${tf}/ON] ${code} 진입목록(${(r.trades||[]).length}건): ${_trades}`);
                    }catch(_e){ console.warn(`[S246] 진입목록 로그 예외: ${_e.message}`); }
                    ...
                    try{
                      ...
                      console.log(`[S247] [${tf}/ON] ${code} 거래상세: ${_details}`);
                    }catch(_e){ console.warn(`[S247] 거래상세 로그 예외: ${_e.message}`); }
                    // [S248] 거래 청산 상세
                    try{
                      const _exits = (r.trades || []).map(t =>
                      ...
                      console.log(`[S248] [${tf}/ON] ${code} 청산상세: ${_exits}`);
                    }catch(_e){ console.warn(`[S248] 청산상세 로그 예외: ${_e.message}`); }
                  } else if(r && r.error){
                    console.log(`[S241] [${tf}/ON] ${code} raw에러: ${r.error}`);
                  }
                  // [S241] 필터 적용 전후
                  const _rawTrades = r ? r.totalTrades : 0;
                  const _rawPnl = r ? r.totalPnl : 0;
                  if(_optPerTradeFilter && r && r.trades){
                    r = _optFilterBtResult(r);
                    console.log(`[S241] [${tf}/ON] ${code} 건별필터 적용: 거래 ${_rawTrades}→${r.totalTrades} · 수익 ${(_rawPnl||0).toFixed(2)}%→${(r.totalPnl||0).toFixed(2)}% (minWin=${_optMinWinPnl}% maxLoss=${_optMaxLossPnl}%)`);
                  }
                  if(r && r.error){ _errors++; continue; }
                  if(!r || r.totalTrades < 1){ _zeroTrades++; continue; }
                  sW += r.winRate; sP += r.totalPnl; sT += r.totalTrades; sM += (r.mdd||0);
...
function _optSwitchMarket(m){
  ...
  console.log(`[S256] _optSwitchMarket 진입: m=${m}, _optMarket=${_optMarket}, currentMarket=${typeof window.currentMarket !== 'undefined' ? window.currentMarket : '?'}, syncFlag=${!!window._optMarketSyncFromScreener}`);
  _optVib(10);
  _optMarket = m;
  // 버튼 활성화
  ...
  if(typeof window.switchMarket === 'function' &&
     typeof window.currentMarket !== 'undefined' &&
     m !== window.currentMarket &&
     !window._optMarketSyncFromScreener){
    console.log(`[S256] _optSwitchMarket → switchMarket(${m}) 호출 (currentMarket=${window.currentMarket} → ${m})`);
    try { window.switchMarket(m); } catch(e){ console.warn(`[S256] switchMarket 예외: ${e.message}`); }
  } else {
    console.log(`[S256] _optSwitchMarket → switchMarket 호출 SKIP (이유: switchMarket=${typeof window.switchMarket}, currentMarket=${typeof window.currentMarket !== 'undefined' ? window.currentMarket : '?'}, m=${m}, syncFlag=${!!window._optMarketSyncFromScreener})`);
  }
}
...
function _optSyncFromMarket(m){
  try {
    console.log(`[S256] _optSyncFromMarket 진입: m=${m}, _optMarket=${_optMarket}, modalOpen=${!!document.getElementById('optStockChips')}`);
    if(!m || m === _optMarket) {
      console.log(`[S256] _optSyncFromMarket SKIP — 이미 같은 시장`);
      return;
    }
    if(!document.getElementById('optStockChips')) {
      // 옵티마이저 모달 닫힌 상태 — _optMarket만 변수 갱신 (다음 모달 열 때 적용됨)
      console.log(`[S256] _optSyncFromMarket 모달 닫힘 — 변수만 갱신`);
      _optMarket = m;
      if(typeof _normMkt === 'function') _optBestViewMarket = _normMkt(m);
      return;
    }
    console.log(`[S256] _optSyncFromMarket → _optSwitchMarket(${m}) 호출 (가드 ON)`);
    ...
    try {
      ...
      setTimeout(() => { window._optMarketSyncFromScreener = false; console.log(`[S256] _optMarketSyncFromScreener 가드 해제`); }, 50);
    }
