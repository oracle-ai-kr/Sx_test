...
window._optUpdateCount = function(){
  ...
  // [S315] BT당 추정 시간 0.3 → 0.1초 — 실측 기반 보정 (10회 ≈ 1초)
  const estSec = Math.ceil(total * 0.1);
  ...
  if(_optCheckedParams && _optCheckedParams.size > 0){
    ...
    if(checkedArr.length > 0){
      ...
      // [S315] BT당 추정 시간 0.01 → 0.1초 — 실측 기반 보정
      //   이전 0.01은 명백한 오타로 보임(10배 단위 오류). 메인과 동일 0.1 적용
      const compSec = Math.ceil(compBT * 0.1);
