...
// [S297] 시장 × 탐색 모드별 BUY/SELL/TP/SL 기본범위 (9칸 × 4파라미터)
...
//             = 어느 쪽이든 자동 양방향 동기화
//   적용 파라미터: buyTh, sellTh, tpMult, slMult (4개)
//   비적용: BB/RSI/ATR/MA는 시장·모드 무관하게 OPT_DEFAULTS 값 유지
//   〔이력〕 [S293] OPT_MODE_TP_SL (모드 3 × TP/SL 2개) → [S297] OPT_MARKET_MODE_RANGES (모드 3 × 시장 3 × 4파라미터)
const OPT_MARKET_MODE_RANGES = {
  // 🔥 단타 (profit): 빠른 익절/손절, 진입 임계 낮음
  profit: {
    kr:   { buyTh:{min:25, max:40, step:1}, sellTh:{min:20, max:35, step:1}, tpMult:{min:1.0, max:1.5, step:0.1}, slMult:{min:0.5, max:0.8, step:0.1} },
    us:   { buyTh:{min:20, max:35, step:1}, sellTh:{min:15, max:30, step:1}, tpMult:{min:0.7, max:1.0, step:0.1}, slMult:{min:0.3, max:0.5, step:0.1} },
    coin: { buyTh:{min:30, max:45, step:1}, sellTh:{min:25, max:40, step:1}, tpMult:{min:1.5, max:2.5, step:0.1}, slMult:{min:0.8, max:1.2, step:0.1} },
  },
  // ⚖️ 스윙 (balanced): 중간 여유
  balanced: {
    kr:   { buyTh:{min:35, max:50, step:1}, sellTh:{min:30, max:45, step:1}, tpMult:{min:3.0, max:4.0, step:0.1}, slMult:{min:1.5, max:2.0, step:0.1} },
    us:   { buyTh:{min:40, max:55, step:1}, sellTh:{min:35, max:45, step:1}, tpMult:{min:2.5, max:4.0, step:0.1}, slMult:{min:1.5, max:2.0, step:0.1} },
    coin: { buyTh:{min:35, max:45, step:1}, sellTh:{min:30, max:45, step:1}, tpMult:{min:4.0, max:6.0, step:0.1}, slMult:{min:1.5, max:2.5, step:0.1} },
  },
  // 🛡️ 중기 (safe): 넓은 여유, 국내/해외 동일 · 코인만 별도
  safe: {
    kr:   { buyTh:{min:50, max:65, step:1}, sellTh:{min:45, max:60, step:1}, tpMult:{min:6.0,  max:9.0,  step:0.1}, slMult:{min:2.0, max:3.0, step:0.1} },
    us:   { buyTh:{min:50, max:65, step:1}, sellTh:{min:45, max:60, step:1}, tpMult:{min:6.0,  max:9.0,  step:0.1}, slMult:{min:2.0, max:3.0, step:0.1} },
    coin: { buyTh:{min:55, max:70, step:1}, sellTh:{min:50, max:65, step:1}, tpMult:{min:12.0, max:18.0, step:0.1}, slMult:{min:3.0, max:4.5, step:0.1} },
  },
};
...
async function _optResetRanges(opts){
  ...
  if(_showConfirm){
    const _conf = (typeof window !== 'undefined' && window.sxConfirm) ? window.sxConfirm : (m=>Promise.resolve(confirm(m)));
    // [S297] confirm 메시지에 현재 시장 × 모드의 BUY/SELL/TP/SL 기본범위 동적 표시
