          ...
          try {
            ...
            if (rawRows.length >= 60 && typeof SXE !== 'undefined' && SXE.runBtEngine) {
              ...
              if (btResult && !btResult.error) {
                ...
                // [2026-04 FIX] 분석탭 재판정 정합성 복구 — 스캔 시점에도 SXE.scoreMomentum 계산.
                //   이전: _svMom=null → supervisorJudge가 direction='flat'으로 처리 → 분석탭(dir=up/down)과
                //   판정이 어긋나 "관심으로 걸러진 종목이 상세 진입 시 관망/회피로 바뀌는" 버그 발생.
                //   해결: 스캔워커도 rawRows 기반으로 모멘텀 산출 → 분석탭과 동일 입력 → 동일 C 판정.
                try {
                  const _svAnalScore = qs.readyScore ?? qs.score ?? 0;
                  // [2026-04 FIX] rawRows.length >= 80 조건은 sx_render.js runAnalysis와 동일.
                  //   scoreMomentum은 5봉 룩백 + 내부 가중치 계산을 위해 최소 80봉 필요.
