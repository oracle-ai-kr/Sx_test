  ...
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'knn_dday': {   // [S633] kNN 크로스 임박 D-day — 최신봉만(슬라이드 k=0). ind._knnDday=crossDday 결과(루프 사전계산), 없으면 직접 호출.
        ...
        if (_kd === undefined) { _kd = (self.SXKNN && self.SXKNN.crossDday) ? self.SXKNN.crossDday((ind._advanced && ind._advanced.rows) || ind.candles, { s:5, l:9 }) : null; }
        ...
        if (needCandles) {
          try {
            ...
            for (let k = 0; k < Nactual; k++) {
              ...
              if (self.SXKNN && self.SXKNN.crossDday && techFilters.some(f => f && f.id === 'knn_dday')) { try { indK._knnDday = self.SXKNN.crossDday(candles, { s:5, l:9 }); } catch (_eKd) { indK._knnDday = null; } }
