...
// [v3.12 SECTOR-EXCEL] KRX 엑셀 업종명 기반 매핑 테이블
//   배경: KRX API가 워커 IP에서 502 → 정적 JSON(krx_sectors.json)으로 종목별 업종 주입
//   엑셀 출처: data.krx.co.kr 종목별 시세 → "업종명" 컬럼 (KOSPI 950 + KOSDAQ 1824 = 2772종목)
//   엑셀 업종명 형식: 전기·전자, 화학, 운송장비·부품, IT 서비스 등 (· 구분자, 공백 포함)
//   매칭 로직: includes 검사 → s.sector(엑셀 원본)에 alias 키워드 포함 시 매칭
...
async function _fetchKisData(code) {
  ...
  try {
    ...
    if (ccRes?.output && Array.isArray(ccRes.output) && ccRes.output.length > 0) {
      ...
      //   기존 버그: ccRes.output을 객체처럼 다뤄 c.tday_rltv = undefined → 0 → 모든 종목 탈락
      //   수정: output[0] 사용 (가장 최근 체결)
...
// [v3.12 SECTOR-EXCEL] KRX 엑셀 기반 정적 업종 매핑 (JSON)
//   배경: KRX API의 IDX_NM이 워커 IP 502로 비어있음 → 정적 JSON으로 fallback
//   파일: /krx_sectors.json (KOSPI 950 + KOSDAQ 1824 = 2772종목, 약 65KB)
//   캐시: 24시간 (시장 변동 적음, 주 1회 갱신 권장)
let _krxSectorMap = null;
let _krxSectorTs = 0;
async function loadKrxSectors() {
  if (_krxSectorMap && Date.now() - _krxSectorTs < 24 * 3600 * 1000) return _krxSectorMap;
  try {
    const res = await fetch('./krx_sectors.json');
    if (!res.ok) return {};
    const json = await res.json();
    _krxSectorMap = json || {};
    _krxSectorTs = Date.now();
    console.log(`[SX Worker] krx_sectors.json 로드: ${Object.keys(_krxSectorMap).length}종목`);
    return _krxSectorMap;
  } catch (e) {
    console.warn('[SX Worker] krx_sectors.json 로드 실패:', e.message);
    return {};
  }
}
...
async function fetchStockMaster() {
  try {
    ...
    // [v3.12 SECTOR-EXCEL] 정적 업종 매핑 로드 (병렬)
    const sectorsPromise = loadKrxSectors();
    // [Phase 4-C] KRX market-cap은 가장 핵심 — 502 흡수 위해 재시도 (2회) 적용
    //   타임아웃은 기본 10초, 재시도 시 백오프 500ms→1s
    ...
    let slMap = {};
    try {
      // [Phase 4-C] stock-list도 재시도 적용 (1회)
      const slRes = await _fetchWithRetry(WORKER_BASE + '/krx/stock-list?market=ALL', null, 10000, 1);
      if (slRes && slRes.ok) {
    ...
    // [v3.16 잔재 정리] 공매도/액면가/자본금 fetch + 매핑 제거
    //   사유: short_balance_ratio, short_ratio, face_value, capital 필터 모두 UI에서 제거됨

    let master;
    // [v3.12 SECTOR-EXCEL] 정적 업종 매핑 await
    const sectorsMap = await sectorsPromise;
    if (oraclePool.length > 0) {
      master = oraclePool.map(s => {
        ...
        return {
          ...
          marketCap: krx.MKTCAP ? parseNum(krx.MKTCAP) / 100000000 : parseNum(s.mcap || 0) / 100000000, // [v3.12 UNIT-FIX] ORACLE s.mcap도 원→억원 환산
          foreignRatio: parseFloat(s.foreignRatio || 0),
          listedShares: parseNum(krx.LIST_SHRS || 0),
          // [v3.12 SECTOR-EXCEL] sector: KRX IDX_NM 우선 → 정적 JSON fallback
          volumeRatio: 100, sector: krx.IDX_NM || sectorsMap[s.code] || '', _krxLive: krxPrice > 0,
        };
      ...
      master = raw.map(r => ({
        ...
        // [v3.12 SECTOR-EXCEL] sector: KRX IDX_NM 우선 → 정적 JSON fallback
        volumeRatio: 100, sector: r.IDX_NM || r.sector || sectorsMap[r.ISU_SRT_CD || r.code] || '',
      })).filter(s => s.code && s.name);
    }
    _stockMasterCache = { ts: Date.now(), data: master };
    return master;
  } catch (e) {
    // fallback - KRX 완전 실패 시 ORACLE만으로 (sector 정적 매핑은 별도 lazy 로드)
    const pool = [..._oracleKospi, ..._oracleKosdaq, ..._oracleEtf];
    if (pool.length) {
      const sectorsMap = await loadKrxSectors().catch(() => ({}));
      return pool.map(s => ({ code: s.code, name: s.name, market: s.market || '', price: 0, changeRate: 0, volume: 0, tradeAmount: s.vol || 0, marketCap: (s.mcap || 0) / 100000000, foreignRatio: 0, listedShares: 0, volumeRatio: 100, sector: sectorsMap[s.code] || '' })); // [v3.12 UNIT-FIX + SECTOR-EXCEL]
    }
    return null;
  }
}
...
async function loadCoinMaster() {
  try {
    ...
    return pool.map(s => {
      ...
      // [v3.17 UNIT-FIX] 코인 marketCap 단위 자동 정규화
      //   기존: marketCap: s.mcap || 0 — ORACLE_COIN의 mcap을 단위 변환 없이 그대로 사용
      //   문제:
      //     - sx_conditions.js에서 코인 시가총액 unit:'억원'으로 정의되어 있으나,
      //       워커는 ORACLE_COIN의 s.mcap을 단위 변환 없이 그대로 marketCap에 할당함
      //     - ORACLE 데이터 소스에 따라 mcap이 "원" 단위로 들어올 수 있음 (KR scan_worker L418, L495, L547는
      //       모두 /100000000 환산을 적용하나 코인만 누락되어 있었음)
      //     - 결과: 사용자가 시가총액 1000억원 이상 필터 시 → 원단위 mcap(예: 1e14)이 그대로 비교되어
      //       오히려 모든 종목 통과 / 또는 비트코인 등 시총 큰 코인이 누락되는 일관성 없는 결과
      //   수정: sx_screener.html L5607과 동일한 정책 적용 — 1e8(=1억) 초과 시 원→억원 환산
      //     - 정상 억원 값(예: 비트코인 시총 약 7000만 억원 = 7e7)도 1e8 미만이라 안전
      //     - 원 단위 값(예: 7e15)은 환산되어 7e7로 정상 비교 가능
      //     - 시스템 표준 정책: marketCap은 "억원" 단위로 통일 (KR/US와 동일)
      let _mcap = parseFloat(s.mcap || 0);
      if (_mcap > 100000000) _mcap = _mcap / 100000000;
      return {
        ...
        marketCap: _mcap, foreignRatio: 0, listedShares: 0, volumeRatio: 100, sector: '',
      };
    }).filter(s => s.code);
  } catch (e) { return null; }
}
...
function passFilters(s, getFilter) {
  ...
  //   기존 문제: 가격 의존 필터(price_range/change_rate 등) 활성 시,
  //     KRX market-cap API에서 시가총액은 받았지만 가격은 0인 종목들을 통째로 reject
  //     → "현재가 100만원~" 만 설정해도 모든 종목 reject → 0건
  //   수정: marketCap > 0이면 종목 풀에 정상 등록된 종목으로 간주하고 통과시킴.
  //     가격 의존 검사는 각 필터 항목에서 자체적으로 수행 (price === 0이면 자연히 min 미달로 reject).
  ...
  if (excl && excl.value) {
    // [통합정리] 종목 유형 제외 — 5종 (이름·시장 패턴으로 즉시 처리)
    //   기존에 정의만 되고 처리 코드가 없던 9개 옵션은 공시 기반 제외로 통합 이전됨
    ...
    if (excl.value.includes('etn') && (s.name.includes('ETN') || s.sector === 'ETN' || s.market === 'ETN')) return _reject('exclude_types.etn');
    if (excl.value.includes('spac') && s.name.includes('스팩')) return _reject('exclude_types.spac');
    if (excl.value.includes('reits') && (s.name.includes('리츠') || s.name.includes('REITs') || s.sector === '리츠' || s.sector === 'REITs')) return _reject('exclude_types.reits');
  }
  ...
  for (const f of techFilters) {
    ...
    switch (f.id) {
      case 'ma_cross': {
        ...
        //   기존 v3.10-fix(옵션 A): "직전 봉 ≤ → 현재 봉 >" 단 한 봉의 교차 순간만 통과
        //     → 외부 슬라이딩 루프와 결합돼야 N봉 검사가 되는데 너무 빡빡 (N=5일 때 5봉 중 정확히 그 한 봉)
        //   신규(옵션 B): "N봉 시작점에선 ≤, 현재 봉에선 >"
        //     → N봉 사이 어딘가에서 교차했음을 보장 (1봉전이든 4봉전이든 OK)
        //     → 사용자 직관: "최근 N봉 안에 골든크로스 발생" 그대로 동작
        //     → 정배열 N봉 이상 유지 종목은 시작점에도 이미 > 라 차단됨 (희창님 우려 해결)
        //   N봉 추출: _recent_n_bars 메타조건의 min 값. 미설정 시 N=2 (직전봉→현재봉, 옛 로직과 가장 가까움)
...
async function fetchFinancialData(code, market, marketCap) {
  ...
  try {
    if (market === 'us' || currentMarket === 'us') {
      ...
      if (resp.ok) {
        // [v3.17 US-FIN-FIX] Yahoo /fundamental 응답 구조 정합성 수정
        //   기존 문제: const d = await resp.json(); d.trailingPE로 직접 접근
        //     → 백엔드(workers_v9.js handleFundamental, L1218) 응답 구조는 { ticker, data: {...}, source, ts }
        //     → d.trailingPE는 항상 undefined → result.per, pbr, eps, dividendYield 모두 null
        //     → 사용자가 PER/PBR/배당수익률 필터 적용 시 rangeCheck(null,...)=false → 모든 종목 탈락 버그
        //   수정:
        //     1) const d = json.data 추출 (백엔드 실제 데이터 위치)
        //     2) dividendRaw 파싱 → 백엔드(L1214)에서 이미 dividendYield로 가공 + dividendRaw 삭제됨
        //        → d.dividendRaw 검사는 무의미 → d.dividendYield 직접 사용 (백엔드는 string으로 줌)
        //     3) 폴백 호환성: 일부 캐시된 구버전 응답에 dividendRaw가 남아있을 가능성 대비 폴백 로직 유지
        const json = await resp.json();
        const d = (json && json.data) ? json.data : json; // 폴백: data 없으면 평면 응답 가정
        ...
        // dividendYield: 백엔드에서 이미 가공된 형태로 옴 (string "0.52" 같은). dividendRaw는 백엔드에서 delete됨
        if (d.dividendYield != null) {
          const dy = parseFloat(String(d.dividendYield).replace(/[%,]/g, ''));
          if (!isNaN(dy)) result.dividendYield = dy;
        } else if (d.dividendRaw) {
          // 폴백: 구버전 응답 호환 (백엔드 캐시 잔존물)
          const ym = String(d.dividendRaw).match(/([\d.]+)%/); if (ym) result.dividendYield = parseFloat(ym[1]);
        }
        if (d.fiftyTwoWeekRange) { const parts = String(d.fiftyTwoWeekRange).replace(/,/g, '').split('-').map(s => parseFloat(s.trim())); if (parts.length === 2) { result.week52Low = parts[0]; result.week52High = parts[1]; } }
...
//   ⚠️ sx_render.js calcBtScore 변경 시 반드시 여기도 함께 수정!
...
//   sx_render.js와 동기 — 거래수 부족한 PF의 자동 감점 (거래 5회 PF 99 → 50% 감점 등)
function calcBtScore(btData){
  ...
  // [v3.20 PF-TRUST] PF 점수 × 거래수 신뢰도 (옵션 A) — N_적정 20 (일봉 기준)
...
// S103-fix8: 진입일/청산일이 "오늘 KST"인지 판정 (sx_bt.js의 _btIsToday와 동일 로직)
//   Worker는 sx_bt.js를 import 못 하므로 동일 함수 복제. 원본 변경 시 동기화 필요.
...
function _calcTransitionStats(scores, trades, scores3) {
  ...
  for (var si = 1; si < scores.length; si++) {
    ...
    if (zone !== prevZone) {
      ...
      // [BUGFIX] dead code 제거 — `prevZone === 'ready'` 케이스는 위에서 ready→entry, ready→trend가 모두 처리됨
      //   zone이 3개뿐(ready/entry/trend)이고 zone 변경 시(라인 1316) 진입하므로 ready→ready는 불가능 → 도달 불가 분기 제거
      zoneStart = si;
      prevZone = zone;
    }
  }
...
async function filterByDisclosure(results, gradesToReject, progressCb) {
  // [통합정리] 단순 CRITICAL+SEVERE 고정 → gradesToReject 배열로 동적 처리
  //   gradesToReject: ['CRITICAL'] | ['CRITICAL','SEVERE'] | ['CRITICAL','SEVERE','WARNING']
  //   호재(POSITIVE)는 어떤 경우에도 제외하지 않음
  const grades = (gradesToReject && gradesToReject.length) ? gradesToReject : ['CRITICAL', 'SEVERE'];
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    try {
      ...
      s._disclosureKw = kws;
      // POSITIVE는 절대 제외 대상 아님 — 명시적 가드
      if (kws.some(k => k.grade !== 'POSITIVE' && gradeSet.has(k.grade))) continue;
      filtered.push(s);
...
function _slimResults(arr) {
  return arr.map(s => ({
    ...
    //   기존 누락 시: 메인 스레드에서 모멘텀 보정 발생 여부 판별 불가 (label 텍스트 파싱은 깨지기 쉬움)
...
async function startScan(config) {
  ...
  try {
    ...
    for (let i = 0; i < total;) {
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if(_isBtSupportedTF(currentMarket, currentTF))
        {
          ...
          // [v2.3] 종합행동지침 필터: 9종 verdictAction 직접 매칭으로 변경
          //   (이전: s._btAction 4종 매핑값과 비교 → 9종 선택지와 불일치)
    ...
    if (!_scanAbort && currentMarket === 'kr') {
      ...
      // [통합정리] toggle(boolean) → multi_check(array) 전환
      //   레거시 호환: discFilter.value === true (구버전 토글) → 기본 등급 [CRITICAL, SEVERE]로 매핑
      //                discFilter.value가 배열 → 선택된 등급(소문자) → 대문자 등급명으로 변환
      let _gradesToReject = null;
      if (discFilter && discFilter.value) {
        if (discFilter.value === true) {
          // 레거시: 단순 토글 ON → 기본값
          _gradesToReject = ['CRITICAL', 'SEVERE'];
        } else if (Array.isArray(discFilter.value) && discFilter.value.length > 0) {
          // 신버전: ['critical','severe','warning'] → ['CRITICAL','SEVERE','WARNING']
          _gradesToReject = discFilter.value.map(g => String(g).toUpperCase()).filter(g => ['CRITICAL','SEVERE','WARNING'].includes(g));
        }
      }
      if (_gradesToReject && _gradesToReject.length > 0) {
        self.postMessage({ type: 'progress', current: total, total, name: `공시 검증 중... (${searchResults.length}종목)` });
        searchResults = await filterByDisclosure(searchResults, _gradesToReject, (cur, tot) => {
