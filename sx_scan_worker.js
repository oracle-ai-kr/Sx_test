  ...
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'candle_transition': {   // [S495] 캔들 전이 점수 조건 — 양봉 전이 유망(score≥30) / 음봉 전이 유망(score≤−30)
        ...
        const _ts = _candleTransitionScore(_rowsCT, ind);
    ...
    for (let i = 0; i < total;) {
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if(_isBtSupportedTF(currentMarket, currentTF))
        {
          ...
          // [S474] 매수마커 ▲ 필터 — C 통합판정(s._svVerdict.chartMarker==='buy') 기준 = 차트 보라 ▲와 정확히 동일.
          //   〔S455 버그 수정〕 이전: s._action(A 안전필터 강등 액션) → 차트(C chartMarker)와 엇갈림.
          //     · C는 안전필터 위반 시 '등급 캡'만 하고 chartMarker='buy' 유지 → 차트 보라 ▲ 표시 (이미지: 🔒감지+보라▲).
          //     · A는 같은 필터로 action='HOLD' 강등 → s._action!=='BUY' → 매수마커 필터가 차트와 불일치했음.
          //   〔S474 동작〕 안전필터 매도전환·청산(관망/청산/회피)으로 chartMarker가 null/'sell'이면 자동 제외. 보유중 buy도 동일 인정(project_c 404).
          //   svVerdict 미산출(캔들<60·SXC 미로드) 종목은 차트도 보라마커 안 그림 → 동일하게 제외.
