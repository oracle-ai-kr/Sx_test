...
function checkTechConditions(ind, techFilters, getFilter) {
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'trend_signal': {   // [S560] 단기추세 N1×N2 매수(골든)/매도(데드) — 기간은 trend_ma 메타(단기=min/장기=max, 기본 5×9), _recent_n_bars 윈도우
        if (!Array.isArray(v) || v.length === 0) break;
        const closes = ind.closes;
        if (!Array.isArray(closes)) return false;
        let _sp = 5, _lp = 9;   // 기본 5×9
        const _maF = getFilter && getFilter('trend_ma');
        if (_maF && _maF.value && typeof _maF.value === 'object') {
          const _mn = parseInt(_maF.value.min), _mx = parseInt(_maF.value.max);
          if (Number.isFinite(_mn) && _mn >= 1) _sp = _mn;
          if (Number.isFinite(_mx) && _mx >= 2) _lp = _mx;
        }
        if (_sp >= _lp) return false;   // 단기 < 장기 강제
        if (closes.length < _lp + 1) return false;
        const N = _gcN(getFilter);
        const sLen = closes.length - (N - 1);
        if (sLen < _lp) return false;
        const eFast = sma(closes, _sp), eSlow = sma(closes, _lp);
        const startSlice = closes.slice(0, sLen);
        const sFast = sma(startSlice, _sp), sSlow = sma(startSlice, _lp);
        if (!Number.isFinite(sFast) || !Number.isFinite(sSlow) || !Number.isFinite(eFast) || !Number.isFinite(eSlow)) return false;
        const _gcSig = (sFast <= sSlow && eFast > eSlow);   // 골든크로스 = 매수 신호
        const _dcSig = (sFast >= sSlow && eFast < eSlow);   // 데드크로스 = 매도 신호
        const _wantBuy = v.includes('trend_buy'), _wantSell = v.includes('trend_sell');
        if (_wantBuy && _wantSell) { if (!_gcSig && !_dcSig) return false; }
        else if (_wantBuy) { if (!_gcSig) return false; }
        else if (_wantSell) { if (!_dcSig) return false; }
        break;
      }
      case 'trend_ma': break;   // [S560] 메타조건 — trend_signal이 getFilter로 기간을 읽음. 단독 필터링 안 함.
      case 'gc_ema_20_200': {
        if (v !== '골든크로스' && v !== '데드크로스') break;
        const closes = ind.closes;
