...
function checkTechConditions(ind, techFilters, getFilter) {
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'rs_value': if (!rangeCheck(ind.rs ? ind.rs.rs20 : null, v)) return false; break;
    ...
    for (let i = 0; i < total;) {
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if (indicators && indicators._advanced) {
          ...
          // [S426] C 캡 전용 위반 전수 수집 — 메인 scrQuickScore 미러. SXE._safetyFlags(_sfW) 참조로 분석탭과 캡 판정 일관.
