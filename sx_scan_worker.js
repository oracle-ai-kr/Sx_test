...
async function fetchStockMaster() {
  try {
    ...
    //   〔이력〕 stock-list API에서 PARVAL(액면가)·LIST_SHRS(상장주식수) 빌드하던 slMap도 함께 제거 (수정됨)
...
// ── 주의: 원본(sx_bt.js의 btGetCurrentState 함수) 업데이트 시 여기도 동기화 필요 (text/color 등 UI 필드는 불필요하므로 제외) ──
