...
async function fetchStockMaster() {
  try {
    ...
    // [v3.12 PATCH-1C] 공매도 거래 비중(short_ratio)도 추가 매핑
    //   기존: balance(잔고)만 사용 → short_ratio 조건이 무시되고 통과
    //   수정: 백엔드 응답에 이미 trade(거래)도 포함되어 있으므로 같이 파싱
    //   KRX 필드명 후보:
    //     - 거래비중(TRD_VOL_RTO/SHORT_VOL_RTO/거래량비중): MDCSTAT30101 응답
    //     - 잔고비중(BAL_RTO/SHORT_BAL_RTO): MDCSTAT30501 응답
    //   shortMap[code] = { bal: 잔고비중, trd: 거래비중 } 객체로 확장
    let shortMap = {};
    try {
      const [stkRes, ksqRes] = await Promise.all([
        _fetchWithTimeout(WORKER_BASE + '/krx/short-selling?market=STK').catch(() => null),
        _fetchWithTimeout(WORKER_BASE + '/krx/short-selling?market=KSQ').catch(() => null)
      ]);
      const parseShort = async (res) => {
        ...
        balArr.forEach(r => {
          ...
          if (code && ratio > 0) {
            if (!shortMap[code]) shortMap[code] = { bal: 0, trd: 0 };
            shortMap[code].bal = ratio;
          }
        });
        // [PATCH-1C] 거래비중 — 여러 KRX 필드명 후보 시도 (시기별 변동 방어)
        const trdArr = Array.isArray(d.trade) ? d.trade : (d.trade?.OutBlock_1 || []);
        trdArr.forEach(r => {
          const code = r.ISU_SRT_CD || '';
          // 우선순위: 거래량 비중 → 거래대금 비중 (둘 중 어느 게 들어와도 매칭)
          const ratio = parseFloat(
            r.TRD_VOL_RTO || r.SHORT_VOL_RTO || r.SBSL_VOLU_RTO ||
            r.TRD_VAL_RTO || r.SHORT_VAL_RTO || r.SBSL_TRDVAL_RTO || 0
          );
          if (code && ratio > 0) {
            if (!shortMap[code]) shortMap[code] = { bal: 0, trd: 0 };
            shortMap[code].trd = ratio;
          }
        });
      };
    ...
    if (oraclePool.length > 0) {
      master = oraclePool.map(s => {
        ...
        return {
          ...
          // [v3.12 PATCH-1C] shortMap이 객체로 변경됨 ({bal, trd}) — 잔고/거래 비중 분리
          shortBalanceRatio: (shortMap[s.code]?.bal) || 0,
          shortRatio: (shortMap[s.code]?.trd) || 0,
      ...
      master = raw.map(r => ({
        ...
        // [v3.12 PATCH-1C] shortMap 객체 구조
        shortBalanceRatio: (shortMap[r.ISU_SRT_CD || r.code]?.bal) || 0,
        shortRatio: (shortMap[r.ISU_SRT_CD || r.code]?.trd) || 0,
        // [v3.12 SECTOR-EXCEL] sector: KRX IDX_NM 우선 → 정적 JSON fallback
...
function passFilters(s, getFilter) {
  ...
  // [v3.13 ENV-FIX] _marketEnvData.state[market] 같은 구조는 *존재하지 않음* — 항상 'neutral' 반환되던 버그
  // [v3.15 ENV-FIX2] _marketEnvData가 raw (kospi.cr만 있고 score 없음) — worker에서 직접 classify 필요
  //   원인: marketEnvData = JSON.parse(SX_MARKET_INDEX)는 raw {kospi:{cr,close}, kosdaq:{cr,close}}
  //         MarketEnv.load()는 동적으로 score 계산하지만 worker에 전달되지 않음
  //   증상 (v3.13 후에도): kospi.score, kosdaq.score = undefined → 폴백 → 매칭 실패
  //   수정: worker에서 cr → classify → score 직접 계산 (MarketEnv.load의 classify와 동일 로직)
  ...
  if (envState && envState.value && envState.value !== '설정안함') {
    if (_marketEnvData) {
      ...
      // raw cr → score classify (sx_analysis_engine.js L1777과 동일)
      const _classify = (cr) => {
      ...
      if (currentMarket === 'coin') {
        ...
        const crs = [
          env.sp500?.cr ?? env.sp500?.changeRate,
          env.nasdaq?.cr ?? env.nasdaq?.changeRate,
          env.dow?.cr ?? env.dow?.changeRate
        ].filter(v => v != null);
        if (crs.length) {
        ...
        // 한국
        const crs = [
          env.kospi?.cr ?? env.kospi?.changeRate,
          env.kosdaq?.cr ?? env.kosdaq?.changeRate
        ].filter(v => v != null);
        if (crs.length) {
          const scores = crs.map(cr => _classify(cr).score);
          const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
  ...
  if (ls && ls.value) { if (ls.value.min !== null && (s.listedShares || 0) < ls.value.min) return _reject('listed_shares.min'); if (ls.value.max !== null && (s.listedShares || 0) > ls.value.max) return _reject('listed_shares.max'); }
  const vt = getFilter('volume_turnover');
  if (vt && vt.value) { const turnover = (s.listedShares > 0) ? (s.volume / s.listedShares) * 100 : 0; if (vt.value.min !== null && turnover < vt.value.min) return _reject('volume_turnover.min'); if (vt.value.max !== null && turnover > vt.value.max) return _reject('volume_turnover.max'); }
  const fv = getFilter('face_value');
  if (fv && fv.value) { if (fv.value.min !== null && (s.faceValue || 0) < fv.value.min) return _reject('face_value.min'); if (fv.value.max !== null && (s.faceValue || 0) > fv.value.max) return _reject('face_value.max'); }
  const cap = getFilter('capital');
  if (cap && cap.value) { if (cap.value.min !== null && (s.capital || 0) < cap.value.min) return _reject('capital.min'); if (cap.value.max !== null && (s.capital || 0) > cap.value.max) return _reject('capital.max'); }
  const fe = getFilter('foreign_exhaustion');
  if (fe && fe.value) { const exh = s._foreignExhaustion || 0; if (fe.value.min !== null && exh < fe.value.min) return _reject('foreign_exhaustion.min'); if (fe.value.max !== null && exh > fe.value.max) return _reject('foreign_exhaustion.max'); }
  const ocr = getFilter('open_change_rate');
  if (ocr && ocr.value) { const rate = s._openChangeRate || 0; if (ocr.value.min !== null && rate < ocr.value.min) return _reject('open_change_rate.min'); if (ocr.value.max !== null && rate > ocr.value.max) return _reject('open_change_rate.max'); }
  const sbr = getFilter('short_balance_ratio');
  if (sbr && sbr.value) { if (sbr.value.min !== null && (s.shortBalanceRatio || 0) < sbr.value.min) return _reject('short_balance_ratio.min'); if (sbr.value.max !== null && (s.shortBalanceRatio || 0) > sbr.value.max) return _reject('short_balance_ratio.max'); }
  // [v3.12 PATCH-1C] short_ratio (공매도 거래 비중) — 기존엔 매핑 누락으로 무시되고 통과되던 조건
  const srt = getFilter('short_ratio');
  if (srt && srt.value) { if (srt.value.min !== null && (s.shortRatio || 0) < srt.value.min) return _reject('short_ratio.min'); if (srt.value.max !== null && (s.shortRatio || 0) > srt.value.max) return _reject('short_ratio.max'); }
  return true;
}
