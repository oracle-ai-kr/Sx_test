...
async function fetchStockMaster() {
  try {
    ...
    const res = await _fetchWithTimeout(WORKER_BASE + '/krx/market-cap');
    ...
    let slMap = {};
    try {
      const slRes = await _fetchWithTimeout(WORKER_BASE + '/krx/stock-list?market=ALL');
      if (slRes.ok) {
...
function passFilters(s, getFilter) {
  ...
  if (hasAnyPriceFilter && s.price === 0 && s.tradeAmount === 0 && s.volume === 0 && s.changeRate === 0 && s.marketCap === 0) return _reject('no_price_data');
  ...
  if (sect && sect.value && sect.value.length) { if (!sect.value.some(v => s.sector?.includes(v) || s.name?.includes(v))) return _reject('sector'); }
...
async function _fetchDartFinancial(code, result, marketCap) {
  ...
  // [Phase 3] EBITDA 정확 계산 — 영업이익 + 감가상각비 + 무형자산상각비 (현금흐름표 기준)
  //   현금흐름표는 손익계산서 영업이익에 비현금 비용을 다시 더해주는 구조 → 표준 산출법
  //   양수만 인정: 일부 기업은 현금흐름표에 음수로 표기되는 케이스도 있어 부호 정상화
  if (result.operatingIncome != null) {
    ...
    if (dep > 0 || amr > 0) {
      ...
      // 현금흐름표 미발견 시 보수적 폴백: 영업이익 = EBITDA 근사 (신뢰도 마커 _ebitdaApprox)
...
async function _fetchDartQuarterly(code, result) {
  ...
  for (const yr of yearCandidates) {
    const [r1, r2, r3] = await Promise.allSettled([
      fetch(`${WORKER_BASE}/dart/finance?stock_code=${code}&year=${yr}&report=q1`, { signal: AbortSignal.timeout(10000) }).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`${WORKER_BASE}/dart/finance?stock_code=${code}&year=${yr}&report=half`, { signal: AbortSignal.timeout(10000) }).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`${WORKER_BASE}/dart/finance?stock_code=${code}&year=${yr}&report=q3`, { signal: AbortSignal.timeout(10000) }).then(r => r.ok ? r.json() : null).catch(() => null),
    ]);
  ...
  if (result._reportYear && cum3 && result._reportYear === qYear &&
    ...
    try {
      ...
      const resp = await fetch(`${WORKER_BASE}/dart/finance?stock_code=${code}&year=${prevYr}&report=q3`, { signal: AbortSignal.timeout(10000) });
      if (resp.ok) {
...
async function startScan(config) {
  ...
  // S125: 워커 환경(localStorage 불가)에서 엔진이 매트릭스를 참조할 수 있도록 주입
  //   메인 스레드가 scanPayload에 currentAnalMode, analParamsMatrix를 담아 전달
  //   → 엔진의 _getCurrentMode/_loadParamsMatrix가 이 값을 읽도록 SXE에 저장
  if (typeof SXE !== 'undefined') {
    SXE._workerCurrentMode = config.currentAnalMode || 'balanced';
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if (needCandles) {
          try {
            ...
            if (!candles || candles.length < 20) { _candleFailCount++; continue; }
            indicators = calcIndicators(candles, currentTF);
            if (techFilters.length > 0 && !checkTechConditions(indicators, techFilters, getFilter)) {
              _techFilterStats['tech_all'] = (_techFilterStats['tech_all'] || 0) + 1;
              continue;
            }
