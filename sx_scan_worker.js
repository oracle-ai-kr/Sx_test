...
function checkTechConditions(ind, techFilters, getFilter) {
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'candle_transition': {   // [S495→S622→S647] 캔들 전이 — kNN 우선(라이브 카드 미러), 비활성이면 룰 폴백. 조건검색 임계=±35(라이브 카드는 ±30)
        if (!Array.isArray(v) || v.length === 0) break;
        ...
        //   룰(_candleTransitionScore)은 검증 없는 휴리스틱이라 봉수 부족 종목이 룰로 ±35 넘어 걸리던 오검출 차단(정밀도↑).
        //   정상 fetch(_scCount 200/500봉) 종목은 kNN active라 영향 없음 — 데이터 부실 종목만 빠짐. 라이브 카드는 룰을 '룰 해설(보조)'로 유지(역할 분리).
        if (_ctScore === null) return false;
        const _wantBull = v.includes('trans_bull'), _wantBear = v.includes('trans_bear');
        // [S647] 조건검색 임계 ±35 (라이브 카드 ±30보다 보수적). 워커 kNN 봉수(_scCount 200/500)가 라이브(600)보다 작아
        //   ±30 경계(28↔32) 종목이 봉수차로 흔들림 → 스크리닝은 강한 신호(|점수|≥35)만 거름. 걸린 종목은 라이브에서도 거의 확실히 유망(정밀도↑).
        const _isBull = _ctScore >= 35, _isBear = _ctScore <= -35;
