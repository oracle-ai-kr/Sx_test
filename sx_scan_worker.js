...
function checkTechConditions(ind, techFilters, getFilter) {
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'candle_transition': {   // [S495→S621] 캔들 전이 — kNN 우선(라이브 카드 헤드라인 미러), 비활성이면 룰 폴백. 양봉≥30 / 음봉≤−30
        if (!Array.isArray(v) || v.length === 0) break;
        const _rowsCT = (ind._advanced && ind._advanced.rows) || ind.candles;
        if (!_rowsCT || _rowsCT.length < 25) return false;
        let _ctScore = null;
        // [S621] kNN 우선 — 라이브 카드와 동일 호출(win16/k10/게이팅off). active일 때만 채택.
        try {
          if (self.SXKNN && self.SXKNN.score) {
            const _kn = self.SXKNN.score(_rowsCT, { win: 16, k: 10 });
            if (_kn && _kn.active && typeof _kn.score === 'number') _ctScore = _kn.score;
          }
        } catch (_eKnn) { _ctScore = null; }
        // kNN 비활성(이력부족 등) → 룰 폴백(기존 _candleTransitionScore)
        if (_ctScore === null) {
