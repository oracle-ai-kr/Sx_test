    ...
    for (let i = 0; i < total;) {
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if(_isBtSupportedTF(currentMarket, currentTF))
        {
          ...
          // [S293] 매수 배지 필터 — 4축 판정 결과 action==='매수' (결과탭 🟢매수 배지와 동일)
          const _engineBuyF = getFilter('_engine_buy');
          if (_engineBuyF && _engineBuyF.value && _engineBuyF.value !== '설정안함') {
            if (!s._svVerdict || s._svVerdict.action !== '매수') continue;
          }
          // [S359] C 매수마커(▲) 필터 — svVerdict.chartMarker==='buy' (신규 매수 + 보유중 추가진입 모두 포함)
          //   _engine_buy(action='매수')와 달리 보유중 추가진입(보유 유지+추가매수 고려)도 잡힘.
          ...
          // [S359] C 매도마커(▼) 필터 — svVerdict.chartMarker==='sell' (청산/매도완료 + 보유중 조기청산·부분익절)
