...
function _calcTransitionStats(scores, trades) {
  if (!scores || scores.length < 20) return null;
  // 구간: Ready=score<50, Entry=50~64, Trend=65+
  var r2eAttempts = 0, r2eSuccess = 0;
  var e2tAttempts = 0, e2tSuccess = 0;
  var r2eBars = [], e2tBars = []; // 전이에 걸린 봉 수
  var prevZone = scores[0] < 50 ? 'ready' : scores[0] < 65 ? 'entry' : 'trend';
  var zoneStart = 0;
  for (var si = 1; si < scores.length; si++) {
    var sc = scores[si];
    var zone = sc < 50 ? 'ready' : sc < 65 ? 'entry' : 'trend';
    if (zone !== prevZone) {
...
async function startScan(config) {
  ...
  try {
            ...
            if (rawRows.length >= 60 && typeof SXE !== 'undefined' && SXE.runBtEngine) {
              ...
              if (btResult && !btResult.error) {
                ...
                if (btResult.scores && btResult.scores.length >= 20) {
                  s._btTransitionStats = _calcTransitionStats(btResult.scores, btResult.trades);
                }
                const es = qs.score, bs = s._btScore;
                if (es != null && bs != null) {
                  if (es >= 60 && bs >= 60) s._btAction = '진입 적기';
                  else if (es >= 60 && bs < 60) s._btAction = '단기급등 주의';
                  else if (es < 60 && bs >= 60) s._btAction = '관심 등록';
                  else s._btAction = '회피';
                } else s._btAction = null;
              }
            }
          } catch (_) {}
        }
    ...
    // 완료
    const PARALLEL_LABEL = PARALLEL > 1 ? ' ⚡×' + PARALLEL : '';
    self.postMessage({
      type: 'done',
