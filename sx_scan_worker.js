    ...
    for (let i = 0; i < total;) {
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if (indicators && indicators._advanced) {
          ...
          const reasons = [];
          const _wasBuyW = (action === 'BUY'); // [S361] 안전필터 진입 시점 BUY 여부 (위반 전수 수집 게이트)
          ...
          // [S361] C 캡 전용 위반 전수 수집 — 메인 scrQuickScore 미러. SXE._safetyFlags(_sfW) 참조로 분석탭과 캡 판정 일관.
          //   워커 action/reasons는 _safety_clean 카운트 보존 위해 미변경. 캡 입력은 이 배열로만 전달.
          const _sfW = (typeof SXE !== 'undefined' && SXE._safetyFlags && Object.keys(SXE._safetyFlags).length)
            ? SXE._safetyFlags
            : {threshold:true,volExtreme:true,volHigh:true,rsiDiv:true,stochRsi:true,macdNeg:true,ma60resist:true,bbUpper:true,resistNear:true,debtRatio:true,foreignSell:true,highBeta:true};
          const _safetyViol = [];
          if (_wasBuyW) {
            ...
            if (_sfW.debtRatio && adv._debtRatio != null && adv._debtRatio >= 200) _safetyViol.push('🔒부채비율');
            if (_sfW.foreignSell && adv._foreignConsecSell != null && adv._foreignConsecSell >= 3) _safetyViol.push('🔒외국인매도');
            if (_sfW.highBeta && adv._beta != null && adv._beta > 1.5) _safetyViol.push('🔒고베타');
          ...
          qs._safetyViol = _safetyViol; // [S361] C 캡 전용 안전필터 위반 목록
        ...
        if(_isBtSupportedTF(currentMarket, currentTF))
        {
          ...
          // [S361] 매수마커 ▲ 필터 — svVerdict.chartMarker==='buy'
          //   (구 '매수 배지' action==='매수'를 포괄: 신규 매수 + 보유중 추가진입 모두)
          ...
          // [S361] 매도마커 ▼ 필터 — svVerdict.chartMarker==='sell' (청산/매도완료 + 보유중 조기청산·부분익절)
          const _cSellMarkerF = getFilter('_c_sell_marker');
          if (_cSellMarkerF && _cSellMarkerF.value && _cSellMarkerF.value !== '설정안함') {
            if (!s._svVerdict || s._svVerdict.chartMarker !== 'sell') continue;
          }
