...
function checkTechConditions(ind, techFilters, getFilter) {
  for (const f of techFilters) {
    ...
    switch (f.id) {
      case 'ma_cross': {
        // [v3.10-fix] 정배열 상태가 아닌 "실제 교차 발생" 검사로 교체.
        //   기존: ma5 > ma20이기만 하면 골든크로스로 인정 → 1년 전 교차한 정배열 유지 종목도 모두 통과
        //   수정: prev봉(직전봉) → cur봉(현재봉) 교차 순간만 통과
        //   N봉 윈도우와 결합되면 (외부 슬라이딩 루프에서) "최근 N봉 중 어느 한 봉이 교차 순간"으로 평가됨
        //   ind.closes 사용: calcIndicators가 caller에서 candles.slice 후 호출되므로
        //     ind.closes의 마지막 값이 그 시점 봉의 종가 = "이 시점이 현재봉이라 가정한 평가"
        const m = v.match(/([골데])[든드]크로스\s*\((\d+)×(\d+)\)/);
        if (!m) break;
        const fast = parseInt(m[2]), slow = parseInt(m[3]);
        const cls = ind.closes;
        if (!cls || cls.length < slow + 1) return false; // prev 봉을 위해 +1 필요
        // 현재 시점 (마지막 봉)
        const curFast = sma(cls, fast);
        const curSlow = sma(cls, slow);
        // 직전 시점 (마지막 봉 1개 제외)
        const prevFast = sma(cls.slice(0, -1), fast);
        const prevSlow = sma(cls.slice(0, -1), slow);
        if (curFast == null || curSlow == null || prevFast == null || prevSlow == null) return false;
        if (m[1] === '골') {
          // 골든크로스: 직전엔 ≤, 지금은 > 이어야 진짜 교차
          if (!(prevFast <= prevSlow && curFast > curSlow)) return false;
        } else {
          // 데드크로스: 직전엔 ≥, 지금은 < 이어야 진짜 교차
          if (!(prevFast >= prevSlow && curFast < curSlow)) return false;
        }
        break;
      }
...
async function startScan(config) {
  ...
  try {
    ...
    if (_recentNFilter && _recentNFilter.value) {
      ...
      // range 타입: {min, max} — min을 윈도우 크기로 사용 (사용자가 5 입력 → min=5, max=5 들어옴)
      const n = (typeof v === 'object') ? (v.min ?? v.max) : v;
        ...
        if (needCandles) {
          try {
            ...
            for (let k = 0; k < Nactual; k++) {
              ...
              if (techFilters.length === 0 || checkTechConditions(indK, techFilters, getFilter)) {
                indicators = indK;
                passed = true;
                passedK = k;
                break;
