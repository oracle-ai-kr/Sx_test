  ...
  for (const f of techFilters) {
    ...
    switch (f.id) {
      case 'ma_cross': {
        ...
        const p = pairs[v]; if (!p || p[0] === null || p[1] === null) return false;
    ...
    for (let i = 0; i < total;) {
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if (newFound === 1 || newFound % 5 === 0) {
          self.postMessage({ type: 'result_batch', results: searchResults, newFound, alert: true });
        }
      }
      // 배치 간 yield
