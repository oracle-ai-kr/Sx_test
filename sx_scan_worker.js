        ...
        if (needCandles) {
          try {
            ...
            for (let k = 0; k < Nactual; k++) {
              ...
              // [S652] AND그룹: 전부 충족해야 통과 / OR그룹: 하나라도 충족하면 통과(그룹이 비어있으면 통과로 간주)
              //   조건별 격리 검사 — 어떤 조건이 첫 탈락인지 알기 위해 (AND 우선 진단, OR 전체불충족은 별도 라벨)
              let allPass = true;
              ...
              if (allPass && orTechFilters.length) {
                let _orHit = false;
                for (const _tf of orTechFilters) {
                  if (checkTechConditions(indK, [_tf], getFilter)) { _orHit = true; break; }
                }
                if (!_orHit) { allPass = false; _failedHere = '_or_group'; }
              }
              if (allPass) {
                indicators = indK; passed = true; passedK = k; break;
              }
