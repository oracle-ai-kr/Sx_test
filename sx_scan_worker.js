...
function passFilters(s, getFilter) {
  ...
  //   원인: MarketEnv.getState()는 score 평균으로 동적 계산하는 함수인데 워커가 정적 객체 접근
  //   증상: 약세장에서 "약세 포함" 선택 시 모든 종목 탈락 (3848/3848)
  //   수정: MarketEnv.getState 로직을 워커에 직접 구현 (kospi+kosdaq score 평균)
  ...
  if (envState && envState.value && envState.value !== '설정안함') {
    if (_marketEnvData) {
      ...
      // 시장별 현재 상태 동적 계산 (MarketEnv.getState와 동일 로직)
      let curState = 'neutral';
      const env = _marketEnvData;
      if (currentMarket === 'coin') {
        curState = env.btc ? env.btc.dir : (env.overall?.state || 'neutral');
      } else if (currentMarket === 'us') {
        ...
        } else curState = env.overall?.state || 'neutral';
      } else {
        // 한국
