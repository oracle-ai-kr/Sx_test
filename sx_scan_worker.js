...
// [v3.12 PATCH-1B] KRX 업종 alias 테이블
//   배경: 사용자가 UI에서 "자동차" 선택해도 KRX의 IDX_NM은 "운수장비"로 들어와 매칭 실패
//   매핑: UI 옵션 → KRX IDX_NM 후보 키워드 (한 옵션이 여러 KRX 업종에 매칭 가능)
//   매칭 로직: s.sector가 후보 키워드 중 하나라도 includes하면 매칭 (s.name fallback도 유지)
//   주의: KRX 업종명은 시기별로 미세 변동 있어, "포함 여부"로 느슨하게 매칭
const KRX_SECTOR_ALIASES = {
  '반도체':       ['전기전자', '반도체'],
  '2차전지':      ['전기전자', '화학', '배터리', '2차전지'],
  '바이오':       ['의약품', '바이오', '제약'],
  '자동차':       ['운수장비', '자동차'],
  'IT/소프트웨어': ['서비스업', '소프트웨어', 'IT', '디지털컨텐츠', '인터넷'],
  '금융':         ['금융업', '은행', '증권', '보험'],
  '건설':         ['건설업', '건설'],
  '화학':         ['화학', '석유'],
  '철강':         ['철강금속', '철강'],
...
async function fetchStockMaster() {
  try {
    ...
    if (pool.length) return pool.map(s => ({ code: s.code, name: s.name, market: s.market || '', price: 0, changeRate: 0, volume: 0, tradeAmount: s.vol || 0, marketCap: (s.mcap || 0) / 100000000, foreignRatio: 0, listedShares: 0, volumeRatio: 100, sector: '' })); // [v3.12 UNIT-FIX]
    return null;
  }
}
