...
// [S363] EMA 마지막값 (SMA 시드) — gc_ema_20_200용. period봉 미만이면 NaN.
//   ※ sx_screener.html checkTechConditions의 _emaLast와 동일 미러 (수정 시 양쪽 동기화)
function _emaLast(arr, period) {
  if (!Array.isArray(arr) || arr.length < period) return NaN;
  const k = 2 / (period + 1);
  let sum = 0;
  ...
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'gc_ema_20_200': {
        ...
        const eFast = _emaLast(closes, 20), eSlow = _emaLast(closes, 200);
        const startSlice = closes.slice(0, sLen);
        const sFast = _emaLast(startSlice, 20), sSlow = _emaLast(startSlice, 200);
        if (!Number.isFinite(sFast) || !Number.isFinite(sSlow) || !Number.isFinite(eFast) || !Number.isFinite(eSlow)) return false;
        if (v === '골든크로스' && !(sFast <= sSlow && eFast > eSlow)) return false;
        if (v === '데드크로스' && !(sFast >= sSlow && eFast < eSlow)) return false;
        break;
      ...
      case 'gc_ema_20_120': {
        ...
        const eFast = _emaLast(closes, 20), eSlow = _emaLast(closes, 120);
        const startSlice = closes.slice(0, sLen);
        const sFast = _emaLast(startSlice, 20), sSlow = _emaLast(startSlice, 120);
        if (!Number.isFinite(sFast) || !Number.isFinite(sSlow) || !Number.isFinite(eFast) || !Number.isFinite(eSlow)) return false;
        if (v === '골든크로스' && !(sFast <= sSlow && eFast > eSlow)) return false;
        if (v === '데드크로스' && !(sFast >= sSlow && eFast < eSlow)) return false;
        break;
