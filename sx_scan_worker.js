...
function calcBtScore(btData){
  ...
  return Math.round(Math.min(100, Math.max(0, pnlScore + wrScore + pfScore + mddScore + evScore)));
}
    ...
    for (let i = 0; i < total;) {
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if(_isBtSupportedTF(currentMarket, currentTF))
        {
          ...
          // [S292] 분석엔진 매수판정 필터 — s._svVerdict.action === '매수' 직접 매칭
          //   배너에 🟢매수 표시된 종목만 통과 (종합행동지침 '매수'와 동일 소스이나 단일 ON/OFF로 단순화)
