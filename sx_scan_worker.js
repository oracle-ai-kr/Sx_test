...
async function fetchCandles(code, count, _retry) {
  ...
  try {
    ...
    if (currentMarket === 'coin') {
      ...
      // [S655] Upbit 1회 호출 200봉 한도 — count>200(600/700 정밀모드)이면 sx_bt.js의 btFetchCandlesCoin과
      //   동일한 to커서 페이지네이션(120ms 지연)으로 확장. count<=200(기존 기본 스캔)은 무변경(단일 호출, 추가 지연 없음).
      const UPBIT_PAGE = 200;
      const _pages = Math.max(1, Math.ceil(count / UPBIT_PAGE));
      let allArr = [];
      let _cursor = '';
      for (let p = 0; p < _pages; p++) {
        const toParam = _cursor ? `&to=${encodeURIComponent(_cursor)}` : '';
        const batchCount = Math.min(UPBIT_PAGE, count - allArr.length);
        const pageUrl = `${WORKER_BASE}/upbit/candles?market=${coinMarket}&type=${upbitType}&count=${batchCount}${toParam}`;
        const pageRes = await _fetchWithTimeout(pageUrl);
        if (!pageRes.ok) break;
        const pageJson = await pageRes.json();
        const pageArr = Array.isArray(pageJson) ? pageJson : (pageJson.data || []);
        if (!pageArr.length) break;
        allArr = allArr.concat(pageArr);
        if (pageArr.length < batchCount) break; // 더 이상 과거 데이터 없음
        const _oldest = pageArr[pageArr.length - 1];
        _cursor = _oldest.candle_date_time_utc || _oldest.candle_date_time_kst || '';
        if (!_cursor) break;
        if (p < _pages - 1) await new Promise(r => setTimeout(r, 120)); // [S655] rate limit 방지 (btFetchCandlesCoin과 동일 120ms)
      }
      raw = allArr.map(r => ({
      ...
      })).filter(_sxIsValidCandle).sort((a, b) => (a.date || '').localeCompare(b.date || '')); // [S655] reverse→sort: 페이지 경계 안전성↑ (단일페이지는 결과 동일)
...
function checkTechConditions(ind, techFilters, getFilter) {
  for (const f of techFilters) {
    ...
    switch (f.id) {
      ...
      case 'candle_transition': {   // [S495→S622→S647→S654] 캔들 전이 — kNN 우선(라이브 카드 미러), 비활성이면 룰 폴백. 임계는 실제 확보 봉수에 따라 가변(아래).
        if (!Array.isArray(v) || v.length === 0) break;
        const _rowsCT = (ind._advanced && ind._advanced.rows) || ind.candles;
        if (!_rowsCT || _rowsCT.length < 25) return false;
        let _ctScore = null;
        // [S622] kNN — 슬라이드 루프가 재사용 벡터로 미리 계산해 ind._ctKnn에 실어줌(빠름). 없으면(루프 외 호출) 직접 score().
        try {
          const _kn = (ind._ctKnn !== undefined)
            ? ind._ctKnn
            : ((self.SXKNN && self.SXKNN.score) ? self.SXKNN.score(_rowsCT, { win: 16, k: 10 }) : null);
          if (_kn && _kn.active && typeof _kn.score === 'number') _ctScore = _kn.score;
        } catch (_eKnn) { _ctScore = null; }
        // [S650] kNN 비활성(봉수<56=win16+MIN_BANK40 / 신규상장 / 이력부족) → 제외. 룰 폴백 제거.
        //   룰(_candleTransitionScore)은 검증 없는 휴리스틱이라 봉수 부족 종목이 룰로 경계 넘어 걸리던 오검출 차단(정밀도↑).
        //   정상 fetch 종목은 kNN active라 영향 없음 — 데이터 부실 종목만 빠짐. 라이브 카드는 룰을 '룰 해설(보조)'로 유지(역할 분리).
        if (_ctScore === null) return false;
        const _wantBull = v.includes('trans_bull'), _wantBear = v.includes('trans_bear');
        // [S654→S655] 정밀모드(_scCount 600/700)로 실제 봉수가 충분히 확보됐으면 라이브 카드와 동일 ±30.
        //   [S655] 코인도 to커서 페이지네이션으로 600봉 도달 가능 — 시장 구분 없이 실제 확보 봉수로만 판단.
        //   미달 시(타임아웃·일부 페이지 실패 등 드문 예외)만 기존 보수적 ±35 유지(S647 원래 의도 보존).
        const _ctTh = (_rowsCT.length >= 550) ? 30 : 35;
        const _isBull = _ctScore >= _ctTh, _isBear = _ctScore <= -_ctTh;
      ...
      case 'knn_dday': {   // [S633] kNN 크로스 임박 D-day — 최신봉만(슬라이드 k=0). ind._knnDday=crossDday 결과(루프 사전계산), 없으면 직접 호출.
        ...
        if (_kd === undefined) { _kd = (self.SXKNN && self.SXKNN.crossDday) ? self.SXKNN.crossDday((ind._advanced && ind._advanced.rows) || ind.candles, currentTrendMa) : null; }
        if (!_kd || !_kd.active) return false;
        if ((v === '데드크로스 임박') !== (_kd.type === 'dc')) return false;
        break;
      }
      case 'gc_ema_20_200': {
        if (v !== '골든크로스' && v !== '데드크로스') break;
...
function _slimResults(arr) {
  return arr.map(s => ({
    ...
    _scanResult: s._scanResult ? {
      ...
      volRatio: s._scanResult.volRatio, pbScore: s._scanResult.pbScore, rcpK: s._scanResult.rcpK, rcpPure: s._scanResult.rcpPure, rcpFake: s._scanResult.rcpFake,   // [S894]
...
async function startScan(config) {
  // [S877] 킬스위치 전파 — 메인(🍳 pill) 상태를 스캔 config로 수신. 미전달=코어 기본 ON.
  try{ if(config && typeof config.recipeOn!=='undefined') self.SX_RECIPE_REBOUND = !!config.recipeOn; }catch(_eRcp){}
  // config에서 상태 주입
  ...
  try {
    ...
    // [S652] OR모드 — f.group(등록 시점 토글값, 기본 'and')으로 AND/OR 그룹 분리.
    ...
    // [S654] 600/700봉 정밀모드 — candle_transition/knn_dday(kNN 의존 조건)가 켜져있으면
      ...
      for (let bi = 0; bi < batch.length; bi++) {
        ...
        if (needCandles) {
          try {
            ...
            // [S660] 실제 확보 봉수 보고 — 정밀모드(_scCount 600/700) 적용 시 코인 페이지네이션 등이
            //   목표만큼 실제로 받아오고 있는지 스캔 중 실시간으로 확인할 수 있게(내 필터 탭 표시용).
            self.postMessage({ type: 'candleInfo', count: candles.length, target: _scCount, precision: _needPrecision });
            ...
            // [S317] 내부 _gcN 윈도우 조건이 있으면 외부 슬라이드 비활성화 (Nactual=1) — 내부 윈도우만 사용
            ...
            let _ctKnnPre = null;
            if (self.SXKNN && self.SXKNN.buildVecs && cLen >= 56 && techFilters.some(f => f && f.id === 'candle_transition')) {
              try { _ctKnnPre = self.SXKNN.buildVecs(candles, 16); } catch (_eCtv) { _ctKnnPre = null; }
            }
            for (let k = 0; k < Nactual; k++) {
              ...
              // [S622] 이 슬라이드 지점(봉 cLen-1-k) kNN — 재사용 벡터로 scoreAt. 결과를 indK에 실어 candle_transition case가 읽음.
              if (_ctKnnPre) { try { indK._ctKnn = self.SXKNN.scoreAt(candles, cLen - 1 - k, _ctKnnPre, { k: 10 }); } catch (_eCts) { indK._ctKnn = null; } }
              // [S633] kNN 크로스 임박 D-day — 최신봉 기준 1회(knn_dday는 _hasGcFilter라 k=0만). crossDday가 내부서 maS/maL·라벨 kNN 계산.
              if (self.SXKNN && self.SXKNN.crossDday && techFilters.some(f => f && f.id === 'knn_dday')) { try { indK._knnDday = self.SXKNN.crossDday(candles, currentTrendMa); } catch (_eKd) { indK._knnDday = null; } }
              if (techFilters.length === 0) {
                indicators = indK; passed = true; passedK = k; break;
              }
              // [S653] 그룹간 관계 — OR그룹이 존재하면 "AND그룹 전체충족 OR OR그룹 하나충족"(대안경로 2개).
              //   한쪽 그룹이 비어있으면 그 그룹은 비교 자체에서 빠지고 나머지 그룹 결과만 봄
              //   (techFilters.length>0 보장되므로 andTechFilters/orTechFilters 중 최소 하나는 비어있지 않음).
              //   배경: "OR로 추가" 자체가 이미 대안조건 의도 표시라 별도 결합모드 토글 불필요(S652 그룹간AND 대체).
              const _andApplicable = andTechFilters.length > 0;
              const _orApplicable  = orTechFilters.length > 0;
              let _andOk = true, _andFail = null;
              for (const _tf of andTechFilters) {
                if (!checkTechConditions(indK, [_tf], getFilter)) { _andOk = false; _andFail = _tf.id; break; }
              }
              let _orOk = false;
        ...
        if (indicators && indicators._advanced) {
          ...
          // [S658] kNN 음봉전이 — 분석엔진(3392행대) 미러. 실험적 ML 신호라 기본 OFF. kNN은 sx_candle_knn.js가 importScripts로
          //   이미 로드돼있어 워커에서도 가벼움(distance calc만, LR처럼 학습 없음 — 배치스캔에 안전).
          let _knnBearW = false;
          if (_sfW.knnBearish && typeof SXKNN !== 'undefined' && SXKNN.score) {
            try {
              const _kn2W = SXKNN.score(candles, { win: 16, k: 10 });
              if (_kn2W && _kn2W.active && typeof _kn2W.score === 'number' && _kn2W.score <= -30) _knnBearW = true;
            } catch (_eKn2W) { _knnBearW = false; }
          }
          ...
          {
            const _BEAR_CONT = ['데드크로스','지지선이탈','RSI다이버전스','MACD음전','MA60저항','매물대저항','되돌림주의','이브닝','슈팅','하락장악','가짜돌파','kNN음봉전이']; // [S529→S658] 가짜돌파/kNN음봉전이 추가 (엔진 미러)
            ...
            if (rawRows.length >= 60 && typeof SXE !== 'undefined' && SXE.runBtEngine) {
              ...
              if (btResult && !btResult.error) {
                ...
                try {
                  ...
                  // [S877] 레시피 하이브리드 — 스캔 정합(일봉만·코어 votes). 미산출/비일봉=미주입 → project_c 레거시 폴백(분석탭 S868과 동일 규칙).
                  let _rcpSig = null;
                  ...
                  const _scores4 = {
                    ...
                    recipeSig: _rcpSig || undefined // [S877] 스캔 하이브리드 — 분석탭↔스캔 비대칭 해소
                  };
