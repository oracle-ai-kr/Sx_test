...
(function(global){
  ...
  function _sxModal(opts){
    ...
    return new Promise(function(resolve){
      ...
      // 백드롭 클릭 → 취소(confirm) / 닫기(alert)
      // [S224-fix1] 모바일 ghost click 차단 — 백드롭 닫힌 후 뒤 요소(탭버튼 등)에 클릭이 전달되는 것 방지
      //   원인: touchstart→touchend→mousedown→mouseup→click 시퀀스 + 모달 제거 후 뒤 요소가 동일 위치에 노출
      //   대책: pointerdown 단계에서 미리 ghost click 차단 플래그 설정 + click에서 stopPropagation/preventDefault
      ov.addEventListener('pointerdown', function(e){
        if(e.target === ov){
          ov._backdropPress = true;
          // pointerdown 자체도 다른 핸들러 도달 차단
          e.stopPropagation();
        }
      });
      ov.addEventListener('click', function(e){
        if(e.target === ov){
          // 클릭이 백드롭 자신에게서 발생했음을 확인하고 이벤트 전파 차단
          e.stopPropagation();
          e.preventDefault();
          _vib(8);
          _close(opts.type === 'alert');
          // ghost click 차단 — 모달 제거 후 같은 좌표의 뒤 요소에 합성 click이 도달하는 것 방지
          //   document 레벨 capture 단계에서 1회 차단
          ...
          // 짧은 시간 후 자동 해제 (안전장치 — 다음 정상 클릭이 무시되지 않도록)
          setTimeout(function(){
            document.removeEventListener('click', _killGhost, true);
            document.removeEventListener('mousedown', _killGhost, true);
            document.removeEventListener('mouseup', _killGhost, true);
