...
SXI.practicalGuide = function(verdictAction, analScore, btScore, summary, ind, scores4, momBadge, passCount, passPath){
  ...
  if(verdictAction === '매수'){
    ...
    if(has4){
      ...
      if(_timingGated){
        guide.steps.push(`4축 셋업은 매수 수준(${passCount}/4)인데, 오늘 진입타이밍(A)이 아직 기준선 미달이라 관심 단계입니다.`);
        guide.steps.push(`자리는 갖춰졌으니 서두를 필요 없습니다 — 내일 진입타이밍 점수가 기준을 넘으면 매수로 올라갑니다. 매일 A 점수를 확인하세요.`);
      } else {
