...
SXI.unifiedNarrative = function(verdict, scores, scores4, ind, stock){
  ...
  if(conflictType === 'sell_with_good_score'){
    ...
    if(action === '즉시 청산' || action === '매도 완료'){
      ...
      guide = '부문별 점수가 좋다고 해서 추가 매수할 자리는 아닙니다. "재료가 시장에 다 알려진 상태에서 저항·이격 과다로 도달 → 이익 보존" 구간입니다. BT 손절선 도달 전 분석 엔진이 먼저 위험을 감지한 상태이므로, 점수가 좋아 보여도 규칙대로 매도 실행이 통계적으로 유리합니다.';
      ...
      guide = '지표는 좋아 보이지만, 4축 합격 개수가 부족해 진입 신호로 확정되지 않았습니다. "지표가 좋다 = 사야 한다"가 아니라 "지표 좋고 + 자리도 맞을 때" 사야 통계적으로 유리합니다. 다음 진입 타이밍을 기다리세요.';
    }
  }
    ...
    if(action === '매수' || action === '관심'){
      ...
      guide = '현재 스냅샷 지표(RSI/MFI 등)는 약하지만, 4축 구조와 BT 통계가 진입 우위를 가리킵니다. 과거 비슷한 상황에서 수익이 났던 자리이므로 분할 진입과 명확한 손절 기준을 함께 설정하세요.';
    ...
    if(action === '매수') guide = '4축이 모두 합격하고 부문별 점수도 양호합니다. 가장 신뢰도 높은 진입 자리로, 분할 진입 후 추세 추종 전략이 유리합니다.';
    else if(action === '관심') guide = '점수는 양호하나 4축 일부가 부족합니다. 1~2축이 추가로 합격하면 매수 전환 가능성이 높아지므로 관찰 우선입니다.';
    ...
    guide = '부문별 점수와 4축 판정이 모두 약세를 가리킵니다. 신규 매수는 자제하고, 보유 중이라면 손절 기준을 엄격히 지키세요.';
  }
  else {
    // 중립 — 점수 45~55 범위
    tone = 'neutral';
  ...
  let reconcile = '';
  if(conflictType === 'sell_with_good_score'){
    reconcile = '※ 부문별 점수는 "여기까지 오게 된 재료"를 보고, 4축 판정은 "지금부터의 위험/보상"을 봅니다. 두 결과가 달라 보이는 것은 시간축이 다르기 때문이며, 둘 다 맞는 정보입니다.';
  } else if(conflictType === 'buy_with_bad_score'){
    reconcile = '※ 부문별 점수는 현재 지표 스냅샷이고, 4축 판정은 구조·통계 누적 결과입니다. 점수가 낮아도 "자리"가 좋은 케이스가 종종 발생합니다.';
  }
...
SXI.renderDisclosureCard = function(discItp){
  ...
  if(entries && entries.length > 0){
    ...
    if(tail.length > 0){
      ...
      // .itp-card는 display:none이 기본, .show 클래스로 펼쳐짐 (라인 867 정의)
      // background 영향 안 받게 padding/bg 인라인 스타일로 무력화
      bodyHTML = `<div>${headHTML}</div>
        <div class="itp-toggle-inline" onclick="_sxVib(8);const c=document.getElementById('${tid}');c.classList.toggle('show');this.querySelector('.sb-arrow').textContent=c.classList.contains('show')?'▼':'▶';this.querySelector('.sb-label').textContent=c.classList.contains('show')?'추가 ${tail.length}건 접기':'추가 ${tail.length}건 보기'" style="font-size:10px;color:var(--accent);cursor:pointer;font-weight:600;margin-top:5px;padding-top:4px;border-top:1px dashed var(--border)"><span class="sb-arrow">▶</span> <span class="sb-label">추가 ${tail.length}건 보기</span></div>
        <div class="itp-card disc-more-body" id="${tid}" style="background:transparent;padding:0;margin-top:4px">${tailHTML}</div>`;
    } else {
      bodyHTML = `<div>${headHTML}</div>`;
    }
  } else {
    // 폴백: lineEntries 없으면 기존 동작 (text 통째로)
    bodyHTML = `<div>${discItp.text}</div>`;
  }

  // [S57-compact] 카드 컴팩트화 — 외곽 padding/margin과 내부 요소 간격 축소
  //   1) anal-section 외곽 margin 축소 (기존 클래스 영향 없게 인라인으로)
  //   2) badges 영역 margin-bottom 8 → 5
  //   3) itp-card 내부 padding 축소 + label 박스 margin-bottom 추가 축소
  //   4) summary 구분선 margin-top 6 → 4, padding-top 4 → 3
  //   5) line-height 1.55 → 1.45 (정보 밀도 ↑)
  return `<div class="anal-section" style="margin-bottom:6px">
    <div class="anal-section-title" style="margin-bottom:5px">${discItp.label}</div>
    <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:5px">${badgeHTML}</div>
    <div class="itp-card show" style="white-space:pre-line;padding:7px 9px;line-height:1.45">
      <span class="itp-label ${discItp.tone}" style="margin-bottom:3px">${discItp.label}</span>
      ${bodyHTML}
      <div style="margin-top:4px;padding-top:3px;border-top:1px solid var(--border);font-size:10px;color:var(--text2)">${discItp.summary}</div>
    </div>
  </div>`;
};
...
SXI.practicalGuide = function(verdictAction, analScore, btScore, summary, ind, scores4, momBadge){
  ...
  if(verdictAction === '매수'){
    ...
    if(has4){
      ...
      guide.steps = [
        `${_scoreLine}`,
        `4축 중 ${pass4}개가 합격선을 넘었습니다. 바닥(반등 준비)·반등(상승 시작)·추세(상승 강도)·전략(과거 BT 성과)이 모두 매수에 유리한 상태입니다.`,
      ];
      if(_momText){
    ...
    if(has4){
      guide.steps = [
        `${_scoreLine}`,
        `4축 중 ${pass4}개가 합격선을 넘었습니다. 일부 조건이 부족해 매수 신호 확정에는 한 단계 모자란 상태입니다.`,
      ];
    ...
    if(has4){
      guide.steps = [
        `${_scoreLine}`,
        `4축 중 ${pass4}개만 합격해 매수 신호로는 부족합니다.`,
      ];
    ...
    if(has4){
      guide.steps = [
        `${_scoreLine}`,
        `4축 중 ${pass4}개만 합격해 진입 조건이 거의 갖춰지지 않았습니다.`,
      ];
      if(_momText && momBadge?.direction === 'down'){
