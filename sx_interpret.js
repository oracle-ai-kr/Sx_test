...
SXI.practicalGuide = function(verdictAction, analScore, btScore, summary, ind, scores4, momBadge){
  const s = analScore || 0;
  const b = btScore || 0;
  const sm = summary || {};
  ...
  const b4 = has4 ? (scores4.btScore || 0)    : b;
  // 4축 합격 개수
  const pass4 = has4 ? ((r4>=60?1:0) + (e4>=60?1:0) + (t4>=50?1:0) + (b4>=60?1:0)) : null;
  // 합격/미달 점수 라벨링
  const _ck = (v, th) => v >= th ? '✓' : '✗';
  const _scoreLine = has4
    ? `바닥 ${r4}${_ck(r4,60)} · 반등 ${e4}${_ck(e4,60)} · 추세 ${t4}${_ck(t4,50)} · 전략 ${b4}${_ck(b4,60)} (합격 ${pass4}/4)`
    : '';
  ...
  if(verdictAction === '매수'){
    ...
    if(has4){
      ...
      guide.steps = [
        `${_scoreLine}`,
        `바닥(반등 준비)·반등(상승 시작)·추세(상승 강도)·전략(과거 BT 성과)이 모두 매수에 유리한 상태입니다.`,
      ];
      if(_momText){
    ...
    if(has4){
      guide.steps = [
        `${_scoreLine}`,
        `일부 조건이 부족해 매수 신호 확정에는 한 단계 모자란 상태입니다.`,
      ];
