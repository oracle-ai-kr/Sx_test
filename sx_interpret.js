...
SXI.advTpSl = function(price, atrPct, tpMult, slMult, tf, regime, trendPct, rsiVal, adxVal){
  ...
  // 도달 예상 시간 텍스트
  // [BUGFIX] 분봉/시간봉 케이스 ${bars} 중복 출력 버그 수정
  //   현재 사양: 분기 안에서 단일 표현식으로 정리 → '약 12봉(60분=1시간)' 형태로 명확히
  //   〔이력〕 이전: `약 ${bars}${unit==='5분' ? `×${bars}(...)`: ...}` → '약 12×12(1시간)' 같이 잘못 출력 (수정됨)
  const fmtTime = (bars, unit) => {
    ...
    // 분봉/시간봉: 봉 수 + 환산 시간 같이 표시
    if(unit === '5분')   return `약 ${bars}봉 (${(bars*5/60).toFixed(1)}시간)`;
    if(unit === '15분')  return `약 ${bars}봉 (${(bars*15/60).toFixed(1)}시간)`;
    if(unit === '30분')  return `약 ${bars}봉 (${(bars*30/60).toFixed(1)}시간)`;
    if(unit === '1시간') return `약 ${bars}시간`;
    if(unit === '4시간') return `약 ${bars}봉 (${bars*4}시간)`;
    return `약 ${bars}${unit}`;
  };
...
SXI.priceChannel = function(position, upper, lower, price){
  ...
  if(position==='breakout_up') return {label:'채널 상단 돌파',tone:'bullish',text:`가격(${pFmt(price)})이 최근 가격 범위 상단(${pFmt(upper)})을 돌파했습니다.${range} N일간의 고점을 돌파한 것으로, 새로운 상승 추세의 시작일 수 있습니다. 거래량이 평소의 1.5배 이상 동반되면 진짜 돌파 확률이 높아지며, 거래량 없는 돌파는 거짓 돌파 위험이 있습니다. 돌파 직후 돌파 후 되돌림 시 이전 상단이 지지로 전환되면 추가 진입 구간이 됩니다. 되돌림이 채널 안으로 다시 빠지면 거짓 돌파이므로 즉시 매도하세요.`};
...
SXI.compositeSynergy = function(ind){
  ...
  if(vhfT==='ranging' && eomV!=null && Math.abs(eomV)>0.3){
    if(eomT==='bullish') notes.push({tone:'bullish',icon:'[?]',title:'횡보 속 EOM 매수 신호',text:`VHF 횡보장이지만 EOM ${eomV.toFixed(2)}로 매수 쪽 이동이 용이해지고 있습니다. 박스권 상방 돌파의 선행 신호일 수 있으며, BB 상단 돌파를 주시하세요.`});
    if(eomT==='bearish') notes.push({tone:'bearish',icon:'[?]',title:'횡보 속 EOM 매도 신호',text:`VHF 횡보장이지만 EOM ${eomV.toFixed(2)}로 하락 쪽 이동이 용이해지고 있습니다. 박스권 하방 이탈의 선행 신호일 수 있으며, BB 하단·지지선 이탈에 주의하세요.`});
  }
...
SXI._attachRegimeContext = function(verdictAction, regime){
  ...
  const M = {
    '매수': {
      UP:   ' 시장 레짐이 상승 추세이므로 매수 판정의 신뢰도가 한층 높습니다.',
      DOWN: ' 다만 시장 레짐은 하락 추세라 추세 역방향 진입에 해당하므로, 비중을 줄이고 손절 기준을 더 엄격히 적용하세요.',
      FLAT: ' 시장 레짐이 횡보 구간이라 큰 추세 추진력은 기대하기 어렵습니다. 박스권 상단 저항을 함께 확인하세요.'
    },
    '관심': {
      UP:   ' 시장 레짐이 상승 추세이므로 진입 트리거 발동 시 추세 동조 효과를 기대할 수 있습니다.',
      DOWN: ' 시장 레짐이 하락 추세라 트리거가 나오더라도 단기 반등에 그칠 가능성이 있으니, 관망 비중을 늘리는 편이 안전합니다.',
      FLAT: ' 시장 레짐이 횡보라 트리거 신뢰도가 낮을 수 있습니다. 거래량 동반 돌파 확인 후 진입을 고려하세요.'
    },
    '관망': {
      UP:   ' 시장 레짐은 상승 추세이지만 종목 자체 신호가 부족합니다. 동일 섹터 강세주로 시야를 넓혀보는 것도 방법입니다.',
      DOWN: ' 시장 레짐도 하락 추세라 관망이 가장 적절한 선택입니다. 추세 전환 신호가 나올 때까지 기다리세요.',
      FLAT: ' 시장 레짐도 횡보라 관망이 정석입니다. 박스권 돌파 또는 추세 형성 신호를 기다리세요.'
    },
    '회피': {
      UP:   ' 시장 레짐은 상승 추세이지만 이 종목은 회피 대상입니다. 추세를 따라가려면 다른 강세주로 종목을 바꾸세요.',
      DOWN: ' 시장 레짐도 하락 추세라 회피 판정이 더욱 분명합니다. 신규 진입은 위험합니다.',
      FLAT: ' 시장 레짐이 횡보임에도 회피 신호가 나온 것은 종목 고유 위험이 누적된 결과입니다. 진입을 자제하세요.'
    },
    '보유 유지': {
      UP:   ' 시장 레짐이 상승 추세라 보유 지속의 통계적 근거가 충분합니다.',
      DOWN: ' 다만 시장 레짐은 하락 추세이므로, 익절 목표를 보수적으로 잡고 트레일링 스톱을 좁혀두는 편이 안전합니다.',
      FLAT: ' 시장 레짐이 횡보라 큰 추가 상승은 기대하기 어렵습니다. 박스권 상단 근접 시 부분 익절도 고려하세요.'
    },
    '청산 준비': {
      UP:   ' 시장 레짐이 상승 추세라 약화 신호가 단기 조정에 그칠 수도 있으니, 즉시 청산보다는 분할 익절로 대응하세요.',
      DOWN: ' 시장 레짐도 하락 추세라 약화 신호의 신뢰도가 높습니다. 청산 시점을 앞당기는 것을 적극 검토하세요.',
      FLAT: ' 시장 레짐이 횡보라 추세 회복 동력이 부족합니다. 박스권 하단 이탈 전 비중 축소를 권장합니다.'
    },
    '청산 검토': {
      UP:   ' 시장 레짐은 상승 추세이지만 종목 지표가 명확히 악화된 상태입니다. 종목 고유 약세이므로 보유 유지 명분이 약합니다.',
      DOWN: ' 시장 레짐도 하락 추세라 청산 명분이 매우 분명합니다. 손절 기준을 즉시 점검하고 실행하세요.',
      FLAT: ' 시장 레짐이 횡보라 추세 반전 기대도 약합니다. 청산 후 재진입 기회를 노리는 편이 효율적입니다.'
    },
    '즉시 청산': {
      UP:   ' 시장 레짐이 상승 추세임에도 매도 신호가 나온 것은 종목 고유 약세가 매우 강하다는 뜻입니다. 신호대로 즉시 실행하세요.',
      DOWN: ' 시장 레짐도 하락 추세라 매도 신호 신뢰도가 극대화됩니다. 지체 없이 매도를 실행하세요.',
      FLAT: ' 시장 레짐이 횡보임에도 매도 신호가 발생했습니다. 박스권 이탈 가능성이 있으니 신호대로 실행하세요.'
    },
    '매도 완료': {
      UP:   ' 시장 레짐이 상승 추세라 동일 섹터 다른 강세주로 자금을 회전하면 추세 동조 수익을 노릴 수 있습니다.',
      DOWN: ' 시장 레짐이 하락 추세라 청산 후 재진입을 서두르지 마세요. 시장 회복 신호 확인 후 접근이 안전합니다.',
      FLAT: ' 시장 레짐이 횡보라 새로운 추세 형성 시점까지 현금 비중을 유지하는 것도 좋은 전략입니다.'
    }
  };
...
// ════════════════════════════════════════════════════════════
//  5-B단계: 종합해석 가이드 (S57 신규)
//  ─ 4축 판정(verdict) ↔ 부문별 점수(scores) 충돌/일치를 화해시키는 카드
//  ─ 사용자가 "점수 좋은데 왜 매도?" "점수 나쁜데 왜 매수?" 헷갈리는 케이스에 결론 제공
//  ─ 입력:
//     · verdict : stock._svVerdict (action/passCount/momBadge/...) — unifiedVerdictV2 결과
//     · scores  : calcEnhancedScores 결과 (momentum/value/volume/trend, total)
//     · scores4 : {readyScore, entryScore, trendScore, btScore} — 4축 원점수
//     · ind     : indicators (advanced 포함)
//     · stock   : 종목 객체 (changeRate 등)
//  ─ 출력: {tone, headline, realMessage[], guide, conflictType}
//     · tone         : bullish | bearish | neutral | warning
//     · headline     : 한 줄 결론 ("재료는 좋으나 자리가 위험")
//     · realMessage  : 차트·지표 근거 줄 배열 (3~5개)
//     · guide        : "어떻게 행동해야 하나" 한 문단
//     · conflictType : 'sell_with_good_score' | 'buy_with_bad_score' | 'aligned_bull' | 'aligned_bear' | 'neutral'
// ════════════════════════════════════════════════════════════
SXI.unifiedNarrative = function(verdict, scores, scores4, ind, stock){
  if(!verdict || !scores) return null;
  const action = verdict.action || '';
  // 부문별 평균 (공시 제외 — 4개 평균이 사용자 시각에 가까움)
  const sm = scores.momentum||50, sv=scores.value||50, svo=scores.volume||50, st=scores.trend||50;
  const avgSector = (sm + sv + svo + st) / 4;
  // 4축 합격 개수
  const pass = verdict.passCount || 0;
  // 충돌 유형 분류 ─ 매도 계열 + 점수 양호 / 매수 계열 + 점수 부진 / 정합
  const sellSide = ['즉시 청산','청산 검토','청산 준비','매도 완료','회피','관망'].includes(action);
  const buySide  = ['매수','관심','보유 유지'].includes(action);
  let conflictType = 'neutral';
  if(sellSide && avgSector >= 58) conflictType = 'sell_with_good_score';
  else if(buySide && avgSector <= 42) conflictType = 'buy_with_bad_score';
  else if(buySide && avgSector >= 55) conflictType = 'aligned_bull';
  else if(sellSide && avgSector <= 45) conflictType = 'aligned_bear';
  ...
  if(ind){
    ...
    // 1) 구조 위치 (저항/지지 근접)
    const sp = adv.structPos;
    if(sp){
      const pct = sp.pct ?? sp.percent;
      if(typeof pct === 'number'){
        if(pct >= 80) reasons.push(`구조 위치 ${Math.round(pct)}% (저항근접) → 박스 상단 부근`);
        else if(pct <= 20) reasons.push(`구조 위치 ${Math.round(pct)}% (지지근접) → 박스 하단 부근`);
      }
    }
    // 2) MA 이격 (단기 과열/괴리)
    const ma20 = ind.ma20, ma60 = ind.ma60, last = ind.last||ind.price;
    if(ma20 && ma60 && last){
      const d20 = (last - ma20)/ma20*100;
      const d60 = (last - ma60)/ma60*100;
      if(d20 >= 7 && d60 >= 7) reasons.push(`20일선 +${d20.toFixed(1)}%, 60일선 +${d60.toFixed(1)}% 이격 → 단기 과열`);
      else if(d20 <= -7 && d60 <= -7) reasons.push(`20일선 ${d20.toFixed(1)}%, 60일선 ${d60.toFixed(1)}% 이격 → 단기 과매도`);
    }
    // 3) MA 수렴/분산 + 가격 기울기
    const conv = adv.maConv;
    if(conv && conv.spread != null && conv.spread >= 4){
      reasons.push(`MA 분산 ${conv.spread.toFixed(1)}% → 추세 가속 구간`);
    }
    // 4) 수급 내부 (Chaikin / A/D / AB Ratio)
    const inner = [];
    if(adv.chaikinOsc && adv.chaikinOsc.val < 0) inner.push('Chaikin 음수');
    else if(adv.chaikinOsc && adv.chaikinOsc.val > 0 && adv.chaikinOsc.trend === 'bullish') inner.push('Chaikin 양수'); // [S452] ChaikinOsc.trend는 'bullish'/'bearish' (='up' 아님) → 기존 'up' 비교가 죽은 가지여서 양수 노트 미발동
    if(adv.adLine && adv.adLine.trend === 'down') inner.push('A/D 하락');
    else if(adv.adLine && adv.adLine.trend === 'up') inner.push('A/D 상승');
    if(adv.abRatio && adv.abRatio.val != null){
      if(adv.abRatio.val < 0.9) inner.push('AB Ratio 매도우위');
      else if(adv.abRatio.val > 1.1) inner.push('AB Ratio 매수우위');
    }
    if(inner.length){
      const innerTone = inner.some(s=>/음수|하락|매도우위/.test(s)) ? '내부 수급 약화' : '내부 수급 양호';
      reasons.push(`${inner.join(', ')} → ${innerTone}`);
    }
    // 5) 4축 원점수 요약 (반등/추세)
    if(scores4){
      ...
      reasons.push(`반등 강도 ${e4} (${e4Lbl}), 추세 방향 ${t4} (${t4Lbl}) → 동력 ${e4>=55?'살아있음':'미흡'}`);
    }
  }

  // ─── 충돌 유형별 헤드라인/가이드 ───
  let tone='neutral', headline='', guide='';

  if(conflictType === 'sell_with_good_score'){
    // 부문별 점수 좋은데 매도 신호 — 가장 흔한 혼란 케이스
    tone = 'warning';
    if(action === '즉시 청산' || action === '매도 완료'){
      headline = '재료는 좋았으나 지금은 차익실현 자리';
      guide = '추가 매수할 자리가 아닙니다. "재료가 시장에 다 알려진 상태에서 저항·이격 과다로 도달 → 이익 보존" 구간입니다. BT 손절선 도달 전 분석 엔진이 먼저 위험을 감지한 상태이므로, 점수가 좋아 보여도 규칙대로 매도 실행이 통계적으로 유리합니다.';
    } else if(action === '청산 검토'){
      headline = '재료는 살아있으나 구조가 약해지는 중';
      guide = '점수상 펀더멘털·모멘텀은 양호하지만, 분석 엔진은 구조 약화(저항 근접·내부 수급 약화)를 먼저 감지했습니다. 추격 매수는 보류하고, 보유 중이면 손절·익절 라인을 좁혀 잡으세요.';
    } else if(action === '청산 준비'){
      headline = '재료는 양호 — 구조 약화 초기 신호';
      guide = '아직 매도 확정 단계는 아닙니다. 점수가 양호하므로 즉시 매도보다는, 다음 봉에서 반등 강도가 회복되는지 확인 후 결정해도 늦지 않습니다.';
    } else if(action === '회피' || action === '관망'){
      headline = '점수는 양호 — 그러나 진입 자리는 아님';
      guide = '지표는 좋아 보이지만, 충족 조건이 부족해 진입 신호로 확정되지 않았습니다. "지표가 좋다고 사야 한다"가 아니라 "지표도 좋고 자리도 맞을 때" 사야 통계적으로 유리합니다. 다음 진입 타이밍을 기다리세요.';
    }
  }
  else if(conflictType === 'buy_with_bad_score'){
    // 점수는 부진한데 매수/보유 신호 — 흔치 않지만 BT 통계 우위 케이스
    tone = 'warning';
    if(action === '매수' || action === '관심'){
      headline = '지표는 부진 — 그러나 통계적 진입 자리';
      guide = '현재 스냅샷 지표(RSI/MFI 등)는 약하지만, 분석 조건이 진입 우위를 가리킵니다. 과거 비슷한 상황에서 수익이 났던 자리이므로 분할 진입과 명확한 손절 기준을 함께 설정하세요.';
    } else if(action === '보유 유지'){
      headline = '지표 약화 — 그러나 보유 지속 우위';
      guide = '단기 지표는 약해 보이지만 BT 엔진은 아직 매도 신호를 내지 않았습니다. 충동적 매도보다 손절선 유지가 통계적으로 유리합니다.';
    }
  }
  else if(conflictType === 'aligned_bull'){
    tone = 'bullish';
    headline = '지표·구조·통계 모두 매수 우위';
    if(action === '매수') guide = '가장 신뢰도 높은 진입 자리로, 분할 진입 후 추세 추종 전략이 유리합니다.';
    else if(action === '관심') guide = '점수는 양호하나 조건 일부가 부족합니다. 추가로 조건이 좋아지면 매수 전환 가능성이 높아지므로 관찰 우선입니다.';
    else guide = '지표와 구조가 함께 강합니다. 보유 지속이 원칙이며, MA20/주요 지지선 이탈이 없는 한 변경할 이유가 없습니다.';
  }
  else if(conflictType === 'aligned_bear'){
    tone = 'bearish';
    headline = '지표·구조·통계 모두 매도 우위';
    guide = '분석 판정이 모두 약세를 가리킵니다. 신규 매수는 자제하고, 보유 중이라면 손절 기준을 엄격히 지키세요.';
  }
  else {
    // 중립 — 점수 45~55 범위
    tone = 'neutral';
    headline = '뚜렷한 우위 없음 — 다음 신호 대기';
    guide = '점수와 판정이 모두 중립권에 있습니다. 방향이 정리될 때까지 신규 진입을 보류하는 것이 안전합니다.';
  }

  // 결론 한 줄 추가 — "둘 다 맞는 말" 화해 문구 (충돌 케이스에만)
  let reconcile = '';
  if(conflictType === 'sell_with_good_score'){
    reconcile = '※ 부문별 점수는 "여기까지 오게 된 재료"를 보고, 분석 판정은 "지금부터의 위험/보상"을 봅니다. 두 결과가 달라 보이는 것은 시간축이 다르기 때문이며, 둘 다 맞는 정보입니다.';
  } else if(conflictType === 'buy_with_bad_score'){
    reconcile = '※ 부문별 점수는 현재 지표 스냅샷이고, 분석 판정은 구조·통계 누적 결과입니다. 점수가 낮아도 "자리"가 좋은 케이스가 종종 발생합니다.';
  }
  ...
  return {
    ...
    reconcile,
    conflictType,
    avgSector: Math.round(avgSector),
    passCount: pass,
    action
  };
};
...
SXI.advFundamental = function(fin, price, stock){
  ...
  // [재무 종합평가 시각화] 다른 부문(반등신호/추세/공시)과 동일한 형식의 점수/등급 산출
...
SXI.practicalGuide = function(verdictAction, analScore, btScore, summary, ind, scores4, momBadge, passCount, passPath){
  ...
  const _scoreLine = has4
    ? (_isTrendPath
        ...
        : `반등신호 ${r4}${_ck(r4,60)} · 반등강도 ${e4}${_ck(e4,60)} · 추세방향 ${t4}${_ck(t4,50)} · 모멘텀 ${_momMark} (합격 ${_passReal}/4)`)
    : '';
  ...
  if(verdictAction === '매수'){
    ...
    if(has4){
      ...
      if(_momText){
        guide.steps.push(`${_momText} — 모멘텀이 살아있어 매수 신호의 신뢰도가 높습니다.`);
      } else {
    ...
    guide.crossCheck = '매매전략 점수 변화 추이를 확인하세요. 점수가 계속 하락하면 "매도 검토" 또는 "즉시 매도"로 격상될 수 있습니다.';
 ...
 */
// [S259] market 옵셔널 인자 추가 — _estimateStaleness 정확도 향상용
//   기본값 'kr' (호출자가 market 미전달 시 기존 동작 100% 유지)
//   호출자(sx_render.js) 변경 시 'us'/'coin' 전달하면 staleness label 정확해짐
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats, verdictAction, btStateObj, market) {
  if (!qs) return null;
  ...
  const upsideScore = qs.upsideScore ?? 0;   // [S357] ④ 추가상승
  const upsideNotes = qs.upsideNotes || [];
  ...
  const _readyWeightMap = [
    ...
    {name:'골든크로스', maxW:10, match:['골든크로스']},
  ...
  if (_tone === 'entry') {
    if (readyScore >= 70) readySummary = '반등 신호 조건이 충분히 익었습니다. 과매도·수축·눌림이 다수 겹치고 있어 반등 가능성이 높은 구간입니다.';
    else if (readyScore >= 50) readySummary = '일부 조건이 충족되고 있지만 아직 반등 신호가 완전히 확립되지 않았습니다. 추가 조건 충족을 기다리는 것이 안전합니다.';
    else if (readyScore >= 30) readySummary = '반등 신호 상태가 미흡합니다. 과매도 신호가 약하거나 아직 하락 추세가 진행 중일 수 있습니다.';
    else readySummary = '현재 반등 신호 조건이 거의 없습니다. 추가 하락 가능성을 염두에 두고 관망이 필요합니다.';
  } else if (_tone === 'caution') {
    if (readyScore >= 70) readySummary = '진입타이밍 조건은 충족되어 있지만, 현재 보유 중인 포지션의 관리가 우선입니다. 추가 매수보다는 목표가·손절가 점검에 집중하세요.';
    else if (readyScore >= 50) readySummary = '부분적 반등 신호가 있지만, 현재 국면에서는 신규 진입보다 기존 포지션 관리에 집중하는 것이 적절합니다.';
    else if (readyScore >= 30) readySummary = '진입 준비 수준이 낮습니다. 보유 중이라면 손절 기준을 재점검하고, 추가 매수는 자제하세요.';
    else readySummary = '반등 신호가 거의 없는 상태입니다. 보유 포지션의 리스크 관리를 강화하세요.';
  } else { // exit
    if (readyScore >= 70) readySummary = '반등 신호 조건은 충족되어 있지만, 전체 판정이 매도 방향입니다. 기술적 반등을 노리기보다 현재 포지션 정리를 우선하세요.';
    else if (readyScore >= 50) readySummary = '일부 반등 신호가 있으나 전체적으로 매도 국면입니다. 기술적 반등에 속지 말고 포지션 축소를 검토하세요.';
    else if (readyScore >= 30) readySummary = '반등 신호 조건 미흡 + 매도 판정이 겹치고 있습니다. 추가 하락 위험이 큰 구간으로, 손절 또는 포지션 정리를 고려하세요.';
    else readySummary = '반등 신호 없음 + 매도 판정입니다. 하락 추세가 진행 중이며, 가능한 빨리 포지션을 정리하는 것이 안전합니다.';
  }
  ...
  const _entryWeightMap = [
    ...
    {name:'지지선 컨플루언스', maxW:8, match:['지지선+반전캔들 컨플루언스']}, // [S524] 지지선+반전캔들 결합 가산 — 엔진 scrEntryScore note와 동기화
  ...
  for (var ei = 0; ei < entryNotes.length; ei++) {
    ...
    if (en.indexOf('감점') >= 0 || en.indexOf('과매수') >= 0 || en.indexOf('음전환') >= 0 || en.indexOf('과열') >= 0 || en.indexOf('하락') >= 0 || en.indexOf('반락') >= 0) { // [S524] '반락'=저항선 컨플루언스 경고
  ...
  // S99-2: 3톤 분기 — 강세 해석
  if (_tone === 'entry') {
    if (entryScore >= 70) entrySummary = '복수의 반등 강도 요소가 동시에 감지되고 있습니다. RSI 반등, MACD 전환, 강세 캔들 등이 겹치며 실제 반등이 진행 중일 가능성이 높습니다.';
    else if (entryScore >= 50) entrySummary = '일부 반등 강도가 나타나고 있지만 아직 확정적이지 않습니다. 추가 확인 신호를 기다리는 것이 좋습니다.';
    else if (entryScore >= 30) entrySummary = '반등 강도가 약합니다. 간헐적 양봉이 보이지만 추세 전환을 확신하기 어려운 상태입니다.';
    else entrySummary = '반등 강도가 감지되지 않습니다. 아직 하락 또는 횡보가 지속되고 있을 가능성이 높습니다.';
  } else if (_tone === 'caution') {
    if (entryScore >= 70) entrySummary = '반등 강도가 강하지만, 현재 보유 포지션 기준으로 판단하세요. 추세 지속 여부를 확인하며 익절 타이밍을 가늠하는 구간입니다.';
    else if (entryScore >= 50) entrySummary = '일부 반등 강도가 있으나, 보유 중이라면 추가 매수보다는 현재 수익 보존에 집중하세요.';
    else if (entryScore >= 30) entrySummary = '반등 강도가 약해지고 있습니다. 추세 약화 가능성에 대비해 손절 기준을 타이트하게 조정하세요.';
    else entrySummary = '강세 모멘텀이 거의 없습니다. 보유 중이라면 매도 타이밍을 적극 검토하세요.';
  } else { // exit
    if (entryScore >= 70) entrySummary = '반등 강도가 남아 있지만, 전체 판정은 매도 방향입니다. 일시적 반등에 속지 말고 매도를 진행하세요.';
    else if (entryScore >= 50) entrySummary = '약한 반등 강도가 보이지만 전체적으로 하락 흐름입니다. 기술적 반등은 매도 기회로 활용하세요.';
    else if (entryScore >= 30) entrySummary = '반등 강도 약화 + 매도 판정입니다. 하락 추세가 지속될 가능성이 높으며, 포지션 정리를 서두르세요.';
    else entrySummary = '반등 강도 전무 + 매도 판정입니다. 즉각적인 포지션 정리가 필요합니다.';
  }

  // ── 추세(Trend) 섹션 ──
  // [v1.7] 라벨 "강세/양호/중립/약세/하락" → "강추세/중추세+/중추세/약추세/하락추세"
  //   sx_render.js 헤더 버튼(_btnTrendLevel, 2290줄)과 1:1 동기화
  //   이유: entryScore 점수명 "반등 신호"와 겹치지 않도록 "추세" 접미 명시
  // [v1.8] 라벨 매핑 재정의:
  //   ready 점수명 "바닥 신호" → "반등 신호" (눌림 후 반등 신호 포착 단계)
  //   entry 점수명 "반등 신호" → "반등 강도" (실제 반등 시작 후 모멘텀 강도)
  //   trend 점수명 "추세 강도" → "추세 방향" (시장 레짐 ADX와 의미 분리)
  var trendLevel = trendScore >= 70 ? '강추세' : trendScore >= 60 ? '중추세+' : trendScore >= 50 ? '중추세' : trendScore >= 40 ? '약추세' : '하락추세';
  var trendPosition = '';
  // [S403] trendScore=trendPure(방향 중심 종합 추세) 의미에 맞게 재작성.
  //   기존엔 trendScore 높음을 "단계(중반~후반)"로 단정해 "부분 익절"을 권했으나,
  //   trendPure는 방향 중심 종합 점수지 진행 단계가 아님 → 강한 추세를 "후반·익절"이라 하며 추가상승(여력) 카드와 충돌.
  //   해결: 강도로 표현하고 과열·익절 판단은 추가상승 신호(여력/과열)에 위임.
  if (_tone === 'entry') {
    if (trendScore >= 75) trendPosition = '강한 상승 추세 — 추세 방향·강도가 뚜렷합니다. 추격보다 눌림목 진입이 유리하며, 과열 여부는 추가 상승 신호를 함께 보세요.';
    else if (trendScore >= 60) trendPosition = '상승 추세 진행 — 방향이 확인되고 있으며 추세가 살아 있습니다.';
    else if (trendScore >= 50) trendPosition = '추세 모색 — 방향이 아직 정해지지 않았습니다. 돌파 또는 하락 어디든 갈 수 있습니다.';
    else trendPosition = '추세 미형성 — 하락 또는 횡보 구간입니다.';
  } else if (_tone === 'caution') {
    if (trendScore >= 75) trendPosition = '강한 상승 추세 — 추세 방향·강도가 견조하게 유지되고 있습니다. 과열·익절 시점은 추가 상승 신호(여력·과열)를 기준으로 판단하세요.';
    else if (trendScore >= 60) trendPosition = '상승 추세 진행 — 추세가 유지되고 있으나 약화 징후에 주의하세요.';
    ...
    if (trendScore >= 75) trendPosition = '추세 강도는 높으나 매도 판정과 결합된 상태 — 강한 추세에도 청산 신호가 우세하니 추격 매수는 피하고 보유 시 대응하세요.';
  ...
  // [S402] 추세 분해 — trendPure(qs.trendBreakdown) 기반으로 통일 (헤더 trendScore와 일치).
  //   기존 _breakdown(rawScore) 역분해는 RSI 역추세·변동성·구조 감점 등 trendPure가 제외한 항목을
  //   포함해 헤더(95)와 분해(55)가 충돌했음 → trendBreakdown parts(방향/기울기/MA배열/ADX/구조위치/SAR)로 교체.
  var trendMet = [], trendUnmet = [];
  var trendMetW = [], trendUnmetW = [];
  var _tbParts = qs.trendBreakdown || [];
  for (var ti = 0; ti < _tbParts.length; ti++) {
    var _tpItem = _tbParts[ti];
    if (_tpItem.w > 0) {
      trendMet.push(_tpItem.name);
      trendMetW.push({name: _tpItem.name, weight: '+' + _tpItem.w, maxW: null, group: 'trend'});
    } else if (_tpItem.w < 0) {
      trendUnmet.push(_tpItem.name);
      trendUnmetW.push({name: _tpItem.name, weight: _tpItem.w, maxW: null, group: 'trend'});
    }
  }

  // ── [S357] 추가상승(Upside) 섹션 — 순추세 추격 관점 ──
  //   scrUpsideScore 배점 + notes 문자열 매칭 (ready 패턴 준용)
  const _upsideWeightMap = [
    {name:'이평선 정배열', maxW:12, match:['이평선 정배열','단기>중기 상승배열']},
    {name:'골든크로스', maxW:12, match:['골든크로스']},
    {name:'가격>이평선', maxW:10, match:['가격 20일선 위','가격 60일선 위']},
    {name:'추세 강도(ADX)', maxW:11, match:['강한 상승추세','상승추세(ADX']},
    {name:'고점 갱신(HH)', maxW:10, match:['고점 갱신']},
    {name:'저항/신고가 돌파', maxW:14, match:['박스 상단','저항 돌파','신고가 돌파','고가권']},
    {name:'볼린저 밴드워킹', maxW:6, match:['볼린저 상단 밴드워킹']},
    {name:'OBV 수급', maxW:12, match:['OBV 상승']},
    {name:'거래량 동반', maxW:13, match:['거래량 정배열','돌파 거래량']},
    {name:'MACD 모멘텀', maxW:8, match:['MACD 양봉']},
    {name:'RSI 강세구간', maxW:6, match:['RSI 건강 상승구간','RSI 강세 지속']}
  ];
  var upsideMet = [], upsideUnmet = [];
  for (var uwi = 0; uwi < _upsideWeightMap.length; uwi++) {
    var uw = _upsideWeightMap[uwi];
    var ufound = false;
    for (var uuj = 0; uuj < upsideNotes.length; uuj++) {
      for (var uuk = 0; uuk < uw.match.length; uuk++) {
        if (upsideNotes[uuj].indexOf(uw.match[uuk]) >= 0) { ufound = true; break; }
      }
      if (ufound) break;
    }
    if (ufound) upsideMet.push(uw.name); else upsideUnmet.push(uw.name);
  }
  var _upsideWLookup = {};
  for (var uwl = 0; uwl < _upsideWeightMap.length; uwl++) _upsideWLookup[_upsideWeightMap[uwl].name] = _upsideWeightMap[uwl].maxW;
  var upsideMetW = upsideMet.map(function(c){ return {name: c, weight: _upsideWLookup[c] || 0}; });
  var upsideUnmetW = upsideUnmet.map(function(c){ return {name: c, weight: _upsideWLookup[c] || 0}; });
  // 경고성 notes 분리 (역배열·과열·수급이탈 등 — 추가상승 저해 요인)
  var upsideWarnings = [];
  for (var uwn = 0; uwn < upsideNotes.length; uwn++) {
    var un = upsideNotes[uwn];
    if (un.indexOf('역배열') >= 0 || un.indexOf('저점 하락') >= 0 || un.indexOf('과열') >= 0 ||
        un.indexOf('−DI') >= 0 || un.indexOf('수급 이탈') >= 0 || un.indexOf('음봉') >= 0 ||
        un.indexOf('RSI 약세') >= 0 || un.indexOf('자격 미달') >= 0 || un.indexOf('데드크로스') >= 0) {
      upsideWarnings.push(un);
    }
  }
  var upsideProgress = upsideMet.length + '개 신호 충족';
  var upsideLevel = upsideScore >= 70 ? '강함' : upsideScore >= 50 ? '보통' : upsideScore >= 30 ? '약함' : '미흡';
  var upsideSummary = '';
  if (_tone === 'entry') {
    if (upsideScore >= 70) upsideSummary = '추가 상승 동력이 강합니다. 추세 방향이 강할수록 정배열·돌파·수급이 겹친 추세 추격 매수가 유효합니다.';
    else if (upsideScore >= 50) upsideSummary = '추세 추종 신호가 일부 나타나지만 아직 확정적이지 않습니다. 추세 방향과 정배열 안착·돌파 확인을 함께 보세요.';
    else if (upsideScore >= 30) upsideSummary = '추가 상승 동력이 약합니다. 추세가 아직 형성되지 않았거나 조정 가능성이 있습니다.';
    else upsideSummary = '추가 상승 동력이 거의 없습니다. 순추세 추격에는 부적합한 구간입니다.';
  } else if (_tone === 'caution') {
    if (upsideScore >= 70) upsideSummary = '추가 상승 여력이 충분합니다. 추세 방향이 견조하면 보유 지속 또는 분할 매수를 고려할 수 있습니다.';
    else if (upsideScore >= 50) upsideSummary = '추세가 유지되고 있으나 동력이 둔화될 수 있습니다. 추세 방향과 함께 약화 징후를 주시하세요.';
    else if (upsideScore >= 30) upsideSummary = '추가 상승 동력이 약화되고 있습니다. 추세 둔화에 대비해 익절 기준을 점검하세요.';
    else upsideSummary = '추가 상승 동력이 소진되고 있습니다. 보유 중이라면 추세 이탈 여부를 확인하세요.';
  } else { // exit
    if (upsideScore >= 70) upsideSummary = '추세 신호는 남아 있지만 전체 판정은 매도 방향입니다. 추세 추격보다 포지션 관리를 우선하세요.';
    else if (upsideScore >= 50) upsideSummary = '추세 동력이 약화되며 매도 판정과 결합되고 있습니다. 추격 매수는 위험합니다.';
    else if (upsideScore >= 30) upsideSummary = '추가 상승 동력 약화 + 매도 판정입니다. 추세 이탈 가능성이 높습니다.';
    else upsideSummary = '추가 상승 동력 전무 + 매도 판정입니다. 추세가 꺾였을 가능성이 높습니다.';
  }
  ...
  // ── 데이터 지연 판정 (Staleness) ──
  // [S259] market 전달 — 시장별 종가 시점 정확 처리 (기본 'kr')
  var staleness = _estimateStaleness(lastCandleDate, readyScore, entryScore, trendScore, scoreMom, market);
  ...
  return {
    ...
    trend: {
      ...
      breakdown: { combined: trendScore, parts: (qs.trendBreakdown || []) } // [S402] trendPure 분해 (헤더와 일치)
    },
    upside: {
      score: upsideScore, level: upsideLevel, notes: upsideNotes,
      met: upsideMet, unmet: upsideUnmet, metW: upsideMetW, unmetW: upsideUnmetW,
      warnings: upsideWarnings, progress: upsideProgress, summary: upsideSummary
    },
...
function _estimateTransition(readyS, entryS, trendS, mom, btStats) {
  ...
  if (hasBtStats && btStats.r2e.attempts >= 2) {
    ...
    if (entryS >= 40) { r2eBase += 10; readyToEntry.basis.push('반등 강도 일부 감지 — 전이 진행 중'); }
  }
  ...
  if (hasBtStats && btStats.e2t.attempts >= 2) {
    ...
    if (entryS >= 70) { e2tBase += 10; entryToTrend.basis.push('현재 반등 강도 강함'); }
    else if (entryS < 40) { e2tBase -= 10; entryToTrend.basis.push('현재 반등 강도 약함'); }
    ...
    if (entryS >= 70) { e2tBase += 25; entryToTrend.basis.push('반등 강도 강함 — 반등 확인 단계'); }
    else if (entryS >= 50) { e2tBase += 12; entryToTrend.basis.push('반등 강도 보통 — 추가 확인 필요'); }
    else { entryToTrend.basis.push('반등 강도 부족'); }
 ...
 */
// [S259] market 옵셔널 인자 추가 — 시장별 일봉 종가 시점 정확 처리
//   〔이력〕 이전: YYYYMMDD 형식 무조건 'T15:30:00+09:00'(KST 한국장 종가)로 해석.
//     미국 일봉(EST 16:00 ≈ KST 익일 06:00)은 약 14.5시간, 코인(UTC 00:00 = KST 09:00)은 6.5시간
//     어긋남 → staleness label/decay 부정확 (영향 작음, 영문/코인 사용자 한정)
//   수정: market 옵셔널 인자 추가 (default 'kr', 한국 사용자 영향 0).
//     'us' → EST 16:00 (-05:00) 종가로 해석. 서머타임은 단순화하여 표준시 고정.
//     'coin' → UTC 00:00 종가로 해석.
//   호환: 기존 호출자(market 미전달) 동작 100% 동일. 호출자가 market 전달하면 정확도 향상.
function _estimateStaleness(lastDate, readyS, entryS, trendS, mom, market) {
  ...
  // [S259] 시장별 종가 시점 매핑 (YYYYMMDD 8자리 파싱 시에만 적용)
  var _mkt = (market || 'kr').toLowerCase();
  var _closeTime;
  if (_mkt === 'us') _closeTime = 'T16:00:00-05:00';       // 미국 정규장 종가 (EST, DST 무시)
  else if (_mkt === 'coin') _closeTime = 'T00:00:00Z';     // 코인 UTC 자정
  else {
    // [S575] 한국장 종가 시각을 sx_session.js SSOT에서 읽음 — 9/14 거래시간 확장 대비.
    //   현행 '1530' → 'T15:30:00+09:00' (동작 변화 0). _sxSession 미로드 시 '1530' 폴백.
    //   us/coin은 시간외 미정·자정 엣지케이스 회피 위해 기존 상수 유지.
    var _krClose = '1530';
    try { if (typeof _sxSession === 'function') { var _ks = _sxSession('kr'); if (_ks && _ks.regular && _ks.regular.close && /^\d{4}$/.test(_ks.regular.close)) _krClose = _ks.regular.close; } } catch (e) {}
    _closeTime = 'T' + _krClose.slice(0, 2) + ':' + _krClose.slice(2) + ':00+09:00';
  }
  ...
  try {
    ...
    if (d.length === 8) parsed = new Date(d.substring(0,4)+'-'+d.substring(4,6)+'-'+d.substring(6,8)+_closeTime);
...
function _buildVerdict(verdictAction, btStateObj, transition, staleness, btScore) {
  ...
  if (_va === '매수') {
    ...
    tone = 'cautious';
    lines.push('분석 지표가 약화되고 있습니다. 아직 매도 확정은 아니지만 부분 익절 또는 손절 기준 타이트하게 재조정을 검토하세요.');
    if (btStateObj && btStateObj.pnl != null) {
    ...
    tone = 'bearish';
    lines.push('분석 지표가 확연히 나빠지고 있습니다. 적극적으로 매도를 검토할 구간이며 손실 확대 위험이 있습니다.');
    if (btStateObj && btStateObj.pnl != null) {
      var _pq = btStateObj.pnl >= 0 ? '+' + btStateObj.pnl.toFixed(1) : btStateObj.pnl.toFixed(1);
      lines.push('현재 평가손익 ' + _pq + '%. 추가 하락 전에 포지션 축소 또는 전체 매도 판단이 필요합니다.');
    }
  } else if (_va === '즉시 청산') {
    tone = 'bearish';
    lines.push('지지선 이탈과 하락 가속이 감지됩니다. 지금 즉시 매도하는 것이 안전합니다.');
    if (btStateObj && btStateObj.pnl != null) {
    ...
    if (btStateObj && btStateObj.isWin != null) {
      ...
      if (btStateObj.exitPrice) {
        lines.push('매도가 ' + Math.round(btStateObj.exitPrice).toLocaleString() + (btStateObj.exitDate ? ' (' + btStateObj.exitDate + ')' : '') + '. 다음 진입 시점은 새 BT 신호를 기다리세요.');
      }
    } else {
      tone = 'neutral';
      lines.push('매도가 체결되었습니다. 포지션이 매도된 상태로, 다음 진입 시점까지 관망하세요.');
    }
  } else {
    // fallback
    tone = 'neutral';
