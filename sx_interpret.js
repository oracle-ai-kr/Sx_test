// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v5.3 <!-- S127 "청산 → 매도" UI 용어 치환 (verdictAction 값은 원본 유지). [배경] 사용자 관점에서 "청산"은 강제 종료/손절 뉘앙스가 강해 익절 상황에 위화감 발생 + 초보자에게 "매도"가 보편적. interpret.js UI 문구 전체에서 포지션 종료 행위 표현을 "매도"로 통일, 단 verdictAction 심볼('청산 준비'/'청산 검토'/'즉시 청산'/'매도 완료') 및 case '청산' 등 로직 키는 원본 그대로 보존(프로젝트 C 원칙 ② 단일성). [변경 4영역] (1) SXI.summary stateLine/actionGuide — 보유중 5종 분기(보유유지/청산준비/청산검토/즉시청산/매도완료)에서 "청산" 표현을 "매도"로 교체, "청산가/청산일" → "매도가/매도일". (2) SXI.practicalGuide — verdictAction 9종 가이드 중 청산준비/청산검토/즉시청산/매도완료 4종 본문의 "청산" UI 문구 치환. (3) SXI.threeStageReport — 3톤(entry/caution/exit) 분기 본문, ready/entry/trend 섹션 설명 텍스트 내 "청산" → "매도". (4) _buildVerdict 9종 매핑 본문 + 매도완료 분기의 "청산가/청산일" → "매도가/매도일". [불변] verdictAction 값 자체, _verdictMap 키, switch case 심볼, SXC 반환값 비교 로직 모두 '청산 준비'/'즉시 청산'/'매도 완료' 등 원본 유지. [효과] sx_render.js v7.7과 합쳐 상단 배너 ▼ 매도 + 본문 종합평/행동가이드 매도 톤 완전 정합. --> v5.2 <!-- S103-fix7 Phase3-B-3 SXI.summary C화(6번째 verdictAction 파라미터 추가, 보유중 5종(보유유지/청산준비/청산검토/즉시청산/매도완료)일 때 stateLine 보유자 맥락 재작성, actionGuide/buyTrigger 숨김 - 비보유자 관점 문구로 인한 사용자 혼란 제거) --> v5.1 <!-- S103-fix7 Phase3-B-1 practicalGuide C화(btAction 4종 → verdictAction 9종 직접수신, 신규 5종 가이드: 보유유지/청산준비/청산검토/즉시청산/매도완료) --> v5.0 <!-- S103-fix7 Phase3-A-3 _buildVerdict 감독관기반 전면재작성(점수stage 폐기, verdictAction 9종 직접매핑 → 배너와 완전정합) + threeStageReport 8th param btStateObj 추가 → 매도완료 익절/손절/수익률/진입가 본문 표시 --> v4.9 <!-- S103-fix7 Phase3-A-2 threeStageReport 톤 매핑 재조정("대기"폐지, "관망"→entry(비보유 관찰), "회피"→entry(진입안함), caution=보유중+아직청산X, exit=매도확정) --> v4.8 <!-- S99 -->
...
SXI.practicalGuide = function(verdictAction, analScore, btScore, summary, ind){
  ...
  if(verdictAction === '매수'){
    ...
    guide.crossCheck = '매일 장 마감 후 점수 변화와 감독관 판정을 확인하세요. "매도 준비" 또는 "매도 검토"로 바뀌면 그때 대응하면 됩니다.';
    ...
    guide.steps = [
      ...
      `감독관(분석 엔진)이 "매도 준비" 판정을 내렸습니다. 당장 모두 팔라는 뜻은 아니고, "대응 계획을 준비하라"는 의미입니다.`,
    ...
    guide.steps = [
      ...
      `감독관(분석 엔진)이 "매도 검토" 판정을 내렸습니다. "매도 준비"보다 한 단계 심각한 상태로, 방어적 행동이 필요합니다.`,
      `수익이 나있다면 즉시 일부 또는 전량 익절을 고려하세요. 손실이 나있다면 손절선 근처라면 규칙대로 정리하는 것이 추가 손실을 막는 길입니다.`,
    ];
    guide.nextAction = '감정을 배제하고 매매 계획을 실행하세요. 익절 목표가에 도달하지 않았더라도, 감독관 판정이 악화되면 선제적 매도가 현명할 수 있습니다.';
    ...
    guide.steps = [
      ...
      `감독관(분석 엔진)이 "즉시 매도" 판정을 내렸습니다. 이것은 BT와 분석 엔진이 모두 "지금 나가라"고 말하는 가장 강한 매도 신호입니다.`,
    ...
    guide.steps = [
      ...
      `감독관(분석 엔진)이 "매도 완료" 판정을 내렸습니다. BT 엔진이 매도를 확정한 상태로, 이 포지션은 더 이상 유효하지 않습니다.`,
...
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats, verdictAction, btStateObj) {
  ...
  if (_tone === 'entry') {
    ...
    if (entryScore >= 70) entrySummary = '강세 신호가 남아 있지만, 전체 판정은 매도 방향입니다. 일시적 반등에 속지 말고 매도를 진행하세요.';
