...
SXI.valuationJudge = function(fin, price, stock){
  ...
  // [S325] 목표주가용 Target PER — 시장평균보다 약 35% 높은 적정 멀티플 (낙관)
  //   배경: PER 기반 목표가 공식 — 목표 예상가 = 12M Fwd EPS × Target PER
  //         (사진 3 출처: 일반적인 증권사 목표가 산출 방식)
  //   Target PER 선정 근거:
  //     - KR 15배: KOSPI 적정~약간고평가 경계 (11배 × 1.36)
  //     - US 25배: S&P 500 적정~약간고평가 경계 (20배 × 1.25)
  //   12M Fwd EPS = TTM EPS × (1 + epsGrowth, ±30% 클램프)
  //     - 컨센서스 데이터 없으므로 epsGrowth로 추정 (보수적 캡)
  //     - epsGrowth 없으면 = TTM EPS 그대로 (성장 가정 없음, 단순 PER 상단)
  const targetPer = isUS ? 25 : 15;
  ...
  return {
    ...
    //   - targetPer: KR 15배 / US 25배 (시장평균의 약 1.35배, 적정 상단)
