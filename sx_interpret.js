...
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats, verdictAction, btStateObj) {
  ...
  if (_tone === 'entry') {
    if (readyScore >= 70) readySummary = '바닥 신호 조건이 충분히 익었습니다. 과매도·수축·신호가 다수 겹치고 있어 반등 가능성이 높은 구간입니다.';
    else if (readyScore >= 50) readySummary = '일부 조건이 충족되고 있지만 아직 완전한 바닥 확인은 되지 않았습니다. 추가 조건 충족을 기다리는 것이 안전합니다.';
    else if (readyScore >= 30) readySummary = '바닥 신호 상태가 미흡합니다. 과매도 신호가 약하거나 아직 하락 추세가 진행 중일 수 있습니다.';
    else readySummary = '현재 바닥 신호 조건이 거의 없습니다. 추가 하락 가능성을 염두에 두고 관망이 필요합니다.';
    ...
    if (readyScore >= 70) readySummary = '바닥 신호 조건은 충족되어 있지만, 전체 판정이 매도 방향입니다. 바닥 반등을 노리기보다 현재 포지션 정리를 우선하세요.';
    else if (readyScore >= 50) readySummary = '일부 바닥 신호가 있으나 전체적으로 매도 국면입니다. 기술적 반등에 속지 말고 포지션 축소를 검토하세요.';
    else if (readyScore >= 30) readySummary = '바닥 신호 조건 미흡 + 매도 판정이 겹치고 있습니다. 추가 하락 위험이 큰 구간으로, 손절 또는 포지션 정리를 고려하세요.';
  ...
  if (_tone === 'entry') {
    ...
    if (entryScore >= 70) entrySummary = '반등 신호가 강하지만, 현재 보유 포지션 기준으로 판단하세요. 추세 지속 여부를 확인하며 익절 타이밍을 가늠하는 구간입니다.';
    else if (entryScore >= 50) entrySummary = '일부 반등 신호가 있으나, 보유 중이라면 추가 매수보다는 현재 수익 보존에 집중하세요.';
    else if (entryScore >= 30) entrySummary = '반등 신호가 약해지고 있습니다. 추세 약화 가능성에 대비해 손절 기준을 타이트하게 조정하세요.';
    else entrySummary = '강세 모멘텀이 거의 없습니다. 보유 중이라면 매도 타이밍을 적극 검토하세요.';
  } else { // exit
    if (entryScore >= 70) entrySummary = '반등 신호가 남아 있지만, 전체 판정은 매도 방향입니다. 일시적 반등에 속지 말고 매도를 진행하세요.';
...
function _estimateTransition(readyS, entryS, trendS, mom, btStats) {
  ...
  if (hasBtStats && btStats.r2e.attempts >= 2) {
    ...
    if (entryS >= 40) { r2eBase += 10; readyToEntry.basis.push('반등 신호 일부 감지 — 전이 진행 중'); }
  }
  ...
  if (hasBtStats && btStats.e2t.attempts >= 2) {
    ...
    if (entryS >= 70) { e2tBase += 10; entryToTrend.basis.push('현재 반등 신호 강함'); }
    else if (entryS < 40) { e2tBase -= 10; entryToTrend.basis.push('현재 반등 신호 약함'); }
    ...
    if (entryS >= 70) { e2tBase += 25; entryToTrend.basis.push('반등 신호 강함 — 반등 확인 단계'); }
    else if (entryS >= 50) { e2tBase += 12; entryToTrend.basis.push('반등 신호 보통 — 추가 확인 필요'); }
    else { entryToTrend.basis.push('반등 신호 부족'); }
