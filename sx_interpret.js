// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v4.1 <!-- S81 -->
...
SXI.correlationGuide = function(corrData) {
  ...
  const clusterTexts = clusters.map(cl => {
    ...
    if (cl.avgCorr >= 0.85) return `⚠️ ${names} — 극단적 동조(${avgR}%). 같은 뉴스에 같은 방향으로 급등/급락할 위험이 큽니다.`;
    if (cl.avgCorr >= 0.7) return `🔗 ${names} — 고상관(${avgR}%). 유사한 패턴으로 움직이므로 동시 보유 시 위험이 집중됩니다.`;
    return `📊 ${names} — 상관(${avgR}%).`;
  });
  ...
  // 제안
  const suggestionTexts = suggestions.map(s => `💡 ${s}`);
...
SXI.sectorRadarGuide = function(radarData) {
  ...
  const watchlistTexts = watchlistMatch.map(w => {
    ...
    const status = w.isBull ? '🟢 강세권' : '🔴 약세권';
    return `${w.name} → ${w.sector} ${status} (${sign}${w.sectorChange.toFixed(2)}%, ${w.sectorRank}/${w.totalSectors}위)`;
  });

  // 자금 쏠림 해석
  let capitalFlowText = '';
  if (capitalFlow === 'concentrated') capitalFlowText = '🔥 자금이 소수 섹터에 집중되고 있습니다. 쏠림 장세에 유의하세요.';
  else if (capitalFlow === 'moderate') capitalFlowText = '📊 특정 섹터로 자금 유입이 감지됩니다.';
 ...
 */
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate) {
  if (!qs) return null;
  ...
  const readyConditions = [
    'RSI 과매도','BB 하단','BB 스퀴즈','Stoch 과매도','CCI 과매도',
    '거래량 극감','RSI 다이버전스','OBV 다이버전스','지지선 근접',
    '심리 가격대','하락 충분도','구름 하단'
  ];
  var readyMet = [], readyUnmet = [];
  for (var ri = 0; ri < readyConditions.length; ri++) {
    var rc = readyConditions[ri];
    var found = false;
    for (var rj = 0; rj < readyNotes.length; rj++) {
      if (readyNotes[rj].indexOf(rc.replace(/ /g,'').substring(0,3)) >= 0 ||
          readyNotes[rj].indexOf(rc.substring(0,2)) >= 0) { found = true; break; }
    }
    if (found) readyMet.push(rc); else readyUnmet.push(rc);
  }
  ...
  var transition = _estimateTransition(readyScore, entryScore, trendScore, scoreMom);
  ...
  return {
    ready: {
      ...
      met: readyMet, unmet: readyUnmet, progress: readyProgress, summary: readySummary
    },
    entry: {
...
/**
 * 전이 확률 추정 — 엔진 불변, 점수+모멘텀으로 통계적 추정
 * Ready→Entry 전이: Ready 높고 모멘텀 상승이면 확률 높음
 * Entry→Trend 전이: Entry 높고 추세 점수 상승 중이면 확률 높음
 */
function _estimateTransition(readyS, entryS, trendS, mom) {
  var readyToEntry = { prob: 0, days: '—', basis: [] };
  var entryToTrend = { prob: 0, days: '—', basis: [] };
  ...
  else { readyToEntry.basis.push('준비 점수 부족 — 아직 조건 미성숙'); }
...
function _buildVerdict(readyS, entryS, trendS, transition, staleness, btScore) {
  ...
  if (stage === 'confirmed') {
    ...
    lines.push('Entry→Trend 전이 확률 ' + transition.entryToTrend.prob + '% — ' + transition.entryToTrend.days + ' 이내 추세 확인 예상.');
    ...
    lines.push('Ready→Entry 전이 확률 ' + transition.readyToEntry.prob + '% — ' + transition.readyToEntry.days + ' 이내 반등 신호 예상.');
  } else if (stage === 'late') {
    emoji = '⚠️'; label = '과열 주의';
    tone = 'bearish';
  ...
  return { emoji: emoji, label: label, tone: tone, text: lines.join('\n'), lines: lines, stage: stage };
}
