...
SXI.advMaConv = function(converging, spread, maArrange, bbSqueeze){
  ...
  if(converging && +spread<=2) return {tone:'neutral',label:'강한 수렴 (변곡점 임박)',text:`MA5/20/60 이격 ${sp}%. 이동평균선이 거의 한 점으로 수렴하고 있어 강력한 변곡점이 임박했습니다.${extraStr} 교과서에서는 이 수준의 수렴을 큰 추세의 시작점으로 보고, 볼린저 밴드 스퀴즈와 겹치면 돌파 시 폭발적 움직임을 예상합니다. 다만 측정은 반대쪽을 가리킵니다 — MA 간격이 가장 좁은 구간은 뒤이은 10봉 변동성이 오히려 **낮았습니다**(3시장 공통). ⚠단 **거친 장에서만 뚜렷합니다** — 조용한 장에서는 차이가 거의 없었고(비율 0.91~0.97), 거친 장에서 두드러졌습니다(0.62~0.64). 절대 변동성은 장이 거칠어지면 수렴 구간도 함께 커집니다. 방향을 예단하기보다, 돌파 방향(상방: MA5가 MA60 상향 돌파 + 양봉 + 거래량 증가 / 하방: MA5가 MA60 하향 이탈 + 음봉)을 확인한 후 따라가는 전략이 안전합니다. 거짓 돌파에 대비해 첫 진입은 소액으로 하고, 방향이 확인되면 추가 진입하세요.`};
  if(converging) return {tone:'neutral',label:'수렴 중',text:`MA5/20/60 이격 ${sp}%. 이동평균선이 모이고 있으며 방향 전환 또는 추세 재개의 전조입니다.${extraStr} 수렴이 더 진행되면 변곡점 임박 단계로 넘어갑니다. ⚠측정에서는 좁은 간격이 뒤이은 변동성을 **낮추는** 쪽으로 나왔습니다(3시장 공통) — '폭발 대기'로 읽지 마세요. 단 그 차이는 **거친 장에서만 뚜렷**했습니다. 현재는 방향이 정해지지 않은 관망 구간이며, 수렴 해소(이평선이 다시 벌어지기 시작) 방향이 핵심입니다. 수렴 중 거래량이 줄어드는 것은 정상이며, 거래량 급증과 함께 이평선이 벌어지면 새 추세의 시작입니다.`};
