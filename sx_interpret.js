// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v4.5 <!-- S85 -->
...
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats) {
  ...
  if (_bd && _ind) {
    ...
    var _trendItems = [
      ...
      {name:'변동성', val: (_ind.atr && _ind.atr.pct != null) ? _clamp(Math.tanh((1.6 - _ind.atr.pct) * 0.9) * 18, -18, 18) : 0, maxW:18, group:'signal'},
    ...
    // ── sub 그룹 (보조지표, combined에 ×0.5 적용) — val에 ×0.5 반영하여 실제 기여 표시 ──
    _trendItems.push({name:'Stochastic', val: ((_ind.stoch && (_ind.stoch.k < 20 || _ind.stoch.k > 80)) ? (_ind.stoch.k < 20 ? 3 : -3) : 0) * 0.5, maxW:1.5, group:'sub'});
    _trendItems.push({name:'CCI', val: ((_ind.cci ?? 0) > 100 ? -3 : (_ind.cci ?? 0) < -100 ? 3 : 0) * 0.5, maxW:1.5, group:'sub'});
    _trendItems.push({name:'OBV 신호', val: ((_ind.obv.div==='bullish'?7:_ind.obv.div==='bearish'?-7:0) + (_ind.obv.val > _ind.obv.sig ? 5 : _ind.obv.val < _ind.obv.sig ? -5 : 0)) * 0.5, maxW:6, group:'sub'});
    _trendItems.push({name:'MA 정렬', val: (_ind.maAlign.bullish ? 5 : _ind.maAlign.bearish ? -5 : 0) * 0.5, maxW:2.5, group:'sub'});
    _trendItems.push({name:'BB 신호', val: ((_ind.bb && _ind.bb.price < _ind.bb.lower) ? 3 : (_ind.bb && _ind.bb.price > _ind.bb.upper) ? -3 : 0) * 0.5, maxW:1.5, group:'sub'});
...
function _estimateTransition(readyS, entryS, trendS, mom, btStats) {
  ...
  if (hasBtStats && btStats.r2e.attempts >= 2) {
    ...
    // 현재 점수 상태로 보정 (±15 범위)
    if (readyS >= 70) { r2eBase += 10; readyToEntry.basis.push('현재 준비 점수 높음 — 전이 유리'); }
    else if (readyS < 40) { r2eBase -= 10; readyToEntry.basis.push('현재 준비 점수 낮음 — 전이 불리'); }
    ...
    if (readyS >= 70) { r2eBase += 25; readyToEntry.basis.push('준비 점수 70+ — 반등 조건 충분'); }
    else if (readyS >= 50) { r2eBase += 12; readyToEntry.basis.push('준비 점수 50+ — 조건 축적 중'); }
    else { readyToEntry.basis.push('준비 점수 부족 — 조건 미성숙'); }
    ...
    if (entryS >= 40) { r2eBase += 10; readyToEntry.basis.push('진입 신호 일부 감지 — 전이 진행 중'); }
  }
  ...
  if (hasBtStats && btStats.e2t.attempts >= 2) {
    ...
    if (entryS >= 70) { e2tBase += 10; entryToTrend.basis.push('현재 진입 신호 강함'); }
    else if (entryS < 40) { e2tBase -= 10; entryToTrend.basis.push('현재 진입 신호 약함'); }
    ...
    if (entryS >= 70) { e2tBase += 25; entryToTrend.basis.push('진입 신호 강함 — 반등 확인 단계'); }
    else if (entryS >= 50) { e2tBase += 12; entryToTrend.basis.push('진입 신호 보통 — 추가 확인 필요'); }
    else { entryToTrend.basis.push('진입 신호 부족'); }
...
function _buildVerdict(readyS, entryS, trendS, transition, staleness, btScore) {
  ...
  if (stage === 'confirmed') {
    label = '보유 확인';
    tone = 'bullish';
    lines.push('준비→진입→추세 순서로 조건이 확인되었습니다. 반등이 이미 진행 중이며 추세가 형성된 구간입니다.');
    ...
    label = '진입 검토';
    tone = 'cautious';
    lines.push('반등 준비가 익은 상태에서 실제 반등 신호가 감지되고 있습니다. 진입을 적극 검토할 수 있는 구간입니다.');
    lines.push('진입→추세 전이 확률 ' + transition.entryToTrend.prob + '% — ' + transition.entryToTrend.days + ' 이내 추세 확인 예상.');
    if (btScore != null && btScore >= 60) lines.push('과거 검증도 양호합니다(BT ' + btScore + '점).');
  } else if (stage === 'ready') {
    label = '관심';
    tone = 'neutral';
    lines.push('반등 조건이 익어가고 있지만 아직 실제 반등 신호는 약합니다. 관심 종목으로 등록하고 진입 신호를 기다리세요.');
    lines.push('준비→진입 전이 확률 ' + transition.readyToEntry.prob + '% — ' + transition.readyToEntry.days + ' 이내 반등 신호 예상.');
  } else if (stage === 'late') {
    label = '과열 주의';
    tone = 'bearish';
    lines.push('추세 점수는 높지만 준비/진입 단계를 거치지 않고 올라왔습니다. 이미 많이 오른 상태일 수 있으며 고점 추격 위험이 있습니다.');
    lines.push('신규 진입보다는 눌림목을 기다리거나 관망하는 것이 안전합니다.');
  } else {
    label = '관망';
    tone = 'neutral';
    lines.push('현재 반등 준비도, 진입 신호, 추세 모두 약한 상태입니다. 조건이 충족될 때까지 관망하세요.');
  }
