...
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats, verdictAction, btStateObj, market) {
  ...
  var upsideSummary = '';
  if (_tone === 'entry') {
    if (upsideScore >= 70) upsideSummary = '추가 상승 동력이 강합니다. 정배열·돌파·수급이 겹쳐 추세 추격 매수가 유효한 구간입니다.';
    else if (upsideScore >= 50) upsideSummary = '추세 추종 신호가 일부 나타나지만 아직 확정적이지 않습니다. 정배열 안착이나 돌파 확인을 기다리세요.';
    ...
    if (upsideScore >= 70) upsideSummary = '추가 상승 여력이 충분합니다. 보유 지속 또는 추세 추종 관점의 분할 매수를 고려할 수 있습니다.';
    else if (upsideScore >= 50) upsideSummary = '추세가 유지되고 있으나 동력이 둔화될 수 있습니다. 보유는 유지하되 추세 약화 징후를 주시하세요.';
