...
SXI.renderDisclosureCard = function(discItp){
  ...
  // [S57-compact v2] 카드 컴팩트화 — 위아래 폭 약 30~35% 축소
  //   1) anal-section 외곽 padding 12 → 8 (기본 클래스값 인라인 오버라이드)
  //   2) section-title margin-bottom 8 → 3
  //   3) badges 영역 margin-bottom 5 → 3
  //   4) itp-card 내부 padding 7px 9px → 5px 8px
  //   5) itp-label(중복 소제목) 제거 — anal-section-title과 동일 텍스트라 중복
  //   6) summary 구분선 margin-top 4 → 3, padding-top 3 → 2
  //   7) line-height 1.45 유지 (가독성)
  return `<div class="anal-section" style="margin-bottom:6px;padding:8px">
    <div class="anal-section-title" style="margin-bottom:3px">${discItp.label}</div>
    <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:3px">${badgeHTML}</div>
    <div class="itp-card show" style="white-space:pre-line;padding:5px 8px;line-height:1.45">
      ${bodyHTML}
      <div style="margin-top:3px;padding-top:2px;border-top:1px solid var(--border);font-size:10px;color:var(--text2)">${discItp.summary}</div>
    </div>
  </div>`;
};
