...
//  ⚠[S1337] 이 함수는 `isSqueeze`를 **먼저** 보지만 레짐 서술(`SXI.advRegime`)은 **보지 않는다** — 같은 봉을 두고
//    두 자리가 '극도로 좁다'/'매우 넓다'를 동시에 말한다(실측 kr 22.7%·us 19.8%·coin 44.9%). 어느 쪽이 맞는지는
//    미정(PREREG_S1337_C1 H2 1/3) · **각인만**이고 문구·문턱 무변경. 실측·근거는 `advRegime`의 S1337 각인에 있다.
//    ⚠부수 실측: `보통`(스퀴즈 아님 ∧ 폭≤5) 분기는 **0.05~0.18%** 로 사실상 죽어 있다.
...
SXI.advRegime = function(regime){
  ...
  //  ══════ [S1337] ★★이 4분기는 **포화**이고 바로 아래 BB폭 행과 **모순**이다 — 원인: **(isSqueeze 미참조)** ══════
  //   실측(3시장 134k봉 · 07-01·600봉 정합 스냅 · 엔진 `BollingerBands.calc` 직접 호출 · 재구현 아님):
  //     `bb.width`는 **이미 % 단위**((upper−lower)/mid×100 · engine L609)이고 중위 **kr 16.5 / us 12.9 / coin 13.8**.
  //     ⇒ 아래 4분기는 `>=5` 하나에 **94.8~96.7%** 가 몰린다. `1.5~3`은 0.47~0.84% · `<1.5`(스퀴즈 서술)는
  //       **0.02~0.27%** 로 사실상 죽었다. 문턱 1.5/3/5는 이 풀의 실제 폭보다 한참 낮다.
  //   ★★같은 값을 쓰는 **BB폭 행(`SXI.bbWidth` L238)은 `isSqueeze`를 먼저 보는데 여기는 보지 않는다.**
  //     그래서 한 화면이 **같은 봉**을 두고 *'극도로 좁아진 스퀴즈'*(행)와 *'매우 넓어 급등·급락이 빈번'*(여기)을
  //     **동시에** 말한다. 실측 동시발생 kr **22.7%** · us 19.8% · coin **44.9%**.
  //     ⇒ 모순의 원인은 문턱이 아니라 **이 블록이 상대 감지기를 참조하지 않는 것**이다
  //       (`BollingerBands.squeeze` = 최근 3봉 폭 < 자기 평균의 0.7배 · engine L613 · 발동률 kr 25.9/us 24.8/coin 49.5%).
  //   ⚠**어느 쪽이 맞는지는 못 정했다.** PREREG_S1337_C1 H2(모순군 `isSqueeze∧bbW>5` 의 fwd10 sd비 ≤0.87)가
  //     **1/3**(kr .768 통과 · us .970 · coin .936)으로 사전등록이 요구한 2시장을 못 채웠다. H3 종목쏠림 가드도 0/3
  //     (W 68.1/64.5/51.4%). ⇒ **문턱 완화 0 · 화면 문구 수정 0**(사전등록 §5가 문구 수정을 H2 통과 조건으로 걸었다).
  //   ⚠문턱 교체는 **발동 조건**이라 별도 사전등록 대상(S1282). 새 식을 만들지 말 것 — 후보는 이미 코드에 있는
  //     `BollingerBands.squeeze` 하나다(재구현 금지).
  //   ★H4 **3/3 통과** — 이 포화가 **판정을 실질적으로 움직이지는 않는다**: bbW 분기 제거 가정 시 레짐 라벨 변동
  //     **상한 0.49~0.98%**(3시장 전부 ≤1.0%). 급하지 않다는 근거이지 정합하다는 근거는 아니다.
  //   ⚠내 예상이 틀린 것: H1(절대 문턱이 종목 특성에 지배된다) **1/3 미달**(종목간 분산 비중 .128/.370/.110).
  //     변동은 대부분 **종목 내 시간 변화**였다 — 포화 원인은 종목 간 차이가 아니라 **문턱이 낮다**는 것 하나다.
  //   알갱이: `S1330~S1336/bbw_samples_S1337_{kr,us,coin}.json.gz` (code·date·bbWidth·isSqueeze·fwd10 **원값**)
...
SXI.failureAnalysis = function(btData, indicators, qs, tf){
  ...
  if(longHold.length > 0 && longHold.length >= lossCount * 0.3){
    ...
    lossPatterns.push({
      ...
      //  ⚠[S1369] `TP배수` 설정이 철거돼 이 조언이 **고아**가 됐다 — 없는 설정을 조정하라고 말하게 된다.
      //    청산은 이중ATR(트레일 고점−3×ATR + MA5×20 데드크로스)이라 조정 지점이 다르다. 지우지 않고 그 규칙으로 옮긴다(S1283).
      suggestion: '청산은 이중ATR(고점 − 3×ATR 트레일 + MA5×20 데드크로스)로 나옵니다. 이 패턴은 트레일이 따라 올라가기 전에 추세가 꺾인 경우이므로, 조정 지점은 보유 기간 상한이나 데드크로스 유예입니다. 이 조합은 측정된 적이 없습니다.'
    });
  }
  ...
  if(pf > 0 && pf < 1.0){
    ...
    improvements.push({
      area: '손익비',
      priority: 'mid',
      //  ⚠[S1369] '목표가를 높이거나' — 고정 목표가가 철거됐다(청산=이중ATR·이익 목표 없음). 조정 지점으로 교체.
      text: `손익비(PF) ${pf}은 수수료와 슬리피지를 감안하면 실질 수익이 거의 없는 수준입니다. 청산은 이중ATR이라 고정 목표가가 없으므로 조정 지점은 트레일 폭(고점 − 3×ATR)이나 초기 손절(진입가 − 2×ATR)입니다. 교과서에서는 손절을 타이트하게 가져가라고 봅니다 — 이 조합은 측정된 적이 없습니다.`
    });
  }
