...
SXI.unifiedNarrative = function(verdict, scores, scores4, ind, stock){
  ...
  if(conflictType === 'sell_with_good_score'){
    ...
    if(action === '즉시 청산' || action === '매도 완료'){
      ...
      guide = '아직 매도 확정 단계는 아닙니다. 점수가 양호하므로 즉시 매도보다는, 다음 봉에서 반등 전환이 회복되는지 확인 후 결정해도 늦지 않습니다.';
...
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats, verdictAction, btStateObj, market) {
  ...
  if (_tone === 'entry') {
    if (readyScore >= 70) readySummary = '반등 준비 조건이 충분히 익었습니다. 과매도·수축·눌림이 다수 겹치고 있어 반등 가능성이 높은 구간입니다.';
    else if (readyScore >= 50) readySummary = '일부 조건이 충족되고 있지만 아직 반등 준비가 완전히 갖춰지지 않았습니다. 추가 조건 충족을 기다리는 것이 안전합니다.';
    else if (readyScore >= 30) readySummary = '반등 준비 상태가 미흡합니다. 과매도 신호가 약하거나 아직 하락 추세가 진행 중일 수 있습니다.';
    else readySummary = '현재 반등 준비 조건이 거의 없습니다. 추가 하락 가능성을 염두에 두고 관망이 필요합니다.';
  } else if (_tone === 'caution') {
    if (readyScore >= 70) readySummary = '진입타이밍 조건은 충족되어 있지만, 현재 보유 중인 포지션의 관리가 우선입니다. 추가 매수보다는 목표가·손절가 점검에 집중하세요.';
    else if (readyScore >= 50) readySummary = '부분적 반등 준비가 있지만, 현재 국면에서는 신규 진입보다 기존 포지션 관리에 집중하는 것이 적절합니다.';
    else if (readyScore >= 30) readySummary = '진입 준비 수준이 낮습니다. 보유 중이라면 손절 기준을 재점검하고, 추가 매수는 자제하세요.';
    else readySummary = '반등 준비가 거의 없는 상태입니다. 보유 포지션의 리스크 관리를 강화하세요.';
  } else { // exit
    if (readyScore >= 70) readySummary = '반등 준비 조건은 충족되어 있지만, 전체 판정이 매도 방향입니다. 기술적 반등을 노리기보다 현재 포지션 정리를 우선하세요.';
    else if (readyScore >= 50) readySummary = '일부 반등 준비가 있으나 전체적으로 매도 국면입니다. 기술적 반등에 속지 말고 포지션 축소를 검토하세요.';
    else if (readyScore >= 30) readySummary = '반등 준비 조건 미흡 + 매도 판정이 겹치고 있습니다. 추가 하락 위험이 큰 구간으로, 손절 또는 포지션 정리를 고려하세요.';
    else readySummary = '반등 준비 없음 + 매도 판정입니다. 하락 추세가 진행 중이며, 가능한 빨리 포지션을 정리하는 것이 안전합니다.';
  }
  ...
  // S99-2: 3톤 분기 — 강세 해석
  if (_tone === 'entry') {
    if (entryScore >= 70) entrySummary = '복수의 반등 전환 요소가 동시에 감지되고 있습니다. RSI 반등, MACD 전환, 강세 캔들 등이 겹치며 실제 반등이 진행 중일 가능성이 높습니다.';
    else if (entryScore >= 50) entrySummary = '일부 반등 전환이 나타나고 있지만 아직 확정적이지 않습니다. 추가 확인 신호를 기다리는 것이 좋습니다.';
    else if (entryScore >= 30) entrySummary = '반등 전환이 약합니다. 간헐적 양봉이 보이지만 추세 전환을 확신하기 어려운 상태입니다.';
    else entrySummary = '반등 전환이 감지되지 않습니다. 아직 하락 또는 횡보가 지속되고 있을 가능성이 높습니다.';
  } else if (_tone === 'caution') {
    if (entryScore >= 70) entrySummary = '반등 전환이 강하지만, 현재 보유 포지션 기준으로 판단하세요. 추세 지속 여부를 확인하며 익절 타이밍을 가늠하는 구간입니다.';
    else if (entryScore >= 50) entrySummary = '일부 반등 전환이 있으나, 보유 중이라면 추가 매수보다는 현재 수익 보존에 집중하세요.';
    else if (entryScore >= 30) entrySummary = '반등 전환이 약해지고 있습니다. 추세 약화 가능성에 대비해 손절 기준을 타이트하게 조정하세요.';
    else entrySummary = '강세 모멘텀이 거의 없습니다. 보유 중이라면 매도 타이밍을 적극 검토하세요.';
  } else { // exit
    if (entryScore >= 70) entrySummary = '반등 전환이 남아 있지만, 전체 판정은 매도 방향입니다. 일시적 반등에 속지 말고 매도를 진행하세요.';
    else if (entryScore >= 50) entrySummary = '약한 반등 전환이 보이지만 전체적으로 하락 흐름입니다. 기술적 반등은 매도 기회로 활용하세요.';
    else if (entryScore >= 30) entrySummary = '반등 전환 약화 + 매도 판정입니다. 하락 추세가 지속될 가능성이 높으며, 포지션 정리를 서두르세요.';
    else entrySummary = '반등 전환 전무 + 매도 판정입니다. 즉각적인 포지션 정리가 필요합니다.';
  }
...
function _estimateTransition(readyS, entryS, trendS, mom, btStats) {
  ...
  if (hasBtStats && btStats.r2e.attempts >= 2) {
    ...
    if (entryS >= 40) { r2eBase += 10; readyToEntry.basis.push('반등 전환 일부 감지 — 전이 진행 중'); }
  }
  ...
  if (hasBtStats && btStats.e2t.attempts >= 2) {
    ...
    if (entryS >= 70) { e2tBase += 10; entryToTrend.basis.push('현재 반등 전환 강함'); }
    else if (entryS < 40) { e2tBase -= 10; entryToTrend.basis.push('현재 반등 전환 약함'); }
    ...
    if (entryS >= 70) { e2tBase += 25; entryToTrend.basis.push('반등 전환 강함 — 반등 확인 단계'); }
    else if (entryS >= 50) { e2tBase += 12; entryToTrend.basis.push('반등 전환 보통 — 추가 확인 필요'); }
    else { entryToTrend.basis.push('반등 전환 부족'); }
