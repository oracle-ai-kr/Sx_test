// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v4.6 <!-- S86 -->
//  S86: sub signal정밀화(엔진로직맞춤+VolumeOSC/ADX추가) + ATR.soften정밀화(volSoft참조) + 라벨변경(진입타이밍/강세/추세) + 전이텍스트일관성
...
SXI.practicalGuide = function(btAction, analScore, btScore, summary, ind){
  const s = analScore || 0;
  const b = btScore || 0;
  const sm = summary || {};
  ...
  if(btAction === '진입 적기'){
    ...
    guide.steps = [
      ...
      `두 점수가 모두 60점 이상이므로, "지금 기술적으로 좋고 + 과거에도 이런 상황에서 수익이 났다"는 의미입니다.`,
    ];
    if(safetyCount > 0){
      guide.steps.push(`다만 안전필터에서 ${safetyCount}건의 주의 사항이 감지되었습니다. 한 번에 전 자금을 투입하지 말고, 2~3회에 나눠서 분할 매수하는 것을 권합니다.`);
    }
    guide.nextAction = '분할 매수를 시작할 수 있는 구간입니다. 전체 투자금의 30~50%로 1차 진입 후, 상승 확인 시 추가 매수하세요.';

    if(s >= 75 && b >= 70){
      guide.nextAction = '강한 매수 조건입니다. 1차 비중을 50%까지 가져갈 수 있으며, 상승 추세 확인 시 나머지를 추가 매수하세요.';
    }

    guide.caution = '손절 기준을 반드시 정하세요. 매수가 대비 -3~5% 하락 시 손절하는 것이 일반적입니다. 손절 없이 버티면 큰 손실로 이어질 수 있습니다.';
    guide.crossCheck = '같은 업종의 다른 종목도 함께 확인해보세요. 업종 전체가 상승 중이면 신뢰도가 높고, 이 종목만 오르고 있다면 신중하게 접근하세요.';

  } else if(btAction === '단기급등 주의'){
    ...
    guide.steps = [
      ...
      `쉽게 말해 "좋은 종목인데 지금은 싼 자리가 아니다"라는 뜻입니다. 관심 종목에 등록해두고, 진입타이밍 점수가 60점 이상으로 올라올 때 다시 확인하세요.`,
    ];
    ...
    guide.nextAction = '이 종목은 넘기고 다른 종목을 찾아보세요. 결과 탭에서 "진입 적기" 판정을 받은 종목을 우선 검토하는 것이 효율적입니다.';
 ...
 */
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats) {
  if (!qs) return null;
  ...
  var trendPosition = '';
  if (trendScore >= 75) trendPosition = '추세 중반~후반 — 이미 상당히 올라왔을 가능성이 있습니다. 신규 진입보다는 보유 확인 구간입니다.';
  ...
  if (_bd && _ind) {
    ...
    // ── sub 그룹 (보조지표, combined에 ×0.5 적용) — 엔진 signal 로직 정밀 재현 ──
    // S86: Stochastic — 엔진: k<20&&k>d→buy(3), k<30&&k>d→buy(3), k>80&&k<d→sell(3), k>70&&k<d→sell(3)
    var _stochSig = 0;
    if (_ind.stoch) { var _sk=_ind.stoch.k, _sd=_ind.stoch.d; if(_sk<20&&_sk>_sd) _stochSig=3; else if(_sk<30&&_sk>_sd) _stochSig=3; else if(_sk>80&&_sk<_sd) _stochSig=-3; else if(_sk>70&&_sk<_sd) _stochSig=-3; }
    _trendItems.push({name:'Stochastic', val: _stochSig * 0.5, maxW:1.5, group:'sub'});
    // S86: CCI — 엔진: <=-200→buy(3), <=-100→buy(3), >=200→sell(3), >=100→sell(3)
    var _cciSig = 0, _cciV = _ind.cci ?? 0;
    if(_cciV<=-200) _cciSig=3; else if(_cciV<=-100) _cciSig=3; else if(_cciV>=200) _cciSig=-3; else if(_cciV>=100) _cciSig=-3;
    _trendItems.push({name:'CCI', val: _cciSig * 0.5, maxW:1.5, group:'sub'});
    // S86: OBV — 엔진: div(±7) + signal(val>sig*1.05→buy(5), val<sig*0.95→sell(5))
    var _obvSig = (_ind.obv.div==='bullish'?7:_ind.obv.div==='bearish'?-7:0);
    if(_ind.obv.val > _ind.obv.sig * 1.05) _obvSig += 5; else if(_ind.obv.val < _ind.obv.sig * 0.95) _obvSig -= 5;
    _trendItems.push({name:'OBV 신호', val: _obvSig * 0.5, maxW:6, group:'sub'});
    // S86: VolumeOSC — 엔진: val>50→buy(2), val<-30→sell(2) ← S85 누락 추가
    var _volOscSig = 0;
    if(_ind.volOsc != null) { if(_ind.volOsc > 50) _volOscSig=2; else if(_ind.volOsc < -30) _volOscSig=-2; }
    _trendItems.push({name:'거래량 오실레이터', val: _volOscSig * 0.5, maxW:1, group:'sub'});
    // MA 정렬 — 엔진 동일 (bullish→buy(5), bearish→sell(5))
    _trendItems.push({name:'MA 정렬', val: (_ind.maAlign.bullish ? 5 : _ind.maAlign.bearish ? -5 : 0) * 0.5, maxW:2.5, group:'sub'});
    // S86: BB 신호 — 엔진: price<=lower→buy(3), price<=lower+20%→buy(3), price>=upper→sell(3), price>=upper-20%→sell(3)
    var _bbSig = 0;
    if(_ind.bb) { var _bbP=_ind.bb.price||_ind.bb.last, _bbL=_ind.bb.lower, _bbU=_ind.bb.upper, _bbM=_ind.bb.middle; if(_bbP<=_bbL) _bbSig=3; else if(_bbP<=_bbL+(_bbM-_bbL)*0.2) _bbSig=3; else if(_bbP>=_bbU) _bbSig=-3; else if(_bbP>=_bbU-(_bbU-_bbM)*0.2) _bbSig=-3; }
    _trendItems.push({name:'BB 신호', val: _bbSig * 0.5, maxW:1.5, group:'sub'});
    // S86: ADX — 엔진: adx>25&&pdi>mdi→buy(2), adx>25&&mdi>pdi→sell(2) ← S85 누락 추가
    var _adxSig = 0;
    if(_ind.adx) { if(_ind.adx.adx>25&&_ind.adx.pdi>_ind.adx.mdi) _adxSig=2; else if(_ind.adx.adx>25&&_ind.adx.mdi>_ind.adx.pdi) _adxSig=-2; }
    _trendItems.push({name:'ADX', val: _adxSig * 0.5, maxW:1, group:'sub'});
    // Ichimoku signal — 엔진: ind.ichimoku.signal (sub에서 add(signal,3))
    if(_ind.ichimoku && _ind.ichimoku.signal) { var _iSig=_ind.ichimoku.signal; _trendItems.push({name:'일목 신호', val: (_iSig.type==='buy'?3:_iSig.type==='sell'?-3:0) * 0.5, maxW:1.5, group:'sub'}); }
    // A/D signal — 엔진: ind.ad.signal (sub에서 add(signal,4))
    if(_ind.ad && _ind.ad.signal) { var _adSig=_ind.ad.signal; _trendItems.push({name:'A/D 신호', val: (_adSig.type==='buy'?4:_adSig.type==='sell'?-4:0) * 0.5, maxW:2, group:'sub'}); }
    // 캔들 패턴 — 엔진 동일
    _trendItems.push({name:'캔들 패턴', val: (_ind.candle.bullish ? _clamp(_ind.candle.score, 0, 12) : _ind.candle.bearish ? -_clamp(Math.abs(_ind.candle.score), 0, 12) : 0) * 0.5, maxW:6, group:'sub'});
    // 거래량 패턴 — 엔진 동일
    _trendItems.push({name:'거래량 패턴', val: (_ind.volPattern.bullish ? _clamp(_ind.volPattern.score, 0, 10) : _ind.volPattern.bearish ? -_clamp(Math.abs(_ind.volPattern.score), 0, 10) : 0) * 0.5, maxW:5, group:'sub'});
    // ── aux 그룹 (추가보조) ──
  ...
  // ── 최종 판정 (Verdict) ──
  var verdict = _buildVerdict(readyScore, entryScore, trendScore, transition, staleness, btScore);
...
/**
 * 최종 판정 — "늦었는지 지금도 괜찮은지" 종합 결론
 */
function _buildVerdict(readyS, entryS, trendS, transition, staleness, btScore) {
  var lines = [];
  var tone = 'neutral'; // bullish / cautious / bearish / neutral
  var label = '관망';

  // 단계 판별
  var stage = 'wait'; // wait / ready / entering / confirmed / late
  if (trendS >= 60 && entryS >= 50) { stage = 'confirmed'; }
  else if (entryS >= 60 && readyS >= 50) { stage = 'entering'; }
  else if (readyS >= 60) { stage = 'ready'; }
  else if (trendS >= 70 && readyS < 30) { stage = 'late'; }

  if (stage === 'confirmed') {
    label = '보유 확인';
    tone = 'bullish';
    lines.push('진입타이밍→강세→추세 순서로 조건이 확인되었습니다. 반등이 이미 진행 중이며 추세가 형성된 구간입니다.');
    if (btScore != null && btScore >= 60) lines.push('과거 백테스트에서도 유사 패턴이 양호한 수익을 보였습니다(BT ' + btScore + '점).');
    else if (btScore != null && btScore < 40) lines.push('다만 과거 백테스트 결과는 저조합니다(BT ' + btScore + '점). 포지션 크기를 줄이거나 손절을 타이트하게 설정하세요.');
  } else if (stage === 'entering') {
    label = '진입 검토';
    tone = 'cautious';
    lines.push('반등 조건이 익은 상태에서 실제 강세 신호가 감지되고 있습니다. 진입을 적극 검토할 수 있는 구간입니다.');
    lines.push('강세→추세 전이 확률 ' + transition.entryToTrend.prob + '% — ' + transition.entryToTrend.days + ' 이내 추세 확인 예상.');
    if (btScore != null && btScore >= 60) lines.push('과거 검증도 양호합니다(BT ' + btScore + '점).');
  } else if (stage === 'ready') {
    label = '관심';
    tone = 'neutral';
    lines.push('반등 조건이 익어가고 있지만 아직 실제 강세 신호는 약합니다. 관심 종목으로 등록하고 강세 신호를 기다리세요.');
    lines.push('강세 전이 확률 ' + transition.readyToEntry.prob + '% — ' + transition.readyToEntry.days + ' 이내 반등 신호 예상.');
  } else if (stage === 'late') {
    label = '과열 주의';
    tone = 'bearish';
    lines.push('추세 점수는 높지만 진입타이밍/강세 단계를 거치지 않고 올라왔습니다. 이미 많이 오른 상태일 수 있으며 고점 추격 위험이 있습니다.');
    lines.push('신규 진입보다는 눌림목을 기다리거나 관망하는 것이 안전합니다.');
  } else {
    label = '관망';
    tone = 'neutral';
    lines.push('현재 진입타이밍, 강세 신호, 추세 모두 약한 상태입니다. 조건이 충족될 때까지 관망하세요.');
  }

  // 데이터 지연 보정
  if (staleness.decay < 1.0 && staleness.hoursAgo > 24) {
    lines.push('');
