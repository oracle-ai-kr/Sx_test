...
SXI.advTpSl = function(price, atrPct, tpMult, slMult, tf, regime, trendPct, rsiVal, adxVal){
  ...
  // 도달 예상 시간 텍스트
  // [BUGFIX] 분봉/시간봉 케이스에서 ${bars}가 중복되던 버그 수정
  //   기존: `약 ${bars}${unit==='5분' ? `×${bars}(...)`: ...}` → '약 12×12(1시간)' 같이 출력
  //   수정: 분기 안에서 단일 표현식으로 정리 → '약 12봉(60분=1시간)' 형태로 명확히
  ...
  if(_isHolding){
    ...
    switch(verdictAction){
      ...
      case '즉시 청산':
        tone='bearish';
        stateLine='매도 신호 발생';
        mainText='BT 엔진이 매도 신호를 확인했습니다. 계획대로 매도를 실행하는 것이 통계적으로 유리합니다.';
...
SXI.advFundamental = function(fin, price, stock){
  ...
  //   기본값 false (구조적으로 안전한 쪽 — 기존 동작 유지)
...
SXI.renderDisclosureCard = function(discItp){
  ...
  if(entries && entries.length > 0){
    ...
    // 폴백: lineEntries 없으면 기존 동작 (text 통째로)
...
//   - 기존 호출(analScore 단일)도 호환 (scores4 미전달 시 v1 텍스트 폴백)
SXI.practicalGuide = function(verdictAction, analScore, btScore, summary, ind, scores4, momBadge){
  ...
  // [추이반영] 모멘텀 방향 단축 변수 — 보유중 4종 분기에서 추세 단언 모순 방지에 사용
  //   문제 사례: '청산 준비' 분기는 점수 추이가 'up'/'flat'일 때도
  //   "지표 모멘텀이 하락 방향으로 바뀌고 있습니다"로 단언 → 차트 추이와 모순
  //   해결: momDir별로 step 텍스트 분기 (down=기존 단언 유지, up/flat=현재 상태 묘사)
  const _momDir = momBadge ? momBadge.direction : 'flat';
  const _isMomUp = _momDir === 'up';
  const _isMomDown = _momDir === 'down';
  const _isMomFlat = _momDir === 'flat';
  ...
  if(verdictAction === '매수'){
    ...
    if(has4){
      ...
      if(_momText && momBadge?.direction === 'down'){
        ...
        guide.steps.push(`다만 ${_momText} — 점수는 낮지만 개선되는 중이라 회복 시 \"관망\" 또는 \"관심\"으로 격상될 수 있습니다. 관심 종목 등록만 해두고 추가 상승을 기다리세요.`);
      } else {
    ...
    // [추이반영] 모멘텀 정보 추가 — 같은 "보유 유지"라도 추세에 따라 안정성이 다름
    if(_momText && _isMomUp){
      guide.steps.push(`${_momText} — 점수가 개선되는 중이라 보유의 신뢰도가 높습니다.`);
    } else if(_momText && _isMomDown){
      guide.steps.push(`다만 ${_momText} — 아직 보유 유지 구간이지만 점수가 꺾이는 중이라 다음 봉에서 "청산 준비"로 격하될 가능성에 대비하세요.`);
    }
    ...
    // 보유 중 + 분석 약화 (단계적 대비)
    // [추이반영] title/step/caution 모두 점수 추이에 맞춰 분기
    //   문제: '청산 준비'는 passCount=2면 모멘텀과 무관하게 트리거되므로,
    //         점수 추이가 'up'/'flat'일 때도 "하락 방향으로 바뀌고 있다" 단언이 나가 모순
    //   해결: momDir별로 같은 판정 안에서 어조와 근거를 다르게 제시
    if(_isMomDown){
      // 점수도 하락 추이 — 기존 단언 유지 (가장 위험한 케이스)
      ...
      // 점수 추이는 개선 중 — 절대 수준이 합격선 미달이라 청산 준비로 잡힌 케이스
      guide.title = '점수 절대 수준이 낮아 경계 단계입니다';
      guide.steps = [
        `진입타이밍 ${s}점, 매매전략 ${b}점 — 보유 중인 종목이 4축 합격 기준에 한 단계 미달이라 "매도 준비" 단계로 잡혔습니다.`,
        `다만 ${_momText} — 점수는 개선되는 중이라 다음 봉에서 한 축이라도 합격선을 넘기면 "보유 유지"로 복귀합니다.`,
        `당장 매도하기보다는 추이를 한두 봉 더 지켜보고, 합격선 회복 여부에 따라 결정하는 것이 합리적입니다.`,
      ];
      guide.caution = '점수 추이가 좋다고 안심하지 마세요. 절대 수준이 낮아 한 번에 다시 꺾일 수 있으므로, 손절가(SL)는 반드시 유지하세요.';
    } else {
      // 점수 횡보 — 약화 임계 근처에서 정체
      guide.title = '점수가 약화 임계 근처에서 정체 중입니다';
      guide.steps = [
        `진입타이밍 ${s}점, 매매전략 ${b}점 — 보유 중이지만 4축 합격 기준에 한 단계 미달이라 "매도 준비" 단계로 잡혔습니다. 점수 추이는 횡보 상태입니다.`,
        `분석 엔진이 "매도 준비" 판정을 내렸습니다. 당장 모두 팔라는 뜻은 아니고, "대응 계획을 준비하라"는 의미입니다.`,
        `수익이 나있다면 분할 익절을 고려하세요. 일부(예: 30~50%)를 미리 팔아 수익을 확정하고, 나머지는 방향이 명확해질 때까지 유지할 수 있습니다.`,
      ];
      guide.caution = '"곧 다시 오를 거야"라는 막연한 기대는 위험합니다. 점수가 합격선 아래에서 정체된 상태이므로, 추가 약화에 대비한 계획이 필요합니다.';
    }
    ...
    // 보유 중 + 분석 악화 (적극 청산 검토)
    // [추이반영] passCount=1이라 매우 약한 상태. 추이가 up이어도 회복까지 거리가 멀다.
    if(_isMomDown){
      ...
      guide.title = '점수 절대 수준이 매우 낮은 단계입니다';
      guide.steps = [
        `진입타이밍 ${s}점, 매매전략 ${b}점 — 4축 중 1개만 합격선을 유지하고 있어 "매도 검토" 단계로 잡혔습니다.`,
        `${_momText} — 점수가 일부 개선되고 있지만 절대 수준이 매우 낮아 회복까지 거리가 멉니다. 단기 반등에 기댄 보유는 위험합니다.`,
        `수익이 나있다면 일부 익절로 리스크를 줄이고, 손실 구간이라면 손절선 준수를 우선하세요. 추이 개선이 합격선 회복으로 이어지지 않으면 정리 쪽이 안전합니다.`,
      ];
    } else {
      guide.title = '분석 지표가 약한 수준에서 정체 중입니다';
      guide.steps = [
        `진입타이밍 ${s}점, 매매전략 ${b}점 — 보유 중인 종목이 4축 중 1개만 합격선을 유지해 "매도 검토" 단계로 잡혔습니다. 점수 추이는 횡보 상태입니다.`,
        `분석 엔진이 "매도 검토" 판정을 내렸습니다. "매도 준비"보다 한 단계 심각한 상태로, 방어적 행동이 필요합니다.`,
        `수익이 나있다면 즉시 일부 또는 전량 익절을 고려하세요. 추가 약화 시 "즉시 매도"로 격상될 수 있으므로 선제적 대응이 안전합니다.`,
      ];
    }
    ...
    // 보유 중 + 매도 신호 발생 (즉시 실행)
    // [추이반영] BT 매도 신호 직격이라 추이와 무관하게 매도가 답.
    //   다만 추이가 up이면 사용자가 "회복 중인데 왜 매도?"라고 헷갈릴 수 있어 보강.
    ...
    if(_isMomUp){
      guide.steps.push(`${_momText} — 점수가 일부 개선되어 보일 수 있지만, BT 매도 신호가 우선합니다. 짧은 기술적 반등에 속아 매도를 미루면 손실이 커지기 쉽습니다.`);
    }
...
SXI.threeStageReport = function(qs, scoreMom, btResult, btScore, lastCandleDate, btTransitionStats, verdictAction, btStateObj) {
  ...
  // [추이반영] scoreMom에서 3개 점수의 델타 추출
  //   엔진의 delta는 "현재 vs 1~3봉전 평균" 정의 (sx_analysis_engine.js scoreMomentum)
  //   해석 텍스트는 점수 구간만 보고 "약해지고 있다" 같은 추세성 표현을 출력해
  //   실제 추이(+10 상승)와 모순되는 케이스 발생 → 추이 prefix로 보정
  var _readyDelta = (scoreMom && typeof scoreMom.delta === 'number') ? scoreMom.delta : 0;
  var _entryDelta = (scoreMom && typeof scoreMom.entryDelta === 'number') ? scoreMom.entryDelta : 0;
  var _trendDelta = (scoreMom && typeof scoreMom.trendDelta === 'number') ? scoreMom.trendDelta : 0;

  // 추이 라벨 분류: rising(+5↑) / falling(-5↓) / flat
  function _trendKind(d){
    if (d >= 5) return 'rising';
    if (d <= -5) return 'falling';
    return 'flat';
  }
  // 점수 구간별 베이스 텍스트 앞에 붙일 추이 어구
  //   "약해지고 있다" 같은 단정 표현은 베이스에서 빼고 여기서만 명시
  function _trendPrefix(delta, kind){
    if (kind === 'rising') return '추이상 ' + (delta>0?'+':'') + delta + '점 개선 중 — ';
    if (kind === 'falling') return '추이상 ' + delta + '점 약화 중 — ';
    return ''; // flat은 prefix 생략
  }
  ...
  // [추이반영] 5봉 추이 prefix 적용
  readySummary = _trendPrefix(_readyDelta, _trendKind(_readyDelta)) + readySummary;
  ...
  var entrySummary = '';
  // S99-2: 3톤 분기 — 강세 해석
  // [추이반영] 베이스 텍스트는 점수 구간(=현재 상태)만 묘사하도록 중립화
  //   "약해지고 있다", "강도가 강하지만" 등 추세성 표현은 _trendPrefix가 담당
  if (_tone === 'entry') {
    ...
    else if (entryScore >= 30) entrySummary = '반등 강도가 약한 수준입니다. 간헐적 양봉이 보이지만 추세 전환을 확신하기 어려운 상태입니다.';
    else entrySummary = '반등 강도가 감지되지 않습니다. 아직 하락 또는 횡보가 지속되고 있을 가능성이 높습니다.';
  } else if (_tone === 'caution') {
    if (entryScore >= 70) entrySummary = '반등 강도가 강한 수준입니다. 보유 포지션 기준으로 추세 지속 여부를 확인하며 익절 타이밍을 가늠하는 구간입니다.';
    else if (entryScore >= 50) entrySummary = '일부 반등 강도가 있으나, 보유 중이라면 추가 매수보다는 현재 수익 보존에 집중하세요.';
    else if (entryScore >= 30) entrySummary = '반등 강도가 약한 수준입니다. 손절 기준을 명확히 두고 대응하세요.';
    ...
    else if (entryScore >= 30) entrySummary = '반등 강도 약함 + 매도 판정입니다. 하락 추세가 지속될 가능성이 높으며, 포지션 정리를 서두르세요.';
    else entrySummary = '반등 강도 전무 + 매도 판정입니다. 즉각적인 포지션 정리가 필요합니다.';
  }
  // [추이반영] 5봉 추이 prefix 적용
  entrySummary = _trendPrefix(_entryDelta, _trendKind(_entryDelta)) + entrySummary;
  ...
  // [추이반영] 5봉 추이 prefix 적용
  trendPosition = _trendPrefix(_trendDelta, _trendKind(_trendDelta)) + trendPosition;
