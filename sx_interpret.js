...
// ════════════════════════════════════════════════════════════
SXI.valuationJudge = function(fin, price, stock){
  if(!fin) return null;

  const isUS = stock && stock._mkt === 'us';
  const benchPer = isUS ? 20 : 11;
  ...
  for(const f of sorted){
    ...
    if(fForm === '8-K' || fForm === '8-K/A'){
      ...
      if(codes.length === 0){
        ...
        lines.push(`【${fDate}】 8-K — 분류 항목 없는 변동 공시. SEC 원문 확인 권장.`);
        lineEntries.push({ date: fDate, tone:'neutral', title:'8-K (기타)', desc:'분류 항목 없는 변동 공시', url: fUrl });
      ...
      for(const code of codes){
        ...
        lines.push(`【${fDate}】 8-K Item ${code} — ${meta.name}\n${meta.impact}`);
        lineEntries.push({ date: fDate, tone: badgeType, title:`Item ${code} ${meta.name}`, desc: meta.impact, url: fUrl });
      ...
      lines.push(`【${fDate}】 ${fForm} (${label}) — 정기 공시. 사업 현황, 위험요인, 재무제표 전체 확인 가능.`);
      lineEntries.push({ date: fDate, tone:'bullish', title:`${fForm} ${label}`, desc:'정기 연간보고서 — 사업 현황 + 재무제표 + 위험요인 공시', url: fUrl });
      hasPositive = true;
      ...
      lines.push(`【${fDate}】 ${fForm} (분기보고서) — 분기 실적 + 재무 상태 업데이트.`);
      lineEntries.push({ date: fDate, tone:'bullish', title:`${fForm} 분기보고서`, desc:'분기 실적·재무 상태 정기 공시', url: fUrl });
      hasPositive = true;
      totalScore += 55; scoreCount++;
    } else if(fForm.startsWith('SC 13D')){
      badges.push({ type:'warning', text:`${fForm} (5%+ 지분 — 액티브)`, date: fDate, url: fUrl, score: 40 });
      lines.push(`【${fDate}】 ${fForm} — 5% 이상 지분 액티브 신고. 적대적 인수 또는 행동주의 투자자 등장 가능성. 누가 신고했는지 SEC 원문 확인.`);
      lineEntries.push({ date: fDate, tone:'warning', title:`${fForm} 5%+ 지분`, desc:'액티브 신고 — 적대적/행동주의 가능성', url: fUrl });
      hasWarning = true;
      ...
      lines.push(`【${fDate}】 ${fForm} — 5% 이상 지분 패시브 신고. 기관 매수 (호재).`);
      lineEntries.push({ date: fDate, tone:'bullish', title:`${fForm} 5%+ 지분`, desc:'패시브 매수 신고 — 기관 매집 호재', url: fUrl });
      hasPositive = true;
