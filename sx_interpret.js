// ════════════════════════════════════════════════════════════
//  SIGNAL X — Interpret Engine v5.3 <!-- S139 Phase 1 종합평 UI-C 정합 재설계: 비보유 4종(매수/관심/관망/회피)을 action(BUY/SELL/HOLD)+score 지시문에서 verdictAction 기반 해설 톤으로 전환. "약한 매수 신호 / 매수 우위 구간" 같은 지시문 제거 → "기술적 매수 구간 / 조건부 진입 구간 / 방향 미결 구간 / 진입 부적정" 해설 톤. actionGuide/buyTrigger 제거 (C 권한 보존 — 행동 지시는 C 전담). invalidation만 유지 (기술적 무효화는 팩트 정보). regimeAdvice 필드 신규 — SXI._regimeNeedsCaution(ind.regime label/direction/adx/bbWidth + psycho 기반 4조건) + SXI._buildRegimeAdvice(verdictAction × 주의 타입 조합 문구). 레짐 주의 필요 시에만 "💡 시장 환경 고려" 블록 렌더. 9종 전부 regimeAdvice 적용(보유중/비보유/fallback). 프로젝트 C v3.0 원칙 ②(C 단일성) + 연속성 문서 권한 분리 원칙 구현. --> v5.2 <!-- S103-fix7 Phase3-B-3 SXI.summary C화(6번째 verdictAction 파라미터 추가, 보유중 5종(보유유지/청산준비/청산검토/즉시청산/매도완료)일 때 stateLine 보유자 맥락 재작성, actionGuide/buyTrigger 숨김 - 비보유자 관점 문구로 인한 사용자 혼란 제거) --> v5.1 <!-- S103-fix7 Phase3-B-1 practicalGuide C화(btAction 4종 → verdictAction 9종 직접수신, 신규 5종 가이드: 보유유지/청산준비/청산검토/즉시청산/매도완료) --> v5.0 <!-- S103-fix7 Phase3-A-3 _buildVerdict 감독관기반 전면재작성(점수stage 폐기, verdictAction 9종 직접매핑 → 배너와 완전정합) + threeStageReport 8th param btStateObj 추가 → 매도완료 익절/손절/수익률/진입가 본문 표시 --> v4.9 <!-- S103-fix7 Phase3-A-2 threeStageReport 톤 매핑 재조정("대기"폐지, "관망"→entry(비보유 관찰), "회피"→entry(진입안함), caution=보유중+아직청산X, exit=매도확정) --> v4.8 <!-- S99 -->
...
//        stateLine, actionGuide, invalidation, buyTrigger, regimeAdvice
// ════════════════════════════════════════════════════════════

// ════════════════════════════════════════════════════════════
//  S139 Phase 1: 종합평 UI-C 정합 — 레짐 기반 환경 권고 헬퍼
//  배경: C 판정(SXC)이 매수/매도 전담, 종합평은 C 해설 + 레짐 환경 권고로 역할 분리
//  철학: C 판정을 뒤집지 않고, 시장 환경 주의가 필요할 때만 추가 권고
// ════════════════════════════════════════════════════════════

// 레짐 주의 필요 여부 판정
// 실제 ind.regime 필드 구조: {label, icon, direction, score, adx, bbWidth}
// - label: '추세+변동'|'추세장'|'횡보장'|'전환기'|'불명'
// - direction: 'UP'|'DOWN'|'FLAT'
// - adx, bbWidth: number
SXI._regimeNeedsCaution = function(ind){
  if(!ind || !ind.regime) return { need:false, reasons:[] };
  const r = ind.regime;
  const psy = ind.psycho?.psycho;
  const reasons = [];

  // 조건 1: 추세+고변동 (label 기반)
  if(r.label === '추세+변동'){
    const adxStr = (r.adx != null) ? r.adx.toFixed(0) : '?';
    const bbStr = (r.bbWidth != null) ? r.bbWidth.toFixed(1) : '?';
    reasons.push({ type:'volatile_trend', text:`추세+고변동 구간 (ADX ${adxStr}, BB폭 ${bbStr})` });
  }
  // 조건 2: 강한 추세 (추세+변동에 이미 잡혔으면 중복 방지)
  else if(r.adx != null && r.adx >= 40){
    reasons.push({ type:'strong_trend', text:`강한 추세 (ADX ${r.adx.toFixed(0)})` });
  }

  // 조건 3: 하락 레짐
  if(r.direction === 'DOWN'){
    reasons.push({ type:'downtrend', text:`하락 레짐 (${r.label || '방향 하락'})` });
  }

  // 조건 4: 심리도 극단
  if(psy != null){
    if(psy >= 85){
      reasons.push({ type:'overheat', text:`심리도 극단 과열 (${psy.toFixed(0)}%)` });
    } else if(psy <= 15){
      reasons.push({ type:'oversold', text:`심리도 극단 과매도 (${psy.toFixed(0)}%)` });
    }
  }
...
// verdictAction × 레짐 주의 조합으로 환경 고려 문구 생성
// 반환: { text, reasonSummary } 또는 null (주의 불필요 시)
SXI._buildRegimeAdvice = function(verdictAction, cautionInfo){
  if(!cautionInfo || !cautionInfo.need) return null;

  const reasonSummary = cautionInfo.reasons.map(r=>r.text).join(' · ');
  const types = cautionInfo.reasons.map(r=>r.type);
  const hasVolatile = types.indexOf('volatile_trend') >= 0 || types.indexOf('strong_trend') >= 0;
  const hasDown = types.indexOf('downtrend') >= 0;
  const hasOverheat = types.indexOf('overheat') >= 0;
  const hasOversold = types.indexOf('oversold') >= 0;

  const isHolding = ['보유 유지','청산 준비','청산 검토','즉시 청산','매도 완료'].indexOf(verdictAction) >= 0;
  const isBuyGroup = verdictAction === '매수' || verdictAction === '관심';
  const isWaitGroup = verdictAction === '관망' || verdictAction === '회피';

  let advice = '';

  if(isBuyGroup){
    if(hasVolatile && hasDown){
      advice = '변동성이 크면서 시장 전체가 하락 레짐입니다. 기술 분석상 진입 가능 구간이어도 시장 역풍에 휩쓸릴 위험이 크므로 진입 규모를 평소보다 크게 축소하고 손절선을 엄격하게 적용하세요.';
    } else if(hasVolatile){
      advice = '기술 분석상 진입 가능 구간이지만, 변동성이 큰 구간에서는 반대 방향 전환 시 손실도 빠릅니다. 진입 규모와 손절선을 보수적으로 잡는 것을 권장합니다.';
    } else if(hasDown){
      advice = '시장 전체가 하락 레짐입니다. 개별 종목 기술 신호가 좋아도 시장 하락에 휩쓸릴 위험이 있으므로 비중 축소를 검토하세요.';
    } else if(hasOverheat){
      advice = '시장 심리가 극단 과열 구간입니다. 차익실현 매물 출회 위험이 크므로 진입 타이밍을 다시 점검하세요.';
    } else if(hasOversold){
      advice = '시장 심리가 극단 과매도입니다. 반등 여지는 있으나 하락 관성도 강하므로 분할 접근을 권장합니다.';
    }
  } else if(isWaitGroup){
    if(hasVolatile){
      advice = '변동성이 큰 구간에서는 섣부른 진입보다 관망이 안전합니다. 명확한 방향이 확인된 후 접근하세요.';
    } else if(hasDown){
      advice = '시장 전체가 하락 레짐이므로 관망/회피 판단의 신뢰도가 더 높습니다.';
    } else if(hasOverheat){
      advice = '시장 심리 과열 구간이라 관망/회피 판단의 신뢰도가 더 높습니다. 조정 확인 후 접근을 검토하세요.';
    } else if(hasOversold){
      advice = '시장 심리 과매도 구간입니다. 반등 가능성은 있으나 확정 신호를 확인한 후 접근하세요.';
    }
  } else if(isHolding){
    if(hasVolatile && hasDown){
      advice = '변동성이 크면서 시장 전체가 하락 레짐입니다. 보유 중이라도 손절선 관리를 평소보다 엄격하게 적용하고 비중 축소를 함께 검토하세요.';
    } else if(hasVolatile){
      advice = '변동성이 큰 구간입니다. 보유 중이라도 손절선 관리를 평소보다 엄격하게 적용하세요.';
    } else if(hasDown){
      advice = '시장 전체가 하락 레짐입니다. 개별 종목 지표가 유지되더라도 비중 축소를 함께 검토하세요.';
    } else if(hasOverheat){
      advice = '시장 심리가 극단 과열입니다. 차익실현 매물 출회 가능성에 대비해 익절 계획을 점검하세요.';
    } else if(hasOversold){
      advice = '시장 심리가 극단 과매도입니다. 반등 여지는 있으나 추가 하락 대비 손절 기준을 재확인하세요.';
    }
  }

  if(!advice) return null;
  return { text: advice, reasonSummary };
};
...
//  S139 Phase 1: 비보유 4종도 verdictAction 기반 해설 톤으로 재설계(지시문 제거) + regimeAdvice 추가
SXI.summary = function(action, score, reasons, ind, verdictAction){
  ...
  if(_isHolding){
    ...
    // S139 Phase 1: 보유중에도 레짐 주의 시 환경 권고
    const _cautionHold = SXI._regimeNeedsCaution(ind);
    const _regimeAdviceHold = SXI._buildRegimeAdvice(verdictAction, _cautionHold);
    return {tone,mainText,keyReasons,risks,composites,stateLine,actionGuide,invalidation,buyTrigger,regimeAdvice:_regimeAdviceHold};
  }

  // S139 Phase 1: 비보유 4종(매수/관심/관망/회피) verdictAction 기반 해설 톤 재설계
  //   기존 action(BUY/SELL/HOLD) + score 기반 지시문 → C 판정 해설 + 무효화 조건만 유지
  //   actionGuide/buyTrigger 제거 (월권 제거 — 행동 지시는 C 전담)
  //   regimeAdvice 신규 — 레짐 주의 시 시장 환경 고려 권고
  const _isNonHolding = verdictAction && ['매수','관심','관망','회피'].indexOf(verdictAction) >= 0;
  if(_isNonHolding){
    switch(verdictAction){
      case '매수':
        tone='bullish';
        stateLine='기술적 매수 구간';
        mainText='C 엔진이 기술적 진입 적정으로 판단했습니다. 지표 근거는 아래 "핵심 이유"와 "진입 검토"를 참고하세요.';
        invalidation=(vwapPos==='above'||vwapPos==='above_far')
          ? 'VWAP 이탈 또는 최근 스윙 저점 이탈 시 매수 해석이 약화됩니다.'
          : 'MA20 이탈 또는 직전 저점 이탈 시 매수 해석이 약화됩니다.';
        break;
      case '관심':
        tone='bullish';
        stateLine='조건부 진입 구간';
        mainText='C 엔진이 신호 감지 후 확인 대기로 판단했습니다. 명확한 추가 신호가 나타날 때까지 관찰이 필요합니다.';
        invalidation='주요 지지선 이탈 또는 거래량 급감 시 관심 해석이 약화됩니다.';
        break;
      case '관망':
        tone='neutral';
        stateLine='방향 미결 구간';
        mainText='C 엔진이 매수/매도 어느 쪽도 우위 아님으로 판단했습니다. 명확한 방향이 나타날 때까지 기다리는 것이 안전합니다.';
        invalidation='추세 지표가 한 방향으로 정렬되면 관망 해석이 전환됩니다.';
        break;
      case '회피':
        tone='bearish';
        stateLine='진입 부적정';
        mainText='C 엔진이 기술적 진입 조건 미충족으로 판단했습니다. 신규 진입은 권하지 않습니다.';
        invalidation='주요 이평선 회복 + 거래량 동반 양봉 출현 시 회피 해석이 약화됩니다.';
        break;
    }
    // 비보유에서도 actionGuide/buyTrigger 제거 (C 권한 보존)
    actionGuide='';
    buyTrigger='';
    if(reasons && reasons.length) mainText+=' 단, 안전필터에서 '+reasons.join(', ')+' 조건이 감지되어 주의가 필요합니다.';
    const _cautionNonHold = SXI._regimeNeedsCaution(ind);
    const _regimeAdviceNonHold = SXI._buildRegimeAdvice(verdictAction, _cautionNonHold);
    return {tone,mainText,keyReasons,risks,composites,stateLine,actionGuide,invalidation,buyTrigger,regimeAdvice:_regimeAdviceNonHold};
  }

  // Fallback: verdictAction 미지정 시 기존 BUY/SELL/HOLD 분기 유지 (레거시 호환)
  if(action==='BUY'){
    tone='bullish';
  ...
  if(reasons && reasons.length) mainText+=' 단, 안전필터에서 '+reasons.join(', ')+' 조건이 감지되어 주의가 필요합니다.';
  // S139 Phase 1: Fallback 경로에도 regimeAdvice 추가 (verdictAction null이면 null 반환되어 안전)
  const _cautionFb = SXI._regimeNeedsCaution(ind);
  const _regimeAdviceFb = SXI._buildRegimeAdvice(verdictAction, _cautionFb);
  return {tone,mainText,keyReasons,risks,composites,stateLine,actionGuide,invalidation,buyTrigger,regimeAdvice:_regimeAdviceFb};
};
