...
SXI.compositeMomentum = function(ind){
  ...
  else if(macdG && macdH<0 && macdPH!=null && macdH>macdPH) notes.push({tone:'bullish',icon:'[~]',title:'MACD 골든크로스 + 모멘텀 회복',text:'하락 모멘텀이 약해지며 방향 전환 초기 신호가 나타납니다. 거래량 증가 동반 여부를 함께 확인하세요 — 이 조합은 측정된 적이 없습니다.'});
...
SXI.compositeSynergy = function(ind){
  ...
  if(psyV!=null && psyV<=25 && abT==='bullish')
    notes.push({tone:'bullish',icon:'🌱',title:'심리도 침체 + AB 매수 전환 → 바닥 반전 초기',text:`심리도 ${psyV.toFixed(0)}%(과매도) 상태에서 AB Ratio가 매수 우위로 전환되었습니다. 매도 에너지가 소진된 가운데 장중 매수세가 살아나는 첫 신호입니다. RSI 반등·거래량 증가가 함께 오는지 확인하세요 — 이 조합은 측정된 적이 없습니다.`});
...
SXI._attachRegimeContext = function(verdictAction, regime){
  ...
  const M = {
    '매수': {
      UP:   ' 시장 레짐은 상승 추세입니다. 레짐이 판정 적중을 높인다는 측정은 없습니다(원장 추세신호 3시장 측정 미달).',
    ...
    '관심': {
      ...
      FLAT: ' 시장 레짐은 횡보입니다. 레짐이 트리거 적중을 낮춘다는 측정은 없습니다(원장 추세신호 3시장 측정 미달). 거래량 동반 돌파 확인 후 진입을 고려하세요.'
    },
    '관망': {
    ...
    '청산 준비': {
      ...
      DOWN: ' 시장 레짐도 하락 추세입니다. 레짐이 약화 신호 적중을 높인다는 측정은 없습니다(원장 추세신호 3시장 측정 미달). 청산 시점을 앞당기는 것을 적극 검토하세요.',
    ...
    '즉시 청산': {
      ...
      DOWN: ' 시장 레짐도 하락 추세입니다. 레짐이 매도 신호 적중을 높인다는 측정은 없습니다(원장 추세신호 3시장 측정 미달). 청산 시점은 미리 정한 규칙대로 판단하세요.',
  ...
  if(verdictAction === '매수'){
    ...
    if(has4){
      ...
      guide.steps = [
        ...
        (b >= 60
          ? `매매전략 ${b}점 — 과거 성과 자체는 나쁘지 않습니다. 다만 지금 진입 조건이 갖춰지지 않았습니다.`
          : b > 0
          ? `매매전략 ${b}점 — 과거에도 이런 조건에서 매매했을 때 성과가 좋지 않았습니다.`
          : `매매전략 점수 없음 — 과거 성과는 확인되지 않았습니다.`),
