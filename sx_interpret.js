...
SXI.renderDisclosureCard = function(discItp){
  ...
  if(entries && entries.length > 0){
    ...
    if(tail.length > 0){
      ...
      // [S57-compact v3] 토글 영역 여백 축소 — margin-top 5→2, padding-top 4→2
      bodyHTML = `<div style="margin:0">${headHTML}</div>
        <div class="itp-toggle-inline" onclick="_sxVib(8);const c=document.getElementById('${tid}');c.classList.toggle('show');this.querySelector('.sb-arrow').textContent=c.classList.contains('show')?'▼':'▶';this.querySelector('.sb-label').textContent=c.classList.contains('show')?'추가 ${tail.length}건 접기':'추가 ${tail.length}건 보기'" style="font-size:10px;color:var(--accent);cursor:pointer;font-weight:600;margin-top:2px;padding-top:2px;border-top:1px dashed var(--border)"><span class="sb-arrow">▶</span> <span class="sb-label">추가 ${tail.length}건 보기</span></div>
        <div class="itp-card disc-more-body" id="${tid}" style="background:transparent;padding:0;margin-top:2px">${tailHTML}</div>`;
    } else {
      bodyHTML = `<div style="margin:0">${headHTML}</div>`;
    }
  } else {
    // 폴백: lineEntries 없으면 기존 동작 (text 통째로)
    bodyHTML = `<div style="margin:0">${discItp.text}</div>`;
  }

  // [S57-compact v3] 카드 컴팩트화 — 위아래 폭 약 35~40% 축소
  //   v2에서 인라인 스타일이 CSS .anal-section/.itp-card 기본값에 일부 밀린 듯 → !important로 강제
  //   1) anal-section 외곽 padding 12 → 8 !important (CSS .anal-section{padding:12px} 덮어쓰기)
  //   2) section-title margin-bottom 8 → 3 !important
  //   3) badges 영역 margin-bottom 5 → 3
  //   4) itp-card 내부 padding 7px 10px → 5px 8px !important (CSS .itp-card{padding:7px 10px} 덮어쓰기)
  //   5) itp-label(중복 소제목) 제거됨
  //   6) bodyHTML 내부 <div>에 margin:0 명시 (브라우저 기본 div margin 차단)
  //   7) summary 구분선 margin-top 4 → 2, padding-top 3 → 2
  //   8) 토글 영역 margin-top/padding-top 5/4 → 2/2
  return `<div class="anal-section" style="margin-bottom:6px;padding:8px !important">
    <div class="anal-section-title" style="margin-bottom:3px !important">${discItp.label}</div>
    <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:3px">${badgeHTML}</div>
    <div class="itp-card show" style="white-space:pre-line;padding:5px 8px !important;line-height:1.45;margin:0">
      ${bodyHTML}
      <div style="margin-top:2px;padding-top:2px;border-top:1px solid var(--border);font-size:10px;color:var(--text2)">${discItp.summary}</div>
    </div>
  </div>`;
};
