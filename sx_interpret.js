...
SXI.advMaConv = function(converging, spread, maArrange, bbSqueeze){
  ...
  if(converging && +spread<=2) return {tone:'neutral',label:'강한 수렴 (변곡점 임박)',text:`MA5/20/60 이격 ${sp}%. 이동평균선이 거의 한 점으로 수렴하고 있어 강력한 변곡점이 임박했습니다.${extraStr} 교과서에서는 이 수준의 수렴을 큰 추세의 시작점으로 보고, 볼린저 밴드 스퀴즈와 겹치면 돌파 시 폭발적 움직임을 예상합니다. 다만 측정은 반대쪽을 가리킵니다 — MA 간격이 가장 좁은 구간은 뒤이은 10봉 변동성이 오히려 **낮았습니다**(3시장 공통). 장 상태를 같은 조건으로 맞춰 비교해도 차이가 남았습니다(3시장 전부 5개 국면 구간 모두). ⚠단 절대 변동성 자체는 장이 거칠어지면 수렴 구간도 함께 커집니다 — '항상 조용하다'가 아니라 '같은 장에서 덜 출렁인다'는 뜻입니다. 방향을 예단하기보다, 돌파 방향(상방: MA5가 MA60 상향 돌파 + 양봉 + 거래량 증가 / 하방: MA5가 MA60 하향 이탈 + 음봉)을 확인한 후 따라가는 전략이 안전합니다. 거짓 돌파에 대비해 첫 진입은 소액으로 하고, 방향이 확인되면 추가 진입하세요.`};
