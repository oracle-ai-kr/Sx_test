...
SXI.calcHistoricalPer = function(historicalEps, candles){
  ...
  for(const e of historicalEps){
    ...
    for(const c of candles){
      ...
      const cd = String(c.date).slice(0, 10);      // YYYY-MM-DD
...
SXI.valuationJudge = function(fin, price, stock, historicalPer){
  if(!fin) return null;
  ...
  // [S327 Phase 2] 종목별 historical 평균 PER 활용
  //   historicalPer 인자가 유효(≥2년 흑자, PER 5~80 범위)하면 시장평균 대신 사용
  //   - benchPer (적정): historical 평균 그대로
  //   - targetPer (목표): historical 평균 × 1.20 (낙관 시나리오, 시장평균 방식과 동일 비율)
  //   인자 없거나 무효하면 시장평균 폴백 (KR 11/15, US 20/25)
  ...
  if(historicalPer && historicalPer.avgPer != null && historicalPer.validYears >= 2){
    ...
    targetPer = historicalPer.avgPer * 1.20;     // 종목별 평균의 약 1.2배 = 낙관선
