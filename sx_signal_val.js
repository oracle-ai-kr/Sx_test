...
(function(){
  ...
  function computeMetrics(records){
    ...
    for(var i = 0; i < n; i++){
      var r = records[i], ret = r.ret, au = ret > 0 ? 1 : 0, ad = au ? 1 : -1;
  ...
  function runHorizonSweep(rows, opts){
    ...
    var driftAcc = HS.map(function(){ return { s:0, n:0 }; });
    var nPts = 0;
    for(var e = warmup; e <= lastE; e += step){
      var trend = _regimeAt(ctx.closes, e, opts.regimeBand), ce = ctx.closes[e];
      nPts++;
      var rets = HS.map(function(h){ return ce > 0 ? ctx.closes[e + h] / ce - 1 : 0; });
      HS.forEach(function(h, hi){ driftAcc[hi].s += rets[hi]; driftAcc[hi].n++; });
      for(var s = 0; s < SIGNALS.length; s++){
        var p = SIGNALS[s].fn(ctx, e);                          // prob 1회 → 모든 호라이즌 공유
        if(p == null) continue;
        for(var hi = 0; hi < HS.length; hi++) recs[s][hi].push({ prob:p, ret:rets[hi], trend:trend });
      }
    }
    var signals = SIGNALS.map(function(sig, s){
      return { key:sig.key, label:sig.label, byH: HS.map(function(h, hi){ return computeMetrics(recs[s][hi]); }) };
    });
    var drift = HS.map(function(h, hi){ return driftAcc[hi].n ? Math.round(driftAcc[hi].s / driftAcc[hi].n * 1e4) / 100 : null; });
    return { ok:true, horizons:HS, nPts:nPts, step:step, warmup:warmup, drift:drift, signals:signals, knnReady:!!ctx.SK, lrReady:!!ctx.LR };
  }
