...
(function(global){
  ...
  //     - 기본값 enabled=false → 사용자가 명시적으로 ON 해야 작동 (안전)
  //     - 시장별 target ATR%: KR 2.0%, COIN 4.0%, US 1.5% (실측 변동성 기준)
  //     - clamp [0.3, 3.0]: 극단값 차단 (의미 없는 진입 / 과도한 레버리지 방지)
  //
  //   사용처:
  //     - sx_screener.html 설정탭 UI ([S301] 이번 작업)
  //     - sx_analysis_engine.js sxRunBtEngine ([S297] 다음 작업)
  //     - sx_scan_worker.js 미러 동기화 ([S297] 동시 적용)
  // ═════════════════════════════════════════════════════════════════

  const VOL_TARGET_KEY = 'SX_BT_VOL_TARGETING';
  const VOL_TARGET_DEFAULTS = Object.freeze({
    enabled: false,    // 기본 OFF (사용자가 명시적으로 ON)
