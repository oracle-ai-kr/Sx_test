...
(function(global){
  ...
  //     - [S308] 기본값 enabled=true → 변동성 정규화로 종목 간 안전성 자동 보장
  //               (이전: enabled=false 안전 우선, [S304] 도입 + 실측 검증 후 ON으로 변경)
  //               기존에 OFF로 저장한 사용자는 모달에서 [기본값 복원] 클릭하면 ON으로 전환
  ...
  const VOL_TARGET_DEFAULTS = Object.freeze({
    enabled: true,     // [S308] 기본 ON — 변동성 정규화 자동 적용 (이전: false 안전 우선)
