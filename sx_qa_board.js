...
//  버전: v4 · [S1172] importAll의 text 필수 조건 완화(기존 id엔 불필요) · [S1171] 예상 찍기 제거 · [S1170] askText 지시문 · [S1168] 유사검색 한국어 접두 일치 · v1 [S1167] 신설
//
//  역할:
...
(function (global) {
  ...
  var ST = {
    ...
    2: { key: 'no',      icon: '⚫', name: '아니다',  desc: '쟀고 안 됐다 · 갈리지 않음(무변별) 포함' },   // [S1275] 경계 명시
    3: { key: 'yes',     icon: '🟢', name: '그렇다',  desc: '쟀고 됐다' },
    4: { key: 'cant',    icon: '⚪', name: '못 잰다', desc: '번역 불가 · 데이터 없음 · 쟀지만 단답 불가' }   // [S1275] Q3·Q5형(시장 상반·구조적 부족) 포함
  };
  ...
  var DEPTH = {
    //  [S1174] 이름표(되나/언제/왜)를 화면에서 뺀다 — 배지 옆에 붙으면 **판정처럼 읽힌다.**
    //    깊이는 얼마나 파고들었나이지 결과가 아니다. 뜻은 desc(툴팁)로만 남긴다.
    1: { dots: '●○○', desc: '되나 — 규칙 하나를 나이브와 비교했다' },
    2: { dots: '●●○', desc: '언제 — 조건(레짐·시장·기간)으로 갈라 어디서 갈리는지 봤다' },
    3: { dots: '●●●', desc: '왜 — 경계의 후보를 세우고, 후보끼리 다른 예측을 뽑아 갈랐다' }
  };
  ...
  function askText(f) {
    ...
    return list(f).then(function (rows) {
      ...
      //  [S1275] 위 한 줄은 이진 문항("X가 나이브를 이기나?")만 상정했다. "A인가 B인가"·"어디가
      //    가장 ~인가" 같은 선택형에서 경계가 새어나갔다(S1257 Q3 사전등록이 무변별을 cant로
      //    매핑했다 — 실체는 무변별이 아니라 시장 상반이었고 버킷은 맞았지만 문구가 부정확했다).
      //    그래서 갈림길을 사전등록 단계에서 못박도록 조항을 명시한다.
      L.push('   선택형("A인가 B인가"·"어디가 가장 ~인가")도 같다 — **전 시장 무변별이면 `no`**');
      L.push('   ("그 축으로는 갈리지 않는다"). `cant`는 **시장 간 방향 상반**이거나 판정 가능');
      L.push('   구역이 **구조적으로 부족**해 단답 자체가 성립하지 않을 때만 쓴다.');
      L.push('   ⚠ 사전등록의 답 매핑에 이 갈림길을 미리 적을 것 — 재고 나서 정하면 사후 조정이 된다.');
