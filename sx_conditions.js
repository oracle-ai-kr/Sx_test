...
const SX_CONDITIONS = [
  ...
  {id:'scope',name:'범위지정',phase:'p1',groups:[
    ...
    {id:'scope_basic',name:'기본 정보',conditions:[
      ...
      //   KR 기본값 1~5봉 (일봉 기준 일주일). 다른 시장: US 1~5, COIN 1~3.
      //   처리: sx_scan_worker.js에서 id 명시 체크로 메타조건 추출 (카테고리 위치 무관)
      {id:'_recent_n_bars',name:'최근 N봉',type:'range',unit:'봉',min:1,max:60,default:{min:1,max:5},source:'calc_candle',desc:'조건 평가 대상 범위. MA 크로스: N봉 사이 어딘가에 교차 발생 (오늘=N2, 일주일=N7~10, 한달=N20~25). 그 외 조건: N봉 중 한 봉이라도 동시 충족 시 통과',recommend:'1~3 (오늘) / 5~10 (일주일) / 20~25 (한 달)'},
  ...
  {id:'backtest',name:'백테스트',phase:'p3',groups:[
    {id:'ta_bt',name:'백테스트 (실시간)',conditions:[
      ...
      {axis4:'buy',id:'_c_buy_marker',name:'C 매수마커 ▲',type:'select',options:['설정안함','매수마커 있음'],default:'설정안함',source:'calc_candle',desc:'C판정 보라 매수마커(▲) — 신규 매수 + 보유중 추가진입(점선) 모두 포함. 차트 보라 ▲와 동일 (svVerdict.chartMarker=buy)',recommend:'매수마커 있음 → 신규진입·추가진입 후보'},
      {axis4:'sell',id:'_c_sell_marker',name:'C 매도마커 ▼',type:'select',options:['설정안함','매도마커 있음'],default:'설정안함',source:'calc_candle',desc:'C판정 보라 매도마커(▼) — 청산/매도완료 + 보유중 조기청산·부분익절(점선) 포함. 차트 보라 ▼와 동일 (svVerdict.chartMarker=sell)',recommend:'매도마커 있음 → 청산·부분익절 검토 후보'},
    ]},
  ]},
];
