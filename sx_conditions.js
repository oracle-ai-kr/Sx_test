...
const SX_CONDITIONS = [
  ...
  {id:'price_analysis',name:'시세분석',phase:'p1',groups:[
    ...
    {id:'volume_cond',name:'거래량/거래대금',conditions:[
      ...
      {id:'volume_avg20_ratio',name:'20일 평균 대비 거래량',type:'range',unit:'%',min:0,max:10000,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'당일거래량÷20일평균×100'},
  ...
  {id:'technical',name:'기술적분석',phase:'p2',groups:[
    {id:'ta_ma',name:'이동평균선',conditions:[
      ...
      {id:'_recent_n_bars',name:'최근 N봉',type:'range',unit:'봉',min:1,max:60,uiHint:'max',default:{min:null,max:5},source:'calc_candle',desc:'조건 평가 대상 범위. MA 크로스: N봉 사이 어딘가에 교차 발생 (오늘=N2, 일주일=N7~10, 한달=N20~25). 그 외 조건: N봉 중 한 봉이라도 동시 충족 시 통과'},
    ...
    {id:'ta_trend',name:'추세지표',conditions:[
      ...
      {id:'adx_value',name:'ADX (추세강도)',type:'range',unit:'',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle'},
    ...
    {id:'ta_channel',name:'채널/밴드',conditions:[
      ...
      // [v3.10] 신규 안전 가드 — BT 진입 게이트와 동기화 가능. 고점 추격 방어용.
      //   period_change(5일 고정)와 다른 점: N가변, "고점" 기반(상승률 기반 아님)
      {id:'recent_high_proximity',name:'최근 N봉 고점 근접도',type:'range',unit:'%',min:0,max:100,uiHint:'max',default:{min:null,max:null},source:'calc_candle',desc:'현재가÷최근N봉 고가×100. 95% 이상=고점 근접 → max 90~95로 설정 시 추격 방어 (BT 게이트 highProximity와 동기화). 봉수는 sx_scan_worker.js의 _RECENT_HIGH_PROX_N(기본 20봉)에서 조정'},
      {id:'recent_n_change',name:'최근 N봉 대비 상승률',type:'range',unit:'%',min:-50,max:100,uiHint:'max',default:{min:null,max:null},source:'calc_candle',desc:'(현재가-N봉전 시작가)÷N봉전×100. max 15~25% 설정 시 단기 급등 추격 방어 (BT 게이트 recentHigh와 동기화). 봉수는 _RECENT_N_CHANGE_N(기본 5봉)에서 조정'},
    ]},
  ...
  {id:'engine_verdict',name:'엔진 판정',phase:'p3',groups:[
    {id:'ta_signal',name:'분석 판정',conditions:[
      {id:'score_range',name:'추세 방향 점수',type:'range',unit:'',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'추세 방향 0~100 (분석탭 "추세 방향")'},
      {id:'_ready_score',name:'반등 신호 점수',type:'range',unit:'',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'반등 신호 0~100 (분석탭 "반등 신호" — 과매도+수축+눌림)'},
      {id:'_entry_score',name:'반등 강도 점수',type:'range',unit:'',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'반등 강도 0~100 (분석탭 "반등 강도" — RSI반등+MACD전환)'},
      ...
      {id:'_regime_label',name:'시장 레짐',type:'select',options:['설정안함','추세장','횡보장','추세+변동','전환기'],default:'설정안함',source:'calc_candle',desc:'ADX+BB폭 기반 시장 레짐'},
      ...
      {id:'_pullback_score',name:'눌림목 점수',type:'range',unit:'',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'눌림목 매수 적합도'},
    ]},
  ]},
  {id:'backtest',name:'백테스트',phase:'p3',groups:[
    {id:'ta_bt',name:'백테스트 (실시간)',conditions:[
      {id:'_bt_score',name:'매매전략 점수',type:'range',unit:'',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'BT 수익률+승률+MDD+PF 종합 0~100'},
      {id:'_bt_pnl',name:'BT 수익률',type:'range',unit:'%',min:-100,max:200,default:{min:null,max:null},source:'calc_candle',desc:'200봉 백테스트 총수익률'},
      {id:'_bt_winrate',name:'BT 승률',type:'range',unit:'%',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'승리 거래 비율'},
      {id:'_bt_trades',name:'BT 거래수',type:'range',unit:'회',min:0,max:100,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'총 청산 거래 수'},
      {id:'_bt_mdd',name:'BT MDD',type:'range',unit:'%',min:0,max:100,uiHint:'max',default:{min:null,max:null},source:'calc_candle',desc:'최대 낙폭 (절대값)'},
      {id:'_bt_pf',name:'BT 손익비',type:'range',unit:'',min:0,max:10,uiHint:'min',default:{min:null,max:null},source:'calc_candle',desc:'총수익/총손실 비율 (Profit Factor — PF≥2 우수, ≥1.5 양호, <1 손실 우세)'},
...
const COIN_CONDITIONS = [
  ...
  {id:'technical',name:'기술적분석',phase:'p2',groups:[
    ...
    {id:'ta_channel',name:'채널/밴드',conditions:[
      ...
      {id:'recent_high_proximity',name:'최근 N봉 고점 근접도',type:'range',unit:'%',min:0,max:100,uiHint:'max',default:{min:null,max:null},source:'calc_candle',desc:'현재가÷최근N봉 고가×100. 95% 이상=고점 근접 (BT 게이트 highProximity와 동기화)'},
      {id:'recent_n_change',name:'최근 N봉 대비 상승률',type:'range',unit:'%',min:-50,max:100,uiHint:'max',default:{min:null,max:null},source:'calc_candle',desc:'(현재가-N봉전)÷N봉전×100. 단기 급등 추격 방어 (BT 게이트 recentHigh와 동기화)'},
    ]},
    {id:'ta_other',name:'기타 지표',conditions:[
...
const PRESETS_KR_P2 = [
  ...
  {id:'p2_kr_reversal',name:'반등 시작',phase:'p2',group:'attack',desc:'RSI 과매도 + MACD 골든 + BB 하단 + 심리도 과매도 (최근 5봉 윈도우)',locked:true,conditions:{rsi_value:{min:20,max:35},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',psycho_value:{min:null,max:25},_recent_n_bars:{min:null,max:5}}},
  {id:'p2_kr_breakout',name:'급등 시작',phase:'p2',group:'attack',desc:'20일 신고가 + 거래량 200%↑ + VWAP위 + MACD매수 + EOM매수 + 5봉간 +25% 미만 (이미 급등한 것 제외, 최근 3봉)',locked:true,conditions:{new_high_low:'20일 신고가',volume_avg20_ratio:{min:200,max:null},vwap_position:'VWAP 위 (강세)',macd_signal:'MACD > Signal (매수)',eom_trend:'매수세 (상승)',recent_n_change:{min:null,max:25},_recent_n_bars:{min:null,max:3}}},
  {id:'p2_kr_candle_rev',name:'캔들 반전',phase:'p2',group:'attack',desc:'반전패턴 + RSI 과매도 + 거래량↑ (최근 3봉 윈도우)',locked:true,conditions:{reversal_pattern:['morning_star','bullish_engulfing','morning_doji_star','abandoned_baby_bull','hammer','piercing','tweezer_bottom','harami_bull','dragonfly_doji'],rsi_value:{min:null,max:35},volume_avg20_ratio:{min:150,max:null},_recent_n_bars:{min:null,max:3}}},
  // ── 준비 ──
  ...
  {id:'p2_kr_bear_env',name:'약세장 방어',phase:'p2',group:'env',desc:'시장약세 + RSI반등 + MACD골든 + BB하단 (최근 5봉 윈도우)',locked:true,conditions:{mkt_env_state:'약세 포함 (mild_bear+bear)',rsi_value:{min:25,max:40},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',_recent_n_bars:{min:null,max:5}}},
];
...
const PRESETS_COIN_P2 = [
  ...
  {id:'p2_coin_reversal',name:'과매도 반등',phase:'p2',group:'attack',desc:'RSI 과매도 + MACD 골든 + BB 하단 + 심리도 과매도 (최근 5봉 윈도우)',locked:true,conditions:{rsi_value:{min:20,max:35},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',psycho_value:{min:null,max:25},_recent_n_bars:{min:null,max:5}}},
  {id:'p2_coin_breakout',name:'급등 시작',phase:'p2',group:'attack',desc:'등락률 5%↑ + 거래량 200%↑ + VWAP위 + MACD매수 + EOM매수 + 5봉간 +40% 미만 (이미 급등한 것 제외, 최근 3봉)',locked:true,conditions:{change_rate:{min:5,max:null},volume_avg20_ratio:{min:200,max:null},vwap_position:'VWAP 위 (강세)',macd_signal:'MACD > Signal (매수)',eom_trend:'매수세 (상승)',recent_n_change:{min:null,max:40},_recent_n_bars:{min:null,max:3}}},
  // ── 준비 ──
  ...
  {id:'p2_coin_bear_env',name:'약세장 방어',phase:'p2',group:'env',desc:'BTC약세 + RSI과매도 + MACD골든 + BB하단 (최근 5봉 윈도우)',locked:true,conditions:{mkt_env_state:'약세 포함 (mild_bear+bear)',rsi_value:{min:20,max:35},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',_recent_n_bars:{min:null,max:5}}},
];
...
const PRESETS_US_P2 = [
  ...
  {id:'p2_us_reversal',name:'과매도 반등',phase:'p2',group:'attack',desc:'RSI 과매도 + MACD 골든 + BB 하단 + 심리도 과매도 (최근 5봉 윈도우)',locked:true,conditions:{rsi_value:{min:20,max:35},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',psycho_value:{min:null,max:25},_recent_n_bars:{min:null,max:5}}},
  {id:'p2_us_breakout',name:'급등 시작',phase:'p2',group:'attack',desc:'20일 신고가 + 거래량 200%↑ + VWAP위 + MACD매수 + EOM매수 + 5봉간 +20% 미만 (이미 급등한 것 제외, 최근 3봉)',locked:true,conditions:{new_high_low:'20일 신고가',volume_avg20_ratio:{min:200,max:null},vwap_position:'VWAP 위 (강세)',macd_signal:'MACD > Signal (매수)',eom_trend:'매수세 (상승)',recent_n_change:{min:null,max:20},_recent_n_bars:{min:null,max:3}}},
  ...
  {id:'p2_us_bear_env',name:'약세장 방어',phase:'p2',group:'env',desc:'시장약세 + RSI과매도 + MACD골든 + BB하단 + PER저평가 + 배당 (최근 5봉 윈도우)',locked:true,conditions:{mkt_env_state:'약세 포함 (mild_bear+bear)',rsi_value:{min:0,max:30},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',per:{min:0,max:15},dividend_yield:{min:2,max:null},_recent_n_bars:{min:null,max:5}}},
];
