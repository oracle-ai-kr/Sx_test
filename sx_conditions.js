...
const SX_CONDITIONS = [
  ...
  {id:'scope',name:'범위지정',phase:'p1',groups:[
    ...
    {id:'scope_exclude',name:'제외 조건',conditions:[
      // [통합정리] 12개 옵션 중 9개가 처리 코드 누락으로 무시되던 문제 해결
      //   - 이름·시장 패턴으로 잡을 수 있는 5종만 종목 유형 제외로 유지
      //   - 관리종목/거래정지/정리매매/투자경고/불성실공시 등은 공시 기반 제외로 통합
      //   - 단기과열·초저유동성은 데이터 소스 부재로 일단 제외 (추후 추가 가능)
...
const COIN_CONDITIONS = [
  ...
  {id:'scope',name:'범위지정',phase:'p1',groups:[
    {id:'scope_basic',name:'기본 정보',conditions:[
      ...
      // [v3.17 DEDUP-FIX] 코인 거래대금 중복 제거
      //   기존: scope_basic의 'trade_amount'(거래대금) + volume_cond의 'trade_amount_24h'(24h 거래대금)
      //         두 조건 모두 워커가 s.tradeAmount 같은 필드를 검사 (sx_scan_worker.js L770, L772)
      //         → 코인은 24시간 시장이라 "당일 거래대금"="24시간 거래대금" 동일 개념
      //         → 사용자에게 두 개의 거래대금 필터가 같은 데이터를 검사하는 혼란 야기
      //   수정: scope_basic의 trade_amount 제거, 24h 거래대금(volume_cond)으로 일원화
      //         (KR은 일중 거래 시간이 한정적이라 trade_amount 유지가 맞음 — 코인만 수정)
    ]},
  ]},
  {id:'price_analysis',name:'시세분석',phase:'p1',groups:[
    {id:'price_cond',name:'가격 조건',conditions:[
  ...
  // [정리] 코인 재무분석 카테고리 제거 (2026-05)
  //   PER/PBR/ROE/배당수익률은 암호화폐에 존재하지 않는 개념.
  //   기존에는 정의만 있고 데이터 소스(yahoo_fundamental)가 한국/미국 주식 전용이라
  //   코인에서 사용 시 rangeCheck(null,...)=false → 모든 코인 탈락하는 버그였음.
  ...
  // [v3.17 ENV-FIX] 코인 시장환경 — NASDAQ/SP500 제거, BTC 등락률 추가
  //   사유:
  //     1) NASDAQ/SP500은 미국 주식 지표 → 코인 시장과 직접 연관성 약함 (간접 영향만 존재)
  //     2) 워커(sx_scan_worker.js L812~814)는 currentMarket==='coin'일 때 env.btc.cr로 시장상태 판정
  //        → UI에 NASDAQ/SP500 등락률 필터를 노출해도 워커가 처리 안 함 (작동 안 하는 조건이었음)
  //     3) 워커 L844에 mkt_env_btc_chg 처리 코드는 이미 존재하나 UI 정의가 누락되어 있었음
  //   변경:
  //     - mkt_env_state desc: 'KOSPI/KOSDAQ 종합' → 'BTC 등락률 기준' (코인 시장 흐름 명시)
  //     - mkt_env_nasdaq_chg, mkt_env_sp500_chg 제거 (코인에서는 무의미)
  //     - mkt_env_btc_chg 신규 추가 (워커가 이미 처리 중이었던 조건의 UI 정의)
  //     - 코인 변동성 반영하여 range를 ±10% → ±20%, recommend 기준도 -2% 하락으로 완화
  {id:'market_env',name:'시장환경',phase:'p2',groups:[
    {id:'env_state',name:'시장 상태',conditions:[
...
const US_CONDITIONS = [
  ...
  {id:'fundamental',name:'재무분석',phase:'p1',groups:[
    {id:'fund_valuation',name:'밸류에이션',conditions:[
      ...
      // [정리] 미국 ROE 제거 (2026-05)
      //   워커의 fetchFinancialData에서 ROE는 DART/네이버 한국 전용 경로에서만 채워짐.
      //   미국은 Yahoo /fundamental 경로를 타는데 ROE를 받아오지 않음 → 항상 null
      //   → rangeCheck(null,...)=false → 모든 미국 종목 탈락하던 버그.
