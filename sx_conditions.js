// ════════════════════════════════════════════════════════════
//  SIGNAL X — Conditions & Presets v2.1 <!-- S80 -->
//  S80: 3단 점수 필터(_ready_score/_entry_score/_trend_score) + 9프리셋(P1지금들어가?/P2맞는거같은데?/P3맞네!!!)
//  S68: 조건트리 3단계 루트 폴더 + 프리셋 Phase별 분리 + 조합 시스템
//  Phase 1: 기본필터 (범위/시세/재무/순위)
//  Phase 2: 엔진기반 (기술/패턴/시장환경)
//  Phase 3: 검증 (엔진판정/BT)
...
// ── 3단계 Phase 정의 ──
const SX_PHASES = [
  {id:'p1', name:'Phase 1', label:'기본 필터', desc:'시장/시세/재무 기반 1차 스크리닝', icon:'🏗️', color:'#4a90d9'},
  {id:'p2', name:'Phase 2', label:'기술 분석', desc:'캔들+지표 엔진 기반 정밀 분석',   icon:'📊', color:'#e8a838'},
  {id:'p3', name:'Phase 3', label:'검증',      desc:'BT + 엔진 판정으로 최종 검증',     icon:'🎯', color:'#50c878'},
];
...
// ── KR 프리셋 ──
const PRESETS_KR_P1 = [
  {id:'p1_kr_large',name:'대형주 기본',phase:'p1',desc:'시총 1000억↑ + 거래대금 500↑ + 위험종목 제외',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:1000,max:null},trade_amount:{min:500,max:null}}},
  {id:'p1_kr_mid',name:'중소형주',phase:'p1',desc:'시총 200~5000억 + 거래대금 300↑',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:200,max:5000},trade_amount:{min:300,max:null}}},
  {id:'p1_kr_value',name:'가치주',phase:'p1',desc:'PER 0~15 + PBR 0~1.5 + ROE 5%↑ + 부채 200↓',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:1000,max:null},per:{min:0,max:15},pbr:{min:0,max:1.5},roe:{min:5,max:null},debt_ratio:{min:0,max:200},trade_amount:{min:500,max:null}}},
  {id:'p1_kr_growth',name:'고성장',phase:'p1',desc:'PEG 0~1.2 + 매출성장 15%↑ + 영업이익 증감 15%↑',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:500,max:null},peg:{min:0,max:1.2},revenue_growth:{min:15,max:null},operating_profit_growth:{min:15,max:null},trade_amount:{min:300,max:null}}},
  {id:'p1_kr_quality',name:'수익성 우량',phase:'p1',desc:'영업이익률 15%↑ + ROE 10%↑ + PSR 0~5',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:1000,max:null},operating_margin:{min:15,max:null},roe:{min:10,max:null},psr:{min:0,max:5},trade_amount:{min:500,max:null}}},
  {id:'p1_kr_roa',name:'고ROA 저부채',phase:'p1',desc:'ROA 5%↑ + 부채 100↓ + 영업이익률 8%↑',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:1000,max:null},roa:{min:5,max:null},debt_ratio:{min:0,max:100},operating_margin:{min:8,max:null},trade_amount:{min:500,max:null}}},
  {id:'p1_kr_foreign',name:'외국인 선호',phase:'p1',desc:'외국인 5일+ 연속 순매수 + 시총 3000억↑',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:3000,max:null},foreign_net_buy_days:{min:5,max:null},trade_amount:{min:500,max:null}}},
  {id:'p1_kr_ready',name:'지금 들어가?',phase:'p1',desc:'준비 점수 60↑ — 반등 준비 상태 감지 (관심 발굴)',locked:true,conditions:{_ready_score:{min:60,max:null}}},
  {id:'p1_kr_dividend',name:'고배당 안정',phase:'p1',desc:'배당 2%↑ + 부채 100↓ + 시총 3000억↑',locked:true,conditions:{exclude_types:['managed','warning','preferred','suspended','cleanup','etf','etn','spac'],market_cap:{min:3000,max:null},dividend_yield:{min:2,max:null},debt_ratio:{min:0,max:100},trade_amount:{min:500,max:null}}},
];
...
const PRESETS_KR_P2 = [
  ...
  {id:'p2_kr_entry_sig',name:'맞는거 같은데?',phase:'p2',desc:'준비 50↑ + 진입 60↑ — 반등 신호 감지 (진입 판단)',locked:true,conditions:{_ready_score:{min:50,max:null},_entry_score:{min:60,max:null}}},
...
const PRESETS_KR_P3 = [
  {id:'p3_kr_confirmed',name:'맞네!!!',phase:'p3',desc:'진입 50↑ + 추세 60↑ — 반등 확인 + 추세 확인 (보유 확인)',locked:true,conditions:{_entry_score:{min:50,max:null},score_range:{min:60,max:null}}},
...
const PRESETS_COIN_P1 = [
  {id:'p1_coin_basic',name:'기본 필터',phase:'p1',desc:'거래대금 500↑',locked:true,conditions:{trade_amount:{min:500,max:null}}},
  {id:'p1_coin_ready',name:'지금 들어가?',phase:'p1',desc:'준비 점수 60↑ — 반등 준비 상태 감지',locked:true,conditions:{_ready_score:{min:60,max:null}}},
  {id:'p1_coin_large',name:'대형 코인',phase:'p1',desc:'시총 1조↑ + 거래대금 1000↑',locked:true,conditions:{market_cap:{min:10000,max:null},trade_amount:{min:1000,max:null}}},
];
...
const PRESETS_COIN_P2 = [
  ...
  {id:'p2_coin_entry_sig',name:'맞는거 같은데?',phase:'p2',desc:'준비 50↑ + 진입 60↑ — 반등 신호 감지',locked:true,conditions:{_ready_score:{min:50,max:null},_entry_score:{min:60,max:null}}},
...
const PRESETS_COIN_P3 = [
  {id:'p3_coin_confirmed',name:'맞네!!!',phase:'p3',desc:'진입 50↑ + 추세 60↑ — 반등 확인',locked:true,conditions:{_entry_score:{min:50,max:null},score_range:{min:60,max:null}}},
...
// ── US 프리셋 ──
const PRESETS_US_P1 = [
  {id:'p1_us_basic',name:'기본 필터',phase:'p1',desc:'기본 스크리닝',locked:true,conditions:{}},
  {id:'p1_us_ready',name:'지금 들어가?',phase:'p1',desc:'준비 점수 60↑ — 반등 준비 상태 감지',locked:true,conditions:{_ready_score:{min:60,max:null}}},
  {id:'p1_us_value',name:'가치주',phase:'p1',desc:'PER 0~15 + PBR 0~1.5 + ROE 5%↑ + 배당 1%↑',locked:true,conditions:{per:{min:0,max:15},pbr:{min:0,max:1.5},roe:{min:5,max:null},dividend_yield:{min:1,max:null}}},
];
...
const PRESETS_US_P2 = [
  ...
  {id:'p2_us_entry_sig',name:'맞는거 같은데?',phase:'p2',desc:'준비 50↑ + 진입 60↑ — 반등 신호 감지',locked:true,conditions:{_ready_score:{min:50,max:null},_entry_score:{min:60,max:null}}},
...
const PRESETS_US_P3 = [
  {id:'p3_us_confirmed',name:'맞네!!!',phase:'p3',desc:'진입 50↑ + 추세 60↑ — 반등 확인',locked:true,conditions:{_entry_score:{min:50,max:null},score_range:{min:60,max:null}}},
