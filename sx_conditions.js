...
const SX_CONDITIONS = [
  ...
  {id:'engine_verdict',name:'엔진 판정',phase:'p3',groups:[
    {id:'ta_signal',name:'분석 판정',conditions:[
      ...
      {axis4:true,id:'_ready_score',name:'반등 신호 점수',type:'range',unit:'',min:0,max:100,default:{min:null,max:null},source:'calc_candle',desc:'반등 신호 0~100 (과매도+수축+눌림) - 50↑=신호 발현 시작',recommend:'50↑ (반등 신호) / 70↑ (강한 신호)'},
      {axis4:true,id:'_entry_score',name:'반등 강도 점수',type:'range',unit:'',min:0,max:100,default:{min:null,max:null},source:'calc_candle',desc:'반등 강도 0~100 (RSI반등+MACD전환) - 진입 타이밍 판단',recommend:'50↑ (진입 가능) / 70↑ (강력 진입)'},
      ...
      {axis4:true,id:'_engine_buy',name:'매수 배지',type:'select',options:['설정안함','매수 판정'],default:'설정안함',source:'calc_candle',desc:'엔진 4축 판정 결과가 매수인 종목 - 결과탭 상단 🟢매수 배지와 동일',recommend:'매수 판정 → 4축 전부 합격 종목만 필터'},
    ]},
  ]},
  {id:'backtest',name:'백테스트',phase:'p3',groups:[
    {id:'ta_bt',name:'백테스트 (실시간)',conditions:[
      ...
      {axis4:true,id:'_bt_buy_marker',name:'BT 매수마커',type:'select',options:['설정안함','매수마커 있음'],default:'설정안함',source:'calc_candle',desc:'BT엔진이 오늘 매수 신호를 낸 종목 - 차트에 ▲마커 표시된 상태 (_isBuySignal=true)',recommend:'매수마커 있음 → 당일 신규 매수 진입 신호 종목만 필터'},
    ]},
  ]},
];
