...
const SX_CONDITIONS = [
  ...
  {id:'engine_verdict',name:'엔진 판정',phase:'p3',groups:[
    {id:'ta_signal',name:'분석 판정',conditions:[
      ...
      {axis4:'buy',id:'_c_buy_marker',name:'매수마커 ▲',type:'select',options:['설정안함','매수마커 있음'],default:'설정안함',source:'calc_candle',desc:'C판정 보라 매수마커(▲) — 신규 매수(기존 매수 배지 포괄) + 보유중 추가진입. 차트 보라 ▲와 동일 (svVerdict.chartMarker=buy)',recommend:'매수마커 있음 → 신규진입·추가진입 후보 (4축 매수 판정 포괄)'},
      {axis4:'sell',id:'_c_sell_marker',name:'매도마커 ▼',type:'select',options:['설정안함','매도마커 있음'],default:'설정안함',source:'calc_candle',desc:'C판정 보라 매도마커(▼) — 청산/매도완료 + 보유중 조기청산·부분익절. 차트 보라 ▼와 동일 (svVerdict.chartMarker=sell)',recommend:'매도마커 있음 → 청산·부분익절 검토 후보'},
    ]},
  ]},
