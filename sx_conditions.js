...
const SX_CONDITIONS = [
  ...
  {id:'technical',name:'기술적분석',phase:'p2',groups:[
    {id:'ta_ma',name:'이동평균선',conditions:[
      // [v3.10] 최근 N봉 윈도우 — 모든 기술적분석/패턴 조건의 평가 대상 봉 범위 제한.
      //   기본 1봉(=마지막 봉만, 기존 동작 호환). 5봉 입력 시 "최근 5봉 중 한 봉이라도
      //   조건을 동시 충족하면 통과". 1년 전 발생한 골든크로스가 정배열 유지로
      //   계속 잡히던 문제 해결. 처리 위치: sx_scan_worker.js 스캔 루프.
      {id:'_recent_n_bars',name:'최근 N봉',type:'range',unit:'봉',min:1,max:60,default:{min:1,max:1},source:'calc_candle',desc:'조건 평가 대상 범위. MA 크로스: N봉 사이 어딘가에 교차 발생 (오늘=N2, 일주일=N7~10, 한달=N20~25). 그 외 조건: N봉 중 한 봉이라도 동시 충족 시 통과',recommend:'1~3 (오늘) / 5~10 (일주일) / 20~25 (한 달)'},
      {id:'ma_cross',name:'MA 크로스',type:'select',options:['설정안함','골든크로스 (5×20)','데드크로스 (5×20)','골든크로스 (20×60)','데드크로스 (20×60)'],default:'설정안함',source:'calc_candle',desc:'단기 이동평균이 장기를 돌파 - 골든크로스=상승 신호, 데드크로스=하락 신호'},
    ...
    {id:'ta_trend',name:'추세지표',conditions:[
      {id:'macd_signal',name:'MACD 시그널',type:'select',options:['설정안함','MACD > Signal (매수)','MACD < Signal (매도)','골든크로스 (3일 이내)','데드크로스 (3일 이내)'],default:'설정안함',source:'calc_candle',desc:'MACD vs 시그널선 - 매수=상승 모멘텀, 골든크로스=최근 전환'},
      ...
      {id:'eom_trend',name:'EOM (이동용이도)',type:'select',options:['설정안함','매수세 (상승)','매도세 (하락)','골든크로스','데드크로스'],default:'설정안함',source:'calc_candle',desc:'가격 움직임의 용이도 - 거래량 대비 가격 변동 (양수=쉽게 상승)'},
    ...
    {id:'ta_momentum',name:'모멘텀지표',conditions:[
      ...
      {id:'stoch_cross',name:'Stochastic 크로스',type:'select',options:['설정안함','%K > %D (매수)','%K < %D (매도)','골든크로스 (과매도권)','데드크로스 (과매수권)'],default:'설정안함',source:'calc_candle',desc:'스토캐스틱 신호선 교차 - 과매도권 골든크로스=강한 매수 신호'},
      ...
      {id:'chaikin_osc',name:'Chaikin Oscillator',type:'select',options:['설정안함','양수 (매집)','음수 (분산)','골든크로스','데드크로스'],default:'설정안함',source:'calc_candle',desc:'A/D선 모멘텀 - 양수=세력 매집중, 음수=분산중'},
    ]},
    ...
    {id:'ta_ichimoku',name:'일목균형표',conditions:[
      ...
      {id:'ichimoku_cross',name:'전환선/기준선',type:'select',options:['설정안함','전환선 > 기준선 (매수)','전환선 < 기준선 (매도)'],default:'설정안함',source:'calc_candle',desc:'단기-중기 신호 - 골든크로스=매수, 데드크로스=매도'},
      {id:'ichimoku_cloud_change',name:'구름 전환',type:'select',options:['설정안함','구름 양전환 (적→녹)','구름 음전환 (녹→적)'],default:'설정안함',source:'calc_candle',desc:'미래 구름 색상 변화 - 양전환=대전환 신호, 음전환=하락 신호'},
    ]},
    {id:'ta_volume',name:'거래량 지표',conditions:[
      ...
      {id:'ad_trend',name:'A/D선 추세',type:'select',options:['설정안함','상승 (매집)','하락 (분산)','매집 다이버전스 (가격↓ A/D↑)'],default:'설정안함',source:'calc_candle',desc:'누적 분산선 - 매집 다이버전스=가격 하락에도 매집 (반전 신호)'},
      {id:'volume_avg20_ratio',name:'20일 평균 대비 거래량',type:'range',unit:'%',min:0,max:1000,default:{min:null,max:null},source:'calc_candle',desc:'당일 거래량 ÷ 20일 평균 ×100 (100=평균, 200=2배, 300=3배)',recommend:'150↑ (활발) / 200↑ (거래량 폭발)'},
      {id:'volume_consec_inc',name:'거래량 연속 증가',type:'range',unit:'일',min:0,max:30,default:{min:null,max:null},source:'calc_candle',desc:'직전 봉 대비 거래량 증가 연속 일수 - 매집/관심도 측정',recommend:'3↑ (관심 증가) / 5↑ (강한 매집)'},
    ]},
    {id:'ta_price_action',name:'시세 액션',conditions:[
      {id:'new_high_low',name:'신고가/신저가',type:'select',options:['설정안함','52주 신고가','20일 신고가','52주 신저가','20일 신저가'],default:'설정안함',source:'calc_candle',desc:'기간 내 신고가/신저가 발생 - 신고가=강한 매수세, 신저가=약세 가속'},
      {id:'gap_type',name:'갭 발생',type:'select',options:['설정안함','상승갭 종목','하락갭 종목','갭상승 후 지지','갭하락 후 저항','상승갭 제외'],default:'설정안함',source:'calc_candle',desc:'시가 갭 패턴 - 갭상승 후 지지=상승 흐름 유지 (강력)'},
      {id:'intraday_range',name:'당일 변동폭',type:'range',unit:'%',min:0,max:50,default:{min:null,max:null},source:'calc_candle',desc:'(고가-저가)÷시가×100 - 당일 변동성 측정',recommend:'3↑ (활발) / 8↑ (강한 변동)'},
      {id:'period_change',name:'5일간 등락률',type:'range',unit:'%',min:-50,max:100,default:{min:null,max:null},source:'calc_candle',desc:'5일 전 대비 현재가 등락률 - 단기 모멘텀 측정',recommend:'10↑ (단기 상승) / 15↑ (강한 모멘텀)'},
      {id:'price_vs_ma',name:'이동평균선 대비 위치',type:'select',options:['설정안함','MA5 위','MA5 아래','MA20 위','MA20 아래','MA60 위','MA60 아래','MA120 위','MA120 아래'],default:'설정안함',source:'calc_candle',desc:'현재가가 특정 MA 위/아래 - MA60 위=중장기 상승추세'},
      {id:'consecutive_up_down',name:'연속 상승/하락 일수',type:'range',unit:'일',min:-20,max:20,default:{min:null,max:null},source:'calc_candle',desc:'연속 상승(+) 또는 하락(-) 일수',recommend:'2↑ (상승 흐름) / -2↓ (하락 가속)'},
    ]},
    {id:'ta_momentum_ext',name:'모멘텀 확장',conditions:[
      {id:'trix_signal',name:'TRIX',type:'select',options:['설정안함','골든크로스','데드크로스','양수 (상승추세)','음수 (하락추세)'],default:'설정안함',source:'calc_candle',desc:'3중 EMA 모멘텀 - 노이즈 적은 추세 지표'},
      {id:'stoch_slow_cross',name:'Stochastic Slow',type:'select',options:['설정안함','골든크로스 (과매도권)','데드크로스 (과매수권)'],default:'설정안함',source:'calc_candle',desc:'스토캐스틱 평활화 - 일반 Stoch보다 부드러운 신호'},
      {id:'macd_osc',name:'MACD Oscillator',type:'select',options:['설정안함','양수 (매수세)','음수 (매도세)','반전 상승','반전 하락'],default:'설정안함',source:'calc_candle',desc:'MACD-Signal 차이 - 반전=모멘텀 변화 시작'},
    ...
    {id:'ta_other',name:'기타 지표',conditions:[
      ...
      {id:'tlb_direction',name:'삼선전환도',type:'select',options:['설정안함','양전환 (상승)','음전환 (하락)'],default:'설정안함',source:'calc_candle',desc:'Three Line Break - 추세 전환 시점 정확히 포착'},
      ...
      {id:'sonar_signal',name:'Sonar 모멘텀',type:'select',options:['설정안함','매수 신호','매도 신호'],default:'설정안함',source:'calc_candle',desc:'25일 가격 모멘텀 - 추세 시작 포착'},
  ...
  {id:'market_env',name:'시장환경',phase:'p2',groups:[
    {id:'env_state',name:'시장 상태',conditions:[
      {id:'mkt_env_state',name:'시장 환경',type:'select',options:['설정안함','강세 포함 (mild_bull+bull)','강세 (bull)','약세강세 (mild_bull)','중립 (neutral)','약세약세 (mild_bear)','약세 (bear)','약세 포함 (mild_bear+bear)'],default:'설정안함',source:'oracle_index',desc:'KOSPI/KOSDAQ 종합 - 강세장만 매수, 약세장 회피용'},
    ]},
  ...
  {id:'engine_verdict',name:'엔진 판정',phase:'p3',groups:[
    {id:'ta_signal',name:'분석 판정',conditions:[
      {id:'score_range',name:'추세 방향 점수',type:'range',unit:'',min:0,max:100,default:{min:null,max:null},source:'calc_candle',desc:'추세 방향 0~100 (분석탭 "추세 방향") - 50↑=상승, 70↑=강한 상승',recommend:'50↑ (상승 추세) / 70↑ (강한 상승)'},
      {id:'_ready_score',name:'반등 신호 점수',type:'range',unit:'',min:0,max:100,default:{min:null,max:null},source:'calc_candle',desc:'반등 신호 0~100 (과매도+수축+눌림) - 50↑=신호 발현 시작',recommend:'50↑ (반등 신호) / 70↑ (강한 신호)'},
      {id:'_entry_score',name:'반등 강도 점수',type:'range',unit:'',min:0,max:100,default:{min:null,max:null},source:'calc_candle',desc:'반등 강도 0~100 (RSI반등+MACD전환) - 진입 타이밍 판단',recommend:'50↑ (진입 가능) / 70↑ (강력 진입)'},
  ...
  {id:'backtest',name:'백테스트',phase:'p3',groups:[
    {id:'ta_bt',name:'백테스트 (실시간)',conditions:[
      ...
      {id:'_dir_mom',name:'방향 전이',type:'select',options:['설정안함','상승 전이중','하락 전이중','횡보'],default:'설정안함',source:'calc_candle',desc:'A분석 모멘텀 방향 - 상승 전이중=함정 진입 방어 효과'},
    ]},
  ]},
];
...
const COIN_CONDITIONS = [
  ...
  {id:'technical',name:'기술적분석',phase:'p2',groups:[
    ...
    {id:'ta_trend',name:'추세지표',conditions:[
      {id:'macd_signal',name:'MACD 시그널',type:'select',options:['설정안함','MACD > Signal (매수)','MACD < Signal (매도)'],default:'설정안함',source:'calc_candle',desc:'MACD vs 시그널선 - 매수=상승 모멘텀, 골든크로스=최근 전환'},
      {id:'adx_value',name:'ADX (추세강도)',type:'range',unit:'',min:0,max:100,default:{min:null,max:null},source:'calc_candle',desc:'추세의 강도 (방향 무관) - 25↑=추세 형성, 40↑=강한 추세, 20↓=횡보',recommend:'25↑ (추세 형성) / 40↑ (강한 추세)'},
      {id:'parabolic_sar',name:'Parabolic SAR',type:'select',options:['설정안함','SAR 아래 (상승 추세)','SAR 위 (하락 추세)'],default:'설정안함',source:'calc_candle',desc:'추세 추종 지표 - SAR 아래=상승, SAR 위=하락 (포지션 청산 신호)'},
      {id:'eom_trend',name:'EOM (이동용이도)',type:'select',options:['설정안함','매수세 (상승)','매도세 (하락)','골든크로스','데드크로스'],default:'설정안함',source:'calc_candle',desc:'가격 움직임의 용이도 - 거래량 대비 가격 변동 (양수=쉽게 상승)'},
      {id:'vhf_state',name:'VHF (추세판별)',type:'select',options:['설정안함','추세장 (>0.4)','횡보장 (<0.3)','보통'],default:'설정안함',source:'calc_candle',desc:'추세 vs 횡보 판별 - 추세장=방향성 매매, 횡보장=박스권 매매'},
  ...
  ],recommend:'1~3 (오늘) / 5~10 (일주일) / 20~25 (한 달)'},
  {id:'pattern',name:'패턴분석',phase:'p2',groups:[
    {id:'pat_basic',name:'기본 캔들',conditions:[
      {id:'candle_type',name:'캔들 유형',type:'multi_check',options:[
        {id:'long_yang',name:'장대양봉'},{id:'long_eum',name:'장대음봉'},{id:'doji',name:'도지 (십자)'},
...
const US_CONDITIONS = [
  ...
  {id:'technical',name:'기술적분석',phase:'p2',groups:[
    {id:'ta_ma',name:'이동평균선',conditions:[
      ...
      {id:'_recent_n_bars',name:'최근 N봉',type:'range',unit:'봉',min:1,max:60,default:{min:1,max:1},source:'calc_candle',desc:'조건 평가 대상 범위. MA 크로스: N봉 사이 어딘가에 교차 발생 (오늘=N2, 일주일=N7~10, 한달=N20~25). 그 외 조건: N봉 중 한 봉이라도 동시 충족 시 통과',recommend:'1~3 (오늘) / 5~10 (일주일) / 20~25 (한 달)'},
      {id:'ma_cross',name:'MA 크로스',type:'select',options:['설정안함','골든크로스 (5×20)','데드크로스 (5×20)','골든크로스 (20×60)','데드크로스 (20×60)'],default:'설정안함',source:'calc_candle',desc:'단기 이동평균이 장기를 돌파 - 골든크로스=상승 신호, 데드크로스=하락 신호'},
      {id:'ma_arrangement',name:'MA 배열',type:'select',options:['설정안함','정배열 (3개)','역배열 (3개)'],default:'설정안함',source:'calc_candle',desc:'단기>중기>장기 순서 - 정배열=상승추세, 역배열=하락추세 (4개=더 강한 신호)'},
    ]},
    {id:'ta_trend',name:'추세지표',conditions:[
      {id:'macd_signal',name:'MACD 시그널',type:'select',options:['설정안함','MACD > Signal (매수)','MACD < Signal (매도)'],default:'설정안함',source:'calc_candle',desc:'MACD vs 시그널선 - 매수=상승 모멘텀, 골든크로스=최근 전환'},
      {id:'adx_value',name:'ADX (추세강도)',type:'range',unit:'',min:0,max:100,default:{min:null,max:null},source:'calc_candle',desc:'추세의 강도 (방향 무관) - 25↑=추세 형성, 40↑=강한 추세, 20↓=횡보',recommend:'25↑ (추세 형성) / 40↑ (강한 추세)'},
...
const PRESETS_KR_P2 = [
  ...
  {id:'p2_kr_bottom_pick',name:'바닥 후보',phase:'p2',group:'defense',desc:'반등 신호 70↑ + 반등 강도 70↑ + RSI 30↓ (과매도+반등 동력)',locked:true,conditions:{_ready_score:{min:70,max:100},_entry_score:{min:70,max:100},rsi_value:{min:0,max:30}}},
  // ── 환경 ──
  ...
  {id:'p2_kr_bear_env',name:'약세장 방어',phase:'p2',group:'env',desc:'시장약세 + RSI반등 + MACD골든 + BB하단 (최근 5봉 윈도우)',locked:true,conditions:{mkt_env_state:'약세 포함 (mild_bear+bear)',rsi_value:{min:25,max:40},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',_recent_n_bars:{min:5,max:5}}},
];
...
const PRESETS_KR_P3 = [
  ...
  {id:'p3_kr_c_buy',  name:'매수',phase:'p3',desc:'C판정 매수 + 4축 합격 + 상승 전이중 — 신규 진입 최적 후보',locked:true,conditions:{_ready_score:{min:60,max:100},_entry_score:{min:60,max:100},score_range:{min:50,max:100},_bt_score:{min:60,max:100},_dir_mom:'상승 전이중',_bt_action:'매수'}},
  {id:'p3_kr_c_watch',name:'관심',phase:'p3',desc:'C판정 관심 + 4축 부분 합격 + 상승 전이중 — 선행 포착 후보',locked:true,conditions:{_ready_score:{min:50,max:100},_entry_score:{min:50,max:100},score_range:{min:40,max:100},_bt_score:{min:50,max:100},_dir_mom:'상승 전이중',_bt_action:'관심'}},
  {id:'p3_kr_c_observe',name:'관망',phase:'p3',desc:'C판정 관망 + 4축 약 합격 + 상승 전이중 — 추가 모멘텀 대기 후보',locked:true,conditions:{_ready_score:{min:40,max:100},_entry_score:{min:40,max:100},score_range:{min:30,max:100},_bt_score:{min:40,max:100},_dir_mom:'상승 전이중',_bt_action:'관망'}},
];
...
const PRESETS_COIN_P2 = [
  ...
  {id:'p2_coin_bottom_pick',name:'바닥 후보',phase:'p2',group:'defense',desc:'반등 신호 70↑ + 반등 강도 70↑ + RSI 30↓ (과매도+반등 동력)',locked:true,conditions:{_ready_score:{min:70,max:100},_entry_score:{min:70,max:100},rsi_value:{min:0,max:30}}},
  // ── 환경 ──
  ...
  {id:'p2_coin_bear_env',name:'약세장 방어',phase:'p2',group:'env',desc:'BTC약세 + RSI과매도 + MACD골든 + BB하단 (최근 5봉 윈도우)',locked:true,conditions:{mkt_env_state:'약세 포함 (mild_bear+bear)',rsi_value:{min:20,max:35},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',_recent_n_bars:{min:5,max:5}}},
];

const PRESETS_COIN_P3 = [
  // [v2.6] 4축 점수 + 방향 전이 + 종합행동지침 6조건 프리셋
  //   KR과 동일 기준 — 점수/모멘텀/판정은 시장 무관하게 정규화되어 동일 임계 적용
  {id:'p3_coin_c_buy',  name:'매수',phase:'p3',desc:'C판정 매수 + 4축 합격 + 상승 전이중 — 신규 진입 최적 후보',locked:true,conditions:{_ready_score:{min:60,max:100},_entry_score:{min:60,max:100},score_range:{min:50,max:100},_bt_score:{min:60,max:100},_dir_mom:'상승 전이중',_bt_action:'매수'}},
  {id:'p3_coin_c_watch',name:'관심',phase:'p3',desc:'C판정 관심 + 4축 부분 합격 + 상승 전이중 — 선행 포착 후보',locked:true,conditions:{_ready_score:{min:50,max:100},_entry_score:{min:50,max:100},score_range:{min:40,max:100},_bt_score:{min:50,max:100},_dir_mom:'상승 전이중',_bt_action:'관심'}},
  {id:'p3_coin_c_observe',name:'관망',phase:'p3',desc:'C판정 관망 + 4축 약 합격 + 상승 전이중 — 추가 모멘텀 대기 후보',locked:true,conditions:{_ready_score:{min:40,max:100},_entry_score:{min:40,max:100},score_range:{min:30,max:100},_bt_score:{min:40,max:100},_dir_mom:'상승 전이중',_bt_action:'관망'}},
];
...
const PRESETS_US_P2 = [
  ...
  // [v1.9-1] 바닥 후보 — 점수 시스템 기반 종합 바닥 발굴. 분석 점수만 사용 → BT 무관 모든 TF 활성
  {id:'p2_us_bottom_pick',name:'바닥 후보',phase:'p2',group:'defense',desc:'반등 신호 70↑ + 반등 강도 70↑ + RSI 30↓ (과매도+반등 동력)',locked:true,conditions:{_ready_score:{min:70,max:100},_entry_score:{min:70,max:100},rsi_value:{min:0,max:30}}},
  // ── 환경 ──
  ...
  {id:'p2_us_bear_env',name:'약세장 방어',phase:'p2',group:'env',desc:'시장약세 + RSI과매도 + MACD골든 + BB하단 + PER저평가 + 배당 (최근 5봉 윈도우)',locked:true,conditions:{mkt_env_state:'약세 포함 (mild_bear+bear)',rsi_value:{min:0,max:30},macd_signal:'골든크로스 (3일 이내)',bb_position:'하단 이탈',per:{min:0,max:15},dividend_yield:{min:2,max:null},_recent_n_bars:{min:5,max:5}}},
];

const PRESETS_US_P3 = [
  // [v2.6] 4축 점수 + 방향 전이 + 종합행동지침 6조건 프리셋
  //   KR/COIN과 동일 기준 — 점수/모멘텀/판정은 시장 무관하게 정규화되어 동일 임계 적용
  {id:'p3_us_c_buy',  name:'매수',phase:'p3',desc:'C판정 매수 + 4축 합격 + 상승 전이중 — 신규 진입 최적 후보',locked:true,conditions:{_ready_score:{min:60,max:100},_entry_score:{min:60,max:100},score_range:{min:50,max:100},_bt_score:{min:60,max:100},_dir_mom:'상승 전이중',_bt_action:'매수'}},
  {id:'p3_us_c_watch',name:'관심',phase:'p3',desc:'C판정 관심 + 4축 부분 합격 + 상승 전이중 — 선행 포착 후보',locked:true,conditions:{_ready_score:{min:50,max:100},_entry_score:{min:50,max:100},score_range:{min:40,max:100},_bt_score:{min:50,max:100},_dir_mom:'상승 전이중',_bt_action:'관심'}},
  {id:'p3_us_c_observe',name:'관망',phase:'p3',desc:'C판정 관망 + 4축 약 합격 + 상승 전이중 — 추가 모멘텀 대기 후보',locked:true,conditions:{_ready_score:{min:40,max:100},_entry_score:{min:40,max:100},score_range:{min:30,max:100},_bt_score:{min:40,max:100},_dir_mom:'상승 전이중',_bt_action:'관망'}},
];
