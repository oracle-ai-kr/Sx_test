...
(function(){
  ...
  function run(rows, tf, market, opts){
    ...
    for(var e = warmup; e <= lastE; e += step){
      ...
      scoreRecs.push(_ax);
      // [S721] 재료 ablation 연동 — opts.ablate 시 봉별 ready 분해(SXUV 미러). 드리프트가드: 미러 score≠실제 readyScore면 null.
      var _rab = null;
      if(opts.ablate && records && G.SXUV && G.SXUV.readyBreakdown){
        try{ var _rb = G.SXUV.readyBreakdown(qs.ind); if(_rb && _rb.score === (qs.readyScore||0)) _rab = _rb.bd.map(function(b){ return b.scoreWithout; }); else _driftN++; }catch(_e){ _rab = null; _driftN++; }
      }
      if(records){ var _rec = { p:pass, path:path, lt:(sc.ltAlign||'off'), act:act, ret:retC, retO:retO, cap:_capped?1:0, viol:_hasViol?1:0, rd:_ax.rd, en:_ax.en, tr:_ax.tr, up:_ax.up }; if(_rab) _rec.rab = _rab; records.push(_rec); } // [S691] lt=ltAlign · [S721] rab=재료 ablation
    }
  ...
  // [S721] 재료 ablation IC — ready 재료를 하나씩 빼면 readyScore IC가 어떻게 변하나(ΔIC). 전체/강세봉(bull)/약세봉(bear).
  //   records = run({raw:true, ablate:true}) 모음(rab 필요). Δ>0=빼면 IC↑(노이즈/해로움) · Δ<0=기여. baseline=rd(전체 readyScore) IC. 익일(retO) 기준.
  function ablateIC(records){
    if(!Array.isArray(records)) return { ok:false, reason:'records 없음' };
    var feats = (G.SXUV && G.SXUV.READY_FEATS) ? G.SXUV.READY_FEATS : null;
    if(!feats) return { ok:false, reason:'SXUV.READY_FEATS 미로드' };
    var recs = records.filter(function(r){ return r.rab && r.rab.length === feats.length; });
    if(recs.length < 12) return { ok:false, reason:'재료분해 표본부족(' + recs.length + '·미러 드리프트 가능)', n:recs.length };
    function icSet(rs){
      if(rs.length < 12) return null;
      var base = _spearman(rs, 'rd', 'retO');
      var items = feats.map(function(f, i){
        var ic = _spearman(rs, function(r){ return r.rab[i]; }, 'retO');
        return { key:f, ic:ic, d:(ic != null && base != null) ? Math.round((ic - base) * 1000) / 1000 : null };
      });
      return { n:rs.length, base:base, items:items };
    }
    return {
      ok:true, n:recs.length, feats:feats,
      all:  icSet(recs),
