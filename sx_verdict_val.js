...
(function(){
  ...
  // [S681] 점수 신뢰성 — 축점수↔이후수익 순위상관(Spearman IC). recs:[{rd,en,tr,up,c,o}], key=축 또는 [S721]접근자함수, fill='c'|'o'.
  function _spearman(recs, key, fill){
    var n = recs.length; if(n < 10) return null;
    var gk = (typeof key === 'function') ? key : function(r){ return r[key]; };   // [S721] 함수면 접근자(ablated 점수)
    var ix = []; for(var i=0;i<n;i++) ix.push(i);
    var bs = ix.slice().sort(function(a,b){ return gk(recs[a])-gk(recs[b]); });
  ...
  function run(rows, tf, market, opts){
    ...
    var allRets = [], scoreRecs = [], nPts = 0, errN = 0, _driftN = 0;   // [S681] scoreRecs · [S721] _driftN=미러 드리프트 봉수
    ...
    for(var e = warmup; e <= lastE; e += step){
      ...
      try{
        ...
        if(!sc) { errN++; continue; }
        if(opts.recipeSig){ try{ var _rsg = opts.recipeSig(qs, slice, e); if(_rsg) sc.recipeSig = _rsg; }catch(_eRs){} }   // [S859] 레시피표 A/B — 풀링 호출측 콜백 주입(봉별 발동 평가·qs.ind 재사용=추가비용 저)
        if(opts.cv2){ try{ var _c2v = opts.cv2(qs, slice, e, sc.recipeSig); if(_c2v) sc.cv2 = _c2v; }catch(_eC2){} }   // [S908] cv2 판정 A/B — 라이브(S896)와 동일 주입. 콜백이 null이면 엔진 경로(토글 OFF 반영)
      ...
      scoreRecs.push(_ax);
      // [S721/722/723] 재료 ablation 연동 — opts.ablate 시 봉별 축 분해(SXUV 미러). 드리프트가드: 미러 score≠실제 점수면 제외.
      var _rab = null, _uab = null, _tab = null, _eab = null;
      if(opts.ablate && records && G.SXUV){
        var _S = G.SXUV, _ind = qs.ind, _tsc = (qs.trendScore != null ? qs.trendScore : (qs.score || 0));
        try{ if(_S.readyBreakdown){ var _rb = _S.readyBreakdown(_ind); if(_rb && _rb.score === (qs.readyScore||0)) _rab = _rb.bd.map(function(b){ return b.scoreWithout; }); else _driftN++; } }catch(_e1){}
        try{ if(_S.upsideBreakdown){ var _ub = _S.upsideBreakdown(_ind); if(_ub && _ub.score === (qs.upsideScore||0)) _uab = _ub.bd.map(function(b){ return b.scoreWithout; }); else _driftN++; } }catch(_e2){}
        try{ if(_S.trendBreakdown){ var _tb = _S.trendBreakdown(_ind); if(_tb && _tb.score === _tsc) _tab = _tb.bd.map(function(b){ return b.scoreWithout; }); else _driftN++; } }catch(_e3){}
        try{ if(_S.entryBreakdown){ var _eb = _S.entryBreakdown(_ind); if(_eb && _eb.score === (qs.entryScore||0)) _eab = _eb.bd.map(function(b){ return b.scoreWithout; }); else _driftN++; } }catch(_e4){}
      }
      var _sb5x9 = 0, _sb5x20 = 0; if(e >= 8){ var _s5=0,_s9=0,_q; for(_q=e-4;_q<=e;_q++) _s5+=closes[_q]; for(_q=e-8;_q<=e;_q++) _s9+=closes[_q]; _sb5x9 = ((_s5/5) > (_s9/9)) ? 1 : 0; if(e >= 19){ var _s20=0; for(_q=e-19;_q<=e;_q++) _s20+=closes[_q]; _sb5x20 = ((_s5/5) > (_s20/20)) ? 1 : 0; } } // [S725-측정] MA5>MA9(5×9)·MA5>MA20(5×20)
      if(records){ var _rec = { p:pass, path:path, lt:(sc.ltAlign||'off'), sa:(sc.maAlignBull?1:0), sb:_sb5x9, s20:_sb5x20, act:act, ret:retC, retO:retO, cap:_capped?1:0, viol:_hasViol?1:0, rd:_ax.rd, en:_ax.en, tr:_ax.tr, up:_ax.up }; if(_rab) _rec.rab = _rab; if(_uab) _rec.uab = _uab; if(_tab) _rec.tab = _tab; if(_eab) _rec.eab = _eab; records.push(_rec); } // [S691] lt · [S721-723] rab/uab/tab/eab · [S725-측정] sa=5>20>60 · sb=5×9 · s20=5×20
    }
    ...
    var SCORE_AXES = [{k:'rd',label:'반등준비'},{k:'en',label:'반등전환'},{k:'tr',label:'추세방향'},{k:'up',label:'추가상승'}]; // [S690] 카드 라벨 동기화(ready=반등준비/entry=반등전환/trend=추세방향)
  ...
  function pool(records){
    ...
    var AX = [{k:'rd',label:'반등준비'},{k:'en',label:'반등전환'},{k:'tr',label:'추세방향'},{k:'up',label:'추가상승'}]; // [S690] 카드 라벨 동기화
    ...
    // [S690] 경로 분할 검증 — C는 경로별 다른 축 사용(rebound:반등준비+반등전환+추세 / trend:추세+추가상승). 섞으면 안 쓰는 경로 축 IC가 오염.
    //   경로별로 활성 축만 IC + 경로 수익(익일) + 경로내 등급격차 → 각 축을 '쓰이는 경로에서' 격리 평가 + 분기 결정 검증.
    var PATH_ACT = { rebound:['rd','en','tr'], trend:['tr','up'] }, AXLBL = { rd:'반등준비', en:'반등전환', tr:'추세방향', up:'추가상승' };
    ...
    // [S691] 경로 게이트 실험 — bull-rebound이 등급역전(-2.12p)의 원인인지 측정. rebound을 ltAlign(bull/bear/기타)로 분해.
    //   bull-rebound 격차가 음(<非bull)이면 "강세 장기배열 종목은 rebound 금지"가 정당. upIC(추가상승)=trend 재라우팅 가치(+면 trend로, −면 none).
    function _gateCell(filt){
      var rs = records.filter(function(r){ return r.path === 'rebound' && filt(r); });
      var en = rs.filter(function(r){ return r.p >= 3; }), av = rs.filter(function(r){ return r.p <= 1; });
      var eo = en.length ? _stat(en, 'retO').ret : null, ao = av.length ? _stat(av, 'retO').ret : null;
      return { n:rs.length, enN:en.length, avN:av.length,
        gap:(eo != null && ao != null) ? Math.round((eo - ao) * 100) / 100 : null,
        eo:(eo != null) ? Math.round(eo * 100) / 100 : null, ao:(ao != null) ? Math.round(ao * 100) / 100 : null,
        upIC:rs.length >= 15 ? _spearman(rs, 'up', 'retO') : null, retO:rs.length ? _stat(rs, 'retO').ret : null };
    }
    var gate = { bull:_gateCell(function(r){ return r.lt === 'bull'; }), nonbull:_gateCell(function(r){ return r.lt !== 'bull'; }), bear:_gateCell(function(r){ return r.lt === 'bear'; }), off:_gateCell(function(r){ return r.lt !== 'bull' && r.lt !== 'bear'; }), bearSaO:_gateCell(function(r){ return r.lt === 'bear' && r.sa === 1; }), bearSaX:_gateCell(function(r){ return r.lt === 'bear' && r.sa !== 1; }), bearSbO:_gateCell(function(r){ return r.lt === 'bear' && r.sb === 1; }), bearSbX:_gateCell(function(r){ return r.lt === 'bear' && r.sb !== 1; }), bearS20O:_gateCell(function(r){ return r.lt === 'bear' && r.s20 === 1; }), bearS20X:_gateCell(function(r){ return r.lt === 'bear' && r.s20 !== 1; }), afterSaGate:_gateCell(function(r){ return !(r.lt === 'bear' && r.sa !== 1); }), afterSbGate:_gateCell(function(r){ return !(r.lt === 'bear' && r.sb !== 1); }), afterS20Gate:_gateCell(function(r){ return !(r.lt === 'bear' && r.s20 !== 1); }) }; // [S692] off=중립 · [S725-측정] bearSa/Sb/S20=역배열 rebound를 5>20>60·5×9·5×20로 분해 · afterXGate=그 단기약세 차단 시 rebound 전체
    return {
      ok:true, n:records.length,
      axes: axes,
      byPath: byPath,   // [S690] 경로별 활성 축 IC + 경로 수익·등급격차
      gate: gate,       // [S691] bull-rebound 게이트 실험(ltAlign 분해)
  ...
  // [S721/722] 재료 ablation IC — 축 재료를 하나씩 빼면 그 축 IC가 어떻게 변하나(ΔIC). 전체/강세봉(bull)/약세봉(bear).
  //   records=run({raw:true,ablate:true}) 모음(rab/uab/tab 필요). axisKey=rd(반등준비)/up(추가상승)/tr(추세방향).
  //   Δ>0=빼면 IC↑(노이즈/해로움)·Δ<0=기여. baseline=그 축 전체점수 IC. 익일(retO) 기준. 경로 필터는 호출측에서 records.filter.
  var _ABL_CFG = {
    rd: { sc:'rd', ab:'rab', label:'반등준비', feats:function(){ return (G.SXUV && G.SXUV.READY_FEATS) || null; } },
    en: { sc:'en', ab:'eab', label:'반등전환', feats:function(){ return (G.SXUV && G.SXUV.ENTRY_FEATS) || null; } },
    up: { sc:'up', ab:'uab', label:'추가상승', feats:function(){ return (G.SXUV && G.SXUV.UPSIDE_FEATS) || null; } },
    tr: { sc:'tr', ab:'tab', label:'추세방향', feats:function(){ return (G.SXUV && G.SXUV.TREND_FEATS) || null; } }
  };
  function ablateIC(records, axisKey){
    axisKey = axisKey || 'rd';
    var cfg = _ABL_CFG[axisKey]; if(!cfg) return { ok:false, reason:'unknown axis ' + axisKey };
    if(!Array.isArray(records)) return { ok:false, reason:'records 없음' };
    var feats = cfg.feats(); if(!feats) return { ok:false, reason:'FEATS 미로드' };
    var recs = records.filter(function(r){ return r[cfg.ab] && r[cfg.ab].length === feats.length; });
    if(recs.length < 12) return { ok:false, reason:'재료분해 표본부족(' + recs.length + '·미러 드리프트 가능)', n:recs.length };
    function icSet(rs){
      if(rs.length < 12) return null;
      var base = _spearman(rs, cfg.sc, 'retO');
      var items = feats.map(function(f, i){
        var ic = _spearman(rs, function(r){ return r[cfg.ab][i]; }, 'retO');
        return { key:f, ic:ic, d:(ic != null && base != null) ? Math.round((ic - base) * 1000) / 1000 : null };
      });
      return { n:rs.length, base:base, items:items };
    }
    return {
      ok:true, n:recs.length, axis:axisKey, label:cfg.label, feats:feats,
      all:  icSet(recs),
      bull: icSet(recs.filter(function(r){ return r.lt === 'bull'; })),
      bear: icSet(recs.filter(function(r){ return r.lt === 'bear'; }))
    };
  }

  G.SXVVAL = { run:run, pool:pool, ablateIC:ablateIC, _assembleScores:_assembleScores, _stat:_stat };
})();
