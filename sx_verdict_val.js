...
(function(){
  ...
  function run(rows, tf, market, opts){
    ...
    for(var e = warmup; e <= lastE; e += step){
      ...
      scoreRecs.push(_ax);
      if(records) records.push({ p:pass, path:path, lt:(sc.ltAlign||'off'), act:act, ret:retC, retO:retO, cap:_capped?1:0, viol:_hasViol?1:0, rd:_ax.rd, en:_ax.en, tr:_ax.tr, up:_ax.up }); // [S691] lt=ltAlign(게이트 실험)
    }
