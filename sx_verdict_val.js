...
(function(){
  ...
  //   records=run({raw:true,ablate:true}) 모음(rab/uab/tab 필요). axisKey=rd(반등신호)/up(추가상승)/tr(추세방향).
  //   Δ>0=빼면 IC↑(노이즈/해로움)·Δ<0=기여. baseline=그 축 전체점수 IC. 익일(retO) 기준. 경로 필터는 호출측에서 records.filter.
  var _ABL_CFG = {
    rd: { sc:'rd', ab:'rab', label:'반등신호', feats:function(){ return (G.SXUV && G.SXUV.READY_FEATS) || null; } },
    en: { sc:'en', ab:'eab', label:'반등강도', feats:function(){ return (G.SXUV && G.SXUV.ENTRY_FEATS) || null; } },
