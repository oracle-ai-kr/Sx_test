...
(function(){
  ...
  function run(rows, tf, market, opts){
    ...
    for(var e = warmup; e <= lastE; e += step){
      ...
      if(path === 'rebound' || path === 'trend') byPathPass[path][pass].push(ret);
      var act = v.action || '-';
      ...
      if(pass >= 3){
        if(_hasViol) capRets.push(ret); else entryUncapRets.push(ret);
      }
      if(records) records.push({ p:pass, path:path, ret:ret, cap:_capped?1:0, viol:_hasViol?1:0 });
    }
    ...
    return {
      ...
      byPathPass: {   // [S674] 분기별 passCount 분리
        rebound: byPathPass.rebound.map(function(r, p){ return { pass:p, stat:_stat(r) }; }),
        trend:   byPathPass.trend.map(function(r, p){ return { pass:p, stat:_stat(r) }; })
      },
      byVerdict: verdictArr,
      safetyCap: { capped:_stat(capRets), uncapped:_stat(entryUncapRets) },
      records: records   // [S674] opts.raw 시에만 채워짐(다종목 풀링용)
    };
  }
