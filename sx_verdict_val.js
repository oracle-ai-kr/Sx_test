...
(function(){
  ...
  function run(rows, tf, market, opts){
    ...
    for(var e = warmup; e <= lastE; e += step){
      ...
      scoreRecs.push(_ax);
      // [S721/722] 재료 ablation 연동 — opts.ablate 시 봉별 축 분해(SXUV 미러). 드리프트가드: 미러 score≠실제 점수면 제외.
      var _rab = null, _uab = null, _tab = null;
      ...
      if(records){ var _rec = { p:pass, path:path, lt:(sc.ltAlign||'off'), act:act, ret:retC, retO:retO, cap:_capped?1:0, viol:_hasViol?1:0, rd:_ax.rd, en:_ax.en, tr:_ax.tr, up:_ax.up }; if(_rab) _rec.rab = _rab; if(_uab) _rec.uab = _uab; if(_tab) _rec.tab = _tab; records.push(_rec); } // [S691] lt · [S721/722] rab/uab/tab=재료 ablation
    }
