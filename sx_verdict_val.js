...
(function(){
  ...
  function run(rows, tf, market, opts){
    ...
    for(var e = warmup; e <= lastE; e += step){
      ...
      scoreRecs.push(_ax);
      if(records) records.push({ p:pass, path:path, ret:retC, retO:retO, cap:_capped?1:0, viol:_hasViol?1:0, rd:_ax.rd, en:_ax.en, tr:_ax.tr, up:_ax.up });
    }
    ...
    var SCORE_AXES = [{k:'rd',label:'반등'},{k:'en',label:'진입'},{k:'tr',label:'추세'},{k:'up',label:'추가상승'}];
