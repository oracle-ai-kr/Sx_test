...
(function(){
  ...
  async function runBasket(market, tf, onProgress){
    ...
    for(var i=0; i<list.length; i++){
      ...
      var rows = null;
      try { rows = await btFetchCandles(s.code, isCoin, tf || 'day', 250); } catch(e){ rows = null; }
      if(!Array.isArray(rows) || rows.length < MIN_BARS + 5){ skipped++; continue; }
      var bt = null;
