// ════════════════════════════════════════════════════════════════
//  sx_candle_bt.js  —  [S525] 캔들 전이 미니 백테스트  (+[S530] 뒤로가기 닫기·임계값 선택)  (+[S601] 자기유사도 kNN 대안경로 비교)
...
(function(){
  ...
  function runBacktest(rows, market, tf, opts){
    ...
    for(var i = startI; i <= n - 2; i++){
      ...
      samples.push({ score: r.score, knn: _knn, up: up, regime: rg });
      if(up) baseUp++;
  ...
  function _renderKnn(a){
    ...
    if(kn.pred === 0){
      ...
      return '<div style="margin-top:12px;padding:9px 11px;background:var(--surface2);border-radius:9px;border-left:3px solid #7c3aed">'
        ...
        + '<div style="font-size:10px;color:var(--text3);margin-top:3px">' + why + '</div></div>';
    }
    ...
    return '<div style="margin-top:12px;padding:10px 12px;background:var(--surface2);border-radius:10px;border-left:3px solid #7c3aed">'
      ...
      + '<div style="font-size:9px;color:var(--text3);margin-top:5px;line-height:1.6">최근 8봉을 정규화 벡터로 만들어 같은 종목 과거 8봉 중 가장 닮은 ' + ((typeof SXKNN!=='undefined'&&SXKNN.K)||25) + '개의 다음봉 색으로 예측. 룩어헤드 없음 · 베이스라인 동일.</div>'
      + '</div>';
  }
