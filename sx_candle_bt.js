...
(function(){
  ...
  var THRESHOLDS = [30, 40, 50];// 선택 가능한 예측 임계 (|score| ≥ thr 일 때만 '예측')
  ...
  function _thrButtons(cur){
    return '<div style="display:flex;gap:6px;justify-content:center;margin:0 0 12px">'
      + THRESHOLDS.map(function(t){
          var on = (t === cur);
          return '<button onclick="window.SXCandleBT&&SXCandleBT.setThr(' + t + ')" style="flex:1;font-size:12px;font-weight:' + (on ? 800 : 600) + ';padding:7px 0;border-radius:8px;cursor:pointer;'
            + (on ? 'color:#fff;background:#7c3aed;border:1px solid #7c3aed' : 'color:var(--text2);background:var(--surface2);border:1px solid var(--border)') + '">|점수|≥' + t + '</button>';
        }).join('')
      + '</div>';
  }
  ...
  function open(){
    ...
    ov.innerHTML =
      ...
      + '</div>'
      + '<div style="font-size:10px;color:var(--text3);margin-bottom:12px">다음봉 색(양/음) 예측의 과거 적중률 · 임계값을 바꿔 정확도↔표본 균형을 보세요' + nm + '</div>'
  ...
  window.SXCandleBT = { open: open, close: _close, setThr: setThr, run: runBacktest, _remove: _removeOverlay };
})();
