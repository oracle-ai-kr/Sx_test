// ════════════════════════════════════════════════════════════════
//  sx_candle_bt.js  —  [S525] 캔들 전이 미니 백테스트  (+[S530] 뒤로가기 닫기·임계값 선택)  (+[S601] 자기유사도 kNN 대안경로 비교)  (+[S603] kNN 창길이/K 토글 A/B)  (+[S604] 9조합 자동탐색·JSON 다운로드)
...
(function(){
  ...
  function _renderKnnGrid(thr){
    ...
    var arr=_knnGrid.combos.map(function(c){
      ...
      var be=Math.max(a.bullEdge==null?-99:a.bullEdge, a.bearEdge==null?-99:a.bearEdge);
      return { key:c.win+'×'+c.K, win:c.win, K:c.K, hitRate:a.hitRate, pred:a.pred, edge:(be<=-99?null:be) };
    });
    arr.sort(function(x,y){ var hx=x.hitRate==null?-1:x.hitRate, hy=y.hitRate==null?-1:y.hitRate; if(hy!==hx) return hy-hx; return y.pred-x.pred; });
    var bestKey=null; for(var i=0;i<arr.length;i++){ if(arr[i].pred>=10){ bestKey=arr[i].key; break; } }
    var head='<div style="display:flex;font-size:9px;color:var(--text3);font-weight:700;padding:3px 4px;border-bottom:1px solid var(--border)">'
      + '<span style="flex:1.2">조합(창×K)</span><span style="flex:1;text-align:right">적중률</span><span style="flex:1;text-align:right">edge</span><span style="flex:0.8;text-align:right">건수</span></div>';
    var body=arr.map(function(r){
      var on=(r.key===bestKey);
      var hc=r.hitRate==null?'var(--text3)':r.hitRate>=55?'#16a34a':r.hitRate>=50?'#f59e0b':'#dc2626';
      var ec=r.edge==null?'var(--text3)':r.edge>0?'#16a34a':r.edge<0?'#dc2626':'var(--text3)';
      return '<div style="display:flex;align-items:center;font-size:11px;padding:4px;border-radius:5px;'+(on?'background:#7c3aed18;':'')+'">'
    ...
    return '<div style="margin-top:8px;padding:8px 9px;background:var(--surface);border:1px solid var(--border);border-radius:9px">'
      + '<div style="font-size:10px;font-weight:800;color:var(--text);margin-bottom:4px">🔍 9조합 자동탐색 <span style="font-size:9px;font-weight:600;color:var(--text3)">· |점수|≥'+thr+' 기준 · ⭐=신뢰(예측≥10) 최상위</span></div>'
      + head + body
      + '<div style="font-size:9px;color:var(--text3);margin-top:5px;line-height:1.6">임계 버튼을 바꾸면 이 표도 다시 집계돼요. 예측 건수 적은 행은 운빨일 수 있으니 ⭐(건수 충분) 위주로 보세요.</div></div>';
  }
