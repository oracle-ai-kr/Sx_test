...
(function(){
  ...
  function _poolBlock(){
    return '<div style="margin-top:8px">'
      + '<button onclick="window.SXCandleBT&&SXCandleBT.runPoolCompareUI()" style="width:100%;font-size:11px;font-weight:700;padding:8px 0;border-radius:8px;cursor:pointer;color:#0ea5e9;background:var(--surface);border:1px solid #0ea5e966">🌐 다종목 풀링 뱅크 비교 (이 종목)</button>'
      + '<button onclick="window.SXCandleBT&&SXCandleBT.runPoolVerifyUI()" style="width:100%;font-size:11px;font-weight:700;padding:8px 0;border-radius:8px;cursor:pointer;color:#16a34a;background:var(--surface);border:1px solid #16a34a66;margin-top:5px">🔬 풀링 로직 검증 (관심목록 전체)</button>'
  ...
  async function runPoolCompare(onProgress){
    ...
    if(typeof _getWatchlist!=='function' || typeof btFetchCandles!=='function') return { ok:false, reason:'관심목록/캔들 접근 불가' };
    var win = _knnWin, k = _knnK, isCoin = (mk==='coin');
    var list = (_getWatchlist(mk)||[]).filter(function(s){ return s && s.code; });
    var CAP = 10, others=[];
    for(var i=0; i<list.length && others.length<CAP; i++){
      var s=list[i];
      if(ctx.name && (s.name===ctx.name || s.code===ctx.code)) continue;   // 자기 중복 제외
      if(onProgress) onProgress(others.length+1, Math.min(CAP,list.length), s.name||s.code);
      await _sleep(0);
      var r=null; try { r=_toScreenerRows(await btFetchCandles(s.code, isCoin, tf, 250)); } catch(e){ r=null; }
      if(Array.isArray(r) && r.length >= win+SXKNN.MIN_BANK+5) others.push({ name:s.name||s.code, rows:r });
      await _sleep(20);
    }
    if(!others.length) return { ok:false, reason:'풀에 넣을 타종목이 없음 — 관심목록('+mk+')에 종목을 더 추가하세요' };
    if(onProgress) onProgress(0,0,'⏳ 풀링 뱅크 분석 중… (잠시)');
    await _sleep(15);
    // 풀뱅크 = 자기(현재 종목 풀시리즈) + 타종목(250봉). 자기 단독 prebuild로 baseline.
    var stocksRows = [{ name:ctx.name||'(현재)', rows:rows }].concat(others);
    ...
    return { ok:true, name:ctx.name||'(현재)', nOthers:others.length, others:others.map(function(o){return o.name;}),
  ...
  async function runPoolCompareUI(){
    ...
    el.innerHTML='<div style="text-align:center;padding:14px 2px;font-size:11px;color:var(--text3)">풀링 뱅크 준비 — 관심목록 캔들 수집 중…</div>';
    var res;
    try {
  ...
  async function runPoolVerify(onProgress){
    ...
    if(typeof _getWatchlist!=='function' || typeof btFetchCandles!=='function') return { ok:false, reason:'관심목록/캔들 접근 불가' };
    if(typeof SXKNN==='undefined' || !SXKNN.buildPoolBank) return { ok:false, reason:'kNN 풀링 미로드' };
    var win=_knnWin, k=_knnK, CAP=10;
    var list=(_getWatchlist(mk)||[]).filter(function(s){ return s && s.code; });
    var stocksRows=[];
    for(var i=0;i<list.length && stocksRows.length<CAP;i++){
      var s=list[i];
      if(onProgress) onProgress(stocksRows.length+1, Math.min(CAP,list.length), s.name||s.code);
      await _sleep(0);
      // [S617] 자기 기준을 라이브 카드와 동일하게 600봉 fetch (250봉은 자기 후보를 굶겨 결과를 왜곡)
      var r=null; try{ r=_toScreenerRows(await btFetchCandles(s.code,isCoin,tf,600)); }catch(e){}
      if(Array.isArray(r) && r.length>=win+SXKNN.MIN_BANK+5) stocksRows.push({name:s.name||s.code, rows:r});
      await _sleep(15);
    }
    if(stocksRows.length<3) return { ok:false, reason:'유효 종목 3개 미만 — 관심목록('+mk+')을 늘리세요' };
    var per=[];
  ...
  async function runPoolVerifyUI(){
    var el=document.getElementById('sxPoolResult'); if(!el) return;
    el.innerHTML='<div style="text-align:center;padding:14px 2px;font-size:11px;color:var(--text3)">풀링 검증 준비 — 관심목록 캔들 수집…</div>';
    var res;
    try {
  ...
  async function _ensureRepBank(mk, tf, win){
    ...
    for(var i=0;i<list.length;i++){
      ...
      await _sleep(0);
      var r=null; try { r=_toScreenerRows(await btFetchCandles(code, isCoin, tf, 250)); } catch(e){ r=null; }
      if(Array.isArray(r) && r.length >= win+SXKNN.MIN_BANK+5){ stocksRows.push({ name:nm, rows:r }); names.push(nm); }
      await _sleep(15);
    }
  ...
  async function evalPoolAuto(rows, mk, tf, code, name){
    try {
      ...
      var selfBank = SXKNN.buildPoolBank([{ name:name||'(self)', rows:rows }], win);
      var combined = { bank: selfBank.bank.concat(rep.bank), win:win, W:rep.W };   // 자기+대표 = self+others
      var n=rows.length, startI=Math.max(MIN_BARS, n-1-DEFAULT_MAX);
      var sP=0,sH=0,pP=0,pH=0;
      for(var q=startI;q<=n-2;q++){
        var up=_isUp(rows[q+1]);
      ...
      var applies = !!(delta!=null && delta>=3 && pP>=8 && sP>=8 && poolCur && poolCur.active);
      return { applies:applies, delta:delta, selfHit:selfHit, pooledHit:pooledHit, selfN:sP, pooledN:pP, poolCur:poolCur, poolSize:combined.bank.length, donors:rep.donors };
