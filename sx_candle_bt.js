...
(function(){
  ...
  async function runPoolCompare(onProgress){
    ...
    var selfBank = SXKNN.buildPoolBank([{ name:ctx.name||'(현재)', rows:rows }], win);
    var pool = { bank: selfBank.bank.concat(rep.bank), win:win, W:rep.W };
    ...
    return { ok:true, name:ctx.name||'(현재)', nOthers:rep.donors.length, others:rep.donors,
